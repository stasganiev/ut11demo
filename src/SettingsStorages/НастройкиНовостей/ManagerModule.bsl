...
//
// - "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных":
//    - Для этого ключа объекта отсутствует ключ настроек;
//    - Отбор по пользователю - нет.
//
// - "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования":
//    - Для этого ключа объекта отсутствует ключ настроек;
//    - Отбор по пользователю - нет.
//
...
//    - Отбор по пользователю - есть.
//
// - "ЛентыНовостейПодпискаВключена" - доступные пользователю ленты новостей, на которые он подписан самостоятельно или принудительно;
//    - Для этого ключа объекта отсутствует ключ настроек;
//    - Отбор по пользователю - есть.
//
// - "ЛентыНовостейПодпискаВключенаПользователем" - доступные пользователю ленты новостей, на которые он подписан самостоятельно (но НЕ принудительно);
//    - Для этого ключа объекта отсутствует ключ настроек;
//    - Отбор по пользователю - есть.
//
// - "ЛентыНовостейПодпискаОтключенаПользователем" - доступные пользователю ленты новостей, от которых он отписался;
//    - Для этого ключа объекта отсутствует ключ настроек;
//    - Отбор по пользователю - есть.
//
// - "ЛентыНовостейВсеНастройки" - все настройки лент новостей - состояние подписки (принудительное и самостоятельное), способ оповещения;
//    - Ключ настроек = Неопределено (по всем лентам новостей), или ссылка на ленту новостей.
//    - Отбор по пользователю - есть.
//
...
Процедура ОбработкаЗагрузки(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если ТипЗнч(ИмяПользователяИБ) = ТипПользователь Тогда
		лкИмяПользователяИБ = ОбработкаНовостейВызовСервера.ПолучитьИмяПользователяИБ(ИмяПользователяИБ);
	ИначеЕсли ПустаяСтрока(ИмяПользователяИБ) Тогда
	...
	Если (КлючОбъекта = "НастройкиПолученияНовостей") // Структура
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если ТипЗнч(ЗначениеКонстанты) = ТипХранилищеЗначения Тогда
				ХранилищеНастроекНовостей = ЗначениеКонстанты.Получить();
			Иначе
				ХранилищеНастроекНовостей = Неопределено;
			КонецЕсли;
			Если ТипЗнч(ХранилищеНастроекНовостей) = ТипСтруктура Тогда
				ХранилищеНастроекНовостейЭталон = Новый Структура;
					ХранилищеНастроекНовостейЭталон.Вставить("НастройкиПолученияНовостей"          , Неопределено);
					ХранилищеНастроекНовостейЭталон.Вставить("НастройкиАвтоудаленияНовостей"       , Неопределено);
					ХранилищеНастроекНовостейЭталон.Вставить("СистемнаяИнформация_ВерсияПриложения", Неопределено);
					ХранилищеНастроекНовостейЭталон.Вставить("СостояниеПодсистемы"                 , Неопределено);
					ХранилищеНастроекНовостейЭталон.Вставить("НастройкиЖурналаРегистрации"         , Неопределено);
				ЗаполнитьЗначенияСвойств(ХранилищеНастроекНовостейЭталон, ХранилищеНастроекНовостей);
				Настройки = ХранилищеНастроекНовостейЭталон[КлючОбъекта];
	...
	// Глобальные настройки.
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором") Тогда // Структура
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если ТипЗнч(ХранилищеНастроекНовостей) = ТипСтруктура Тогда
				ХранилищеНастроекНовостейЭталон = Новый Структура;
					ХранилищеНастроекНовостейЭталон.Вставить("НастройкиПоказаНовостейНастроенныеАдминистратором", Неопределено);
				ЗаполнитьЗначенияСвойств(ХранилищеНастроекНовостейЭталон, ХранилищеНастроекНовостей);
				Настройки = ХранилищеНастроекНовостейЭталон[КлючОбъекта];
	...
	// Настройки для области данных.
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных")
			ИЛИ (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования") Тогда // Структура

		// Эта настройка может храниться или в константе НастройкиНовостей (для модели сервиса в неразделенном режиме или в коробке),
		//  или в регистре сведений "НастройкиНовостейОбластиДанных".
		// Причем для области данных настройки может и не быть. Тогда брать настройку из константы.
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			НастройкаПолученаИзОбластиДанных = Ложь;
			...
			Если ОбщегоНазначения.РазделениеВключено()
				...
				Запрос.Текст = "
					...
					|ИЗ
					|	РегистрСведений.НастройкиНовостейОбластиДанных КАК Рег
					|ГДЕ
					|	Рег.Настройка = &Настройка_СписокНовостей_ИнтервалАвтообновления
					|";
				Запрос.УстановитьПараметр("Настройка_СписокНовостей_ИнтервалАвтообновления", "СписокНовостей_ИнтервалАвтообновления");
				РезультатЗапроса = Запрос.Выполнить();
				Если НЕ РезультатЗапроса.Пустой() Тогда
					Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
					Если Выборка.Следующий() Тогда
						Настройки.СписокНовостей_ИнтервалАвтообновления = Выборка.Значение;
						Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, "");
						// Для разделенного режима есть особенности:
						// - значение "0" означает удаление настройки и использование значения из константы;
						// - Допустимые значения: 0, 60, 180, 360. При этом "0" допустим только для "ДляРедактирования".
						Если Настройки.СписокНовостей_ИнтервалАвтообновления <> 0 Тогда
							НастройкаПолученаИзОбластиДанных = Истина;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;

			// Если настройки нет для области данных, то взять глобальную.
			Если НастройкаПолученаИзОбластиДанных = Ложь Тогда
				Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных") Тогда
					ОбработкаЗагрузки(
						"НастройкиПоказаНовостейНастроенныеАдминистратором",
						КлючНастроек,
						Настройки,
						ОписаниеНастроек,
						ИмяПользователяИБ);
				ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования") Тогда
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключена" Тогда
		Настройки = ОбработкаНовостейВызовСервера.ПолучитьЛентыНовостейПодпискаВключена(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);

	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключенаПользователем" Тогда
		Настройки = ОбработкаНовостейВызовСервера.ПолучитьЛентыНовостейПодпискаВключенаПользователем(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);

	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаОтключенаПользователем" Тогда
		Настройки = ОбработкаНовостейВызовСервера.ПолучитьЛентыНовостейПодпискаОтключенаПользователем(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);

	ИначеЕсли КлючОбъекта = "ЛентыНовостейВсеНастройки" Тогда
		Если (ТипЗнч(КлючНастроек) = ТипЛентаНовостей) И (НЕ КлючНастроек.Пустая()) Тогда
			ОтборПоЛентеНовостей = КлючНастроек;
		Иначе
			ОтборПоЛентеНовостей = Неопределено;
		КонецЕсли;
		Настройки = ОбработкаНовостейВызовСервера.ПолучитьНастройкиЛентНовостей(лкИмяПользователяИБ, ОтборПоЛентеНовостей);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);
...
Процедура ОбработкаПолученияОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором")
			ИЛИ (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных")
			ИЛИ (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования")
...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	// Глобальные настройки.
	Если (КлючОбъекта = "НастройкиПолученияНовостей")
			ИЛИ (КлючОбъекта = "НастройкиАвтоудаленияНовостей")
			ИЛИ (КлючОбъекта = "СистемнаяИнформация_ВерсияПриложения")
			ИЛИ (КлючОбъекта = "СостояниеПодсистемы")
			ИЛИ (КлючОбъекта = "НастройкиЖурналаРегистрации") Тогда
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось записать константу НастройкиНовостей.
						...
						|%5'"),
					КлючОбъекта,
					ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(Настройки, "=", Символы.ПС),
	...
	// Глобальные настройки.
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором") Тогда
	...
	// Настройки для области данных.
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных")
			ИЛИ (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования") Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			// - значение "0" означает удаление настройки и использование значения из константы;
			// - Допустимые значения: 0, 60, 180, 360. При этом "0" допустим только для "ДляРедактирования".
			Запись = РегистрыСведений.НастройкиНовостейОбластиДанных.СоздатьМенеджерЗаписи();
				Запись.Настройка = "СписокНовостей_ИнтервалАвтообновления";
			Если Настройки.СписокНовостей_ИнтервалАвтообновления = 0 Тогда
				Запись.Удалить();
			Иначе
				Запись.Значение  = Настройки.СписокНовостей_ИнтервалАвтообновления;
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключена" Тогда
		// Данные для этой настройки НЕ должны сохраняться таким способом.
		// Поэтому попытка записи - ошибочна.
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Хранилище настроек новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи настроек новостей. Настройки для ключа [%1] не должны записываться через ХранилищеНастроек.НастройкиНовостей.'"),
				КлючОбъекта), // Комментарий
			ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ОбработкаУстановкиОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключена" Тогда
		// Т.к. эта настройка хранится в разных местах базы данных, то для нее нет возможности установить описание.
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключенаПользователем" Тогда
		// Т.к. эта настройка хранится в разных местах базы данных, то для нее нет возможности установить описание.
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаОтключенаПользователем" Тогда
		// Т.к. эта настройка хранится в разных местах базы данных, то для нее нет возможности установить описание.
...
Функция ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек) Экспорт
	...
	ТипЛентаНовостей                     = Тип("СправочникСсылка.ЛентыНовостей");
	ТипПользовательСистемыВзаимодействия = Тип("ПользовательСистемыВзаимодействия");

	Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором") Тогда
		// Для этого ключа объекта не действует КлючНастроек.
		// Обязательные поля:
		// - СписокНовостей_ИнтервалАвтообновления - Число(6.0), неотрицательное, по-умолчанию 360 минут (6 часов), минимум 1 минута.
		//     Если настройка отсутствует в области данных в модели сервиса, то значение берется из этой, неразделенной настройки.
		//     В этом случае значение должно быть 60, 180 или 360. Если это не так, то изменить на 0.
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			Если ТипЗнч(Результат.СписокНовостей_ИнтервалАвтообновления) = ТипЧисло
					И (Результат.СписокНовостей_ИнтервалАвтообновления >= 1)
					И (Результат.СписокНовостей_ИнтервалАвтообновления <= 999999) Тогда
				// Корректное значение
			Иначе
				Результат.СписокНовостей_ИнтервалАвтообновления = 360;
			КонецЕсли;
	...
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных")
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных") Тогда
				...
				// Значение может наследоваться из "НастройкиПоказаНовостейНастроенныеАдминистратором".
				Если ТипЗнч(Результат.СписокНовостей_ИнтервалАвтообновления) = ТипЧисло
						И (Результат.СписокНовостей_ИнтервалАвтообновления >= 1)
						И (Результат.СписокНовостей_ИнтервалАвтообновления <= 999999) Тогда
					// Корректное значение
			...
			ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанныхДляРедактирования") Тогда
				...
				Если ТипЗнч(Результат.СписокНовостей_ИнтервалАвтообновления) = ТипЧисло
						И (
							(Результат.СписокНовостей_ИнтервалАвтообновления = 0)
							ИЛИ (Результат.СписокНовостей_ИнтервалАвтообновления = 60)
							ИЛИ (Результат.СписокНовостей_ИнтервалАвтообновления = 180)
							ИЛИ (Результат.СписокНовостей_ИнтервалАвтообновления = 360)) Тогда
					// Корректное значение
				Иначе
					Результат.СписокНовостей_ИнтервалАвтообновления = 0;
				КонецЕсли;
			...
			Иначе
				...
				Для Каждого ТекущийЭлемент Из Результат.СписокРаскрытыхПапок Цикл
					Если ТипЗнч(ТекущийЭлемент.Значение) = ТипСтруктура Тогда
						ТекущийЭлементЗначениеЭталон = Новый Структура;
							ТекущийЭлементЗначениеЭталон.Вставить("ВариантОтбора" , Неопределено);
							ТекущийЭлементЗначениеЭталон.Вставить("ЗначениеОтбора", Неопределено);
						ЗаполнитьЗначенияСвойств(ТекущийЭлементЗначениеЭталон, ТекущийЭлемент.Значение);
						ЭтоПравильныйЭлемент = Истина;
						Если НЕ ((ТипЗнч(ТекущийЭлементЗначениеЭталон.ВариантОтбора) = ТипЧисло)
								И ТекущийЭлементЗначениеЭталон.ВариантОтбора >= 0
								И ТекущийЭлементЗначениеЭталон.ВариантОтбора <= МаксимальныйВариантОтбора) Тогда
							ЭтоПравильныйЭлемент = Ложь;
						КонецЕсли;
						Если (ТипЗнч(ТекущийЭлементЗначениеЭталон.ЗначениеОтбора) <> ТипСтрока
								И ТипЗнч(ТекущийЭлементЗначениеЭталон.ЗначениеОтбора) <> ТипЛентаНовостей
								И ТекущийЭлементЗначениеЭталон.ЗначениеОтбора <> Неопределено) Тогда // Для папок "Все" нет значения отбора
							ЭтоПравильныйЭлемент = Ложь;
						КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеПанелиКонтекстныхНовостей" Тогда
		...
		НастройкиЭталон = Новый Структура;
			НастройкиЭталон.Вставить("ОбластьДанных"       , 0);
			НастройкиЭталон.Вставить("Метаданные"          , "");
			НастройкиЭталон.Вставить("Форма"               , "");
			НастройкиЭталон.Вставить("ДатаОтключения"      , '00010101');
			НастройкиЭталон.Вставить("ДатаПоследнейНовости", '00010101');
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			Если ТипЗнч(НастройкиЭталон.ДатаОтключения) <> ТипДата Тогда
				НастройкиЭталон.ДатаОтключения = '00010101';
			КонецЕсли;
			Если ТипЗнч(НастройкиЭталон.ДатаПоследнейНовости) <> ТипДата Тогда
				НастройкиЭталон.ДатаПоследнейНовости = '00010101';
			КонецЕсли;
			Если НЕ ПустаяСтрока(ИмяМетаданных) Тогда
				НастройкиЭталон.Метаданные = ИмяМетаданных;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ИмяФормы) Тогда
				НастройкиЭталон.Форма = ИмяФормы;
			КонецЕсли;
		КонецЕсли;
		Настройки = НастройкиЭталон;
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеЛентыНовостей" Тогда
		...
		Если ТипЗнч(Настройки) = ТипМассив Тогда
			Для Каждого ТекущаяЛентаНовостей Из Настройки Цикл
				Если ТипЗнч(ТекущаяЛентаНовостей) = ТипЛентаНовостей Тогда
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейВсеНастройки" Тогда
		// Могли передать отбор по ленте новостей. Тогда в итоговом массиве должны быть настройки только этой ленты новостей.
		Если (ТипЗнч(КлючНастроек) = ТипЛентаНовостей) И (НЕ КлючНастроек.Пустая()) Тогда
			ОтборПоЛентеНовостей = КлючНастроек;
		Иначе
			ОтборПоЛентеНовостей = Неопределено;
		...
		ПравильныйМассив = Новый Массив;
		Если ТипЗнч(Настройки) = ТипМассив Тогда
			Для Каждого ТекущиеНастройки Из Настройки Цикл
				Если ТипЗнч(ТекущиеНастройки) = ТипСтруктура Тогда
					Если (ОтборПоЛентеНовостей = Неопределено) ИЛИ (ОтборПоЛентеНовостей = ТекущиеНастройки.ЛентаНовостей) Тогда
						НастройкиЭталон = Новый Структура;
							НастройкиЭталон.Вставить("КодЛентыНовостей"      , "");
							НастройкиЭталон.Вставить("ЛентаНовостей"         , Справочники.ЛентыНовостей.ПустаяСсылка());
							НастройкиЭталон.Вставить("ОбязательныйКанал"     , Ложь);
							НастройкиЭталон.Вставить("СпособОповещения"      , Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
							НастройкиЭталон.Вставить("ПодпискаПринудительно" , "Неопределено");
							НастройкиЭталон.Вставить("ПодпискаСамостоятельно", "Подписан");
						ЗаполнитьЗначенияСвойств(НастройкиЭталон, ТекущиеНастройки);
						ПравильныйМассив.Добавить(ТекущиеНастройки);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		// Если явно передали ленту новостей и по каким-то причинам настройки не заполнены, то вставить одну строку по-умолчанию.
		Если (ОтборПоЛентеНовостей <> Неопределено) И (ПравильныйМассив.Количество() = 0) Тогда
			НастройкиЭталон = Новый Структура;
				НастройкиЭталон.Вставить("КодЛентыНовостей"      , ОтборПоЛентеНовостей.Код);
				НастройкиЭталон.Вставить("ЛентаНовостей"         , ОтборПоЛентеНовостей);
				НастройкиЭталон.Вставить("ОбязательныйКанал"     , ОтборПоЛентеНовостей.ОбязательныйКанал);
				НастройкиЭталон.Вставить("СпособОповещения"      , Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
				НастройкиЭталон.Вставить("ПодпискаПринудительно" , "Неопределено");
				НастройкиЭталон.Вставить("ПодпискаСамостоятельно", "Подписан");
			ПравильныйМассив.Добавить(НастройкиЭталон);
		КонецЕсли;
		Результат = ПравильныйМассив;
	...
	ИначеЕсли КлючОбъекта = "СистемнаяИнформация_ВерсияПриложения" Тогда
		...
		Если ТипЗнч(Результат) <> ТипСтрока Тогда
			Результат = "0.0.0.0"; // Это версия по-умолчанию.
		КонецЕсли;
