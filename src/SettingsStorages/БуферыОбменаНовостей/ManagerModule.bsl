	...
	ИначеЕсли ВРег(КлючОбъекта) = ВРег("Документы.Новости.Действия") Тогда
		...
		Если ТипЗнч(Настройки) = ТипСписокЗначений Тогда
			Для каждого ТекущееЗначение Из Настройки Цикл
				...
				ТекущееЗначениеЗначениеЭталон = Новый Структура;
					ТекущееЗначениеЗначениеЭталон.Вставить("ПараметрыДействий", Неопределено);
				ЗаполнитьЗначенияСвойств(ТекущееЗначениеЗначениеЭталон, ТекущееЗначение.Значение);

				НовоеТекущееЗначение = ОбработкаНовостейКлиентСервер.ПолучитьОписаниеПолейБуфераОбмена(КлючОбъекта);
				ЗаполнитьЗначенияСвойств(НовоеТекущееЗначение, ТекущееЗначение.Значение);
				СписокПараметровДействий = Новый СписокЗначений;
				Если ТипЗнч(ТекущееЗначениеЗначениеЭталон.ПараметрыДействий) = ТипСписокЗначений Тогда
					Для каждого ТекущийПараметрДействия Из ТекущееЗначениеЗначениеЭталон.ПараметрыДействий Цикл
