...
Процедура НачатьПроверкуКодовМаркировкиСредствамиККТ(ПараметрыНачалаПроверки) Экспорт
	...
	ЕстьДанныеДляПроверкиИСМП  = Ложь;
	ЕстьДанныеДляПроверкиЕГАИС = Ложь;
	...
	Если ЕстьДанныеДляПроверкиИСМП Тогда
	...
	Иначе
		...
		Если ЕстьДанныеДляПроверкиЕГАИС Тогда
			...
			Если ПараметрыПроверки.Результат.ЕстьОшибки Тогда
				Если ПараметрыПроверки.РежимБРМК Тогда
					Оповестить(
...
// Имя события оповещения прогресс получения идентификаторов запроса ГИСМТ.
// 
// Возвращаемое значение:
//  Строка - Имя события оповещения прогресс получения идентификаторов запроса ГИСМТ
...
// 	ДополнительныеПараметры - (См. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыЗавершенияОбработкиШтрихкода).
...
Процедура ДополнитьРезультатПроверкиСредствамиККТДаннымиОстаткаЧастичногоВыбытия(ПараметрыПроверки, ЭлементПроверки, РезультатПроверки)
	...
	Если Не ПараметрыПроверки.ЭтоСканирование
		...
		ПредставлениеЕдиницы = ИнтеграцияИСКлиентСервер.ПредставлениеЕдиницыЧастичногоВыбытия(ЭлементПроверки.ВидПродукции);
...
Процедура ПроверкаИдентификатораГИСМТПоКодуМаркировки(ПараметрыПроверки)
	...
	Для Каждого ЭлементПроверки Из ЭлементыПроверки Цикл
		...
		ЭлементДанныхЗапроса.Вставить("ИндексЭлемента",               ИндексЭлемента);
