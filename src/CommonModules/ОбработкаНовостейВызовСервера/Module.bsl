...
//
Процедура ДополнительноОбработатьТекстНовостиПередПоказом(Знач ОбъектНовости, ТекстХТМЛ) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьТекстНовостиПередПоказом(ОбъектНовости, ТекстХТМЛ);
...
Процедура ДополнительноОбработатьТекстСпискаНовостейПередПоказом(ТекстХТМЛ) Экспорт
	...
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьТекстСпискаНовостейПередПоказом(ТекстХТМЛ);
...
//
Процедура ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, Знач НовостьХДТО) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьХДТО);
...
//
Процедура ДополнительноОбработатьНовостьПослеПолученияПослеЗаписи(Знач НовостьСсылка) Экспорт

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьНовостьПослеПолученияПослеЗаписи(НовостьСсылка);
...
//
Функция ПолучитьЛентуНовостейПоКоду(Знач ЛентаНовостейКод) Экспорт
...
//
Функция ПолучитьОтключенныеЛентыНовостей(Знач ИмяПользователяИБ) Экспорт
...
//
Функция ПолучитьЛентыНовостейПодпискаВключена(Знач ИмяПользователяИБ) Экспорт
...
//
Функция ПолучитьЛентыНовостейПодпискаВключенаПользователем(Знач ИмяПользователяИБ) Экспорт
...
//
Функция ПолучитьЛентыНовостейПодпискаОтключенаПользователем(Знач ИмяПользователяИБ) Экспорт
...
//
Функция ПолучитьНастройкиЛентНовостей(Знач Пользователь, Знач ЛентаНовостей = Неопределено) Экспорт
...
//    * СпособОповещения       - ПеречислениеСсылка.СпособыОповещенияПользователяОНовостях.
//
Функция ПолучитьНастройкиОповещенияЛентыНовостей(Знач Пользователь, Знач ЛентаНовостей) Экспорт
...
//
Функция ПолучитьИмяПользователяИБ(Знач ПользовательСсылка = Неопределено) Экспорт
...
//
Процедура ДополнительноОбработатьНовостиПослеПолучения(Знач ТаблицаДатЗагрузокНовостейПередПолучением) Экспорт
...
//
Процедура ИзменитьПользовательскуюВидимостьЛентНовостей(Знач ЛентыНовостей, Знач Видимость = Ложь, Знач Пользователь = Неопределено) Экспорт
...
//
Процедура ИзменитьПользовательскиеСпособыОповещенияЛентНовостей(Знач ЛентыНовостей, Знач НастройкиОповещения, Знач Пользователь = Неопределено) Экспорт
...
// Преобразует значение из XML в дату.
//
// Параметры:
//  Значение - Строка - значение, которое надо преобразовать.
//
// Возвращаемое значение:
//   Дата - преобразованное значение.
//
Функция XMLЗначениеДата(Знач Значение) Экспорт

	ТипДата   = Тип("Дата");

	Возврат XMLЗначение(ТипДата, Значение);

КонецФункции

// Преобразует значение из XML в число.
//
// Параметры:
//  Значение - Строка - значение, которое надо преобразовать.
//
...
//
Функция XMLЗначениеЧисло(Знач Значение) Экспорт

	ТипЧисло  = Тип("Число");

	Возврат XMLЗначение(ТипЧисло, Значение);
...
//
Процедура ПодготовитьПараметрыНавигационнойСсылки(Знач Объект, Знач УИНДействия, Действие, СписокПараметров) Экспорт
...
//
Функция ПолучитьХТМЛТекстНовостей(Знач Новости, Знач ПараметрыОтображения = Неопределено) Экспорт
...
Функция ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором() Экспорт
	...
	Возврат ОбработкаНовостейСлужебный.ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором();
...
//
Функция ПолучитьНовостиПоУИН(Знач УИННовости, Знач ЛентаНовостейКод = Неопределено) Экспорт

	Возврат ОбработкаНовостейСлужебный.ПолучитьНовостиПоУИН(УИННовости, ЛентаНовостейКод);
...
//
Процедура СбросДатыПоследнейУспешнойЗагрузкиНовостей(Знач ЛентыНовостей, Знач ПомечатьНовостиНаУдаление = Ложь) Экспорт
...
//
Процедура СбросДатыПоследнейУспешнойЗагрузкиКлассификатора(Знач Списки) Экспорт
...
//
Функция ПолучитьИнформациюОНовости(Знач УИННовости, ПрочиеПараметры) Экспорт
...
Процедура ИзменитьПризнакПрочитатьПозже(Знач МассивНовостей, Знач ПрочитатьПозже = Истина) Экспорт
	...
	ОбработкаНовостей.ИзменитьПризнакПрочитатьПозже(МассивНовостей, ПрочитатьПозже);
...
//
Функция НайтиНовости(Знач СтруктураПараметровПоиска) Экспорт

	Возврат ОбработкаНовостейСлужебный.НайтиНовости(СтруктураПараметровПоиска);
...
//
Функция НайтиКонтекстнуюНовостьВМассиве(Знач АдресМассиваНовостей, Знач НомерСтрокиНовости) Экспорт

	Возврат ОбработкаНовостейСлужебный.НайтиКонтекстнуюНовостьВМассиве(АдресМассиваНовостей, НомерСтрокиНовости);
...
//  КонтекстВыполнения - Структура, Неопределено - структура контекста выполнения.
//
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(Знач МассивЛентНовостей, КонтекстВыполнения = Неопределено) Экспорт
...
//  КонтекстВыполнения - Структура, Неопределено - структура контекста выполнения.
//
Процедура ПересчитатьОтборыПоНовостям_Пользовательские(Знач Пользователь = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
...
//
Функция ПолучитьЗначениеПредопределеннойКатегории(Знач Категория) Экспорт
...
Функция ПолучитьАдресДляПолученияНовостей(
			Знач ЛентаНовостей,
			Знач ДатаЗагрузкиПоследнихНовостейНаКлиенте = Неопределено,
			Знач ДатаПоследнейПопыткиЗагрузкиНовостей = Неопределено) Экспорт
...
Функция ПолучитьКонтекстныеНовостиДляФормы(
			...
			Знач ЗаголовокФормыКонтекстныхНовостей = "",
			Знач ИдентификаторыСобытийПриОткрытии = "ПриОткрытии",
			Знач НастройкиПолученияНовостей = Неопределено) Экспорт
...
//
Процедура ПанельКонтекстныхНовостей_ИзменитьВидимость(Новости, Знач Видимость) Экспорт
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
			Знач ИмяСобытия,
			Знач ИдентификаторШага,
			Знач УровеньРегистрации = "Ошибка",
			Знач ОбъектМетаданных = "",
			Знач Данные = Неопределено,
			Знач Комментарий = "",
			Знач ВестиПодробныйЖурналРегистрации = Истина) Экспорт
...
//
Функция ВыгрузитьВсеСобытияЖурналаРегистрации(Знач ПараметрыОтбора, Знач ПараметрыФайлаВыгрузки) Экспорт
...
// Т.к. изменение некоторых свойств может привести к неявному серверному вызову,
//  то лучше в явном виде совершить один серверный вызов, где настроить все необходимые свойства.
// Поиск в синтакс-помощнике по фразе "Изменение свойства на клиенте может потребовать обращения к серверу".
//
// Параметры:
...
//  НастройкиСозданияПодменю - Структура - структура с ключами:
//    * РазмерПодменюПостоянныхКонтекстныхНовостей - Число;
//    * РазмерПодменюКонтекстныхНовостей - Число.
//
// Возвращаемое значение:
...
Функция ДобавитьПодменюПросмотраСпискаНовостей(
			...
			Знач ТаблицаНовостей,
			Знач НастройкиСозданияПодменю) Экспорт
	...
	Если (ТипЗнч(ЭлементКоманднаяПанель) = ТипГруппаФормы) Тогда
		...
		Если ЭлементПодменюНовости = Неопределено Тогда // Еще подменю не создавали
			ЭлементПодменюНовости = Форма.Элементы.Добавить("Новость_ПодменюНовости", ТипГруппаФормы, ЭлементКоманднаяПанель);
		...
		Если (ТипЗнч(ЭлементПодменюНовости) = ТипГруппаФормы) Тогда
			...
			Если (ТипЗнч(ТаблицаНовостей) = Тип("ДанныеФормыКоллекция")
				...
				Если (ТаблицаНовостей.Количество() > 0) Тогда
					...
					// Блок: Постоянные новости.
					// Добавить несколько первых новостей.
					//  Количество таких новостей можно переопределить тут:
					...
					ПодменюПостоянныхНовостей = Форма.Элементы.Добавить("ПодменюНовости_ПодменюСпискаПостоянныхНовостей", ТипГруппаФормы, ЭлементПодменюНовости);
					...
					С = НастройкиСозданияПодменю.РазмерПодменюПостоянныхКонтекстныхНовостей;
					Для каждого ТекущаяНовость Из ТаблицаНовостей Цикл
						Если ТекущаяНовость.ЭтоПостояннаяНовость = Истина Тогда
							...
							Если НайденнаяСтрока = Неопределено Тогда
								...
								ПунктМеню = Форма.Элементы.Добавить(Идентификатор, ТипКнопкаФормы, ПодменюПостоянныхНовостей);
					...
					// Блок: Обычные новости.
					// Добавить несколько (ОбработкаНовостейКлиентСерверПереопределяемый.ПереопределитьРазмерПодменюКонтекстныхНовостей())
					//  первых новостей.
					// В таблице могут быть повторения (новость для формы элемента и формы списка), поэтому новость добавлять только один раз.
					ПодменюОбычныхНовостей = Форма.Элементы.Добавить("ПодменюНовости_ПодменюСпискаНовостей", ТипГруппаФормы, ЭлементПодменюНовости);
					ПодменюОбычныхНовостей.Заголовок = НСтр("ru='Подменю списка новостей'");
					ПодменюОбычныхНовостей.Вид       = ВидГруппыФормы.ГруппаКнопок;
					СписокДобавленныхНовостей = Новый СписокЗначений;
					С = НастройкиСозданияПодменю.РазмерПодменюКонтекстныхНовостей;
					Для каждого ТекущаяНовость Из ТаблицаНовостей Цикл
						Если ТекущаяНовость.ЭтоПостояннаяНовость = Ложь Тогда
							...
							Если НайденнаяСтрока = Неопределено Тогда
								...
								ПунктМеню = Форма.Элементы.Добавить(Идентификатор, ТипКнопкаФормы, ПодменюОбычныхНовостей);
									ПунктМеню.ИмяКоманды = НоваяКоманда.Имя;
					...
					ПунктМеню = Форма.Элементы.Добавить("Новость_Список", ТипКнопкаФормы, ЭлементПодменюНовости);
