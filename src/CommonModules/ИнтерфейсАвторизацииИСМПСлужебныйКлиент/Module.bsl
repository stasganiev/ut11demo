...
Процедура Подписать(Сообщения, ПараметрыЗапроса, ОповещениеПриЗавершении) Экспорт
	...
	Сертификаты = ОбщегоНазначенияИСВызовСервера.СертификатыПользователяДляПодписиПоОрганизации(ПараметрыЗапроса.Организация,
		Истина,
		ПараметрыЗапроса.РазрешенаПодписьДоверенногоЛицаСМЧД);
	...
	ОбщегоНазначенияИСКлиент.Подписать(ОписаниеДанных, ЭтотОбъект, ПодписатьЗавершение);
