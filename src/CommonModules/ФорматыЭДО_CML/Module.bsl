...
Процедура ПроверитьТаблицуТоваровCML(ДеревоДанных, Ошибки)
	...
	КлассификаторЕдиницИзмерения = ФорматыЭДО.КлассификаторЕдиницИзмерения();
	...
	Для Каждого Товар Из Товары.Строки Цикл
		...
		ПроверитьДанныеЕдиницыИзмеренияПоКлассификатору(Товар, Ошибки, Путь + "БазоваяЕдиницаКод",
			КлассификаторЕдиницИзмерения, НомерСтроки);
...
Процедура ПроверитьДанныеЕдиницыИзмеренияПоКлассификатору(ДеревоДанных, Ошибки, ПолноеИмяРеквизита, Классификатор,
	...
	Если ЗначениеЗаполнено(Значение) Тогда
		ЕдиницаИзмеренияКодНорм = СокрЛП(Значение);
		
		Если Не ФорматыЭДО.ЭтоЕдиницаИзмеренияКлассификатора(ЕдиницаИзмеренияКодНорм, Классификатор) Тогда
			ШаблонОшибки = НСтр("ru = 'значение %1 не найдено в классификаторе ОКЕИ'");
			ТекстОшибки = СтрШаблон(ШаблонОшибки, ЕдиницаИзмеренияКодНорм);
...
Процедура СформироватьДанныеПоТоваруCML_206(Товар, СтрокаДереваДанных, ПространствоИменСхемы, Ошибки, МассивФайлов = Неопределено)
	...
	Если Товар.Свойства().Получить("Налоги") <> Неопределено И Товар.Свойства().Получить("Налоги") <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтавкаНДС) Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Налог, "Ставка", СтавкаНДС, , Ошибки);
		КонецЕсли;
...
Процедура ПрочитатьСтрокуТоваровCML(ТоварXDTO, НовыйЭД, СписокТЧ, ПоляПоискаТовара, ДанныеРазбора, Ошибка)
	...
	СвойстваБазовойЕдиницы = Новый Структура;
	...
	Для Каждого ТекСвойство Из ТоварXDTO.Свойства() Цикл
		...
		ИначеЕсли ВРег(ТекСвойство.Имя) = ВРег("БазоваяЕдиница") Тогда
			...
			СвойстваБазовойЕдиницы.Вставить("Код", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗначениеСвойства, "Код"));
			
			Если ЗначениеСвойства.Свойства().Получить("Наименование") <> Неопределено Тогда
				СвойстваБазовойЕдиницы.Вставить("Наименование", ЗначениеСвойства.Наименование);
			КонецЕсли;
			
			Если ЗначениеСвойства.Свойства().Получить("НаименованиеКраткое") <> Неопределено Тогда
				СвойстваБазовойЕдиницы.Вставить("Наименование", ЗначениеСвойства.НаименованиеКраткое);
			КонецЕсли;
			
			Если ЗначениеСвойства.Свойства().Получить("НаименованиеПолное") <> Неопределено Тогда
				СвойстваБазовойЕдиницы.Вставить("НаименованиеПолное", ЗначениеСвойства.НаименованиеПолное);
			КонецЕсли;
			
			Если ЗначениеСвойства.Свойства().Получить("МеждународноеСокращение") <> Неопределено Тогда
				СвойстваБазовойЕдиницы.Вставить("МеждународноеСокращение", ЗначениеСвойства.МеждународноеСокращение);
			КонецЕсли;
			
			ОписаниеОбъекта = Неопределено;
			Если НЕ (СвойстваБазовойЕдиницы.Свойство("Наименование", ОписаниеОбъекта)
					ИЛИ СвойстваБазовойЕдиницы.Свойство("НаименованиеПолное", ОписаниеОбъекта)) Тогда
				ОписаниеОбъекта = "";
			КонецЕсли;
			
			ЕдиницаИзмерения = ЭлектронноеВзаимодействие.НайтиСсылку(
				"ЕдиницыИзмерения", СвойстваБазовойЕдиницы.Код, СвойстваБазовойЕдиницы);
			
			НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "ЕдиницыИзмерения");
			
			НайденнаяСтрока = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьСтрокуВДеревеРазбора(НайденныйТипВДереве, 
				СвойстваБазовойЕдиницы.Код, ОписаниеОбъекта, ЕдиницаИзмерения, СвойстваБазовойЕдиницы, ДанныеРазбора, Ошибка);
			
			ПоляПоискаТовара.Вставить("ЕдиницаИзмерения", НайденнаяСтрока.ИндексСтроки);
			
			СписокТЧ.Добавить(СвойстваБазовойЕдиницы, "СвойстваЕдиницы");
	...
	Если Не ЗначениеЗаполнено(ИдТовара) Тогда
		...
		ЕдиницаИзмерения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СвойстваБазовойЕдиницы, "Наименование", "");
		ЕдиницаИзмеренияКод = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СвойстваБазовойЕдиницы, "Код", "");
		
		СопоставлениеПоУмолчанию = НовыеДанныеСопоставления();
		СопоставлениеПоУмолчанию.Наименование = НаимТовара;
		СопоставлениеПоУмолчанию.ЕдиницаИзмерения = ЕдиницаИзмерения;
