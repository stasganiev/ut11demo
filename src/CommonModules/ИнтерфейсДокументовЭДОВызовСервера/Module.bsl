...
Функция ПараметрыОтправкиПолученияДокументовЭДО() Экспорт
	Возврат ИнтерфейсДокументовЭДО.ПараметрыОтправкиПолученияДокументовЭДО();
КонецФункции
...
Функция ДанныеФайла(Знач ФайлСсылка, Знач ПараметрыДанныхФайла = Неопределено, ЭтоМассовоеШтампирование = Ложь) Экспорт
	...
	Если Обработки.ФорматДоговорныйДокумент101.ЭтоФайлФормата(ДанныеФайла.ПолноеНаименованиеВерсии)
...
//
Процедура ОбработкаПолученияФормыЭлектронногоДокумента(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	
	Если ВидФормы <> "ФормаОбъекта"
		ИЛИ Не ИнтерфейсДокументовЭДО.ИспользоватьИнтеграциюОблачногоЭДО() Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоОблачныйЭДО = Ложь;
	Если Параметры.Свойство("ЭтоОблачныйЭДО") Тогда
		ЭтоОблачныйЭДО = Параметры.ЭтоОблачныйЭДО;
	ИначеЕсли Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ЭтоОблачныйЭДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Ключ, "ЭтоОблачныйЭДО");
	КонецЕсли;
	
	ЭтоВходящийЭДО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоВходящийЭДО", Ложь);
	
	Если ЭтоОблачныйЭДО
		И ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияОблачногоЭДО") Тогда
		СтандартнаяОбработка = Ложь;
		ОбщийМодульИнтеграцияОблачногоЭДО = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияОблачногоЭДО");
		ВыбраннаяФорма = ОбщийМодульИнтеграцияОблачногоЭДО.ИмяФормыПросмотраЭлектронногоДокумента(ЭтоВходящийЭДО);
	КонецЕсли;
