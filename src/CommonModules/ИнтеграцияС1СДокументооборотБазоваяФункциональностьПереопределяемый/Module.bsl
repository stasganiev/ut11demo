...
Процедура ПередДобавлениемКомандПечати(ИмяФормы, КомандыПечати, СтандартнаяОбработка) Экспорт
	...
	Если ИмяФормы = Метаданные.Справочники.ДоговорыКонтрагентов.Формы.ФормаЭлемента.ПолноеИмя()
			...
			//++ Локализация
			Или ИмяФормы = Метаданные.Документы.КоммерческоеПредложениеПоставщика.Формы.ФормаДокумента.ПолноеИмя()
			Или ИмяФормы = Метаданные.Документы.КоммерческоеПредложениеПоставщика.Формы.ФормаСписка.ПолноеИмя()
			//-- Локализация
			...
			Или ИмяФормы = Метаданные.Документы.УстановкаЦенНоменклатуры.Формы.ФормаСписка.ПолноеИмя() Тогда
...
Процедура ПриОпределенииЗначенияРеквизитаПоОбъектуXDTO(Результат, ЗначениеЗаполнения, ТипРеквизита, ИмяРеквизита,
	...
	Если ТипЗнч(ЗначениеЗаполнения) = Тип("ОбъектXDTO")
		...
		ИначеЕсли ЗначениеЗаполнения.objectID.ID = "Утвержден" Тогда //@NON-NLS-1
			СостояниеУтвержденияДО = "Confirmed";
		ИначеЕсли ЗначениеЗаполнения.objectID.ID = "НеУтвержден" Тогда //@NON-NLS-1
			СостояниеУтвержденияДО = "NotConfirmed";
		ИначеЕсли ЗначениеЗаполнения.objectID.ID = "НаУтверждении" Тогда //@NON-NLS-1
			СостояниеУтвержденияДО = "AwaitingConfirmation";
			
		Иначе
			// Состояние в ДО не известно. Оставим прежнее значение реквизита.
	...
	ИначеЕсли ТипЗнч(ЗначениеЗаполнения) = Тип("Структура")
			И ЗначениеЗаполнения.ТипXDTOОбъекта = "DMDocument"
			И СтрНайти(ЗначениеЗаполнения.ИмяСвойства, "status") = 1 Тогда
		...
		ИначеЕсли ЗначениеЗаполнения.ЗначениеСвойства = "Confirmed"
				Или ЗначениеЗаполнения.ЗначениеСвойства = "NotConfirmed"
				Или ЗначениеЗаполнения.ЗначениеСвойства = "AwaitingConfirmation" Тогда
			СостояниеУтвержденияДО = ЗначениеЗаполнения.ЗначениеСвойства;
	...
	ИначеЕсли ЗначениеЗаполнено(СостояниеУтвержденияДО) Тогда
		Результат = ИнтеграцияС1СДокументооборотУП.СостояниеУтвержденияУП(
			ТипРеквизита,
			ИмяРеквизита,
			ТипОбъекта,
...
Процедура ПриОпределенииНеобходимостиВыводитьКомандуПрисоединенныхФайловДО(Форма, Результат,
	...
	Если Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента"
		...
		Если Результат И Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
			// В форме договоров нужно скрыть страницу файлов БСП.
...
Процедура ПриСозданииПоСсылке(ОбъектИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено) Экспорт
	...
	Если ОбъектXDTO.objectID.type = "DMSubdivision"
		...
		ОбъектИС.ТекущийРуководитель = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеРеквизитаПоОбъектуXDTO(
			ОбъектXDTO.head,
