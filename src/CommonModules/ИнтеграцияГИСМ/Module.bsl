...
Функция ПодсистемаНеИспользуется() Экспорт
	...
	Модуль = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИСМПВызовСервера");
	ДатаНачалаЭксплуатацииИСМП = Модуль.ДатаОбязательнойМаркировкиПродукции(Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха);
	Если Не ЗначениеЗаполнено(ДатаНачалаЭксплуатацииИСМП) Тогда
		ДатаНачалаЭксплуатацииИСМП = РегистрыСведений["НастройкиУчетаМаркируемойПродукцииИСМП"].ДатаНачалаУчетаМаркируемойПродукции(
			Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха);
	КонецЕсли;
	
	ДатаСеанса = ТекущаяДатаСеанса();
	Возврат ДатаСеанса >= ДатаНачалаЭксплуатацииИСМП;
	
КонецФункции

Процедура СообщитьОбОтключенииПодсистемы(Отказ) Экспорт
	
	ТекстСообщения = НСтр("ru = 'В соответствии с Постановлением Правительства Российской Федерации от 06.04.2022 г. № 600
	                            |отключен устаревший информационный ресурс для маркировки изделий из натурального меха,
	                            |с 1 сентября 2022 года работа с маркировкой изделий из натурального меха ведется через документы
	                            |рабочего места Обмен с ИС МП (обувь, одежда, табак,...).'");
...
Функция СертификатыДляПодписанияНаСервере() Экспорт
	...
	СертификатыОрганизацийДляОбменаНаСервере.Колонки.Добавить("GLN");
	...
	Для Каждого ДанныеСертификата Из ДанныеСертификатовИзНастроекОбмена Цикл
		...
		СтрокаТЧ.GLN                    = ДанныеСертификата.GLN;
...
Функция ПредставлениеСтатусаГИСМ(СтатусГИСМ, ВходящиеДальнейшиеДействия, ДопустимыеДействия, ПрефиксКоманды = "") Экспорт
	...
	Если ТипЗнч(СтатусГИСМ) <> Тип("ФорматированнаяСтрока") Тогда
		...
		СтатусыЦветТекстаТребуетВниманияГосИС = СтатусыЦветТекстаТребуетВниманияГосИС();
...
Процедура ОтразитьНеЗаказаннуюНоменклатуруВСтроке(Выборка, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			ТекстОписанияПроблемы = НСтр("ru = '""%1"" отсутствует в списке заказанных КиЗ.'");
...
Функция КонтрагентПоДаннымXML(ДанныеДокумента, ДанныеОтправителя) Экспорт
	...
	Контрагент = ИнтеграцияИС.КонтрагентПоИННКПП(Данные.ИНН, Данные.КПП);
...
Функция ЕстьПравоСозданияКонтрагента() Экспорт
	...
	ОписаниеТипов = Метаданные.ОпределяемыеТипы.КонтрагентГосИС.Тип;
...
Функция ДокументОснованиеВозвратПоставщику(ДокументСсылка) Экспорт
	...
	ДокументОснованиеВозвратПоставщику = Ложь;
	ИнтеграцияГИСМПереопределяемый.ДокументОснованиеВозвратПоставщику(
		ДокументСсылка,
		ДокументОснованиеВозвратПоставщику);
		
	Возврат ДокументОснованиеВозвратПоставщику;
...
// Обработчик одноименного события в формах списков документов ГИСМ
//
// Параметры:
...
Процедура ПриСозданииНаСервереФормыСпискаДокументов(Форма, Настройки) Экспорт
	...
	МетаРеквизитОснование      = Метаданные.Документы[ИмяДокумента].Реквизиты.Основание;
	...
	Для Каждого ТипОснования Из ТипыДокументаОснования Цикл
		...
		СтрокиСоединенияДокументов.Добавить("	ЛЕВОЕ СОЕДИНЕНИЕ Документ." + МетаданныеОснования.Имя + " КАК ДоступКДокументу" + МетаданныеОснования.Имя + "
			|		ПО Таблица.Основание = ДоступКДокументу" + МетаданныеОснования.Имя + ".Ссылка");
	...
	Для Каждого ИмяСписка Из Новый Структура(ИменаВсехСписков) Цикл
		...
		Список.ТекстЗапроса = СтрЗаменить(
			...
			?(СпискиКОформлению.Свойство(ИмяСписка.Ключ),
				СтрЗаменить(ТекстСоединения, "Таблица.Основание", "СтатусыГИСМПереопределяемый.Документ"),
				ТекстСоединения));
	...
	Иначе
		...
		Попытка
			...
			Если ДанныеЗагруженногоДокумента <> Неопределено Тогда
				УдалитьИзОчередиПолученияДанных(ДанныеДляПолученияДокумента);
			КонецЕсли;
		...
		Исключение
			...
			ОписаниеПоддерживаемогоТипа = ПоддерживаемыеТипыДокументов().Получить(ДанныеДляПолученияДокумента.action_id);
...
Функция ЗагрузитьУведомлениеОбИзмененииСтатусаПриВыпускеКиЗ(ДанныеДокумента, Сообщения)
	...
	ОписаниеТипаДокумента = ПоддерживаемыеТипыДокументов().Получить(ДанныеДокумента.action_id);
	...
	ИнтеграцияГИСМВызовСервера.ДобавитьЗаписьВПротоколОбмена(
		Сообщения.ТекстВходящегоСообщенияXML,
		Реквизиты);
...
Функция ЗагрузитьУведомлениеОВыпускеКиЗ(ДанныеДокумента, Сообщения)
	...
	Для Каждого СтрокаТЧ Из ДанныеДокумента.emits.by_sign_type Цикл
		...
		ДляМаркировкиОстатков = (СпособВводаТовараВОбращениеПоКоду(СтрокаТЧ.sign_initial_type) = СпособВводаТовараВОбращение(Истина));
		СпособВыпускаВОборот  = СпособВыпускаВОборотПоКоду(СтрокаТЧ.origin_id);
		РазмерКиЗ             = РазмерКиЗПоКоду(СтрокаТЧ.sign_size_type);
		ВидКиЗ                = ВидКиЗПоКоду(СтрокаТЧ.sign_type);
...
Функция ЗагрузитьУведомлениеОПоступленииМаркированныхТоваров(ДанныеДокумента, ДанныеОтправителя, НомерДокумента, Сообщения)
	...
	ДокументОбъект.ВидОборотаТовара = ВидОборотаТовараПоКоду(ДанныеДокумента.turnover_type);
	...
	ОрганизацияПодразделение = ОрганизацияПодразделениеПоGLN(ДанныеДокумента.receiver_gln);
	...
	Контрагент = КонтрагентПоДаннымXML(ДанныеДокумента, ДанныеОтправителя);
...
Функция ОтправитьЗапросДляПередачиДанных(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	АдресСервера = "wcf01.furs.mark.crpt.ru";
	Порт         = 80;
	
	ВозвращаемоеЗначение = ОтправитьДляОбработки(
		ТекстПодписанногоИсходящегоСообщенияXML,
		АдресСервера, Порт,
		"IncomingService.svc", "http://tempuri.org/IIncomingService/CommunicateMessage");
...
Функция ОтправитьЗапросДляПолученияДанных(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	АдресСервера = "wcf02.furs.mark.crpt.ru";
	Порт         = 80;
	
	ВозвращаемоеЗначение = ОтправитьДляОбработки(
		ТекстПодписанногоИсходящегоСообщенияXML,
		АдресСервера, Порт,
		"OutgoingService.svc", "http://tempuri.org/IOutgoingService/CommunicateMessage");
...
Функция ОтправитьЗапросДляПолученияРеестров(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	АдресСервера = "wcf03.furs.mark.crpt.ru";
	Порт         = 80;
	
	ВозвращаемоеЗначение = ОтправитьДляОбработки(
		ТекстПодписанногоИсходящегоСообщенияXML,
		АдресСервера, Порт,
		"RegistryService.svc", "http://tempuri.org/IRegistryService/CommunicateMessage");
...
//
// Параметры:
//  ТекстПодписанногоИсходящегоСообщенияXML - Строка - текст отправляемого запроса в формате XML.
...
Функция ОтправитьЗапрос(ТекстПодписанногоИсходящегоСообщенияXML, ВариантЗапроса) Экспорт
	...
	Если ВариантЗапроса = ПредопределенноеЗначение("Перечисление.ВариантыЗапросовГИСМ.ПередачаДанных") Тогда
		РезультатОтправки = ИнтеграцияГИСМ.ОтправитьЗапросДляПередачиДанных(ТекстПодписанногоИсходящегоСообщенияXML);
	ИначеЕсли ВариантЗапроса = ПредопределенноеЗначение("Перечисление.ВариантыЗапросовГИСМ.ПолучениеДанных") Тогда
		РезультатОтправки = ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияДанных(ТекстПодписанногоИсходящегоСообщенияXML);
	ИначеЕсли ВариантЗапроса = ПредопределенноеЗначение("Перечисление.ВариантыЗапросовГИСМ.ПолучениеРеестров") Тогда
		РезультатОтправки = ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияРеестров(ТекстПодписанногоИсходящегоСообщенияXML);
	КонецЕсли;
...
//   ТекстОшибки
//
Функция ОтправитьДляОбработки(ТелоЗапроса, АдресСервера, Порт, АдресРесурса, SOAPAction) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ГИСМ'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка отправки POST-запроса к серверу ГИСМ'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
//
Функция СтатусыЦветТекстаТребуетВниманияГосИС()
	
	СтатусыЦветТекстаТребуетВниманияГосИС = Новый Массив;
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыУведомленийОПоступленииМаркированныхТоваровГИСМ.Аннулировано);
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.ОтклоненоГИСМ);
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.ОтклоненоКлиентом);
	...
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыИнформированияГИСМ.ОтклоненоГИСМ);
	
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыИнформированияГИСМ.ОтклоненоГИСММаркировкаТоваров);
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыИнформированияГИСМ.ОтклоненоГИСМПеремаркировкаТоваров);
	СтатусыЦветТекстаТребуетВниманияГосИС.Добавить(Перечисления.СтатусыИнформированияГИСМ.ОтклоненоГИСМСписаниеКиЗ);
	
	Возврат СтатусыЦветТекстаТребуетВниманияГосИС;
...
// 
// Возвращаемое значение:
//  Строка - EPC в шестнадцатеричном виде.
//
