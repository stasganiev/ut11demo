...
Функция СообщениеНеНайденыНастройкиПодключения(СообщенияXML, СообщениеXML, ХозяйствующийСубъект, НастройкиОбменаВЕТИС = Неопределено) Экспорт
	...
	Если НастройкиОбменаВЕТИС <> Неопределено
		...
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(
...
Функция ИнтервалМеждуЗапросамиСписковВыдержан(ПараметрыОбмена, Операция) Экспорт
	...
	Если ЭтоЗапросСписка(Операция) Тогда
		...
		Если ПараметрыОбмена.ОжидатьНаСервере Тогда
			ОбщегоНазначенияИС.Ожидать(ВремяОжиданияСледующегоЗапроса, ПараметрыОбмена);
		КонецЕсли;
...
Функция СгенерироватьСерии(Объект, Товары, ВыделенныеСтроки, ПараметрыУказанияСерий, ТоварыУточнение = Неопределено) Экспорт
	...
	РезультатЗаполненияСерий = ИнтеграцияИС.РезультатГенерацииИЗаполненияСерий();
...
Функция ЗагрузитьХозяйствующийСубъект(ДанныеХозяйствующегоСубъекта, СправочникОбъект = Неопределено, ПредприятиеGLN = Неопределено, ПараметрыОбмена = Неопределено, ТребуетсяПоиск = Истина) Экспорт
	...
	Иначе
		...
		Если СправочникОбъект.СоответствуетОрганизации Тогда
			СоответствующийОбъектИБ = ОбщегоНазначенияИС.ОрганизацияПоИННКПП(СправочникОбъект.ИНН, СправочникОбъект.КПП);
		Иначе
...
Функция ЗагрузитьПродукцию(ЭлементДанных, СправочникОбъект = Неопределено, ПараметрыОбмена = Неопределено, ТребуетсяПоиск = Истина) Экспорт
	...
	Если СтрДлина(ЭлементДанных.name) > 150 Тогда
		СправочникОбъект.ПолноеНаименование = ЭлементДанных.name;
	КонецЕсли;
...
Функция ЗагрузитьНаименованиеПродукции(ДанныеПродукции, СправочникОбъект = Неопределено, ПараметрыОбмена = Неопределено, БезЗаписи = Ложь, ТребуетсяПоиск = Истина) Экспорт
	...
	СправочникОбъект.Наименование        = Лев(ДанныеПродукции.Наименование, 150);
	СправочникОбъект.ПолноеНаименование  = ДанныеПродукции.Наименование;
...
Процедура ЗаполнитьСведенияОПартии(ВетеринарноСопроводительныйДокументОбъект, batch, ПараметрыОбмена)
	...
	Иначе
		...
		Если batch.productItem.packaging <> Неопределено Тогда
			...
			Если batch.productItem.packaging.unit <> Неопределено Тогда
...
Функция НайтиСоздатьПродукциюСХешем(ДанныеПродукции, КодТипаПродукции, ПродукцияИдентификатор, ВидПродукцииИдентификатор, ФасовкаУпаковкаИдентификатор, ФасовкаЕдиницаИзмеренияИдентификатор) Экспорт
	...
	ХешированиеДанныхОбъект.Добавить(СтрШаблон("4:%1",  ЗначениеДляРасчетаХэша(ЗначениеДляРасчетаХэша(Лев(ДанныеПродукции.Наименование, 255)))));
...
Функция ТекстЗапросаДвижениеСерийТоваров(МетаданныеДокумента) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ ИСТИНА КАК Поле1";
			...
			Если ОбработаныВсеДанные
				...
				Если ЗначениеЗаполнено(ДанныеДокумента.Документ) Тогда
					...
					Если ДокументОбъект <> Неопределено Тогда
						...
						ДокументОбъект.Товары.Сортировать("Продукция,ЗаписьСкладскогоЖурнала");
...
Функция ПолучитьСопоставленныеТовары(ТабличнаяЧасть, НоменклатураДляВыбора = Неопределено, Продукция = Неопределено, ЗаписьЖурнала = Неопределено, ИспользоватьОтборПоЗаписямЖурнала = Ложь) Экспорт
	...
	Если ИспользоватьОтборПоЗаписямЖурнала Тогда
		Запрос.Текст = ТекстЗапросаСопоставленныеТоварыСЗаписямиЖурнала();
	Иначе 
		Запрос.Текст = ТекстЗапросаСопоставленныеТоварыБезЗаписиЖурнала();
	КонецЕсли;
...
Процедура ПроверитьОчиститьЕдиницуИзмеренияВЕТИС(СтрокаТабличнойЧасти, РеквизитыПродукции = Неопределено) Экспорт
	...
	Иначе
		ДоступныеЕдиницыИзмерения = ИнтеграцияВЕТИСПовтИсп.ДоступныеЕдиницыИзменения(Продукция, РеквизитыПродукции);
		Если ДоступныеЕдиницыИзмерения.Найти(СтрокаТабличнойЧасти.ЕдиницаИзмеренияВЕТИС) = Неопределено Тогда
			Если РеквизитыПродукции = Неопределено Тогда
				ФасовкаЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Продукция, "ФасовкаЕдиницаИзмерения");
			Иначе
				ФасовкаЕдиницаИзмерения = РеквизитыПродукции.ФасовкаЕдиницаИзмерения;
			КонецЕсли;
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Если Не РазделениеВключено
		И Не ПолучитьФункциональнуюОпцию("ВестиУчетПодконтрольныхТоваровВЕТИС") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		Для Каждого ЭлементАрхива Из ЧтениеZIP.Элементы Цикл
			ИмяВременногоФайла = ИнтеграцияИС.ОбъединитьПути(ВременныйКаталог, ЭлементАрхива.ПолноеИмя);
	...
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка подготовки исходных файлов для загрузки классификатора ВетИС %1:
			           |%2'",
		...
		ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			ТекстОшибки,
			НСтр("ru = 'Работа с классификаторами ВетИС'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура УстановитьПараметрыСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ИмяПараметра = Метаданные.ПараметрыСеанса.ПараметрыЛогированияЗапросовВЕТИС.Имя Тогда
...
Процедура ПриОпределенииНастроекРаботыСФайлами(Настройки) Экспорт
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ДокументыВЕТИС.Тип.Типы() Цикл
		...
		Если ТипЗнч(ЭлементОчереди.Сообщение) = Тип("СправочникСсылка.ВЕТИСПрисоединенныеФайлы") Тогда
			...
			Если РеквизитыСообщения.Следующий() Тогда
				...
				Иначе
					...
					ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(РеквизитыСообщения.Ссылка);
					...
					ДанныеОбработки = ОбработатьОтветНаПередачуДанных(РезультатОтправкиЗапроса, ПараметрыЗапроса, Неопределено, ПараметрыОбмена);
		...
		Иначе
			...
			Если ПараметрыПреобразования <> Неопределено
				...
				Если РезультатПоиска <> Неопределено
					И РезультатПоиска.ИмяПоля = ИмяСвойства
...
Функция ОбъектXDTOПоТекстуСообщенияXML(ТекстСообщенияXML, Тип = Неопределено) Экспорт
	...
	Если Тип = Неопределено Тогда
		СоздаваемыйТип = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(Неопределено, ИмяКорневогоЭлементаXML(ТекстСообщенияXML)).Тип();
	ИначеЕсли ТипЗнч(Тип) = Тип("Строка") Тогда
		СоздаваемыйТип = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(Неопределено, Тип).Тип();
	Иначе
		СоздаваемыйТип = Тип;
	КонецЕсли;
	...
	Если ХСПредприятияСОшибкамиСервиса <> Неопределено Тогда
		...
		Для Каждого СтрокаТЧ Из ХСПредприятияСОшибкамиСервиса Цикл
			...
			Для Каждого ЭлементОчереди Из СтрокаТЧ.ЭлементыОчереди Цикл
				...
				ДанныеФормированияРезультата = ДатаФормированияРезультата(ЭлементОчереди, ПараметрыОбмена, Неопределено);
				...
				ЗаписатьОшибкуСервисаВЖурналРегистрации(
					СтрШаблон(
				...
				Если ЗаписьОчереди.Выбран()
					И ЗаписьОчереди.Количество() > 0 Тогда
	...
	Исключение
		...
		Попытка
			...
			ОбъектXDTO = ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(
				РаботаСXMLИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ТекстВходящегоСообщенияXML),
				РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяТипа));
...
Процедура УстановитьПараметрыСтраницы(Запрос, НомерСтраницы, КоличествоЭлементовНаСтранице = Неопределено) Экспорт
	...
	Запрос.listOptions = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(Запрос, "listOptions");
...
Процедура УстановитьИнтервалЗапросаИзменений(Запрос, Интервал, ХранилищеВременныхДат) Экспорт
	...
	Запрос.updateDateInterval = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(Запрос, "updateDateInterval");
...
Процедура УстановитьАдрес(ОбъектXDTO, ДанныеАдреса, ИмяПоля = "address") Экспорт
	...
	ОбъектXDTO[ИмяПоля] = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, ИмяПоля);
	
	ОбъектXDTO[ИмяПоля].country      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "country");
	ОбъектXDTO[ИмяПоля].country.guid = ДанныеАдреса.СтранаGUID;
	
	ОбъектXDTO[ИмяПоля].region      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "region");
	ОбъектXDTO[ИмяПоля].region.guid = ДанныеАдреса.РегионGUID;
	
	Если ЗначениеЗаполнено(ДанныеАдреса.РайонGUID) Тогда
		ОбъектXDTO[ИмяПоля].district      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "district");
	...
	Если ДанныеАдреса.Свойство("ГородGUID") И ЗначениеЗаполнено(ДанныеАдреса.ГородGUID) Тогда
		...
		ОбъектXDTO[ИмяПоля].locality      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "locality");
		ОбъектXDTO[ИмяПоля].locality.guid = ДанныеАдреса.ГородGUID;
		
		Если ЗначениеЗаполнено(ДанныеАдреса.НаселенныйПунктGUID) Тогда
			ОбъектXDTO[ИмяПоля].subLocality      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "subLocality");
	...
	Если ЗначениеЗаполнено(ДанныеАдреса.УлицаGUID) Тогда
		ОбъектXDTO[ИмяПоля].street      = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO[ИмяПоля], "street");
...
Функция ОбработатьСообщениеОбОшибке(РезультатОтправкиЗапроса, ВозвращаемоеЗначение) Экспорт
	...
	Если РезультатОтправкиЗапроса.КодСостояния = 500 Тогда
		...
		Попытка
			...
			ОбъектXDTO = РаботаСXMLИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(
...
Процедура ДанныеКонтрагентаПоИННФоновоеЗадание(ПараметрыФормирования, АдресХранилища) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКонтрагентами") Тогда
		...
		Если ПараметрыФормирования.ЭтоЮридическоеЛицо Тогда
			ДанныеКонтрагентаПолные = МодульРаботаСКонтрагентами.СведенияОЮридическомЛицеПоИНН(ПараметрыФормирования.ИНН);
			ДанныеКонтрагента = Новый Структура;
			ДанныеКонтрагента.Вставить("ИНН",                ПараметрыФормирования.ИНН);
			ДанныеКонтрагента.Вставить("ОписаниеОшибки",     ДанныеКонтрагентаПолные.ОписаниеОшибки);
			ДанныеКонтрагента.Вставить("КПП",                "");
			ДанныеКонтрагента.Вставить("ОГРН",               "");
			ДанныеКонтрагента.Вставить("Наименование",       "");
			ДанныеКонтрагента.Вставить("НаименованиеПолное", "");
			ДанныеКонтрагента.Вставить("ПравоваяФорма",      "");
			ЗаполнитьЗначенияСвойств(ДанныеКонтрагента, ДанныеКонтрагентаПолные.ЕГРЮЛ,, "ИНН,ОписаниеОшибки");
		Иначе
			ДанныеКонтрагента = МодульРаботаСКонтрагентами.РеквизитыПредпринимателяПоИНН(ПараметрыФормирования.ИНН);
			ОчиститьТаблицыЗначений(ДанныеКонтрагента);
		КонецЕсли;
		
		ПоместитьВоВременноеХранилище(ДанныеКонтрагента, АдресХранилища);
	
	КонецЕсли;
	
КонецПроцедуры

Процедура ОчиститьТаблицыЗначений(СтруктураРеквизитов) Экспорт
	Для Каждого КлючИЗначение Из СтруктураРеквизитов Цикл
		ТипЗначения = ТипЗнч(КлючИЗначение.Значение);
		Если ТипЗначения = Тип("ТаблицаЗначений") Тогда
			СтруктураРеквизитов[КлючИЗначение.Ключ] = Неопределено;
		ИначеЕсли ТипЗначения = Тип("Структура") Тогда
			ОчиститьТаблицыЗначений(СтруктураРеквизитов[КлючИЗначение.Ключ]);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция ПодготовитьЗапросНаОбновлениеВетеринарноСопроводительногоДокумента(ДанныеВСД, УникальныйИдентификатор, ТекстОшибки = "") Экспорт
	...
	ПараметрыОбмена = ПараметрыОбмена(ХозяйствующиеСубъекты);
...
Процедура СформироватьДанныеДокументаОснования(ТаблицаДанныхДокументаОснования, ДокументОснование, ДанныеСформированы) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		|	ПроизводственнаяОперацияВЕТИСТовары.СкоропортящаясяПродукция            КАК СкоропортящаясяПродукция
		|ИЗ
		...
		|
		|	ВЫБРАТЬ
		|	ПроизводственнаяОперацияВЕТИССырье.Номенклатура,
		|	ПроизводственнаяОперацияВЕТИССырье.Характеристика,
		|	ПроизводственнаяОперацияВЕТИССырье.Серия,
		|	СУММА(ПроизводственнаяОперацияВЕТИССырье.Количество),
		|	ДАТАВРЕМЯ(1,1,1),
		|	ДАТАВРЕМЯ(1,1,1),
		|	Неопределено,
		|	Неопределено,
		...
		|ГДЕ
		|	ПроизводственнаяОперацияВЕТИССырье.Ссылка = &ДокументСсылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ПроизводственнаяОперацияВЕТИССырье.Номенклатура,
		|	ПроизводственнаяОперацияВЕТИССырье.Характеристика,
	...
	|	
		...
		|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент,
		...
		Пока Выборка.Следующий() Цикл
			...
			ДанныеИдентификатораПроисхожденияВЕТИС = Новый Структура(
				"ИдентификаторПроисхожденияВЕТИС, НачалоПериода, КонецПериода, ТочностьЗаполнения, СкоропортящаясяПродукция");
...
// Возвращает данные документа ВЕТИС для заполнения  документа Маркировка товаров ИСМП.
// 
// Параметры:
//  ЗаполнениеВыполнено - Булево - Заполнение выполнено
//  ДанныеЗаполнения - Произвольный - Данные заполнения
//  ДокументОбъект - ДокументОбъект - Заполняемый документ "Маркировка товаров ИС МП"
// 
// Возвращаемое значение:
//  Неопределено, Структура - Штрихкоды упаковок, присутствующие в документе "Производственная операция ВетИС"
//
Функция ДанныеЗаполненияДокументаМаркировкаТоваровИСМП(ЗаполнениеВыполнено, ДанныеЗаполнения, ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Если ДанныеЗаполнения.Свойство("ИмяФильтра")
			...
			ВидПродукции = ДанныеЗаполнения.ЗначениеФильтра;
		КонецЕсли;
	...
	ТранспортнаяОперация = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС");
	
	Если ТипОснования <> ПроизводственнаяОперация И ТипОснования <> ТранспортнаяОперация Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ВидПродукции = Неопределено Тогда
		...
		Иначе
			ВидПродукции = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
		КонецЕсли;
	КонецЕсли;
	
	ЗаполнениеВыполнено = Истина;
	Результат = Новый Структура("Шапка, Товары, ШтрихкодыУпаковок");
	Результат.Вставить("ВидПродукции", ВидПродукции);
	
	Если ТипОснования = ПроизводственнаяОперация Тогда
		Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫБОР
	...
	Результат.Шапка = Запрос.Выполнить().Выгрузить();
	...
	Если Не ДокументОбъект.ДополнительныеСвойства.Свойство("НеЗаполнятьТабличнуюЧасть") Тогда 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВетеринарноСопроводительныйДокументВЕТИС КАК ВСД
		|		ПО ТаблицаТовары.ВетеринарноСопроводительныйДокумент = ВСД.Ссылка
		|ГДЕ
		...
		|"
		+
		ШтрихкодированиеОбщегоНазначенияИС.ТекстЗапросаСвойстваМаркируемойПродукции("ТаблицаТовары")
		+
		"
		|;
		|/////////////////////////////////////////////
		...
		|ГДЕ
		|	СвойстваМаркируемойПродукции.ВидПродукции В (&ВидПродукции)
		|;
		|/////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаТовары.Номенклатура      КАК Номенклатура,
		...
		|	ТаблицаТовары.ВетеринарноСопроводительныйДокумент КАК ИдентификаторПроисхожденияВЕТИС,
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ВСД.СрокГодностиНачалоПериода, ДЕНЬ) <> НАЧАЛОПЕРИОДА(ВСД.СрокГодностиКонецПериода, ДЕНЬ)
		|				И ВСД.СрокГодностиКонецПериода <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ДАТАВРЕМЯ(1,1,1)
		|		ИНАЧЕ ВСД.СрокГодностиНачалоПериода
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВетеринарноСопроводительныйДокументВЕТИС КАК ВСД
		|		ПО ТаблицаТовары.ВетеринарноСопроводительныйДокумент = ВСД.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаОтбораНоменклатуры КАК ТаблицаОтбораНоменклатуры
		|		ПО ТаблицаТовары.Номенклатура = ТаблицаОтбораНоменклатуры.Номенклатура
		|ГДЕ
		|	ТаблицаТовары.Ссылка = &ДокументОснование
		...
		|	ТаблицаТовары.Серия,
		|	ТаблицаТовары.ВетеринарноСопроводительныйДокумент,
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ВСД.СрокГодностиНачалоПериода, ДЕНЬ) <> НАЧАЛОПЕРИОДА(ВСД.СрокГодностиКонецПериода, ДЕНЬ)
		|				И ВСД.СрокГодностиКонецПериода <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ДАТАВРЕМЯ(1,1,1)
		...
		|/////////////////////////////////////////////
		|";
		Если ТипОснования = ТранспортнаяОперация Тогда
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ ПЕРВЫЕ 0
			|	ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка) КАК ШтрихкодУпаковки,
			|	ЗНАЧЕНИЕ(Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ПустаяСсылка) КАК ИдентификаторПроисхожденияВЕТИС,
			|	ДАТАВРЕМЯ(1,1,1) КАК СрокГодности
			|";
		Иначе
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ
			|	Штрихкоды.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
			|	Товары.ВетеринарноСопроводительныйДокумент КАК ИдентификаторПроисхожденияВЕТИС,
			|	ВЫБОР
			|		КОГДА НАЧАЛОПЕРИОДА(Товары.СрокГодностиНачалоПериода, ДЕНЬ) <> НАЧАЛОПЕРИОДА(Товары.СрокГодностиКонецПериода, ДЕНЬ)
			|			И Товары.СрокГодностиКонецПериода <> ДАТАВРЕМЯ(1,1,1)
			|			ТОГДА ДАТАВРЕМЯ(1,1,1)
			|		ИНАЧЕ Товары.СрокГодностиНачалоПериода
			|	КОНЕЦ КАК СрокГодности
			|ИЗ
			|	Документ.ПроизводственнаяОперацияВЕТИС.Товары КАК Товары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводственнаяОперацияВЕТИС.УпаковкиВЕТИС КАК УпаковкиВЕТИС
			|		ПО УпаковкиВЕТИС.Ссылка = &ДокументОснование
			|		И УпаковкиВЕТИС.ИдентификаторСтрокиТовары = Товары.ИдентификаторСтроки
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводственнаяОперацияВЕТИС.ШтрихкодыУпаковок КАК Штрихкоды
			|		ПО Штрихкоды.Ссылка = &ДокументОснование
			|		И Штрихкоды.ИдентификаторСтроки = УпаковкиВЕТИС.ИдентификаторСтроки
			|		И Штрихкоды.ШтрихкодУпаковки <> ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаОтбораНоменклатуры КАК ТаблицаОтбораНоменклатуры
			|		ПО Товары.Номенклатура = ТаблицаОтбораНоменклатуры.Номенклатура
			|ГДЕ
			|	Товары.Ссылка = &ДокументОснование
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПроизводственнаяОперацияВЕТИС") Тогда
			Запрос.Текст = СтрШаблон(ТекстЗапроса, "ПроизводственнаяОперацияВЕТИС");
		Иначе
			Запрос.Текст = СтрШаблон(ТекстЗапроса, "ВходящаяТранспортнаяОперацияВЕТИС");
		КонецЕсли;
		
		Пакет = Запрос.ВыполнитьПакет();
		ВГраница = Пакет.ВГраница();
		Результат.Товары = Пакет[ВГраница-1].Выгрузить();
		Результат.ШтрихкодыУпаковок = Пакет[ВГраница].Выгрузить();
...
//
Функция СоздатьВетеринарноСопроводительныйДокументВЕТИСДляЗагрузки(Идентификатор) Экспорт
...
Функция ИдентификаторыКлассификаторов() Экспорт
	...
	Идентификаторы.Вставить(
		"ProductsAndGoalsCertSpecVETIS",
		Новый Структура(
			"Наименование, ВидКлассификатора",
			НСтр("ru = 'Допустимые цели и виды продукции (Аттестованный специалист) ВетИС'"),
			Перечисления.РолиПользователейВЕТИС.АттестованныйСпециалист));
	Идентификаторы.Вставить(
		"ProductsAndGoalsAuthPersonVETIS",
		Новый Структура(
			"Наименование, ВидКлассификатора",
			НСтр("ru = 'Допустимые цели и виды продукции (Уполномоченное лицо) ВетИС'"),
...
Функция ПроблемыКлассификаторовВПанелиОбмена(РасширенноеОписаниеТребуетсяЗагрузка = Ложь) Экспорт
	...
	Если РазделениеВключено Тогда
		...
		ТаблицаИсточник.Колонки.Добавить("ИдентификаторКлассификатора", ОбщегоНазначения.ОписаниеТипаСтрока("255"));
		ТаблицаИсточник.Колонки.Добавить("РольПользователя",            Новый ОписаниеТипов("ПеречислениеСсылка.РолиПользователейВЕТИС"));
		
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	ТаблицаИсточник.ИдентификаторКлассификатора,
		|	ТаблицаИсточник.РольПользователя
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВидыПродукцииПоГруппамВЕТИС.РольПользователя,
		|	ИСТИНА КАК НаличиеЗаписей
		|ПОМЕСТИТЬ ЗаписиРегистра
		|ИЗ
		|	РегистрСведений.ВидыПродукцииПоГруппамВЕТИС КАК ВидыПродукцииПоГруппамВЕТИС
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаИсточник.ИдентификаторКлассификатора,
		|	ТаблицаИсточник.РольПользователя,
		|	ЕСТЬNULL(ЗаписиРегистра.НаличиеЗаписей, ЛОЖЬ) КАК НаличиеЗаписей
		|ИЗ
		|	ТаблицаИсточник КАК ТаблицаИсточник
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаписиРегистра КАК ЗаписиРегистра
		|		ПО ТаблицаИсточник.РольПользователя = ЗаписиРегистра.РольПользователя";
		
		Для Каждого КлючИЗначение Из ИдентификаторыКлассификаторов Цикл
			
			ОписаниеКлассификатора = КлючИЗначение.Значение;
			НоваяСтрока = ТаблицаИсточник.Добавить();
			НоваяСтрока.ИдентификаторКлассификатора = КлючИЗначение.Ключ;
			НоваяСтрока.РольПользователя            = ОписаниеКлассификатора.ВидКлассификатора;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ВыборкаДетальныеЗаписи.НаличиеЗаписей Тогда
				Продолжить;
			КонецЕсли;
			
			ОписаниеКлассификатора = ИдентификаторыКлассификаторов[ВыборкаДетальныеЗаписи.ИдентификаторКлассификатора];
			ДоступныеКлассификаторыДляЗагрузки.Добавить(ОписаниеКлассификатора.Наименование);
...
Функция ПоляСтрокиДанныхКлассификатора(ИмяМетаданных) Экспорт
	
	СтруктураПолей = Новый Структура();
	СтруктураПолей.Вставить("ГруппаПриказа");
	
	Если ИмяМетаданных = Метаданные.РегистрыСведений.ДопустимыеЦелиПоГруппамВЕТИС.ПолноеИмя() Тогда
		СтруктураПолей.Вставить("ЦельНаименование");
		СтруктураПолей.Вставить("ЦельИдентификатор");
	ИначеЕсли ИмяМетаданных = Метаданные.РегистрыСведений.ВидыПродукцииПоГруппамВЕТИС.ПолноеИмя() Тогда
		СтруктураПолей.Вставить("ВидПродукцииНаименование");
		СтруктураПолей.Вставить("ВидПродукцииИдентификатор");
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Не определены поля строка данных классификатора %1.'"),
			ИмяМетаданных);
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из СтруктураПолей Цикл
		СтруктураПолей[КлючИЗначение.Ключ] = КлючИЗначение.Ключ;
	КонецЦикла;
...
Функция ТекстЗапросаСопоставленныеТоварыБезЗаписиЖурнала()
	...
	|	&ПустаяЗаписьЖурнала КАК ЗаписьСкладскогоЖурнала,
...
Функция ТекстЗапросаСопоставленныеТоварыСЗаписямиЖурнала()
	...
	|СГРУППИРОВАТЬ ПО
	|	ТабличнаяЧасть.Продукция,
	|	ТабличнаяЧасть.ЗаписьСкладскогоЖурнала
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СоответствиеНоменклатуры.Номенклатура) КАК КоличествоНоменклатура,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СоответствиеНоменклатуры.Характеристика) КАК КоличествоХарактеристика
	|ПОМЕСТИТЬ ВтКоличествоСопоставлено2
	|ИЗ
	|	Товары КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК СоответствиеНоменклатуры
	|		ПО СоответствиеНоменклатуры.Продукция = ТабличнаяЧасть.Продукция
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКоличествоСопоставлено1 КАК ВтКоличествоСопоставлено1
	|		ПО ВтКоличествоСопоставлено1.Продукция = ТабличнаяЧасть.Продукция
	|		И ВтКоличествоСопоставлено1.ЗаписьСкладскогоЖурнала = ТабличнаяЧасть.ЗаписьСкладскогоЖурнала
	|ГДЕ
	|	ВтКоличествоСопоставлено1.Продукция ЕСТЬ NULL
	|СГРУППИРОВАТЬ ПО
	|	ТабличнаяЧасть.Продукция,
	|	ТабличнаяЧасть.ЗаписьСкладскогоЖурнала
	|;
	...
	|	ТабличнаяЧасть.Продукция,
	|	ТабличнаяЧасть.ЗаписьСкладскогоЖурнала,
	|	ТабличнаяЧасть.КоличествоНоменклатура,
	|	ТабличнаяЧасть.КоличествоХарактеристика,
	|	1,
	|	ЛОЖЬ
	|ИЗ
	|	ВтКоличествоСопоставлено2 КАК ТабличнаяЧасть
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	КоличествоСопоставлено.ЕстьЗаписьЖурнала КАК ЕстьЗаписьЖурнала
	|ПОМЕСТИТЬ ВтСопоставлено
	|ИЗ
	|	Товары КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтКоличествоСопоставлено КАК КоличествоСопоставлено
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК СоответствиеНоменклатуры
	|		ПО СоответствиеНоменклатуры.Продукция = КоличествоСопоставлено.Продукция
	|		И (КоличествоСопоставлено.ЕстьЗаписьЖурнала = Ложь
	|			ИЛИ КоличествоСопоставлено.ЗаписьСкладскогоЖурнала = СоответствиеНоменклатуры.ЗаписьСкладскогоЖурнала)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура УстановитьВидимостьКомандыВыполнитьОбмен(Форма, ИмяЭлемента) Экспорт
	...
	Если НЕ ОбщегоНазначенияИС.ПравоОпределяемогоТипа(Метаданные.ОпределяемыеТипы.ДокументыВЕТИС, "Изменение") Тогда
		Кнопка = Форма.Элементы.Найти(ИмяЭлемента);
		Если Кнопка <> Неопределено Тогда
			Кнопка.Видимость = Ложь;
		КонецЕсли;
