...
Процедура ПодобратьВариантыСопоставленияПоНатуральнымКлючам(НаборНоменклатурыКонтрагентов, ПроцентТочностиПоискаНаименований) Экспорт
	...
	ИмяСправочникаЕдиниц = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("ЕдиницыИзмерения");
	МетаданныеЕдиниц = Метаданные.Справочники[ИмяСправочникаЕдиниц];
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("КодОКЕИ", МетаданныеЕдиниц) Тогда
		ИмяКодаЕдиницы = "КодОКЕИ";
	Иначе
		ИмяКодаЕдиницы = "Код";
	КонецЕсли;
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ + ВЫБОР
	|		КОГДА СправочникНоменклатура.ЕдиницаИзмерения." + ИмяКодаЕдиницы + " = ВТДанныеНоменклатурыКонтрагентов.ЕдиницаИзмеренияКод
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник." + ИмяСправочникаНоменклатура + " КАК СправочникНоменклатура
		...
		Если ВыборкаВесВыборки.Следующий() Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				ЗаполнитьВариантСопоставленияНоменклатуры(ВариантСопоставления,
					НоменклатураКонтрагентов.ВариантыСопоставленияНоменклатурыБЭД,
					ВариантСопоставления.НоменклатураИБ);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеВходящегоОбъекта(ТранспортныйКонтейнер,
	...
	ЭД = ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеФайла);
	...
	ИначеЕсли ЕстьСвойство(ЭД, "ИдФайл") Тогда
		Если ЭтоПакет Тогда
			...
			ДокументЭД = ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеДокумента);
		Иначе
			ДокументЭД = ЭД;
		КонецЕсли;
		ИдФайл = ЭД.ИдФайл;
		...
		Если ТипЭлементаВерсииЭД = "ИОП" Тогда
			...
			ДокументооборотИД = ПолучитьИдДокументооборотаИзИОПИлиУУ(
				ДокументЭД.Документ.СвИзвПолуч.СведПолФайл, ДокументооборотИД);
		...
		ИначеЕсли ТипЭлементаВерсииЭД = "УУЭСФ" Тогда
			...
			ДокументооборотИД = ПолучитьИдДокументооборотаИзИОПИлиУУ(
				ДокументЭД.Документ.СвУведУточ.СведПолФайл, ДокументооборотИД);
		...
		ИначеЕсли ТипЭлементаВерсииЭД = "ПДО" Тогда
			...
			ДокументооборотИД = ПолучитьИдДокументооборотаПоИдФайла(
				ДокументЭД.Документ.СведПодтв.СведОтпрФайл.ИмяПостФайла, ДокументооборотИД); 
		ИначеЕсли ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаЗаказчика()
			ИЛИ ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаПодрядчика() Тогда
...
Процедура ПолучитьНовыеДокументыЕИС(
	...
	ИмяСобытия = НСтр("ru = 'Получение транспортных контейнеров из ЕИС'",
		КодЯзыка);
	Попытка
		ПолучитьНовыеДокументыПоставщика(МассивКонтейнеров, КонтекстОперации);
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов поставщика'");
		ОписаниеОшибки = ИнформацияОбОшибке();
		КраткоеОписание = ОбработкаОшибок.КраткоеПредставлениеОшибки(ОписаниеОшибки);
		ПодробноеПредставление = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ОписаниеОшибки);
		Идентификатор = "СбойПриПолученииДокументовПоставщика";
		ВидОшибки = ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, КраткоеОписание);
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов поставщика'");
		КраткоОбОшибке = НСтр("ru = 'Ошибка при получении документов поставщика'");
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
			ВидОшибки,
			ПодробноОбОшибке,
			КраткоОбОшибке);
		...
		ЗаписьЖурналаРегистрации(
			...
			ЗаголовокПроблемы + ":" + символы.ПС + ПодробноеПредставление);
	КонецПопытки;
	...
	Попытка
		ПолучитьНовыеДокументыЗаказчика(
			МассивКонтейнеров, КлючиСинхронизации, КонтекстДиагностики, КонтекстОперации);
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов заказчика'");
		ОписаниеОшибки = ИнформацияОбОшибке();
		КраткоеОписание = ОбработкаОшибок.КраткоеПредставлениеОшибки(ОписаниеОшибки);
		Идентификатор = "СбойПриПолученииДокументовЗаказчика";
		ВидОшибки = ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, КраткоеОписание);
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов заказчика'");
		КраткоОбОшибке = НСтр("ru = 'Ошибка при получении документов заказчика'");
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
			ВидОшибки,
			ПодробноОбОшибке,
			КраткоОбОшибке);
...
Процедура ЗаполнитьИзДанныхКонтракта(Контракт, Объект, ЭтоКонтрактДляЗаказчика = Ложь) Экспорт
	...
	Объект.ЭтоЗакрытыйКонтракт = Контракт.ЭтоЗакрытыйКонтракт;
	Объект.ЗакупкаУЕдинственногоПоставщика = Контракт.ЗакупкаУЕдинственногоПоставщика;
...
Функция СформироватьПечатнуюФормуДляЕИС(ДанныеФайла, ПараметрыПечати)
	...
	ОбъектXDTO = ОбъектXDTOИзДвоичныхДанных(ДанныеФайла);
...
Функция СтатусыОтправок(Организация, ЭтоСтатусыЗаказчика = Ложь)
	...
	|	И СообщениеЭДО.Дата >= &ДатаОтсечки";
	...
	ЗапросОтправок.УстановитьПараметр("СпособОбмена",
		Перечисления.СпособыОбменаЭД.ЧерезСерверЕИС);
	// Пакеты удаляются из шины ЕИС по истечении двух месяцев.
	СекундХраненияВШинеЕИС = 40 * 24 * 3600;
	ТекущаяДата = ТекущаяДатаСеанса();
	ЗапросОтправок.УстановитьПараметр("ДатаОтсечки",
		ТекущаяДата - СекундХраненияВШинеЕИС);
...
// Аналог процедуры см. СопоставлениеНоменклатурыКонтрагентовСлужебный.ЗаполнитьВариантСопоставленияНоменклатуры
...
// Аналог функции см. СопоставлениеНоменклатурыКонтрагентовСлужебный.НовыйВариантСопоставленияНоменклатурыБЭД.
// 
// Возвращаемое значение:
...
Функция ПолучитьШапкуПакетаЕИСЗаказчика(ОбъектЭД,
	...
	Если ТипЗнч(ОбъектЭД) = Тип("ДвоичныеДанные") Тогда
		Объект = ОбъектXDTOИзДвоичныхДанных(ОбъектЭД);
	КонецЕсли;
	...
	Если ТипЭлементаВерсииЭД = "ПакетЕИС" Тогда
		...
		ДанныеДокумента = ОбъектXDTOИзДвоичныхДанных(Контент);
	...
	ИначеЕсли ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаПодрядчика() Тогда
		Результат.Отправитель = Объект.Документ.СвАктСдПр.СвПодр.СвСторДог.ИнфДляУчаст;
		Результат.Получатель = Объект.Документ.СвАктСдПр.СвЗак.СвСторДог.ИнфДляУчаст;
		СведенияОКонтрагенте = ИзвлечьСведенияОКонтрагентеИзАктаСтройки(ОбъектЭД);
	Иначе
	...
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл
			...
			Если ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПокупателяУПД()
				ИЛИ ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПокупателяУКД()
				ИЛИ ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаЗаказчика() Тогда
			...
			Если НЕ (ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПоставщикаУПД()
				...
				ИЛИ ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаПодрядчика())
...
Функция ИсключаемыеПриЗагрузкеВидыДокументовДляПоставщика()
	...
	ВидАктОПриемкеВыполненныхРаботПоставщик = "ON_AKTREZRABP";
	ВидАктОПриемкеВыполненныхРаботЗаказчик = "ON_AKTREZRABZ";
	...
	МассивИсключаемыхВидов.Добавить(ВидАктОПриемкеВыполненныхРаботПоставщик);
	МассивИсключаемыхВидов.Добавить(ВидАктОПриемкеВыполненныхРаботЗаказчик);
	
	Возврат МассивИсключаемыхВидов;
...
Процедура ПолучитьДокументы(
	...
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		МассивИсключаемыхВидов = ИсключаемыеПриЗагрузкеВидыДокументовДляПоставщика();
...
Функция ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеДокумента, ПроверятьКодировку = Ложь)

	Возврат ЭлектронноеАктированиеЕИС.ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеДокумента, ПроверятьКодировку);
...
Процедура ПолучитьРезультатыОбработкиПакетов(
	...
	Пока ВыборкаОтправок.Следующий() Цикл
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru='Ошибка при получении пакета %2 с квитанцией ЕИС: %1'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатОбработкиПакета.ОписаниеОшибки, ИдентификаторПакетаБЭД);
			ЭлектронноеАктированиеЕИС.ЗаписатьСобытиеВЖурнал(
				ТекстОшибки, , , УровеньЖурналаРегистрации.Ошибка);
...
Функция ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, ТекстОшибки)
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = Идентификатор;
	ВидОшибки.ЗаголовокПроблемы = ЗаголовокПроблемы;
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Произошла ошибка:'") + " " + ТекстОшибки;
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'См. подробности ошибки в журнале регистрации'");
...
Функция ТипЭлементаВерсииАктаПодрядчика()
	
	Возврат "ИнформацияПодрядчика";
	
КонецФункции

Функция ТипЭлементаВерсииАктаЗаказчика()
	
	Возврат "ИнформацияЗаказчика";
...
Процедура ИзвлечьВложения(СтруктураЭлектронногоДокумента,
	...
	Если ЭлементВложений <> Неопределено Тогда
		...
		Для Каждого ЭлементВложение Из МассивВложений Цикл
			...
			ДанныеФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(
				ИмяФайлаВложения);
			
			Если НРег(ДанныеФайла.Расширение) = ".xml" Тогда
				ИмяФайлаВложения = ДанныеФайла.ИмяБезРасширения + ".xml_eis";
			КонецЕсли;
...
Функция ИзвлечьСведенияОКонтрагентеИзАктаСтройки(ОбъектЭД)
	
	Результат = ЭлектронноеАктированиеЕИС.НовыеСведенияОКонтрагенте();
	
	СведенияОКонтрагенте = ОбъектЭД.Документ.СвАктСдПр.СвПодр.СвСторДог.ИдСВ;
	
	ЗаполнитьСведенияОКонтрагенте(Результат, СведенияОКонтрагенте, "СвЮЛУч", "СвИП");
...
Функция ОпределитьПараметрыЭлектронногоДокумента(ИдФайл)
	...
	ИначеЕсли СтрНайти(ИдФайл, "DP_PDOTPR") Тогда
		...
		ТипДокумента = "SendingTimeConfirmation";
		КодТранзакции = "SendConfirmation";
	ИначеЕсли СтрНайти(ИдФайл, "ON_AKTREZRABP") Тогда
		// Акт приемки строительных работ информация подрядчика ЕД-7-26/691.
		ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаПодрядчика();
		ТипДокумента = "FormalizedKC2Contractor";
		КодТранзакции = "MainDocument";
	ИначеЕсли СтрНайти(ИдФайл, "ON_AKTREZRABZ") Тогда
		// Акт приемки строительных работ информация заказчика ЕД-7-26/691.
		ТипЭлементаВерсииЭД = ТипЭлементаВерсииАктаЗаказчика();
		ТипДокумента = "FormalizedKC2Customer";
		КодТранзакции = "CustomerTitle";
	КонецЕсли;
...
Функция НовоеОписаниеКарточкиПакета()
	...
	Карточка.Вставить("ДополнительныеПараметры", Новый Структура);
...
Функция ПолучитьИдДокументооборотаИзИОПИлиУУ(УзелИмениДокумента, ДокументооборотИДПоУмолчанию)
	
	Если ЕстьСвойство(УзелИмениДокумента, "ИмяПолФайла") Тогда
		ДокументооборотИД = ПолучитьИдДокументооборотаПоИдФайла(
			УзелИмениДокумента.ИмяПолФайла, ДокументооборотИДПоУмолчанию);
	Иначе
		// Для старых версий.
		ДокументооборотИД = ПолучитьИдДокументооборотаПоИдФайла(
			УзелИмениДокумента.ИмяПостФайла, ДокументооборотИДПоУмолчанию);
	КонецЕсли;
