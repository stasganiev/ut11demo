...
Процедура ЗаполнитьДанные_ИзвещениеОПолучении(Данные, ДанныеДляФормирования, Отказ) Экспорт
	...
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, "ВерсияФормата"
																, ВерсияФорматаКвитанций()); 
...
Функция ВерсияФорматаКвитанций()
	
	Если  ТекущаяДатаСеанса()<Дата("20230519") Тогда
		Возврат "1.02";
	Иначе
		Возврат "1.03";
	КонецЕсли;
...
Процедура ЗаполнитьДанныеНоменклатурыДляСопоставленияКоммерческихПредложений(Элемент, НовЗапись) Экспорт
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "product.supplierNomenclature") <> Неопределено Тогда
		...
		Штрихкоды = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "product.supplierNomenclature.barcodes");
