...
// Получает URL для перехода в веб-версию СПАРК.
//
// Параметры:
//	ИНН - Строка - ИНН Контрагента для получения ссылки.
//
// Возвращаемое значение:
//	Структура - результат получения:
//		* URL - Строка - ссылка для перехода в веб-версию СПАРК;
//		* ВидОшибки - ПеречислениеСсылка.ВидыОшибокСПАРКРиски - ошибка, возникшая при
//			обращении к сервису 1СПАРК Риски;
//		* СообщениеОбОшибке - Строка - краткое описание ошибки, которое может быть
//			отображено пользователю;
...
//			быть записано в журнал регистрации;
//
Функция URLВебВерсииСПАРК(ИНН) Экспорт 
	
	Результат = НовыйРезультатВызоваСервиса();
	Результат.Вставить("URL" , Неопределено);
	
	ДанныеАутентификации = Неопределено;
	ПередВызовомОперацииСервиса(Результат, , ДанныеАутентификации, Истина);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат Результат;
	КонецЕсли;
	
	ВызватьОперациюПолучитьURLВебВерсииСПАРК(
		ИНН,
		Результат,
		ДанныеАутентификации);
...
Процедура ЗаписатьДанныеАутентификации(ЗаписьДанныхСообщения, ДанныеАутентификации)
	...
	Если ДанныеАутентификации.ИспользуетсяТикет Тогда
...
//
Процедура ПрочитатьЗначениеИзСтруктуры(
	...
	Если Данные.Свойство(ИмяКлюча) Тогда // АПК:1415 Данные из внешних источников.
...
Функция НовыйТелоЗапросаEnterpreneurMonitoringEventsSearch(
	...
	Если ТипПараметрОтбора = Тип("Число") Тогда
		...
		НачальныйИдентификатор = СтрЗаменить(Строка(ПараметрОтбора), Символы.НПП, "");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(НачальныйИдентификатор);
	ИначеЕсли ТипПараметрОтбора = Тип("Массив") Тогда
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("innList");
		ЗаписьДанныхСообщения.ЗаписатьНачалоМассива();
		Для Итератор = НачальныйИндекс По КонечныйИндекс Цикл
...
#Область ПереходВВебВерсиюСПАРКРиски

////////////////////////////////////////////////////////////////////////////////
// Вызов операции /web-cards/get-card-url

Процедура ВызватьОперациюПолучитьURLВебВерсииСПАРК(
		ИНН,
		Результат,
		ДанныеАутентификации)
	
	URLОперации = URLОперацииСервиса("web-cards/get-card-url");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
	
	ТелоЗапроса = НовыйТелоЗапросаWebCardsGetCardUrl(
		ИНН,
		ДанныеАутентификации,
	...
	Попытка
		ОбработатьТелоОтветаWebCardsGetCardUrl(ТелоОтвета, Результат);
	Исключение
		ПриОшибкеОбработкиОтветаСервиса(ИнформацияОбОшибке(), URLОперации, ТелоОтвета, Результат);
	КонецПопытки;
...
Функция НовыйТелоЗапросаWebCardsGetCardUrl(
		ИНН,
		ДанныеАутентификации,
		ДополнительныеПараметрыЗапроса)
	...
	ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("inn");
	ЗаписьДанныхСообщения.ЗаписатьЗначение(ИНН);
...
Процедура ОбработатьТелоОтветаWebCardsGetCardUrl(ТелоОтвета, Результат)
	...
	Результат.URL = ДанныеОтвета.cardUrl;
...
Функция ЗагрузитьИндексыКонтрагентов(
	...
	Результат = НовыйРезультатВызоваСервиса();
	Результат.Вставить("ЗначенияИндексов", Неопределено);
...
Функция ОписаниеТаблицыИндексов(ОписаниеКонтрагентов)
	...
	Для Каждого ТекущийКонтрагент Из ОписаниеКонтрагентов Цикл
		...
		// Уже без пробелов.
		НоваяСтрока.ИНН        = ТекущийКонтрагент.ИНН;
		НоваяСтрока.Активен    = Истина;
		// Вид ошибки есть общий (для всего результата) и для каждого ИНН.
		НоваяСтрока.ВидОшибки  = Перечисления.ВидыОшибокСПАРКРиски.НеизвестныйИНН;
