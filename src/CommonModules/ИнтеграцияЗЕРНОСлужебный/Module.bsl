...
Функция ПодготовитьДанныеКПередачеПоНастройкеРегламентногоЗадания(ПараметрыОбмена) Экспорт
	...
	Если ДобавитьИнтервал Тогда
		ПараметрыЗапроса.Вставить("Интервал", ИнтеграцияЗЕРНОКлиентСервер.СтруктураИнтервала(
			НачалоДня(ТекущаяДатаСеанса()) - (ПараметрыНастройкиОбмена.ДнейЗагрузки - 1) * 86400));
	КонецЕсли;
...
Процедура ПостроитьОчередьСообщений(ПараметрыОбмена, СообщениеИлиДокумент = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПередаватьПосле) Тогда
			НоваяЗависимость = ПараметрыОбмена.ЗависимостиСообщений.Добавить();
			НоваяЗависимость.Сообщение1 = Выборка.ПередаватьПосле;
			НоваяЗависимость.Сообщение2 = Выборка.Сообщение;
...
Функция ПодготовитьКПередачеУниверсально(ВходящиеДанные, ПараметрыОбмена, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ТребуетсяОжидание = ТребуетсяОжидание(ДатаСледующейИтерации, ЕстьОшибки, ПараметрыОбмена);
...
//   * КонвертЗапросXML            - Строка - Текст сообщения XML.
//   * КонвертРезультатXML         - Строка - Текст сообщения XML.
//   * КонвертПодтверждениеXML     - Строка - Текст сообщения XML.
//   * Организация                 - ОпределяемыйТип.Организация - Организация.
//   * Операция                    - ПеречислениеСсылка.ВидыОперацийЗЕРНО - Вид операции.
//   * ТипСообщения                - ПеречислениеСсылка.ТипыЗапросовИС - Тип запроса.
//   * СсылкаНаОбъект              - ДокументСсылка, СправочникСсылка, ПеречислениеСсылка - Ссылка на объект.
...
//   * ИмяКорневогоСвойстваXDTO    - Строка, Неопределено - Имя корневого свойства
...
Функция СтруктураСообщенияXML() Экспорт
	...
	СообщениеXML.Вставить("ЗагружатьДо");
	СообщениеXML.Вставить("ЗагружатьПосле");
	СообщениеXML.Вставить("ПередаватьПосле");
	
	// Для использования в механизмах электронной подписи
	СообщениеXML.Вставить("ТекстСообщенияЗапросXML");
	СообщениеXML.Вставить("КонвертЗапросXML");
	СообщениеXML.Вставить("КонвертРезультатXML");
	СообщениеXML.Вставить("КонвертПодтверждениеXML");
	СообщениеXML.Вставить("ПодписываемоеТекущееДействие");
	СообщениеXML.Вставить("Сертификат");
	...
	СообщениеXML.Вставить("ВерсияГосИС");
	СообщениеXML.Вставить("Операция");
	СообщениеXML.Вставить("ВидПродукции");
	...
	СообщениеXML.Вставить("Подразделение");
	СообщениеXML.Вставить("СсылкаНаОбъект");
...
Функция ОбъектXDTOВXML(ОбъектXDTO, ПространствоИмен, ИмяТипа, ПараметрыОбмена = Неопределено) Экспорт
	...
	Если ПараметрыОптимизации.ВерсияAPI <> ПакетыXDTO.ВерсияAPI Тогда
	...
	Если ПараметрыОптимизации.ВерсияСервисаAPI <> ПакетыXDTO.ВерсияСервисаAPI Тогда
...
Процедура ОбработатьДанныеРезультатаОтвета(ЗНАЧ ИсходныйТекстОтвета, ПараметрыОбмена, ВозвращаемоеЗначение) Экспорт
	...
	Если ПараметрыОптимизации.ВерсияAPI <> ПакетыXDTO.ВерсияAPI Тогда
		Для Каждого ПакетXDTO Из ПакетыXDTO.СписокAPI Цикл
			ИсходныйТекстОтвета = СтрЗаменить(
				ИсходныйТекстОтвета,
				СтрЗаменить(
					ПакетXDTO.ПространствоИмен,
					ПакетыXDTO.ВерсияAPI,
	...
	Попытка
		...
		ТекстВложенногоСообщенияXML = ПолучитьЗначениеИзДокументаDOMПоИмениПоля(ИсходныйТекстОтвета, "Envelope",, Истина);
		Если ТекстВложенногоСообщенияXML = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось определить корневой элемент конверта'");
		КонецЕсли;
		ОбъектXDTOКонверт = РаботаСXMLИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ТекстВложенногоСообщенияXML);
		Если ОбъектXDTOКонверт.Свойства().Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось определить корневой элемент тела сообщения'");
		Иначе
			СвойствоОтвета = ОбъектXDTOКонверт.Свойства()[0];
			ТекстВложенногоСообщенияXML = ПолучитьЗначениеИзДокументаDOMПоИмениПоля(ИсходныйТекстОтвета, СвойствоОтвета.ЛокальноеИмя);
		КонецЕсли;
		
		Если СвойствоОтвета.ЛокальноеИмя = "Fault" Тогда
			ОбъектXDTO = ОбъектXDTOКонверт.Fault;
		Иначе
			ОбъектXDTO = ПрочитатьОбъект(ТекстВложенногоСообщенияXML, СвойствоОтвета.URIПространстваИмен, СвойствоОтвета.ЛокальноеИмя);
		КонецЕсли;
	...
	ПреобразованныйОбъектXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(СвойствоОтвета.URIПространстваИмен, СвойствоОтвета.ЛокальноеИмя);
	
	ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(ОбъектXDTO, ПреобразованныйОбъектXDTO, ПараметрыПреобразования);
...
Функция ПараметрыПреобразования() Экспорт
	...
	ОбъектСпискаСтрок = РаботаСXMLИС.ОбъектXDTO(ПространствоИменПоИмениПакетаXDTO("zerno_dictionary"), "StringListType");
	ПараметрыПреобразования.ТипыИмеющиеВложенныеСвойства.Вставить(ОбъектСпискаСтрок.Тип(), ОбъектСпискаСтрок.Свойства()[0]);
	
	ВерсияСервисаAPI = ИнтеграцияЗЕРНО.ПараметрыОптимизации().ВерсияСервисаAPI;
	
	#Область КорневыеТипы
	ПространстваИмен = Новый Массив();
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_dictionaries"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_sdiz"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_lots"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_grainmonitor"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_gpb"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_api_gpb_sdiz"));
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_ws_faults", ВерсияСервисаAPI));
	...
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_dictionary"));
	
	Для Каждого ПространствоИмен Из ПространстваИмен Цикл
		ПакетXDTO = ФабрикаXDTO.Пакеты.Получить(ПространствоИмен);
		Для Каждого Свойство Из ПакетXDTO.КорневыеСвойства Цикл
			ПараметрыПреобразования.ПространстваИменПоИменамСвойств.Вставить(Свойство.Имя, Свойство.URIПространстваИмен);
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ПараметрыПреобразования.СоответствиеСвойствСОпечатками Цикл
		ПространствоИмен = ПараметрыПреобразования.ПространстваИменПоИменамСвойств[КлючИЗначение.Ключ];
		Если ПространствоИмен <> Неопределено Тогда
			ПараметрыПреобразования.ПространстваИменПоИменамСвойств.Вставить(КлючИЗначение.Значение, ПространствоИмен);
		КонецЕсли;
	КонецЦикла;
	
	РеквизитыТипа = РеквизитыКорневогоТипа("КодОтвета");
	ПараметрыПреобразования.КорневыеТипы.Вставить(
		ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_ws_types", ВерсияСервисаAPI), "ResponseCodeType"),
		РеквизитыТипа);
		
	РеквизитыТипа = РеквизитыКорневогоТипа("ИдентификаторЗаявки");
	РеквизитыТипа.ИмяПоля = "MessageID";
	ПараметрыПреобразования.КорневыеТипы.Вставить(
		ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_ws_types", ВерсияСервисаAPI), "UUID"),
		РеквизитыТипа);
	...
	ПараметрыПреобразования.ТипыИмеющиеСодержаниеТега.Вставить(
		ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "ValueQualityIndicatorType"),
		РеквизитыТипа);
	...
	Для Каждого ВидКлассификатора Из Перечисления.ВидыКлассификаторовЗЕРНО Цикл
		...
		ТипПоля = ФабрикаXDTO.Тип(
			ПространствоИменПоИмениПакетаXDTO("zerno_dictionary"),
	...
	Для Каждого КлючИЗначение Из ПараметрыПреобразования.ТипыОрганизацийПоИменамСвойств Цикл
		...
		ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_organizations"), КлючИЗначение.Ключ);
	...
	// Первичные документы
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "DocNumberDateType");
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_sdiz"), "ConfirmingDocumentType");
	РеквизитыТипа = Новый Структура();
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "DeclarationOfConformitType");
	РеквизитыТипа = Новый Структура();
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "PhytosanitaryCertificateType");
	РеквизитыТипа = Новый Структура();
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "VeterinaryCertificateType");
	РеквизитыТипа = Новый Структура();
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_common"), "DocOtherNumberDateType");
	РеквизитыТипа = Новый Структура();
	РеквизитыТипа.Вставить("ТипДокумента", Перечисления.ТипыПервичныхДокументовЗЕРНО.Товаросопроводительный);
	РеквизитыТипа.Вставить("ТипПоля",      ТипПоля);
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_gpb_sdiz"), "ConfirmingDocumentType");
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_sdiz"), "AcceptanceElevatorType");
	РеквизитыТипа = Новый Структура();
	...
	// Данные адреса
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_organizations"), "AddressRFType");
	...
	// Реестр партий
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_lots"), "RecordLotType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "number");
	
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_lots"), "RecordGpbType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "number");
	
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_lots"), "ParentLotType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "lotNumber");
	
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_gpb"), "ParentGpbType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "gpbNumber");
	
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_sdiz"), "RecordSDIZType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "lotNumber");
	
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_gpb_sdiz"), "RecordGpbSDIZType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.РеестрПартийЗЕРНО, "gpbNumber");
	
	//Результаты исследования
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_grainmonitor"), "RecordResearchType");
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_grainmonitor"), "RecordPrimaryStoragePlaceType");
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_sdiz"), "RecordSDIZElevatorType");
	ДобавитьРеквизитыТипа(ПараметрыПреобразования, ТипПоля, Метаданные.Справочники.СДИЗЗЕРНО, "number");
	...
	ТипПоля = ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_dictionary"), "StringListType");
	РеквизитыТипа = Новый Структура();
...
Функция ПолучитьЗначениеИзДокументаDOMПоИмениПоля(ТекстВходящегоСообщенияSOAP, ИмяПоля, ИсходныйXML = Неопределено, ПервыйДочерний = Ложь) Экспорт
	...
	УзелDOM     = Неопределено;
	УзлыПоИмени = ДокументDOM.ПолучитьЭлементыПоИмени("*", ИмяПоля);
	...
	Если УзелDOM <> Неопределено Тогда
		...
		Если ПервыйДочерний Тогда
			УзелDOM = УзелDOM.ПервыйДочерний;
			Если УзелDOM = Неопределено Тогда
				Возврат Текст;
			КонецЕсли;
		КонецЕсли;
...
Функция ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(ОбъектXDTOИсточник, ОбъектXDTOПриемник, ПараметрыПреобразования) Экспорт
	...
	Для Каждого СвойствоИсточника Из ОбъектXDTOИсточник.Свойства() Цикл
		...
		ПространствоИменСвойстваПриемника = ПараметрыПреобразования.ПространстваИменПоИменамСвойств[ИмяСвойства];
...
Функция ОрганизацияКонтрагентXDTO(Реквизиты, СообщениеXML, ПредставлениеРевизита = "") Экспорт
	
	ПространствоИмен          = ПространствоИменПоИмениПакетаXDTO("zerno_organizations");
	...
	Если Реквизиты.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ЮридическоеЛицо Тогда
		...
		ЗаполнитьСвойствоXDTO(АдресРФXDTO, "PostalCode", Реквизиты.ЮридическийАдресИндекс, КешОшибок);
		ЗаполнитьСвойствоXDTO(АдресРФXDTO, "Address",    Реквизиты.ЮридическийАдрес,       КешОшибок);
	...
	ИначеЕсли Реквизиты.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ИностраннаяОрганизация
		...
		Если ЗначениеЗаполнено(Реквизиты.ИНН) Тогда
			...
			КонецЕсли;
			СтранаРегистрацииXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, "Country", ТипОрганизацииXDTO);
			ЗаполнитьСвойствоXDTO(СтранаРегистрацииXDTO, "alpha_3", Реквизиты.КодАльфа3,   КешОшибок);
		...
		Иначе // ИностраннаяОрганизацияБезРегистрацииВРФ
			...
			ЗаполнитьСвойствоXDTO(АдресXDTO, "Address", Реквизиты.ЮридическийАдресИндекс, КешОшибок);
...
Функция АдресРФXDTO(Адрес, Знач АдресСтрокой, СообщениеXML) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеАдреса.Комментарий) Тогда
		МестоположениеСКомменатрием = АдресСтрокой + ", " + ДанныеАдреса.Комментарий;
	Иначе
		МестоположениеСКомменатрием = АдресСтрокой;
	КонецЕсли;
	
	ЗаполнитьСвойствоXDTO(АдресРФXDTO, "PostalCode", ДанныеАдреса.Индекс, СообщениеXML);
	ЗаполнитьСвойствоXDTO(АдресРФXDTO, "Address", МестоположениеСКомменатрием, СообщениеXML);
	
	Возврат АдресРФXDTO;
	
КонецФункции

Функция АдресРФПоКлючуАдресовXDTO(АдресСтрокой, Индекс, СообщениеXML) Экспорт
...
Процедура ЗаполнитьСтрануНазначения(ОбъектXDTOРодитель, КодАльфа3, ПространствоИмен, СообщениеXML) Экспорт
	
	СтранаНазначенияXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(
		ПространствоИмен, "CountryDestination", ОбъектXDTOРодитель);
	ЗаполнитьСвойствоXDTO(
		СтранаНазначенияXDTO, "alpha_3", КодАльфа3, СообщениеXML);
	ЗаполнитьСвойствоXDTO(
		ОбъектXDTOРодитель, "CountryDestination", СтранаНазначенияXDTO, СообщениеXML);
...
Функция ПространствоИменПоИмениПакетаXDTO(ИмяПакета, Знач Версия = Неопределено) Экспорт
	
	Если Версия = Неопределено Тогда
		Версия = ИнтеграцияЗЕРНО.ВерсияAPI();
	КонецЕсли;
...
Процедура ЗапроситьРезультатОбработкиВСервисеТранзакция(ЭлементОчереди, ПараметрыОбмена)
	...
	Исключение
		...
		ПараметрыОшибки = ИнтеграцияЗЕРНОКлиентСервер.ПараметрыНумерованнойОшибки(КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура УдалитьСообщениеВиртуальнойОчереди(ЭлементОчередиДокумент, ПараметрыОбмена) Экспорт
	...
	Если ТипЗнч(ЭлементОчередиДокумент) <> Тип("СтрокаТаблицыЗначений") Тогда
		Для Каждого СтрокаТЧ Из ПараметрыОбмена.ОчередьСообщений Цикл
			Если СтрокаТЧ.СсылкаНаОбъект = ЭлементОчередиДокумент Тогда
	...
	Для Каждого Сообщение Из КоллекцияСообщенийXML Цикл
		...
		Если ПодготовленоКПередаче Тогда
			...
			Если ЗначениеЗаполнено(Сообщение.ПередаватьПосле) Тогда
				НоваяЗависимость = ПараметрыОбмена.ЗависимостиСообщений.Добавить();
				НоваяЗависимость.Сообщение1 = Сообщение.ПередаватьПосле;
				НоваяЗависимость.Сообщение2 = ИсходящееСообщение;
				НоваяЗависимость.Загрузка   = Ложь;
				НоваяЗависимость.Передача   = Истина;
			КонецЕсли;
