...
Функция ПлатежнаяСсылка(Знач ОснованиеПлатежа, ОтсутствуетДоступ = Ложь) Экспорт
	...
	ВходящиеПараметры.Вставить("КонтактныеДанныеЧека", Неопределено);
	...
	Возврат ДанныеСервиса.ПлатежнаяСсылка;
...
Функция ОбновитьАналитикиУчета(НастройкаОнлайнОплаты, АналитикиУчета, ЗамещатьЗаполненные  = Ложь) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ВызватьИсключение НСтр("ru='Ошибка обновления аналитик учета'");
...
Функция ДобавитьНастройку(УИД, ДанныеЗаполнения) Экспорт
	...
	АналитикиУчета = Новый Структура;
	АналитикиУчета.Вставить("ДополнительныеНастройки", Неопределено);
	
	ЗаполнитьЗначенияСвойств(АналитикиУчета, ДанныеЗаполнения);
	
	// Проверка ключей аналитики на допустимость переданных ключей.
	Если АналитикиУчета.ДополнительныеНастройки <> Неопределено
		И ТипЗнч(АналитикиУчета.ДополнительныеНастройки) = Тип("Структура") Тогда
		
		ПроверкаСтруктурыДополнительныхНастроек(АналитикиУчета.ДополнительныеНастройки, Результат);
...
// Возвращаемое значение:
//  Структура:
//    * ОнлайнОплатаДоступна - Булево
//    * ИспользоватьОнлайнОплаты - Неопределено - настройка подсистемы онлайн оплаты не доступна.
//                               - Булево
//    * НастройкаИнтеграцииДоступна - Неопределено - подсистема онлайн оплаты не доступна.
//                                  - Булево
//    * ЕстьПравоПросмотраСтатусовОнлайнОплат - Неопределено - настройка подсистемы онлайн оплаты не доступна.
//                                            - Булево
//    * ИспользоватьАвтоматическоеПолучениеУведомленияОтОнлайнОплат - Неопределено - настройка подсистемы онлайн оплаты
//      не доступна.
//                                                                  - Булево
//    * ПредставлениеРасписания - Неопределено - настройка подсистемы онлайн оплаты не доступна.
//                              - Строка
//
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы() Экспорт
	
	ОнлайнОплатаДоступна = ПравоДоступа("Просмотр", Метаданные.Справочники.НастройкиОнлайнОплат);
	
	Результат = Новый Структура();
	Результат.Вставить("ОнлайнОплатаДоступна"                                       , ОнлайнОплатаДоступна);
	Результат.Вставить("ИспользоватьОнлайнОплаты"                                   , Неопределено);
	Результат.Вставить("НастройкаИнтеграцииДоступна"                                , Неопределено);
	Результат.Вставить("ЕстьПравоПросмотраСтатусовОнлайнОплат"                      , Неопределено);
	Результат.Вставить("ИспользоватьАвтоматическоеПолучениеУведомленияОтОнлайнОплат", Неопределено);
	Результат.Вставить("ПредставлениеРасписания"                                    , Неопределено);
	
	Если Не ОнлайнОплатаДоступна Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ИспользоватьОнлайнОплаты              = Константы.ИспользоватьОнлайнОплаты.Получить();
	Результат.НастройкаИнтеграцииДоступна           = НастройкаИнтеграцииДоступна();
	Результат.ЕстьПравоПросмотраСтатусовОнлайнОплат =
		ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СтатусыОнлайнОплат);
	
	Если Не Результат.НастройкаИнтеграцииДоступна
			И Не Результат.ЕстьПравоПросмотраСтатусовОнлайнОплат Тогда
		Возврат Результат;
	ИначеЕсли Не Результат.НастройкаИнтеграцииДоступна Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		ПараметрыЗадания.Вставить("ИмяМетода", ПараметрыЗадания.Метаданные.ИмяМетода);
	КонецЕсли;
	...
	Если СписокЗаданий.Количество() Тогда
		Результат.ИспользоватьАвтоматическоеПолучениеУведомленияОтОнлайнОплат = СписокЗаданий[0].Использование;
		Результат.ПредставлениеРасписания = ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРасписания(
			СписокЗаданий[0].Расписание);
	КонецЕсли;
