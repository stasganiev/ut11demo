...
Процедура ПроверитьЗаполнениеОплатыПлатежнымиКартами(Объект, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ОтчетОРозничныхВозвратах") 
		...
		Если Объект.ОплатаПлатежнымиКартами.Количество() > 0
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Объект, "ОплатаПлатежнымиКартами", , Отказ);
	...
	Для Каждого СтрокаТЧ Из Объект.ОплатаПлатежнымиКартами Цикл
		...
		Если (ТипЗнч(Объект) = Тип("ДокументОбъект.ОтчетОРозничныхВозвратах")
			...
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				Объект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОплатаПлатежнымиКартами", СтрокаТЧ.НомерСтроки, "ВидОплаты"),
				,
				Отказ);
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ЭквайринговыйТерминал)
			...
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				Объект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОплатаПлатежнымиКартами", СтрокаТЧ.НомерСтроки, "ЭквайринговыйТерминал"),
				,
				Отказ);
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМ") 
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.ДоговорПодключения)
				...
				ОбщегоНазначения.СообщитьПользователю(
					Текст,
					Объект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОплатаПлатежнымиКартами", СтрокаТЧ.НомерСтроки, "ДоговорПодключения"),
					,
					Отказ);
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|	ЧекККМТовары.ДокументРеализации КАК ДокументРеализации
		|	
		...
		ТаблицаИндексовПоАналитике = МассивРезультатов[2].Выгрузить();
		ОбщегоНазначенияУТ.ПронумероватьТаблицуЗначений(ТаблицаИндексовПоАналитике,"Индекс");
		
		ТаблицаИндексовПоНоменклатуре = МассивРезультатов[3].Выгрузить();
		...
		Если Не ТолькоЗаполнить Тогда
			...
			Иначе
				ОписаниеОшибки = НСтр("ru = 'При проверке заполнения перед проведением документа %Документ% произошла ошибка. Документ записан, но не проведен.'");
...
Функция ЗаполнитьОтчетОРозничныхВозвратах(ОтчетОРозничныхВозвратахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|	Серии.ДокументРеализации 			 КАК ДокументРеализации
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЧекККМВозвратСерии.Помещение,
		|		ЧекККМВозвратСерии.Серия,
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных возвратах произошла ошибка.
		                            |Дополнительное описание:
		                            |%ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ИнформацияОбОшибке().Описание);
...
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Запрос.Текст = "
		...
		|	ЧекККМТовары.Ссылка.Организация,
		|	ЧекККМТовары.Ссылка.НалогообложениеНДС,
		|	ЕСТЬNULL(ОтчетОРозничныхПродажах.Ссылка, НЕОПРЕДЕЛЕНО)
		|ИЗ
		|	Документ.ЧекККМКоррекции.Товары КАК ЧекККМТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
		|		ПО ЧекККМТовары.Ссылка.ЧекККМ.КассоваяСмена = ОтчетОРозничныхПродажах.КассоваяСмена
		|			И ЧекККМТовары.Ссылка.ЧекККМ.НалогообложениеНДС = ОтчетОРозничныхПродажах.НалогообложениеНДС
		|ГДЕ
		|	ЧекККМТовары.Ссылка.КассоваяСмена = &КассоваяСмена
		|	И (ЧекККМТовары.Ссылка.Проведен
		|		ИЛИ &ВключатьНепроведенныеЧеки)
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента)
		...
		|	ЧекККМТовары.Ссылка.Организация,
		|	ЧекККМТовары.Ссылка.НалогообложениеНДС,
		|	ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПродажи),
		|	ЕСТЬNULL(ОтчетОРозничныхПродажах.Ссылка, НЕОПРЕДЕЛЕНО)
		|ИЗ
		|	Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТовары
		|		ПО (ЧекККМКоррекции.ЧекККМ = ЧекККМТовары.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
		|		ПО ЧекККМТовары.Ссылка.КассоваяСмена = ОтчетОРозничныхПродажах.КассоваяСмена
		|ГДЕ
		|	ЧекККМТовары.Ссылка.КассоваяСмена <> &КассоваяСмена
		|	И ЧекККМТовары.Ссылка.НалогообложениеНДС = ОтчетОРозничныхПродажах.НалогообложениеНДС
		|	И ЧекККМКоррекции.КассоваяСмена = &КассоваяСмена
		|	И (ЧекККМКоррекции.Проведен
		|		ИЛИ &ВключатьНепроведенныеЧеки)
		|	И ЧекККМКоррекции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
...
Функция ВыполнитьАвтоматическуюИнкассациюДС(КассаККМ, ОшибкаПриСнятииZОтчета)
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РозничныеПродажи.Установить_ПечататьБумажныйЧек";
	Обработчик.Версия = "11.5.14.10";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f6f68d81-0690-8403-83d9-28af33fb35ce");
	
	ШаблонКомментария = НСтр("ru = 'Установка константы ""%1"". Если включена функциональная опция ""%2"", то значение константы влияет на вид чека, который будет устанавливается по умолчанию во всех фискальных формах.'");
	Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, Метаданные.Константы.ПечататьБумажныйЧек.Имя
														, Метаданные.Константы.ИспользоватьРозничныеПродажи.Синоним);
	Читаемые = Новый Массив;
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Константы.ПечататьБумажныйЧек.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Константы.ПечататьБумажныйЧек.ПолноеИмя());
	Обработчик.БлокируемыеОбъекты = СтрСоединить(Блокируемые, ",");
	
	РозничныеПродажиЛокализация.ПриДобавленииОбработчиковОбновления(Обработчики);
КонецПроцедуры

Процедура Установить_ПечататьБумажныйЧек() Экспорт
	
	МенеджерЗначения = Константы.ПечататьБумажныйЧек.СоздатьМенеджерЗначения();
