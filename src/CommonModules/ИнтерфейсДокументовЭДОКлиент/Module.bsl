...
Процедура СформироватьПодписатьОтправитьЭД_Договор(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрыПросмотра.ПредварительныйПросмотр = Неопределено
		Или ПараметрыПросмотра.ПредварительныйПросмотр.РезультатПодготовкиДанных = Неопределено Тогда
		
		СформироватьПодписатьОтправитьЭДЗавершить(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрКоманды),
			Новый Структура);
		Возврат;
	...
	Если РезультатПодготовкиДанных.Ошибка Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("СформироватьПодписатьОтправитьЭДЗавершить", ЭтотОбъект,
			Новый Структура);
		...
		Параметры.СписокДокументовКОтправке = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрКоманды);
		ПоказатьПроблемыПриОбработкеДокументов(ОписаниеОповещения, Параметры);
		Возврат;
	КонецЕсли;
	
	НастройкиФормирования = ПараметрыПросмотра.ПредварительныйПросмотр.РезультатПоискаНастроек.Настройки;
	
	Если НастройкиФормирования.Направление <> ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Внутренний") Тогда
	
		Если НастройкиФормирования.НастройкиОтправки.Формат = "ON_DOGDOC_1_999_01_01_01_01" Тогда
			...
			ОбработчикЗавершения = Новый ОписаниеОповещения("ЗапроситьФайлЗавершение", ФормаДоговорныйДокумент101Клиент,
				 ПараметрКоманды);
			
			ПараметрыПодготовки = ФормаДоговорныйДокумент101Клиент.ПараметрыПодготовкиФайла();
			ПараметрыПодготовки.ОбработчикЗавершения = ОбработчикЗавершения;
			ПараметрыПодготовки.Данные = ПараметрКоманды;
			
			ФормаДоговорныйДокумент101Клиент.ЗапроситьИПодготовитьФайл(ПараметрыПодготовки);
			
			Возврат;
			
		ИначеЕсли НастройкиФормирования.НастройкиОтправки.Формат = "ON_SODSD_1_999_02_01_01_01" Тогда
			
			ФормаДоговорныйДокументXMLКлиент = ФормаДоговорныйДокументXMLКлиент();
			ОбработчикЗавершения = Новый ОписаниеОповещения("ПродолжитьФормированиеЭлектронногоДокументаДляОтправки",
				ФормаДоговорныйДокументXMLКлиент, ПараметрКоманды);
				
			ПараметрыПодготовки = ФормаДоговорныйДокументXMLКлиент.ПараметрыПодготовкиФайла();
			ПараметрыПодготовки.ОбработчикЗавершения = ОбработчикЗавершения;
			ПараметрыПодготовки.Данные = ПараметрКоманды;
			ПараметрыПодготовки.ИдентификаторОсновногоФайла = РезультатПодготовкиДанных.ИдентификаторОсновногоФайла;
...
Процедура ОтправитьПолучитьДокументыЭДО(Форма, ОповещениеОЗавершении = Неопределено) Экспорт
	
	ПараметрыОтправкиПолучения = ИнтерфейсДокументовЭДОВызовСервера.ПараметрыОтправкиПолученияДокументовЭДО();
	
	Если ОповещениеОЗавершении = Неопределено Тогда
		ОповещениеОЗавершении = Новый ОписаниеОповещения;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыОтправкиПолучения.УчетныеЗаписиЭДОБезПринятыхУсловийСервиса) Тогда
		
		НачатьОтправкуПолучениеДокументовЭДО(ОповещениеОЗавершении,
			ПараметрыОтправкиПолучения.ПараметрыВыполненияДействий);
		
	Иначе
		
		КонтекстПринятияУсловий = Новый Структура;
		КонтекстПринятияУсловий.Вставить("ОповещениеОЗавершении", ОповещениеОЗавершении);
		КонтекстПринятияУсловий.Вставить("ПараметрыВыполненияДействий",
			ПараметрыОтправкиПолучения.ПараметрыВыполненияДействий);
...
Функция ФормаДоговорныйДокумент101Клиент() Экспорт
	...
	Если Форма = Неопределено Тогда
		Форма = ПолучитьФорму("Обработка.ФорматДоговорныйДокумент101.Форма.ВыполнениеКлиентскихМетодов");
...
//
Функция ФормаДоговорныйДокументXMLКлиент() Экспорт
	...
	Если Форма = Неопределено Тогда
		Форма = ПолучитьФорму("Обработка.ФорматДоговорныйДокументИнформацияОтправителяЭДО.Форма.ВыполнениеКлиентскихМетодов");
		ПараметрыПриложения[ИмяПараметра].Вставить("ФормаВыполнениеКлиентскихМетодов", Форма);
	КонецЕсли;
...
// Параметры:
//  Оповещение - ОписаниеОповещения - оповещение, которое будет вызвано по завершению получения проштампованного файла
//  ДлительнаяОперация - см. ДлительныеОперации.ВыполнитьФункцию
Процедура НачатьПолучениеПроштампованногоФайлаPDF(Оповещение, ДлительнаяОперация) Экспорт
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
	ПараметрыОжидания.ВыводитьСообщения = Ложь;
...
Процедура ОткрытьЭлектронныйДокументПоПараметрамПросмотра(ПараметрыПросмотра)
	...
	Если ПредварительныйПросмотр.РезультатПодготовкиДанныхОблачногоЭДО <> Неопределено Тогда
		ОбработатьРезультатПодготовкиДанныхПросмотраОблачногоЭДО(
			ПредварительныйПросмотр.РезультатПодготовкиДанныхОблачногоЭДО);
		Возврат;
	КонецЕсли;
...
// 
// Параметры:
//  Результат - Неопределено, см. ИнтерфейсДокументовЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
//  Контекст - Структура:
...
Процедура ОткрытьНовыйЭлектронныйДокументПоПодготовленнымДанным(ОбъектУчета, НастройкиФормирования, РезультатПодготовкиДанных)
	...
	Если НастройкиФормирования.Направление <> ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Внутренний") Тогда
		...
		Если НастройкиФормирования.НастройкиОтправки.Формат = "ON_DOGDOC_1_999_01_01_01_01" Тогда
			...
			ФормаДоговорныйДокумент101Клиент = ФормаДоговорныйДокумент101Клиент();
			ОбработчикЗавершения = Новый ОписаниеОповещения("ЗагрузитьИзФайлаЗавершение", ФормаДоговорныйДокумент101Клиент,
				ПараметрыФормы);
				
			ПараметрыПодготовки = ФормаДоговорныйДокумент101Клиент.ПараметрыПодготовкиФайла();
			ПараметрыПодготовки.ОбработчикЗавершения = ОбработчикЗавершения;
			ПараметрыПодготовки.Данные = ПараметрыФормы;
		...
		ИначеЕсли НастройкиФормирования.НастройкиОтправки.Формат = "ON_SODSD_1_999_02_01_01_01" Тогда
			...
			ФормаДоговорныйДокументXMLКлиент = ФормаДоговорныйДокументXMLКлиент();
			ОбработчикЗавершения = Новый ОписаниеОповещения("ПродолжитьФормированиеЭлектронногоДокумента",
				ФормаДоговорныйДокументXMLКлиент, ПараметрыФормы);
...
//  * АдресДанных - Строка
Процедура ОбработатьРезультатПодготовкиДанныхПросмотраОблачногоЭДО(РезультатПодготовкиДанных)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЭтоОблачныйЭДО", Истина);
	ПараметрыФормы.Вставить("ПредварительныйПросмотр", Новый Структура);
	ПараметрыФормы.ПредварительныйПросмотр.Вставить("АдресДанных", РезультатПодготовкиДанных.АдресДанных);
	ОткрытьФормуЭлектронногоДокумента(ПараметрыФормы);
...
//  КонтекстДействийПоЭДО - см. НовыйКонтекстДействийПоЭДО
//
Процедура ВыполнитьДействияПоЭДО(КонтекстДействийПоЭДО)
...
Процедура ОтправитьПолучитьДокументыЭДОПослеПринятияУсловийСервиса(УсловияПриняты, КонтекстПринятияУсловий) Экспорт
	...
	НачатьОтправкуПолучениеДокументовЭДО(КонтекстПринятияУсловий.ОповещениеОЗавершении,
		КонтекстПринятияУсловий.ПараметрыВыполненияДействий);
...
//
Процедура СформироватьИДобавитьПроизвольныйДокументВПакетЗавершение(ДлительнаяОперация, Контекст) Экспорт
	...
	Результат = ПолучитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата); // См. ИнтеграцияОблачногоЭДО.СформироватьИДобавитьПроизвольныйДокументВПакет
	
	Если ЗначениеЗаполнено(Результат.ЭлектронныйДокумент) Тогда
		Оповестить(ИмяСобытияОбновленияСостоянияЭДО(), Результат.ЭлектронныйДокумент);
	КонецЕсли;
...
//  ДокументКУдалениюИзПакета - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Процедура УдалитьДокументИзПакета(ИдентификаторПакета, ДокументКУдалениюИзПакета)
	...
	Иначе		
		Оповестить("УдалениеДокументаИзПакета", ДокументКУдалениюИзПакета, ИдентификаторПакета);	
	КонецЕсли;
...
// Параметры:
//  ДлительнаяОперация - см. ДлительныеОперации.ВыполнитьФункцию
//  КонтекстУдаленияИзПакета - Структура:
//  * ИдентификаторПакета - Строка
//  * ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Процедура УдалитьДокументИзПакетаПослеОбработкиВОблачномЭДО(ДлительнаяОперация, КонтекстУдаленияИзПакета) Экспорт
	...
	Результат = ПолучитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата); // см. ИнтеграцияОблачногоЭДО.УдалитьДокументИзПакета
	
	Если Результат.Отказ Тогда
		ПоказатьПредставлениеОшибокКонтекстаДиагностики(Результат.КонтекстДиагностики);
		Возврат;
	КонецЕсли;
	
	УдалитьДокументИзПакета(КонтекстУдаленияИзПакета.ИдентификаторПакета, КонтекстУдаленияИзПакета.ЭлектронныйДокумент);
