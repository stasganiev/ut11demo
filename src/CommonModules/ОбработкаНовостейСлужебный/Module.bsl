...
////////////////////////////////////////////////////////////////////////////////
// Подсистема "Новости".
// ОбщийМодуль.ОбработкаНовостейСлужебный.
//
////////////////////////////////////////////////////////////////////////////////
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
		...
		Обработчик.Процедура           = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск";
	...
	Обработчик = Обработчики.Добавить();
		Обработчик.Версия              = ""; // Только для начального заполнения.
		Обработчик.Процедура           = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск";
	...
	Обработчик = Обработчики.Добавить();
		Обработчик.Версия              = "*";
		Обработчик.Процедура           = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию";
		Обработчик.ОбщиеДанные         = Истина;
	...
	// При обновлении в модели сервиса нельзя использовать комбинацию "ОбщиеДанные = Ложь" и "Версия = *".
	// Для запуска обработчика обновления для всех версий, для разделенных данных надо запустить специальный обработчик
	//  с настройками "ОбщиеДанные = Истина" и "УправлениеОбработчиками = Истина", который уже будет
	//  возвращать список обработчиков для области данных.
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
			Обработчик.Версия                  = "*";
			Обработчик.Процедура               = "ОбработкаНовостейСлужебный.ЗаполнитьОбработчикиРазделенныхДанных";
			Обработчик.ОбщиеДанные             = Истина;
			...
			Обработчик.РежимВыполнения         = "Оперативно";
	Иначе
		// При обновлении в "коробке" вызываем обработчик напрямую.
		Обработчик = Обработчики.Добавить();
			Обработчик.Версия              = "*";
			Обработчик.Процедура           = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию";
			Обработчик.ОбщиеДанные         = Ложь;
			Обработчик.НачальноеЗаполнение = Ложь;
			Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
			Обработчик.РежимВыполнения     = "Оперативно";
...
//     * РазделенныеОбработчики - ТаблицаЗначений, Неопределено - см. ОбновлениеИнформационнойБазы.НоваяТаблицаОбработчиковОбновления.
//       В случае прямого вызова (не через механизм обновления версии ИБ) передается Неопределено.
// 
...
#Область МинимизацияСерверныхВызовов

// Для минимизации серверных вызовов надо заполнять часто используемые параметры в этом методе,
//  чтобы потом получать их через
//  СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ИнтернетПоддержкаПользователей.Новости.
// Не использовать этот метод для вызова из "ПриНачалеРаботыСистемы" или "ПослеНачалаРаботыСистемы ".
// См. процедуру ИнтеграцияПодсистемБИП.ПриДобавленииПараметровРаботыКлиента.
//
// Параметры:
//  ПараметрыИПП - Структура - произвольные параметры.
//
Процедура ПриДобавленииПараметровРаботыКлиента(ПараметрыИПП) Экспорт

	ПараметрыНовости = Новый Структура;
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостями"                        , ОбработкаНовостей.РазрешенаРаботаСНовостями());
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостямиЧерезИнтернет"           , ОбработкаНовостей.РазрешенаРаботаСНовостямиЧерезИнтернет());
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостямиТекущемуПользователю"    , ОбработкаНовостей.РазрешенаРаботаСНовостямиТекущемуПользователю());
		ПараметрыНовости.Вставить("НастройкиПоказаНовостейНастроенныеАдминистратором", ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором());

	ПараметрыИПП.Вставить("Новости", ПараметрыНовости);

КонецПроцедуры

// Для минимизации серверных вызовов надо заполнять часто используемые параметры в этом методе,
//  чтобы потом получать их через
//  СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске().ИнтернетПоддержкаПользователей.Новости.
// Не использовать этот метод для вызова из "ПриНачалеРаботыСистемы" или "ПослеНачалаРаботыСистемы ".
// См. процедуру ИнтеграцияПодсистемБИП.ПриДобавленииПараметровРаботыКлиентаПриЗапуске.
//
// Параметры:
//  ПараметрыИПП - Структура - произвольные параметры.
//
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(ПараметрыИПП) Экспорт
...
// Процедура вызывается из подписки на событие - см. ИнтеграцияПодсистемБИП.ПередЗапускомПрограммы.
// Раньше вызывалось из ОбработкаНовостейКлиент.ПриНачалеРаботыСистемы.
//
Процедура ПередЗапускомПрограммы() Экспорт

	// Если это запуск из регламентного задания, то не выполнять метод.
	...
	Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиТекущемуПользователю() Тогда // На текущий момент еще не установлен СтатусПодсистемы.
		Если НЕ ОбщегоНазначения.РазделениеВключено() Тогда
			...
			ПроверитьИзменениеВерсииПлатформы(); // Мы НЕ в привилегированном режиме
		КонецЕсли;
	КонецЕсли;
	...
	//  Индикатором, что базу долго не запускали может служить значение даты для операции "БИП:Новости/ОбновлениеПериодическихСвойств"
	//   из регистра сведений ОперацииРегламентныхЗаданий*.
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая()
			И ОбработкаНовостей.РазрешенаРаботаСНовостями() Тогда
		ПятьДнейНазад = ТекущаяУниверсальнаяДата() - 5*24*60*60; // 5 суток назад.
		ДатаПоследнегоОбновления = РасписанияРегламентныхЗаданий.ДатаСледующегоВыполнения("БИП:Новости/ОбновлениеПериодическихСвойств");

		Если ДатаПоследнегоОбновления <= ПятьДнейНазад Тогда
			УстановитьСостояниеПодсистемы("ТребуетсяПересчетОтборовПослеДлительногоОжидания");
			Если ДатаПоследнегоОбновления = '00010101' Тогда
				ТекстСообщения = НСтр("ru='ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: пересчет периодических реквизитов не осуществлялся ни разу.
			...
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: прошло длительное время после последнего пересчета периодических реквизитов (%1).
						|Подсистема Новости переведена из состояния [Активна] в состояние [ТребуетсяПересчетОтборовПослеДлительногоОжидания].
						|Это не является ошибкой. Но важные и очень важные новости не будут всплывать, т.к. возможно, что они уже просрочены.
						|В самое ближайшее время будет произведен автоматический пересчет отборов и подсистема вернется в состояние [Активна].'"),
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				...
				НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеПодсистемы'", КодОсновногоЯзыка), // ИдентификаторШага.
	...
	ОбработкаНовостей.ЭтоВнешнийПользовательСВозможностьюЧтениеНовостей();
...
Процедура ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке(Типы) Экспорт
	...
	// Из всех справочников подсистемы Новости может выгружаться только Справочник.Новости
	//  и Справочник.ЛентыНовостей (т.к. является частью естественного ключа для "Новости").
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	// Для ссылочных типов (справочники, документы, бизнес-процессы, задачи, планы счетов, планы обмена,
	//  планы видов характеристик, планы видов расчета) для дополнения массива необходимо
	//  использовать метод "ВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки".
	// Он находится в подсистеме ТехнологияСервиса.ВыгрузкаЗагрузкаДанных.
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ВыгрузкаЗагрузкаДанных") Тогда
		...
		МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(
			Типы,
			Метаданные.Справочники.ЗначенияКатегорийНовостей,
			МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеИзменять());
		МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(
			Типы,
			Метаданные.Справочники.ЛентыНовостей,
			МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеИзменять());
		МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(
			Типы,
			Метаданные.Справочники.Новости,
			МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеИзменять());
		МодульВыгрузкаЗагрузкаДанных.ДополнитьТипомИсключаемымИзВыгрузкиЗагрузки(
			Типы,
			Метаданные.ПланыВидовХарактеристик.КатегорииНовостей,
			МодульВыгрузкаЗагрузкаДанных.ДействиеСоСсылкамиНеИзменять());
	Иначе
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ПараметрыОкруженияБИП_Новости", "ОбработкаНовостейСлужебный.УстановкаПараметровСеанса");
...
//  вызов ОбработкаНовостейСлужебный.ПриОпределенииНазначенияРолей(НазначениеРолей).
//
// Параметры:
...
Процедура УстановкаПараметровСеанса(ИмяПараметра = Неопределено, УстановленныеПараметры = Неопределено) Экспорт
	...
	Результат = Новый Структура;
		...
		ИнтеграцияПодсистемБИП.ЗначениеПредопределеннойКатегории("ProductName", ИмяКонфигурации); // Обязательно изменить имя конфигурации.
...
#Область СтандартныеПодсистемы_РаботаВМоделиСервиса_ОчередьЗаданий

// См. ОчередьЗаданийПереопределяемый.ПриПолученииСпискаШаблонов.
Процедура ПриПолученииСпискаШаблонов(Шаблоны) Экспорт

	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных.Имя);
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных.ИмяМетода);
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	НоваяСтрока.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных;
	НоваяСтрока.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.РазрешенаРаботаСНовостями;
...
// Получение настроек чтения новостей, настроенные администратором.
//
// Параметры:
//  ДляРедактирования - Булево - если ИСТИНА, то в значении ключа "СписокНовостей_ИнтервалАвтообновления" может быть значение 0.
//
// Возвращаемое значение:
//  Произвольное.
//
Функция ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором(ДляРедактирования = Ложь) Экспорт

	// Вне зависимости от того, есть роли "ЧтениеНовостей" / "ПолныеПрава" или нет, административные настройки надо прочитать.
	УстановитьПривилегированныйРежим(Истина);

	Если ОбщегоНазначения.РазделениеВключено()
			И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		// Программа в разделенном режиме.
		Если ДляРедактирования = Истина Тогда
		...
		Иначе
			Результат = ХранилищаНастроек.НастройкиНовостей.Загрузить(
				"НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных", // Идентификатор.
				"");
		КонецЕсли;
	Иначе
		Результат = ХранилищаНастроек.НастройкиНовостей.Загрузить(
...
#Область РегламентныеЗаданияНовостей
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеПериодическихСвойствНовостей"; // Идентификатор.
...
#Область Проверки
	...
	Если НЕ Проверка_РазрешенаРаботаСНовостями(КонтекстВыполнения) Тогда
		Возврат;
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Обновление периодических свойств новостей'")); // Идентификатор.
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, ОписаниеРезультата);

#Область Обработка_ОбновлениеРасписания

	// Сохранить дату последнего выполнения / обновления.
	НачатьШагВыполнения(КонтекстВыполнения, "ОбновлениеРасписания"); // Идентификатор.
	Операция = "БИП:Новости/ОбновлениеПериодическихСвойств"; // Идентификатор операции расписания.
	ПараметрыВыполненияОперации = ПараметрыВыполненияОперацииРегламентныхЗаданий(Операция);
	РасписанияРегламентныхЗаданий.ОперацияВыполнена(Операция, ПараметрыВыполненияОперации);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Установка времени следующего запуска'"));
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеПривязокКМетаданным"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Обновление привязок к метаданным'")); // Идентификатор.
...
#Область Обработка_ОбновлениеРасписания
	...
	// Сохранить дату последнего выполнения / обновления.
	НачатьШагВыполнения(КонтекстВыполнения, "ОбновлениеРасписания"); // Идентификатор.
	Операция = "БИП:Новости/ОбновлениеПривязокКМетаданным"; // Идентификатор операции расписания.
	ПараметрыВыполненияОперации = ПараметрыВыполненияОперацииРегламентныхЗаданий(Операция);
	РасписанияРегламентныхЗаданий.ОперацияВыполнена(Операция, ПараметрыВыполненияОперации);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Установка времени следующего запуска'"));
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОптимизироватьОтборыПоНовостям"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ВыполнениеЗапроса

	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/УдалениеОтборовПоНесуществующимКатегориям"); // Идентификатор.
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных"); // Идентификатор.
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Пользовательские
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_Пользовательские"); // Идентификатор.
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Общие
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/УдалениеОтборовПоНесуществующимКатегориям_Общие"); // Идентификатор.
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:УдалениеЗначенийОтборов_ДляОбластиДанных"); // Идентификатор.
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'ОптимизироватьОтборыПоНовостямДляОбластиДанных - отключено'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:УдалениеЗначенийОтборов_Пользовательские"); // Идентификатор.
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'ОптимизироватьОтборыПоНовостямПользовательские - отключено'"));
...
#Область Оптимизация_ЛишниеКатегорииВНовостях
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:Оптимизация_ЛишниеКатегорииВНовостях"); // Идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	КонтекстВыполненияЭталон = НовыйКонтекстВыполнения(НаименованиеМетода, Истина);
	КонтекстВыполненияЭталон.ВходящиеПараметры.Вставить("УстановитьПривилегированныйРежим", Ложь);
	ЗаполнитьЗначенияСвойств(КонтекстВыполненияЭталон.ВходящиеПараметры, КонтекстВыполнения.ВходящиеПараметры);
	Если КонтекстВыполненияЭталон.ВходящиеПараметры.УстановитьПривилегированныйРежим = Истина Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
...
#Область Проверки
	...
	Если НЕ Проверка_ДоступныНеразделенныеДанные(КонтекстВыполнения) Тогда
		Возврат;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПраваДоступа"); // Идентификатор.
	Если (НЕ ПривилегированныйРежим())
			И (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся) <> Истина) Тогда
		// Метод может вызываться после изменения версии платформы, когда первый вход в базу
		//  совершил пользователь без права записи в этот регистр сведений.
		// Метод ПроверитьИзменениеВерсииПлатформы всегда переходит в ПривилегированныйРежим.
		ТекстСообщения = НСтр("ru='Отсутствует право на изменение для РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.'");
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.ACCESS_DENIED/FILTERS_RECALC", ТекстСообщения);
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Ошибка; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:КатегорииЗагружены"); // Идентификатор.
	...
	Если (КатегорияВерсияПродукта.Пустая())
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CATEGORIES_NOT_FOUND/FILTERS_RECALC", ТекстСообщения);
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Ошибка; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат;
	КонецЕсли;
...
#Область СохранениеВерсииПлатформы
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:СохранениеВерсииПлатформы"); // Идентификатор.
...
#Область ОсновныеДействия
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/Пересчет"); // Идентификатор.
	...
	Запрос.Текст = "
		...
		|// Осуществляется поиск хотя бы одного подходящего интервала.
		|ВЫБРАТЬ
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса/Пересчет"); // Идентификатор.
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Запись.Записать(Истина);
		КонецЦикла;
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru = 'Редко-меняющиеся отборы обновлены'"));
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru = 'Запрос пустой'"));
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Общие"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнениеЗначенийАвтоматическихОтборов

	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"ЗаполнитьЗначенияАвтоматическихОтборов_Общие",
		НаименованиеМетода + "/ЗаполнитьЗначенияАвтоматическихОтборов_Общие");

		ЗаполнитьЗначенияАвтоматическихОтборов_Общие(КонтекстВыполненияВложенный); // Если недоступны разделенные данные, то ничего не сделает.

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, "", КонтекстВыполненияВложенный);

#КонецОбласти

#Область Обработка_ПересчетОтборов

#Область Обработка_ПересчетОтборов_ВыполнениеЗапроса
	...
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "
		|// {Доступные категории и Исключение лент новостей без категорий. Начало} // ПересчитатьОтборыПоНовостям_Общие
		...
		|;
		|// {Доступные категории и Исключение лент новостей без категорий. Конец}
		|
		...
		|
		|";
	Запрос.УстановитьПараметр("КатегорияПродукт",         М.КатегорияПродукт());
	Запрос.УстановитьПараметр("КатегорияВерсияПродукта",  М.КатегорияВерсияПродукта());
	Запрос.УстановитьПараметр("КатегорияВерсияПлатформы", М.КатегорияВерсияПлатформы());

	РезультатЗапроса = Запрос.Выполнить(); // ПересчитатьОтборыПоНовостям_Общие() // АПК:1328 Конкурентная работа маловероятна.

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
...
#Область Обработка_ПересчетОтборов_ЗаполнениеДанными
...
#Область Обработка_ПересчетПользовательскихОтборов

	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"ПересчитатьОтборыПоНовостям_Пользовательские",
		НаименованиеМетода + "/ПересчитатьОтборыПоНовостям_Пользовательские");
	...
	Если ПересчитатьПоТекущемуПользователю = Истина Тогда
		...
		КоличествоОтборов = КоличествоНастроенныхОтборов(ТекущийПользователь);
		Если (КоличествоОтборов.КоличествоПользовательскихОтборов > 0) Тогда
			// Пересчитать отборы по всем пользователям.
			ПересчитатьОтборыПоНовостям_Пользовательские(ТекущийПользователь, КонтекстВыполненияВложенный);
		КонецЕсли;
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пересчет пользовательских отборов (%1 штук) (по текущему пользователю (%2)).'"),
				КоличествоОтборов,
				ТекущийПользователь),
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
			НСтр("ru = 'Пересчет пользовательских отборов (по всем пользователям)'"),
			КонтекстВыполненияВложенный);
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_ДляОбластиДанных"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ОсновныеДействия_ЗаполнениеЗначенийАвтоматическихОтборов
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		...
		НаименованиеМетода + "/ЗаполнитьЗначенияАвтоматическихОтборов_ДляОбластиДанных");

		ЗаполнитьЗначенияАвтоматическихОтборов_ДляОбластиДанных(КонтекстВыполненияВложенный); // Если недоступны разделенные данные, то ничего не сделает.

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, "", КонтекстВыполненияВложенный);

#КонецОбласти

#Область ОсновныеДействия_ПересчетОтборов

#Область ОсновныеДействия_ПересчетОтборов_ВыполнениеЗапроса
	...
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "
		|// {Доступные категории и Исключение лент новостей без категорий. Начало} // ПересчитатьОтборыПоНовостям_ДляОбластиДанных
		|// Из всех проверок необходимо исключить категории:
		|//  КатегорияПродукт, КатегорияВерсияПродукта, КатегорияВерсияПлатформы,
		|//  т.к. по ним существует отдельный расчет отборов, который хранится
		|//  в регистре сведений РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Результат запроса: %1.
				|ИспользоватьНомерОбластиДанных: %2'"),
			?(РезультатЗапроса.Пустой(),
				НСтр("ru='пустой'"),
				НСтр("ru='есть данные'")),
			ИспользоватьНомерОбластиДанных));

#КонецОбласти

#Область ОсновныеДействия_ПересчетОтборов_ЗаполнениеДанными
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Пользовательские"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучениеСпискаПользователей

	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса/ПолучениеСпискаПользователей"); // Идентификатор.
...
#Область Обработка_ОчисткаПредыдущегоРезультата

	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОчисткаПредыдущегоРезультата"); // Идентификатор.
...
#Область Обработка_ФормированиеСпискаНовостейИсключений

	// 3. Сформировать список новостей-исключений.
	Если ПропуститьПересчет <> Истина Тогда

#Область Обработка_ФормированиеСпискаНовостейИсключений_ВыполнениеЗапроса
		...
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		Запрос.Текст = "
			|// {Доступные категории и Исключение лент новостей без категорий. Начало} // ПересчитатьОтборыПоНовостям_Пользовательские
			|// Из всех проверок необходимо исключить категории:
			|//  КатегорияПродукт, КатегорияВерсияПродукта, КатегорияВерсияПлатформы,
			|//  т.к. по ним существует отдельный расчет отборов, который хранится
			|//  в регистре сведений РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.
			...
			|//  и ЗначенияКатегорийПользовательские, т.е. отсутствует метод ЗаполнитьЗначенияАвтоматическихОтборов_Пользовательские,
			|//  т.к. его вызов привел бы к дополнительным существенным затратам времени и дисковым операциям.
...
#Область Обработка_ФормированиеСпискаНовостейИсключений_РаботаСДаннымиЗапроса
...
#КонецОбласти
	...
	КонецЕсли; // ПропуститьПересчет <> Истина
...
Процедура ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения, ПринудительнаяЗапись = Ложь) Экспорт
	...
	// Завершить текущий шаг (если его не завершили принудительно с помощью "ЗавершитьШагВыполнения").
	ТипСтруктура = Тип("Структура");

	ТекущийШагВыполнения = ОбработкаНовостейКлиентСервер.НовыйОписательШагаВыполнения();
	Если ТипЗнч(КонтекстВыполнения.Вспомогательное.ТекущийШагВыполнения) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(ТекущийШагВыполнения, КонтекстВыполнения.Вспомогательное.ТекущийШагВыполнения, "ИдентификаторШага, ВремяНачала");
	...
	// Если не надо ничего записывать - завершить выполнение.
	Если (КонтекстВыполнения.ЖурналРегистрации.Записывать = Ложь) И (ПринудительнаяЗапись <> Истина) Тогда
		Возврат;
	КонецЕсли;

	Если ВРег(КонтекстВыполнения.ЖурналРегистрации.Уровень) = ВРег("Информация") Тогда
		УровеньЖурналаРегистрацииЛокальный = УровеньЖурналаРегистрации.Информация;
	ИначеЕсли ВРег(КонтекстВыполнения.ЖурналРегистрации.Уровень) = ВРег("Предупреждение") Тогда
		УровеньЖурналаРегистрацииЛокальный = УровеньЖурналаРегистрации.Предупреждение;
	ИначеЕсли ВРег(КонтекстВыполнения.ЖурналРегистрации.Уровень) = ВРег("Примечание") Тогда
		УровеньЖурналаРегистрацииЛокальный = УровеньЖурналаРегистрации.Примечание;
	ИначеЕсли ВРег(КонтекстВыполнения.ЖурналРегистрации.Уровень) = ВРег("Ошибка") Тогда
		УровеньЖурналаРегистрацииЛокальный = УровеньЖурналаРегистрации.Ошибка;
	ИначеЕсли ТипЗнч(КонтекстВыполнения.ЖурналРегистрации.Уровень) = Тип("УровеньЖурналаРегистрации") Тогда
		УровеньЖурналаРегистрацииЛокальный = КонтекстВыполнения.ЖурналРегистрации.Уровень;
	Иначе
		УровеньЖурналаРегистрацииЛокальный = УровеньЖурналаРегистрации.Ошибка;
	КонецЕсли;
	...
	// В Данные поместить время выполнения.
	Данные = 0;
	Если КонтекстВыполнения.ЖурналРегистрации.ЛогШаговВыполнения.Количество() > 0 Тогда
		НомерПервогоШага    = 0;
		НомерПоследнегоШага = КонтекстВыполнения.ЖурналРегистрации.ЛогШаговВыполнения.Количество()-1;
		ВремяНачала    = КонтекстВыполнения.ЖурналРегистрации.ЛогШаговВыполнения[НомерПервогоШага].ВремяНачала;
		ВремяОкончания = КонтекстВыполнения.ЖурналРегистрации.ЛогШаговВыполнения[НомерПоследнегоШага].ВремяОкончания;
		Данные = ВремяОкончания - ВремяНачала;
	КонецЕсли;

	КомментарийЛокальный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Метод: %1
			|Идентификатор контекста: %2
			|Подробности:
			|%3'"),
		КонтекстВыполнения.Общее.НаименованиеМетода,
		?(
				ПустаяСтрока(КонтекстВыполнения.Общее.ИдентификаторРодителя),
				"",
				КонтекстВыполнения.Общее.ИдентификаторРодителя + "/")
			+ КонтекстВыполнения.Общее.Идентификатор,
		СтрСоединить(ОбработкаНовостейКлиентСервер.ПолныйЛогШаговВыполнения(КонтекстВыполнения), Символы.ПС));

	ИмяСобытияЛокальный = КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия;
	Если ПустаяСтрока(ИмяСобытияЛокальный) Тогда
		ИмяСобытияЛокальный = НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка);
	КонецЕсли;
...
Функция Проверка_ДоступныНеразделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:ДоступныНеразделенныеДанные'")); // Идентификатор.
	КонецЕсли;
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		Если Результат = Ложь Тогда
			ЗавершитьШагВыполнения(
				КонтекстВыполнения,
				"ERR.WRONG_SPLIT_MODE/NEEDED_UNSPLITTED_DATA",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При работе в модели сервиса доступ к неразделенным данным возможен только в неразделенном сеансе.
			...
			ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Иначе
			ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Доступны неразделенные данные'"));
		КонецЕсли;
	КонецЕсли;
...
Функция Проверка_ДоступныРазделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:ДоступныРазделенныеДанные'")); // Идентификатор.
	КонецЕсли;
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		Если Результат = Ложь Тогда
			ЗавершитьШагВыполнения(
				КонтекстВыполнения,
				"ERR.WRONG_SPLIT_MODE/NEEDED_SPLITTED_DATA",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Необходим доступ к разделенным данным.
						|ОбщегоНазначения.РазделениеВключено() = %1
						|ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2
						|ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3'"),
					ОбщегоНазначения.РазделениеВключено(),
			...
			ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Иначе
			ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Доступны разделенные данные'"));
		КонецЕсли;
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗагрузитьВсеСтандартныеЗначения"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	Шаг = НаименованиеМетода + "/Загрузка";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		...
		КонтекстВыполненияЭталон = Новый Структура;
			КонтекстВыполненияЭталон.Вставить("ДополнительныеПараметрыВыполнения", Неопределено);
		ЗаполнитьЗначенияСвойств(КонтекстВыполненияЭталон, КонтекстВыполнения);
		Если ТипЗнч(КонтекстВыполненияЭталон.ДополнительныеПараметрыВыполнения) = ТипСтруктура Тогда
			КонтекстВыполненияВложенный.Вставить("ДополнительныеПараметрыВыполнения", КонтекстВыполненияЭталон.ДополнительныеПараметрыВыполнения);
		КонецЕсли;
...
Функция ПолучитьАдресWSDLОпределенияВебСервисаInformation() Экспорт
	...
	ПараметрыОкруженияБИП_НовостиЭталон = Новый Структура;
		ПараметрыОкруженияБИП_НовостиЭталон.Вставить("АдресWSDLОпределенияВебСервисаInformation", Неопределено);
	ЗаполнитьЗначенияСвойств(ПараметрыОкруженияБИП_НовостиЭталон, ПараметрыОкруженияБИП_Новости);

	Если (ПараметрыОкруженияБИП_НовостиЭталон.АдресWSDLОпределенияВебСервисаInformation <> Неопределено)
			И (НЕ ПустаяСтрока(ПараметрыОкруженияБИП_НовостиЭталон.АдресWSDLОпределенияВебСервисаInformation)) Тогда
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.СоединитьсяССерверомНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Протокол"); // Идентификатор.
	Если ((ВРег(Протокол) <> ВРег("http")))
			И ((ВРег(Протокол) <> ВРег("https"))) Тогда // file?
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Протокол получения новостей [%1] отличается от http и https.
				|Поэтому соединение с сервером новостей не устанавливалось.'"),
			Протокол);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.UNKNOWN_PROTOCOL", ТекстСообщения);
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Предупреждение; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат Ложь;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Порт"); // Идентификатор.
	...
	Если ГдеДвоеточие > 0 Тогда
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка определения порта из имени сервера %1 (%2).'"),
				ИмяСервера,
				Протокол);
			ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.UNKNOWN_PORT", ТекстСообщения);
			КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Ошибка; // Идентификатор.
			ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
			Возврат Ложь;
		КонецПопытки;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:НачалоПопытокСоединения"); // Идентификатор.
	...
	ПопыткаСоединения = 0;
	Пока (СоединениеУстановлено <> Истина)
			И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл

		НачатьШагВыполнения(КонтекстВыполнения, "Основные:ПопыткаСоединения"); // Идентификатор.
		Попытка
			...
			ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Попытка %1 / %2: Протокол %3, Имя сервера %4:%5. Соединение успешно.'"),
					ПопыткаСоединения,
		...
		Исключение
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.CONNECTION.NOT_CONNECTED",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Попытка %1 / %2: Протокол %3, Имя сервера %4:%5. Ошибка соединения по причине:
						|%6'"),
					ПопыткаСоединения,
					НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
					Протокол,
					...
					ИмяСервера_Порт,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
			// Это восстановимая ошибка. Имеет смысл пробовать дальше.
		КонецПопытки;
	КонецЦикла;
...
Процедура ПодготовитьПараметрыНавигационнойСсылки(Знач Объект, УИНДействия, Действие, СписокПараметров) Экспорт
	...
	ИнтеграцияПодсистемБИП.ДополнительноПодготовитьПараметрыНавигационнойСсылки(Объект, УИНДействия, Действие, СписокПараметров);
...
#Область Проверки
	...
	// В неразделенном режиме никакие новости не должны всплывать.
	// В конфигурации есть общие реквизиты с разделением и включена ФО РаботаВМоделиСервиса.
	Если ОбщегоНазначения.РазделениеВключено() Тогда
...
#Область Обработка
	...
	Если (СостояниеПодсистемы = "Активна")
		...
		Запрос.Текст = 
			...
			|	Спр.ЛентаНовостей.ОбязательныйКанал                      КАК ЛентаНовостейОбязательныйКанал,
			|	Спр.ЛентаНовостей.ПометкаУдаления                        КАК ЛентаНовостейПометкаУдаления,
			...
			|ГДЕ
			|	НЕ Спр.ПометкаУдаления // Обязательная проверка новости на пометку удаления
			|
			...
			|		КОГДА Спр.ЛентаНовостейПометкаУдаления   ТОГДА
			|			&СпособОповещенияОтключеноСсылка
			|		КОГДА Спр.ЛентаНовостейОбязательныйКанал ТОГДА
			...
			|		И Спр.ЛентаНовостей = РегОповещения.ЛентаНовостей
			|
			|ГДЕ
				...
				Если Выборка.Важность = 1 Тогда // Очень важная
					ОписаниеНовости = Новый Структура;
						...
						Если Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
							ОписаниеНовости.Вставить("СпособОповещения", "Отключено"); // Идентификатор.
						ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
							ОписаниеНовости.Вставить("СпособОповещения", "СистемаВзаимодействия"); // Идентификатор.
						Иначе
							ОписаниеНовости.Вставить("СпособОповещения", "ПоУмолчанию"); // Идентификатор.
						КонецЕсли;
...
#Область Завершение
	...
	ИнтеграцияПодсистемБИП.ПолучитьНовостиСНапоминаниями(
		ОченьВажныеНовости,
		ВажныеНовости,
		ДополнительныеПараметры,
...
Процедура СбросДатыПоследнейУспешнойЗагрузкиНовостей(ЛентыНовостей, ПомечатьНовостиНаУдаление = Ложь) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр(
				"ru='ОбработкаНовостейСлужебный.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Должно запускаться в неразделенном сеансе.'",
				КодОсновногоЯзыка);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;

	ЛентыНовостейВременный = Новый Массив;
	Если ТипЗнч(ЛентыНовостей) = ТипЛентыНовостей Тогда
		ЛентыНовостейВременный.Добавить(ЛентыНовостей);
	ИначеЕсли ТипЗнч(ЛентыНовостей) = ТипМассив Тогда
		ЛентыНовостейВременный = ЛентыНовостей;
	Иначе
		Возврат;
	КонецЕсли;

	ЛокальныеЛентыНовостей = Новый Массив;
	Для Каждого ТекущаяЛентаНовостей Из ЛентыНовостейВременный Цикл
		Если (ТипЗнч(ТекущаяЛентаНовостей) = ТипЛентыНовостей)
				И (НЕ ТекущаяЛентаНовостей.Пустая()) Тогда
			ЛокальныеЛентыНовостей.Добавить(ТекущаяЛентаНовостей);
		КонецЕсли;
	КонецЦикла;

	Если ЛокальныеЛентыНовостей.Количество() = 0 Тогда
		ТекстСообщения = НСтр(
			"ru='ОбработкаНовостейСлужебный.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Не удалось сформировать список лент новостей'",
			КодОсновногоЯзыка);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Запрос.Текст = "
		...
		|	Справочник.Новости КАК Спр
		|ГДЕ
		|	Спр.ЛентаНовостей В (&ЛентыНовостей)
		|	И Спр.ПометкаУдаления = ЛОЖЬ
		|УПОРЯДОЧИТЬ ПО
		|	Спр.ЛентаНовостей
		|";
	Запрос.УстановитьПараметр("ЛентыНовостей", ЛокальныеЛентыНовостей);
...
#Область Шаблоны
	...
	ШаблонРезультатаРегламентныеОперации = Символы.ПС
		+ НСтр("ru='### РегистрСведений.ДатыЗагрузкиПоследнихНовостей
...
#Область ПолучениеДанных
	...
	Если МассивЛентНовостей.Количество() > 0 Тогда
		...
		Для Каждого ТекущаяЛентаНовостей Из МассивЛентНовостей Цикл
			ПолучитьДополнительныеСтандартныеЗначенияНовостей(ТекущаяЛентаНовостей.ЛентаНовостей, ВсеПоставляемыеДанные);
		КонецЦикла;
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	ТекущееРегламентноеЗадание = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей("ОбработкаНовостейСлужебный.ПолучитьИнформациюОНовости");
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание.ИнформацияОбОшибке) = ТипИнформацияОбОшибке Тогда
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить(
					...
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(РегламентноеЗадание_ПоследнееЗадание.ИнформацияОбОшибке));
			Иначе
...
Функция НайтиНовости(СтруктураПараметровПоиска) Экспорт
	...
	СтруктураПараметровПоискаЭталон = Новый Структура;
		СтруктураПараметровПоискаЭталон.Вставить("СтрокаПоиска"        , Неопределено);
		СтруктураПараметровПоискаЭталон.Вставить("ПоискДатаОТ"         , Неопределено);
		СтруктураПараметровПоискаЭталон.Вставить("ПоискДатаДО"         , Неопределено);
		СтруктураПараметровПоискаЭталон.Вставить("НовостиОбластьПоиска", Неопределено);
	ЗаполнитьЗначенияСвойств(СтруктураПараметровПоискаЭталон, СтруктураПараметровПоиска);

	Если ТипЗнч(СтруктураПараметровПоискаЭталон.СтрокаПоиска) <> ТипСтрока Тогда
		СтруктураПараметровПоискаЭталон.СтрокаПоиска = "";
	КонецЕсли;
	Если ТипЗнч(СтруктураПараметровПоискаЭталон.ПоискДатаОТ) <> ТипДата Тогда
		СтруктураПараметровПоискаЭталон.ПоискДатаОТ = '00010101';
	КонецЕсли;
	Если ТипЗнч(СтруктураПараметровПоискаЭталон.ПоискДатаДО) <> ТипДата Тогда
		СтруктураПараметровПоискаЭталон.ПоискДатаДО = '00010101';
	КонецЕсли;
	Если (ТипЗнч(СтруктураПараметровПоискаЭталон.НовостиОбластьПоиска) = ТипМассив)
			И (СтруктураПараметровПоискаЭталон.НовостиОбластьПоиска.Количество() > 0) Тогда
		// Правильный список.
	Иначе
		СтруктураПараметровПоискаЭталон.НовостиОбластьПоиска = Неопределено;
	КонецЕсли;
	...
	Если СтрДлина(СтруктураПараметровПоискаЭталон.СтрокаПоиска) = 32 Тогда
		ЛокальнаяСтрокаПоискаУИН =
			Сред(СтруктураПараметровПоискаЭталон.СтрокаПоиска, 1, 8) + "-"
			+ Сред(СтруктураПараметровПоискаЭталон.СтрокаПоиска, 9, 4) + "-"
			+ Сред(СтруктураПараметровПоискаЭталон.СтрокаПоиска, 13, 4) + "-"
			+ Сред(СтруктураПараметровПоискаЭталон.СтрокаПоиска, 17, 4) + "-"
			+ Сред(СтруктураПараметровПоискаЭталон.СтрокаПоиска, 21, 12);
	ИначеЕсли СтрДлина(СтруктураПараметровПоискаЭталон.СтрокаПоиска) = 36 Тогда
		ЛокальнаяСтрокаПоискаУИН = СтруктураПараметровПоискаЭталон.СтрокаПоиска;
	КонецЕсли;
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Если НЕ ПустаяСтрока(СтруктураПараметровПоискаЭталон.СтрокаПоиска) Тогда
			...
			СписокДанных.СтрокаПоиска = СокрЛП(СтруктураПараметровПоискаЭталон.СтрокаПоиска);
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='ОбработкаНовостейСлужебный.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%1] по причине:
						|%2'"),
					СтруктураПараметровПоискаЭталон.СтрокаПоиска,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Разное'", КодОсновногоЯзыка), // ИмяСобытия.
					НСтр("ru='Новости. Разное. Ошибка полнотекстового поиска'", КодОсновногоЯзыка), // ИдентификаторШага.
	...
	Если СтруктураПараметровПоискаЭталон.ПоискДатаОТ <> '00010101' Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&МетаУсловиеДатаОТ", "Спр.ДатаПубликации >= &ПоискДатаОТ");
		Запрос.УстановитьПараметр("ПоискДатаОТ", СтруктураПараметровПоискаЭталон.ПоискДатаОТ);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&МетаУсловиеДатаОТ", "ИСТИНА");
	КонецЕсли;
	// Проверка: Дата окончания.
	Если СтруктураПараметровПоискаЭталон.ПоискДатаДО <> '00010101' Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&МетаУсловиеДатаДО", "Спр.ДатаПубликации <= &ПоискДатаДО");
		Запрос.УстановитьПараметр("ПоискДатаДО", СтруктураПараметровПоискаЭталон.ПоискДатаДО);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&МетаУсловиеДатаДО", "ИСТИНА");
	КонецЕсли;
	// Проверка: область поиска.
	Если (СтруктураПараметровПоискаЭталон.НовостиОбластьПоиска = Неопределено) Тогда
	...
	Иначе // Массив ненулевого размера - проверяется выше.
		...
		Запрос.УстановитьПараметр("НовостиОбластьПоиска", СтруктураПараметровПоискаЭталон.НовостиОбластьПоиска);
	КонецЕсли;

	// В строке поиска не должно быть двойных кавычек.
	СтруктураПараметровПоискаЭталон.СтрокаПоиска = СтрЗаменить(СтруктураПараметровПоискаЭталон.СтрокаПоиска, Символ(34), Символ(34) + Символ(34)); // Заменить одну двойную кавычку на две двойные кавычки.
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#СтрокаПоиска#", СтруктураПараметровПоискаЭталон.СтрокаПоиска);
...
Функция ПолучитьОбъектНовости(
	...
	// При загрузке новости не сразу писать в регистры сведений (по каждой записи), а "копить" записи в объекте.
	// Запись через наборы записей будет осуществляться в модуле объекта в ПриЗаписи.
	Объект.ДополнительныеСвойства.Вставить("РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записи"    , Новый Массив);
	Объект.ДополнительныеСвойства.Вставить("РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записывать", Ложь);
	Объект.ДополнительныеСвойства.Вставить("РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записи"            , Новый Массив);
	Объект.ДополнительныеСвойства.Вставить("РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записывать"        , Ложь);
...
#Область ИнициализацияМетода
	...
	НастройкиПолученияНовостейЭталон = Новый Структура;
		НастройкиПолученияНовостейЭталон.Вставить("ПолучатьКатегорииНовостей", Ложь);
	Если ТипЗнч(НастройкиПолученияНовостей) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(НастройкиПолученияНовостейЭталон, НастройкиПолученияНовостей);
	КонецЕсли;
	НастройкиПолученияНовостейЭталон.ПолучатьКатегорииНовостей = ?(НастройкиПолученияНовостейЭталон.ПолучатьКатегорииНовостей = Ложь, Ложь, Истина);
...
#Область Обработка
	...
	Если Вариант = "Для форм, все поля" Тогда // АПК:1297 это идентификатор.
		...
		Запрос.Текст = "
			...
			|	ВЫБОР // {Получение способа оповещения}
			|		КОГДА Рег.Новость.ЛентаНовостей.ПометкаУдаления   ТОГДА
			|			&СпособОповещенияОтключеноСсылка
			|		КОГДА Рег.Новость.ЛентаНовостей.ОбязательныйКанал ТОГДА
			|			&СпособОповещенияПоУмолчаниюСсылка
			|		ИНАЧЕ
			|			ЕСТЬNULL(РегОповещения.СпособОповещенияПользователяОНовостях, &СпособОповещенияПоУмолчаниюСсылка)
			|	КОНЕЦ                                                    КАК СпособОповещенияСсылка,
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Если НастройкиПолученияНовостейЭталон.ПолучатьКатегорииНовостей = Истина Тогда
			...
			Для Каждого ТекущаяНовость Из Таблица Цикл
				...
				КонецЕсли;
				// {При добавлении способов оповещения исправлять здесь}.
				Если ТекущаяНовость.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
					ТекущаяНовость.СпособОповещения = "Отключено"; // Идентификатор.
				ИначеЕсли ТекущаяНовость.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
					ТекущаяНовость.СпособОповещения = "СистемаВзаимодействия"; // Идентификатор.
				Иначе
					ТекущаяНовость.СпособОповещения = "ПоУмолчанию"; // Идентификатор.
				КонецЕсли;
			КонецЦикла;
			Если НастройкиПолученияНовостейЭталон.ПолучатьКатегорииНовостей = Истина Тогда
	...
	ИначеЕсли Вариант = "Для форм, простой" Тогда // АПК:1297 это идентификатор.
		...
		Запрос.Текст = "
			...
			|			ЕСТЬNULL(РегОповещения.СпособОповещенияПользователяОНовостях, &СпособОповещенияПоУмолчаниюСсылка)
			|	КОНЕЦ,
			|	&СпособОповещенияПоУмолчаниюСтрока
			|
			|УПОРЯДОЧИТЬ ПО
			|	Рег.Новость.ДатаПубликации УБЫВ
			|";
	...
	ИначеЕсли Вариант = "Для формы контекстных новостей" Тогда // АПК:1297 это идентификатор.
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Для Каждого ТекущаяНовость Из Таблица Цикл
				...
				Иначе
					ТекущаяНовость.СпособОповещения = "ПоУмолчанию"; // Идентификатор.
				КонецЕсли;
				// ИконкаНовости - только если новость помечена как ПривязкаКМетаданным.ЭтоПостояннаяНовость
				ТекущаяНовость.ИконкаНовости = Неопределено;
				Если ТекущаяНовость.ЭтоПостояннаяНовость = Истина Тогда
					ТекущаяНовость.ИконкаНовости = БиблиотекаКартинок.НовостиПостоянная;
				КонецЕсли;
			КонецЦикла;
		Иначе
			Таблица = Новый ТаблицаЗначений;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ВсеОбновленияНовостей"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Все обновления новостей'", КодОсновногоЯзыка);
...
#Область Проверки
	...
	Если НЕ Проверка_МонопольныйРежимОтключен(КонтекстВыполнения) Тогда
		Возврат;
	КонецЕсли;
...
#Область Обработка
	...
	// Даты последних обновлений хранятся в подсистеме "РасписанияРегламентныхЗаданий".
	...
	// 7. Регулярное заполнение значений категорий (общих).

#Область Обработка_ЗагрузкаНовостей

	// 1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения "ЛентаНовостей").
	Операция = "ЗагрузкаНовостей"; // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ПолучитьИОбработатьНовостиПоЛентамНовостей"); // Идентификатор.
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока Выборка.Следующий() Цикл
				Операция = "БИП:Новости/ПолучениеНовостей/" + Выборка.КодЛентыНовостей; // Идентификатор операции расписания.
				Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
		...
		Если МассивЛентНовостей.Количество() > 0 Тогда
			...
			ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Нет лент новостей для регламентного обновления'"));
		КонецЕсли;

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, ЛогШаговВыполнения, КонтекстВыполненияВложенный);

#КонецОбласти

#Область Обработка_ОбновлениеКлассификаторов

	// 2. Обновление классификаторов.
	Операция = "БИП:Новости/ОбновлениеКлассификаторов"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновитьСтандартныеСпискиДляНовостей_Ежедневно"); // Идентификатор.
		...
		Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
			...
			Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
				ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
				ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполненияВложенный);
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
			КонецЕсли;

		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Обновление классификаторов через интернет отключено'"));
		КонецЕсли;
...
#Область Обработка_АвтоудалениеНовостей

	// 3. Автоудаление новостей.
	Операция = "БИП:Новости/АвтоудалениеНовостей"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "УдалитьНовости_Ежедневно"); // Идентификатор.

		// Обновление привязок к метаданным каждые 30 минут (а также после получения новостей).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			УдалитьНовости_Ежедневно(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;
...
#Область Обработка_ОбновлениеПериодическихСвойств

	// 4. Обновление периодических свойств (важность, актуальность).
	Операция = "БИП:Новости/ОбновлениеПериодическихСвойств"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновлениеПериодическихСвойствНовостей"); // Идентификатор.

		// Обновление привязок к метаданным каждые 30 минут (а также после получения новостей).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			ОбновлениеПериодическихСвойствНовостей(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;
...
#Область Обработка_ОбновлениеПривязокКМетаданным

	// 5. Обновление привязок к метаданным (+ важность).
	Операция = "БИП:Новости/ОбновлениеПривязокКМетаданным"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ОбновлениеПривязокКМетаданным"); // Идентификатор.

		// Обновление привязок к метаданным каждые 30 минут (а также после получения новостей).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			ОбновлениеПривязокКМетаданным(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;
...
#Область Обработка_ПроверкаВерсииПлатформы

	// 6. Проверка версии платформы.
	Операция = "БИП:Новости/ПроверкаВерсииПлатформы"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ПроверитьИзменениеВерсииПлатформы"); // Идентификатор.

		// Проверка версии платформы каждые 6 часов (а также при старте клиентского приложения).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			ПроверитьИзменениеВерсииПлатформы(КонтекстВыполненияВложенный); // Мы в привилегированном режиме
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_Общие

	// Вызов переопределения, чтобы заполнили данные для области данных и для пользователей.
	Операция = "БИП:Новости/ЗаполнитьПрограммныеЗначенияКатегорий_Общие"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ЗаполнитьПрограммныеЗначенияКатегорий_Общие"); // Идентификатор.

		// Обновление привязок к метаданным каждые 30 минут (а также после получения новостей).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			ЗаполнитьПрограммныеЗначенияКатегорий_Общие(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, СтрСоединить(ЛогШаговВыполнения, Символы.ПС), КонтекстВыполненияВложенный);

#КонецОбласти

#Область Обработка_ВсеОбновленияНовостейДляОбластиДанных

	// Ели регламентное задание запускается в коробке, то вызывать также метод для областей данных.
	Операция = "БИП:Новости/ВсеОбновленияНовостейДляОбластиДанных"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ВсеОбновленияНовостейДляОбластиДанных"); // Идентификатор.
		...
		Если НЕ ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
				ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
				ВсеОбновленияНовостейДляОбластиДанных(КонтекстВыполненияВложенный);
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ВсеОбновленияНовостейДляОбластиДанных(КонтекстВыполнения = Неопределено) Экспорт
	...
	// Если идет обновление конфигурации, то все регламентные задания блокируются.
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных);

#Область ИнициализацияМетода

	КодОсновногоЯзыка  = ОбщегоНазначения.КодОсновногоЯзыка();

	НаименованиеМетода = "ОбработкаНовостейСлужебный.ВсеОбновленияНовостейДляОбластиДанных"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Все обновления новостей для области данных'", КодОсновногоЯзыка);
...
#Область Проверки
	...
	Если НЕ Проверка_ДоступныРазделенныеДанные(КонтекстВыполнения) Тогда
		Возврат;
	КонецЕсли;
...
#Область Обработка
	...
	// Даты последних обновлений хранятся в подсистеме "РасписанияРегламентныхЗаданий".
	// Измерение ВидОбновления может содержать следующие значения:
	// 1. Регулярное заполнение значений категорий (для области данных).

#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных

	// Вызов переопределения, чтобы заполнили данные для области данных и для пользователей.
	Операция = "БИП:Новости/ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"; // Идентификатор операции расписания.
	ЛогШаговВыполнения = Новый Массив;
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"); // Идентификатор.

		// Обновление привязок к метаданным каждые 30 минут (а также после получения новостей).
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Наступил срок выполнения.'"));
			ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Срок не наступил.'"));
		КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.УдалитьНовости_Ежедневно"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучитьСписокДанныхДляУдаления

	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'")); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_Удаление

	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, НСтр("ru='УдалениеНовостей_Удаление'")); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;

		УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, КонтекстВыполненияВложенный);

	ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтрСоединить(ЛогШаговВыполнения, Символы.ПС), КонтекстВыполненияВложенный);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПолучитьИОбработатьНовостиПоЛентамНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_Инициализация
	...
	РазрешенаРаботаСНовостямиЧерезИнтернет = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет();
	ДобавитьШагВыполнения(КонтекстВыполнения, "Получение:РазрешенаРаботаСНовостямиЧерезИнтернет", "",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
#Область Обработка_ВыполнениеЗапроса

	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ВыполнениеЗапроса"); // Идентификатор.
	...
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	Спр.ЛентаНовостей            КАК ЛентаНовостей,
		|	МАКСИМУМ(Спр.ДатаПубликации) КАК ПоследняяНовостьДатаПубликации
		|ПОМЕСТИТЬ ВТ_ПоследниеДатыНовостей
		|ИЗ
		|	Справочник.Новости КАК Спр
		|СГРУППИРОВАТЬ ПО
		|	Спр.ЛентаНовостей
		...
		|
		|ВЫБРАТЬ
		|	ВТ_ПоследниеДатыНовостей.ЛентаНовостей                  КАК ЛентаНовостей,
		|	ВТ_ПоследниеДатыНовостей.ПоследняяНовостьДатаПубликации КАК ПоследняяНовостьДатаПубликации,
		|	Спр.УИННовости                                          КАК ПоследняяНовостьУИН
		|ПОМЕСТИТЬ ВТ_ПоследниеНовости
		|ИЗ
		|	ВТ_ПоследниеДатыНовостей
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|	Справочник.Новости КАК Спр
		|	ПО
		|		ВТ_ПоследниеДатыНовостей.ЛентаНовостей = Спр.ЛентаНовостей
		|		И ВТ_ПоследниеДатыНовостей.ПоследняяНовостьДатаПубликации = Спр.ДатаПубликации
		|ИНДЕКСИРОВАТЬ ПО
		|	ЛентаНовостей
		|;
		|
		...
		|	ЕСТЬNULL(
		|		ВТ_ПоследниеНовости.ПоследняяНовостьДатаПубликации,
		|		&ПустаяДата)                         КАК ПоследняяНовостьДатаПубликации,
		|	ЕСТЬNULL(
		|		ВТ_ПоследниеНовости.ПоследняяНовостьУИН,
		...
		|	ВТ_ПоследниеНовости
		|	ПО
		|		Спр.Ссылка = ВТ_ПоследниеНовости.ЛентаНовостей
		|
		|ГДЕ
		|	(
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.QUERY.EMPTY",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей
					...
					|Возможно, список лент новостей пустой, или у всех лент новостей установлен
					|признак ЛокальнаяЛентаНовостей (что означает, что по ней не надо загружать новости),
					|или у всех лент новостей установлена пометка удаления.'"),
				МассивЛентНовостей.Количество()));
	КонецЕсли;

#КонецОбласти

#Область Обработка_ОбработкаЗапроса

	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ОбработкаРезультатаЗапроса"); // Идентификатор.
	Если ЕстьЛентыНовостей = Истина Тогда
		...
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
			КонтекстВыполнения,
			"ЗагрузкаИПарсингНовостей",
			НаименованиеМетода + "/ЗагрузкаИПарсингНовостей");
		...
		// {Раздельное обновление ИБ}
		КонтекстВыполненияВложенный.ВходящиеПараметры.Вставить("ИсточникЗапуска", "ПолучитьИОбработатьНовостиПоЛентамНовостей");
		КонтекстВыполненияВложенный.ВходящиеПараметры.Вставить("ОбрабатыватьОбластиДанных", Истина);
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "",
			НСтр("ru='Загрузка и парсинг новостей завершены.'"), КонтекстВыполненияВложенный);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновитьСтандартныеСпискиДляНовостей_Ежедневно"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
		КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Все обновления новостей'", КодОсновногоЯзыка);
	КонецЕсли;
...
#Область Обработка_Инициализация
	...
	ЛогШаговВыполнения.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Истина, // Если дошли сюда, значит "Истина".
			РазрешенаРаботаСНовостямиЧерезИнтернет));
...
#КонецОбласти
	...
	Если РазрешенаРаботаСНовостямиЧерезИнтернет Тогда
		...
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения,
			"Обработки.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера",
			"Обработки.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера"); // Идентификатор.
...
#Область Обработка_ОбновлениеРасписания
	...
	Если Результат = Истина Тогда
		РасписанияРегламентныхЗаданий.ОперацияВыполнена(Операция, ПараметрыВыполненияОперации);
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Классификаторы обновлены без ошибок/'"));
	Иначе
		РасписанияРегламентныхЗаданий.ОтложитьОперациюСлучайно(Операция, 1*60*60, -15*60, +15*60); // 1 час +- 15 минут.
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Были ошибки обновления классификаторов. Время следующего запуска изменено.'"));
	КонецЕсли;
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", ЛогШаговВыполнения);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПересчитатьНеобработанныеКатегорииНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ОбновлениеКатегорийИЗначений

#Область Обработка_ОбновлениеКатегорийИЗначений_ПолучениеДанных

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ПолучениеДанных"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
		...
		Иначе
			...
			ЛогШаговВыполнения.Добавить(НСтр("ru='Результаты[0] - пустой'"));
		КонецЕсли;
		...
		Иначе
			...
			ЛогШаговВыполнения.Добавить(НСтр("ru='Результаты[2] - пустой'"));
		КонецЕсли;
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='ЕстьДанныеДляОбработки: %1'"),
				ЕстьДанныеДляОбработки));

	ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтрСоединить(ЛогШаговВыполнения, Символы.ПС));

	// В конфигурации есть общие реквизиты с разделением и включена ФО РаботаВМоделиСервиса.
	Если ЕстьДанныеДляОбработки = Ложь Тогда
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		Возврат;
	КонецЕсли;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ДобавлениеИндексовИКолонок

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ДобавлениеИндексовИКолонок"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеКатегорий_Простые

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеПростыхКатегорий"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеКатегорий_ИнтервалыВерсий

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеКатегорийИнтервалыВерсий"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеПростыхЗначений

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеПростыхЗначений"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
		...
		Для каждого ТекущаяСтрока Из ТаблицаКатегорийИЗначенийПростые Цикл
			...
			Если ТекущаяСтрока.КатегорияНовостей = М.КатегорияПродукт()
					ИЛИ ТекущаяСтрока.КатегорияНовостей = М.КатегорияВерсияПродукта()
					ИЛИ ТекущаяСтрока.КатегорияНовостей = М.КатегорияВерсияПлатформы() Тогда
...
#Область Обработка_ОбновлениеКатегорийИЗначений_ЗаполнениеЗначенийИнтерваловВерсий

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОбновлениеКатегорийИЗначений/ЗаполнениеЗначенийИнтервалыВерсий"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_УдалениеНеправильныхЗаписей

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:УдалениеНеправильныхЗаписей"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ПереносПравильныхЗаписей

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ПереносПравильныхЗаписей"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ОчисткаМусора

	НачатьШагВыполнения(КонтекстВыполнения, "Обработка:ОчисткаМусора"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПроверитьИзменениеВерсииПлатформы"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучениеНастроек

	НачатьШагВыполнения(КонтекстВыполнения, "ПолучитьТекущиеНастройки"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
...
#Область Обработка_ПересчетОтборов
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПересчетОтборов"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
	...
	Если (ТипЗнч(ЛокальнаяПоследняяВерсия) <> ТипСтрока) // Первый запуск
		...
		ИмяМетода = "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся";
		ПараметрыМетода = Новый Массив;
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
			КонтекстВыполнения,
			НаименованиеМетода + "/ПересчитатьОтборыПоНовостям_РедкоМеняющиеся");
		КонтекстВыполненияВложенный.ВходящиеПараметры.Вставить("УстановитьПривилегированныйРежим", Истина);
		ПараметрыМетода.Добавить(КонтекстВыполненияВложенный);
		Попытка
			...
			Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
				...
				МассивФоновыхЗаданий.Добавить(
					ФоновыеЗадания.Выполнить(
						...
						"", // Ключ
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2'"),
							ЛокальнаяПоследняяВерсия,
							ЛокальнаяТекущаяВерсия)));
				// Последняя версия платформы сохранится в "ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
				ЛогШаговВыполнения.Добавить(НСтр("ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)'"));
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru='Редко меняющиеся отборы уже пересчитываются (запущено фоновое задание из другого сеанса)'"));
			КонецЕсли;
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
			ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполненияВложенный); // здесь сохранится последняя версия платформы
		КонецЕсли;
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Сохраненная версия платформы:'") + " " + ЛокальнаяПоследняяВерсия);
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Текущая версия платформы:'") + " " + ЛокальнаяТекущаяВерсия);
	Иначе
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Сохраненная версия платформы:'") + " " + ЛокальнаяПоследняяВерсия);
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Текущая версия платформы:'") + " " + ЛокальнаяТекущаяВерсия);
		ЛогШаговВыполнения.Добавить(НСтр("ru='Редко меняющиеся отборы останутся как есть'"));
	КонецЕсли;
...
#Область Обработка_СохранениеНастроек

	// Сохранение версии платформы выполняется в ПересчитатьОтборыПоНовостям_РедкоМеняющиеся.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.НайтиИОбработатьНеактивныеЛентыНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ОсновныеДействия
	...
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	Рег.ЛентаНовостей                        КАК ЛентаНовостей,
		|	Рег.ЛентаНовостей.Код                    КАК ЛентаНовостейКод,
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Цикл:ЛентыНовостей",
		НаименованиеМетода + "/Цикл:ЛентыНовостей"); // Идентификатор.
	Если НЕ Результат.Пустой() Тогда

		Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			НачатьШагВыполнения(КонтекстВыполненияВложенный, "Цикл:ЛентаНовостей:" + Выборка.ЛентаНовостейКод);
			ЛогШаговВыполнения = Новый Массив;
			...
			ЛогШаговВыполнения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Лента новостей %1, частота обновления ленты новостей: %2, дата последней успешной загрузки: %3, неудач: %4, последняя попытка загрузки: %5. %6'"),
				Выборка.ЛентаНовостей,
			...
			Если Выборка.ЛентаНовостейАктивна = Истина Тогда
				...
				Если СтруктураНовости.Ссылка.Пустая() Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие'") + ": "
						+ НСтр("ru='Лента новостей активна, новости о неактивности нет'"));
				ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие'") + ": "
						+ НСтр("ru='Лента новостей активна, новость о неактивности уже была помечена на удаление'"));
				Иначе
					// Пометить найденную новость на удаление.
					НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
					НовостьОбъект.УстановитьПометкуУдаления(Истина);
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие'") + ": "
						+ НСтр("ru='Лента новостей активна, новость о неактивности будет помечена на удаление'"));
				КонецЕсли;
			Иначе
				...
				Если СтруктураНовости.Ссылка.Пустая() Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие'") + ": "
						+ НСтр("ru='Лента новостей НЕактивна, новость о неактивности будет создана'"));
				Иначе
					НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие'") + ": "
						+ НСтр("ru='Лента новостей НЕактивна, новость о неактивности будет обновлена'"));
				КонецЕсли;
				...
				Отказ = Ложь;
				ИнтеграцияПодсистемБИП.ПереопределитьДействияКогдаНайденаНеактивнаяЛентаНовостей(Выборка.ЛентаНовостей, НовостьОбъект, Отказ);
				...
				Если Отказ <> Истина Тогда
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %1
								|Подробно:
								|%2'"),
							Выборка.ЛентаНовостей,
							ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					...
					КонецПопытки;
				Иначе
					ЛогШаговВыполнения.Добавить(НСтр("ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей'"));
				КонецЕсли;
			КонецЕсли;
			ЗавершитьШагВыполнения(КонтекстВыполненияВложенный, "", ЛогШаговВыполнения);
		КонецЦикла;
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, "",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Окончание проверки лент новостей на актуальность.
			...
			?(Результат.Пустой(), НСтр("ru='Запрос выполнен.'"), НСтр("ru='Запрос пустой.'"))),
			КонтекстВыполненияВложенный);
...
Процедура ПриЗаписиПользователя(ПользовательОбъект, Отказ) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи") Тогда
		...
		ДополнительныеСвойстваЭталон = Новый Структура;
			ДополнительныеСвойстваЭталон.Вставить("ЭтоНовыйОбъект", Ложь);
		Если ТипЗнч(ПользовательОбъект.ДополнительныеСвойства) = ТипСтруктура Тогда
			ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ПользовательОбъект.ДополнительныеСвойства);
		КонецЕсли;

		Если ДополнительныеСвойстваЭталон.ЭтоНовыйОбъект = Истина Тогда
			ЭтоНовыйПользователь = Истина;
		КонецЕсли;

		// Выполнить какие-то специфические действия при записи пользователя.
		ИнтеграцияПодсистемБИП.ОбработатьЗаписьПользователя(ПользовательОбъект, ЭтоНовыйПользователь, Отказ);
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	ТекущееРегламентноеЗадание = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей("ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий");
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		ЕстьОшибка = Истина;
	...
	Иначе
		...
		Иначе
			...
			ЕстьОшибка = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий:
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Функция ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей(ИсточникВызова)
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.СбросУНовостейПризнакаТребуетсяПересчетОтборов"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
Функция УдалениеНовостей_ПолучитьСписокДанныхДляУдаления(ТекущаяДатаДляРасчетов, Отборы)
	...
	Запрос.Текст = "
		...
		|	И Рег.КатегорияНовостей = &КатегорияНовостейВерсияПлатформы
		...
		|ГДЕ
		|	Рег.Новость.ПометкаУдаления = ЛОЖЬ
		|	И Рег.КатегорияНовостей = &КатегорияНовостейВерсияПродукта
	...
	Запрос.УстановитьПараметр("ПустаяДата"                      , '00010101');
	Запрос.УстановитьПараметр("КатегорияНовостейВерсияПродукта" , М.КатегорияВерсияПродукта());
	Запрос.УстановитьПараметр("КатегорияНовостейВерсияПлатформы", М.КатегорияВерсияПлатформы());
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.УдалениеНовостей_ПометитьНаУдалениеДанные"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_Инициализация
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация"); // Идентификатор.
	...
	ДанныеДляУдаленияЭталон = Новый Структура;
		ДанныеДляУдаленияЭталон.Вставить("НовостиДляУдаления"                            , Неопределено);
		ДанныеДляУдаленияЭталон.Вставить("НовостиДляУстановкиПометкиУдаления"            , Неопределено);
		ДанныеДляУдаленияЭталон.Вставить("КатегорииНовостейПростыеНеобработанные"        , Неопределено);
		ДанныеДляУдаленияЭталон.Вставить("КатегорииНовостейИнтервалыВерсийНеобработанные", Неопределено);
	ЗаполнитьЗначенияСвойств(ДанныеДляУдаленияЭталон, ДанныеДляУдаления);

#КонецОбласти

#Область Обработка_УдалитьНовости

	// Удалить новости.
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУдаления) = ТипМассив)
			И ДанныеДляУдаленияЭталон.НовостиДляУдаления.Количество() > 0 Тогда
		// ////? Удалить новости по списку. Для этого необходимо наличие полных прав, монопольного режима и т.п.
		ТекстСообщения = СтрЗаменить(
			НСтр("ru='Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей).
				|Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.'"),
			"%ДанныеДляУдаления.НовостиДляУдаления.Количество()%",
			ДанныеДляУдаленияЭталон.НовостиДляУдаления.Количество());
...
#Область Обработка_ОчисткаРегистровСведенийШаг1

	НачатьШагВыполнения(КонтекстВыполнения, "ОчиститьРегистрыСведений.Шаг1"); // Идентификатор.
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУдаления) = ТипМассив)
		...
		Для Каждого РегистрДляОчистки Из МассивРегистровДляОчистки Цикл
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаленияЭталон.НовостиДляУдаления Цикл
...
#Область Обработка_ПометкаНовостейНаУдаление

	НачатьШагВыполнения(КонтекстВыполнения, "ПометкаНаУдалениеНовостей"); // Идентификатор.

	// Установить пометку удаления устаревшим новостям.
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления) = ТипМассив)
			И ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления.Количество() > 0 Тогда
		Для Каждого ТекущаяНовость Из ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления Цикл
			НовостьОбъект = ТекущаяНовость.ПолучитьОбъект();
		...
		КонецЦикла;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Все устаревшие новости успешно помечены на удаление (%1 новостей).'"),
			ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления.Количество());
	Иначе
...
#Область Обработка_Очистка_КатегорииНовостейПростыеНеобработанные

	НачатьШагВыполнения(КонтекстВыполнения, "Очистка_КатегорииНовостейПростыеНеобработанные"); // Идентификатор.

	// Записи регистров сведений удалять без учета того, что какие-то новости могли быть НЕ удалены.
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.КатегорииНовостейПростыеНеобработанные) = ТипМассив)
			И (ДанныеДляУдаленияЭталон.КатегорииНовостейПростыеНеобработанные.Количество() > 0) Тогда
		Т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
		Для Каждого ТекущаяЗапись Из ДанныеДляУдаленияЭталон.КатегорииНовостейПростыеНеобработанные Цикл
			Запись = РегистрыСведений.КатегорииНовостейПростыеНеобработанные.СоздатьМенеджерЗаписи();
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДанныеДляУдаленияЭталон.КатегорииНовостейПростыеНеобработанные.Количество(),
			(Т2-Т1));
	Иначе
...
#Область Обработка_Очистка_КатегорииНовостейИнтервалыВерсийНеобработанные

	НачатьШагВыполнения(КонтекстВыполнения, "Очистка_КатегорииНовостейИнтервалыВерсийНеобработанные"); // Идентификатор.

	// Записи регистров сведений удалять без учета того, что какие-то новости могли быть НЕ удалены.
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.КатегорииНовостейИнтервалыВерсийНеобработанные) = ТипМассив)
			И (ДанныеДляУдаленияЭталон.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество() > 0) Тогда
		Т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
		Для Каждого ТекущаяЗапись Из ДанныеДляУдаленияЭталон.КатегорииНовостейИнтервалыВерсийНеобработанные Цикл
			Запись = РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(Запись, ТекущаяЗапись);
			Запись.Удалить();
		КонецЦикла;
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДанныеДляУдаленияЭталон.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество(),
			(Т2-Т1));
	Иначе
...
#Область Обработка_ОчисткаРегистровСведенийШаг2

	НачатьШагВыполнения(КонтекстВыполнения, "ОчиститьРегистрыСведений.Шаг2"); // Идентификатор.
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления) = ТипМассив)
		...
		Для Каждого РегистрДляОчистки Из МассивРегистровДляОчистки Цикл
			Т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления Цикл
				НаборЗаписей = РегистрыСведений[РегистрДляОчистки].СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Новость.Установить(ТекущаяЗапись, Истина);
				НаборЗаписей.Очистить();
				НаборЗаписей.Записать(Истина);
...
Функция ПолучитьКонтекстныеНовостиДляФормы(
	...
	Если ПолучатьКонтекстныеНовости = Истина Тогда
		...
		ИнтеграцияПодсистемБИП.ПереопределитьСписокЛентНовостейДляКонтекстныхНовостей(СписокЛентНовостей);
		...
		МассивСтруктурНовостей = ОбщегоНазначения.ТаблицаЗначенийВМассив(
			ПолучитьКонтекстныеНовости(
				...
				"Для форм, все поля", // Вариант списка новостей - со всеми полями // АПК:1297 это идентификатор.
				НастройкиПолученияНовостей));
	КонецЕсли;

	ИнтеграцияПодсистемБИП.ДополнительноОбработатьМассивКонтекстныхНовостей(
		ИдентификаторМетаданных,
		ИдентификаторФормы,
	...
	Для Каждого ТекущаяНовость Из МассивСтруктурНовостей Цикл
		...
		// СпособОповещения тут не учитывается, т.к. это не "напрыгивающие" новости.
		Если ТекущаяНовость.ЭтоПостояннаяНовость = Истина Тогда
	...
	Если МассивИдентификаторовСобытийПриОткрытии.Количество() > 0 Тогда
		Для Каждого ТекущаяНовость Из МассивСтруктурНовостей Цикл
			Если НЕ ТекущаяНовость.ОповещениеВключено Тогда // Не Показывать оповещение.
				Продолжить;
			КонецЕсли;
			Если ТекущаяНовость.Важность = 1 Тогда // Очень важная
				Для Каждого ТекущийИдентификаторСобытия Из МассивИдентификаторовСобытийПриОткрытии Цикл
					Если СтрСравнить(ТекущаяНовость.Событие, ТекущийИдентификаторСобытия) = 0
							И СтрСравнить(ТекущаяНовость.Форма, ИдентификаторФормы) = 0 Тогда
						// {При добавлении способов оповещения исправлять здесь}.
						// В итоговое количество не будут попадать те новости, у которых СпособОповещения = Отключено.
						// Если надо, чтобы попадали, то в переопределении надо изменять СпособОповещения.
						Если ТекущаяНовость.СпособОповещения <> "Отключено" Тогда
			...
			ИначеЕсли ТекущаяНовость.Важность = 2 Тогда // Важная
				Для Каждого ТекущийИдентификаторСобытия Из МассивИдентификаторовСобытийПриОткрытии Цикл
					Если СтрСравнить(ТекущаяНовость.Событие, ТекущийИдентификаторСобытия) = 0 // Равны без учета регистра
							И СтрСравнить(ТекущаяНовость.Форма, ИдентификаторФормы) = 0 Тогда // Равны без учета регистра
						// {При добавлении способов оповещения исправлять здесь}.
						// В итоговое количество не будут попадать те новости, у которых СпособОповещения = Отключено.
						// Если надо, чтобы попадали, то в переопределении надо изменять СпособОповещения.
						Если ТекущаяНовость.СпособОповещения <> "Отключено" Тогда
						...
						КонецЕсли;
						// Тем не менее, несмотря на СпособОповещения, в список новостей такая новость попадет.
						ОписаниеНовости = Новый Структура;
							...
							ОписаниеНовости.Вставить("СпособОповещения"       , ТекущаяНовость.СпособОповещения);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПравильностьВнедренияПодсистемы"); // Идентификатор.

	Результат = ПроверитьПравильностьВнедренияПодсистемы("ТолькоПереопределения");
	Если НЕ ПустаяСтрока(Результат) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены ошибки внедрения подсистемы ИнтернетПоддержкаПользователей.Новости.
				|Обнаруженные ошибки внедрения:
				|%1'"),
			Результат);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.SUBSYSTEM_PLUGGED_INCORRECT/UPDATE",
			ТекстСообщения);
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Ошибка; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
		// Это критичная ошибка, прервать выполнение.
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
#Область ОсновныеДействия_ВключитьРаботуСНовостями
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:ВключитьРаботуСНовостями"); // Идентификатор.
	ОбработкаНовостей.РазрешитьРаботуСНовостями(); // Установка константы "РазрешенаРаботаСНовостями" в Истина
	Константы.РазрешенаРаботаСНовостямиЧерезИнтернет.Установить(Истина);
#КонецОбласти

#Область ОсновныеДействия_ОчисткаДатыЗагрузкиПоследнихНовостей
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Очистка:РС.ДатыЗагрузкиПоследнихНовостей"); // Идентификатор.
	НаборЗаписей = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьНаборЗаписей();
	НаборЗаписей.Записать(Истина);
#КонецОбласти

#Область ОсновныеДействия_ЗагрузкаСтандартныхЗначений
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Заполнение:СтандартныеЗначения",
		НаименованиеМетода + "/Заполнение:СтандартныеЗначения");
		...
		ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполненияВложенный);

	ЗавершитьШагВыполнения(КонтекстВыполнения, 0, "", КонтекстВыполненияВложенный);
#КонецОбласти

#Область ОсновныеДействия_ОчисткаДатыЗагрузкиПоследнихНовостейМинус3Месяца
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:ДатаЗагрузкиНовостейМинус3Месяца"); // Идентификатор.
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	Спр.Ссылка КАК ЛентаНовостей
...
#Область ОсновныеДействия_УстановкаРасписания
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:Расписание"); // Идентификатор.
	ТекущееРегламентноеЗадание = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей("ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск");
	Если ТекущееРегламентноеЗадание <> Неопределено Тогда
		ТекущееРегламентноеЗадание.Наименование = НСтр("ru='Все обновления новостей'");
		ТекущееРегламентноеЗадание.Расписание   = ПолучитьРасписаниеОбновленийНовостей(Ложь);
		ТекущееРегламентноеЗадание.Записать();
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru='Успешная установка расписания регламентного задания.'"));
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SCHEDULED_JOB.ABSENT/UPDATE", НСтр("ru='Регламентное задание ВсеОбновленияНовостей не найдено.'"));
	КонецЕсли;
#КонецОбласти
...
#Область ПереопределяемыеДействия
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПереопределяемыеДействия"); // Идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
#Область Проверки
	...
	Если НЕ Проверка_ЭтоОбычныйПользователь(КонтекстВыполнения) Тогда
		Возврат;
	КонецЕсли;
...
#Область ОсновныеДействия
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"ОсновныеДействия",
		НаименованиеМетода + "/ОсновныеДействия");
		...
		Если РазрешенаРаботаСНовостями = Истина Тогда
			...
			ЗавершитьШагВыполнения(КонтекстВыполнения, 0, "", КонтекстВыполненияВложенный);
		Иначе
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SUBSYSTEM.OFF/UPDATE",
				НСтр("ru='Работа с новостями отключена. Обработчик пропущен.'"), КонтекстВыполненияВложенный);
		КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);

	НачатьШагВыполнения(КонтекстВыполнения, "Запуск:" + НаименованиеМетода
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Переход на версию %1 (общие данные)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки())); // Идентификатор.
...
#КонецОбласти
	...
	Иначе
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			0, // Данные
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Обновление ИБ'", КодОсновногоЯзыка);
...
Функция ПроверитьПравильностьВнедренияПодсистемы(ВариантПроверки = "Все") Экспорт
	...
	НаименованиеМетода = "ОбработкаНовостейСлужебный.ПроверитьПравильностьВнедренияПодсистемы"; // Идентификатор.

	Результат = Новый Массив;

	// 1. Проверить правильность настройки состава общих реквизитов.
#Область Проверка_СоставОбщихРеквизитов
	Если ВариантПроверки = "Все" Тогда
		...
		РезультатЛокальный = Новый Массив;
		...
		Если НайденныйРеквизит <> Неопределено Тогда
			...
			Проверка_ОбщиеРеквизиты(НайденныйРеквизит, Свойство_НеИспользовать, Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                     РезультатЛокальный);
		КонецЕсли;

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
		...
		Если НайденныйРеквизит <> Неопределено Тогда
			...
			Проверка_ОбщиеРеквизиты(НайденныйРеквизит, Свойство_Использовать  , Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                     РезультатЛокальный);
		КонецЕсли;

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
#Область Проверка_ПраваРолей
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Проверка_Права(РольАС, "Редактирование", Ложь,   Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                 РезультатЛокальный);
		...
		Проверка_Права(РольАС, "Чтение", Истина, Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                РезультатЛокальный);

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки прав для роли АдминистраторСистемы:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
		...
		Проверка_Права(РольПП, "Редактирование", Ложь  , Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                 РезультатЛокальный);
		...
		Проверка_Права(РольПП, "Чтение", Истина, Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                 РезультатЛокальный);

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки прав для роли ПолныеПрава:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
#Область Проверка_Переопределения_ИдентификаторКонфигурации
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда

		РезультатЛокальный = Новый Массив;
		ИмяКонфигурации = ОбработкаНовостейПовтИсп.ПолучитьЗначениеПредопределеннойКатегории("ИмяКонфигурации"); // Только имя продукта
		Если ВРег(ИмяКонфигурации) = ВРег("Unknown") Тогда
			РезультатЛокальный.Добавить(НСтр("ru='Не настроен идентификатор конфигурации.
		...
		Если СписокЗапрещенныхСимволов.Количество() > 0 Тогда
			РезультатЛокальный.Добавить(НСтр("ru='Неправильно настроен идентификатор конфигурации.
				|Для его настройки необходимо внести изменения в общий модуль
				|ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,
				|для строки условия ProductName.
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки кода конфигурации:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
#Область Проверка_Переопределения_ВерсияКонфигурации
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если МассивСлов.Количество() <> 4 Тогда
			РезультатЛокальный.Добавить(НСтр("ru='Неправильно настроена версия конфигурации (количество чисел отлично от 4).
		...
		Иначе
			...
			Если НайденныеНеправильныеСимволы.Количество() > 0 Тогда
				...
				СНайденныеНеправильныеСимволы = СтрЗаменить(СНайденныеНеправильныеСимволы, "#", "");
				РезультатЛокальный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Неправильно настроена версия конфигурации (есть неправильные символы).
							|В метаданных, в поле Версия должны быть данные вида А.Б.В.Г (четыре числа, разделенные точкой).
							|Если по каким-то причинам версия в метаданных задана в отличном от А.Б.В.Г формате,
							|то необходимо внести изменения в общий модуль
							|ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,
							|для строки условия ProductVersion.
							|Список неправильных символов: %1'"),
						СНайденныеНеправильныеСимволы));
			КонецЕсли;
		КонецЕсли;

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки версии конфигурации:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
#КонецОбласти
	...
	// 5. Проверить правильность настройки ИсторияДанных и прав для ролей АдминистраторСистемы и ПолныеПрава.
#Область Проверка_ИсторияДанных
	Если ВариантПроверки = "Все" Тогда

		ПраваИсторииДанных = Новый Массив;
			ПраваИсторииДанных.Добавить("ЧтениеИсторииДанных");
			ПраваИсторииДанных.Добавить("ПросмотрИсторииДанных");
			ПраваИсторииДанных.Добавить("ЧтениеИсторииДанныхОтсутствующихДанных"); // Для констант - отсутствует
			ПраваИсторииДанных.Добавить("ИзменениеИсторииДанныхОтсутствующихДанных"); // Для констант - отсутствует
			ПраваИсторииДанных.Добавить("ИзменениеИсторииДанных");
			ПраваИсторииДанных.Добавить("ИзменениеНастроекИсторииДанных");
			ПраваИсторииДанных.Добавить("ИзменениеКомментарияВерсииИсторииДанных");
			ПраваИсторииДанных.Добавить("РедактированиеКомментарияВерсииИсторииДанных");
			ПраваИсторииДанных.Добавить("ПереходНаВерсиюИсторииДанных");
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Ложь);
			КонтекстВыполнения.ВходящиеПараметры.Вставить("РольАС"               , Метаданные.Роли.АдминистраторСистемы);
			КонтекстВыполнения.ВходящиеПараметры.Вставить("РольПП"               , Метаданные.Роли.ПолныеПрава);
			КонтекстВыполнения.ВходящиеПараметры.Вставить("ПраваИсторииДанных"   , ПраваИсторииДанных);
			КонтекстВыполнения.ВходящиеПараметры.Вставить("ИсторияДанныхВключена", Метаданные.СвойстваОбъектов.ИспользованиеИсторииДанных.Использовать);

		РезультатЛокальный = Новый Массив;
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Константы.НастройкиНовостей,                                     РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Константы.РазрешенаРаботаСНовостями,                             РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Константы.РазрешенаРаботаСНовостямиЧерезИнтернет,                РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Справочники.ЗначенияКатегорийНовостей,                           РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Справочники.ЛентыНовостей,                                       РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.Справочники.Новости,                                             РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.ПланыВидовХарактеристик.КатегорииНовостей,                       РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ДатыЗагрузкиПоследнихНовостей,                  РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей,       РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ЗначенияКатегорийОбщие,                         РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ЗначенияКатегорийДляОбластиДанных,              РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ЗначенияКатегорийПользовательские,              РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.КатегорииНовостейИнтервалыВерсий,               РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные, РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.КатегорииНовостейПростые,                       РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.КатегорииНовостейПростыеНеобработанные,         РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.НастройкиНовостейОбластиДанных,                 РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.НастройкиПользователейБИП,                      РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ОтборыПоЛентамНовостейОбщие,                    РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ОтборыПоЛентамНовостейДляОбластиДанных,         РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ОтборыПоЛентамНовостейПользовательские,         РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ОтключенныеЛентыНовостей,                       РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ОтключенныеПанелиКонтекстныхНовостей,           РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ПериодическиеСвойстваНовостей,                  РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.ПривязкаНовостейКМетаданным,                    РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.РазрешенныеДляРедактированияЛентыНовостей,      РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_Общие,             РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_ДляОбластиДанных,  РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_Пользовательские,  РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся,   РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.СвойстваНовостей,                               РезультатЛокальный);
		Проверка_ИсторияДанных(КонтекстВыполнения, Метаданные.РегистрыСведений.СостоянияНовостей,                              РезультатЛокальный);

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru='Ошибки настройки истории данных:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
Процедура Проверка_ОбщиеРеквизиты(
	...
	Если НайденныйЭлемент <> Неопределено Тогда
		...
		Иначе
			...
			Результат.Добавить(ТекущийРезультат);
		КонецЕсли;
	Иначе
...
//  Результат                 - Массив из Строка - сюда будет дописан текст ошибки.
//
Процедура Проверка_ИсторияДанных(
			КонтекстВыполнения,
			ЛокальныйОбъектМетаданных,
			Результат)

	РольАС                = КонтекстВыполнения.ВходящиеПараметры.РольАС;
	РольПП                = КонтекстВыполнения.ВходящиеПараметры.РольПП;
	ПраваИсторииДанных    = КонтекстВыполнения.ВходящиеПараметры.ПраваИсторииДанных;
	ИсторияДанныхВключена = КонтекстВыполнения.ВходящиеПараметры.ИсторияДанныхВключена;

	// Если хотя бы где-то включена галочка или есть право на историю данных, то это ошибка.
	// 1. История данных
	Если ЛокальныйОбъектМетаданных.ИсторияДанных = ИсторияДанныхВключена Тогда
		Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У объекта метаданных [%1] включена работа с историей данных. Должна быть отключена.'"),
			ЛокальныйОбъектМетаданных));
	КонецЕсли;
	// 2. Права
	Для Каждого ТекущееПраво Из ПраваИсторииДанных Цикл
		// У констант отсутствуют права "ЧтениеИсторииДанныхОтсутствующихДанных" и "ИзменениеИсторииДанныхОтсутствующихДанных".
		Если Метаданные.Константы.Содержит(ЛокальныйОбъектМетаданных) Тогда
			Если ТекущееПраво = "ЧтениеИсторииДанныхОтсутствующихДанных" Тогда
				Продолжить;
			КонецЕсли;
			Если ТекущееПраво = "ИзменениеИсторииДанныхОтсутствующихДанных" Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		ТекущееЗначениеПрава = ПравоДоступа(ТекущееПраво, ЛокальныйОбъектМетаданных, РольАС);
		Если ТекущееЗначениеПрава = Истина Тогда
			Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У объекта метаданных [%1] для роли [%2] включено право [%3]. Должно быть отключено.'"),
				ЛокальныйОбъектМетаданных,
				РольАС,
				ТекущееПраво));
		КонецЕсли;
		ТекущееЗначениеПрава = ПравоДоступа(ТекущееПраво, ЛокальныйОбъектМетаданных, РольПП);
		Если ТекущееЗначениеПрава = Истина Тогда
			Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У объекта метаданных [%1] для роли [%2] включено право [%3]. Должно быть отключено.'"),
				ЛокальныйОбъектМетаданных,
...
Функция ВыгрузитьВсеСобытияЖурналаРегистрации(ПараметрыОтбора, ПараметрыФайлаВыгрузки = Неопределено) Экспорт
	...
	ПараметрыФайлаВыгрузкиЭталон = Новый Структура;
		ПараметрыФайлаВыгрузкиЭталон.Вставить("Архивировать", Ложь);
	Если ТипЗнч(ПараметрыФайлаВыгрузки) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыФайлаВыгрузкиЭталон, ПараметрыФайлаВыгрузки);
	КонецЕсли;
	ПараметрыФайлаВыгрузкиЭталон.Архивировать = ?(ПараметрыФайлаВыгрузкиЭталон.Архивировать = Истина, Истина, Ложь);
	...
	Попытка
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если ФайлВременный.Существует() Тогда
				Если ПараметрыФайлаВыгрузкиЭталон.Архивировать = Истина Тогда
	...
	Если ТипЗнч(ОбъектХДТО) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:БазовыйФункционал.Отладка'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО'", КодОсновногоЯзыка), // ИдентификаторШага.
...
//
Функция Проверка_РазрешенаРаботаСНовостями(КонтекстВыполнения)

	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:РазрешенаРаботаСНовостями'")); // Идентификатор.
	Результат = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями();

	Если Результат = Ложь Тогда
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.NEWS_SUBSYSTEM_OFF", НСтр("ru='Работа с новостями запрещена'"));
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru='Работа с новостями разрешена'"));
	КонецЕсли;
...
//
Функция Проверка_МонопольныйРежимОтключен(КонтекстВыполнения)

	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru='Проверка:МонопольныйРежим'")); // Идентификатор.
	Результат = МонопольныйРежим();
	...
	Если Результат = Истина Тогда
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.EXCLUSIVE_MODE_ON", НСтр("ru='Информационная база работает в монопольном режиме.'"));
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru='Информационная база работает в обычном (НЕ монопольном) режиме.'"));
	КонецЕсли;
...
//
Функция Проверка_ЭтоОбычныйПользователь(КонтекстВыполнения)

	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:Пользователи.ЭтоСеансВнешнегоПользователя"); // Идентификатор.
	Результат = Пользователи.ЭтоСеансВнешнегоПользователя();

	Если Результат = Истина Тогда
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.EXTERNAL_USER_PROHIBITED",
			НСтр("ru='Работа с новостями не поддерживается для внешнего пользователя.'"));
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Предупреждение; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru='Это обычный пользователь.'"));
	КонецЕсли;
...
Функция КомментарийДляЖурналаРегистрации(ОписаниеШага, ШагВыполнения, КонтекстВыполнения, Вариант = "Простой") Экспорт
	...
	Если Вариант = "Расширенный" Тогда
		...
		ТекстСообщения = СтрЗаменить(
			ТекстСообщения,
			"%РезультатПоШагам%",
			ОбработкаНовостейКлиентСервер.ПолныйЛогШаговВыполнения(КонтекстВыполнения, 0));
...
Процедура ПолучитьДополнительныеСтандартныеЗначенияКлассификаторов(ИмяОбъектаМетаданных, СтандартныеЗначения) Экспорт
	...
	ИнтеграцияПодсистемБИП.ПолучитьДополнительныеСтандартныеЗначенияКлассификаторов(ИмяОбъектаМетаданных, СтандартныеЗначения);
...
Процедура ПолучитьДополнительныеСтандартныеЗначенияНовостей(ЛентаНовостей, СтандартныеЗначения) Экспорт
	...
	ИнтеграцияПодсистемБИП.ПолучитьДополнительныеСтандартныеЗначенияНовостей(ЛентаНовостей, СтандартныеЗначения);
...
Процедура ДополнительноОбработатьКлассификаторПослеПолученияПослеЗаписи(КлассификаторСсылка) Экспорт
	...
	ТипЗначенияКатегорийНовостей = Тип("СправочникСсылка.ЗначенияКатегорийНовостей");

	// 1. Основные действия.

	Если ТипЗнч(КлассификаторСсылка) = ТипКатегорияНовостей Тогда

		// 1.1. Возможна ситуация, когда происходит задвоение категории новостей.
		// Если категория новостей загружена с сервера и вызван этот метод,
		//  то считаем, что именно это - правильная категория новостей.
		//  Остальные категории новостей с таким же кодом "вычищаем".
		ИсправитьЗадвоенияКатегорийНовостейПослеЗаписи(КлассификаторСсылка);

	ИначеЕсли ТипЗнч(КлассификаторСсылка) = ТипЗначенияКатегорийНовостей Тогда

		// 1.2. Возможна ситуация, когда происходит задвоение значений категории новостей.
		//  Если значение категории новостей загружено с сервера и вызван этот метод,
		//   то считаем, что именно это - правильное значение категории новостей.
		//   Остальные значения категорий новостей с таким же кодом "вычищаем".
		ИсправитьЗадвоенияЗначенийКатегорийНовостейПослеЗаписи(КлассификаторСсылка);

	КонецЕсли;

	// 2. Переопределение.
#Область Переопределение
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьКлассификаторПослеПолученияПослеЗаписи(КлассификаторСсылка);
...
//  КлассификаторСсылка - СправочникСсылка - Классификатор, который был только что записан.
//
Процедура ИсправитьЗадвоенияКатегорийНовостейПослеЗаписи(КлассификаторСсылка)
...
#Область ПоискНеправильныхКатегорий
	...
	ПравильнаяКатегорияКод    = КлассификаторСсылка.Код;
	ПравильнаяКатегорияСсылка = КлассификаторСсылка;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		...
		|	И ПВХ.Ссылка <> &КатегорияСсылка
		|";
	Запрос.УстановитьПараметр("КатегорияСсылка", ПравильнаяКатегорияСсылка);
	Запрос.УстановитьПараметр("КатегорияКод"   , ПравильнаяКатегорияКод);
				...
				Если НеправильныеКатегории.Найти(ТекущаяДоступнаяКатегория.КатегорияНовостей) <> Неопределено Тогда
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							...
							ПравильнаяКатегорияСсылка,
							СокрЛП(ПравильнаяКатегорияСсылка.УникальныйИдентификатор())));
					ТекущаяДоступнаяКатегория.КатегорияНовостей = ПравильнаяКатегорияСсылка;
				КонецЕсли;
			КонецЦикла;
			Попытка
				ЛогШаговВыполнения.Добавить(
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			Запись = РегистрыСведений[ОбъектМетаданных].СоздатьМенеджерЗаписи();
				Запись.Новость                   = Выборка.Новость;
				Запись.КатегорияНовостей         = ПравильнаяКатегорияСсылка;
...
//  КлассификаторСсылка - СправочникСсылка - Классификатор, который был только что записан.
//
Процедура ИсправитьЗадвоенияЗначенийКатегорийНовостейПослеЗаписи(КлассификаторСсылка)

	РазделениеВключено = ОбщегоНазначения.РазделениеВключено();
	КодОсновногоЯзыка  = ОбщегоНазначения.КодОсновногоЯзыка();

	// Надо обрабатывать?
	Если (КлассификаторСсылка.ПометкаУдаления = Истина) Тогда
		Возврат;
	КонецЕсли;

	// 1.2.1. Найти значения категорий новостей с таким же кодом, но с другой ссылкой - это ошибочные значения категорий.
#Область ПоискНеправильныхЗначенийКатегорий

	НеправильныеЗначенияКатегорий     = Новый Массив;
	ПравильноеЗначениеКатегорииКод    = КлассификаторСсылка.Код;
	ПравильноеЗначениеКатегорииСсылка = КлассификаторСсылка;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	Спр.Ссылка КАК ЗначениеКатегории
		|ИЗ
		|	Справочник.ЗначенияКатегорийНовостей КАК Спр
		|ГДЕ
		|	Спр.Код = &ЗначениеКатегорииКод
		|	И Спр.Ссылка <> &ЗначениеКатегорииСсылка
		|";
	Запрос.УстановитьПараметр("ЗначениеКатегорииСсылка", ПравильноеЗначениеКатегорииСсылка);
	Запрос.УстановитьПараметр("ЗначениеКатегорииКод"   , ПравильноеЗначениеКатегорииКод);
	РезультатЗапроса = Запрос.Выполнить(); // АПК:1328, Конкурентная работа невозможна - выполняется уникальным регламентным заданием.
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			НеправильныеЗначенияКатегорий.Добавить(Выборка.ЗначениеКатегории);
		КонецЦикла;
	КонецЕсли;
...
#КонецОбласти
	...
	// Есть что обрабатывать?
	Если НеправильныеЗначенияКатегорий.Количество() <= 0 Тогда
		Возврат;
	КонецЕсли;

	// 1.2.2. Ошибочные значения категорий новостей пометить на удаление и изменить код.
#Область УдалениеНеправильныхЗначенийКатегорий

	Для Каждого ТекущееЗначениеКатегории Из НеправильныеЗначенияКатегорий Цикл
		ЛогШаговВыполнения = Новый Массив;
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Найдены дубликаты значений категории с кодом [%1]'", КодОсновногоЯзыка),
				ПравильноеЗначениеКатегорииКод));
		Объект = ТекущееЗначениеКатегории.ПолучитьОбъект();
			// Пометить на удаление, сгенерировать новый код, начинающийся с "DEL_00000_", но не более 20 символов (длина кода).
			НовыйКод = "";
			Для С=0 По 99999 Цикл // Найти первый незанятый код.
				НовыйКод = Лев("DEL_" + Формат(С, "ЧЦ=5; ЧН=00000; ЧВН=; ЧГ=0;") + "_" + Объект.Код, 20); // длина кода - 20 символов.
				Запрос.Текст = "
					...
					|	Спр.Код = &ЗначениеКатегорииКод
					|";
				Запрос.УстановитьПараметр("ЗначениеКатегорииКод", НовыйКод);
				РезультатЗапроса = Запрос.Выполнить();
				Если РезультатЗапроса.Пустой() Тогда
					// Найден незанятый код.
					Прервать;
		...
		Попытка
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Попытка записи значения категории новостей с повторяющимся кодом (замена кода и пометка удаления):
						|Новый код [%1] (%2)'", КодОсновногоЯзыка),
					НовыйКод,
					ТекущееЗначениеКатегории.УникальныйИдентификатор()));
			Объект.ОбменДанными.Загрузка = Истина;
			ЛогШаговВыполнения.Добавить(НСтр("ru='Блокировка объекта.'", КодОсновногоЯзыка));
			Объект.Заблокировать();
			ЛогШаговВыполнения.Добавить(НСтр("ru='Начало записи объекта.'", КодОсновногоЯзыка));
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ЗадвоенныеЗначенияКатегорий. УдалениеНеправильныхЗначенийКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			Метаданные.Справочники.ЗначенияКатегорийНовостей, // ОбъектМетаданных
			ТекущееЗначениеКатегории, // Данные
			СтрСоединить(ЛогШаговВыполнения, Символы.ПС), // Комментарий
			Истина); // ВестиПодробныйЖурналРегистрации - писать такие сообщения всегда.
	КонецЦикла;
...
#КонецОбласти
	...
	// 1.2.3. Заменить ошибочные значения категорий новостей на правильные.
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных

	Запрос = Новый Запрос;
	Запрос.Текст = "
		|// 0. КатегорииНовостейПростые.
		|ВЫБРАТЬ
		|	Рег.Новость                   КАК Новость,
		|	Рег.КатегорияНовостей         КАК КатегорияНовостей,
		|	Рег.ЗначениеКатегорииНовостей КАК ЗначениеКатегорииНовостей,
		...
		|	РегистрСведений.КатегорииНовостейПростые КАК Рег
		|ГДЕ
		|	Рег.ЗначениеКатегорииНовостей В (&НеправильныеЗначенияКатегорий)
		|;
		|
		|// 1. ЗначенияКатегорий*.
		|ВЫБРАТЬ
		...
		|
		|";
	Запрос.УстановитьПараметр("НеправильныеЗначенияКатегорий", НеправильныеЗначенияКатегорий);
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_КатегорииНовостейПростые
	// 0. КатегорииНовостейПростые.
	ОбъектМетаданных = "КатегорииНовостейПростые";
	РезультатЗапроса = РезультатыЗапроса[0];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
		// Добавить правильные значения категории.
		ЛогШаговВыполнения.Добавить(НСтр("ru='Добавление правильных значений категорий'", КодОсновногоЯзыка));
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			Запись = РегистрыСведений[ОбъектМетаданных].СоздатьМенеджерЗаписи();
				Запись.Новость                   = Выборка.Новость;
				Запись.КатегорияНовостей         = Выборка.КатегорияНовостей;
				Запись.ЗначениеКатегорииНовостей = ПравильноеЗначениеКатегорииСсылка; // Возможно расхождение владельца.
				Запись.УсловиеОтбора             = Выборка.УсловиеОтбора;
			Запись.Записать(Истина);
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Замена значения категории [%1] (%2) в [%3] для новости [%4], [%5].'", КодОсновногоЯзыка),
					Выборка.ЗначениеКатегорииНовостей,
					СокрЛП(Выборка.ЗначениеКатегорииНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных,
					Выборка.Новость,
					Выборка.Новость.УИННовости));
		КонецЦикла;
		// Удалить неправильные значения категорий.
		ЛогШаговВыполнения.Добавить(НСтр("ru='Удаление неправильных значений категорий'", КодОсновногоЯзыка));
		Для Каждого НеправильноеЗначениеКатегории Из НеправильныеЗначенияКатегорий Цикл
			НаборЗаписей = РегистрыСведений[ОбъектМетаданных].СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.ЗначениеКатегорииНовостей.Установить(НеправильноеЗначениеКатегории, Истина);
			НаборЗаписей.Записать(Истина); // Очистка.
		КонецЦикла;
		// Запись в журнал регистрации.
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru='Новости. Сервис и регламент. ЗадвоенныеЗначенияКатегорий. ЗаменаНеправильныхЗначенийКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений[ОбъектМетаданных], // ОбъектМетаданных
			, // Данные
			СтрСоединить(ЛогШаговВыполнения, Символы.ПС), // Комментарий
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ЗначенияКатегорийДляОбластиДанных
	// 1. РегистрСведений.ЗначенияКатегорийДляОбластиДанных.
	ОбъектМетаданных = "ЗначенияКатегорийДляОбластиДанных";
	РезультатЗапроса = РезультатыЗапроса[1];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Замена значения категории [%1] (%2) в [%3].'", КодОсновногоЯзыка),
					Выборка.ЗначениеКатегорииНовостей,
					СокрЛП(Выборка.ЗначениеКатегорииНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных));
		КонецЦикла;
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ЗначенияКатегорийОбщие
	// 2. РегистрСведений.ЗначенияКатегорийОбщие.
	ОбъектМетаданных = "ЗначенияКатегорийОбщие";
	РезультатЗапроса = РезультатыЗапроса[2];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ЗначенияКатегорийПользовательские
	// 3. РегистрСведений.ЗначенияКатегорийПользовательские.
	ОбъектМетаданных = "ЗначенияКатегорийПользовательские";
	РезультатЗапроса = РезультатыЗапроса[3];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ОтборыПоЛентамНовостейДляОбластиДанных
	// 4. РегистрСведений.ОтборыПоЛентамНовостейДляОбластиДанных.
	ОбъектМетаданных = "ОтборыПоЛентамНовостейДляОбластиДанных";
	РезультатЗапроса = РезультатыЗапроса[4];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Замена значения категории [%1] (%2) в [%3] для ленты новостей [%4].'", КодОсновногоЯзыка),
					Выборка.ЗначениеКатегорииНовостей,
					СокрЛП(Выборка.ЗначениеКатегорииНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных,
					Выборка.ЛентаНовостей));
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ОтборыПоЛентамНовостейОбщие
	// 5. РегистрСведений.ОтборыПоЛентамНовостейОбщие.
	ОбъектМетаданных = "ОтборыПоЛентамНовостейОбщие";
	РезультатЗапроса = РезультатыЗапроса[5];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ОтборыПоЛентамНовостейПользовательские
	// 6. РегистрСведений.ОтборыПоЛентамНовостейПользовательские.
	ОбъектМетаданных = "ОтборыПоЛентамНовостейПользовательские";
	РезультатЗапроса = РезультатыЗапроса[6];
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения = Новый Массив;
...
#Область ИнициализацияМетодаМетода
...
#Область Проверки
	...
	Если (Результат.ТекущееСостояние <> "Активна")
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ОбработкаНовостейСлужебный.УстановитьСостояниеПодсистемы: ожидается состояние подсистемы:
				| - Активна;
				| - ТребуетсяПересчетОтборовПослеДлительногоОжидания;
				| - СейчасЗагружаютсяНовости.
				|Передано значение: [%1].'", КодОсновногоЯзыка),
			СостояниеПодсистемы);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Функция ВсеНастройкиОперацийРегламентныхЗаданий() Экспорт
	...
	Возврат ОбработкаНовостейПовтИсп.ВсеНастройкиОперацийРегламентныхЗаданий();
...
Функция ПараметрыВыполненияОперацииРегламентныхЗаданий(Операция) Экспорт
	...
	ВсеНастройкиОперацийРегламентныхЗаданий = ВсеНастройкиОперацийРегламентныхЗаданий();

	Результат = ВсеНастройкиОперацийРегламентныхЗаданий.Получить(Операция);
...
Функция ПолучитьЧастьЗапросаНовостейИсключений() Экспорт
	...
	СтрокиЗапросаНовостейИсключений = Новый Массив;
	// {Блок запроса новостей-исключений. Начало
	СтрокиЗапросаНовостейИсключений.Добавить("
		...
		|");
	// Можно не реализовывать уникальность через "ОБЪЕДИНИТЬ", подойдет "ОБЪЕДИНИТЬ ВСЕ" т.к. проверяется вхождение новости в этот список.
	СтрокиЗапросаНовостейИсключений.Добавить(Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС);
	// Условие "Рег.РезультатОтбора = ЛОЖЬ" не нужно, т.к. в регистре и так хранятся только новости-исключения.
	СтрокиЗапросаНовостейИсключений.Добавить("
	...
	// Вариант 1: В конфигурации есть общие реквизиты с разделением и включена ФО РаботаВМоделиСервиса И если вошли в область данных.
	// Вариант 2: Программа не работает в модели сервиса, а работает в режиме коробки.
	// ... тогда добавить условия отборов по областям данных и по пользователям.
	...
	Результат = СтрСоединить(СтрокиЗапросаНовостейИсключений, Символы.ПС);
...
Процедура ЗаполнитьЗначенияАвтоматическихОтборов_Общие(КонтекстВыполнения = Неопределено)
	...
	АвтоматическийРасчет = Истина;
	ИнтеграцияПодсистемБИП.РазрешитьАвтоматическийРасчетЗначенийКатегорий(АвтоматическийРасчет);
	ОбработкаНовостейПереопределяемый.РазрешитьАвтоматическийРасчетЗначенийКатегорий(АвтоматическийРасчет);
	Если АвтоматическийРасчет <> Истина Тогда
...
#Область МножественноеЗначениеОтбора
		...
		МассивКодовКатегорий = Новый Массив;
		ИнтеграцияПодсистемБИП.ЗаполнитьКодыКатегорийДляАвтоматическогоРасчетаОтборовНовостей(
			...
			Пока Выборка.СледующийПоЗначениюПоля("КатегорияНовостей") Цикл
				...
				НаборЗаписей.Отбор["КатегорияНовостей"].Установить(ТекущаяКатегорияНовостей, Истина);
					...
					Пока Выборка.СледующийПоЗначениюПоля("ЗначениеКатегорииНовостей") Цикл
						...
						Иначе
							// Значения нет в кэше. В зависимости от настроек - рассчитать автоматически, или считать,
							//  что значение не заполнено (т.е. новость - показать).
							Если АвтоматическийРасчет = Ложь Тогда
								РезультатРасчета = Истина; // Новость - показать.
							Иначе
...
//
Процедура ЗаполнитьПрограммныеЗначенияКатегорий_Общие(КонтекстВыполнения = Неопределено)

#Область ИнициализацияМетода

	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗаполнитьПрограммныеЗначенияКатегорий_Общие"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_Общие
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ЗаполнитьПрограммныеЗначенияКатегорий_Общие"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;

		ОбработкаНовостейПереопределяемый.ЗаполнитьПрограммныеЗначенияКатегорий_Общие();
...
//
Процедура ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных(КонтекстВыполнения = Неопределено)

#Область ИнициализацияМетода

	НаименованиеМетода = "ОбработкаНовостейСлужебный.ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных"); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;

		ОбработкаНовостейПереопределяемый.ЗаполнитьПрограммныеЗначенияКатегорий_ДляОбластиДанных();
...
Процедура РассчитатьОтборПоКатегории(
	...
	ИнтеграцияПодсистемБИП.РассчитатьОтборПоКатегории(
		КатегорияНовостей,
		КодКатегорииНовостей,
		ЗначениеКатегорииНовостей,
		РезультатРасчета,
	...
	ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionOn"))
		...
		Исключение
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие'", КодОсновногоЯзыка), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
	...
	// Разделенная категория. Требуется вход в область данных.
	ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionDataAreaOn"))
		...
		Если ИспользоватьНомерОбластиДанных = Истина Тогда
			...
			Исключение
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
					НСтр("ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных'", КодОсновногоЯзыка), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
#Область ОптимизацияРаботыСОбъектами
...
Функция ПолучитьОписательЛентыНовостей(Ссылка) Экспорт
	...
	ТипСсылка = Тип("СправочникСсылка.ЛентыНовостей");
	ТипУИН    = Тип("УникальныйИдентификатор");

	ЛентаНовостейСсылка = Справочники.ЛентыНовостей.ПустаяСсылка();

	Результат = Новый Структура;
		Результат.Вставить("Ссылка"      , ЛентаНовостейСсылка);
		Результат.Вставить("ВерсияДанных", "");

	Если ТипЗнч(Ссылка) = ТипСсылка Тогда
		ЛентаНовостейСсылка = Ссылка;
	ИначеЕсли ТипЗнч(Ссылка) = ТипУИН Тогда
		// Может быть "Объект не найден". Проверять также Версию данных.
		ЛентаНовостейСсылка = Справочники.ЛентыНовостей.ПолучитьСсылку(Ссылка);
	Иначе
		Возврат Результат;
	КонецЕсли;

	Результат.Ссылка = ЛентаНовостейСсылка;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	Справочник.ЛентыНовостей КАК Спр
		|ГДЕ
		|	Спр.Ссылка = &ЛентаНовостейСсылка
		|";
	Запрос.УстановитьПараметр("ЛентаНовостейСсылка", ЛентаНовостейСсылка);
	...
	Результат.ВерсияДанных = Выборка.ВерсияДанных;
...
Функция ПолучитьОписательКатегорииНовостей(Ссылка) Экспорт
	...
	ТипСсылка = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей");
	ТипУИН    = Тип("УникальныйИдентификатор");

	КатегорияНовостейСсылка = ПланыВидовХарактеристик.КатегорииНовостей.ПустаяСсылка();

	Результат = Новый Структура;
		Результат.Вставить("Ссылка"      , КатегорияНовостейСсылка);
		Результат.Вставить("ВерсияДанных", "");

	Если ТипЗнч(Ссылка) = ТипСсылка Тогда
		КатегорияНовостейСсылка = Ссылка;
	ИначеЕсли ТипЗнч(Ссылка) = ТипУИН Тогда
		// Может быть "Объект не найден". Проверять также Версию данных.
		КатегорияНовостейСсылка = ПланыВидовХарактеристик.КатегорииНовостей.ПолучитьСсылку(Ссылка);
	Иначе
		Возврат Результат;
	КонецЕсли;

	Результат.Ссылка = КатегорияНовостейСсылка;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	КатегорииНовостей.Ссылка       КАК Ссылка,
		|	КатегорииНовостей.ВерсияДанных КАК ВерсияДанных
		|ИЗ
		|	ПланВидовХарактеристик.КатегорииНовостей КАК КатегорииНовостей
		|ГДЕ
		|	КатегорииНовостей.Ссылка = &КатегорияНовостейСсылка
		|";
	Запрос.УстановитьПараметр("КатегорияНовостейСсылка", КатегорияНовостейСсылка);
...
Функция ПолучитьОписательЗначенияКатегорийНовостей(Ссылка) Экспорт
	...
	ТипСсылка = Тип("СправочникСсылка.ЗначенияКатегорийНовостей");
	ТипУИН    = Тип("УникальныйИдентификатор");

	ЗначениеКатегорийНовостейСсылка = Справочники.ЗначенияКатегорийНовостей.ПустаяСсылка();

	Результат = Новый Структура;
		Результат.Вставить("Ссылка"      , ЗначениеКатегорийНовостейСсылка);
		Результат.Вставить("ВерсияДанных", "");

	Если ТипЗнч(Ссылка) = ТипСсылка Тогда
		ЗначениеКатегорийНовостейСсылка = Ссылка;
	ИначеЕсли ТипЗнч(Ссылка) = ТипУИН Тогда
		// Может быть "Объект не найден". Проверять также Версию данных.
		ЗначениеКатегорийНовостейСсылка = Справочники.ЗначенияКатегорийНовостей.ПолучитьСсылку(Ссылка);
	Иначе
		Возврат Результат;
	КонецЕсли;

	Результат.Ссылка = ЗначениеКатегорийНовостейСсылка;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	Спр.Ссылка       КАК Ссылка,
		...
		|	Справочник.ЗначенияКатегорийНовостей КАК Спр
		|ГДЕ
		|	Спр.Ссылка = &ЗначениеКатегорийНовостейСсылка
		|";
	Запрос.УстановитьПараметр("ЗначениеКатегорийНовостейСсылка", ЗначениеКатегорийНовостейСсылка);
