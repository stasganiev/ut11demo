...
// Возвращаемое значение:
//  Строка
Функция ИмяФормыПросмотраЭлектронногоДокумента(ЭтоВходящийЭДО) Экспорт
	Возврат ?(ЭтоВходящийЭДО,
		"Обработка.ИнтеграцияОблачногоЭДО.Форма.ПросмотрВходящегоДокумента",
		"Обработка.ИнтеграцияОблачногоЭДО.Форма.ПросмотрИсходящегоДокумента");
КонецФункции
...
Функция ПараметрыПросмотраЭлектронногоДокументаПоОбъектуУчета(ОбъектУчета, ПараметрыОткрытия) Экспорт
	...
	Если ЗначениеЗаполнено(ОписанияДокументовОбъектаУчета) Тогда
		Организация = ОписанияДокументовОбъектаУчета[0].Организация;
	ИначеЕсли ЗначениеЗаполнено(РеквизитыАктуальныхДокументов) Тогда 
		Организация = РеквизитыАктуальныхДокументов[0].Организация;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Организация)
		ИЛИ Не ОрганизацияПодключена(Организация) Тогда
	...
	ИначеЕсли РежимПросмотра = РежимМножественногоПросмотра() Тогда
		...
		ПараметрыПросмотраСтатусов.ЭтоОблачныйЭДО = Истина;
		ПараметрыПросмотраСтатусов.АдресДанных = ПоместитьВоВременноеХранилище(ОписанияДокументовОбъектаУчета,
	...
	ИначеЕсли РежимПросмотра = РежимПредварительногоПросмотра() Тогда
		...
		ПредварительныйПросмотр = ИнтерфейсДокументовЭДО.НовыеПараметрыПредварительногоПросмотраЭлектронногоДокумента();
		
		РезультатПроверки = ИнтерфейсДокументовЭДО.ПроверитьГотовностьОбъектаУчетаКДокументообороту(ОбъектУчета);
		ПредварительныйПросмотр.РезультатПроверкиГотовности = РезультатПроверки;
		Если РезультатПроверки.Успех Тогда
			ПредварительныйПросмотр.РезультатПодготовкиДанныхОблачногоЭДО = Новый Структура("АдресДанных",
				ПоместитьВоВременноеХранилище(ОписанияДокументовОбъектаУчета, Новый УникальныйИдентификатор));
			ПараметрыПросмотра.ПредварительныйПросмотр = ПредварительныйПросмотр;
		КонецЕсли;
...
//  ЭлектронныйДокументВПакете - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
// 
// Возвращаемое значение:
//  Структура:
...
Функция СформироватьИДобавитьПроизвольныйДокументВПакет(ОписаниеДокументаЭДО, ОписаниеФайла, ЭлектронныйДокументВПакете) Экспорт
	...
	ПараметрыФайлаВФорматеСервиса = НовыеПараметрыФайлаДляФормированияВФорматеСервиса();
	ПараметрыФайлаВФорматеСервиса.ИмяФайла = ОписаниеФайла.ИмяФайла;
	ПараметрыФайлаВФорматеСервиса.Данные = Base64Строка(ОписаниеФайла.ДвоичныеДанные);
	
	НаборОписанийДокументовЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеДокументаЭДО);
	КлючиНастроек = КлючиНастроекВФорматеСервиса(НаборОписанийДокументовЭДО);
	КлючНастройки = КлючиНастроек[0];
	...
	ДанныеДляФормирования.Идентификатор = ОписаниеДокументаЭДО.Идентификатор;
	ДанныеДляФормирования.Номер = ОписаниеДокументаЭДО.Номер;
	ДанныеДляФормирования.Дата = ОписаниеДокументаЭДО.Дата;
	ДанныеДляФормирования.Сумма = ОписаниеДокументаЭДО.Сумма;
	ДанныеДляФормирования.ОсновнойФайл = ПараметрыФайлаВФорматеСервиса;
	
	ДанныеДокументовЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДляФормирования);
	
	ИдентификаторДокументаВПакете = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокументВПакете,
		"ИдентификаторДокумента");
	...
	ОписаниеМетода = СервисОблачногоЭДО.ОписаниеМетодаДобавитьВПакетКДокументуЭДО(ИдентификаторДокументаВПакете,
		ОписаниеДокументаЭДО.Идентификатор);
	СервисОблачногоЭДО.ДобавитьОписаниеМетодаВОперацию(ОперацияСервиса, ОписаниеМетода);
	...
	РезультатФормирования = РезультатыМетодов[0]; // см. СервисОблачногоЭДО.НовыйРезультатМетодаСформироватьОтветныеТитулыДокументовЭДО
	
	Если ЗначениеЗаполнено(РезультатФормирования.Ошибки) Тогда
		Результат.ОшибкиФормирования = РезультатФормирования.Ошибки;
		Возврат Результат;
	КонецЕсли;
	
	ОписанияСостоянийЭДО = ОписанияСостоянийДокументовИзФорматаСервиса(РезультатФормирования.СостоянияДокументовЭДО);
	ОписаниеСостоянияЭДО = ОписанияСостоянийЭДО[0];
	
	ОбновитьСостояниеДокументаЭДО(ОписаниеСостоянияЭДО);
	
	Результат.ЭлектронныйДокумент = ОписанияСостоянийЭДО[0].ЭлектронныйДокумент;
	
	РезультатДобавленияВПакет = СервисОблачногоЭДО.НовыйРезультатДобавленияВПакетКДокументуЭДО();
	РезультатДобавленияВПакет = РезультатыМетодов[1]; // см. СервисОблачногоЭДО.НовыйРезультатДобавленияВПакетКДокументуЭДО
	
	Результат.ИдентификаторПакета = РезультатДобавленияВПакет.ИдентификаторПакета;
...
Функция ОбновитьДокументЭДО(ОписаниеДокументаЭДО) Экспорт
...
Процедура ОбработатьРезультатМетодаПолученияИзмененныхДокументыЭДО(ИдентификаторУчетнойЗаписи, РезультатМетода, РезультатДействий)
	...
	ДатаСинхронизации = ПрочитатьДатуJSON(РезультатМетода.ДатаОкончанияВыборки, ФорматДатыJSON.ISO);
...
Процедура ОбработатьРезультатМетодаИзмененияПравилВизуализацииДокументовЭДО(ИдентификаторУчетнойЗаписи, РезультатМетода)
	...
	Если ЗначениеЗаполнено(ДатыОбновленияПравил) Тогда
		...
		Для Каждого ДатаОбновленияПравила Из ДатыОбновленияПравил Цикл
			ДатаОбновления = ПрочитатьДатуJSON(ДатаОбновленияПравила.ДатаОбновления, ФорматДатыJSON.ISO);
...
// Параметры:
//  СостояниеДокументаВФорматаСервиса - см. СервисОблачногоЭДО.НовыйРезультатПолученияСостоянияДокументаЭДО
// 
// Возвращаемое значение:
...
Функция СостояниеДокументаИзФорматаСервиса(СостояниеДокументаВФорматаСервиса)
	...
	СостояниеДокументаПолное.ДатаИзменения = ПрочитатьДатуJSON(СостояниеДокументаВФорматаСервиса.ДатаИзменения,
		ФорматДатыJSON.ISO);
...
Функция РежимПросмотраЭлектронногоДокументаПоОбъектуУчета(ОписанияДокументовОбъектаУчета, РеквизитыАктуальныхДокументов)
	...
	Если ОписаниеДокументаПоОбъектуУчета.Организация = РеквизитыАктуальногоДокумента.Организация
		...
		И (Не УказанДоговор ИЛИ ОписаниеДокументаПоОбъектуУчета.Договор = РеквизитыАктуальногоДокумента.ДоговорКонтрагента)
...
Функция ВизуализацияДокументаИзФорматаСервиса(СвойстваВизуализацииВФорматеСервиса)
	...
	Если ЗначениеЗаполнено(СвойстваВизуализацииВФорматеСервиса.ДатаОбновленияПравила) Тогда
		СвойстваВизуализации.ДатаОбновленияПравила = ПрочитатьДатуJSON(
			СвойстваВизуализацииВФорматеСервиса.ДатаОбновленияПравила, ФорматДатыJSON.ISO);
	КонецЕсли;
...
Функция ОписанияУстановленныхПодписейИзФорматаСервиса(ОписанияПодписейВФорматеСервиса)
	...
	Для Каждого ОписаниеПодписиВФорматеСервиса Из ОписанияПодписейВФорматеСервиса Цикл
		...
		ОписаниеПодписи.ДатаПроверки = ПрочитатьДатуJSON(ОписаниеПодписиВФорматеСервиса.ДатаПроверки, ФорматДатыJSON.ISO);
		ОписаниеПодписи.ДатаПодписи = ПрочитатьДатуJSON(ОписаниеПодписиВФорматеСервиса.ДатаПодписи, ФорматДатыJSON.ISO);
...
//  * ИмяФайла - Строка
//  * Данные - Строка
Функция НовыеПараметрыФайлаДляФормированияВФорматеСервиса()
	Параметры = Новый Структура;
...
Функция ДоговорыИзФорматаСервиса(ДоговорыВФорматеСервиса, ОрганизацииПоИдентификаторам, КонтрагентыПоИдентификаторам)
	...
	Для Каждого ДоговорВФорматеСервиса Из ДоговорыВФорматеСервиса Цикл
		...
		Дата = ПрочитатьДатуJSON(ДоговорВФорматеСервиса.Дата, ФорматДатыJSON.ISO);
		...
		РеквизитыДоговора.Дата = ПрочитатьДатуJSON(ДоговорВФорматеСервиса.Дата, ФорматДатыJSON.ISO);
...
Функция ОписанияДокументовИзФорматаСервиса(ОписанияДокументовВФорматеСервиса)
	...
	Для Каждого ОписаниеДокументаВФорматеСервиса Из ОписанияДокументовВФорматеСервиса Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеДокументаВФорматеСервиса.Дата) Тогда
			ОписаниеДокументаЭДО.Дата = ПрочитатьДатуJSON(ОписаниеДокументаВФорматеСервиса.Дата, ФорматДатыJSON.ISO);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеДокументаВФорматеСервиса.ДатаИзмененияСостояния) Тогда
			ОписаниеДокументаЭДО.ДатаИзмененияСостояния = ПрочитатьДатуJSON(
				ОписаниеДокументаВФорматеСервиса.ДатаИзмененияСостояния, ФорматДатыJSON.ISO);
		КонецЕсли;
...
Функция ОписанияСостоянийДокументовИзФорматаСервиса(ОписанияСостоянийВФорматеСервиса, ДокументыПоИдентификаторам = Неопределено)
	...
	Для Каждого ОписаниеСостоянияВФорматеСервиса Из ОписанияСостоянийВФорматеСервиса Цикл
		...
		ОписаниеСостоянияЭДО.ДатаИзменения = ПрочитатьДатуJSON(ОписаниеСостоянияВФорматеСервиса.ДатаИзменения,
			ФорматДатыJSON.ISO);
...
Функция ПриглашенияИзФорматаСервиса(ПриглашенияВФорматеСервиса)
	...
	Для Каждого ПриглашениеВФорматеСервиса Из ПриглашенияВФорматеСервиса Цикл
		...
		Приглашение.ДатаИзмененияСтатуса = ПрочитатьДатуJSON(ПриглашениеВФорматеСервиса.ДатаИзмененияСтатуса,
			ФорматДатыJSON.ISO);
...
Функция СтатусыПриглашенийИзФорматаСервиса(СтатусыПриглашенийВФорматеСервиса)
	...
	Для Каждого СтатусПриглашенияВФорматеСервиса Из СтатусыПриглашенийВФорматеСервиса Цикл
		...
		СтатусПриглашения.ДатаИзмененияСтатуса = ПрочитатьДатуJSON(СтатусПриглашенияВФорматеСервиса.ДатаИзмененияСтатуса,
			ФорматДатыJSON.ISO);
