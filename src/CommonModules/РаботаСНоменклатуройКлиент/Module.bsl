...
//
Процедура ПолучитьДанныеСертификатаСоответствияИзСервиса(Номер, ДатаРегистрации, ДатаОкончанияДействия, ФормаВладелец,
	ОповещениеФормы, ВыводитьОкноОжидания = Истина) Экспорт
	
	КонтекстПроверки = "РаботаСНоменклатуройКлиент.ПолучитьДанныеСертификатаСоответствияИзСервиса";
	...
	ВидДокумента = РаботаСНоменклатуройСлужебныйКлиентСервер.ВидДокументаСертификат();
	
	ДанныеДокументаАккредитацииВФоне(Номер, ДатаРегистрации, ДатаОкончанияДействия, ФормаВладелец, 
		ОповещениеФормы, ВыводитьОкноОжидания, ВидДокумента);
...
//  ВыводитьОкноОжидания - Булево - одноименный параметр из см. ДлительныеОперацииКлиент.ПараметрыОжидания
//
Процедура ПолучитьДанныеДекларацииСоответствияИзСервиса(Номер, ДатаРегистрации, ДатаОкончанияДействия, ФормаВладелец,
	ОповещениеФормы, ВыводитьОкноОжидания = Истина) Экспорт
	
	КонтекстПроверки = "РаботаСНоменклатуройКлиент.ПолучитьДанныеДекларацииСоответствияИзСервиса";
	РаботаСНоменклатуройСлужебныйКлиентСервер.ПроверитьВходящиеПараметрыДокументаАккредитации(Номер, 
		ДатаРегистрации, ДатаОкончанияДействия, КонтекстПроверки);
	
	ВидДокумента = РаботаСНоменклатуройСлужебныйКлиентСервер.ВидДокументаДекларация();
	
	ДанныеДокументаАккредитацииВФоне(Номер, ДатаРегистрации, ДатаОкончанияДействия, ФормаВладелец, ОповещениеФормы, 
		ВыводитьОкноОжидания, ВидДокумента);
...
Процедура ДанныеДокументаАккредитацииВФоне(Номер, ДатаРегистрации, ДатаОкончанияДействия, ФормаВладелец, 
		ОповещениеФормы, ВыводитьОкноОжидания, ВидДокумента)
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ФормаВладелец);
	ПараметрыОжидания.ВыводитьОкноОжидания = ВыводитьОкноОжидания;
	ПараметрыОжидания.ВыводитьПрогрессВыполнения = ВыводитьОкноОжидания;
	
	ДлительнаяОперация = РаботаСНоменклатуройСлужебныйВызовСервера.ДанныеДокументаАккредитацииВФоне(
		ФормаВладелец.УникальныйИдентификатор, Номер, ДатаРегистрации, ДатаОкончанияДействия, ВидДокумента);
	
	Если ТипЗнч(ДлительнаяОперация) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось запустить операцию получения данных ФСА'");
	КонецЕсли;
	
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		ПослеПолученияДанныхДокументаАккредитации(ДлительнаяОперация, ОповещениеФормы);
	Иначе
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеПолученияДанныхДокументаАккредитации", ЭтотОбъект,
			ОповещениеФормы);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
	КонецЕсли;
...
Процедура ПослеПолученияДанныхДокументаАккредитации(Результат, ОповещениеФормы) Экспорт
	
	ДанныеДокумента = РаботаСНоменклатуройСлужебныйКлиентСервер.НовыеДанныеДокументаАккредитации();
	...
	Если Статус = "Ошибка" Тогда
		...
		ВыполнитьОбработкуОповещения(ОповещениеФормы, ДанныеДокумента);
		Возврат;
	ИначеЕсли Статус <> "Выполнено" Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеСертификата = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	ВыполнитьОбработкуОповещения(ОповещениеФормы, ДанныеСертификата);
