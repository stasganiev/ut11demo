...
Функция ДанныеАдресаОбъекта(Ссылка) Экспорт
	
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ЗемельныеУчасткиИС") Тогда
		ДанныеАдреса = ИнтеграцияЗЕРНО.ДанныеЗемельногоУчастка(Ссылка);
	Иначе
		...
		ДанныеАдреса.Вставить("КомментарийАдреса",
			ИнтеграцияИС.КомментарийКонтактнойИнформации(ДанныеАдреса.Адрес));
	КонецЕсли;
