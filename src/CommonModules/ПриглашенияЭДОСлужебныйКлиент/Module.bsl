...
//  См. ПриглашенияЭДОКлиентСервер.НовоеПриглашение
Функция НовоеПриглашение(Вид) Экспорт
	
	Возврат ПриглашенияЭДОКлиентСервер.НовоеПриглашение(Вид);
...
// Параметры:
//  РезультатВвода - Неопределено,Строка
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ПослеВводаПричиныОтказаОтПриглашения(РезультатВвода, Контекст) Экспорт
	...
	ПриглашенияЧерезОблачныйЭДО = Новый Массив;
	
	Приглашения = Контекст.Приглашения;
	КоличествоПриглашений = Приглашения.Количество();
	Пока КоличествоПриглашений > 0 Цикл
		КоличествоПриглашений = КоличествоПриглашений - 1;
		Приглашение = Приглашения[КоличествоПриглашений];
		...
		Если Приглашение.ЭтоОблачныйЭДО Тогда
			ПриглашенияЧерезОблачныйЭДО.Добавить(Приглашение);
			Приглашения.Удалить(КоличествоПриглашений);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ПриглашенияЧерезОблачныйЭДО) Тогда
		ВыполнитьДействиеСПриглашениямиЧерезОблачныйЭДО(ПриглашенияЧерезОблачныйЭДО, Контекст);
	Иначе
		ВыполнитьДействиеСПриглашениямиЧерезВстроенныйЭДО(Контекст);
	КонецЕсли;
...
//  Приглашения - Массив из см. ПриглашенияЭДОКлиент.НовоеВходящееПриглашение
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ВыполнитьДействиеСПриглашениямиЧерезОблачныйЭДО(Приглашения, Контекст)
	
	Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатДействияСПриглашениямиЧерезОблачныйЭДО",
		ЭтотОбъект, Контекст);
	
	Действие = Контекст.Действие;
	ДействияСПриглашениями = ДействияСПриглашениями();
	
	ОбщийМодульИнтеграцияОблачногоЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияОблачногоЭДОКлиент");
	
	Если Действие = ДействияСПриглашениями.Отправить Тогда
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент.ОтправитьПриглашения(Оповещение, Приглашения);
	ИначеЕсли Действие = ДействияСПриглашениями.Принять Тогда
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент.ПринятьПриглашения(Оповещение, Приглашения);
	ИначеЕсли Действие = ДействияСПриглашениями.Отклонить
		ИЛИ Действие = ДействияСПриглашениями.Отозвать Тогда
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент.ОтклонитьПриглашения(Оповещение, Приглашения);
	Иначе
		ВыполнитьОбработкуОповещения(Оповещение);
	КонецЕсли;
...
// Параметры:
//  Результат - см. ИнтеграцияОблачногоЭДОВызовСервера.РезультатДействияСПриглашениямиВФоне
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ОбработатьРезультатДействияСПриглашениямиЧерезОблачныйЭДО(Результат, Контекст) Экспорт
	
	Контекст.Обработано = Контекст.Обработано + Результат.Обработано;
...
// Параметры:
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ВыполнитьДействиеСПриглашениямиЧерезВстроенныйЭДО(Контекст)
	
	Если Не ЗначениеЗаполнено(Контекст.Приглашения) Тогда
		ЗавершитьВыполнениеДействияСПриглашениями(Контекст);
		Возврат;
	КонецЕсли;
	...
	СинхронизацияЭДОКлиент.АвторизоватьсяВСервисеЭлектронногоДокументооборота(
		СлужебноеОповещение, Контекст.КонтекстДиагностики, Контекст.УчетныеЗаписи, ПараметрыПодписки);
...
Процедура ОбработатьРезультатОтправкиПриглашений(Результат, Контекст) Экспорт
	...
	РезультатОповещения.Вставить("КоличествоОтправленных", Контекст.Обработано);
	...
	Если Результат <> Неопределено И Результат.Статус <> "Ошибка" Тогда
		...
		РезультатОповещения.КоличествоОтправленных = РезультатОповещения.КоличествоОтправленных
			+ РезультатОтправки.КоличествоОтправленных;
		РезультатОповещения.КонтекстДиагностики = РезультатОтправки.КонтекстДиагностики;
	КонецЕсли;
	
	Если РезультатОповещения.КоличествоОтправленных > 0 Тогда
		РезультатОповещения.Успех = Истина;
	КонецЕсли;
...
Процедура ОбработатьРезультатОтветаНаВходящиеПриглашения(Результат, Контекст) Экспорт
	...
	РезультатОповещения = Новый Структура;
	РезультатОповещения.Вставить("КоличествоОбработанных", Контекст.Обработано);
	РезультатОповещения.Вставить("Успех", Ложь);
	РезультатОповещения.Вставить("КонтекстДиагностики", Контекст.КонтекстДиагностики);
	...
	Иначе 
		РезультатОтправки = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		РезультатОповещения.КоличествоОбработанных = РезультатОповещения.КоличествоОбработанных
			+ РезультатОтправки.КоличествоОбработанных;
		РезультатОповещения.КонтекстДиагностики = РезультатОтправки.КонтекстДиагностики;
	КонецЕсли;
	
	Если РезультатОповещения.КоличествоОбработанных > 0 Тогда
		РезультатОповещения.Успех = Истина;
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Структура - Новый контекст выполнения действия с приглашениями:
//  * Приглашения - Массив из см. ПриглашенияЭДОКлиент.НовоеВходящееПриглашение
//  * Оповещение  - ОписаниеОповещения
//  * Форма  - ФормаКлиентскогоПриложения
//  * Действие - Строка
//  * ОбработчикДействия - Неопределено,ОписаниеОповещения
//  * ОбработчикРезультатаВыполненияДействия - Неопределено,ОписаниеОповещения
//  * УчетныеЗаписи - Массив из Строка
//  * ПриглашенияТребуетсяПричинаОтказа - Массив из см. КлючЗаписиПриглашения
//  * СообщениеПриОшибке - Строка -
//  * КонтекстДиагностики - см. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//  * УчетныеЗаписиБезДоступаКСертификатам - Массив из Строка
//  * Обработано - Число
Функция НовыйКонтекстВыполненияДействияСПриглашениями(Приглашения, Оповещение, Форма)
	...
	Контекст.Вставить("Обработано", 0);
