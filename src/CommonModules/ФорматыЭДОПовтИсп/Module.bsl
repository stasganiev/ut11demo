...
Функция ДоступныеТипыЭДВКарточкеЭлектронногоДокумента() Экспорт
	...
	ОбъектПакетаXDTO = ФабрикаXDTO.Пакеты.Получить(Метаданные.ПакетыXDTO.КарточкаПередаваемыхДанных.ПространствоИмен);
	Фасеты = ОбъектПакетаXDTO.Получить("Definition").Свойства.Получить("Type").Тип.Свойства.Получить("Name").Тип.Фасеты;
		
	Массив = Новый Массив;
	Для Каждого Фасет Из Фасеты Цикл
		Если Фасет.Вид = ВидФасетаXDTO.Перечисление Тогда
			Массив.Добавить(Фасет.Значение);
		КонецЕсли;
	КонецЦикла;
