...
//   Структура:
//    * ГлавныйФайл - ДвоичныеДанные
//    * ДополнительныйФайл - ДвоичныеДанные
//    * ГлавныйФайлПодписи - ТаблицаЗначений:
//    ** Имя - Строка
//    ** Путь - Строка
//    * ДополнительныйФайлПодписи - ТаблицаЗначений:
//    ** Имя - Строка
//    ** Путь - Строка
//   * Доверенности - Соответствие из КлючИЗначение:
//       ** Ключ - Строка - Имя файла
//       ** Значение - Структура:
//          *** ФайлПодписиМЧД - Строка
//          *** ИменаФайловПодписей - Массив из Строка
//          *** СвойстваМЧД - См. ТранспортныеКонтейнерыЭДО.ПрочитатьКарточкуМЧД
//   * ДоверенностиПоСсылке - Соответствие из КлючИЗначение:
//       ** Ключ - Строка - Имя файла
//       ** Значение - Структура:
//          *** ИменаФайловПодписей - Массив из Строка
//          *** СвойстваМЧД - См. ТранспортныеКонтейнерыЭДО.ПрочитатьКарточкуМЧД
//   * Путь - Строка
Функция СтруктураФайловЭД()
	...
	ТаблицаПодписей.Колонки.Добавить("Имя", ОписаниеТиповСтрока);
	ТаблицаПодписей.Колонки.Добавить("Путь", ОписаниеТиповСтрока);
	...
	СтруктураФайловЭД.Вставить("Путь", "");
	
	Возврат СтруктураФайловЭД;
...
Функция ДвоичныеДанныеТранспортногоКонтейнера(ДанныеОбъектов, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектов Цикл
		...
		// Сохраняем основной файл и дополнительный файл с подписями.
		СтруктураФайловЭД = СтруктураФайловЭД();
		СтруктураФайловЭД.Путь = Строка(Сч) + "/";
...
Функция СформироватьПрисоединенныйФайлТранспортногоКонтейнераОператораЭДО(ТранспортныйКонтейнер, ДанныеОбъектов,
	...
	//Получаем представление ссылки чтобы закэшировать, в ином случае можем ловить ошибку в транзакции
	Если ДанныеОбъектов.Количество() > 0 Тогда
		Представление = Строка(ДанныеОбъектов[0].Объект);
	КонецЕсли;
	...
	Исключение
		...
		Если ДанныеОбъектов.Количество() > 0 Тогда
			ДополнительныеПараметрыОшибки.Вставить("СсылкаНаОбъект", ДанныеОбъектов[0].Объект);
		КонецЕсли;
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(НСтр(
			...
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ТекстСообщения, ДополнительныеПараметрыОшибки);
...
Процедура ПодготовитьДанныеПоКарточке(ОписаниеОбъекта, ДеревоКарточки)
	...
	Если ЭтоИзвещениеОПолученииДляОператора(ТипЭлементаРегламента) Тогда
	...
	Если Не ОписаниеОбъекта.СпособОбмена = Перечисления.СпособыОбменаЭД.БыстрыйОбмен Тогда
		...
		Если РегламентыЭДО.ЭтоЭлементРегламентаОтправителя(ОписаниеОбъекта.ТипЭлементаРегламента)
		...
		Если СинхронизацияЭДО.ЭтоПрямойОбмен(ОписаниеОбъекта.СпособОбмена) Тогда
			...
			ВидДокумента = ОписаниеОбъекта.ВидДокумента;
...
Процедура ПодготовитьДанныеПоТранспортнойИнформации(ОписаниеОбъектов, ДанныеФайлов, ДеревоТранспортнойИнформации)
	...
	Для Каждого ОписаниеОбъекта Из ОписаниеОбъектов Цикл
		...
		СтрокаТаблицы.ИД = ОписаниеОбъекта.ИдентификаторДокументооборота;
...
Функция ПараметрыДокумента(ТипДокументаСтрокой, ТипРегламента, КодТранзакции, ДляДополнительныхДанных = Ложь) Экспорт
	...
	Параметры.ТипЭлементаРегламента = ПолучитьТипЭлементаРегламента(КодТранзакции, ТипДокументаСтрокой);
...
// Возвращает тип элемента регламента по коду транзакции и тпу документа.
//
// Параметры:
//   КодТранзакции - Строка
//   ТипДокумента - Строка
...
//
Функция ПолучитьТипЭлементаРегламента(КодТранзакции, ТипДокумента)
	...
	Если ОтфильтрованнаяТаблица.Количество() = 1 Тогда
		ТипЭлементаРегламента = ОтфильтрованнаяТаблица[0].ТипЭлементаРегламента;
	ИначеЕсли ОтфильтрованнаяТаблица.Количество() > 1 Тогда
		СрокаТаблицы = ОтфильтрованнаяТаблица.Найти(ТипДокумента, "ТипДокумента");
		Если СрокаТаблицы = Неопределено Тогда
			СрокаТаблицы = ОтфильтрованнаяТаблица.Найти("", "ТипДокумента");
		КонецЕсли;
		Если СрокаТаблицы <> Неопределено Тогда
...
Функция ТаблицаТиповЭлементовРегламента()
	...
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"MainDocumentRejectSendConfirmation", Перечисления.ТипыЭлементовРегламентаЭДО.УОУ_ПДО);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"MainDocumentRejectPostDateConfirmation", Перечисления.ТипыЭлементовРегламентаЭДО.УОУ_ПДП);		
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"CustomerTitlePostDateConfirmation", Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияПолучателя_ПДП);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"CustomerTitleSendConfirmation", Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияПолучателя_ПДО);
   	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"CustomerTitleReceiveNotice", Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияПолучателя_ИОП);	
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица,
		"CustomerTitle", Перечисления.ТипыЭлементовРегламентаЭДО.ДопСведенияПоРезультатамПриемки,
		"FormalizedDocumentOnAdditionalInformation");
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица, "CustomerInformationReceiveNotice",
...
Функция ТаблицаТиповРегламентов()
	...
	НоваяСтрока = Таблица.Добавить();
	НоваяСтрока.КодРегламента = "Formalized";
	НоваяСтрока.КодДокумента = "FormalizedAcceptanceOfMaterialValues";
	НоваяСтрока.ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях;
...
// Сохраняет подпись МЧД
//
// Параметры:
//  ОписаниеОбъекта - СтрокаТаблицыЗначений: см. СинхронизацияЭДО.НовыеДанныеОбъектов
//  АдресКаталога - см. ФайловаяСистема.СоздатьВременныйКаталог
//  СоответствиеФайлЭД - См.СтруктураФайловЭД
//  Подпись - см. СинхронизацияЭДО.НовоеОписаниеДанныхОбъекта
//  ИмяФайлаПодписиДокумента - Строка
Процедура СохранитьПодписьМЧД(ОписаниеОбъекта, АдресКаталога, СоответствиеФайлЭД, Подпись, ИмяФайлаПодписиДокумента)
	...
	Если МЧД = Неопределено Или МЧД.Подпись.ДвоичныеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если МЧД.ЭтоФайловаяДоверенность Тогда
		...
		ДекодированныеДвоичныеДанные = 
			ОбщегоНазначенияБЭДКлиентСервер.ДекодированныеДвоичныеДанные(МЧД.Подпись.ДвоичныеДанные);
		ДекодированныеДвоичныеДанные.Записать(АдресКаталога + ИмяФайлаПодписиМЧД);
	...
	Если ТипЗнч(СоответствиеФайлЭД) = Тип("Структура") Тогда
		...
		Иначе
			...
			Если ДанныеМЧД <> Неопределено Тогда
				ИменаФайловПодписей = ДанныеМЧД.ИменаФайловПодписей;
			КонецЕсли;
