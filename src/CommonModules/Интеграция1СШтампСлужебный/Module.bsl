...
Функция РезультатДобавленияШтамповВДокумент(Знач ПараметрыДобавленияШтампов, Знач ТаймаутПолученияСтатусаВыполненияЗаданияВСервисе = Неопределено) Экспорт
	...
	Результат.ИнформацияОбОшибках = ОтветСервиса;
	Результат.ДвоичныеДанные      = ОтветСервиса.Данные;
...
Функция ТелоЗапросаДобавитьШтампыВДокумент(ПараметрыДобавленияШтампов)
	...
	СловарьПравилРазмещения   = СловарьПравилРазмещения();
	...
	Для Каждого ПараметрыКартинкиШтампа Из ПараметрыДобавленияШтампов.ПараметрыШтампов Цикл
		...
		СтруктураСвойствШтампа.location.coordinates.top          = ПараметрыКартинкиШтампа.Размещение.Координаты.Верх;
		СтруктураСвойствШтампа.location.coordinates.left         = ПараметрыКартинкиШтампа.Размещение.Координаты.Лево;
		...
		СтруктураСвойствШтампа.location.position  = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			...
			ПараметрыКартинкиШтампа.Размещение.Расположение);
		
		СтруктураСвойствШтампа.addingRules        = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			СловарьПравилРазмещения,
			ПараметрыКартинкиШтампа.ПравилаРазмещения);
...
Функция ТипИсходящегоКонтентаДанныеДокументаИШтампов()
	...
	Возврат "application/vnd.1cstamp.stamp.v1+json";
...
//    * addingRules - Строка        - Значение правила размещения картинки штампа на страницах документа.
...
Функция НоваяСтруктураСвойствШтампаJSON()
	...
	СтруктураШтампа.Вставить("addingRules");
	СтруктураШтампа.Вставить("coefficient");
	СтруктураШтампа.Вставить("indentation");
	СтруктураШтампа.Вставить("settings");
	
	СвойстваШтампа = Новый Структура;
	
	СвойстваШтампа.Вставить("data");
...
Функция СловарьПравилРазмещения()
