...
// Определяет, используется ли сервис 1С:Штамп.
// 
// Возвращаемое значение:
// 	Булево - Если Истина, то использование сервиса включено
Функция ИспользуетсяСервис1СШтамп() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ИспользоватьСервис1СШтамп = ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьСервис1СШтамп");
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ИспользоватьСервис1СШтамп = Перечисления.ВариантыИспользованияСервиса1СШтамп.Использовать;
	
КонецФункции

// Определяет, используется ли просмотр PDF документов.
// 
// Возвращаемое значение:
// 	Булево - Если Истина, то использование включено
Функция ИспользуетсяПросмотрPDFДокументов() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.ИспользоватьПросмотрPDFДокументов.Получить();
	УстановитьПривилегированныйРежим(Ложь);
...
Функция НастройкиОтправкиИнтеркампани(КлючНастроекОтправки) Экспорт
	...
	Возврат НастройкиОтправкиЭДО.НастройкиОтправкиИнтеркампани(КлючНастроекОтправки);
...
// Возвращаемое значение:
//  - Неопределено
//  - См. ОтражениеВУчетеЭДО.НастройкиОтраженияВУчете
Функция НастройкиОтраженияВУчете(КлючНастроек) Экспорт
	
	Возврат ОбщийМодульОтражениеВУчетеЭДО().НастройкиОтраженияВУчете(КлючНастроек);
...
Процедура ЗаписатьНастройкуПолученияДокументов(КлючНастроек, СпособОбработки, НеПредлагатьСохранятьНастройки, Отказ) Экспорт
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().ЗаписатьНастройкуПолученияДокументов(КлючНастроек, СпособОбработки,
...
Процедура СоздатьНастройкиОтраженияВУчете(Организация, Контрагент, ИдентификаторОрганизации, ИдентификаторКонтрагента,
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().СоздатьНастройкиОтраженияВУчете(Организация, Контрагент, ИдентификаторОрганизации,
...
// Возвращаемое значение:
//  - Неопределено
//  - См. ОтражениеВУчетеЭДО.ПолучитьНастройкиПолученияПоИдентификаторам
//
Функция ПолучитьНастройкиПолученияПоИдентификаторам(ИдентификаторОтправителя, ИдентификаторПолучателя, Организация, Контрагент, СоздаватьНастройки = Истина) Экспорт
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		Возврат ОбщийМодульОтражениеВУчетеЭДО().ПолучитьНастройкиПолученияПоИдентификаторам(ИдентификаторОтправителя,
...
// См. НастройкиВнутреннегоЭДО.УстановитьПризнакИспользованияВнутреннегоЭДО
...
Функция СоответствиеСтарыхФорматовНовым() Экспорт
	...
	СоответствиеФорматов.Вставить("Договорный документ_ФНС 1.01",
		Обработки.ФорматДоговорныйДокумент101.ПространствоИмен());
...
// 
// Параметры:
//  ИдентификаторУчетнойЗаписи - см. СинхронизацияЭДОСобытия.ПриУдаленииУчетнойЗаписи.ИдентификаторУчетнойЗаписи
Процедура ПриУдаленииУчетнойЗаписи(ИдентификаторУчетнойЗаписи) Экспорт
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().ПриУдаленииУчетнойЗаписи(ИдентификаторУчетнойЗаписи);
	КонецЕсли;
...
// 
// Параметры:
//  Списки - см. УправлениеДоступомПереопределяемый.ПриЗаполненииСписковСОграничениемДоступа.Списки
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().ПриЗаполненииСписковСОграничениемДоступа(Списки);
	КонецЕсли;
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Если ИнтеграцияЭДО.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание);
	КонецЕсли;
...
// Возвращаемое значение:
//  ОбщийМодуль
Функция ОбщийМодульОтражениеВУчетеЭДО()
	Возврат ИнтеграцияЭДО.ОбщийМодульОтражениеВУчетеЭДО();
КонецФункции
