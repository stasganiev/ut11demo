...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
		...
		Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
			...
			ДобавитьВЗаголовкиЗапросаВерсиюБЭД(ПараметрыВыполненияОперации.Заголовки, КлючСинхронизации.СпособОбмена);
		
			ПараметрыВыполненияОперации.ТелоЗапроса = ДвоичныеДанныеСертификата;
			ПараметрыВыполненияОперации.Таймаут = 20;
...
// Добавляет в заголовки http запроса версию БЭД при обмене через сервис 1С ЭДО.
// 
// Параметры:
...
//  СпособОбмена - ПеречислениеСсылка.СпособыОбменаЭД
Процедура ДобавитьВЗаголовкиЗапросаВерсиюБЭД(Заголовки, СпособОбмена)

	Если СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
		ВерсияБЭД = ОбновлениеИнформационнойБазыБЭД.ВерсияБиблиотеки();
		Заголовки.Вставить("X-Platform-Version", ВерсияБЭД);
	КонецЕсли;
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	Если Не ЗначениеЗаполнено(ПараметрыРегистрации.Идентификатор) Тогда
		ПараметрыРегистрации.Идентификатор = РаботаСФайламиБЭД.НовыйUUID();
	КонецЕсли;
	...
	Доверенность = ПараметрыРегистрации.Доверенности.Получить(ПараметрыРегистрации.СертификатПодписанта);
	Если ЗначениеЗаполнено(Доверенность) Тогда
		ИННДоверителя = МашиночитаемыеДоверенности.ИННДоверителя(Доверенность);
	КонецЕсли;
	...
	РеквизитыПакета.Вставить("УникальныйИдентификаторЗаявки1СЭДО", ПараметрыРегистрации.Идентификатор);
	...
	РеквизитыПакета.Вставить("ИННДоверителя",			 ИННДоверителя);
	...
	// Служебные данные
	ЗначенияРеквизитов.Вставить("ИдФайл",   "registration_" + ПараметрыРегистрации.Идентификатор);
	ЗначенияРеквизитов.Вставить("ВерсПрог", ФорматыЭДО.ВерсияПрограммы());
	...
	СертификатыПользователя.Колонки.Добавить("Передоверие");
	СертификатыПользователя.Колонки.Добавить("ИННДоверителя");
	СертификатыПользователя.Колонки.Добавить("ОбезличенныйСертификат");
	СертификатыПользователя.Колонки.Добавить("СведСистОтм");
	...
	Для каждого Сертификат Из ПараметрыРегистрации.СертификатыПодписейОрганизации Цикл
		...
		НоваяСтрока.ОбезличенныйСертификат = Не ТекущийСертификат.Субъект.Свойство("SN");
		...
		Если ТребуетсяДоверенность Тогда
			...
			Если ЗначениеЗаполнено(Доверенность) Тогда
				...
				НоваяСтрока.Передоверие = РеквизитыПакета.ИННДоверителя <> РеквизитыПакета.ИННОрганизации;
				НоваяСтрока.ИННДоверителя = РеквизитыПакета.ИННДоверителя;
				НоваяСтрока.СведСистОтм = МашиночитаемыеДоверенности.СведенияОбИнформационнойСистеме();
			КонецЕсли;
		КонецЕсли;
	...
	НайденныеОбезличенныеСертификаты = СертификатыПользователя.НайтиСтроки(Новый Структура("ОбезличенныйСертификат", Истина));
	Если НайденныеОбезличенныеСертификаты.Количество() > 0 Тогда
		ОбезличенныеСертификатыПользователя = СертификатыПользователя.Скопировать(НайденныеОбезличенныеСертификаты);
		ЭлектронноеВзаимодействие.ЗагрузитьТаблицуВДерево(ДеревоРегистрационнойИнформации, ОбезличенныеСертификатыПользователя,
			"УчастЭДО.СертифОбезл");
	КонецЕсли;
	
	НайденныеСертификаты = СертификатыПользователя.НайтиСтроки(Новый Структура("ОбезличенныйСертификат", Ложь));
	Если НайденныеСертификаты.Количество() > 0 Тогда
		СертификатыПользователя = СертификатыПользователя.Скопировать(НайденныеСертификаты);
	...
	Если ПараметрыРегистрации.СертификатПодписанта <> Неопределено Тогда
		ПараметрыСертификатов = КриптографияБЭД.СвойстваСертификатов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыРегистрации.СертификатПодписанта));
		ПараметрыСертификатаПодписанта = ПараметрыСертификатов[ПараметрыРегистрации.СертификатПодписанта];
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыСертификатов.Значение) Тогда
			Для Каждого СтрСертификат Из СтрокаТаблицыСертификатов.Строки Цикл
				...
				МЧД = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(ПутьКОписанию + ".Документ.УчастЭДО.СертифДолжн.МЧД");
				ЗначенияСвойствXDTO = Новый Соответствие;
				ЗначенияСвойствXDTO.Вставить("Передоверие", "УчастЭДО.СертифДолжн.НомерСтроки.Передоверие");
				ЗначенияСвойствXDTO.Вставить("ИННДоверителя", "УчастЭДО.СертифДолжн.НомерСтроки.ИННДоверителя");
				ЗначенияСвойствXDTO.Вставить("НомерМЧД", "УчастЭДО.СертифДолжн.НомерСтроки.НомерМЧД");
				ЗначенияСвойствXDTO.Вставить("СведСистОтм", "УчастЭДО.СертифДолжн.НомерСтроки.СведСистОтм");
				ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
				ПараметрыЗаполнения.НеобязательныеСвойства = "Передоверие,ИННДоверителя,НомерМЧД,СведСистОтм";
				ОшибкиМЧД = Новый Массив;
				РаботаСФайламиБЭД.ЗаполнитьСвойстваXDTOИзДерева(МЧД, ЗначенияСвойствXDTO, СтрСертификат, ОшибкиМЧД,
					ПараметрыЗаполнения);
					
				Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(МЧД, "НомерМЧД") <> Неопределено Тогда
					Сертификат.МЧД.Добавить(МЧД);
					Для Каждого Ошибка Из ОшибкиМЧД Цикл
		...
		СтрокаТаблицыСертификатов = ДеревоДанных.Строки.Найти("УчастЭДО.СертифОбезл", "ПолныйПуть", Истина);
		Если ЗначениеЗаполнено(СтрокаТаблицыСертификатов.Значение) Тогда
			Для Каждого СтрСертификат Из СтрокаТаблицыСертификатов.Строки Цикл
				Сертификат = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(ПутьКОписанию + ".Документ.УчастЭДО.СертифОбезл");
				ЗначенияСвойствXDTO = Новый Соответствие;
				ЗначенияСвойствXDTO.Вставить("ДатаНачСертиф", "УчастЭДО.СертифОбезл.НомерСтроки.ДатаНачСертиф");
				ЗначенияСвойствXDTO.Вставить("ДатаКонСертиф", "УчастЭДО.СертифОбезл.НомерСтроки.ДатаКонСертиф");
				ЗначенияСвойствXDTO.Вставить("ОтпСертиф", "УчастЭДО.СертифОбезл.НомерСтроки.ОтпСертиф");
				ЗначенияСвойствXDTO.Вставить("Сертификат", "УчастЭДО.СертифОбезл.НомерСтроки.Сертификат");
				ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
				ПараметрыЗаполнения.СвойстваСоЗначениямиДата = "ДатаНачСертиф, ДатаКонСертиф";
				РаботаСФайламиБЭД.ЗаполнитьСвойстваXDTOИзДерева(Сертификат, ЗначенияСвойствXDTO, СтрСертификат, Ошибки,
					ПараметрыЗаполнения);
				
				УчастникЭДО.СертифОбезл.Добавить(Сертификат);
			КонецЦикла;
		КонецЕсли;
...
// Возвращает список электронных документов.
// 
// Параметры:
//  КлючСинхронизации - См. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
//  ДатаПоследнегоЗапроса - Дата
//  АдресРесурса - Строка
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики 
//  ВидСервисаЭДО - Неопределено, ПеречислениеСсылка.ВидыСервисовЭДО - если ВидСервисаЭДО = Неопределено - 
//                                                                     это документы ЭДО
// 
// Возвращаемое значение:
//  См. НовыйРезультатПолученияСпискаДокументов
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, КонтекстДиагностики, ВидСервисаЭДО = Неопределено)
	...
	ПараметрыВыполненияОперации.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	ПараметрыВыполненияОперации.Таймаут = 60;	
	Если ВидСервисаЭДО = Неопределено Тогда
		ДобавитьВЗаголовкиЗапросаВерсиюБЭД(ПараметрыВыполненияОперации.Заголовки, КлючСинхронизации.СпособОбмена);
	КонецЕсли;
