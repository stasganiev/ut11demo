...
Функция СведенияОЮридическомЛицеПоИНН(Знач ИНН) Экспорт
	...
	Результат = СлужебнаяСведенияОЮридическомЛицеПоИНН(
		ИНН,
		URLRESTОперацииFindCounterAgentsByInn(ИНН),
		ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов());
...
Функция РеквизитыПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	Результат = СлужебнаяСведенияОПредпринимателеПоИНН(
		ИНН,
		URLRESTОперацииFindPersonByInn(ИНН),
		ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов());
...
Функция ЮридическиеЛицаПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	URLОперации = URLRESTОперацииFindCorporationsAndUnitsByName(
		Наименование,
		КодРегиона,
		Адрес);
	...
	Если ТипЗнч(ЗначениеCorporations) = Тип("Массив") Тогда
		Для Каждого ТекущееCorporation Из ЗначениеCorporations Цикл
			...
			ЗаполнитьСведенияОПодразделениях(ТекущееCorporation, РеквизитыОрганизации);
...
Функция ЮридическиеЛицаПоИНН(Знач ИНН, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Асинхронный поиск юридических лиц по ИНН'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ИдентификаторУслуги = ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов();
	URLОперации = URLRESTОперацииFindCorporationsByINN(
		ИНН,
		КодРегиона,
		Адрес);
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
	...
	ЗаполнитьЗначенияСвойств(
		Результат,
		РезультатВызова,
		"ОжиданиеОтвета, ОписаниеОшибки");
	
	Если Результат.ОжиданиеОтвета
		Или ЗначениеЗаполнено(Результат.ОписаниеОшибки) Тогда
			Возврат Результат;
	КонецЕсли;
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Успешно завершен асинхронный поиск юридических лиц по ИНН'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция ИнформацияОСвязяхЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	ЗначениеterminationDate = ТелоОтветаОбъект.Получить("terminationDate");
	Если ЗначениеterminationDate <> Неопределено Тогда
		Результат.ДатаПрекращенияДеятельности = ДатаИзJSON(ЗначениеterminationDate);
	КонецЕсли;
	
	ЗначениеstandardCharterNumber = ТелоОтветаОбъект.Получить("standardCharterNumber");
	Если ЗначениеstandardCharterNumber <> Неопределено Тогда
		Результат.НомерТиповогоУстава = ЗначениеstandardCharterNumber;
	КонецЕсли;
	...
	ЗаполнитьДанныеОРуководителях(ТелоОтветаОбъект, Результат);
	...
	ЗаполнитьИнформациюОДанныхУчредителей(ТелоОтветаОбъект, Результат);
...
Функция ИнформацияОСвязяхПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	ЗаполнитьОсновныеРеквизитыПредпринимателя(ТелоОтветаОбъект, Результат, Истина);
	ЗаполнитьИнформациюОСтатусе(ТелоОтветаОбъект, Результат);
	ЗаполнитьКодОКВЭД(ТелоОтветаОбъект, Результат);
	ЗаполнитьВидДеятельности(ТелоОтветаОбъект, Результат);
	ЗаполнитьИнформациюОВидахДеятельности(ТелоОтветаОбъект, Результат);
...
Функция НаличиеПодразделенийПоИНН(ИНН) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Синхронная проверка наличия подразделений по ИНН.'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьПодразделения", Ложь);
	Результат.Вставить("ОписаниеОшибки", Неопределено);
	
	Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИНН, Истина, Результат.ОписаниеОшибки) Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru='Получение данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ИдентификаторУслуги = ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов();
	URLОперации = URLRESTОперацииCheckCorporationUnitsByInn(ИНН);
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
		ИдентификаторУслуги,
		URLОперации);
	ЗаполнитьЗначенияСвойств(
	...
	Если ЗначениеЗаполнено(РезультатВызова.ОписаниеОшибки) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ЕстьПодразделения = РезультатВызова.ТелоОтветаОбъект.Получить("unitsExist");
	
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Успешно завершена синхронная проверка наличия подразделений по ИНН.'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
//                             полученный в методе ИнформацияОПодразделенияхЮридическогоЛицаПоИНН
//
// Возвращаемое значение:
...
Функция ИнформацияОПодразделенииЮридическогоЛицаПоУникальномуНомеру(УникальныйНомер) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Асинхронное получение сведений о подразделении юридического лица по уникальному номеру %1'"),
			УникальныйНомер),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
		
	Результат = НовыеСведенияПодразделения();
	
	Если Не ЗначениеЗаполнено(УникальныйНомер) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не указан уникальный номер подразделения.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru='Получение данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	URLОперации = URLRESTОперацииFindCorporationUnitById(УникальныйНомер);
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
		ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов(),
		URLОперации);
	...
	ЗаполнитьДанныеПодразделения(
		Результат,
		РезультатВызова.ТелоОтветаОбъект);
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение сведений о подразделении юридического лица по уникальному номеру %1'"),
			УникальныйНомер),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция ИнформацияОПодразделенииЮридическогоЛицаПоИННКПП(ИНН, КПП) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Асинхронное получение сведений о подразделении юридического лица по ИНН/КПП %1/%2'"),
			ИНН,
			КПП), // Заполнить данные в %, проверить по всему модулю такие вхождения.
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не указан ИНН подразделения юридического лица.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru='Получение данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ИначеЕсли Не ЗначениеЗаполнено(КПП) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не указан КПП подразделения юридического лица.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru='Получение данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	URLОперации = URLRESTОперацииFindCorporationUnitByINNAndKPP(ИНН, КПП);
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
		ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов(),
		URLОперации);
	ЗаполнитьЗначенияСвойств(Результат, РезультатВызова, "ОписаниеОшибки");
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение сведений о подразделении по по ИНН/КПП %1/%2'"),
			ИНН,
			КПП),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
#Область ИнтеграцияСБиблиотекойРегламентированнойОтчетности

// Возвращает сведения об юридическом лице для заполнения сертификата.
//
// Параметры:
//	ИНН - Строка - ИНН юридического лица.
//
...
//		РаботаСКонтрагентамиКлиент.ОбработатьОшибку.
//
Функция ДанныеЮридическогоЛицаДляЗаполненияСертификата(Знач ИНН) Экспорт
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных для сертификата юридического лица по ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Результат = СлужебнаяСведенияОЮридическомЛицеПоИНН(
		ИНН,
		URLRESTОперацииFindCounterAgentsInfoByInn(ИНН));
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение данных для сертификата юридического лица по ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
// Возвращает сведения об индивидуальном предпринимателе для заполнения сертификата.
//
// Параметры:
//	ИНН - Строка - ИНН индивидуального предпринимателя, реквизиты которого надо получить.
//
...
//								приведен по классификатору редакции 2 или редакции 1.
//
Функция ДанныеПредпринимателяДляЗаполненияСертификата(Знач ИНН) Экспорт
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных для сертификата предпринимателя по ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Результат = СлужебнаяСведенияОПредпринимателеПоИНН(
		ИНН,
		URLRESTОперацииFindPersonInfoByInn(ИНН));
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение данных для сертификата предпринимателя по ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Процедура ПриЗаполненииХостовСервисовИнтернетПоддержки(ХостыСервисовИнтернетПоддержки) Экспорт
	...
	ХостыСервисовИнтернетПоддержки.Вставить(
		СтруктураURL.Хост,
		НСтр("ru = '1С:Контрагент'"));
	ХостыСервисовИнтернетПоддержки.Вставить(
		ПроверкаКонтрагентовФНС.ХостСервисаПроверкиСостоянийФНС(),
		НСтр("ru = 'Проверка контрагентов по данным ФНС'"));
...
// См. ИнтеграцияПодсистемБИП.ПриДобавленииПараметровРаботыКлиента
//
...
// См. ИнтеграцияПодсистемБИП.ПриФормированииСпискаУслуг
//
...
Процедура ПриФормированииТекстовПредупреждений(ДанныеСервиса, ТекстыПредупреждений) Экспорт
	...
	Если ДанныеСервиса.Ключ <> ИдентификаторСервиса() Тогда
		Возврат;
	КонецЕсли;
	
	ТекстыУведомленийСервиса = Новый Соответствие;
	Для Каждого ДанныеВидаУведомлений Из ДанныеСервиса.Значение Цикл
		Если ДанныеВидаУведомлений.ВидУведомления = ВидУведомленияЗавершаетсяСрокДействияСервиса() Тогда
			ТекстыУведомленийСервиса.Вставить(
				ТекстУведомленияЗавершаетсяСрокДействияСервиса(
				ДанныеВидаУведомлений),
			Неопределено);
		ИначеЕсли ДанныеВидаУведомлений.ВидУведомления = ВидУведомленияЗаканчиваютсяСчетныеОпции() Тогда
			ТекстыУведомленийСервиса.Вставить(
				ТекстУведомленияЗаканчиваютсяСчетныеОпции(
				ДанныеВидаУведомлений),
			Неопределено);
		Иначе
			Продолжить;
	...
	Для Каждого ТекстУведомления Из ТекстыУведомленийСервиса Цикл
		ТекстыПредупреждений.Добавить(ТекстУведомления.Ключ);
	КонецЦикла;
...
// Возвращает значение константы "ИспользоватьПроверкуКонтрагентов".
//
// Возвращаемое значение:
//  Число
//
Функция ИспользоватьПроверкуКонтрагентов() Экспорт
...
Процедура ПроверитьДоступностьСервисаКонтрагент(ОписаниеОшибки) Экспорт
	...
	Попытка
		Прокси = ИнтернетПоддержкаПользователей.СоздатьWSПрокси(ПараметрыПодключения);
	Исключение
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ЗначенияЭквивалентны(Значение1, Значение2)
	...
	ИначеЕсли ТипЗначение1 = Тип("Структура") Или ТипЗначение1 = Тип("ФиксированнаяСтруктура") Тогда
		...
		Для Каждого КлючЗначение Из Значение1 Цикл
			...
			Если Не Значение2.Свойство(КлючЗначение.Ключ, ЗначениеСвойства2) Тогда // АПК:1415 Реквизиты добавляются программно
				Возврат Ложь;
			КонецЕсли;
...
Функция ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов() Экспорт
...
Функция ИдентификаторУслугиИнформацияОСвязяхКонтрагентов() Экспорт
...
Функция URLRESTОперацииPing() Экспорт
...
Функция URLRESTОперацииFindCorporationsAndUnitsByName(Наименование, КодРегиона, Адрес)
	
	Возврат БазовыйURLСервиса() + "/rest/corporation/v1/find-corporations-and-units-by-name"
...
Функция URLRESTОперацииFindCorporationsByINN(ИНН, КодРегиона, Адрес)
	
	Возврат БазовыйURLСервиса() + "/rest/corporation/v1/find-corporations-by-inn"
		+ "?inn=" + КодироватьСтроку(ИНН, СпособКодированияСтроки.КодировкаURL)
		+ ?(ЗначениеЗаполнено(КодРегиона), "&regionCode=" + КодироватьСтроку(КодРегиона, СпособКодированияСтроки.КодировкаURL), "")
		+ ?(ЗначениеЗаполнено(Адрес), "&address=" + КодироватьСтроку(Адрес, СпособКодированияСтроки.КодировкаURL), "");
...
Функция URLRESTОперацииFindCounterAgentsInfoByInn(ИНН)
	
	Возврат БазовыйURLСервиса()
			+ "/rest/certificate-verification/v1/find-counter-agents-info-by-inn?inn="
			+ КодироватьСтроку(ИНН, СпособКодированияСтроки.КодировкаURL);
	
КонецФункции

Функция URLRESTОперацииFindPersonInfoByInn(ИНН)
	
	Возврат БазовыйURLСервиса()
			+ "/rest/certificate-verification/v1/find-person-info-by-inn?inn="
			+ КодироватьСтроку(ИНН, СпособКодированияСтроки.КодировкаURL);
	
КонецФункции

Функция URLRESTОперацииCheckCorporationUnitsByInn(ИНН)
	
	Возврат БазовыйURLСервиса() + "/rest/corporation/v1/check-corporation-units-by-inn?inn="
		+ КодироватьСтроку(ИНН, СпособКодированияСтроки.КодировкаURL);
	
КонецФункции

Функция URLRESTОперацииFindCorporationUnitById(УникальныйНомер)
	
	Возврат БазовыйURLСервиса() + "/rest/corporation/v1/find-corporation-unit-by-id?id="
		+ КодироватьСтроку(Формат(УникальныйНомер, "ЧГ="), СпособКодированияСтроки.КодировкаURL);
	
КонецФункции

Функция URLRESTОперацииFindCorporationUnitByINNAndKPP(ИНН, КПП)
	
	Возврат БазовыйURLСервиса() + "/rest/corporation/v1/find-corporation-unit-by-inn-and-kpp?"
		+ "inn=" + КодироватьСтроку(ИНН, СпособКодированияСтроки.КодировкаURL)
		+ "&kpp=" + КодироватьСтроку(КПП, СпособКодированияСтроки.КодировкаURL);
...
Функция ПроверитьУслугуИВызватьОперациюRESTСервиса(
	...
	Иначе
		...
		ОбработатьОшибкуОперацииСервиса(
			URLОперации,
			ПараметрыВызоваОперации,
			РезультатОперации,
			ОписаниеОшибки400,
...
Процедура ОбработатьОшибкуОперацииСервиса(
	URLОперации,
	ПараметрыВызоваОперации,
	РезультатОперации,
	ОписаниеОшибки400,
	...
	ТипОшибки = "";
	Если ЗначениеЗаполнено(РезультатОперации.Содержимое) Тогда
		ТипОшибки = ОпределитьТипОшибкиСервиса(
			РезультатОперации.КодСостояния,
			РезультатОперации.Содержимое);
	КонецЕсли;
	
	Результат.ОписаниеОшибки = ПереопределитьОписаниеОшибки(
		РезультатОперации,
		ТипОшибки,
		ОписаниеОшибки400,
		URLОперации,
...
//
Функция ОпределитьТипОшибкиСервиса(
	КодСостояния,
	ТелоJSON)
	...
	// есть вероятность получить неформализованное сообщение.
	Попытка
		
		ДанныеОшибок = ТелоОтветаОбъектИзСтроки(ТелоJSON).Получить("errors");
		
		Для Каждого ДанныеОшибки Из ДанныеОшибок Цикл
			Если КодСостояния = ДанныеОшибки.Получить("status") Тогда
				ТипОшибки = ДанныеОшибки.Получить("type");
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
// Определяет по коду ошибки описание ошибки.
//
// Параметры:
//  РезультатОперации - Структура - содержит результат операции;
//  ТипОшибки - Строка - тип ошибки сервиса;
//  ОписаниеОшибки400 - Строка - предопределенное описание ошибки для кода 400;
//  URLОперации - Строка - URL вызываемой операции;
//  ПараметрыВызоваОперации - Структура - параметры вызова операции.
//
// Возвращаемое значение:
//  Строка - описание ошибки.
//
Функция ПереопределитьОписаниеОшибки(
	РезультатОперации,
	ТипОшибки,
	ОписаниеОшибки400,
	URLОперации,
	...
	Если РезультатОперации.КодСостояния = 400 Тогда
		
		Если ТипОшибки = "VALIDATION_ERROR_INN" Тогда
			Если ОписаниеОшибки400 = Неопределено Тогда
				ОписаниеОшибки = "ОшибкаПроверкиИНН";
				ТекстОшибки    = НСтр("ru = 'Поле ИНН должно иметь длину 10 или 12 цифр'");
			Иначе
				ОписаниеОшибки = ОписаниеОшибки400;
				ТекстОшибки    = ОписаниеОшибки400;
			КонецЕсли;
		ИначеЕсли ТипОшибки = "VALIDATION_ERROR_NAME" Тогда
			ОписаниеОшибки  = "ОшибкаПроверкиНаименования";
			ТекстОшибки     = НСтр("ru = 'Поле Наименование является обязательным.'");
		ИначеЕсли ТипОшибки = "VALIDATION_ERROR_REGION_CODE" Тогда
			ОписаниеОшибки  = "ОшибкаПроверкиКодаРегиона";
			ТекстОшибки     = НСтр("ru = 'Поле Код региона должно состоять из двух цифр.'");
		ИначеЕсли ТипОшибки = "VALIDATION_ERROR_KPP" Тогда
			ОписаниеОшибки  = "ОшибкаПроверкиКПП";
			ТекстОшибки     = НСтр("ru = 'Поле КПП должно иметь длину 9 цифр.'");
		ИначеЕсли ТипОшибки = "VALIDATION_ERROR_ID" Тогда
			ОписаниеОшибки  = "ОшибкаПроверкиУникальногоНомера";
			ТекстОшибки     = НСтр("ru = 'Уникальный номер передан некорректно.'");
		Иначе
			ОписаниеОшибки  = "НеизвестнаяОшибка";
	...
	ИначеЕсли РезультатОперации.КодСостояния = 401
		И ТипОшибки = "UNAUTHORIZED_ERROR" Тогда
		
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			ОписаниеОшибки = "ОшибкаАутентификацииВСервисе";
			ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Ошибка аутентификации в сервисе. Обратитесь к администратору.'"),
				ИдентификаторСервисаКонтрагент(),
				НСтр("ru='Аутентификация'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Иначе
			ОписаниеОшибки = "НеУказаныПараметрыАутентификации";
			ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru = 'Интернет-поддержка пользователей не подключена (401).'"),
				ИдентификаторСервисаКонтрагент(),
				НСтр("ru='Аутентификация'", ОбщегоНазначения.КодОсновногоЯзыка()));
		КонецЕсли;
		
	ИначеЕсли РезультатОперации.КодСостояния = 402 Тогда
		
		ОписаниеОшибки = "Сервис1СКонтрагентНеПодключен";
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Сервис 1С:Контрагент не подключен (402)'"),
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru='Доступ'", ОбщегоНазначения.КодОсновногоЯзыка()));
		
	ИначеЕсли РезультатОперации.КодСостояния = 403
	...
	ИначеЕсли РезультатОперации.КодСостояния = 429
		И ТипОшибки = "REQUEST_LIMIT_EXCEEDING_ERROR" Тогда
		
		ОписаниеОшибки = "ПревышеноКоличествоПопыток";
		ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Превышено количество попыток. Повторите попытку позже.'"),
			ИдентификаторСервисаКонтрагент(),
...
Функция НовыеСведенияЮридическогоЛица(ИНН)
...
Функция НовыеРеквизитыЮридическогоЛица()
	...
	РеквизитыОрганизации.Вставить("Подразделения");
...
#Область Поздразделения

Функция НовыеСведенияПодразделения()
...
Функция НовыеРеквизитыПодразделения()
	...
	РеквизитыОрганизации.Вставить("ЮридическийАдрес");
	РеквизитыОрганизации.Вставить("ВидДеятельности");
	РеквизитыОрганизации.Вставить("ВидПодразделения");
...
Функция НоваяИнформацияОСвязяхЮридическогоЛица()
	...
	ИнформацияОСвязях.Вставить("ДанныеУчредителей", НовыеДанныеУчредителей());
...
Функция НовыеДанныеУчредителей()

	Результат = Новый Структура;
	Результат.Вставить("СкрытыеДанные",              Ложь);
	Результат.Вставить("ПредставлениеСкрытыхДанных", "");
	Результат.Вставить("Учредители",                 НоваяТаблицаУчредители());
...
Функция СлужебнаяСведенияОЮридическомЛицеПоИНН(
	ИНН,
	URLОперации,
	ИдентификаторУслуги = "")
	
	Результат = НовыеСведенияЮридическогоЛица(ИНН);
	...
	ЗаполнитьДанныеЮридическогоЛица(
		Результат,
		РезультатВызова.ТелоОтветаОбъект,
		ИНН);
...
Процедура ЗаполнитьДанныеЮридическогоЛица(
	Результат,
	ТелоОтветаОбъект,
	ИНН)
	...
	Если ЗначениеCorporation = Неопределено Тогда
		...
		Если ТипЗнч(ЗначениеCorporations) = Тип("Массив")
			И ЗначениеCorporations.Количество() > 0 Тогда
	...
	Если ЗначениеCorporation <> Неопределено Тогда
		...
		КонецЕсли;
		РеквизитыЮридическогоЛица.Удалить("Подразделения");
	...
	Если ТипЗнч(ЗначениеForeignRepresentations) = Тип("Массив") Тогда
		...
		Для Каждого ТекущийЭлементJSON Из ЗначениеForeignRepresentations Цикл
			...
			// Сжатие данных иностранных юр. лиц - для сокращения количества одинаковых объектов в памяти
...
Процедура ЗаполнитьИнформациюОбУчредителях(Объект, ИнформацияОСвязях)
	...
	ЗаполнитьТаблицуИнформацииОбУчредителях(ИнформацияОСвязях.Учредители, ЗначениеFounderInfos);
...
Функция СлужебнаяСведенияОПредпринимателеПоИНН(
	ИНН,
	URLОперации,
	ИдентификаторУслуги = "")
	...
	ЗаполнитьДанныеПредпринимателя(
		Результат,
		РезультатВызова.ТелоОтветаОбъект);
...
Процедура ЗаполнитьДанныеПредпринимателя(
	Результат,
	ТелоОтветаОбъект)
...
Процедура ЗаполнитьОсновныеРеквизитыПредпринимателя(
	ТелоОтветаОбъект,
	Реквизиты,
	ЭтоИнформацияОСвязях = Ложь)
...
Функция АдресКИИзJSON(АдресJSON)
	...
	Если ЗначениеЗаполнено(ЗначениеStead) Тогда
	...
	Если ЗначениеЗаполнено(ЗначениеSteadId) Тогда
...
Процедура ЗаполнитьСведенияОПодразделениях(ТелоОтветаОбъект, Реквизиты)
	
	ДанныеПодразделений = ТелоОтветаОбъект.Получить("unitInfos");
	Если ТипЗнч(ДанныеПодразделений) <> Тип("Массив") Тогда
		Возврат;
	КонецЕсли;
	
	Подразделения = Новый Массив;
	Для Каждого ДанныеПодразделения Из ДанныеПодразделений Цикл
		Подразделение = Новый Структура;
		Подразделение.Вставить("ИНН",              ДанныеПодразделения.Получить("inn"));
		Подразделение.Вставить("Наименование",     ДанныеПодразделения.Получить("name"));
		Подразделение.Вставить("Идентификатор",    ДанныеПодразделения.Получить("id"));
		
		Подразделение.Вставить("ЮридическийАдрес");
		ЗаполнитьЮридическийАдрес(ДанныеПодразделения.Получить("address"), Подразделение);
		
		Подразделения.Добавить(Подразделение);
	КонецЦикла;
	
	Реквизиты.Вставить("Подразделения", Подразделения);
	
КонецПроцедуры

Процедура ЗаполнитьДанныеПодразделения(Результат, ДанныеJSON)
	
	Если ДанныеJSON.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Результат.ИНН = ДанныеJSON.Получить("inn");
	
	РеквизитыПодразделения = НовыеРеквизитыПодразделения();
	РеквизитыПодразделения.ИНН = ДанныеJSON.Получить("inn");
	РеквизитыПодразделения.Наименование = СокрЛП(ДанныеJSON.Получить("name"));
	РеквизитыПодразделения.РегистрационныйНомер = ДанныеJSON.Получить("ogrn");
	РеквизитыПодразделения.КПП = ДанныеJSON.Получить("kpp");
	РеквизитыПодразделения.ВидДеятельности = Новый Структура(
		"Код, Наименование",
		ДанныеJSON.Получить("okvedCode"),
		ДанныеJSON.Получить("okvedName"));
	ЗаполнитьВидПодразделения(
		ДанныеJSON.Получить("type"),
		РеквизитыПодразделения);
	ЗаполнитьЮридическийАдрес(
		ДанныеJSON.Получить("address"),
		РеквизитыПодразделения);
	
	Результат.ЕГРЮЛ = РеквизитыПодразделения;
	
КонецПроцедуры

Процедура ЗаполнитьВидПодразделения(ВидПодразделения, Реквизиты)
	
	Если ВидПодразделения = "BRANCH" Тогда
		Реквизиты.ВидПодразделения = ВидПодразделенияФилиал();
	ИначеЕсли ВидПодразделения = "REPRESENTATION" Тогда
		Реквизиты.ВидПодразделения = ВидПодразделенияПредставительство();
	КонецЕсли;
	
КонецПроцедуры

Функция ВидПодразделенияФилиал()
	Возврат "Филиал";
КонецФункции

Функция ВидПодразделенияПредставительство()
	Возврат "Представительство";
КонецФункции
...
Процедура ЗаполнитьСведенияОбИностранномЮридическомЛице(СведенияОбИностранномЮридическомЛице, СведенияJSON)
	...
	Если ТипЗнч(ЗначениеAuthorizedCapital) = Тип("Соответствие") Тогда
		СведенияОбИностранномЮридическомЛице.УставныйКапитал = Новый Структура;
		ЗначениеValue = ЗначениеAuthorizedCapital.Получить("value");
		Если ЗначениеValue <> Неопределено Тогда
			СведенияОбИностранномЮридическомЛице.УставныйКапитал.Вставить("Размер", Число(ЗначениеValue));
		КонецЕсли;
		ЗначениеCurrencyCode = ЗначениеAuthorizedCapital.Получить("currencyCode");
		Если ЗначениеCurrencyCode <> Неопределено Тогда
			СведенияОбИностранномЮридическомЛице.УставныйКапитал.Вставить("КодВалюты", Число(ЗначениеCurrencyCode));
		КонецЕсли;
		ЗначениеCurrencyName = ЗначениеAuthorizedCapital.Получить("currencyName");
		Если ЗначениеCurrencyName <> Неопределено Тогда
			СведенияОбИностранномЮридическомЛице.УставныйКапитал.Вставить("НаименованиеВалюты", ЗначениеCurrencyName);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюОДанныхУчредителей(Объект, ИнформацияОСвязях)
	
	ЗначениеfounderInfoMeta = Объект.Получить("founderInfoMeta");
	Если ТипЗнч(ЗначениеfounderInfoMeta) = Тип("Соответствие")
		И ЗначениеfounderInfoMeta.Получить("text") <> Неопределено Тогда
		ИнформацияОСвязях.ДанныеУчредителей.СкрытыеДанные = Истина;
		ИнформацияОСвязях.ДанныеУчредителей.ПредставлениеСкрытыхДанных = ЗначениеfounderInfoMeta.Получить("text");
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьТаблицуИнформацииОбУчредителях(ИнформацияОСвязях.ДанныеУчредителей.Учредители, ЗначениеFounderInfos);
	
КонецПроцедуры

Процедура ЗаполнитьТаблицуИнформацииОбУчредителях(ТаблицаИнформацииОбУчредителях, ДанныеСервиса)
	
	Для Каждого ТекущееFounderInfos Из ДанныеСервиса Цикл
		
		СтрокаУчредителя = ТаблицаИнформацииОбУчредителях.Добавить();
		...
		Иначе
			...
			ИначеЕсли ЗначениеType = "FOREIGN_CORPORATION" Тогда
				...
				КонецЕсли;
			ИначеЕсли ЗначениеType = "MUTUAL_FUND" Тогда
				// Учредители - ПИФЫ.
				СтрокаУчредителя.Учредитель = ТекущееFounderInfos.Получить("name");
			ИначеЕсли ЗначениеType = "MUNICIPALITY" Тогда
				// Учредители - муниципалитеты.
				СтрокаУчредителя.Учредитель = ТекущееFounderInfos.Получить("name");
			КонецЕсли;
...
#Область УведомленияПортала1СИТС

// Возвращает уведомление пользователю портала 1С:ИТС по указанной опции.
// Когда уведомления нет или включено "Напомнить позже", тогда возвращается Неопределено.
//
// Параметры:
//  ИдентификаторОпции - Строка - идентификатор опции услуги портала 1С:ИТС.
// 
// Возвращаемое значение:
//  Структура, Неопределено - данные уведомления по опции 1С.
//
Функция УведомлениеПользователя(ИдентификаторОпции) Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторПортала1СИТС") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	МодульМониторПортала1СИТС = ОбщегоНазначения.ОбщийМодуль("МониторПортала1СИТС");
	Если Не МодульМониторПортала1СИТС.ПоказыватьУведомлениеПортала("1С:Контрагент", ИдентификаторОпции) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	УведомленияСервиса = МодульМониторПортала1СИТС.УведомленияПользователя(
		ИдентификаторСервиса());
	
	УведомлениеОпцииПользователя = Неопределено;
	Для Каждого УведомлениеСервиса Из УведомленияСервиса Цикл
		Если УведомлениеСервиса.ИдентификаторОпции = ИдентификаторОпции Тогда
			УведомлениеОпцииПользователя = УведомлениеСервиса;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если УведомлениеОпцииПользователя = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	УведомлениеОпцииПользователя.Вставить("Сервис", "1С:Контрагент");
	Если УведомлениеОпцииПользователя.ВидУведомления = ВидУведомленияЗавершаетсяСрокДействияСервиса() Тогда
		
		УведомлениеОпцииПользователя.Вставить("ТекстУведомления",
			ТекстУведомленияЗавершаетсяСрокДействияСервиса(
				УведомлениеОпцииПользователя));
		УведомлениеОпцииПользователя.Вставить("ПодробныйТекстУведомления",
			ПодробныйТекстУведомленияЗавершаетсяСрокДействияСервиса(
				УведомлениеОпцииПользователя));
		
	ИначеЕсли УведомлениеОпцииПользователя.ВидУведомления = ВидУведомленияЗаканчиваютсяСчетныеОпции() Тогда
		УведомлениеОпцииПользователя.Вставить("ТекстУведомления",
			ТекстУведомленияЗаканчиваютсяСчетныеОпции(
				УведомлениеОпцииПользователя));
		УведомлениеОпцииПользователя.Вставить("ПодробныйТекстУведомления",
			ПодробныйТекстУведомленияЗаканчиваютсяСчетныеОпции(
				УведомлениеОпцииПользователя));
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат УведомлениеОпцииПользователя;
	
КонецФункции

Функция ТекстУведомленияЗавершаетсяСрокДействияСервиса(ДополнительныеПараметры)
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Срок действия 1С:Контрагент заканчивается %1 года.'"),
		Формат(ДополнительныеПараметры.ДатаОкончания, "ДЛФ=D"));
	
КонецФункции

Функция ТекстУведомленияЗаканчиваютсяСчетныеОпции(ДополнительныеПараметры)

	Если ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов() Тогда
		Возврат НСтр("ru = 'Скоро будет превышен лимит автозаполнений контрагентов в сервисе 1С:Контрагент.'");
	ИначеЕсли ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиИнформацияОСвязяхКонтрагентов() Тогда
		Возврат НСтр("ru = 'Скоро будет превышен лимит на получение отчетов ""Досье контрагента"" в сервисе 1С:Контрагент.'");
	Иначе
		Возврат НСтр("ru = 'Скоро будет превышен лимит в сервисе 1С:Контрагент.'");
	КонецЕсли;
	
КонецФункции

Функция ПодробныйТекстУведомленияЗавершаетсяСрокДействияСервиса(ДополнительныеПараметры)
	
	Возврат СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = '1С:Контрагент действует <b>до %1</b>. Для продления доступа обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"),
		Формат(ДополнительныеПараметры.ДатаОкончания, "ДЛФ=D"));
	
КонецФункции

Функция ПодробныйТекстУведомленияЗаканчиваютсяСчетныеОпции(ДополнительныеПараметры)
	
	Если ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиЗаполнениеРеквизитовКонтрагентов() Тогда
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Заканчивается доступное количество автозаполнений по ИНН или наименованию в сервисе 1С:Контрагент. Для увеличения количества обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"));
	ИначеЕсли ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиИнформацияОСвязяхКонтрагентов() Тогда
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Заканчивается доступное количество отчетов ""Досье контрагента"" в сервисе 1С:Контрагент. Для увеличения количества обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"));
	Иначе
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Заканчивается доступное количество в сервисе 1С:Контрагент. Для увеличения количества обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"));
	КонецЕсли;
	
КонецФункции

// Возвращает идентификатор вида уведомления "Завершается срок действия".
// 
// Возвращаемое значение:
//  Строка - идентификатор вида уведомления.
//
Функция ВидУведомленияЗавершаетсяСрокДействияСервиса()
	Возврат "OPTION_SOON_VALIDITY_PERIOD_EXPIRATION";
КонецФункции

// Возвращает идентификатор вида уведомления "Заканчиваются счетные опции".
// 
// Возвращаемое значение:
//  Строка - идентификатор вида уведомления.
//
Функция ВидУведомленияЗаканчиваютсяСчетныеОпции()
	Возврат "OPTION_LOW_VOLUME_LEFT";
КонецФункции
