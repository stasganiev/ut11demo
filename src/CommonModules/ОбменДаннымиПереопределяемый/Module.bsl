...
Процедура ПриВыгрузкеДанных(СтандартнаяОбработка,
	...
	ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ЗарегистрироватьИзмененияНаУзлеПоОстаткамНаСкладе(Получатель);
...
//
Процедура ПриПереопределенииРеквизитовСсылочногоТипаУзлаПланаОбмена(УзелПланаОбмена, Результат) Экспорт
	Если ТипЗнч(УзелПланаОбмена) = Тип("ПланОбменаОбъект.СинхронизацияДанныхЧерезУниверсальныйФормат") 
		И ПолучитьФункциональнуюОпцию("ИспользоватьУправленческуюОрганизацию") 
		И НЕ УзелПланаОбмена.ВыгружатьУправленческуюОрганизацию Тогда
		
		Для каждого Стр Из Результат Цикл 
			Стр.РеквизитыРегистрации = СтрЗаменить(Стр.РеквизитыРегистрации, ",УправленческаяОрганизация", "");
			Если Стр.СтруктураРеквизитовРегистрации.Свойство("УправленческаяОрганизация") Тогда
				Стр.СтруктураРеквизитовРегистрации.Удалить("УправленческаяОрганизация");
			КонецЕсли;
			Прервать;
		КонецЦикла;
	КонецЕсли;	
...
// Устарела. В процедуре нет необходимости, т.к. используется параметр Глобальный 
...
Процедура ПриПроверкеКорректностиИмениПланаОбменаEnterpriseData(ИмяПланаОбмена, ВариантНастройки, ПланОбменаРаспознан) Экспорт
	...
	Если ВариантНастройки = "ОбменУПРМК"
		И ИмяПланаОбмена = "ОбменСУРМК" Тогда
		
		ИмяПланаОбмена = "СинхронизацияДанныхЧерезУниверсальныйФормат";
		ПланОбменаРаспознан = Истина;
