...
// * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками, Неопределено - действующая настройка обмена с банком.
//
Функция ПараметрыОбменаПоНомеруСчета(Знач БанковскийСчет) Экспорт
	...
	СтруктураВозврата = ПараметрыОбменаПоРеквизитам(РеквизитыСчета);
...
// 
// Параметры:
// 	НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена организации с банком
// Возвращаемое значение:
// 	Неопределено, Структура - При ошибке возвращается Неопределено. В остальных случая структура с полями:
// * РеквизитыСчета - Структура - реквизиты банковского счета организации:
//  ** Организация - ОпределяемыйТип.Организация - ссылка на организацию
...
//
Функция ПараметрыОбменаПоНастройкеОбмена(Знач НастройкаОбмена) Экспорт
	
	Если Не ОбменСБанками.ПравоВыполненияОбмена(Истина) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РеквизитыОбмена = РеквизитыНастройкиОбмена(НастройкаОбмена);
	
	СтруктураВозврата = ПараметрыОбменаПоРеквизитам(РеквизитыОбмена);
...
// Получает реквизиты банковского счета организации по БИК банка.
//
// Параметры:
...
//   * Город -  - ссылка на банк.
//   * БИК - Строка - БИК банка.
//
Функция РеквизитыБанка(БИК) Экспорт
...
// Получает реквизиты электронных документов по владельцам, помещая их в массив структур
//
// Параметры:
//  МассивДокументовСсылки - Массив - ДокументСсылка - объект информационной базы
//                                  - ДокументСсылка.СообщениеОбменСБанками - сообщение обмена с банком.
//  ПоискПоУчетнымДокументам - Булево - Истина, если ищем по объектам информационной базы: Ложь -  ищем по документам
//                                      СообщениеОбменСБанками.
// 
// Возвращаемое значение:
//  РеквизитыЭлектронныхДокументов - Массив - массив структур РеквизитыЭлектронногоДокумента
//    РеквизитыЭлектронногоДокумента - Структура - реквизиты электронного документа
//      * СсылкаНаОбъект - ДокументСсылка - объект информационной базы.
//      * СообщениеОбмена - ДокументСсылка.СообщениеОбменСБанками - сообщение обмена с банком
//                      - Неопределено - электронный документ не сформирован
//      * Состояние - ПеречислениеСсылка.СостоянияОбменСБанками - текущее состояние ЭДО
//      * ПрограммаБанка - ПеречислениеСсылка.ПрограммыБанка - технология обмена.
//      * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//
Функция РеквизитыЭлектронныхДокументов(Знач МассивДокументовСсылки, ПоискПоУчетнымДокументам = Истина) Экспорт
	
	РеквизитыЭлектронныхДокументов = Новый Массив;
	Запрос = Новый Запрос;
	Если ПоискПоУчетнымДокументам Тогда 
		...
		|	РегистрСведений.СостоянияОбменСБанками КАК СостоянияОбменСБанками
		|ГДЕ
		|	СостоянияОбменСБанками.СсылкаНаОбъект В(&МассивДокументовСсылки)";
	...
	Иначе
		...
		|	СостоянияОбменСБанками.СсылкаНаОбъект КАК СсылкаНаОбъект,
		|	СообщениеОбменСБанками.Состояние КАК Состояние,
		|	СообщениеОбменСБанками.ПрограммаБанка КАК ПрограммаБанка,
		...
		|		ПО СообщениеОбменСБанками.Ссылка = СостоянияОбменСБанками.СообщениеОбмена
		|ГДЕ
		|	СообщениеОбменСБанками.Ссылка В(&МассивДокументовСсылки)";
		
	КонецЕсли;
	Запрос.УстановитьПараметр("МассивДокументовСсылки", МассивДокументовСсылки);
	УстановитьПривилегированныйРежим(Истина);
	ТаблицаРеквизитовЭлектронныхДокументов = Запрос.Выполнить().Выгрузить();
	УстановитьПривилегированныйРежим(Ложь);
	РеквизитыЭлектронныхДокументов = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаРеквизитовЭлектронныхДокументов);
	
	Возврат РеквизитыЭлектронныхДокументов;
...
//
//  МассивДокументов - Массив из ЛюбаяСсылка
//                   - ФиксированныйМассив из ЛюбаяСсылка - ссылки на объекты.
//                     Если массив пуст, то результатом будет пустое соответствие.
//  ЗапрашиваемыеРеквизиты - Строка - имена реквизитов перечисленные через запятую, в формате требований к свойствам
...
// 
Функция ЗначенияРеквизитовОбъектов(Знач МассивДокументов, ЗапрашиваемыеРеквизиты) Экспорт
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивДокументов, ЗапрашиваемыеРеквизиты);
...
Функция ПараметрыОбменаПоВидуЭД(Знач НастройкаОбмена, Знач ВидЭД) Экспорт
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НастройкиОбменаПоВидамДокументов.СпособПодтвержденияДокумента, ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)) <> ЗНАЧЕНИЕ(перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)
	|			ТОГДА  НастройкиОбменаПоВидамДокументов.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись)			
	|		ИНАЧЕ ЕСТЬNULL(НастройкиОбменаПоВидамДокументов.ИспользоватьЭП, ЛОЖЬ)
...
// Возвращает двоичные данные сообщения обмена
//
// Параметры:
//  Письмо - ДокументСсылка.ПисьмоОбменСБанками - ссылка на письмо.
//
// Возвращаемое значение:
//  Структура:
//    * АдресФайла - Строка
//    * ИмяФайла -Строка.
//
Функция ПолучитьФайлДляТехническойПоддержки(Письмо) Экспорт 
...
// Получает информацию о состоянии переписки с банками
// 
// Возвращаемое значение:
//  Структура:
//    * ЕстьПравоЧтенияДанных - Булево - право чтения электронных документов для текущего пользователя.
//    * ИспользуетсяОбменСБанками - Булево
//    * ЕстьНастройка - Булево - есть доступная настройка обмена с письмами
//    * КоличествоНепрочитанных - Число - общее количество непрочитанных писем.
//    * СведенияОНепрочитанных - Массив Из Структура:
//      * ИспользуетсяОбменСБанками - Булево
//      * Количество - Число
//
Функция ТекущееСостояниеПисемСБанками() Экспорт  		
	...
	ИспользуетсяОбменСБанками = ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками");

	СведенияОНастройке = Новый Структура();   	
	СведенияОНастройке.Вставить("ЕстьПравоЧтенияДанных", ЕстьПравоЧтенияДанных);  	
	СведенияОНастройке.Вставить("ИспользуетсяОбменСБанками",ИспользуетсяОбменСБанками);
	СведенияОНастройке.Вставить("ЕстьНастройка", Ложь);  
	СведенияОНастройке.Вставить("КоличествоНепрочитанных", 0);
	СведенияОНастройке.Вставить("СведенияОНепрочитанных", Новый Массив);   
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТекстЕслиБанкНеУказан",НСтр("ru = 'Банк (не указан)'"));
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЕСТЬNULL(ПисьмоОбменСБанками.Банк.Наименование, &ТекстЕслиБанкНеУказан) КАК Банк,
	|	КОЛИЧЕСТВО(ПисьмоОбменСБанками.Ссылка) КАК Непрочитанно
	|ИЗ
	|	Документ.ПисьмоОбменСБанками КАК ПисьмоОбменСБанками
	|ГДЕ
	|	НЕ ПисьмоОбменСБанками.ПометкаУдаления
	|	И ПисьмоОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Входящий)
	|	И НЕ ПисьмоОбменСБанками.Прочитано
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ПисьмоОбменСБанками.Банк.Наименование, &ТекстЕслиБанкНеУказан)
	|ИТОГИ ПО
	|	Банк";
	Результат = Запрос.ВыполнитьПакет();
	Настройки = Результат[0].Выбрать();
	ЕстьНастройка = Настройки.Следующий();

	СведенияОНастройке.Вставить("ЕстьНастройка",ЕстьНастройка);
	МассивНепрочитанных = Новый Массив;   
	ВсегоНепрочитанно = 0;
	ВыборкаБанков = Результат[1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); 
	Пока ВыборкаБанков.Следующий() Цикл
		ВыборкаНепрочитанных = ВыборкаБанков.Выбрать();
		ВыборкаНепрочитанных.Следующий();
		КоличествоНепрочитанно = ВыборкаНепрочитанных.Непрочитанно;
		ВсегоНепрочитанно = ВсегоНепрочитанно+КоличествоНепрочитанно;
		
		СведенияОНепрочитанных = Новый Структура;
		СведенияОНепрочитанных.Вставить("Банк",ВыборкаБанков.Банк);
		СведенияОНепрочитанных.Вставить("Количество", КоличествоНепрочитанно);
		МассивНепрочитанных.Добавить(СведенияОНепрочитанных);
	КонецЦикла;
	
	СведенияОНастройке.Вставить("СведенияОНепрочитанных", МассивНепрочитанных);
	СведенияОНастройке.Вставить("КоличествоНепрочитанных", ВсегоНепрочитанно);

	Возврат СведенияОНастройке;
	
КонецФункции   

// Рассчитывает количество писем по отправителям. 
//
// Параметры:
...
// Возвращаемое значение: 
// Массив из Структура:
//   * НазваниеБанка - Строка
//   * КоличествоПисем - Число
//
Функция РассчитатьПолученныеПисьмаПоБанкам(ПолученныеПисьма) Экспорт   
	
	Запрос = Новый Запрос; 
	Запрос.УстановитьПараметр("ПолученныеПисьма",ПолученныеПисьма);
	Запрос.Текст =     
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПисьмоОбменСБанками.Банк.Наименование КАК НазваниеБанка,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПисьмоОбменСБанками.Ссылка) КАК КоличествоПисем
	|ИЗ
	|	Документ.ПисьмоОбменСБанками КАК ПисьмоОбменСБанками
	|ГДЕ
	|	ПисьмоОбменСБанками.Ссылка В(&ПолученныеПисьма)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПисьмоОбменСБанками.Банк.Наименование";
	
	РезультатЗапроса = Запрос.Выполнить();   
	ТаблицаОтправителей =  РезультатЗапроса.Выгрузить();	 
	СведенияОбОтправителях = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаОтправителей);
	
	Возврат СведенияОбОтправителях;
	
КонецФункции      

// Рассчитывает количество неотправленных писем. 
//
// Возвращаемое значение: 
//   Число - количество писем  
Функция КоличествоНеотправленныхПисем() Экспорт 
	
	МассивОтбора = ОбменСБанкамиСлужебный.СтатусыИсходящихПисем();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПисьмоОбменСБанками.Ссылка) КАК КоличествоНеотправленных
	|ИЗ
	|	Документ.ПисьмоОбменСБанками КАК ПисьмоОбменСБанками
	|ГДЕ
	|	ПисьмоОбменСБанками.Статус В(&МассивОтбора)
	|	И ПисьмоОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Исходящий)
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Возврат ВыборкаДетальныеЗаписи.КоличествоНеотправленных;
	Иначе
		Возврат 0;
	КонецЕсли; 
...
// Получает параметры открытия нового электронного письма
// 
// Параметры:
...
// Возвращаемое значение:
//  Структура:
//   * Отправитель - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты, Неопределено - СистемнаяУчетнаяЗаписьЭлектроннойПочты  
//   * Тема - Строка
//   * Текст - Строка
//   * Вложения - Массив из ОпределяемыйТип.ПрисоединенныйФайл
//
Функция ПараметрыПересылкиНовогоЭлектронногоПисьма(Письмо) Экспорт
	
	ВложенныеФайлы = Новый Массив;
	РаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(Письмо, ВложенныеФайлы);
	Вложения = Новый Массив;
	Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл 
		ДвоичныеДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(ВложенныйФайл);
		АдресФайла = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла,Новый УникальныйИдентификатор);  	
		Представление = ОбменСБанкамиСлужебныйКлиентСервер.ПредставлениеПрисоединенногоФайла(
			ВложенныйФайл.Наименование, ВложенныйФайл.Расширение);		
		ОписаниеВложения = Новый Структура("Представление, АдресВоВременномХранилище", Представление, АдресФайла);
		Вложения.Добавить(ОписаниеВложения);
	КонецЦикла; 
	
	РеквизитыПисьма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Письмо,
		"Организация.Наименование, Банк, Направление, Дата, ВходящаяДата, Тема, Текст");
	
	НазваниеБанка = РеквизитНаименованиеБанка(РеквизитыПисьма.Банк);       
	ЭтоИсходящийЭД = РеквизитыПисьма.Направление = Перечисления.НаправленияЭДО.Исходящий;
	...
	ТекстПисьма = СтрШаблон("
		...
		?(ЭтоИсходящийЭД,РеквизитыПисьма.ОрганизацияНаименование, НазваниеБанка),
		?(ЭтоИсходящийЭД,НазваниеБанка, РеквизитыПисьма.ОрганизацияНаименование),
		?(ЭтоИсходящийЭД,РеквизитыПисьма.Дата,РеквизитыПисьма.ВходящаяДата),
		РеквизитыПисьма.Тема,
		РеквизитыПисьма.Текст);   
		                	
	ПараметрыПисьма = Новый Структура;     
	ПараметрыПисьма.Вставить("Тема", СтрШаблон("Fwd: %1",РеквизитыПисьма.Тема));
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		...
		ПараметрыПисьма.Вставить("Отправитель", МодульРаботаСПочтовымиСообщениями.СистемнаяУчетнаяЗапись());
	КонецЕсли;
...
Процедура ОтправитьОдноразовыйПарольВСбербанк(Знач НастройкаОбмена, Знач КлючСессии, Знач ОдноразовыйПароль, Ошибка, ТребуетсяСменаПароля) Экспорт
	...
	WSПрокси = ОбменСБанкамиСлужебныйПовтИсп.WSПроксиСбербанк().WSПрокси;
...
Функция ТребуетсяКриптографияДляОбработкиДокументов(Знач МассивОбъектов)
	...
	Для Каждого СсылкаНаДокумент Из МассивОбъектов Цикл
		...
		БИК = ОбменСБанкамиСлужебный.БИК(ПараметрыЭД.Банк);
		Запрос.Текст = СтрЗаменить(	
			Запрос.Текст, 
			"ТЧ_НастройкиОбмена.Ссылка.Банк.Код", 
			"ТЧ_НастройкиОбмена.Ссылка.Банк." + ИмяПрикладногоРеквизитаБИК);
...
Функция ТекстЗапросаНастроекКриптографии()
	...
	|		ИЛИ ТЧ_НастройкиОбмена.Ссылка.Банк.Код = &БИК)
	|	И ТЧ_НастройкиОбмена.ИсходящийДокумент = &ВидЭД
	|	И (ВЫБОР
	|		КОГДА
	|			ТЧ_НастройкиОбмена.СпособПодтвержденияДокумента <> ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)
	|			ТОГДА ТЧ_НастройкиОбмена.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись)
	|		ИНАЧЕ ТЧ_НастройкиОбмена.ИспользоватьЭП
	|	КОНЕЦ
	|	ИЛИ ТЧ_НастройкиОбмена.Ссылка.АутентификацияПоСертификату)
	|	И ТЧ_НастройкиОбмена.Ссылка.ПрограммаБанка В (ЗНАЧЕНИЕ(Перечисление.ПрограммыБанка.АльфаБанкОнлайн),
	|		ЗНАЧЕНИЕ(Перечисление.ПрограммыБанка.АсинхронныйОбмен))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПараметрыОбменаПоРеквизитам(РеквизитыОбмена)
	
	Если РеквизитыОбмена.Свойство("НастройкаОбмена") Тогда
		НастройкаОбмена = РеквизитыОбмена.НастройкаОбмена;
	Иначе
		НастройкаОбмена = ОбменСБанкамиСлужебный.НастройкаОбмена(РеквизитыОбмена.Организация, РеквизитыОбмена.Банк);
	КонецЕсли;
	...
	Если НастройкаОбмена = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'Нет действующей настройки обмена через сервис 1С:Директ банк.
									|Организация: %1
									|Банк %2'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, РеквизитыОбмена.Организация, РеквизитыОбмена.Банк);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	СтруктураВозврата.Вставить("РеквизитыСчета", РеквизитыОбмена);
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИсходящиеДокументы.ИсходящийДокумент КАК ИсходящийДокумент,
	|	ВЫБОР
	|		КОГДА ИсходящиеДокументы.СпособПодтвержденияДокумента <> ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)
	|			ТОГДА ИсходящиеДокументы.СпособПодтвержденияДокумента
	|		КОГДА ИсходящиеДокументы.ИспользоватьЭП
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ПустаяСсылка)
	|	КОНЕЦ КАК СпособПодтвержденияДокумента
	|ИЗ
	|	Справочник.НастройкиОбменСБанками.ИсходящиеДокументы КАК ИсходящиеДокументы
	|ГДЕ
	|	ИсходящиеДокументы.Ссылка = &НастройкаОбмена";
	...
	НастройкиПодписанияПоВидамЭД = Новый Соответствие;
	
	Пока Выборка.Следующий() Цикл
		НастройкиПодписанияПоВидамЭД.Вставить(Выборка.ИсходящийДокумент, Выборка.СпособПодтвержденияДокумента);
	КонецЦикла;
	
	РеквизитыНастройкиОбмена.Вставить("НастройкиПодписанияПоВидамЭД", НастройкиПодписанияПоВидамЭД);
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	Если НЕ Результат.Статус Тогда
		ТипОшибкиИнтернетСоединения = 
			ОбменСБанкамиСлужебныйКлиентСервер.ТипОшибкиИнтернетСоединения(Результат.СообщениеОбОшибке);
		Если ЗначениеЗаполнено(ТипОшибкиИнтернетСоединения) Тогда 
				
			ДеталиОшибкиИнтернетСоединения = ОбменСБанкамиСлужебныйКлиентСервер.ДеталиОшибкиИнтернетСоединения(ТипОшибкиИнтернетСоединения);
			...
			Шаблон = НСтр("ru = 'Ошибка аутентификации на сервере банка по сертификату.
								...
								|Рекомендация: %3
								|%4'");
			ТекстОшибки = СтрШаблон(Шаблон, АдресСервера, ДеталиОшибкиИнтернетСоединения.КраткоеПредставлениеТипаОшибки, 
															ДеталиОшибкиИнтернетСоединения.РекомендацияПользователю,
															СформироватьРекомендацииПроверкиСетевойДоступности());
							
		ИначеЕсли ЗначениеЗаполнено(Результат.КодСостояния) Тогда
			Шаблон = НСтр("ru = 'Ошибка аутентификации на сервере банка по сертификату.
								|Адрес ресурса: %1
								|Код ошибки: %2
								|%3'");
...
Функция БазоваяАутентификация(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ДанныеАутентификации, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	СтруктураВозврата.Вставить("КраткоеПредставлениеОшибки");
	...
	Если Не Результат.Статус Тогда
		Если ЗначениеЗаполнено(Результат.КодСостояния) Тогда
			...
			СтруктураВозврата.КраткоеПредставлениеОшибки = Результат.КодСостояния;
		Иначе                                                                     
			Шаблон = НСтр("ru = 'Ошибка отправки данных аутентификации на сервер банка.
								|Адрес ресурса: %1
								|Ошибка: %2'");
			...
			СтруктураВозврата.КраткоеПредставлениеОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
...
Функция ЗапросыВыписок(
	...
	ПереданаТолькоДатаОкончания = ЗначениеЗаполнено(ДатаОкончания) И Не ЗначениеЗаполнено(ДатаНачала);
	...
	Если ПереданаТолькоДатаОкончания И ЗначениеЗаполнено(ДатаНачала) И ДатаНачала > ДатаОкончания Тогда
		// Дата начала обмена не передана и определена по параметрам обмена, но она
		// больше переданной даты окончания, поэтому запросим выписку только за дату окончания.
...
Функция ПодписываетсяЗапросВыписки(Знач НастройкаОбмена)
	...
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент = ЗНАЧЕНИЕ(Перечисление.ВидыЭДОбменСБанками.ЗапросВыписки)";
...
Функция ПолныйАдресСервисаСбербанк() Экспорт
	
	ИспользуетсяТестовыйРежим = Ложь;
	ОбменСБанкамиПереопределяемый.ПроверитьИспользованиеТестовогоРежима(ИспользуетсяТестовыйРежим);
		
	Если ИспользуетсяТестовыйРежим Тогда
		ПолныйАдресСервиса = "https://edupirsms.testsbi.sberbank.ru:9443/sbns-upg/upg";
	Иначе
		ПолныйАдресСервиса = "https://upgsbi.sberbank.ru:9443/sbns-upg/upg";
	КонецЕсли;
	
    Возврат ПолныйАдресСервиса;
	
КонецФункции

Функция СообщенияОбменаПоСтатусам(МассивДокументов, МассивСтатусов)
	
	ВозвращаемыйМассивДокументов = Новый Массив;
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СообщениеОбменСБанками.Ссылка КАК Ссылка
	...
	|	Документ.СообщениеОбменСБанками КАК СообщениеОбменСБанками
	|ГДЕ
	|	СообщениеОбменСБанками.Статус В(&МассивСтатусов)
	|	И СообщениеОбменСБанками.Ссылка В(&МассивДокументов)";
	Запрос.УстановитьПараметр("МассивСтатусов", МассивСтатусов);
	Запрос.УстановитьПараметр("МассивДокументов", МассивДокументов);
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		ВозвращаемыйМассивДокументов = Результат.Выгрузить().ВыгрузитьКолонку("Ссылка");	
	КонецЕсли;
	Возврат ВозвращаемыйМассивДокументов;
...
//   * НомерСчета - Строка - номер счета.
//
Функция РеквизитыБанковскогоСчетаОрганизации(Знач Ссылка)
...
//
// Параметры:
//  Ссылка - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена организации с банком
// 
// Возвращаемое значение:
//  Структура - реквизиты счета. Содержит поля:
//   * Организация - ОпределяемыйТип.Организация - ссылка на организацию.
...
//   * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на переданную в параметрах настройку обмена.
//
Функция РеквизитыНастройкиОбмена(Знач Ссылка)
	
	// Номер счета не заполняется - если там будет Неопределено, то запрос выписки будет выполнен по всем счетам банка.
	
	ВозвращаемоеЗначение = Новый Структура("Организация, Банк, НомерСчета, НастройкаОбмена");
	
	ВозвращаемоеЗначение.НастройкаОбмена = Ссылка;
	
	ЗначенияРеквизитовОснование = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Организация, Банк");
	ЗаполнитьЗначенияСвойств(ВозвращаемоеЗначение, ЗначенияРеквизитовОснование);
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Возвращает название банка из документа ПисьмоОбменСБанками.
//
// Параметры:
//  БанкСсылка - ОпределяемыйТип.БанкОбменСБанками
// 
// Возвращаемое значение:
//  Строка
//
Функция РеквизитНаименованиеБанка(БанкСсылка)
	
	НазваниеРеквизитаБанк = ОбменСБанкамиСлужебный.ИмяПрикладногоРеквизита("Банк.Наименование"); 
	ЗначениеРеквизитаОбъекта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанкСсылка, НазваниеРеквизитаБанк);
	
	Если ЗначениеРеквизитаОбъекта = Неопределено Тогда
		Возврат "";
	Иначе
		Возврат ЗначениеРеквизитаОбъекта;
	КонецЕсли;
...
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияПолученияИзвещенияОСостоянииЭД(
	Знач ПараметрыПроцедуры,
	Знач УникальныйИдентификатор) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение извещения о состоянии электронного документов'");
...
// Запускает на сервере фоновое задание по формированию запросов о состояниях документов.
//
// Параметры:
//   ПараметрыЗапроса - Структура - первый параметр для процедуры ОбменСБанкамиСлужебный.СформироватьЗапросыСостоянийЭД;
//   УникальныйИдентификатор - Строка - идентификатор формы для создания временного хранилища;
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияСформироватьЗапросыСостоянийЭД(
	Знач ПараметрыЗапроса,
	Знач УникальныйИдентификатор) Экспорт

	НаименованиеЗадания = НСтр("ru = 'Формирование запросов состояний электронных документов в банк'");
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НаименованиеЗадания;

	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиСлужебный.СформироватьЗапросыСостоянийЭД", ПараметрыЗапроса, ПараметрыВыполнения);

	Возврат Результат
	
КонецФункции

// Запускает на сервере фоновое задание по отправке запросов о состояниях документов из банка.
//
// Параметры:
//   ПараметрыПроцедуры - Структура - первый параметр для процедуры ОбменСБанкамиСлужебный.ОтправитьЗапросыСостоянийЭДВБанк;
//   УникальныйИдентификатор - Строка - идентификатор формы для создания временного хранилища;
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияОтправитьЗапросыОСостоянияхЭД(
	Знач ПараметрыПроцедуры,
	Знач УникальныйИдентификатор) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Отправка запросов о состояниях электронных документов'");

	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиСлужебный.ОтправитьЗапросыСостоянийЭДВБанк", ПараметрыПроцедуры, ПараметрыВыполнения);

КонецФункции

// Запускает на сервере фоновое задание по получению извещения о состояниях из банка.
//
// Параметры:
//   ПараметрыПроцедуры - Структура - первый параметр для процедуры ОбменСБанкамиСлужебный.ПолучитьИзвещенияОСостоянияхЭДАсинхронно;
//   УникальныйИдентификатор - Строка - идентификатор формы для создания временного хранилища;
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияПолученияИзвещениийОСостоянийЭД(
	Знач ПараметрыПроцедуры,
	Знач УникальныйИдентификатор) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение извещения о состояниях электронных документов'");

	ПараметрыПроцедуры.ИдентификаторСессии = СтрокаИзДвоичныхДанных(
		ПараметрыПроцедуры.ИдентификаторСессии);
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиСлужебный.ПолучитьИзвещенияОСостоянияхЭДАсинхронно", ПараметрыПроцедуры, ПараметрыВыполнения);

КонецФункции

// Запускает на сервере фоновое задание по формированию запроса GenSMSSign.
//
// Параметры:
//   ПараметрыЗапроса - Структура - первый параметр для процедуры ОбменСБанкамиСлужебный.СформироватьЗапросПодписанияПоSMS;
//   УникальныйИдентификатор - Строка - идентификатор формы для создания временного хранилища;
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияГенерацииЗапросаПодписанияПоSMS(Знач ПараметрыЗапроса, Знач УникальныйИдентификатор) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Формирование GenSMSSign.'");
	
	Результат =  ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиСлужебный.СформироватьЗапросПодписанияПоSMS", ПараметрыЗапроса, ПараметрыВыполнения);
		
	Возврат Результат;
	
КонецФункции

// Запускает на сервере фоновое задание по отправке запроса GenSMSSign в банк и получения ответа GenSMSSignResponse.
//
// Параметры:
//   ПараметрыПроцедуры - Структура - первый параметр для процедуры ОбменСБанкамиСлужебный.ОтправитьЗапросИПолучитьСсылкуДляПодписанияПоSMS;
//   УникальныйИдентификатор - Строка - идентификатор формы для создания временного хранилища;
//
// Возвращаемое значение:
//   Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияОтправкиЗапросаИПолученияСсылкиДляПодписанияПоSMS(Знач ПараметрыПроцедуры, Знач УникальныйИдентификатор) Экспорт
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение GenSMSSignResponse'");

	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиСлужебный.ОтправитьЗапросИПолучитьСсылкуДляПодписанияПоSMS", ПараметрыПроцедуры, ПараметрыВыполнения);

КонецФункции

Функция ПодготовитьПараметрыКПодписаниюПоSMS(МассивСсылокНаОбъект, НастройкаОбмена = Неопределено) Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	СообщенияОбменаКПодписаниюКодомИзSMS = СообщенияОбменаИзДокументовКПодписаниюКодомИзSMS(МассивСсылокНаОбъект);
	Если СообщенияОбменаКПодписаниюКодомИзSMS.Количество() <> 0 Тогда
			
		ДополнительныеПараметры.Вставить("СообщенияОбменаКПодписаниюКодомИзSMS", СообщенияОбменаКПодписаниюКодомИзSMS);
		Если НастройкаОбмена = Неопределено Тогда
			
			СсылкаНаДокумент = СообщенияОбменаКПодписаниюКодомИзSMS[0];
			НастройкаОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаДокумент, "НастройкаОбмена");
			ДополнительныеПараметры.Вставить("НастройкаОбмена", НастройкаОбмена);
			
		КонецЕсли;
		РаспределитьДокументыПоСтатусам(ДополнительныеПараметры);
...
// Формирует массив сообщений которые необходимо будет подписать по SMS из полученного массива сообщений.
//
// Параметры:
//   МассивСообщенийОбмена - Массив из ДокументСсылка.СообщениеОбменСБанками - массив сообщений
//    из которого нужно выбрать только те, кторые нужно будет подписать по SMS;  
//
// Возвращаемое значение:
//   Массив из ДокументСсылка.СообщениеОбменСБанками - массив сообщений, формируемый процедурой
//
Функция СообщенияОбменаКПодписаниюКодомИзSMS(МассивСообщенийОбмена)
	...
	|		ПО СообщениеОбменСБанками.ВидЭД = НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент
	|			И СообщениеОбменСБанками.НастройкаОбмена = НастройкиОбменСБанкамиИсходящиеДокументы.Ссылка
	|ГДЕ
	|	СообщениеОбменСБанками.Ссылка В(&МассивСообщенийОбмена)
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.SMS)";
	Запрос.УстановитьПараметр("МассивСообщенийОбмена", МассивСообщенийОбмена);
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
КонецФункции

// Формирует массив сообщений которые необходимо будет подписать по SMS из полученного массива документов.
//
// Параметры:
//   МассивДокументов - Массив из ДокументСсылка.СообщениеОбменСБанками - массив платежных документов
//    из которого нужно выбрать связанные сообщения обмена и только те, кторые нужно будет подписать по SMS;
//
// Возвращаемое значение:
//   Массив из ДокументСсылка.СообщениеОбменСБанками - массив сообщений, формируемый процедурой
//
Функция СообщенияОбменаИзДокументовКПодписаниюКодомИзSMS(МассивДокументов)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СостоянияОбменСБанками.СообщениеОбмена КАК Ссылка
	|ИЗ
	|	РегистрСведений.СостоянияОбменСБанками КАК СостоянияОбменСБанками
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НастройкиОбменСБанками.ИсходящиеДокументы КАК
	|			НастройкиОбменСБанкамиИсходящиеДокументы
	|		ПО СостоянияОбменСБанками.СообщениеОбмена.ВидЭД = НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент
	|		И СостоянияОбменСБанками.СообщениеОбмена.НастройкаОбмена = НастройкиОбменСБанкамиИсходящиеДокументы.Ссылка
	|ГДЕ
	|	НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.SMS)
	|	И СостоянияОбменСБанками.СсылкаНаОбъект В (&МассивДокументов)
	|	И СостоянияОбменСБанками.СообщениеОбмена.ПрограммаБанка = ЗНАЧЕНИЕ(Перечисление.ПрограммыБанка.АсинхронныйОбмен)";
	Запрос.УстановитьПараметр("МассивДокументов", МассивДокументов);
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
КонецФункции

// Возвращает доступность подписания указанного вида электронного документа для указанной настройки обмена
//
// Параметры:
//  ВидЭД - ПеречислениеСсылка.ВидыЭДОбменСБанками - вид документа, для которого определяется доступность подписания.
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//
// Возвращаемое значение:
//  Булево - доступность подписания по SMS.
//
Функция ДоступноПодписаниеПоSMS(ВидЭД, НастройкаОбмена) Экспорт
	...
	|	Справочник.НастройкиОбменСБанками.ИсходящиеДокументы КАК НастройкиОбменСБанкамиИсходящиеДокументы
	|ГДЕ
	|	НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент = &ИсходящийДокумент
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.СпособПодтвержденияДокумента = ЗНАЧЕНИЕ(Перечисление.СпособыПодтвержденияДокументовОбменСБанками.SMS)
	|	И НастройкиОбменСБанкамиИсходящиеДокументы.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("ИсходящийДокумент", ВидЭД);
	Запрос.УстановитьПараметр("Ссылка", НастройкаОбмена);
	Возврат ?(Запрос.Выполнить().Пустой(), Ложь, Истина);
	
КонецФункции

// Распределяет только что отправленные документы по массивам для дальнейшей обработки, при подписании по SMS
//
Процедура РаспределитьДокументыПоСтатусам(ДополнительныеПараметры)
	
	// Разделим документы, которые уже могут быть подписаны, или уже в статусе ТребуетсяПодтверждение
	МассивСтатусов = Новый Массив;
	МассивСтатусов.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыОбменСБанками.Отправлен"));
	МассивСтатусов.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыОбменСБанками.Доставлен"));
	СообщенияОбменаДоСтатусаНеПодтвержден = СообщенияОбменаПоСтатусам(
		ДополнительныеПараметры.СообщенияОбменаКПодписаниюКодомИзSMS, МассивСтатусов);
	
	МассивСтатусов = Новый Массив;
	МассивСтатусов.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыОбменСБанками.НеПодтвержден"));
	СообщенияОбменаВСтатусеНеПодтвержден = СообщенияОбменаПоСтатусам(
		ДополнительныеПараметры.СообщенияОбменаКПодписаниюКодомИзSMS, МассивСтатусов);
		
	ДополнительныеПараметры.Вставить("СообщенияОбменаДоСтатусаНеПодтвержден", СообщенияОбменаДоСтатусаНеПодтвержден);
	ДополнительныеПараметры.Вставить("СообщенияОбменаВСтатусеНеПодтвержден", СообщенияОбменаВСтатусеНеПодтвержден);
	...
	ДополнительныеПараметры.Вставить("КоличествоСекунд", 1);
	
КонецПроцедуры

// Определяет возможность подписания по SMS документов находящихся в ДополнительныеПараметры и подготавливает
//  необходимые параметры 
//
// Параметры:
//  РезультатОперации -Структура - результат выполнения фонового задания ОбменСБанкамиСлужебный.СформироватьИОтправитьПакетыВБанк
//  МассивСсылокНаОбъект - Массив Из ДокументСсылка - объект информационной базы
//  ИмяПроцедурыПослеПодтвержденияКодомИзSMS - Строка - 
//   ОбменСБанкамиСлужебныйКлиент.ПослеОтправкиПлатежныхДокументовВБанк
//  СообщениеОбмена - ДокументСсылка.СообщениеОбменСБанками.
//
// Возвращаемое значение:
//  Структура - параметры необходимые для подписания по SMS
//
Функция ВозможноПодписаниеПоSMS(РезультатОперации, МассивСсылокНаОбъект, ИмяПроцедурыПослеПодтвержденияКодомИзSMS, СообщениеОбмена) Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ВозможноПодписаниеПоSMS", Ложь);
	
	Если ЗначениеЗаполнено(СообщениеОбмена) И ЭтоЗапросНаОтзывЭД(СообщениеОбмена) Тогда
		
		МассивДокументов = Новый Массив;
		МассивДокументов.Добавить(СообщениеОбмена);
		СообщенияОбменаКПодписаниюКодомИзSMS = СообщенияОбменаКПодписаниюКодомИзSMS(МассивДокументов);
		Если СообщенияОбменаКПодписаниюКодомИзSMS.Количество() > 0 Тогда
			ДополнительныеПараметры.Вставить("ЗапросНаОтзывЭД", СообщениеОбмена);
		КонецЕсли;
		
	Иначе
		
		СообщенияОбменаКПодписаниюКодомИзSMS = СообщенияОбменаКПодписаниюКодомИзSMS(
			РезультатОперации.ОтправленныеСообщенияОбмена);
		
		СообщенияОбменаИзДокументовКПодписаниюКодомИзSMS =
			СообщенияОбменаИзДокументовКПодписаниюКодомИзSMS(
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СообщенияОбменаКПодписаниюКодомИзSMS,
			СообщенияОбменаВСтатусеНеПодтвержден);
		
	КонецЕсли;
	
	Если СообщенияОбменаКПодписаниюКодомИзSMS.Количество() > 0 Тогда
		
		ДополнительныеПараметры.Вставить("СообщенияОбменаКПодписаниюКодомИзSMS",
			СообщенияОбменаКПодписаниюКодомИзSMS);
		ДополнительныеПараметры.Вставить("ИмяПроцедурыПослеПодтвержденияКодомИзSMS",
			ИмяПроцедурыПослеПодтвержденияКодомИзSMS);
		ДополнительныеПараметры.Вставить("РезультатОперацииПослеПодтвержденияКодомИзSMS", РезультатОперации);
		РаспределитьДокументыПоСтатусам(ДополнительныеПараметры);
		
		ДополнительныеПараметры.Вставить("ВозможноПодписаниеПоSMS", Истина);
...
// Определяет, что ВидЭД у СообщениеОбмена равен ЗапросНаОтзывЭД
//
// Параметры:
//  СообщениеОбмена -ДокументСсылка.СообщениеОбменСБанками - ссылка на сообщение обмена
//
// Возвращаемое значение:
//  Булево - признак, что ВидЭД у СообщениеОбмена равен ЗапросНаОтзывЭД
//
Функция ЭтоЗапросНаОтзывЭД(СообщениеОбмена) Экспорт
	
	ЭтоЗапросНаОтзывЭД = Ложь;
	ВидЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СообщениеОбмена, "ВидЭД");
	Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросНаОтзывЭД Тогда
		ЭтоЗапросНаОтзывЭД = Истина;
	КонецЕсли;
	
	Возврат ЭтоЗапросНаОтзывЭД;
...
Функция СформироватьРекомендацииПроверкиСетевойДоступности()Экспорт
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая(СтрокаСоединенияИнформационнойБазы()) Тогда
		Если ОбщегоНазначения.КлиентПодключенЧерезВебСервер() Тогда//
			Шаблон = НСтр("ru='Проверку следует выполнить на локальном компьютере %1, где запущен веб-сервер под учетной записью пользователя, от имени которого он работает'");
		Иначе
			Шаблон = НСтр("ru='Проверку следует выполнить на локальном компьютере %1, где работает пользователь 1С под учетной записью пользователя операционной системы'");
		КонецЕсли;
	Иначе
		Шаблон = НСтр("ru='Проверку следует выполнить на сервере 1С:Предприятия %1, где выполняется процесс rphost под учетной записью пользователя, от имени которого он работает (по умолчанию USR1CV8)'");	
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ИмяКомпьютера = "";
	Иначе
		ИмяКомпьютера = СтрШаблон("(%1)", ИмяКомпьютера());
	КонецЕсли;
	
	Возврат СтрШаблон(Шаблон, ИмяКомпьютера);
