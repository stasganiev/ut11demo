...
Функция ПараметрыПредпросмотраЧека(ПараметрыОперации) Экспорт
	...
	ПараметрыПредпросмотраЧека.Вставить("ВзаиморасчетыОбновлены", ВзаиморасчетыОбновлены());
	
	// ОбновитьВзаиморасчеты
	ОбновитьВзаиморасчеты(ПараметрыОперации, ВзаиморасчетыОбновлены);
	
	// ПараметрыЕдиногоЧека
	ПараметрыПредпросмотраЧека.Вставить("ДокументыЕдиногоЧека", ПараметрыЕдиногоЧека(ПараметрыОперации));
	...
	ПараметрыПредпросмотраЧека.Вставить("ОбъектыРасчетовПоДокументу", ОбъектыРасчетовПоДокументу);
	
	// ПризнакиСпособаРасчетаАвто
	ПризнакиСпособаРасчетаАвто = ПризнакиСпособаРасчетаАвто(ПараметрыОперации, ОбъектыРасчетовПоДокументу, ВзаиморасчетыОбновлены);
	ПараметрыПредпросмотраЧека.Вставить("ПризнакиСпособаРасчетаАвто", ПризнакиСпособаРасчетаАвто);
	
	// ДанныеФискальнойОперации
	ДанныеФискальнойОперации = ДанныеФискальнойОперации();
	ПараметрыПредпросмотраЧека.Вставить("ДанныеФискальнойОперации", ДанныеФискальнойОперации);

	// ПараметрыФискальногоЧека
	ПараметрыПредпросмотраЧека.Вставить("ПараметрыОперацииФискализацииЧека", ПараметрыОперацииФискализацииЧека(ПараметрыОперации));
		
	// ПараметрыПерсональныхДанныхПокупателя
	ПараметрыПроверкиРазрешенияНаВводПерсональныхДанных = Новый Структура;
	ПараметрыПроверкиРазрешенияНаВводПерсональныхДанных.Вставить("ЕдиныйЧек", ПараметрыПредпросмотраЧека.ПараметрыОперацииФискализацииЧека.ЕдиныйЧек);
	ПараметрыПроверкиРазрешенияНаВводПерсональныхДанных.Вставить("ДокументСсылка", ПараметрыПредпросмотраЧека.ПараметрыОперацииФискализацииЧека.ДокументОснование);
	ПараметрыПроверкиРазрешенияНаВводПерсональныхДанных.Вставить("Контрагент", ПараметрыОперации.Контрагент);	
	ПараметрыПредпросмотраЧека.Вставить("РазрешенВводПерсональныхДанных", РазрешенВводПерсональныхДанных(ПараметрыПроверкиРазрешенияНаВводПерсональныхДанных));
...
Функция ОбновитьПараметрыФискальногоЧека(ПараметрыОперацииЧека, ВерсияФФД) Экспорт
	...
	ОбновитьПараметрыФискальногоЧекаПараметрыОтправкиЭлектронногоЧека(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека);
	
	ОбновитьПараметрыФискальногоЧекаПерсональныеДанные(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека, ВерсияФФД);
	
	ОбновитьПараметрыФискальногоЧекаПризнакСпособаРасчетаПозицийЧека(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека);
	...
	ТипОборудования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОперацииЧека.ОборудованиеККТ, "ТипОборудования");
	Если ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ Тогда
		ФорматноЛогическийКонтрольКлиентСервер.ПровестиФорматноЛогическийКонтроль(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека.ОборудованиеККТ);
	КонецЕсли;
	//-- Локализация
...
Функция РазрешенВводПерсональныхДанных(ПараметрыОперацииЧека) Экспорт
	...
	РазрешенВводПерсональныхДанных = (ЗначениеЗаполнено(ПараметрыОперацииЧека.ЕдиныйЧек) И ПараметрыОперацииЧека.ЕдиныйЧек)
									Или ДокументыРазрешенныеКВводуПерсональныхДанных.Найти(ТипЗнч(ПараметрыОперацииЧека.ДокументСсылка)) <> Неопределено;
	
	Если РазрешенВводПерсональныхДанных Тогда
		ЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОперацииЧека.Контрагент, "ЮрФизЛицо");
	...
	Если РазрешенВводПерсональныхДанных И ТипЗнч(ПараметрыОперацииЧека.ДокументСсылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОперацииЧека.ДокументСсылка, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации Тогда
			РазрешенВводПерсональныхДанных = Ложь;
		КонецЕсли;
	КонецЕсли;
	//-- Локализация
...
Функция ВзаиморасчетыРассчитаны(ПараметрыОперации) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		ДатаРасчета = РаспределениеВзаиморасчетовВызовСервера.НачалоРасчетов(ТекущаяДатаСеанса(), АналитикиРасчета, "РасчетыСКлиентами");
		Если ДатаРасчета <> Неопределено И ДатаРасчета < ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОперации.ДокументСсылка, "Дата") Тогда
...
Функция ПризнакиСпособаРасчетаАвто(ПараметрыОперации, ОбъектыРасчетовПоДокументу, ВзаиморасчетыОбновлены)
	...
	Для Каждого ДанныеПоОбъектуРасчета Из ОбъектыРасчетовПоДокументу Цикл
		...
		Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
			ПризнакиСпособаРасчетаАвтоПоОбъекту = ОпределитьАвтоПризнакСпособаРасчетовПоОфлайнРасчетам(ПараметрыОперации, ДанныеПоОбъектуРасчета);
		Иначе
			ПризнакиСпособаРасчетаАвтоПоОбъекту = ОпределитьАвтоПризнакСпособаРасчетовПоОнлайнРасчетам(ПараметрыОперации, ДанныеПоОбъектуРасчета);
		КонецЕсли;
...
Функция ШтрихкодыУпаковокЗаполнены(Объект) Экспорт
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции() Тогда
		ИнтеграцияИСУТ.ПроверитьЗаполнениеАкцизныхМарок(Объект, Результат);
	КонецЕсли;
...
Процедура ОбновитьПараметрыФискальногоЧекаПараметрыКоррекции(ПараметрыОперацииФискализацииЧека, ПараметрыОперацииЧека)
	...
	Если ПараметрыОперацииЧека.ТипФискальногоДокумента = Перечисления.ТипыФискальныхДокументовККТ.КассовыйЧекКоррекции Тогда
		...
		ФормированиеПараметровФискальногоЧекаСервер.ПараметрыФискальногоЧекаЗаполнитьПараметрыКоррекции(
			ПараметрыОперацииФискализацииЧека,
			?(ПараметрыОперацииЧека.ВидЧекаКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОперацииЧека.ДокументСсылка, "ДокументОснование"), Неопределено),
...
// Возвращает массив значений видов возможных фискальных чеков документа
// 
// Параметры:
...
//
// Возвращаемое значение:
// 	Массив, Неопределено - Массив значений видов возможных фискальных чеков документа
Функция ПараметрыВидовФискальногоЧека(ДокументСсылка, ИмяКомандыПробитияЧека)
	...
	Если ДокументОплатыСВозможностьюПробитияЧеков(ДокументСсылка)
		...
		МодульЛокализации.ЗаполнитьВидыФискальныхЧековПоДокументу(
			...
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ХозяйственнаяОперация"),
			ИмяКомандыПробитияЧека);
...
Процедура ДокументыЕдиногоЧекаЗаполнитьТаблицу(ДокументСсылка, ТаблицаДокументовЕдиногоЧека, ИмяКомандыПробитияЧека)
	...
	Для Каждого ДокументОплаты Из ДокументыОплаты Цикл
		...
		СтрокаДокументЕдиногоЧека.Сумма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОплаты, "СуммаДокумента");
	КонецЦикла;
	...
	Если ОбъектыРасчетов <> Неопределено
		...
		Для Каждого ДокументВзаимозачета Из ДокументыВзаимозачета Цикл
			...
			СтрокаДокументЕдиногоЧека.Сумма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументВзаимозачета, "СуммаРегл");
		КонецЦикла;
	КонецЕсли;
...
Функция ПредоплатаПоДокументуПоставкиВРамкахОдногоДняЛокализация(ДокументСсылка)
	...
	Для Каждого ДокументОплаты Из ДокументыОплаты Цикл
		...
		СуммаПредоплаты = СуммаПредоплаты + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОплаты, "СуммаДокумента");
	КонецЦикла;
...
Функция ДокументыВзаимозачета(ДокументыПоставки)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВзаимозачетЗадолженностиДебиторскаяЗадолженность.Ссылка.Дата";
...
Функция ДатаОплатыОтличаетсяОтДатыПоставки(ДокументОплаты, ОбъектыРасчетов)
	...
	ДатаДокументаОплаты = НачалоДня(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОплаты, "Дата"));
	Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
		
		ДатаОбъектаРасчета = НачалоДня(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОбъектыРасчетов.Заказ, "Дата"));
...
Функция РассчитатьСтруктуруОплатДокументаПоставки(ПризнакиСпособаРасчетаАвто, СуммаДокумента, ПризнакСпособаРасчетаЗаданВручную = Ложь, УточнениеТипаОплатПоСуммам = Неопределено)
