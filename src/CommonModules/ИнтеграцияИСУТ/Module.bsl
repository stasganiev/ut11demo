	...
	Для Каждого СтрокаТовары Из Объект[ИмяТаблицыТовары] Цикл
		...
		Если ПроверитьСерииВТаблицеСерии
			...
			Для Каждого СтрокаТовар Из СтрокиТоваров Цикл
				...
				КонецЦикла;
				Если КоличествоМарок <> КоличествоЕдиницТоваров Тогда
					ПредставлениеНоменклатуры = ИнтеграцияИС.ПредставлениеНоменклатуры(СтрокаТовар.Номенклатура, СтрокаТовар.Характеристика,, СтрокаТовар.Серия);
...
Функция СписокИспользуемыхПодсистем(ДобавитьОписание = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		Если ВидыПродукцииИСМП.Количество() Тогда
			МассивИспользуемыхПодсистем.Добавить(ИнтеграцияИСМПКлиентСервер.ПредставлениеПодсистемы() 
...
Функция ИспользуютсяПодсистемыГОСИС() Экспорт
	...
	Возврат ИнтеграцияИСПовтИсп.ВедетсяУчетМаркируемойПродукции() 
...
// Заполняет серии табличной части из связанных с прикладным документом приходных и расходных ордеров на товары.
//
// Параметры:
//  ИмяТабличнойЧасти  - Строка - табличная часть документа ГосИС в которую надо указать связанные серии, с колонками:
...
//  ДокументГосИС - ДокументОбъект - заполняемый документ ГосИС
//  РеквизитыСерий - Структура - реквизиты табличной части заполняемые из серии
//                 - Неопределено - если нужно заполнить только серию
//
Процедура ЗаполнитьСерииПоСкладскимДвижениям(ИмяТабличнойЧасти, ДокументГосИС, РеквизитыСерий = Неопределено) Экспорт
	
	ДокументОснование = ДокументГосИС.ДокументОснование;
	ТабличнаяЧасть = ДокументГосИС[ИмяТабличнойЧасти];
	...
	СтрокиКЗаполнению = ТабличнаяЧасть.НайтиСтроки(Новый Структура("Серия", Справочники.СерииНоменклатуры.ПустаяСсылка()));
	Если СтрокиКЗаполнению.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ЕстьПолеКоличествоУпаковок = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокиКЗаполнению[0], "КоличествоУпаковок");
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Распоряжение", ДокументОснование);
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	СкладскиеДвижения КАК Таблица
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	|	Порядок";
	
	Если РеквизитыСерий <> Неопределено Тогда
		Для Каждого КлючИЗначение Из РеквизитыСерий Цикл
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
				"Таблица.Серия КАК Серия,",
				СтрШаблон("Таблица.Серия.%1 КАК %2,", КлючИЗначение.Ключ, КлючИЗначение.Значение) + "
					|	Таблица.Серия КАК Серия,");
		КонецЦикла;
	КонецЕсли;
	...
	ДвиженияСерийТоваров = Запрос.Выполнить().Выгрузить();
	ДвиженияСерийТоваров.Индексы.Добавить("Номенклатура,Характеристика");
	Отбор = Новый Структура("Номенклатура,Характеристика");
	Для Каждого СтрокаБезСерии Из СтрокиКЗаполнению Цикл
		Знак = ?(СтрокаБезСерии.Количество < 0, -1, 1);
		Количество = СтрокаБезСерии.Количество * Знак;
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаБезСерии);
		ДанныеСерий = ДвиженияСерийТоваров.НайтиСтроки(Отбор);
		Для Каждого СтрокаСерии Из ДанныеСерий Цикл
			Заполнить = Мин(Количество, СтрокаСерии.Количество);
			Если Заполнить = 0 Тогда
				Продолжить;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СтрокаБезСерии.Серия) Тогда
				ЗаполнитьЗначенияСвойств(СтрокаБезСерии,СтрокаСерии,,"Количество");
			Иначе
				НоваяСтрока = ТабличнаяЧасть.Вставить(ТабличнаяЧасть.Индекс(СтрокаБезСерии));
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаБезСерии);
				ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаСерии,,"Количество");
				НоваяСтрока.Количество = Знак * Заполнить;
				Если ЕстьПолеКоличествоУпаковок Тогда
					НоваяСтрока.КоличествоУпаковок = НоваяСтрока.Количество / СтрокаБезСерии.Количество * СтрокаБезСерии.КоличествоУпаковок;
					СтрокаБезСерии.КоличествоУпаковок = СтрокаБезСерии.КоличествоУпаковок - НоваяСтрока.КоличествоУпаковок;
				КонецЕсли;
				СтрокаБезСерии.Количество = СтрокаБезСерии.Количество - НоваяСтрока.Количество;
			КонецЕсли;
			Количество = Количество - Заполнить;
			СтрокаСерии.Количество = СтрокаСерии.Количество - Заполнить;
		КонецЦикла;
	КонецЦикла;
	
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ДвиженияСерийТоваров.Количество() = 0 Тогда
		...
		Если ЗначениеЗаполнено(ЗаказКлиента) Тогда 
			ЗаполнитьСерииПоСкладскимДвижениямОтЗаказаКлиента(ИмяТабличнойЧасти, ДокументГосИС, ЗаказКлиента, РеквизитыСерий);
		КонецЕсли;
	КонецЕсли;
...
Функция КоэффициентыПоВидамУпаковок(Форма)
	...
	Номенклатура = Форма.Объект.Ссылка;
	МассивУпаковок = Новый Массив();
	МассивУпаковок.Добавить(Новый Структура("Упаковка,Номенклатура", Форма.ПотребительскаяУпаковкаИС,   Номенклатура));
	МассивУпаковок.Добавить(Новый Структура("Упаковка,Номенклатура", Форма.УпаковкаЧастичногоВыбытияИС, Номенклатура));

	КоэффициентыУпаковок = Справочники.УпаковкиЕдиницыИзмерения.КоэффициентыУпаковок(МассивУпаковок)[Номенклатура];
...
// Заполняет серии табличной части из связанных с заказом клиента расходных ордеров на товары и уже оформленных документов.
//
// Параметры:
//  ИмяТабличнойЧасти  - Строка - табличная часть документа ГосИС в которую надо указать связанные серии, с колонками:
//  * Номенклатура - СправочникСсылка.Номенклатура - номенклатура
...
//  * Количество - Число - количество
//  ДокументГосИС - ДокументОбъект - заполняемый документ ГосИС
//  ЗаказКлиента - ДокументСсылка.ЗаказКлиента - заказ клиента по которому оформлены расходные ордера
//  РеквизитыСерий - Структура - реквизиты табличной части заполняемые из серии
//                 - Неопределено - если нужно заполнить только серию
//
Процедура ЗаполнитьСерииПоСкладскимДвижениямОтЗаказаКлиента(ИмяТабличнойЧасти, ДокументГосИС, ЗаказКлиента, РеквизитыСерий)
	...
	ИмяДокументаГосИС = ДокументГосИС.Метаданные().Имя;
	
	ЕстьПолеКоличествоУпаковок = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокиКЗаполнению[0], "КоличествоУпаковок");
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Распоряжение", ЗаказКлиента);
	Запрос.УстановитьПараметр("Ссылка", ДокументГосИС.Ссылка);
	...
	|ИЗ
	|	Документ.%1 КАК ДокументИС
	|ГДЕ
	|	ДокументИС.Проведен
	|	И ДокументИС.Ссылка <> &Ссылка
	|	И ВЫРАЗИТЬ(ДокументИС.ДокументОснование КАК Документ.РеализацияТоваровУслуг).ЗаказКлиента = &Распоряжение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.%1.%2 КАК ОформленныеСерии
	|	ГДЕ	ОформленныеСерии.Ссылка В
	|		(ВЫБРАТЬ
	|			Ссылка
	|		Из
