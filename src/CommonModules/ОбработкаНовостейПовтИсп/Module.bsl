...
Функция ПолучитьЛентуНовостейПоКоду(ЛентаНовостейКод) Экспорт
	...
	Если ТипЗнч(ЛентаНовостейКод) <> ТипСтрока
			ИЛИ ПустаяСтрока(ЛентаНовостейКод)
			ИЛИ СтрДлина(ЛентаНовостейКод) > 20 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='ПолучитьЛентуНовостейПоКоду: параметр [ЛентаНовостейКод] (%1) недопустимого типа [%2] или пустая строка или строка длиной более 20 символов [%3].'"),
				ЛентаНовостейКод,
				ТипЗнч(ЛентаНовостейКод),
				СтрДлина(ЛентаНовостейКод));
...
#Область ЛентыНовостей

// Возвращает список категорий новостей, настроенных в табличной части "ДоступныеКатегорииНовостей" ленты новостей.
//
// Параметры:
//  ЛентаНовостей - СправочникСсылка.ЛентыНовостей - обрабатываемая лента новостей.
//
// Возвращаемое значение:
//   Массив из ПланВидовХарактеристикСсылка.КатегорииНовостей - список элементов табличной части "ДоступныеКатегорииНовостей".
//
Функция ДоступныеКатегорииНовостейЛентыНовостей(ЛентаНовостей) Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Результат = РезультатЗапроса.Выгрузить(ОбходРезультатаЗапроса.Прямой).ВыгрузитьКолонку("КатегорияНовостей");
	КонецЕсли;
...
Функция ПредопределенныеКатегорииНовостей() Экспорт
	...
	Коды = Новый Массив;
		Коды.Добавить("Product");
		Коды.Добавить("ProductVersion");
		Коды.Добавить("PlatformVersion");

	Результат = Новый Структура;
	Для Каждого ТекущийКод Из Коды Цикл
		Результат.Вставить(ТекущийКод, ПланыВидовХарактеристик.КатегорииНовостей.ПустаяСсылка());
	КонецЦикла;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			Результат[Выборка.Код] = Выборка.Ссылка;
		КонецЦикла;
	КонецЕсли;
