...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ИЛИ МашиночитаемыеДоверенностиКонтрагентов.ВариантЗаполненияПолномочий = 
	|				ЗНАЧЕНИЕ(Перечисление.ВариантыЗаполненияПолномочийМЧД.КлассификаторФНС)
...
Функция ПолучитьЧастичныеДанныеДоверенностиНаСервереМЧД(НомерДоверенности, ТокенДоступа = "") Экспорт
	...
	Попытка
		...
		Результат.ДатаОбновленияСтатуса = ?(ТипЗнч(СтруктураОтвета) = Тип("Структура") И СтруктураОтвета.Свойство("statusDate"), 
			СтроковыеФункцииКлиентСервер.СтрокаВДату(СтруктураОтвета.statusDate), Неопределено);
...
#Область Справочники_КлассификаторПолномочийФНСМЧД003_ОбработатьДанныеДляПереходаНаНовуюВерсию

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.9.12.4";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Процедура = "Справочники.КлассификаторПолномочийФНСМЧД003.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("47512c62-7886-4b7a-8450-db6dc8a12e8e");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ЭлектронноеВзаимодействие.ЗаполнениеДанныхОбновления";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Справочники.КлассификаторПолномочийФНСМЧД003.ПолноеИмя());
	Читаемые.Добавить(Метаданные.Справочники.ВидыДокументовЭДО.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Справочники.КлассификаторПолномочийФНСМЧД003.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");

	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление классификатора полномочий ФНС (МЧД 003).'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Процедура = "Справочники.КлассификаторПолномочийФНСМЧД003.ОбработатьДанныеДляПереходаНаНовуюВерсиюНачальноеЗаполнение";
	...
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление классификатора полномочий ФНС (МЧД 003) (начальное заполнение).'");
...
Функция ПолучитьДоверенностиОрганизации(Отбор) Экспорт
	...
	|			МашиночитаемыеДоверенностиОрганизацийПредставители
	|		ПО МашиночитаемыеДоверенностиОрганизаций.Ссылка = МашиночитаемыеДоверенностиОрганизацийПредставители.Ссылка
	|		И МашиночитаемыеДоверенностиОрганизаций.ДоверительЮЛ_ИНН = &ДоверительЮЛ_ИНН
	|		И МашиночитаемыеДоверенностиОрганизаций.ДоверительЮЛ_КПП В (&ДоступныеКППДоверителя)
	|		И МашиночитаемыеДоверенностиОрганизацийПредставители.ФЛ_ИНН = &ПредставительИНН
	|		И НЕ МашиночитаемыеДоверенностиОрганизаций.ПометкаУдаления
	|		И МашиночитаемыеДоверенностиОрганизаций.СтатусВРеестреФНС В (&СтатусВРеестреФНС)
	|		И НЕ МашиночитаемыеДоверенностиОрганизаций.СовместныеПолномочия
	...
	|	ВТ_Доверенности.ДатаВыдачи,
	|	ВТ_Доверенности.ДатаОкончания,
	|	ВТ_Доверенности.СтатусВРеестреФНС,
	|	ВТ_Доверенности.Верна,
	|	ВТ_Доверенности.Отозвана,
	|	ВТ_Доверенности.ДатаОтзыва,
	|	ВТ_Доверенности.ПолномочияОграничены,
	|	ВТ_Доверенности.ПолномочияУказаныИзКлассификатора
	|ИЗ
	|	ВТ_Доверенности КАК ВТ_Доверенности";
	
	РеквизитыОрганизации = ИнтеграцияЭДО.РегистрационныеДанныеОрганизации(Доверитель);
	ДоступныеКППДоверителя = Новый Массив;
	ДоступныеКППДоверителя.Добавить(РеквизитыОрганизации.КПП);
	ДоступныеКППДоверителя.Добавить(ОбщегоНазначенияБЭД.КППГоловнойОрганизации(РеквизитыОрганизации.КПП));
	
	Запрос.УстановитьПараметр("Организация", Доверитель);
	Запрос.УстановитьПараметр("ДоверительЮЛ_ИНН", РеквизитыОрганизации.ИНН);
	Запрос.УстановитьПараметр("ДоступныеКППДоверителя", ДоступныеКППДоверителя);
...
//
Процедура ОбновитьКлассификаторПолномочийФНС_МЧД003(ДанныеКлассификатора) Экспорт
	...
	Попытка
		...
		РодителиПолномочий.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.КлассификаторПолномочийФНСМЧД003"));
		
		Для Каждого ДанныеРодителя Из РодителиПолномочий Цикл
			
			СсылкаДанныхИБ = Справочники.КлассификаторПолномочийФНСМЧД003.ПолучитьСсылку(
		...
		Пока ВыборкаДетальныеЗаписиРодителей.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаДетальныеЗаписиРодителей.Ссылка) Тогда
				...
				СсылкаНаГруппу = Справочники.КлассификаторПолномочийФНСМЧД003.ПолучитьСсылку(
		...
		|ВЫБРАТЬ
		|	КлассификаторПолномочийФНСМЧД003.Ссылка,
		|	КлассификаторПолномочийФНСМЧД003.ДатаИзменения,
		|	КлассификаторПолномочийФНСМЧД003.КодКлассификатора КАК КодКлассификатора
		|ПОМЕСТИТЬ ТаблицаТекущихДанных
		|ИЗ
		|	Справочник.КлассификаторПолномочийФНСМЧД003 КАК КлассификаторПолномочийФНСМЧД003
		|ГДЕ
		...
		ЗапросЭлементов.УстановитьПараметр("ТаблицаНовыхДанныхЭлементов", ДанныеКлассификатора.КлассификаторПолномочийФНСМЧД003);
...
Функция ПрочитатьАрхивДоверенности(ДвоичныеДанныеАрхива)
	...
	ДанныеФайлов = МашиночитаемыеДоверенностиКлиентСервер.НовыеДанныеДляЗагрузкиМЧД();
	ДанныеФайлов.ДанныеДоверенности = ДанныеДоверенности;
	...
	Для Каждого РасширениеПодписи Из МассивРасширений Цикл
		...
		Для Индекс = 0 По ВсеФайлыСРасширением.ВГраница() Цикл
			...
			Если ЭтоЗаявлениеНаОтмену Тогда
				ЗаполняемаяКоллекция = ДанныеФайлов.ДанныеПодписейЗаявленияНаОтмену;
			Иначе
				ЗаполняемаяКоллекция = ДанныеФайлов.ДанныеПодписей;
			КонецЕсли;
			
			ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ВсеФайлыСРасширением[Индекс].ПолноеИмя);
			ДанныеПодписи = ОбщегоНазначенияБЭДКлиентСервер.ДекодированныеДвоичныеДанные(ДвоичныеДанныеФайла);
	...
	Если ДанныеФайлов.ДанныеПодписей.Количество() > 0 Тогда
		ДанныеФайлов.ДанныеПодписи = ДанныеФайлов.ДанныеПодписей[0];
	КонецЕсли;
	Если ДанныеФайлов.ДанныеПодписейЗаявленияНаОтмену.Количество() > 0 Тогда
		ДанныеФайлов.ДанныеПодписиЗаявленияНаОтмену = ДанныеФайлов.ДанныеПодписейЗаявленияНаОтмену[0];
	КонецЕсли;
			...
			Если Не ПустаяСтрока(НаименованиеФайлаДоверенности) 
				...
				Для Каждого ЭлементАрхива Из ОбъектЧтение.Элементы Цикл
					Если ЭтоПодписьМЧД(ЭлементАрхива, НаименованиеФайлаДоверенности) Тогда
							ДвоичныеДанные = Новый ДвоичныеДанные(КаталогРаспаковки + ЭлементАрхива.Имя);
							Результат.ДанныеПодписи = ОбщегоНазначенияБЭДКлиентСервер.ДекодированныеДвоичныеДанные(ДвоичныеДанные);
					ИначеЕсли ЭтоПодписьЗаявленияНаОтмену(ЭлементАрхива, НаименованиеФайлаЗаявленияНаОтмену) Тогда
							ДвоичныеДанные = Новый ДвоичныеДанные(КаталогРаспаковки + ЭлементАрхива.Имя);
							Результат.ДанныеПодписиЗаявленияНаОтмену = ОбщегоНазначенияБЭДКлиентСервер.ДекодированныеДвоичныеДанные(ДвоичныеДанные);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из РезультатПолученияПолномочий.Полномочия Цикл
			...
			Если ЗначениеЗаполнено(РеквизитыПолномочия.Скрипт) Тогда
				...
				Попытка
					...
					ОбщегоНазначения.ВыполнитьВБезопасномРежиме(РеквизитыПолномочия.Скрипт, ПараметрыПроверки);
					
					Успех = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ, "Успех", Ложь);
					ТекстОшибки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ, "ТекстОшибки", "");
			...
			Если ПроверятьДополнительныйСкрипт Тогда
				...
				Попытка
					...
					ОбщегоНазначения.ВыполнитьВБезопасномРежиме(
						РеквизитыПолномочия.ДополнительныйСкрипт, ПараметрыПроверки);
...
Функция ЗапросДанныхМЧДДляПодписания(Доверенности, ОтпечаткиСертификатов, КонтекстДиагностики)
	...
	ШаблонТекстаЗапросаДанныхДоверенностей = 
		...
		|	ДоверенностиДляОбработки.ИдентификаторОрганизации КАК ИдентификаторОрганизации,
	...
	ТекстПоляОрганизацииИНН = СтрШаблон("ДоверенностиДляОбработки.Организация.%1", ИмяРеквизитаИННОрганизации);
	ТекстПоляОрганизацииОГРН = СтрШаблон("ДоверенностиДляОбработки.Организация.%1",
...
//     ** Значение - Строка
//
Функция СоздатьНаборСертификатовДляПодписанияМЧД(ИННОрганизации, ПараметрыОтбораСертификата, СертификатыДляПодписания)
	...
	Для Каждого СтрокаТаблицы Из СертификатыДляПодписания Цикл
		...
		Если СтрокаТаблицы.ИННОрганизации <> ИННОрганизации Тогда
			Продолжить;
		КонецЕсли;
...
Функция НомераДоверенностейСИННДоверителейПоУчастникамB2B(ДоверительИНН, ПредставительИНН, ДействительныеНаДату,
	...
	Если ПодобратьМЧДКонтрагента Тогда
		...
		|		И ВЫБОР
		|			КОГДА МашиночитаемыеДоверенностиКонтрагентов.ИННДоверителяРодительскойДоверенности <> """"
		|				ТОГДА МашиночитаемыеДоверенностиКонтрагентов.ИННДоверителяРодительскойДоверенности = &ДоверительИНН
		|			ИНАЧЕ МашиночитаемыеДоверенностиКонтрагентов.ДоверительИНН = &ДоверительИНН
		|		КОНЕЦ
	...
	Иначе	
		...
		|		И ВЫБОР
		|			КОГДА МашиночитаемыеДоверенностиОрганизаций.ИННДоверителяРодительскойДоверенности <> """"
		|				ТОГДА МашиночитаемыеДоверенностиОрганизаций.ИННДоверителяРодительскойДоверенности = &ДоверительИНН
		|			ИНАЧЕ МашиночитаемыеДоверенностиОрганизаций.ДоверительЮЛ_ИНН = &ДоверительИНН
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	ДоверительЯвляетсяУполномоченнымПредставителемОрганизации =
		...
		Или (ДоверенностьВыданаГосОрганомУЦ
				И КриптографияБЭД.ЭтоСертификатДолжностногоЛицаГосОрганаУЦ(Сертификат));
