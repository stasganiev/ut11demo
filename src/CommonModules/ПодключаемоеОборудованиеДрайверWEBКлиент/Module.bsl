...
Процедура НачатьПолучениеОбъектаДрайвера(ОповещениеПриЗавершении, ПараметрыПодключения) Экспорт
	...
	Иначе
		...
		Параметры.ТекстПояснения = ТекстСообщения;
		ВнешниеКомпонентыБПОКлиент.ПодключитьКомпоненту(Оповещение,  ПараметрыПодключения.ИдентификаторОбъекта, ПараметрыПодключения.ВерсияДрайвера, Параметры);
	КонецЕсли;
...
//  Контекст - см. КонтекстПолученияОписанияДрайвера.
//
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
				...
				КонецЕсли;
				РезультатВызова = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ДанныеОперации.Электронно, ДанныеЧекаXML, ДанныеОперации.РезультатОперацииXML); // Струкутра 
...
Функция ТекущееСостояние(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
			РезультатВызова = ОбъектДрайвера.ПолучитьТекущееСостояние(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, ДанныеОперации.РезультатXML); // ОбъектВнешнейКомпоненты
		Иначе
			РезультатВызова = ОбъектДрайвера.ПолучитьТекущееСостояние(ПараметрыПодключения.ИДУстройства, ДанныеОперации.НомерЧекаККТ, ДанныеОперации.НомерСменыККТ, // ОбъектВнешнейКомпоненты
...
Функция ОперацияФискальногоНакопителя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = ОбъектДрайвера.ОперацияФН(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ТипОперации, ДанныеОперации.ПараметрыXML);
		Если НЕ РезультатВызова.Значение Тогда   
			РезультатВыполнения = ПолучитьОшибкуДрайвера(ОбъектДрайвера);
			Возврат РезультатВыполнения;
		КонецЕсли;                            
...
Функция НапечататьКопиюЧека(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ДанныеОперации.Свойство("Аппаратно") И ДанныеОперации.Аппаратно Тогда
		...
		Попытка
			РезультатВызова = ОбъектДрайвера.НапечататьКопиюЧека(ПараметрыПодключения.ИДУстройства, НомерЧека);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина); 
...
Функция ЗапросКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = ОбъектДрайвера.ЗапросКМ(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, "");   
		Если РезультатВызова.Значение Тогда  
			ПараметрыВызова = РезультатВызова.Параметры;     
