...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры, ДополнительныеСвойстваПроведения = Неопределено) Экспорт
	...
	|	ОсновныеПараметрыРасчета.ПустойКорОбъектРасчетов КАК ПустойКорОбъектРасчетов,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК Справочник
	...
	ИспользоватьРегистрКоличествоЗаписейРегистраторовРасчетов = ОбработчикОбновленияВыполнен(
	...
	ИспользоватьНовыеРеквизитыСвязиДвижений = ОбработчикОбновленияВыполнен(МассивОбработчиков);
	ГлобальныеПеременные.Вставить("ИспользоватьНовыеРеквизитыСвязиДвижений", ИспользоватьНовыеРеквизитыСвязиДвижений);
	...
	Для Каждого СтрокаОсновныхПараметровРасчета Из ТаблицаОсновныхПараметровРасчета Цикл
			...
			Если Индексы.ИндексОплаты <= ЗаписейОплаты - 1 И Индексы.ИндексОтгрузкиПоставки <= ЗаписейОтгрузкиПоставки - 1 Тогда 
				...
				Если СтрокаОтгрузкиПоставки.СуммаДляРаспределения > 0 И СтрокаОплаты.СуммаДляРаспределения > 0 Тогда
					...
					Если СтрокаОплаты.ПорядокОперации < СтрокаОтгрузкиПоставки.ПорядокОперации 
						...
						Если ПропускОплаты = 0 И ЗачетОплатПоДатеПлатежа 
							И Индексы.ИндексОплаты < ЗаписейОплаты - 1 Тогда
							
							Индекс = Индексы.ИндексОплаты + 1; 
							Пока Истина Цикл
								Если Индекс <= ЗаписейОплаты-1
									И РасчетыОплаты[Индекс].ПорядокОперации < СтрокаОтгрузкиПоставки.ПорядокОперации Тогда
									Если РасчетыОплаты[Индекс].ЕстьОстаток
										И СтрокаОплаты.ПорядокЗачетаПоДатеПлатежа > РасчетыОплаты[Индекс].ПорядокЗачетаПоДатеПлатежа
					...
					Иначе
						...
						Если НЕ СтрокаОплаты.ЭтоКорректировка 
							...
							Если СтрокаОтгрузкиПоставки.СуммаДляРаспределения > 0 И СтрокаОплаты.СуммаДляРаспределения > 0 Тогда
								...
								// Поищем их и увеличим пропуск строк отгрузки поставки
								Если ПропускОтгрузкиПоставки = 0 И ЗачетОплатПоДатеПлатежа 
									И Индексы.ИндексОтгрузкиПоставки < ЗаписейОтгрузкиПоставки - 1 Тогда
									
									Индекс = Индексы.ИндексОтгрузкиПоставки + 1; 
									Пока Истина Цикл
										Если Индекс <= ЗаписейОтгрузкиПоставки-1
											И РасчетыОтгрузкиПоставки[Индекс].ПорядокОперации < СтрокаОплаты.ПорядокОперации Тогда
											Если РасчетыОтгрузкиПоставки[Индекс].ЕстьОстаток
												И СтрокаОтгрузкиПоставки.ПорядокЗачетаПоДатеПлатежа > РасчетыОтгрузкиПоставки[Индекс].ПорядокЗачетаПоДатеПлатежа Тогда
		...
		Если НЕ СтрокаОсновныхПараметровРасчета.НачальноеЗаполнение И СтрокаОсновныхПараметровРасчета.ЗаписыватьИзменения И СтрокаОсновныхПараметровРасчета.РассчитыватьФакт Тогда
			...
			Если ДополнительныеСвойстваПроведения <> Неопределено Тогда
				Запрос.Текст = "
				|ВЫБРАТЬ *
		...
		Если НЕ СтрокаОсновныхПараметровРасчета.НачальноеЗаполнение И ДополнительныеСвойстваПроведения <> Неопределено И ГлобальныеПеременные.РассчитыватьФакт Тогда
...
Процедура ЗаполнитьОперативныеВзаиморасчетыПоТаблице(Параметры, ДополнительныеСвойства = Неопределено) Экспорт
	...
	ЭтоОтменаПроведения = ДополнительныеСвойства <> Неопределено
	...
	Если Параметры.РаспределениеСУчетомПриемников Тогда
		ДополнитьТаблицуИзмененийПриемниками(ТаблицаКРаспределению, Параметры, ЭтоОтменаПроведения);
	КонецЕсли;
...
Функция ТаблицаСвободныхРегистраторовРасчета(Параметры)
	...
	Иначе
		...
		|ИЗ
		|	Документ.РегистраторРасчетов КАК РегистраторРасчетов
		|		ЛЕВОЕ СОЕДИНЕНИЕ &ИмяТаблицыРасчетыПоСрокам КАК Таблица
		|		ПО Таблица.Регистратор = РегистраторРасчетов.Ссылка
		|ГДЕ
		...
		|	И РегистраторРасчетов.ТипРасчетов = &ТипРасчетов
		|	И НЕ РегистраторРасчетов.Ссылка В (&СписокПолныхРегистраторовРасчетыПоСрокам)
		|СГРУППИРОВАТЬ ПО
		|	РегистраторРасчетов.Ссылка";
		...
		Если РассчитыватьПлан Тогда
			...
			|ИЗ
			|	Документ.РегистраторРасчетов КАК РегистраторРасчетов
			|		ЛЕВОЕ СОЕДИНЕНИЕ &ИмяТаблицыРасчетыПланОплат КАК Таблица
			|		ПО Таблица.Регистратор = РегистраторРасчетов.Ссылка
			|ГДЕ
			|	РегистраторРасчетов.ОбъектРасчетов = &ОбъектРасчетов
			|	И РегистраторРасчетов.АналитикаУчетаПоПартнерам = &АналитикаУчетаПоПартнерам
			|	И РегистраторРасчетов.Валюта = &Валюта
			|	И РегистраторРасчетов.ТипРасчетов = &ТипРасчетов
			|	И НЕ РегистраторРасчетов.Ссылка В (&СписокПолныхРегистраторовРасчетыПланОплат)
			|СГРУППИРОВАТЬ ПО
			|	РегистраторРасчетов.Ссылка
			|
			|ОБЪЕДИНИТЬ ВСЕ
			...
			|ИЗ
			|	Документ.РегистраторРасчетов КАК РегистраторРасчетов
			|		ЛЕВОЕ СОЕДИНЕНИЕ &ИмяТаблицыПланОтгрузокПоставок КАК Таблица
			|		ПО Таблица.Регистратор = РегистраторРасчетов.Ссылка
			|ГДЕ
			|	РегистраторРасчетов.ОбъектРасчетов = &ОбъектРасчетов
			|	И РегистраторРасчетов.АналитикаУчетаПоПартнерам = &АналитикаУчетаПоПартнерам
			|	И РегистраторРасчетов.Валюта = &Валюта
			|	И РегистраторРасчетов.ТипРасчетов = &ТипРасчетов
			|	И НЕ РегистраторРасчетов.Ссылка В (&СписокПолныхРегистраторовРасчетыПланОтгрузокПоставок)
			|СГРУППИРОВАТЬ ПО
			|	РегистраторРасчетов.Ссылка";
		...
		Запрос.Текст = СтрСоединить(МассивТекстовЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицыРасчетыПоСрокам",      "РегистрНакопления." + ТаблицаРасчетыПоСрокам);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицыРасчетыПланОплат",     "РегистрНакопления." + ТаблицаРасчетыПланОплат);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицыПланОтгрузокПоставок", "РегистрНакопления." + ТаблицаРасчетыПланОтгрузокПоставок);
...
Процедура СформироватьВТДанныхРегистров(Запрос, ГлобальныеПеременные)
	...
	Если ГлобальныеПеременные.ИспользоватьНовыеРеквизитыСвязиДвижений Тогда
...
Функция ТекстЗапросаРасчетыПоСрокамОстатки(ГлобальныеПеременные, РасчетыСКлиентами = Истина, ИмяТаблицы = "")
	...
	|;
	|ВЫБРАТЬ
	|	ДвиженияПоРегистру.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	ДвиженияПоРегистру.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ДвиженияПоРегистру.Валюта КАК Валюта,
	|	ДвиженияПоРегистру.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ДвиженияПоРегистру.ДатаПлановогоПогашения КАК ДатаПлановогоПогашения,
	|	ДвиженияПоРегистру.ДатаВозникновения КАК ДатаВозникновения,
	|	ДвиженияПоРегистру.ПорядокЗачета КАК ПорядокЗачета,
	|	ДвиженияПоРегистру.ПорядокОперации КАК ПорядокОперации,
	|	ДвиженияПоРегистру.СвязанныйДокумент КАК СвязанныйДокумент,
	|	ДвиженияПоРегистру.ВалютаДокумента КАК ВалютаДокумента,
	|	ДвиженияПоРегистру.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	|	ДвиженияПоРегистру.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ДвиженияПоРегистру.ДокументРегистратор КАК ДокументРегистратор,
	|	ДвиженияПоРегистру.Активность КАК Активность,
	|	ДвиженияПоРегистру.ВидДвижения КАК ВидДвижения
	|ПОМЕСТИТЬ ВТ_Движения
	|ИЗ РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК ДвиженияПоРегистру
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОсновныеПараметрыРасчета КАК ВтПараметры
	|		ПО ВтПараметры.ПараметрАналитикаУчетаПоПартнерам = ДвиженияПоРегистру.АналитикаУчетаПоПартнерам 
	|			И ВтПараметры.ПараметрОбъектРасчетов = ДвиженияПоРегистру.ОбъектРасчетов 
	|			И ВтПараметры.ПараметрВалютаРасчетов = ДвиженияПоРегистру.Валюта
	|			И ВтПараметры.ПараметрЭтоРасчетыСКлиентами = &РасчетыСКлиентами
	|ИНДЕКСИРОВАТЬ ПО 
	|	АналитикаУчетаПоПартнерам,
	|	ОбъектРасчетов,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Движения КАК ВспомогательнаяИнформация
...
Функция ЗаписиДоНачалаРасчета(Запрос, ГлобальныеПеременные, ИмяТаблицы, ЭтоРасчетыСКлиентами = Ложь)
	...
	Иначе
		...
		|				И Таблица.Валюта = ВтПараметры.ПараметрВалютаРасчетов
		|ГДЕ
		|	Таблица.Период < ВтПараметры.ПараметрНачалоДняПорядка
		|	И ВтПараметры.ПараметрЭтоРасчетыСКлиентами = &ЭтоРасчетыСКлиентами
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	|	Расчеты.Сторно КАК Сторно,
	|	ВтПараметрыРасчета.ПараметрПорядок КАК Порядок,
	|	(Расчеты.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности) КАК Взаимозачет,
	...
	|					И (РасчетыПоСрокам.ОбъектРасчетов <> Расчеты.ОбъектРасчетов ИЛИ РасчетыПоСрокам.АналитикаУчетаПоПартнерам <> Расчеты.АналитикаУчетаПоПартнерам)
	|					И РасчетыПоСрокам.Активность
	...
	|			ПО Расчеты.ОбъектРасчетов = ВтПараметрыРасчета.ПараметрОбъектРасчетов
	|				И Расчеты.АналитикаУчетаПоПартнерам = ВтПараметрыРасчета.ПараметрАналитикаУчетаПоПартнерам
	|				И Расчеты.ВалютаРасчетов = ВтПараметрыРасчета.ПараметрВалютаРасчетов
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ВЫБОР КОГДА ДатаПлановогоПогашения = ДАТАВРЕМЯ(1,1,1)
	|		ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ,
	|	ДатаПлановогоПогашения";
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	|	РасчетыОтгрузкиПоставки.ОбъектРасчетов КАК ОбъектРасчетов,
	|	РасчетыОтгрузкиПоставки.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	РасчетыОтгрузкиПоставки.Валюта КАК Валюта,
	|	РасчетыОтгрузкиПоставки.ВалютаДокумента КАК ВалютаДокумента,
	...
	|	РасчетыОтгрузкиПоставки.Сторно КАК Сторно,
	|	РасчетыОтгрузкиПоставки.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности КАК Взаимозачет,
	...
	|	РасчетыОтгрузкиПоставки.ОбъектРасчетов,
	|	РасчетыОтгрузкиПоставки.АналитикаУчетаПоПартнерам,
	|	РасчетыОтгрузкиПоставки.Валюта,
	...
	|				И РасчетыПоСрокам.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗачетАвансаКлиента)
	|				И РасчетыПоСрокам.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗачетАвансаПоставщику)
	|				И (РасчетыПоСрокам.ОбъектРасчетов <> РасчетыОтгрузкиПоставки.ОбъектРасчетов ИЛИ РасчетыПоСрокам.АналитикаУчетаПоПартнерам <> РасчетыОтгрузкиПоставки.АналитикаУчетаПоПартнерам)
	...
	|	РасчетыПоСрокамОстатки.ОбъектРасчетов                            КАК ОбъектРасчетов,
	|	РасчетыПоСрокамОстатки.АналитикаУчетаПоПартнерам                 КАК АналитикаУчетаПоПартнерам,
	|	РасчетыПоСрокамОстатки.ВалютаРасчетов                            КАК Валюта,
	|	РасчетыПоСрокамОстатки.ДатаПлановогоПогашения                    КАК ДатаПлановогоПогашения,
	|	РасчетыПоСрокамОстатки.ДатаВозникновения                         КАК ДатаВозникновения,
	|	РасчетыПоСрокамОстатки.ПорядокОперации                           КАК ПорядокОперации,
	|	РасчетыПоСрокамОстатки.ПорядокЗачета                             КАК ПорядокЗачетаПоДатеПлатежа,
	...
	#КонецОбласти
		Если НЕ ЭтоРасчетыСКлиентами Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"РегистрНакопления.РасчетыСКлиентамиПоСрокам ","РегистрНакопления.РасчетыСПоставщикамиПоСрокам ");
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ОтборыКорДвижений","ОтборыКорДвиженийПоставщики");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ОтборыКорДвижений","ОтборыКорДвиженийКлиенты");
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЭтоРасчетыСКлиентами Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"ВТРегистраторыОстатков","ВТРегистраторыОстатковПоставщики");
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"ВТРасчетыПоСрокамОстатки","ВТРасчетыПоСрокамОстаткиПоставщики");
	...
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"ВТРегистраторыОстатков","ВТРегистраторыОстатковКлиенты");
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"ВТРасчетыПоСрокамОстатки","ВТРасчетыПоСрокамОстаткиКлиенты");
	КонецЕсли;
...
Процедура ДополнитьТаблицуИзмененийПриемниками(ТаблицаИзменений, Параметры, ЭтоОтменаПроведения, РассчитываемыеИзмерения = Неопределено)
	...
	Обработчики.Добавить("РегистрыНакопления.РасчетыСКлиентами.ОбработатьДанныеДляПереходаНаНовуюВерсию");
	Обработчики.Добавить("РегистрыНакопления.РасчетыСПоставщиками.ОбработатьДанныеДляПереходаНаНовуюВерсию");
	...
	Если ОбработчикОбновленияВыполнен(Обработчики) Тогда
		Запрос.Текст = "
		|ВЫБРАТЬ
		...
		|		ПО РеестрДокументов.Ссылка = Изменения.Документ
		...
		|ГДЕ
		|	НЕ (РасчетыСПоставщиками.ОбъектРасчетов,
		|		РасчетыСПоставщиками.АналитикаУчетаПоПартнерам,
		...
		|		РасчетыСПоставщиками.ВалютаПриемник) В 
		|		(ВЫБРАТЬ 
		|			ОбъектРасчетов, 
		|			АналитикаУчетаПоПартнерам, 
		|			ВалютаРасчетов, 
	...
	Иначе
		...
		|;
		|ВЫБРАТЬ
		|	МИНИМУМ(Изменения.ПорядокФакт)                      КАК ПорядокФакт,
	...
	ТаблицаИзмененийПриемников = ТаблицаИзмененийДляПересчета(Запрос.МенеджерВременныхТаблиц, Параметры.Регистратор, ЭтоОтменаПроведения, РассчитываемыеИзмерения);
	...
	Если ТаблицаИзмененийПриемников.Количество() > 0 Тогда
		...
		КонецЦикла;
		ДополнитьТаблицуИзмененийПриемниками(ТаблицаИзмененийПриемников, Параметры, ЭтоОтменаПроведения, РассчитываемыеИзмерения);
	КонецЕсли;
...
Функция ВыполнитьПереносОфлайнВзаиморасчетов(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	УчитыватьЧислоЗаписейРегистраторов = ОбработчикОбновленияВыполнен(
		"РегистрыСведений.КоличествоЗаписейРегистраторовРасчетов.ОбработатьДанныеДляПереходаНаНовуюВерсию");
		...
		Попытка
			...
			Для Каждого СтрокаКОбработке Из ПорцияДанныхКОбработке Цикл
				...
				Если СтрокаКОбработке.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом Тогда
					ТаблицаДетальныхЗаписей = ТаблицаДетальныхЗаписейКлиенты.СкопироватьКолонки();
					СтрокиДетальныхЗаписей  = ТаблицаДетальныхЗаписейКлиенты.НайтиСтроки(СтруктураОтбора);
				Иначе
					ТаблицаДетальныхЗаписей = ТаблицаДетальныхЗаписейПоставщики.СкопироватьКолонки();
					СтрокиДетальныхЗаписей  = ТаблицаДетальныхЗаписейПоставщики.НайтиСтроки(СтруктураОтбора);
				КонецЕсли;

				Для Каждого СтрокаДетальныхЗаписей Из СтрокиДетальныхЗаписей Цикл
					ЗаполнитьЗначенияСвойств(ТаблицаДетальныхЗаписей.Добавить(), СтрокаДетальныхЗаписей);
				КонецЦикла;
				...
				Если ТаблицаДетальныхЗаписей.Количество() > 0 Тогда
					ЗаписатьНовыеЗаписиРасчетовПоСрокам(ЭтоРасчетыСКлиентами, ТаблицаДетальныхЗаписей, ПорядокОперации, УчитыватьЧислоЗаписейРегистраторов);
...
Процедура ЗаписатьНовыеЗаписиРасчетовПоСрокам(ЭтоРасчетыСКлиентами, ТаблицаДетальныхЗаписей, ПорядокОперации, УчитыватьЧислоЗаписейРегистраторов = Неопределено)
	...
	Если УчитыватьЧислоЗаписейРегистраторов = Неопределено Тогда
		УчитыватьЧислоЗаписейРегистраторов = ОбработчикОбновленияВыполнен(
			"РегистрыСведений.КоличествоЗаписейРегистраторовРасчетов.ОбработатьДанныеДляПереходаНаНовуюВерсию");
	КонецЕсли;
	СтруктураПараметровРегистраторов.Вставить("УчитыватьКоличествоЗаписейРегистраторовРасчетов",
		УчитыватьЧислоЗаписейРегистраторов);
...
Функция ТаблицаИзмененийДляПересчета(МенеджерВременныхТаблиц, Регистратор, ЭтоОтменаПроведения = Ложь, РассчитываемыеИзмерения = Неопределено) Экспорт
	...
	Если РассчитываемыеИзмерения = Неопределено Тогда
		...
		РассчитываемыеИзмерения.Колонки.Добавить("ВалютаРасчетов", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
	КонецЕсли;
	Запрос.УстановитьПараметр("РассчитываемыеИзмерения", РассчитываемыеИзмерения);
	ЗапросУничтоженияВт = "";
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ВтРассчитываемыеИзмерения") <> Неопределено Тогда
		ЗапросУничтоженияВт = "УНИЧТОЖИТЬ ВтРассчитываемыеИзмерения;";
	КонецЕсли;
	Запрос.Текст = ЗапросУничтоженияВт + "
	|ВЫБРАТЬ
	|	РассчитываемыеИзмерения.ОбъектРасчетов КАК ОбъектРасчетов,
	|	РассчитываемыеИзмерения.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	РассчитываемыеИзмерения.ВалютаРасчетов КАК ВалютаРасчетов
	...
	|УПОРЯДОЧИТЬ ПО
	|	МАКСИМУМ(ВложенныйЗапрос.ПоДаннымОбъектаРасчетовИсточника) ВОЗР
	|";
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("РасчетыСКлиентамиИзменения") <> Неопределено Тогда
		...
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК РасчетыСКлиентамиПоСрокам
		|			ЛЕВОЕ СОЕДИНЕНИЕ РасчетыСКлиентамиИзменения КАК Изменения
		|				ПО Изменения.АналитикаУчетаПоПартнерам = РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам
		|					И Изменения.ОбъектРасчетов = РасчетыСКлиентамиПоСрокам.ОбъектРасчетов
		|					И Изменения.ВалютаРасчетов = РасчетыСКлиентамиПоСрокам.Валюта
		|					И Изменения.Сумма <> 0
		|	ГДЕ
		|		&ЭтоОтменаПроведения
		|		И РасчетыСКлиентамиПоСрокам.ДокументРегистратор = &Регистратор
		|		И Изменения.ОбъектРасчетов ЕСТЬ NULL
		|		И НЕ (РасчетыСКлиентамиПоСрокам.ОбъектРасчетов,
		|			РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам,
		|			РасчетыСКлиентамиПоСрокам.Валюта) В 
		|			(ВЫБРАТЬ 
		|				ОбъектРасчетов, 
		|				АналитикаУчетаПоПартнерам, 
		|				ВалютаРасчетов 
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("РасчетыСПоставщикамиИзменения") <> Неопределено Тогда
		...
		|	ИЗ
		|		РегистрНакопления.РасчетыСПоставщикамиПоСрокам КАК РасчетыСПоставщикамиПоСрокам
		|			ЛЕВОЕ СОЕДИНЕНИЕ РасчетыСПоставщикамиИзменения КАК Изменения
		|				ПО Изменения.АналитикаУчетаПоПартнерам = РасчетыСПоставщикамиПоСрокам.АналитикаУчетаПоПартнерам
		|					И Изменения.ОбъектРасчетов = РасчетыСПоставщикамиПоСрокам.ОбъектРасчетов
		|					И Изменения.ВалютаРасчетов = РасчетыСПоставщикамиПоСрокам.Валюта
		|					И Изменения.Сумма <> 0
		|	ГДЕ
		|		&ЭтоОтменаПроведения
		|		И РасчетыСПоставщикамиПоСрокам.ДокументРегистратор = &Регистратор
		|		И Изменения.ОбъектРасчетов ЕСТЬ NULL
		|		И НЕ (РасчетыСПоставщикамиПоСрокам.ОбъектРасчетов,
		|			РасчетыСПоставщикамиПоСрокам.АналитикаУчетаПоПартнерам,
		|			РасчетыСПоставщикамиПоСрокам.Валюта) В 
		|			(ВЫБРАТЬ 
		|				ОбъектРасчетов, 
		|				АналитикаУчетаПоПартнерам, 
		|				ВалютаРасчетов 
...
// Устарела. Обработчик обновления регистра взаиморасчетов выполнен.
// 
// Параметры:
//  ИмяОбработчика - Строка, Массив из Строка -  Имя обработчика
// 
// Возвращаемое значение:
//  Булево -  Обработчик обновления регистра взаиморасчетов выполнен
Функция ОбработчикОбновленияВыполнен(ИмяОбработчика)
	...
	|ГДЕ
	|	ОбработчикиОбновления.ИмяОбработчика В (&ИмяОбработчика)
	|	И ОбработчикиОбновления.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыОбработчиковОбновления.Выполнен)
	|СГРУППИРОВАТЬ ПО
	|	ОбработчикиОбновления.ИмяОбработчика,
	|	ОбработчикиОбновления.Статус";
...
// * ОбъектРасчетов - СправочникСсылка.ОбъектыРасчетов
// * ВалютаРасчетов - СправочникСсылка.Валюты
// * ПустойКорОбъектРасчетов - Булево
// * ПорядокФакт - Строка
// * ПорядокПлан - Строка
// * ЭтоРасчетыСКлиентами - Булево
// * НачальноеЗаполнение - Булево
...
Функция ТаблицаОсновныхПараметровРасчета(ПараметрыРасчета = Неопределено)
	...
	Таблица.Колонки.Добавить("ПустойКорОбъектРасчетов", Новый ОписаниеТипов("Булево"));
...
// * ТипРасчетов - ПеречислениеСсылка.ТипыРасчетовСПартнерами 
//  УчитыватьЧислоЗаписейРегистраторов - Булево
// 
// Возвращаемое значение:
//  Строка -  Текст запроса
//
Функция ТекстЗапросаКоличествоЗаписейВОперативныхРегистрахВзаиморасчетов(ИмяТаблицыДляОбработки, УчитыватьЧислоЗаписейРегистраторов = Неопределено) Экспорт
	...
	Если УчитыватьЧислоЗаписейРегистраторов Тогда
	...
	Иначе
		...
		|	ВложенныйЗапрос.ОбъектРасчетов КАК ОбъектРасчетов,
		|	ВложенныйЗапрос.Валюта КАК Валюта,
		|	СУММА(ВложенныйЗапрос.КоличествоЗаписей) КАК КоличествоЗаписей
		|ИЗ
		|	(ВЫБРАТЬ
		|		ВТДляОбработки.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		|		ВТДляОбработки.ОбъектРасчетов КАК ОбъектРасчетов,
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		|		Регистр.Регистратор ССЫЛКА Документ.РегистраторРасчетов
		|		И Регистр.Активность
		|		И ВТДляОбработки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
		|	СГРУППИРОВАТЬ ПО
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПланОплат КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПланОтгрузок КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоСрокам КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		|		Регистр.Регистратор ССЫЛКА Документ.РегистраторРасчетов
		|		И Регистр.Активность
		|		И ВТДляОбработки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
		|	СГРУППИРОВАТЬ ПО
		|		Регистр.АналитикаУчетаПоПартнерам,
		|		Регистр.ОбъектРасчетов,
		|		Регистр.Валюта,
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПланОплат КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		...
		|	ИЗ
		|		ВтДанныеДляОбработки КАК ВТДляОбработки
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПланПоставок КАК Регистр
		|			ПО ВТДляОбработки.АналитикаУчетаПоПартнерам = Регистр.АналитикаУчетаПоПартнерам
		|			И ВТДляОбработки.ОбъектРасчетов = Регистр.ОбъектРасчетов
		|			И ВТДляОбработки.Валюта = Регистр.Валюта
		|	ГДЕ
		...
		|		ВТДляОбработки.АналитикаУчетаПоПартнерам,
		|		ВТДляОбработки.ОбъектРасчетов,
		|		ВТДляОбработки.Валюта) КАК ВложенныйЗапрос
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.АналитикаУчетаПоПартнерам,
		|	ВложенныйЗапрос.ОбъектРасчетов,
		|	ВложенныйЗапрос.Валюта";
