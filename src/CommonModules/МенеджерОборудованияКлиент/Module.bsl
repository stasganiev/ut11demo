...
Функция ПараметрыПодключенияДрайвера(ДанныеДрайвера) Экспорт
	...
	КонецЕсли;
	Подключение.Вставить("ОбработчикДрайвера", ОбработчикДрайвера);
	Возврат Подключение;
...
Функция ОбновитьРабочееМестоКлиента() Экспорт
	...
	Если Не ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		РабочееМестоИдентификаторКлиента = МенеджерОборудованияКлиент.ИдентификаторКлиентаДляРабочегоМеста();   
...
//
// Параметры:
//  ЗапрашиватьНаСервере - Булево - если Истина, то в случае когда рабочее место отсуттвует на клиенте, 
...
// 
// Параметры:
//   ТипыПО - Массив, Строка - тип оборудования.
// Возвращаемое значение:
...
//
// Параметры:
//  ТипыПодключаемогоОборудования - Структура, Массив, Строка - тип оборудования.
//
// Возвращаемое значение:
...
Процедура НачатьПолучениеОписанияДрайвера(ОповещениеПриЗавершении, ИдентификаторДрайвера = Неопределено, ДанныеДрайвера = Неопределено) Экспорт
	...
	НовоеПодключение = МенеджерОборудованияКлиент.ПараметрыПодключенияДрайвера(ДанныеДрайвера);
...
Асинх Функция УстановитьДрайверОборудованияАсинх(ИдентификаторДрайвера) Экспорт
	...
	Иначе
		...
		ПараметрыУстановки.ТекстПояснения = ТекстСообщения; 
		Результат = Ждать ВнешниеКомпонентыБПОКлиент.УстановитьКомпонентуАсинх(ДанныеДрайвера.ИдентификаторОбъекта, ДанныеДрайвера.ВерсияДрайвера, ПараметрыУстановки);  // Структура
	КонецЕсли;
...
Процедура УстановитьДрайверОборудования(ОповещениеПриЗавершении, ИдентификаторДрайвера) Экспорт
	...
	Иначе
		...
		ПараметрыУстановки.ПредложитьЗагрузить  = НЕ ДанныеДрайвера.СнятСПоддержки;;
		ПараметрыУстановки.ПредложитьУстановить = Истина;
		ВнешниеКомпонентыБПОКлиент.УстановитьКомпоненту(Оповещение, ДанныеДрайвера.ИдентификаторОбъекта, ДанныеДрайвера.ВерсияДрайвера, ПараметрыУстановки);
	КонецЕсли;
...
//
// Параметры:
//  Отбор - Структура, Массив, Строка - тип оборудования для выбора устройства.
...
//  ИдентификаторУстройства - Массив, СправочникСсылка.ПодключаемоеОборудование - подключаемое устройство.
//  СетевоеОборудование - Булево - использовать сетевое оборудование при подключении.
//
// Возвращаемое значение:
//  Массив - Результаты подключения устройств.
//
...
//  ОповещениеПриОтключении - ОписаниеОповещения - событие описания оповещения.
//  ИдентификаторКлиента - УникальныйИдентификатор
//  ИдентификаторУстройства -  Массив, СправочникСсылка.ПодключаемоеОборудование - отключаемое устройство.
//
...
//      
// Возвращаемое значение:
//  Массив - Результаты отключения устройств.
//  
...
Процедура ПереустановитьПомеченныеПоставляемыеДрайверы() Экспорт
	...
	ИменаМакетовДляПереустановки = Новый Массив;
	Для Каждого ИмяМакетаДрайвера Из ПараметрыРаботыКлиента.ИменаМакетовДляПереустановки Цикл
		ИменаМакетовДляПереустановки.Добавить(ИмяМакетаДрайвера);
	КонецЦикла;
	
	ВнешниеКомпонентыБПОКлиент.ПереустановитьКомпоненты(ИменаМакетовДляПереустановки);
...
//
// Параметры:
//   РабочееМестоИдентификаторКлиента - Строка
//
...
Процедура НачатьПодключениеРасширенияПолученияИнформацииОКомпьютере_Завершение(Результат, Контекст) Экспорт
	...
	Иначе
		Если ЗапускПриложения Тогда
			...
			// расширение будет установлено позднее при обращени к оборудованию.
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьУстановкуРасширенияСПомощьюБПО(ОбработкаОповещения)
	...
	Если ВыполнятьУстановку Тогда
		ТекстЗаголовка = НСтр("ru = 'Установка расширения для работы с 1С:Предприятием'");
		Текст = НСтр("ru = 'Для удобной работы с программой предлагается установить расширение для работы с 1С:Предприятием.'");
		...
		Текст = Текст + Символы.ПС + НСтр("ru = 'Установить?'");
		ПоказатьВопрос(Оповещение, Текст, СписокКнопок, , КодВозвратаДиалога.Да, ТекстЗаголовка);
	Иначе
		Если ОбработкаОповещения <> Неопределено Тогда 
			ВыполнитьОбработкуОповещения(ОбработкаОповещения, Неопределено);
		КонецЕсли;     
...
Процедура НачатьУстановкуРасширенияПолученияИнформацииОКомпьютере_Завершение(ОбработкаОповещения) Экспорт
	...
	Контекст = Новый Структура();
	Контекст.Вставить("ОбработкаОповещения", ОбработкаОповещения);
	Контекст.Вставить("УстанавливатьРасширение", Ложь); // если расширение не установлось, повторно не устанавливать
