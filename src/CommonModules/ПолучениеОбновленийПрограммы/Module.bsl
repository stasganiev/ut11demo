...
Функция ИнформацияОВерсияхПлатформы() Экспорт
	...
	// Обновление защищенных версий платформы запрещено
	Если ЭтоЗащищенныйПрограммныйКомплекс() Тогда
		Возврат Результат;
	КонецЕсли;
...
Процедура ИнтернетПоддержкаИСервисы_ПриСозданииНаСервере(Форма) Экспорт
	...
	Элементы.ГруппаОбновлениеПрограммыИсправления.Видимость =
		...
		И ПараметрыДоступа.ДоступнаУстановка;
	Элементы.ГруппаОбновлениеПрограммы_АвтоматическаяПроверкаОбновлений2.Видимость =
		ПараметрыДоступа.ДоступнаПодсистема;
	...
	Если ПараметрыДоступа.ДоступнаПодсистема Тогда
...
Процедура ОбновлениеИнформацииОВерсияхПлатформы() Экспорт
	...
	// Не используется при работе в модели сервиса и на защищенных версиях платформы.
	Если ОбщегоНазначения.РазделениеВключено()
		Или ЭтоЗащищенныйПрограммныйКомплекс() Тогда
...
//    * Конфигурация - Неопределено - сценарий обновления не предполагает поиск обновлений конфигурации или выполняется
//        запрос только исправлений.
//                   - см. НовыйИнформацияОДоступномОбновленииКонфигурации
//    * Платформа - Неопределено - выполняется запрос только исправлений.
...
//    * ОкончаниеПоддержкиТекущейВерсии - Неопределено - текущая версия не находится на длительной поддержке.
...
Функция ИнформацияОДоступномОбновленииСлужебная(
	...
	Если Не ДопПараметры.ТолькоИсправления Тогда
...
Процедура ИнформацияОДоступномОбновленииВызовОперации(
	...
	ОбновлениеПлатформы = ?(
		ЭтоЗащищенныйПрограммныйКомплекс(),
		Неопределено,
...
Функция ИнформацияОВерсияхПлатформыКонфигурации(ИмяПрограммы, ВерсияПрограммы)
	...
	Если ОтветСервиса.КодСостояния = 200 Тогда
		...
		Попытка
			...
			ДанныеОтвета = СтрокуJSONВЗначение(ОтветСервиса.Содержимое);	// Массив из Структура
