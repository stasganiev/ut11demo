	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо"
		И (Сообщение.ЗначенияРеквизитов.Получить("ПредставлениеСсылкиСБП") <> Неопределено
			...
			Если Сообщение.ДополнительныеПараметры.Свойство("ПлатежнаяСсылка")
				...
				СуммаСсылкиСБП  = Сообщение.ПараметрыСообщения.СуммаПлатежнойСсылки;
...
Функция ПолучитьМакетСБП(ИдентификаторМакета) Экспорт
	...
	// Макет не найден или устарел, выполним процедуру обновления макетов.
...
Функция АктуализироватьСтатусОплаты(
	...
	Если РезультатОперации.СтатусОперации <> СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииВыполняется()
		...
		Попытка
			...
			Если ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ПлатежнаяСсылкаСБПc2b") Тогда
				ПараметрыОплатыПлатежнойСсылки = Документы.ПлатежнаяСсылкаСБПc2b.ПараметрыОплатыПлатежнойСсылки(ДокументОплаты);
				ОписаниеПараметровОперации.ДокументОснование = ПараметрыОплатыПлатежнойСсылки.ДокументОснование;
			КонецЕсли;
...
Функция СлужебныйДинамическаяСсылкаЧастичнойОплаты(
	...
	НовыйПлатежнаяСсылкаСБПc2b = Документы.ПлатежнаяСсылкаСБПc2b.ПолучитьСсылку(Новый УникальныйИдентификатор);
	
	ПлатежнаяСсылкаСБПc2b = Документы.ПлатежнаяСсылкаСБПc2b.СоздатьДокумент();
	ПлатежнаяСсылкаСБПc2b.УстановитьСсылкуНового(НовыйПлатежнаяСсылкаСБПc2b);
	
	ПлатежнаяСсылкаСБПc2b.ОснованиеПлатежа = ДанныеЧастичнойОплаты.ОснованиеПлатежа;
	ПлатежнаяСсылкаСБПc2b.Сумма            = ДанныеЧастичнойОплаты.СуммаОплаты;
	ПлатежнаяСсылкаСБПc2b.Дата             = ТекущаяДатаСеанса();
	
	ДанныеЧастичнойОплаты.Вставить("ПлатежнаяСсылкаСБПc2b", ПлатежнаяСсылкаСБПc2b.Ссылка);
	ДанныеЧастичнойОплаты.Вставить("ДатаОплаты",            ПлатежнаяСсылкаСБПc2b.Дата);
	...
	РезультатОперации = СервисПереводыСБПc2b.ДинамическаяСсылка(
		НовыйПлатежнаяСсылкаСБПc2b,
		ПараметрыНастройкиПодключения,
		Неопределено,
		ДанныеЧастичнойОплаты);
		
	Если РезультатОперации.КодОшибки = "" Тогда
		ПлатежнаяСсылкаСБПc2b.Записать();
		РезультатОперации.ПлатежнаяСсылкаСБПc2b = ПлатежнаяСсылкаСБПc2b.Ссылка;
	КонецЕсли;
