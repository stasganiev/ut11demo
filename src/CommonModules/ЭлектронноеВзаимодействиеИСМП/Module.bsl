...
Функция СформироватьКодОСУИзGTIN(СтрокаТовары, СтрокаУпаковки, Поле = "Количество")
	...
	Иначе
		КоличествоПоТегу37 = Окр(СтрокаТовары[Поле] / СтрокаУпаковки.КоличествоВПотребительскойУпаковке);
	КонецЕсли;
	
	Возврат СтрШаблон(
		"%1%2%3%4",
		"02", СтрокаУпаковки.ЗначениеШтрихкода, "37", Формат(КоличествоПоТегу37,"ЧДЦ=0; ЧН=0; ЧГ=;"));
