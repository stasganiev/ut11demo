...
Процедура ЗаполнитьОбъект_Delivery(Шапка, СтрокаТовары, ВспомогательныеДанные, ДополнительныеПараметры, DeliveryXDTO, СообщениеXML) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТовары[ДополнительныеПараметры.ПолеКоличество]) Тогда
		...
		СведенияОПартииПродукции = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(DeliveryXDTO, "consignment");
	...
	Если ТипЗнч(СтрокаТовары.ВСД_Идентификатор) = Тип("Массив") Тогда
		...
		Для каждого ВСД_Идентификатор Из СтрокаТовары.ВСД_Идентификатор Цикл
			...
			ВСД = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(СопроводительныеДокументы, "vetCertificate");
	...
	Если ВспомогательныеДанные.Свойство("СвязанныеДокументы") Тогда
		...
		Для каждого ДанныеСвязанногоДокумента Из ВспомогательныеДанные.СвязанныеДокументы Цикл
			...
			СвязанныйДокумент = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(СопроводительныеДокументы, "relatedDocument");
...
Процедура ЗаполнитьОбъект_ShipmentRoute(Маршрут, ВспомогательныеДанные, ОбъектXDTO, СообщениеXML) Экспорт
	...
	Для каждого ДанныеСтроки Из Маршрут Цикл
		...
		ТочкаМаршрута = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, "routePoint");
...
Процедура ЗаполнитьОбъект_Batch(Данные, ПолеКоличество, ВспомогательныеДанные, ОбъектXDTO, СообщениеXML) Экспорт
	...
	ЭтоУдалениеЗаписиСкладскогоЖурнала = ВспомогательныеДанные.Свойство("ЭтоУдалениеЗаписиСкладскогоЖурнала");
	СведенияОНаименованииОпциональны = ВспомогательныеДанные.Свойство("СведенияОНаименованииОпциональны");
	
	Если Не ЭтоУдалениеЗаписиСкладскогоЖурнала Тогда
	...
	Если (Не СведенияОНаименованииОпциональны Или Данные.ПродукцияОтличается)
		И Не ЭтоУдалениеЗаписиСкладскогоЖурнала Тогда
	...
	Если Не ЭтоУдалениеЗаписиСкладскогоЖурнала Тогда
		...
		Если ВспомогательныеДанные.Свойство("Производители") Тогда
			...
			Для Каждого ПроизводителиНайденнаяСтрока Из ВспомогательныеДанные.Производители Цикл
				...
				ПроизводительПартии = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(СведенияОПроисхождении, "producer");
...
Процедура ЗаполнитьОбъект_VetDocument(Данные, Шапка, ВспомогательныеДанные, ДополнительныеПараметры, ОбъектXDTO, СообщениеXML) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("Регионализация") Тогда
		Для каждого ДанныеРегионализации Из ДополнительныеПараметры.Регионализация Цикл
			...
			Регионализация = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "r13nClause");
	...
	Если ДополнительныеПараметры.Свойство("ЛабораторныеИсследования") Тогда
		...
		Для каждого ДанныеИсследований Из СписокСтрок Цикл
			...
			ДанныеИсследованийXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "laboratoryResearch");
	...
	Если ДополнительныеПараметры.Свойство("Иммунизация") Тогда
		...
		Для каждого ДанныеОбработки Из СписокСтрок Цикл
			...
			ДанныеОбработкиXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "immunization");
	...
	Если ВспомогательныеДанные.Свойство("ИсторияСтатусов") Тогда
		...
		Для каждого ДанныеСтатуса Из ВспомогательныеДанные.ИсторияСтатусов Цикл
			...
			СтатусВСД = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, "statusChange");
...
Процедура ЗаполнитьОбъект_PackageList(Упаковки, ВспомогательныеДанные, ОбъектXDTO, СообщениеXML) Экспорт
	...
	Для каждого ДанныеУпаковки Из Упаковки Цикл
		...
		// Описание упаковки.
		ОписаниеУпаковки = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, "package");
		...
		Если ВспомогательныеДанные.Свойство("ШтрихкодыУпаковок") Тогда
			...
			Для каждого ДанныеШтрихкода Из СписокСтрок Цикл
				...
				Штрихкод = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОписаниеУпаковки, "productMarks");
...
Функция СоздатьОбъектXDTOПоИмениТипа(ОбъектXDTO, ИмяСвойства) Экспорт
	...
	ОбъектXDTO[ИмяСвойства] = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъектXDTO, ИмяСвойства);
...
Процедура ЗагрузитьЗаписиСкладскогоЖурналаПоРезультатамЗапросаВЕТИС(ДанныеЗаписейСкладскогоЖурнала, ТабличнаяЧасть, Документ) Экспорт
	...
	Для Каждого ДанныеЗаписиСкладскогоЖурнала Из ДанныеЗаписейСкладскогоЖурнала Цикл
		...
		ЭтоЖивыеЖивотные = ИнтеграцияВЕТИСВызовСервера.ПродукцияПринадлежитТипуЖивыеЖивотные(ЗаписьСкладскогоЖурналаОбъект.Продукция);
		...
		// Автоматическое объединение записей складского журнала
		Если ДанныеСтроки = Неопределено Тогда
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("Продукция",             ЗаписьСкладскогоЖурналаОбъект.Продукция);
			ПараметрыОтбора.Вставить("ЕдиницаИзмеренияВЕТИС", ДанныеЗаписиСкладскогоЖурнала.ЕдиницаИзмеренияВЕТИС);
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
					...
					ОкончаниеПроизводстваЗСЖ  = Макс(ЗаписьСкладскогоЖурналаОбъект.ДатаПроизводстваНачалоПериода, ЗаписьСкладскогоЖурналаОбъект.ДатаПроизводстваКонецПериода);
					ОкончаниеСрокаГодностиТЧ  = Макс(СтрокаТЧ.СрокГодностиНачалоПериода, СтрокаТЧ.СрокГодностиКонецПериода);
					ОкончаниеСрокаГодностиЗСЖ = Макс(ЗаписьСкладскогоЖурналаОбъект.СрокГодностиНачалоПериода, ЗаписьСкладскогоЖурналаОбъект.СрокГодностиКонецПериода);
					...
					Если СтрокаТЧ.Количество > ЗаписьСкладскогоЖурналаОбъект.КоличествоВЕТИС Тогда
						Продолжить; // не подходит: количество меньше указанного в строке
					// Проверка даты производства по записи журнала
					ИначеЕсли ПроверятьДатуПроизводства
						И (СтрокаТЧ.ДатаПроизводстваНачалоПериода < ЗаписьСкладскогоЖурналаОбъект.ДатаПроизводстваНачалоПериода
							Или ОкончаниеПроизводстваТЧ > ОкончаниеПроизводстваЗСЖ) Тогда
						Продолжить;
					// Проверка срока годности по ВСД
					ИначеЕсли ПроверятьСрокГодности
						И (СтрокаТЧ.СрокГодностиНачалоПериода < ЗаписьСкладскогоЖурналаОбъект.СрокГодностиНачалоПериода
							Или ОкончаниеСрокаГодностиТЧ > ОкончаниеСрокаГодностиЗСЖ) Тогда
						Продолжить;
					КонецЕсли;
					ДанныеСтроки = СтрокаТЧ;
					ДанныеСтроки.ЗаписьСкладскогоЖурнала = ЗаписьСкладскогоЖурналаСсылка;
...
Процедура ОбменДаннымиРегламентноеЗадание(НастройкаРегламентногоЗадания = Неопределено) Экспорт
	...
	Если НастройкаРегламентногоЗадания = Неопределено Тогда
		...
		Если Не Константы.ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС.Получить() Тогда
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли НастройкаРегламентногоЗадания = КлючРегламентногоЗаданияСинхронизацииКлассификаторов() Тогда
		
		ПараметрыОбмена = ИнтеграцияВЕТИС.ПараметрыОбмена(Неопределено, Неопределено, Истина);
		ПараметрыОбмена.ПараметрыПреобразования = ИнтеграцияВЕТИС.ПараметрыПреобразования();
		ИнтеграцияВЕТИСВызовСервера.ВыполнитьСинхронизациюКлассификаторов(Неопределено, ПараметрыОбмена);
		
	ИначеЕсли ТипЗнч(НастройкаРегламентногоЗадания) = Тип("СправочникСсылка.НастройкиРегламентныхЗаданийВЕТИС") Тогда
		
		ОрганизацииВЕТИСДляОбмена = ОрганизацииВЕТИСДляОбменаПоНастройке(НастройкаРегламентногоЗадания);
		Если ОрганизацииВЕТИСДляОбмена = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ЗаписатьВИсториюВыполненияОбменовСВЕТИС(НастройкаРегламентногоЗадания, Истина);
		
		ПараметрыОбмена = ИнтеграцияВЕТИС.ПараметрыОбмена(ОрганизацииВЕТИСДляОбмена, Неопределено, Истина);
		ИнтеграцияВЕТИС.ПостроитьОчередьСообщений(ПараметрыОбмена);
		ИнтеграцияВЕТИСВызовСервера.ВыполнитьОбменУниверсально(ПараметрыОбмена);
		
		ЗаписатьВИсториюВыполненияОбменовСВЕТИС(НастройкаРегламентногоЗадания, Ложь);
...
Функция КлючРегламентногоЗаданияСинхронизацииКлассификаторов() Экспорт
	...
	Возврат НСтр("ru = 'СинхронизацияКлассификаторов'");
...
Процедура ДополнитьЗапросСопоставлениемТоваров(Запрос)
	...
	Запрос.УстановитьПараметр("ПустаяСерия", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("СерияНоменклатурыВЕТИС"));
	Запрос.Текст = Запрос.Текст + 
	"
	|
	|;
...
Функция ВосстановитьДополнитьСырьеПроизводственнаяОперацияВЕТИС(ПараметрыВосстановления, ПараметрыОбмена)
	...
	Если Не ЗаписьИспользованаВОперацииВосстановления Тогда
		...
		Если ДокументОбъект <> Неопределено
			...
			КоличествоВЕТИС = СтрокаТаблицыЭлементов.ЭлементДанныхПредыдущий.batch.volume - СтрокаТаблицыЭлементов.ЭлементДанных.batch.volume;
			Если КоличествоВЕТИС <= 0 Тогда
				Возврат Истина;
			КонецЕсли;
			...
			ТоварыСтрокаТЧ.КоличествоВЕТИС = КоличествоВЕТИС;
			...
			Если СтрокаТаблицыЭлементов.ЭлементДанныхПредыдущий.batch.packageList <> Неопределено
				...
				Для Каждого СтрокаТЧ Из УпаковкиВЕТИС Цикл
					Если СтрокаТЧ.КоличествоУпаковокВЕТИС > 0 Тогда
						УпаковкиСтрокаТЧ = ДокументОбъект.УпаковкиВЕТИС.Добавить();
...
Функция ОрганизацииВЕТИСДляОбменаПоНастройке(Настройка)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Настройка", Настройка);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Настройки.ХозяйствующийСубъект КАК ХозяйствующийСубъект,
	|	Настройки.Ссылка КАК Настройка
	|ПОМЕСТИТЬ ТаблицаХСНастройки
	|ИЗ
	|	Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС КАК ХС
	|		ПО Настройки.ХозяйствующийСубъект = ХС.Ссылка,
	|	Константа.ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС КАК
	|		ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС
	|ГДЕ
	|	Настройки.Ссылка = &Настройка
	|	И НЕ Настройки.ПометкаУдаления
	|	И НЕ ХС.ПометкаУдаления
	|	И НЕ ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС.Значение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ТОГДА Истина
	|		ИНАЧЕ Ложь
	|	КОНЕЦ КАК ПредприятиеИспользование,
	|	НастройкиПредприятия.Предприятие ЕСТЬ NULL КАК ОбщаяНастройка
	|ПОМЕСТИТЬ ТаблицаПредприятияНастройки
	|ИЗ
	|	Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаХСНастройки КАК ХСНастройки
	|		ПО Настройки.Ссылка = ХСНастройки.Настройка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ХСПредприятия.Ссылка КАК ХозяйствующийСубъект,
	|	ХСПредприятия.Предприятие КАК Предприятие
	|ПОМЕСТИТЬ ТаблицаПредприятияХС
	|ИЗ
	|	ТаблицаХСНастройки КАК ХСНастройки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС.Предприятия КАК ХСПредприятия
	|		ПО ХСНастройки.ХозяйствующийСубъект = ХСПредприятия.Ссылка
	|		И НЕ ХСПредприятия.НеИспользовать
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	ХСНастройки.ХозяйствующийСубъект,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПредприятияХС.ХозяйствующийСубъект КАК ХозяйствующийСубъект,
	|	ПредприятияХС.Предприятие КАК Предприятие
	|ПОМЕСТИТЬ ТаблицаПредприятияБезНастроек
	|ИЗ
	|	ТаблицаПредприятияХС КАК ПредприятияХС
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиРегламентныхЗаданийВЕТИС.Предприятия КАК НастройкиПредприятия
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|			ПО НастройкиПредприятия.Ссылка = Настройки.Ссылка
	|		ПО ПредприятияХС.ХозяйствующийСубъект = Настройки.ХозяйствующийСубъект
	|		И ПредприятияХС.Предприятие = НастройкиПредприятия.Предприятие
	|ГДЕ
	|	НастройкиПредприятия.Предприятие ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПредприятияНастройки.Настройка КАК Настройка,
	|	ПредприятияНастройки.ХозяйствующийСубъект КАК ХозяйствующийСубъект,
	|	ПредприятияБезНастроек.Предприятие КАК Предприятие
	|ИЗ
	|	ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПредприятияБезНастроек КАК ПредприятияБезНастроек
	|		ПО ПредприятияНастройки.ХозяйствующийСубъект = ПредприятияБезНастроек.ХозяйствующийСубъект
	|ГДЕ
	|	ПредприятияНастройки.ОбщаяНастройка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПредприятияНастройки.Настройка,
	|	ПредприятияНастройки.ХозяйствующийСубъект,
	|	ПредприятияНастройки.Предприятие
	|ИЗ
	|	ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|ГДЕ
	|	НЕ ПредприятияНастройки.ОбщаяНастройка
	|	И ПредприятияНастройки.ПредприятиеИспользование";
	...
	СтрокаРезультата.Вставить("Организация", Выборка.ХозяйствующийСубъект);
	СтрокаРезультата.Вставить("Предприятия", Предприятия);
...
Процедура ЗаписатьВИсториюВыполненияОбменовСВЕТИС(Настройка, Начало)
	
	ТекущаяДата = ТекущаяДатаСеанса();
	
	Если Начало Тогда
		Запись = РегистрыСведений.ИсторияВыполненияОбменовСВЕТИС.СоздатьМенеджерЗаписи();
	...
	Иначе
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МАКСИМУМ(ИсторияВыполненияОбменовСВЕТИС.ДатаНачала) КАК ДатаНачала
		|ИЗ
		|	РегистрСведений.ИсторияВыполненияОбменовСВЕТИС КАК ИсторияВыполненияОбменовСВЕТИС
		|ГДЕ
		|	ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания = &Настройка
		|	И ИсторияВыполненияОбменовСВЕТИС.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)";
...
Функция ЗапросНастроекРегламентыхЗаданий(ОрганизацииВЕТИС) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДляОтбора.Организация КАК ХозяйствующийСубъект,
	|	ДанныеДляОтбора.Предприятие КАК Предприятие
	|ПОМЕСТИТЬ ДанныеДляОтбора
	|ИЗ &ДанныеДляОтбора КАК ДанныеДляОтбора
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДляОтбора.ХозяйствующийСубъект,
	|	ДанныеДляОтбора.Предприятие,
	|	ТаблицаПредприятияНастройки.Настройка
	|ПОМЕСТИТЬ ТаблицаОтбораПоПредприятиям
	|ИЗ ДанныеДляОтбора КАК ДанныеДляОтбора
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПредприятияНастройки КАК ТаблицаПредприятияНастройки
	|	ПО ДанныеДляОтбора.ХозяйствующийСубъект = ТаблицаПредприятияНастройки.ХозяйствующийСубъект
	|		И ДанныеДляОтбора.Предприятие = ТаблицаПредприятияНастройки.Предприятие
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаОтбораПоПредприятиям.ХозяйствующийСубъект,
	|	ТаблицаОтбораПоПредприятиям.Предприятие,
	|	ЕСТЬNULL(ТаблицаПредприятияНастройки.Настройка, ТаблицаОтбораПоПредприятиям.Настройка) КАК Настройка
	|ПОМЕСТИТЬ ТаблицаОтбора
	|ИЗ ТаблицаОтбораПоПредприятиям КАК ТаблицаОтбораПоПредприятиям
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПредприятияНастройки КАК ТаблицаПредприятияНастройки
	|	ПО ТаблицаОтбораПоПредприятиям.Настройка ЕСТЬ NULL
	|		И ТаблицаПредприятияНастройки.ОбщаяНастройка
	|		И ТаблицаОтбораПоПредприятиям.ХозяйствующийСубъект = ТаблицаПредприятияНастройки.ХозяйствующийСубъект
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ПОМЕСТИТЬ ТаблицаПредприятияХС
	|ИЗ
	|	Справочник.ХозяйствующиеСубъектыВЕТИС КАК ХС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|		ПО ХС.Ссылка = Настройки.ХозяйствующийСубъект
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС.Предприятия КАК ХСПредприятия
	|		ПО ХС.Ссылка = ХСПредприятия.Ссылка
	|ГДЕ
	|	НЕ ХС.ПометкаУдаления
	|	И НЕ ХСПредприятия.НеИспользовать
	|
	|ОБЪЕДИНИТЬ
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПредприятияХС.ХозяйствующийСубъект,
	|	Количество(ПредприятияХС.Предприятие) КАК КоличествоПредприятий
	|ПОМЕСТИТЬ ИтогиПоХС
	|ИЗ
	|	ТаблицаПредприятияХС КАК ПредприятияХС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|		ПО ПредприятияХС.ХозяйствующийСубъект = ПредприятияНастройки.ХозяйствующийСубъект
	|		И ПредприятияХС.Предприятие = ПредприятияНастройки.Предприятие
	|ГДЕ
	|	ПредприятияНастройки.ХозяйствующийСубъект ЕСТЬ NULL
	|СГРУППИРОВАТЬ ПО
	|	ПредприятияХС.ХозяйствующийСубъект
	|;
	|
	...
	|ВЫБРАТЬ
	|	ПредприятияНастройки.Настройка,
	|	ЕСТЬNULL(ИтогиПоХС.КоличествоПредприятий, 0) КАК КоличествоПредприятий
	|ПОМЕСТИТЬ ИтогиПоНастройкам
	|ИЗ
	|	ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИтогиПоХС КАК ИтогиПоХС
	|		ПО ПредприятияНастройки.ХозяйствующийСубъект = ИтогиПоХС.ХозяйствующийСубъект
	|ГДЕ
	|	ПредприятияНастройки.ОбщаяНастройка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|ВЫБРАТЬ
	|	ПредприятияНастройки.Настройка,
	|	КОЛИЧЕСТВО(ПредприятияНастройки.Настройка)
	|ИЗ
	|	ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|ГДЕ
	|	НЕ ПредприятияНастройки.ОбщаяНастройка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания КАК Настройка,
	|	ИсторияВыполненияОбменовСВЕТИС.ДатаНачала КАК ДатаНачала,
	|	РАЗНОСТЬДАТ(ИсторияВыполненияОбменовСВЕТИС.ДатаНачала, ИсторияВыполненияОбменовСВЕТИС.ДатаОкончания, СЕКУНДА) КАК
	|		ВремяВыполнения
	|ПОМЕСТИТЬ ТаблицаВремяВыполнения
	|ИЗ
	|	(ВЫБРАТЬ
	|		ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания КАК НастройкаРегламентногоЗадания,
	|		МАКСИМУМ(ИсторияВыполненияОбменовСВЕТИС.ДатаНачала) КАК ДатаНачала
	|	ИЗ
	|		РегистрСведений.ИсторияВыполненияОбменовСВЕТИС КАК ИсторияВыполненияОбменовСВЕТИС
	|	ГДЕ
	|		ИсторияВыполненияОбменовСВЕТИС.ДатаОкончания <> ДАТАВРЕМЯ(1, 1, 1)
	|	СГРУППИРОВАТЬ ПО
	|		ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания) КАК НачалоВыполнения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияВыполненияОбменовСВЕТИС КАК ИсторияВыполненияОбменовСВЕТИС
	|		ПО НачалоВыполнения.НастройкаРегламентногоЗадания = ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания
	|		И НачалоВыполнения.ДатаНачала = ИсторияВыполненияОбменовСВЕТИС.ДатаНачала
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Настройки.ХозяйствующийСубъект КАК ХозяйствующийСубъект,
	|	ИсторияВыполненияОбменовСВЕТИС.ДатаНачала КАК ДатаНачала,
	|	ИсторияВыполненияОбменовСВЕТИС.ДатаОкончания КАК ДатаОкончания
	|ПОМЕСТИТЬ ТаблицаИсторияВыполнения
	|ИЗ
	|	РегистрСведений.ИсторияВыполненияОбменовСВЕТИС КАК ИсторияВыполненияОбменовСВЕТИС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|		ПО ИсторияВыполненияОбменовСВЕТИС.НастройкаРегламентногоЗадания = Настройки.Ссылка
	|ГДЕ
	|	ИсторияВыполненияОбменовСВЕТИС.ДатаНачала > &ДатаНачалаПроверкиИсторииВыполнения
	|	И ИсторияВыполненияОбменовСВЕТИС.ДатаОкончания <> ДАТАВРЕМЯ(1, 1, 1)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИсторияВыполнения1.Настройка КАК Настройка,
	|	МАКСИМУМ(ИсторияВыполнения2.Настройка.Наименование) КАК НастройкаПересеченияНаименование,
	|	КОЛИЧЕСТВО(ИсторияВыполнения2.Настройка) КАК КоличествоПересечений
	|ПОМЕСТИТЬ ТаблицаПересеченияВыполнения
	|ИЗ
	|	ТаблицаИсторияВыполнения КАК ИсторияВыполнения1
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаИсторияВыполнения КАК ИсторияВыполнения2
	|		ПО ИсторияВыполнения1.Настройка <> ИсторияВыполнения2.Настройка
	|		И ИсторияВыполнения1.ХозяйствующийСубъект = ИсторияВыполнения2.ХозяйствующийСубъект
	|		И ИсторияВыполнения1.ДатаОкончания > ИсторияВыполнения2.ДатаНачала
	|		И ИсторияВыполнения1.ДатаНачала < ИсторияВыполнения2.ДатаОкончания
	|СГРУППИРОВАТЬ ПО
	|	ИсторияВыполнения1.Настройка
	|;
	|ВЫБРАТЬ
	|	ТаблицаОтбора.ХозяйствующийСубъект КАК ХозяйствующийСубъект,
	|	ТаблицаОтбора.Предприятие КАК Предприятие
	|ИЗ ТаблицаОтбора КАК ТаблицаОтбора
	|ГДЕ
	...
	|ВЫБРАТЬ
	|	Настройки.Ссылка КАК Настройка,
	|	Настройки.РегламентноеЗадание,
	|	Настройки.ПометкаУдаления,
	|	Настройки.ХозяйствующийСубъект,
	|	Настройки.ХозяйствующийСубъект.ПометкаУдаления КАК ПометкаУдаленияХС,
	|	ЕСТЬNULL(ИтогиПоНастройкам.КоличествоПредприятий, 0) КАК КоличествоПредприятий,
	|	МАКСИМУМ(ПредприятияНастройки.Предприятие.Наименование) КАК ПредприятиеНаименование,
	|	КОЛИЧЕСТВО(ПредприятияНастройки.Предприятие) КАК КоличествоНеИспользуемыхПредприятий,
	...
	|	ЕСТЬNULL(ПересеченияВыполнения.НастройкаПересеченияНаименование, """") КАК НастройкаПересеченияНаименование,
	|	ЕСТЬNULL(ПересеченияВыполнения.КоличествоПересечений, 0) КАК КоличествоПересечений
	|ИЗ
	|	Справочник.НастройкиРегламентныхЗаданийВЕТИС КАК Настройки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИтогиПоНастройкам КАК ИтогиПоНастройкам
	|		ПО Настройки.Ссылка = ИтогиПоНастройкам.Настройка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПредприятияНастройки КАК ПредприятияНастройки
	|		ПО Настройки.Ссылка = ПредприятияНастройки.Настройка
	|		И НЕ ПредприятияНастройки.ПредприятиеИспользование
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаВремяВыполнения КАК ТаблицаВремяВыполнения
	|		ПО Настройки.Ссылка = ТаблицаВремяВыполнения.Настройка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПересеченияВыполнения КАК ПересеченияВыполнения
	|		ПО Настройки.Ссылка = ПересеченияВыполнения.Настройка
	|ГДЕ
	|	НЕ &ИспользоватьОтбор
	|	ИЛИ Настройки.Ссылка В (ВЫБРАТЬ РАЗЛИЧНЫЕ Настройка	ИЗ ТаблицаОтбора)
	|СГРУППИРОВАТЬ ПО
	|	Настройки.Ссылка,
	|	Настройки.РегламентноеЗадание,
	|	Настройки.ПометкаУдаления,
	|	Настройки.ХозяйствующийСубъект,
	|	Настройки.ХозяйствующийСубъект.ПометкаУдаления,
	|	ЕСТЬNULL(ИтогиПоНастройкам.КоличествоПредприятий, 0),
	|	ЕСТЬNULL(ТаблицаВремяВыполнения.ДатаНачала, ДАТАВРЕМЯ(1, 1, 1)),
	|	ЕСТЬNULL(ТаблицаВремяВыполнения.ВремяВыполнения, 0),
	|	ЕСТЬNULL(ПересеченияВыполнения.НастройкаПересеченияНаименование, """"),
	|	ЕСТЬNULL(ПересеченияВыполнения.КоличествоПересечений, 0)
	|УПОРЯДОЧИТЬ ПО
	|	Настройки.Наименование
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС.Значение КАК АвтоматическийОбмен,
	|	СУММА(ПредприятияХС.КоличествоПредприятий) КАК КоличествоПредприятий
	|ИЗ
	|	Константа.ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС КАК
	|		ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС,
	|	(ВЫБРАТЬ
	|		КОЛИЧЕСТВО(ХС.Ссылка) КАК КоличествоПредприятий
	|	ИЗ
	|		Справочник.ХозяйствующиеСубъектыВЕТИС КАК ХС
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС.Предприятия КАК ХСПредприятия
	|			ПО ХС.Ссылка = ХСПредприятия.Ссылка
	...
	|
	|	ВЫБРАТЬ
	|		КОЛИЧЕСТВО(ХС.Ссылка)
	|	ИЗ
	|		Справочник.ХозяйствующиеСубъектыВЕТИС КАК ХС
	|	ГДЕ
	|		НЕ ХС.ПометкаУдаления) КАК ПредприятияХС
	|СГРУППИРОВАТЬ ПО
	|	ИспользоватьАвтоматическуюОтправкуПолучениеДанныхВЕТИС.Значение";
	
	Запрос.УстановитьПараметр("ДатаНачалаПроверкиИсторииВыполнения", ДатаНачалаПроверкиИсторииВыполнения());
	
	ОрганизацииПредприятияВЕТИС = ИнтеграцияВЕТИС.ТаблицаОрганизацияПредприятие(ОрганизацииВЕТИС);
	Запрос.УстановитьПараметр("ДанныеДляОтбора",   ОрганизацииПредприятияВЕТИС);
	Запрос.УстановитьПараметр("ИспользоватьОтбор", ОрганизацииПредприятияВЕТИС.Количество() > 0);
	
	Возврат Запрос;
		
КонецФункции

Функция ДатаНачалаПроверкиИсторииВыполнения()
	
	Возврат ТекущаяДатаСеанса() - 86400;
...
Функция МаксимальноеКоличествоПредприятийДляНастройкиОбмена() Экспорт
...
Функция СвойстваРегламентногоЗадания() Экспорт
	...
	Возврат "Использование,Расписание,Наименование,Ключ,Предопределенное";
...
Функция ДанныеРегламентныхЗаданий() Экспорт
	...
	ОтборРегламентныхЗаданий.Вставить("Метаданные", Метаданные.РегламентныеЗадания.ОтправкаПолучениеДанныхВЕТИС);
	...
	Для Каждого РегламентноеЗадание Из НайденныеЗадания Цикл
		...
		Если ДанныеРегламентногоЗадания.Свойство("Предопределенное") Тогда
			Если ТипЗнч(РегламентноеЗадание) = Тип("РегламентноеЗадание") Тогда
				ДанныеРегламентногоЗадания.Предопределенное = РегламентноеЗадание.Предопределенное;
			Иначе
				ДанныеРегламентногоЗадания.Предопределенное = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ЕстьПроблемыОтправкиПолученияДанныхВЕТИС(ОрганизацииВЕТИС) Экспорт
	
	ДопустимоеВремяВыполнения = ДопустимоеВремяВыполненияРегламентногоЗаданияПоНастройкеОбмена();
	МаксимальноеКоличествоПредприятий = МаксимальноеКоличествоПредприятийДляНастройкиОбмена();
	
	Запрос = ЗапросНастроекРегламентыхЗаданий(ОрганизацииВЕТИС);
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	ВГраница = РезультатЗапроса.ВГраница();
	
	Выборка = РезультатЗапроса[ВГраница].Выбрать();
	Выборка.Следующий();
	Если Выборка.АвтоматическийОбмен Тогда
		Если Выборка.КоличествоПредприятий > МаксимальноеКоличествоПредприятий Тогда
			Возврат Истина;
		КонецЕсли;
	ИначеЕсли РезультатЗапроса[ВГраница - 1].Пустой() 
		Или Не РезультатЗапроса[ВГраница - 2].Пустой() Тогда
		Возврат Истина;
	Иначе
		ДанныеРегламентныхЗаданий = ДанныеРегламентныхЗаданий();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ПометкаУдаления И Не Выборка.ПометкаУдаленияХС
				Или Не Выборка.ПометкаУдаления И Выборка.ПометкаУдаленияХС
				Или ДанныеРегламентногоЗадания = Неопределено
				Или Не ДанныеРегламентногоЗадания.Использование
				Или Выборка.КоличествоНеИспользуемыхПредприятий > 0
				Или Выборка.КоличествоПредприятий > МаксимальноеКоличествоПредприятий
				Или Выборка.ВремяВыполнения > ДопустимоеВремяВыполнения
				Или Выборка.КоличествоПересечений > 0
				Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
