...
Функция ИдентификаторыУчастника(Знач ИНН, Знач КПП, Знач Наименование = Неопределено, Отказ = Неопределено,
	...
	Идентификаторы.Вставить("ИНН", "");
	Идентификаторы.Вставить("КПП", "");
	...
	ЭтоЮридическоеЛицо = ЭтоИННЮридическогоЛица(ИНН);
	
	Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИНН, ЭтоЮридическоеЛицо, ТекстСообщения) Тогда
		ШаблонОшибки = НСтр("ru = 'Значение ИНН ""%1"" не соответствует требованиям к ИНН: %2'", КодЯзыка);
		Текст        = СтрШаблон(ШаблонОшибки, ИНН, ТекстСообщения);
		
		Ошибки.Добавить(Текст);
	Иначе
		Идентификаторы.ИНН = ИНН;
	КонецЕсли;
	
	ТекстСообщения = "";
	
	Если Не ЭтоЮридическоеЛицо Тогда
		Идентификаторы.КПП = "0"; // по формату используем "0" вместо пустого значения
	ИначеЕсли Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(КПП, ТекстСообщения) Тогда
		ШаблонОшибки = НСтр("ru = 'Значение КПП ""%1"" не соответствует требованиям к КПП: %2'", КодЯзыка);
		Текст        = СтрШаблон(ШаблонОшибки, КПП, ТекстСообщения);
		 
		Ошибки.Добавить(Текст);
	Иначе
		Идентификаторы.КПП = КПП;
	КонецЕсли;
...
Функция ЗначениеИзСтрокиJSON(Значение, ПрочитатьВСоответствие = Ложь) Экспорт
	...
	Попытка
		...
		Результат = ПрочитатьJSON(ЧтениеJSON, ПрочитатьВСоответствие, , , "ФункцияВосстановленияJSON", БизнесСеть);
		ЧтениеJSON.Закрыть();
	Исключение
...
Функция ОбновитьКлючДоступа(ИдентификаторОрганизации, ТокенОбновления, Отказ = Ложь)
	...
	Если ТипЗнч(ДанныеСервиса) <> Тип("Структура") Тогда
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Не верный формат данных в ответе на запрос'", 
...
// Выполняет преобразование значений свойств  при чтении JSON
// 
// Параметры:
//  Свойство                - Строка       - Имя читаемого свойства
//  Значение                - Произвольный - Значение читаемого свойства
//  ДополнительныеПараметры - Структура    - Дополнительные параметры, переданные в проиобразование
//                          - Неопределено - Если параметры отсутствуют
// 
// Возвращаемое значение:
//  Произвольный - Результат чтения значения свойства JSON
Функция ФункцияВосстановленияJSON(Знач Свойство, Знач Значение, Знач ДополнительныеПараметры) Экспорт
	
	Результат = Значение;
	
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		// Недопустимые символы XML в строках могут вызвать ошибки при передаче данные между клиентом и сервером
...
//
// Параметры:
//  ПараметрыЗапроса - ТаблицаЗначений - см. Обработка.БизнесСеть.Форма.ДокументыОбмена.ЗагружаемыеДокументы
...
// Возвращает длину ИНН юридического лица
// 
// Возвращаемое значение:
//  Число - Длина ИНН юридического лица
Функция ДлинаИННЮридическогоЛица()
	Возврат 10; // символов
КонецФункции

// Определяет, является ли владельцем ИНН юридическое лицо
// 
// Параметры:
//  ИНН - Строка - Проверяемый ИНН
// 
// Возвращаемое значение:
//  Булево - Истина, если владельцем ИНН является юридическое лицо.
//           Ложь, в ином случае.
Функция ЭтоИННЮридическогоЛица(Знач ИНН)
	
	КонтекстПроверки = "БизнесСеть.ЭтоИННЮридическогоЛица";
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(КонтекстПроверки, "ИНН", ИНН, Новый ОписаниеТипов("Строка"));
	
	Возврат СтрДлина(ИНН) = ДлинаИННЮридическогоЛица();
