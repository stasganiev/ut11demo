...
Асинх Функция ОбъектДрайвераАсинх(ПараметрыПодключения) Экспорт
	...
	Если ПараметрыПодключения.ПодключениеИзМакета Тогда    
		...
		Параметры.Изолированно = МенеджерОборудованияКлиентПовтИсп.ИзолированноеПодключенияВнешнихКомпонентНаКлиенте(); 
	...
	Иначе
		...
		РезультатПодключения = Ждать ВнешниеКомпонентыБПОКлиент.ПодключитьКомпонентуАсинх(ПараметрыПодключения.ИдентификаторОбъекта, ПараметрыПодключения.ВерсияДрайвера, Параметры);
	КонецЕсли;
	...
	Иначе     
		...
		Если Не ПустаяСтрока(ПараметрыПодключения.ШаблонЛокализации) Тогда   
			Попытка                         
				...
				РезультатВызова = Ждать ОбъектДрайвера.УстановитьЛокализациюАсинх(КодЯзыкаПользователя, ПараметрыПодключения.ШаблонЛокализации);
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьОписаниеАсинх(ОписаниеДрайвера.ОписаниеДрайвераXML);
		Исключение                               
		...
		Если ПараметрыВызова.Количество() > 1 И Не СтрНачинаетсяС(ПараметрыВызова[0], "<?")  Тогда  
			...
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьНомерВерсииАсинх();
		...
		Иначе                                                                       
			...
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьРевизиюИнтерфейсаАсинх();
		...
		Если ОписаниеДрайвера.РевизияИнтерфейса >= 4000 И ОписаниеДрайвера.ПоддерживаетсяЛокализация Тогда   
			ШаблонЛокализации = "";
			Попытка                                    
				РезультатВызова = Ждать ОбъектДрайвера.ПолучитьШаблонЛокализацииАсинх(ШаблонЛокализации);
				Если РезультатВызова.Значение Тогда      
					ПараметрыВызова = РезультатВызова.Параметры;
					ОписаниеДрайвера.ШаблонЛокализации = ПараметрыВызова[0];     
				КонецЕсли
			Исключение
				ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru='Ошибка получения шаблон локализации драйвера.'"));
			КонецПопытки;   
		КонецЕсли;
...
Асинх Функция ПараметрыДрайвераАсинх(ОбъектДрайвера, ОписаниеДрайвера, ПараметрыПодключения) Экспорт
	...
	ПараметрыДрайвера = "";
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьПараметрыАсинх(ПараметрыДрайвера);
	Исключение
	...
	Попытка                                    
		РезультатВызова = Ждать ОбъектДрайвера.ПолучитьДополнительныеДействияАсинх(ДополнительныеДействия);
	Исключение
	...
	Если УстановитьПараметры И ПараметрыПодключения.Параметры <> Неопределено Тогда
		Для Каждого Параметр Из ПараметрыПодключения.Параметры Цикл
			Если Лев(Параметр.Ключ, 2) = "P_" Тогда
				...
				Попытка
					РезультатВызова = Ждать ОбъектДрайвера.УстановитьПараметрАсинх(ИмяПараметра, ЗначениеПараметра) 
				Исключение
					Возврат Ложь;
				КонецПопытки
			КонецЕсли;
...
Асинх Функция ПодключитьУстройствоАсинх(ПараметрыПодключения) Экспорт
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 4000 Тогда   
		Если Не ПустаяСтрока(ПараметрыПодключения.ШаблонЛокализации) Тогда   
			Попытка                 
				...
				РезультатВызова = Ждать ОбъектДрайвера.УстановитьЛокализациюАсинх(КодЯзыка, ПараметрыПодключения.ШаблонЛокализации);
				Если НЕ РезультатВызова.Значение Тогда
					ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru='Ошибка локализации драйвера.'"));
				КонецЕсли
			Исключение
	...
	Исключение
		Возврат ОшибкаВызоваДрайвера("Подключить");
	КонецПопытки;
...
Асинх Функция ОтключитьУстройствоАсинх(ПараметрыПодключения) Экспорт
	...
	КонецЕсли;   
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОтключитьАсинх(ПараметрыПодключения.ИДУстройства);
		РезультатВыполнения = РезультатОперацииНаОборудовании(РезультатВызова.Значение);
	Исключение
		Возврат ОшибкаВызоваДрайвера("Отключить", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ОбработатьСобытиеВеса(ПараметрыПодключения, Данные) Экспорт
	...
	ДанныеСобытия = Новый Структура("Вес,Стабилизация,ИдентификаторУстройства", Вес, Стабилизация, ПараметрыПодключения.ИдентификаторУстройства);
	РезультатОбработки = Новый Структура();
...
Функция ОшибкаВызоваДрайвера(МетодДрайвера = Неопределено, ОписаниеОшибки = Неопределено) Экспорт
...
Асинх Функция ФискализацияЧека(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		ВыполнялосьПодтврждениеПокупки = Ложь;
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3004  Тогда
			...
			Если НЕ ПустаяСтрока(ТекущийСтатус.ИдентификаторЗапроса) Тогда
				Попытка
					РезультатВызова = Ждать ОбъектДрайвера.ПодтвердитьКМАсинх(ПараметрыПодключения.ИДУстройства, ТекущийСтатус.ИдентификаторЗапроса, 0);
					МенеджерОборудованияКлиент.УстановитьПроверкуКодаМаркировки(ПараметрыПодключения.ИдентификаторУстройства);
					ВыполнялосьПодтврждениеПокупки = Истина;
				Исключение
		...
		Попытка
			Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
				...
				КонецЕсли;
				РезультатВызова = Ждать ОбъектДрайвера.СформироватьЧекАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.Электронно, ДанныеЧекаXML, ДанныеОперации.РезультатОперацииXML); 
				ПараметрыВызова = РезультатВызова.Параметры;
		...
		Если РезультатВызова.Значение Тогда              
			...
			Если ТекущийСтатус <> Неопределено Тогда
				...
				ЗаписьОперацииКМ.Вставить("ВыполнялосьПодтврждениеПокупки", ВыполнялосьПодтврждениеПокупки);
	...
	Попытка
		Если ДанныеОперации.Свойство("ТестовыеЧеки") И ДанныеОперации.ТестовыеЧеки.Количество() > 0 Тогда
			Для Каждого ТекстПечати Из ДанныеОперации.ТестовыеЧеки Цикл
				РезультатВызова = Ждать ОбъектДрайвера.НапечататьТекстовыйДокументАсинх(ПараметрыПодключения.ИДУстройства, ТекстПечати);
				Если НЕ РезультатВызова.Значение Тогда
					РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
					Возврат РезультатВыполнения;
				КонецЕсли;
		...
		Если ДанныеОперации.Свойство("КопияЧека") И ДанныеОперации.КопияЧека.Количество() > 0 Тогда
			Для Каждого Копия Из ДанныеОперации.КопияЧека Цикл
				РезультатВызова = Ждать ОбъектДрайвера.НапечататьТекстовыйДокументАсинх(ПараметрыПодключения.ИДУстройства, Копия);
				Если НЕ РезультатВызова.Значение Тогда
					РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
					Возврат РезультатВыполнения;
				КонецЕсли;
...
Асинх Функция Инкассация(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.НапечататьЧекВнесенияВыемкиАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ДанныеЧекаXML, СуммаЧека);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
...
Асинх Функция ОткрытьДенежныйЯщик(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОткрытьДенежныйЯщикАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда   
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);   
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ОтчетБезГашения(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.НапечататьОтчетБезГашенияАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
			Иначе
				РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ТекущееСостояние(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
			РезультатВызова = Ждать ОбъектДрайвера.ПолучитьТекущееСостояниеАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, ДанныеОперации.РезультатXML);
		Иначе
...
Асинх Функция ОперацияФискальногоНакопителя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОперацияФНАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ТипОперации, ДанныеОперации.ПараметрыXML);
		Если НЕ РезультатВызова.Значение Тогда   
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
			Возврат РезультатВыполнения;
		КонецЕсли;                            
...
Асинх Функция НапечататьКопиюЧека(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ДанныеОперации.Свойство("Аппаратно") И ДанныеОперации.Аппаратно Тогда
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.НапечататьКопиюЧекаАсинх(ПараметрыПодключения.ИДУстройства, НомерЧека);
			Если РезультатВызова.Значение Тогда   
				РезультатВыполнения = РезультатОперацииНаОборудовании(Истина); 
...
Асинх Функция ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОткрытьСессиюРегистрацииКМАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда
...
Асинх Функция ЗакрытьСессиюРегистрацииКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ЗакрытьСессиюРегистрацииКМАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
			МенеджерОборудованияКлиент.УстановитьСессиюПроверкиКодовМаркировки(ПараметрыПодключения.ИдентификаторУстройства);
		Иначе
...
Асинх Функция ЗапросКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если НЕ ПустаяСтрока(ИдентификаторЗапроса) Тогда
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПодтвердитьКМАсинх(ПараметрыПодключения.ИДУстройства, ИдентификаторЗапроса, 0);
			МенеджерОборудованияКлиент.УстановитьПроверкуКодаМаркировки(ПараметрыПодключения.ИдентификаторУстройства);
		Исключение
			РезультатВыполнения = ОшибкаВызоваДрайвера("ПодтвердитьКМ", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
			Возврат РезультатВыполнения;
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ЗапросКМАсинх(ПараметрыПодключения.ИДУстройства, ДанныеОперации.ПараметрыXML, "");   
		Если РезультатВызова.Значение Тогда  
			ПараметрыВызова = РезультатВызова.Параметры;     
...
Асинх Функция ПодтвердитьКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПодтвердитьКМАсинх(ПараметрыПодключения.ИДУстройства, ИдентификаторЗапроса, ?(Выбытие, 0, 1));
		Если РезультатВызова.Значение Тогда   
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
			РезультатВыполнения.Вставить("ИдентификаторЗапроса", ИдентификаторЗапроса);
			МенеджерОборудованияКлиент.УстановитьПроверкуКодаМаркировки(ПараметрыПодключения.ИдентификаторУстройства);
...
Асинх Функция ФискализацияЧекаПакетом(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.СформироватьЧекАсинх(ПараметрыПодключения.ИДУстройства, Ложь, ДанныеОперации.ДанныеЧекаXML, ДанныеОперации.РезультатОперацииXML);;
	Исключение
		РезультатВыполнения = ОшибкаВызоваДрайвера("СформироватьЧек", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); 
		Возврат РезультатВыполнения;
	КонецПопытки;
...
Асинх Функция ПараметрыТерминала(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Иначе     
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПечатьКвитанцийНаТерминалеАсинх();
...
Асинх Функция ЭквайринговаяОперация(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Попытка   
		...
		Если РезультатВызова.Значение Тогда        
			...
			КонецЕсли;  
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина,, ПараметрыПодключения.ИдентификаторУстройства);
...
Асинх Функция АварийнаяОтменаОперации(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.АварийнаяОтменаОперацииАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);  
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 3004 Тогда
		Попытка
			...
			Если РезультатВызова.Значение Тогда
				...
				ПараметрыПодключения.ДисплейОтображаетСостоянияРМ = ПараметрыТерминала.Свойство("DisplayWorkplaceStatus") И ВРег(ПараметрыТерминала.DisplayWorkplaceStatus) = "TRUE"; 
...
Асинх Функция ВывестиСтрокуНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ВывестиСтрокуНаДисплейПокупателяАсинх(ПараметрыПодключения.ИДУстройства, СтрокиТекста);
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВывестиQRКодНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ВывестиQRКодНаДисплейПокупателяАсинх(ПараметрыПодключения.ИДУстройства, ЗначениеQRКода, КартинкаQRКода);
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВывестиHTMLНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ВывестиHTMLАсинх(ПараметрыПодключения.ИДУстройства, ТекстHTML);
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВывестиСтатусНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                                                                                    
		РезультатВызова = Ждать ОбъектДрайвера.ОтобразитьСтатусОперацииАсинх(ПараметрыПодключения.ИДУстройства, СтатусОперации);      
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВывестиСтатусРабочегоМестаНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка                                                                                    
		РезультатВызова = Ждать ОбъектДрайвера.ОтобразитьСтатусРабочегоМестаАсинх(ПараметрыПодключения.ИДУстройства, СтатусРабочегоМеста, ЗвуковойСигнал);      
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ОчиститьДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОчиститьДисплейПокупателяАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция Тарировать(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.УстановитьВесТарыАсинх(ПараметрыПодключения.ИДУстройства, ВесТары);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВыгрузитьТовары(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ВыгрузитьТоварыАсинх(ПараметрыПодключения.ИДУстройства, ДанныеДляВыгрузки, СтатусПакета);
		Исключение                  
...
Асинх Функция ОчиститьТоварыВВесах(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОчиститьТоварыАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВыгрузитьИзображенияТоваров(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ВыгрузитьИзображенияТоваровАсинх(ПараметрыПодключения.ИДУстройства, ДанныеДляВыгрузки, СтатусПакета);
		Исключение                  
...
Асинх Функция ПодтвердитьРаспознаваниеАсинх(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ПодтвердитьРаспознаваниеАсинх(ПараметрыПодключения.ИДУстройства, Код);
		Если РезультатВызова.Значение Тогда                    
			Результат = РезультатОперацииНаОборудовании(Истина);
		Иначе
			Результат = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ОчиститьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		РезультатВызова = Ждать ОбъектДрайвера.ОчиститьТаблицуАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ВыгрузитьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ВыгрузитьТаблицуАсинх(ПараметрыПодключения.ИДУстройства, ДанныеДляВыгрузки, СтатусПакета);
		Исключение
...
Асинх Функция ИнициализацияПринтера(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		РезультатВызова = Ждать ОбъектДрайвера.ИнициализацияПринтераАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ПечатьЭтикеток(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Счетчик = 0 По КоличествоПакетов Цикл
		...
		Попытка
			РезультатВызова = Ждать ОбъектДрайвера.ПечатьЭтикетокАсинх(ПараметрыПодключения.ИДУстройства, ДанныеДляВыгрузки, СтатусПакета);
		Исключение
...
Асинх Функция ОткрытьСессиюRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		РезультатВызова = Ждать ОбъектДрайвера.ОткрытьСессиюRFIDАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ЗакрытьСессиюRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
 	...
 	Попытка       
		РезультатВызова = Ждать ОбъектДрайвера.ЗакрытьСессиюRFIDАсинх(ПараметрыПодключения.ИДУстройства);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
...
Асинх Функция ЗаписатьДанныеВМеткуRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		РезультатВызова = Ждать ОбъектДрайвера.ЗаписатьДанныеВМеткуRFIDАсинх(ПараметрыПодключения.ИДУстройства, TID, EPC, Данные, БанкПамяти, Таймаут);
		Если РезультатВызова.Значение Тогда                    
			РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
		Иначе
			РезультатВыполнения = Ждать ПолучитьОшибкуДрайвера(ОбъектДрайвера);
