...
Функция ИндексыСПАРККонтрагента(Контрагент, ВидКонтрагента, ПризнакЗагрузкиДанных = Ложь) Экспорт
	...
	УведомленияМонитораПортала = УведомлениеПользователя(
		СПАРКРискиКлиентСервер.ИдентификаторУслугиИндикаторыРиска());
	Если УведомленияМонитораПортала <> Неопределено Тогда
		Результат.Уведомление = УведомленияМонитораПортала;
	КонецЕсли;
	
	// Если информации в кэше нет, то получить ее фоновым заданием.
	Если ПризнакЗагрузкиДанных
		И НеобходимоЗагрузитьИнформацию Тогда
	...
	Если Не ИННСоответствуетТребованиям(Результат.ИНН, ВидКонтрагента) Тогда
...
Процедура ПриСозданииНаСервере(Форма, КонтрагентОбъект, Контрагент, ВидКонтрагента, ПараметрыОтображения) Экспорт
	...
	Форма.НастройкиСПАРКРиски = Новый Структура;
	Форма.НастройкиСПАРКРиски.Вставить("ИспользованиеРазрешено", ИспользованиеРазрешено);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Версия = "*";
		Обработчик.РежимВыполнения = "Оперативно";
		Обработчик.Процедура = "СПАРКРиски.ЗаполнитьОбработчикиРазделенныхДанных";
		Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Заполнение разделенных обработчиков'");
	Иначе
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия              = "*";
		Обработчик.Процедура           = "СПАРКРиски.НачальноеЗаполнениеДанныхСПАРКРиски";
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия              = "2.5.1.35";
		Обработчик.Процедура           = "СПАРКРиски.УстановитьСвойстваТипаСобытияДействующее";
		Обработчик.НачальноеЗаполнение = Ложь;
		Обработчик.ОбщиеДанные         = Истина;
		Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Установка свойств для типа события Действующее'");
		Обработчик.РежимВыполнения     = "Оперативно";
	КонецЕсли;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Просмотр событий мониторинга 1СПАРК Риски по контрагентам за последние 15 дней.'");
	...
	Если ОтчетНадежностьДебиторов <> Неопределено Тогда
		...
		НастройкиВарианта = ВариантыОтчетов.ОписаниеВарианта(
			...
			"Основной");
		НастройкиВарианта.Описание =
			НСтр("ru = 'Данные дебиторской задолженности с индексами 1СПАРК Риски по контрагентам.'");
	КонецЕсли;
...
// Возвращает значение функциональной опции "ИспользоватьСервисСПАРКРиски".
//
// Возвращаемое значение:
//  Булево
//
...
Процедура ПриФормированииСпискаУслуг(Услуги) Экспорт
	...
	НоваяУслуга = Новый Структура;
	НоваяУслуга.Вставить("Идентификатор", ИдентификаторУслугиВебКарточкиПоКомпаниям());
	НоваяУслуга.Вставить("Наименование" , НСтр("ru = 'Веб-карточки по компаниям'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
...
Функция ДанныеАутентификацииПользователяИнтернетПоддержки(НомерОбласти = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ДанныеАутентификации.Вставить("ИспользуетсяТикет", Истина);
		ДанныеАутентификации.Вставить("Тикет",             "");
		ДанныеАутентификации.Вставить("НомерОбласти",      НомерОбласти);
	Иначе
		ДанныеАутентификации = ИнтернетПоддержкаПользователей.ДанныеАутентификацииПользователяИнтернетПоддержки();
		Если ДанныеАутентификации <> Неопределено Тогда
			ДанныеАутентификации.Вставить("ИспользуетсяТикет", Ложь);
		КонецЕсли;
	КонецЕсли;
		...
		Если Выборка.Критичное Тогда
			ДанныеВыбора.Добавить(Выборка.Ссылка,
				СтроковыеФункции.ФорматированнаяСтрока(
					"<span style=""color: ЦветГрадацияСПАРКВысокийРиск>%1</span>",
...
Функция ПодключаемыеКомандыФормыКонтрагента(ЭтоФормаОбъекта)
	...
	Если ИспользованиеРазрешено("ПереходВВебВерсиюСПАРК;") Тогда
		СтрКоманды = Результат.Добавить();
		СтрКоманды.Имя     = "СПАРКРиски_ПерейтиВСПАРК";
		СтрКоманды.Синоним = НСтр("ru = 'Веб-версия СПАРК'");
	КонецЕсли;
		...
		Для Каждого ТекущееПроверяемоеФоновоеЗадание Из ПроверяемыеФоновыеЗадания Цикл
			...
			Если ФоновоеЗадание <> Неопределено Тогда
				...
				Если ТипЗнч(ФоновоеЗадание.ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
					ТекущееПроверяемоеФоновоеЗадание.Вставить("ПодробноеПредставлениеОшибки",
...
Процедура ЗапуститьФоновоеЗаданиеИндексыСПАРК(
	...
	ТекстОшибки = "";
	Если Не ИННСоответствуетТребованиям(КонтрагентИНН, ВидКонтрагента, ТекстОшибки) Тогда
	...
	// Ограничение: в один момент времени нельзя запустить несколько
	// фоновых заданий по получению индексов для одинакового ИНН.
	...
	// Всегда запускать в фоне, даже если в очереди уже есть фоновые задания.
	...
	ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Выполнено" Тогда
		...
		Если РезультатВыполненияФоновогоЗадания <> Неопределено Тогда
			Если (ТипЗнч(РезультатВыполненияФоновогоЗадания.ЗначенияИндексов) = Тип("ТаблицаЗначений"))
			...
			ВидОшибкиФоновогоЗадания = РезультатВыполненияФоновогоЗадания.ВидОшибки;
			Если Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ПустаяСсылка() Тогда
				Результат.Вставить("ВидОшибки", ВидОшибкиФоновогоЗадания);
			Иначе
				// Некоторые виды общих ошибок приоритетнее локальных ошибок по каждому ИНН.
				Если (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ИспользованиеОтключено)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаАутентификации)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаПодключения)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ПревышеноКоличествоПопытокАутентификации)
						ИЛИ (ВидОшибкиФоновогоЗадания = Перечисления.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит) Тогда
					Результат.Вставить("ВидОшибки", ВидОшибкиФоновогоЗадания);
				КонецЕсли;
			КонецЕсли;
...
Функция URLВебВерсииСПАРК(Контрагент) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВидОшибки",         Неопределено);
	Результат.Вставить("URL",               Неопределено);
	Результат.Вставить("ИНН",               Неопределено);
	
	ЗначенияРеквизитов = ЗначенияРеквизитовКонтрагентов(Контрагент);
	Если ЗначенияРеквизитов.Количество() = 0 Тогда
		Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.НеПодлежитПроверке;
		Возврат Результат;
	КонецЕсли;
	
	Реквизиты = ЗначенияРеквизитов[0];
	Если Не Реквизиты.ПодлежитПроверке Тогда
		Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.НеПодлежитПроверке;
		Возврат Результат;
	КонецЕсли;
	
	ИдентификаторУслуги = ИдентификаторУслугиВебКарточкиПоКомпаниям();
	...
	РезультатПолучения = СервисСПАРКРиски.URLВебВерсииСПАРК(
		Реквизиты.ИНН);
	...
	Если Не ЗначениеЗаполнено(РезультатПолучения.ВидОшибки) Тогда
		Запись = РегистрыСведений.ПереходыВВебВерсиюСПАРК.СоздатьМенеджерЗаписи();
		Запись.Контрагент   = Контрагент;
		Запись.ИНН          = Реквизиты.ИНН;
...
Процедура КонтрагентБИПОбъектПриЗаписиОбработчик(КонтрагентОбъект, Отказ) Экспорт
	...
	// Запись в регистры сведений запрещена для роли "ЧтениеДанныхСПАРКРиски",
	// поэтому писать будем в привилегированном режиме.
	УстановитьПривилегированныйРежим(Истина);
...
Процедура ОбновлениеИндексовСПАРКРиски() Экспорт
	...
	Если Не МонопольныйРежим() Тогда
		...
		Если РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(ИдентификаторОперацииЗагрузкаИндексов()) Тогда
			...
			Операция = ИдентификаторОперацииЗагрузкаИндексов();
			РасписанияРегламентныхЗаданий.ОперацияВыполнена(
				Операция, ПараметрыВыполненияОперации(Операция));
			
		КонецЕсли;
		
		// 2. Постановка контрагентов на мониторинг в сервисе 1СПАРК Риски.
		Операция = ИдентификаторОперацииПостановкаНаМониторинг();
...
Процедура ЗаполнениеКонтрагентовНаМониторингеСПАРКРиски() Экспорт
	...
	Операция = ИдентификаторОперацииЗаполнениеКонтрагентовНаМониторинге();
	Если Не МонопольныйРежим()
		И РасписанияРегламентныхЗаданий.ТребуетсяВыполнение(Операция) Тогда
...
//
Функция ПараметрыВыполненияОперации(Операция)
...
// Заполняет свойства типа событий мониторинга Действующее.
//
Процедура УстановитьСвойстваТипаСобытияДействующее() Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.ТипыСобытийСПАРКРиски КАК ТипыСобытийСПАРКРиски
		|ГДЕ
		|	ТипыСобытийСПАРКРиски.Идентификатор = &Идентификатор
		|	И ТипыСобытийСПАРКРиски.Критичное";
	
	Запрос.УстановитьПараметр("Идентификатор", Новый УникальныйИдентификатор("05b14f88-3832-406c-b789-86e1527aceff"));
...
Процедура ЗаполнитьОбработчикиРазделенныхДанных(Параметры = Неопределено) Экспорт
	...
	Если Параметры <> Неопределено И Не СвойстваКонтрагентовЗаполнены() Тогда
...
// Проверяет заполнение свойств контрагентов СПАРК.
//
// Возвращаемое значение:
//  Булево - если Истина, была выполнено начальное заполнение данных СПАРК.
//
Функция СвойстваКонтрагентовЗаполнены()
...
Функция ИдентификаторУслугиВебКарточкиПоКомпаниям() Экспорт
	
	Возврат "1c-spark-risks-web-access";
...
// Выводит уведомление опции сервиса портала 1С:ИТС в ТабличныйДокумент.
//
// Параметры:
//  ДокументРезультат - ТабличныйДокумент - табличный документ для вывода уведомления.
//
Процедура ВывестиУведомлениеПользователяВОтчете(ДокументРезультат) Экспорт
	
	УведомлениеПользователя = УведомлениеПользователя(
		ИдентификаторУслугиМониторингКомпаний());
	
	Если УведомлениеПользователя = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Макет = ПолучитьОбщийМакет("ОшибкиОтчетовСПАРКРиски");
	Область = Макет.ПолучитьОбласть("УведомлениеМонитораПортала1СИТС");
	Область.Параметры.ТекстУведомления = Формат(УведомлениеПользователя.ТекстУведомления, "ДЛФ=D");
	ОбластьГиперссылка = Область.Область(1,2,1,2);
	ОбластьГиперссылка.Расшифровка = УведомлениеПользователя;
	ДокументРезультат.Вывести(Область);
...
Функция УведомлениеПользователя(ИдентификаторОпции) Экспорт
	...
	Если Не МодульМониторПортала1СИТС.ПоказыватьУведомлениеПортала("1СПАРК Риски", ИдентификаторОпции) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	УведомлениеОпцииПользователя.Вставить("Сервис", "1СПАРК Риски");
	Если УведомлениеОпцииПользователя.ВидУведомления = ВидУведомленияЗавершаетсяСрокДействияСервиса() Тогда
		...
		УведомлениеОпцииПользователя.Вставить("ПодробныйТекстУведомления",
			ПодробныйТекстУведомленияЗавершаетсяСрокДействияСервиса(
				УведомлениеОпцииПользователя));
		УведомлениеОпцииПользователя.Вставить("ОднострочныйТекстУведомления",
			ОднострочныйТекстУведомленияЗавершаетсяСрокДействияСервиса(
				УведомлениеОпцииПользователя));
...
Функция ТекстУведомленияЗавершаетсяСрокДействияСервиса(ДополнительныеПараметры)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Срок действия 1СПАРК Риски заканчивается %1 года.'"),
...
Функция ТекстУведомленияЗаканчиваютсяСчетныеОпции(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиСправкиПоКомпаниям() Тогда
		Возврат НСтр("ru = 'Скоро будет превышен лимит на получение бизнес-справок по контрагентам.'");
	ИначеЕсли ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиВебКарточкиПоКомпаниям() Тогда
		Возврат НСтр("ru = 'Скоро будет превышен лимит переходов в веб-карточки СПАРК по контрагентам.'");
	Иначе
		Возврат НСтр("ru = 'Скоро будет превышен лимит в сервисе 1СПАРК Риски.'");
	КонецЕсли;
...
Функция ПодробныйТекстУведомленияЗавершаетсяСрокДействияСервиса(ДополнительныеПараметры)
	...
	Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '1СПАРК Риски действует <b>до %1</b>. Для продления доступа обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"),
			Формат(ДополнительныеПараметры.ДатаОкончания, "ДЛФ=D"));
...
Функция ПодробныйТекстУведомленияЗаканчиваютсяСчетныеОпции(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиСправкиПоКомпаниям() Тогда
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Заканчивается доступное количество контрагентов, по которым можно получать бизнес-справки в сервисе 1СПАРК Риски. Для увеличения количества доступных бизнес-справок по контрагентам обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"));
	ИначеЕсли ДополнительныеПараметры.ИдентификаторОпции = ИдентификаторУслугиВебКарточкиПоКомпаниям() Тогда
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Заканчивается доступное количество контрагентов, по которым можно переходить в веб-карточки в сервисе 1СПАРК Риски. Для увеличения количества доступных веб-карточек по контрагентам обратитесь к своему обслуживающему партнеру 1С или к любому партнеру в <a href = ""https://its.1c.ru/zakaz"">вашем регионе</a>.'"));
	КонецЕсли;
	
КонецФункции

Функция ОднострочныйТекстУведомленияЗавершаетсяСрокДействияСервиса(ДополнительныеПараметры)
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '1СПАРК Риски заканчивается %1.'"),
		Формат(ДополнительныеПараметры.ДатаОкончания, "ДФ=dd.MM.yy"));
