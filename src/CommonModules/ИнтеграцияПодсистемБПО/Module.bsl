...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	MACАдресРабочегоМеста = "";
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		MACАдресРабочегоМеста = ОбщегоНазначенияБПО.ЗначениеРеквизитаОбъекта(РабочееМесто, "MACАдрес");
	КонецЕсли;
	Параметры.Вставить("MACАдресРабочегоМеста", MACАдресРабочегоМеста);
...
#КонецЕсли
	...
	ИменаМакетовДляПереустановки = МенеджерОборудования.МакетыДляПереустановкиДрайверов(ИдентификаторКлиента, ИмяКомпьютера);
