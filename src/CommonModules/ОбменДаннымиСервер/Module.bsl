...
Функция ВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	Возврат ТранспортСообщенийОбмена.УстановитьВнешнееСоединениеСБазой(НастройкиТранспорта);
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	Иначе
		...
		Соообщение = НСтр("ru = 'Не определен идентификатор транспорта'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		ВызватьИсключение Соообщение;
