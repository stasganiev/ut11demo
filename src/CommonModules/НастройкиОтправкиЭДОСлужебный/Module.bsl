...
Функция НастройкиОтправки(КлючНастроекОтправки) Экспорт
	...
	НастройкиОтправкиДругихПодсистем = НастройкиОтправкиПодсистемыЭлектронноеАктированиеЕИС(КлючНастроекОтправки);
	Если ЗначениеЗаполнено(НастройкиОтправкиДругихПодсистем) Тогда
		Возврат НастройкиОтправкиДругихПодсистем;
	КонецЕсли;
	
	НастройкиОтправкиИнтеркампани = НастройкиОтправкиИнтеркампани(КлючНастроекОтправки)	;
	Если ЗначениеЗаполнено(НастройкиОтправкиИнтеркампани) Тогда
		Возврат НастройкиОтправкиИнтеркампани;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|	И НЕ НастройкиОтправкиПоВидам.СпособОбменаЭД В (&ВнутреннийСпособОбменаЭД, &ПустойСпособОбменаЭД)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ВнутреннийСпособОбменаЭД", Перечисления.СпособыОбменаЭД.Внутренний);
	Запрос.УстановитьПараметр("ПустойСпособОбменаЭД", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Настройка.ГотовностьКОбмену = УчастникиЭДОГотовыКОбмену(Настройка.ИдентификаторОтправителя, 
				Настройка.ИдентификаторПолучателя, Настройка.СпособОбмена);
	КонецЦикла;
...
Функция НастройкиОтправкиИнтеркампани(КлючНастроекОтправки) Экспорт
	...
	Если ИспользуемыеВидыДокументовИнтеркампани.Найти(КлючНастроекОтправки.ВидДокумента) = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|	И НастройкиОтправкиПоВидам.ВидДокумента = &ВидДокумента
		|	И НастройкиОтправкиПоВидам.Договор В (&Договор, &ПустойДоговор)
		|	И НастройкиОтправкиПоВидам.СпособОбменаЭД = &ВнутреннийСпособОбменаЭД
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЭтоНастройкаПоДоговору УБЫВ";
...
// 	- Неопределено - настройки не существуют
// 	- См. НастройкиЭДОКлиентСервер.НоваяНастройкаОтправки
Функция НастройкиОтправкиПодсистемыЭлектронноеАктированиеЕИС(КлючНастроекОтправки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
		...
		Если МодульЭлектронноеАктированиеЕИС.ИспользоватьЭлектронноеАктированиеВЕИС()
			...
			НастройкиОтправкиЕИС = МодульЭлектронноеАктированиеЕИС.НастройкиОтправкиПоКлючу(КлючНастроекОтправки);
			Если ЗначениеЗаполнено(НастройкиОтправкиЕИС) Тогда
				Результат = НастройкиОтправкиЕИС;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоРасширеннаяНастройка(КлючНастроекОтправки) Экспорт
	...
	Запрос.УстановитьПараметр("Отправитель", КлючНастроекОтправки.Отправитель);
	Запрос.УстановитьПараметр("Получатель", КлючНастроекОтправки.Получатель);
	Если ЗначениеЗаполнено(КлючНастроекОтправки.Договор) Тогда
		Запрос.УстановитьПараметр("Договор", КлючНастроекОтправки.Договор);
	Иначе
		ПустойДоговор = ОбменСКонтрагентамиИнтеграция.ВсеПустыеЗначенияДоговораСКонтрагентомЭДО();
		Запрос.УстановитьПараметр("Договор", ПустойДоговор);
	КонецЕсли;
...
Функция ИспользуетсяУПД_УКД(КлючНастроекОтправки) Экспорт
	...
	Запрос.Текст = 
		...
		|			ТОГДА 0
		|		КОГДА НастройкиОтправкиЭлектронныхДокументов.Договор В (&ПустойДоговор)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Порядок
		|ИЗ
...
Функция СсылкаНаОбъектНастройкиЭДО(Организация, Контрагент, ДоговорКонтрагента) Экспорт
	...
	Запрос.Текст = 
		...
		|	И НастройкиЭДО.ДоговорКонтрагента В (&ДоговорКонтрагента)";

	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Запрос.УстановитьПараметр("ДоговорКонтрагента", ДоговорКонтрагента);
	Иначе
		ПустойДоговор = ОбменСКонтрагентамиИнтеграция.ВсеПустыеЗначенияДоговораСКонтрагентомЭДО();
		Запрос.УстановитьПараметр("ДоговорКонтрагента", ПустойДоговор);
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиПоВидамЭлектронныхДокументов(Настройки, ВидыДокументов, СпособОбмена = Неопределено) Экспорт
	...
	Для Каждого ШаблонНастройки Из ШаблоныНастроекПоВидам Цикл
		...
		// Снимаем использование по умолчанию, если тип документа: "Договорный документ"
		Если Настройка.ВидДокумента.ТипДокумента <> Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент Тогда
...
Функция НастройкаПоДоговоруКонтрагента(Знач Договор) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.НастройкиОтправкиЭлектронныхДокументов КАК НастройкиОтправкиЭлектронныхДокументов
		|ГДЕ
		|	НастройкиОтправкиЭлектронныхДокументов.Договор В (&Договор)";
...
Функция ИдентификаторыУчастниковОбмена(УчастникиОбмена) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УчастникиОбмена.ВидДокумента КАК ВидДокумента,
	|	УчастникиОбмена.Организация КАК Организация,
	|	УчастникиОбмена.Контрагент КАК Контрагент,
	|	УчастникиОбмена.ДоговорКонтрагента КАК ДоговорКонтрагента
	|ПОМЕСТИТЬ УчастникиОбмена
	|ИЗ
	|	&УчастникиОбмена КАК УчастникиОбмена
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрСведений.НастройкиОтправкиЭлектронныхДокументовПоВидам КАК НастройкиОтправкиПоВидам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ УчастникиОбмена КАК УчастникиОбмена
	|		ПО НастройкиОтправкиПоВидам.Отправитель = УчастникиОбмена.Организация
	|			И НастройкиОтправкиПоВидам.Получатель = УчастникиОбмена.Контрагент
	|			И (НастройкиОтправкиПоВидам.Договор = УчастникиОбмена.ДоговорКонтрагента
	|				ИЛИ НастройкиОтправкиПоВидам.Договор В (&ПустойДоговор))
	|			И НастройкиОтправкиПоВидам.ВидДокумента = УчастникиОбмена.ВидДокумента
	|			И (НЕ НастройкиОтправкиПоВидам.СпособОбменаЭД В (&ВнутреннийСпособОбменаЭД, &ПустойСпособОбменаЭД))";
	
	ПустойДоговор = ОбменСКонтрагентамиИнтеграция.ВсеПустыеЗначенияДоговораСКонтрагентомЭДО();
	Запрос.УстановитьПараметр("ПустойДоговор", ПустойДоговор);
	Запрос.УстановитьПараметр("УчастникиОбмена", УчастникиОбмена);
	Запрос.УстановитьПараметр("ВнутреннийСпособОбменаЭД", Перечисления.СпособыОбменаЭД.Внутренний);
	Запрос.УстановитьПараметр("ПустойСпособОбменаЭД", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
...
// Возвращаемое значение:
//  Булево
Функция УчастникиЭДОГотовыКОбмену(ИдентификаторОтправителя, ИдентификаторПолучателя, СпособОбмена)

	Результат = Ложь;
	
	Если ИдентификаторОтправителя = ИдентификаторПолучателя Тогда
		Результат = Истина;
	ИначеЕсли СинхронизацияЭДО.ЭтоПрямойОбмен(СпособОбмена) Тогда
		Результат = Истина;	
	Иначе
		КлючПриглашенияНаИдентификатор = ПриглашенияЭДОКлиентСервер.КлючПриглашенияНаИдентификатор();
		КлючПриглашенияНаИдентификатор.ИдентификаторОрганизации = ИдентификаторОтправителя;
		КлючПриглашенияНаИдентификатор.ИдентификаторКонтрагента = ИдентификаторПолучателя;
		...
		КлючПриглашения.ИдентификаторОрганизации = ИдентификаторОтправителя;
		...
		КонецЕсли;
		Результат = ПриглашенияЭДО.ПриглашениеПринято(СтатусПриглашения)
