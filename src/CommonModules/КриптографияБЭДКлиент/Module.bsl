...
Процедура Подписать(ОписаниеДанных, КонтекстДиагностики, Форма = Неопределено, Оповещение = Неопределено,
	ПаролиСертификатов = Неопределено, ТипПодписи = Неопределено) Экспорт
	...
	ЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, Форма, СлужебноеОповещение, ТипПодписи);
...
Процедура ПроверитьПодпись(ОповещениеОЗавершении, ИсходныеДанные, Подпись, КонтекстДиагностики, МенеджерКриптографии = Неопределено) Экспорт
	...
	Контекст = КриптографияБЭДСлужебныйКлиент.НовыйКонтекстПроверкиПодписи();
	Контекст.Подпись = Подпись;
	Контекст.ИсходныеДанные = ИсходныеДанные;
	...
	Оповещение = Новый ОписаниеОповещения("ПроверитьПодписьПослеПолученияСертификатовИзПодписиНаСервере",
		КриптографияБЭДСлужебныйКлиент, Контекст);
	
	КриптографияБЭДСлужебныйКлиент.ПолучитьСертификатыИзПодписиНаСервере(Оповещение, Подпись);
...
// Параметры:
//  Оповещение - ОписаниеОповещения - результат см. НовыйИтогРасшифровкиНабораДанных
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
Процедура НачатьРасшифровкуНабораДанных(Оповещение, НаборДанныхДляРасшифровки, ПаролиСертификатов, КонтекстДиагностики) Экспорт
	КонтекстРасшифровки = КриптографияБЭДСлужебныйКлиент.НовыйКонтекстРасшифровкиНабораДанных();
	КонтекстРасшифровки.ОповещениеОЗавершении = Оповещение;
	КонтекстРасшифровки.КлючиДанных = ОбщегоНазначенияБЭДКлиент.ВыгрузитьКолонку(НаборДанныхДляРасшифровки, "Ключ");
	КонтекстРасшифровки.НаборДанныхДляРасшифровки = НаборДанныхДляРасшифровки;
	КонтекстРасшифровки.ПаролиСертификатов = ПаролиСертификатов;
	КонтекстРасшифровки.КонтекстДиагностики = КонтекстДиагностики;
	КриптографияБЭДСлужебныйКлиент.РасшифроватьНаборДанных(КонтекстРасшифровки);
КонецПроцедуры
...
//  ** Ключ - Строка - идентификатор данных для расшифровки.
//  ** Значение - Булево
Функция НовыйИтогРасшифровкиНабораДанных() Экспорт
	ИтогРасшифровки = Новый Структура;
	ИтогРасшифровки.Вставить("НаборРасшифрованныхДанных", КриптографияБЭДКлиентСервер.НовыйНаборРасшифрованныхДанных());
	ИтогРасшифровки.Вставить("ОшибкиРасшифровкиДанных", Новый Соответствие);
	ИтогРасшифровки.Вставить("ОтказыРасшифровкиДанных", Новый Соответствие);
	ИтогРасшифровки.Вставить("ПаролиСертификатов", НовыеПаролиСертификатов());
	Возврат ИтогРасшифровки;
КонецФункции
...
// Возвращаемое значение:
//  Булево
Функция ЕстьПарольСертификата(ПаролиСертификатов, Сертификат) Экспорт
	Возврат ПаролиСертификатов[Сертификат] <> Неопределено;
КонецФункции
...
Функция СертификатыСПаролемИзНабора(ПаролиСертификатов, Сертификаты) Экспорт
	
	СертификатыСПаролем = Новый Массив;
	
	Для Каждого Сертификат Из Сертификаты Цикл
		Если ЕстьПарольСертификата(ПаролиСертификатов, Сертификат) Тогда
			СертификатыСПаролем.Добавить(Сертификат);
		КонецЕсли;
	КонецЦикла;
	
	Возврат СертификатыСПаролем;
...
// Код ошибки, определяющий сценарий, когда при подготовке данных для подписания, пользователь выбрал сертификат,
// требующий подтверждение машиночитаемой доверенностью, при этом в базе отстутсвует подходящая доверенность.
// 
// Возвращаемое значение:
//  Строка
Функция КодОшибкиВыбораСертификатаТребующегоМЧД() Экспорт
	Возврат "ЭлектронноеВзаимодействие.ДляВыбранногоСертификатаОтсутствуетПодходящаяДоверенность";
КонецФункции
