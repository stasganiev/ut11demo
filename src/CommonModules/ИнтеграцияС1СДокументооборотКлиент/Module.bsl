...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	ИначеЕсли Правила.Количество() = 0 Тогда
		...
		ОткрытьФорму(
			"Обработка.ИнтеграцияС1СДокументооборот.Форма.ВыборТипаОбъектаДокументооборота",
			ПараметрыФормы,,,,,
...
Процедура ОткрытьСрокДействияДокумента(Форма, ТолькоПросмотр = Ложь) Экспорт
	...
	ОткрытьФорму(
		"Обработка.ИнтеграцияС1СДокументооборот.Форма.ИзменениеСрокаДействия",
		ПараметрыФормы,
		Форма,,,,
		Оповещение,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НачатьСогласованиеПослеПоискаСвязанногоОбъекта(Результат, Параметры) Экспорт
	...
	ОткрытаяФорма = ОткрытьФорму(
		"Обработка.ИнтеграцияС1СДокументооборот.Форма.ПрерываниеСогласования",
		ПараметрыФормыПрерывания,,,,,
	...
	Иначе // Есть шаблон(ы).
		...
		Иначе // Предложим выбор.
			...
			ОткрытьФорму(
				...
				ПараметрыФормы,,,,,
				ОписаниеОповещенияПослеВыбораШаблона,
				РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		КонецЕсли;
...
Процедура НачатьСогласованиеПослеВыбораШаблона(Результат, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.ЗапуститьСогласованиеПоШаблону(Шаблон, Параметры.Предмет) Тогда
		Параметры.Вставить(
			"Состояние",
			ПредопределенноеЗначение("Перечисление.СостоянияСогласованияВДокументообороте.НаСогласовании"));
...
Процедура ВыполнитьСогласованиеПослеВводаКомментария(Результат, Параметры) Экспорт
	...
	Если Параметры.Результат = "Согласовано" Тогда //@NON-NLS-1
		Параметры.Вставить(
			"Состояние",
			ПредопределенноеЗначение("Перечисление.СостоянияСогласованияВДокументообороте.Согласован"));
		ПриИзмененииСостоянияСогласования(Параметры);
	...
	ИначеЕсли Параметры.Результат = "НеСогласовано" Тогда //@NON-NLS-1
		Параметры.Вставить(
			"Состояние",
			ПредопределенноеЗначение("Перечисление.СостоянияСогласованияВДокументообороте.НеСогласован"));
		ПриИзмененииСостоянияСогласования(Параметры);
...
Процедура ДобавитьПредметПослеВыбораОбъектаДО(ОбъектДО, Параметры) Экспорт
	...
	Если ОбъектДО = Неопределено Или Параметры.ОписаниеОповещения = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ВыполнитьОбработкуОповещения(
		Параметры.ОписаниеОповещения,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеПредмета));
...
Процедура ПрерватьПроцесс(Форма) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПрерватьПроцессПослеВопроса",
		ЭтотОбъект,
		ПараметрыОповещения);
	
	ОткрытьФорму(
		"Обработка.ИнтеграцияС1СДокументооборот.Форма.ПрерываниеПроцесса",,,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция ЕстьНеЗаполненныеПредметы(Предметы, ИмяКолонкиID, ИмяКолонкиПредмет) Экспорт
	...
	ЕстьНеЗаполненныеПредметы = Ложь;
	Для Каждого СтрокаПредмета Из Предметы Цикл
		Если СтрокаПредмета.РольПредмета = "Заполняемый" //@NON-NLS-1
				И СтрокаПредмета.ОбязательноеЗаполнение = Истина
				И Не ЗначениеЗаполнено(СтрокаПредмета[ИмяКолонкиID]) Тогда
			ОчиститьСообщения();
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не введен заполняемый предмет ""%1"". Заполнить предмет можно на стороне 1С:Документооборот.'"),
				СтрокаПредмета[ИмяКолонкиПредмет]);
			ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
				"ДеревоПриложений",
				Предметы.Индекс(СтрокаПредмета),
				"Предстваление");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, ПутьКТабличнойЧасти);
			ЕстьНеЗаполненныеПредметы = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ЕстьНеЗаполненныеПредметы;
