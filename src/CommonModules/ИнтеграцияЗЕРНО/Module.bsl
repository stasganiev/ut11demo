...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.9.0") >= 0 Тогда
		Параметры.Вставить(ИменаПараметров.ВерсияБСППоддерживаетПроверкуСохраненногоПароляСертификатаНаКлиенте, Истина);
	Иначе
		Параметры.Вставить(ИменаПараметров.ВерсияБСППоддерживаетПроверкуСохраненногоПароляСертификатаНаКлиенте, Ложь);
	КонецЕсли;
...
// * ЮридическийАдрес       - Строка                                   - ЮридическийАдрес
...
Функция НоваяТаблицаОрганизацияКонтрагентПодразделение() Экспорт
	...
	ТаблицаДанных.Колонки.Добавить("ЮридическийАдрес",       ОбщегоНазначения.ОписаниеТипаСтрока(250));
...
Функция СоответствиеКлючейОрганизацийИОрганизаций(Знач Организации)
...
Функция ДанныеЗемельногоУчастка(ЗемельныйУчасток) Экспорт
	...
	Данные.Вставить("КомментарийАдреса",        ИнтеграцияИС.КомментарийКонтактнойИнформации(Данные.Адрес));
