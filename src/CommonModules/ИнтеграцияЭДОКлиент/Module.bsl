...
// Возвращаемое значение:
//  ОбщийМодуль
Функция ОбщийМодульОтражениеВУчетеЭДОКлиент() Экспорт
	Возврат ОбщегоНазначенияКлиент.ОбщийМодуль("ОтражениеВУчетеЭДОКлиент");
КонецФункции
...
Процедура ПерезаполнитьДокумент(ОбъектУчета, ЭлектронныйДокумент = Неопределено, СпособОбработки = "") Экспорт	
	Если ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДОКлиент().ПерезаполнитьДокумент(ОбъектУчета, ЭлектронныйДокумент, СпособОбработки);
	КонецЕсли;
КонецПроцедуры
