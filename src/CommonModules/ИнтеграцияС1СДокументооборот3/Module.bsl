...
Процедура ПолучитьHTMLПредпросмотрОбъектаВФоне(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		Результат = HTMLПредпросмотрОбъекта(Прокси, Параметры.ID, Параметры.Тип, Параметры.ИгнорироватьМаксРазмер);
	Исключение
...
Функция HTMLПредпросмотрОбъекта(Прокси, ID, Тип, ИгнорироватьМаксРазмер = Ложь) Экспорт
	...
	Запрос = HTMLПредпросмотрОбъектаЗапрос(Прокси, ID, Тип, ИгнорироватьМаксРазмер);
...
Процедура ЗаполнитьОбработкуВФорме(Форма, ДанныеОбОбработкеXDTO) Экспорт
	...
	Форма.Элементы.ДействияОтправить.Доступность = Не Форма.ТолькоПросмотр;
	Форма.Элементы.ДействияОстановить.Доступность = Не Форма.ТолькоПросмотр;
...
Процедура ЗаполнитьПечатныеФормыОбъектовМетаданных(metadataObjects)
	...
	Для Каждого ОбъектМетаданныхXDTO Из metadataObjects Цикл
		...
		ПечатныеФормыОбъекта = ОбъектМетаданныхXDTO.printedForms; // СписокXDTO
		...
		Для Каждого Строка Из ТаблицаПечатныхФорм Цикл
			...
			ПечатнаяФормаXDTO.additionalParameters =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПараметрыПечатиВСтроку(Строка.ДополнительныеПараметры);
			
			ПечатныеФормыОбъекта.Добавить(ПечатнаяФормаXDTO);
		КонецЦикла;
...
Процедура ЗаполнитьИнтерактивноеДействие(Прокси, ИнтерактивноеДействие, ИнтерактивноеДействиеXDTO)
	...
	Если НРег(ИнтерактивноеДействиеXDTO.interactiveAction) = "selecttaskperformer" Тогда
...
//
Функция НоваяОбластьДействияЗадачи()
