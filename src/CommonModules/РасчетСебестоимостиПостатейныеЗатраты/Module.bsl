...
//		Используется при формировании временных таблиц ВТФИФОДокументыРеализацииБезПереходаПраваСобственности и ВТОтборФИФО
// 5) Формируется вспомогательная таблица документов реализации с отложенным переходом права собственности - ВТФИФОДокументыРеализацииБезПереходаПраваСобственности
//		Используется при формирования временной таблицы ВТФИФОИтогиРеализацииБезПереходаПраваСобственности
// 6) Формируется таблица с итоговым количеством для документов реализации с отложенным переходом права собственности - 
//	ВТФИФОИтогиРеализацииБезПереходаПраваСобственности
//		Используется при формировании временной таблицы ВТОтборФИФО.
// 7) Формируется таблица для отбора остатков и движений при использовании ФИФО - ВТОтборФИФО
...
// 8) Формируется таблица остатков, на которые будут распределены доп расходы - ВТФИФООстаткиСебестоимости
//		Используется при формировании временной таблицы ВТФИФОДетально
// 9) Формируется таблица реализаций прошлого периода - ВТФИФОРеализацииПрошлыхПериодов
//		Используется при формировании временной таблицы ВТФИФОДетально
// 10) Формируется таблица прочих выбытий - ВТФИФОСписанияИПрочиеВыбытия
//		Используется при формировании временной таблицы ВТФИФОДетально
// 11) Формируется общая таблица выбытий прошлого периода - ВТФИФОДетально
//		Используется при формировании временных таблиц ВТФИФОСводно и ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов
// 12) Формируется сводная таблица выбытий прошлого периода - ВТФИФОСводно
...
// 13) Формируются таблицы для отбора приобретений - ВТОтборДокументыПоступленияСредняя и ВТОтборБезДокументаПоступленияСредняя
...
// 14) Формируется таблица для отбора остатков - ВТОтборСредняя
//		Используется при формировании временной таблицы ВТСредняяОстаткиСебестоимости.
// 15) Формируется таблица остатков, по которым отражены доп расходы - ВТСредняяОстаткиСебестоимости
//		Используется при формировании временной таблицы ВТСредняяСводно.
// 16) Формируется таблица приобретений. Содержит аналитику и количество для каждого приобретения - ВТСредняяПриобретения
...
//		Используется при формировании временной таблицы ВТСредняяСводно.
// 17) Формируется таблица соответствия количества приоберетения и количества остатка - ВТСредняяСводно
//		Используется при формировании временной таблицы ВТСредняяДетально.
// 18) Формируется таблица выбытий прошлого периода - ВТСредняяДетально
...
//		Используется при формировании временной таблицы ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов 
//	
// 19) Формируется таблица базы распределения - ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов
...
Функция ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов()
	...
	ТекстЗапросаВТФИФОИтогиРеализацииБезПереходаПраваСобственности = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Т.Регистратор
		|ПОМЕСТИТЬ ВТФИФОДокументыРеализацииБезПереходаПраваСобственности
		|ИЗ
		...
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров КАК Т
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТФИФОДокументыРеализацииБезПереходаПраваСобственности КАК ОтборРегистратор
	...
	ТекстЗапросаПоСредней =
		...
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров.Остатки(
		|		&ГраницаКонецПредыдущегоПериода, Организация В (&ОрганизацииСУчетомПоСредней)) КАК Остатки
		|ГДЕ
		...
		|	И Списания.Организация В (&ОрганизацииСУчетомПоСредней)
		...
		|ИЗ
		|	ПриемникиПоТоварамПрошлыхПериодовДополненная КАК ДД
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СебестоимостьТоваровОстаткиПрошлогоПериода КАК Остатки
	...
	ТекстЗапросаПоказателиБазыРаспределенияПриемниковПрошлыхПериодов = "
		...
		|	ЕСТЬNULL(ВыбытиеДетально.СтатьяАктивовПассивов, НЕОПРЕДЕЛЕНО) КАК СтатьяАктивовПассивов,
		|	ЕСТЬNULL(ВыбытиеДетально.АналитикаАктивовПассивов, НЕОПРЕДЕЛЕНО) КАК АналитикаАктивовПассивов,
		|	ЕСТЬNULL(ВыбытиеДетально.НастройкаСчетовУчета, НЕОПРЕДЕЛЕНО) КАК НастройкаСчетовУчета,
		...
		|	(ВЫБОР
		|		КОГДА ЕСТЬNULL(ВыбытиеДетально.ИдентификаторФинЗаписи, НЕОПРЕДЕЛЕНО) = НЕОПРЕДЕЛЕНО
		|			ТОГДА Источник.ИдентификаторФинЗаписи
		|		ИНАЧЕ ЕСТЬNULL(ВыбытиеДетально.ИдентификаторФинЗаписи, НЕОПРЕДЕЛЕНО) КОНЕЦ) КАК ИдентификаторФинЗаписи,
...
Функция ТекстЗапросаБазаРаспределенияРасходовНаПродажи()
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажИсхДанные.Количество КАК ЧИСЛО(18, 8)) 
	|				/ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажСуммы.Количество КАК ЧИСЛО(18, 8))
	|	КОНЕЦ КАК ДоляКоличество,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВтБазаРаспределенияПродажСуммы.Вес, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажИсхДанные.Вес КАК ЧИСЛО(18, 8)) 
	|				/ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажСуммы.Вес КАК ЧИСЛО(18, 8))
	|	КОНЕЦ КАК ДоляВес,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВтБазаРаспределенияПродажСуммы.Объем, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажИсхДанные.Объем КАК ЧИСЛО(18, 8)) 
	|				/ ВЫРАЗИТЬ(ВтБазаРаспределенияПродажСуммы.Объем КАК ЧИСЛО(18, 8))
	|	КОНЕЦ КАК ДоляОбъем,
