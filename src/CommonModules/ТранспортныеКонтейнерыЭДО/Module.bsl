...
Функция ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, Статус, КонтекстДиагностики = Неопределено, 
	...
	Исключение
		...
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(
			...
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), ДополнительныеПараметрыОшибки);
...
Функция РаспаковатьФайлКонтейнера(Файл, КонтекстДиагностики = Неопределено) Экспорт
	...
	ИмяФайлаКарточкиМЧД = "";
	Если ЗначениеЗаполнено(ОписаниеКонтейнера.Карточка) Тогда 
		ИмяФайлаКарточкиМЧД = ОписаниеКонтейнера.Карточка.КарточкаМЧД;
	КонецЕсли;
	НашлиКарточкуМЧД = Ложь;
	
	Для каждого РаспакованныйФайл Из РаспакованныеФайлы Цикл
		Если НРег(РаспакованныйФайл.Имя) = НРег(ИмяФайлаКарточкиМЧД)
			ИЛИ НРег(РаспакованныйФайл.Имя) = "warrant.xml" Тогда
...
Процедура ПриЗагрузкеОбъекта(Объект, ДанныеОбъекта, КонтекстДиагностики) Экспорт
	...
	Если ДанныеОбъекта.ИдентификаторПолучателя = ДанныеОбъекта.ИдентификаторОтправителя Тогда
		КонтейнерМожноРаспаковать = Истина;
	Иначе
...
Функция ПривестиСтрокуКФорматуИмени1С(ИсходнаяСтрока)
	...
	Результат = "";
	ИсходнаяСтрокаСокрЛП = СокрЛП(ИсходнаяСтрока);
	ДлинаИсходнойСтрокиБезПробелов = СтрДлина(ИсходнаяСтрокаСокрЛП);
	Для к = 1 По ДлинаИсходнойСтрокиБезПробелов Цикл
		
		ТекущийСимвол = Сред(ИсходнаяСтрокаСокрЛП, к, 1);
		
		Если (НЕ ЗначениеЗаполнено(Результат)) 
			И (НЕ ОбщегоНазначенияБЭДКлиентСервер.ЭтоБукваИлиПодчеркивание(ТекущийСимвол)) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ОбщегоНазначенияБЭДКлиентСервер.ЭтоБукваИлиПодчеркивание(ТекущийСимвол) 
			ИЛИ ОбщегоНазначенияБЭДКлиентСервер.ЭтоЦифра(ТекущийСимвол) Тогда
...
// Возвращает полное имя файла ЭД в каталоге распаковки.
// Метод поиска заключается в считывании на низком уровне данных архива.
// Если методом перебора кодировок найдены данные файла, то ищем в каталоге распаковки уже распакованные данные файла.
// При этом ищем файл, который должен был получиться при преобразовании буфера к строке в системной кодировке.
// Если найден, то считаем, что это нужный файл, копируем его в этот же каталог с правильным именем.
// 
// Параметры:
//  КаталогРаспаковкиОсновной - Строка - Каталог распаковки основной
//  ОжидаемыйОтносительныйПуть - Строка - Искомое имя файла
//  ДвоичныеДанныеКонтейнера - ДвоичныеДанные - Двоичные данные контейнера
// 
// Возвращаемое значение:
//  Строка, Неопределено - Полное имя файла ЭД или Неопределено, когда что-то пошло не так
//
Функция ПолноеИмяФайлаЭДВКонтейнереЧтениемАрхива(КаталогРаспаковкиОсновной, ОжидаемыйОтносительныйПуть,
	ДвоичныеДанныеКонтейнера)
	
	ПрочитанныеДанныеАрхива = РаботаСФайламиБЭД.ПрочитатьАрхив(ДвоичныеДанныеКонтейнера);
	Если ПрочитанныеДанныеАрхива.ЕстьОшибка Тогда
	...
	ИскомыеДанныеФайла = Неопределено;
	Для Каждого ДанныеФайла Из ПрочитанныеДанныеАрхива.Файлы Цикл
		БуферИмениФайла = ДанныеФайла.Name;
		Для Каждого Кодировка Из ВозможныеКодировки Цикл
			ИмяФайлаВКодировке = РаботаСФайламиБЭД.БуферВСтрокуВКодировке(БуферИмениФайла, Кодировка);
			Если ИмяФайлаВКодировке = ОжидаемыйОтносительныйПуть Тогда
				ИскомыеДанныеФайла = ДанныеФайла;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ИскомыеДанныеФайла <> Неопределено Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	БуферИмениФайла = ИскомыеДанныеФайла.Name;
	ИмяФайлаВСистемнойКодировке = РаботаСФайламиБЭД.БуферВСтрокуВКодировке(БуферИмениФайла, КодировкаТекста.Системная);
	Файл = Новый Файл(КаталогРаспаковкиОсновной + ИмяФайлаВСистемнойКодировке);
	Если Не Файл.Существует() Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ИмяФайлаВИскомойКодировке = ОжидаемыйОтносительныйПуть;
	
	КопироватьФайл(Файл.ПолноеИмя, КаталогРаспаковкиОсновной + ИмяФайлаВИскомойКодировке);
	УдалитьФайлы(Файл.ПолноеИмя);
	
	Возврат КаталогРаспаковкиОсновной + ИмяФайлаВИскомойКодировке;
...
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	...
	Если ОшибкаЧтенияКарточки Тогда
		ВидДокументаПоддерживается = Истина;
		ДоступныеТипыЭД = ФорматыЭДОПовтИсп.ДоступныеТипыЭДВКарточкеЭлектронногоДокумента();
		
		Попытка
			ОбъектXDTO = РаботаСФайламиБЭД.ПрочитатьXDTO(ИмяФайла);
			ТипЭлектронногоДокумента = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОбъектXDTO, "Type.Name", Неопределено);
		Исключение
			ТипЭлектронногоДокумента = Неопределено;
		КонецПопытки;
		
		Если ТипЭлектронногоДокумента <> Неопределено И ДоступныеТипыЭД.Найти(ТипЭлектронногоДокумента) = Неопределено Тогда
			ШаблонСообщения =
				НСтр("ru = 'Данный вид электронного документа ""%1"" не поддерживается в текущей версии программы.
				|Необходимо обновить конфигурацию до актуальной версии.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ТипЭлектронногоДокумента);
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения);
			ВидДокументаПоддерживается = Ложь;
		КонецЕсли;
		
		// Если не выводили ошибку по поддержке вида документа, то выводим ошибку чтения карточки
		Если ВидДокументаПоддерживается Тогда
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецЕсли;
...
Функция ПрочитатьФайлМетаданных(ИмяФайла, КаталогРаспаковки, ДвоичныеДанныеКонтейнера)
	...
	Для Каждого Документооборот Из МетаданныеXDTO.DocFlow Цикл
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			Если Документ.Files.MainImage <> Неопределено Тогда
				...
				Исключение
					// Пробуем прочитать архив вручную.
					ПутьПеречитанногоФайла = ПолноеИмяФайлаЭДВКонтейнереЧтениемАрхива(КаталогРаспаковки,
						Документ.Files.MainImage.Path, ДвоичныеДанныеКонтейнера);
					Если ПутьПеречитанногоФайла = Неопределено Тогда
					...
					Иначе
						ПолноеИмяФайла = ПутьПеречитанногоФайла;
					КонецЕсли;
					ДвоичныеДанныеФайлаЭД = Новый ДвоичныеДанные(ПолноеИмяФайла);
...
Процедура ДобавитьДополнительныйПараметр(ДополнительныеПараметры, ДополнительныйПараметр)
	...
	Если ЗначениеЗаполнено(ИмяДополнительногоПараметра) Тогда
