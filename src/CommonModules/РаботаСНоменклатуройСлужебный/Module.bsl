...
// 
// Возвращаемое значение:
//  Неопределено, ДвоичныеДанные - полученные данные.
//
Функция ДвоичныеДанныеПоАдресуURL(АдресURL) Экспорт 
	...
	Попытка
		Соединение = Новый HTTPСоединение(ПараметрыСоединения.ИмяСервера, ПараметрыСоединения.Порт, , ,
			ПараметрыСоединения.Прокси, ПараметрыСоединения.Таймаут, ЗащищенноеСоединение);
	Исключение
		ВызватьИсключение НСтр("ru = 'Ошибка установки соединения при получении файла'");
	КонецПопытки;
	...
	Исключение
		ВызватьИсключение НСтр("ru = 'Ошибка получения файла'");
	КонецПопытки;
	
	Если HTTPОтвет.КодСостояния = КодСостоянияВыполнено() Тогда
...
//
// Параметры:
//   ПараметрыКоманды - см. ОписаниеПараметровКомандыСервиса - параметры вызова или имя команды.
...
Функция ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ) Экспорт
	...
	Если ИспользоватьВыполнениеДействийВПовторе() Тогда
		...
		РезультатЗапроса = ВыполнитьДействиеВПовторе("ЗапросКСервису", ПараметрыКоманды);
...
Функция СодержаниеФайлаСервисаПоАдресу(АдресРесурса, ИмяСобытия) Экспорт 
	...
	ПараметрыКоманды.Вставить("Метод"        , ИнтернетСоединениеБЭД.HTTPМетоды().GET);
...
// Метод-конструктор структуры записи регистра сведений
// 
// Возвращаемое значение:
//  Структура:
//   * Номер - Строка
...
//   * ВидДокумента - Строка
//
Функция НовыеПараметрыДокументаАккредитации() Экспорт
	
	Параметры = Новый Структура();
	Параметры.Вставить("Номер", "");
	Параметры.Вставить("ДатаРегистрации", Дата(1, 1, 1));
	Параметры.Вставить("ДатаОкончанияДействия", Дата(1, 1, 1));
...
Функция СервисАккредитацииДоступен() Экспорт
	...
	ПараметрыКоманды = ПараметрыКомандыСервисДоступен();
	Отказ            = Ложь;
	
	Возврат ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ);
	
КонецФункции

// Получает данные СС/ДС из сервиса
//
// Параметры:
//  ДанныеДокумента - Структура:
//   * Статус - Строка
//   * Данные - Структура
//  ПараметрыДокумента - см. НовыеПараметрыДокументаАккредитации
//
Процедура ПолучитьДанныеДокументаАккредитации(ДанныеДокумента, ПараметрыДокумента) Экспорт
	
	ЗапроситьВСервисе = Ложь;
	АдресДанных       = РегистрыСведений.ОчередьЗапросовФСА.АдресДанных(ПараметрыДокумента);
	Если АдресДанных = Неопределено Тогда
		ЗапроситьВСервисе = Истина;
	ИначеЕсли ПустаяСтрока(АдресДанных) Тогда
		ДанныеДокумента.Статус = СтатусПринятоВОбработку();
		ЗапроситьВСервисе = Истина;
	Иначе
		ЗаполнитьДанныеДокументаПоОтветуСервиса(ДанныеДокумента, АдресДанных, ПараметрыДокумента, ЗапроситьВСервисе);
		РегистрыСведений.ОчередьЗапросовФСА.УдалитьИзОчереди(ПараметрыДокумента);
	КонецЕсли;
	
	Если ЗапроситьВСервисе Тогда
		ПараметрыКоманды = Новый Структура("ДанныеДокумента, ПараметрыДокумента, ПараметрыПовтора", 
			ДанныеДокумента, ПараметрыДокумента, ПараметрыПовтораЗапросовАккредитации());
		ВыполнитьДействиеВПовторе("ЗапросКСервисуАккредитацииВПовторе", ПараметрыКоманды);
	КонецЕсли;
	
	Если ДанныеДокумента.Статус = СтатусПринятоВОбработку() Тогда
		РегистрыСведений.ОчередьЗапросовФСА.ЗарегистрироватьКОбработке(ПараметрыДокумента);
		ДатаСтартаРегламента = ТекущаяДатаСеанса();
		ОбновитьНастройкиРегламентногоЗаданияОбработкиОчередиЗапросовФСА(ДатаСтартаРегламента);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьНастройкиРегламентногоЗаданияОбработкиОчередиЗапросовФСА(
	ПлановаяДатаСледующейПопытки = Неопределено) Экспорт
	
	Если ПлановаяДатаСледующейПопытки = Неопределено Тогда
		ПлановаяДатаСледующейПопытки = РегистрыСведений.ОчередьЗапросовФСА.ПлановаяДатаСледующейПопытки();
	КонецЕсли;
	
	ЕстьЗапросыФСАКОбработке = (ПлановаяДатаСледующейПопытки <> Неопределено);
	
	ПараметрыЗадания = Новый Структура("Использование, Расписание", ЕстьЗапросыФСАКОбработке, 
		Новый РасписаниеРегламентногоЗадания);
		
	Если ЕстьЗапросыФСАКОбработке Тогда
		ДатаСтартаРегламента = ПлановаяДатаСледующейПопытки + ИнтервалДатыВСекундахАккредитация();
		ПараметрыЗадания.Расписание.ДатаНачала  = ДатаСтартаРегламента;
		ПараметрыЗадания.Расписание.ВремяНачала = ДатаСтартаРегламента;
	КонецЕсли;
	
	ОтборЗаданий = Новый Структура;
	ОтборЗаданий.Вставить("Метаданные", Метаданные.РегламентныеЗадания.ОбработкаОчередиЗапросовФСА);
	
	ВсеЗадания         = РегламентныеЗаданияСервер.НайтиЗадания(ОтборЗаданий);
	...
	Если ВыбранноеЗадание <> Неопределено Тогда
		РегламентныеЗаданияСервер.ИзменитьЗадание(ВыбранноеЗадание, ПараметрыЗадания);
	ИначеЕсли ПараметрыЗадания.Использование Тогда
		ПараметрыЗадания.Вставить("Метаданные", ОтборЗаданий.Метаданные);
		РегламентныеЗаданияСервер.ДобавитьЗадание(ПараметрыЗадания);
	КонецЕсли;
	
КонецПроцедуры

Функция АдресДанныхДокументаАккредитации(ПараметрыДокумента) Экспорт
	
	АдресДанных = "";
	
	ПараметрыЗапроса = НовыеПараметрыЗапросаДокументаАккредитации();
	ЗаполнитьПараметрыЗапроса(ПараметрыЗапроса, ПараметрыДокумента);
	
	ПараметрыКоманды = ПараметрыКомандыЗапросаДанныхДокументаАккредитации(ПараметрыЗапроса);
	Отказ            = Ложь;
	ОтветСервиса     = ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ);
	
	Если ПараметрыКоманды.КодСостояния = КодСостоянияВыполнено()
		И Не Отказ 
		И ТипЗнч(ОтветСервиса) = Тип("Строка") Тогда
		АдресДанных = ОтветСервиса;
	КонецЕсли;
...
//   * Данные - Строка - Двоичные данные или строка.
//   * Аутентификация - Булево - Требуется аутентификация.
//   * ТипАутентификации - Строка - basic - по тикету ИТС, bearer - по тикету JWT.
//   * БлокироватьСообщенияОбОшибках - Булево - Не выводить сообщения об ошибках.
//   * Результат - Произвольный - Свойство возврата результата, например "path[0].cat".
//   * Обработка - Произвольный - Описание обработки данных результата.
//   * ДополнительныеПараметры - Произвольный - Дополнительные параметры.
//   * Ошибки - Соответствие - Соответствие ошибок, например: (404, "Не найдено").
//   * КодСостояния - Число - Код состояния, возвращенный сервисом.
//   * ПараметрыСоединенияССервисом - Структура - Параметры соединения для переопределения.
//   * ДопустимоПустоеТелоОтвета - Булево - Истина означает, что пустое тело ответа не является ошибкой.
//   * ТекстОшибки - Строка - может использоваться для уточнения ошибки
//   * ПараметрыПовтора - см. НовыеПараметрыПовтора
//
Функция ОписаниеПараметровКомандыСервиса() Экспорт
	...
	ПараметрыКоманды.Вставить("ТекстОшибки", ""); // может использоваться для уточнения ошибки
	ПараметрыКоманды.Вставить("ПараметрыПовтора", НовыеПараметрыПовтора());
...
Функция ОбработатьОшибкуСервиса(ПараметрыКоманды, КодСостояния, Результат, Отказ)
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(ПараметрыКоманды.Наименование),
...
Процедура ЗарегистрироватьОшибкуСервисаПустоеТелоОтвета(ПараметрыКоманды, КодСостояния, Отказ)
	...
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(ПараметрыКоманды.Наименование),
		УровеньЖурналаРегистрации.Ошибка,,, СтрСоединить(ПодробныйТекстОшибки, Символы.ПС));
...
Функция ВыполнитьЗапросКСервису(ПараметрыКоманды, HTTPЗапрос, Соединение, Отказ)
	...
	Попытка
		...
		Если ВРег(ПараметрыКоманды.Метод) = ИнтернетСоединениеБЭД.HTTPМетоды().GET Тогда
			HTTPОтвет = Соединение.Получить(HTTPЗапрос);
		ИначеЕсли ВРег(ПараметрыКоманды.Метод) = ИнтернетСоединениеБЭД.HTTPМетоды().POST Тогда
			HTTPОтвет = Соединение.ОтправитьДляОбработки(HTTPЗапрос);
		ИначеЕсли ВРег(ПараметрыКоманды.Метод) = ИнтернетСоединениеБЭД.HTTPМетоды().PUT Тогда
			HTTPОтвет = Соединение.Записать(HTTPЗапрос);
		ИначеЕсли ВРег(ПараметрыКоманды.Метод) = ИнтернетСоединениеБЭД.HTTPМетоды().DELETE Тогда
...
Функция ЗапроситьНовыйТикетАутентификацииВСервисе(Соединение, ПараметрыСоединения, ТикетАутентификацииИТС, Отказ)
	...
	Попытка
		...
		HTTPОтвет = Соединение.ВызватьHTTPМетод(ИнтернетСоединениеБЭД.HTTPМетоды().POST, Запрос);
...
Функция ПараметрыКомандыВыгрузкаНоменклатуры() 
	...
	ПараметрыКоманды.Вставить("Метод"        , ИнтернетСоединениеБЭД.HTTPМетоды().POST);
...
Функция НовыеПараметрыПовтора()
	...
	Параметры = Новый Структура;
	Параметры.Вставить("НомерПовтора", 1);
...
Процедура ВыполнитьПаузуПовтора(ПараметрыПовтора)
	...
	ПараметрыПовтора.ВремяОжидания = ПараметрыПовтора.ВремяОжидания + Генератор.СлучайноеЧисло(, 
		ПараметрыПовтора.ВремяОжидания / ПараметрыПовтора.НомерПовтора);
	
КонецПроцедуры

Функция ВыполнитьДействиеВПовторе(Действие, ПараметрыДействия)
	
	ПараметрыПовтора = ПараметрыДействия.ПараметрыПовтора;
	...
	ИначеЕсли Действие = "ЗапросКСервисуАккредитацииВПовторе" Тогда
		Результат = ЗапросКСервисуАккредитацииВПовторе(ПараметрыДействия);
	Иначе 
		Результат = НовыйРезультатДействия();
	КонецЕсли;
	
	Если Результат.Результат = Неопределено И ВыполнятьПовтор Тогда
		ВыполнитьПаузуПовтора(ПараметрыПовтора);
		Процент = 100 * ?(ПараметрыПовтора.КоличествоПовторов = 0,
			1,
			ПараметрыПовтора.НомерПовтора / ПараметрыПовтора.КоличествоПовторов);
		Текст = СтрШаблон(НСтр("ru = 'Попытка %1 из %2'"), 
			ПараметрыПовтора.НомерПовтора,
			ПараметрыПовтора.КоличествоПовторов);
		ДлительныеОперации.СообщитьПрогресс(Процент, Текст);
		Возврат ВыполнитьДействиеВПовторе(Действие, ПараметрыДействия);
	КонецЕсли;
...
Функция ЗапросКСервису(ПараметрыКоманды, ВыполнятьПовтор)
	...
	Если ПараметрыКоманды.Аутентификация = Истина Тогда
		...
		Если ЗаголовокАутентификации.Результат = Неопределено Тогда
			ПараметрыКоманды.ТекстОшибки = ЗаголовокАутентификации.ТекстСообщения;
...
Функция ТикетАутентификацииНаПорталеПоддержки()
	...
	Если ЗначениеЗаполнено(ТикетАутентификацииИТС.КодОшибки) Тогда
		Результат.ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось подключиться к порталу интернет-поддержки по причине: %1'"), 
			ТикетАутентификацииИТС.СообщениеОбОшибке);
	Иначе 
...
Функция ЗаголовокАутентификации(Соединение, ПараметрыКоманды, ВыполнятьПовтор)
	...
	Иначе
		...
		ТипАутентификации = "";
		Если НРег(ПараметрыКоманды.ТипАутентификации) = "basic" Тогда
			ТипАутентификации = "Basic ";
		КонецЕсли;
		
		Если ПустаяСтрока(Результат.Результат) Тогда
			Результат.Результат = Неопределено;
			ВыполнятьПовтор = Ложь;
			ПараметрыКоманды.КодСостояния = 401; // Unauthorized
		Иначе
			Результат.Результат = СтрШаблон("%1%2", ТипАутентификации, СтрокаBase64БезBOM(Префикс + Результат.Результат));
		КонецЕсли;
	КонецЕсли;
...
Функция ДопустимПовторныйЗапрос(ОтветСервиса, ПараметрыКоманды);
	...
	Результат = Ложь;
	Если ОтветСервиса.КодСостояния >= 500 Тогда
		Результат = Истина;
...
Функция ПрефиксАдресаЗапроса()
	
	Возврат "accreditation/hs/api";
	
КонецФункции

Функция КодСостоянияОжидание()
...
Функция СтатусПринятоВОбработку()
	Возврат "ПринятоВОбработку";
КонецФункции

Функция СтатусНеНайдено() Экспорт
	Возврат "НеНайдено";
КонецФункции
...
Функция СтатусСервисНеДоступен() Экспорт
...
Функция ВидДокументаСервисаСертификат()
	Возврат "certificates";
КонецФункции

Функция ВидДокументаСервисаДекларация()
	Возврат "declarations";
КонецФункции

Функция ИнтервалДатыВСекундахАккредитация() Экспорт
...
// Метод-конструктор параметров запроса данных документа аккредитации - сертификата соответствия  или декларации
//  соответствия
//
// Возвращаемое значение:
//  Структура:
//   * endpoint - Строка - имя endpoint-а API
//   * number - Строка - номер документа (СС или ДС).
//   * regDate -Строка - дата регистрации в формате ГГГГММДД.
//   * endDate- Строка - дата окончания действия в формате ГГГГММДД.
//
Функция НовыеПараметрыЗапросаДокументаАккредитации()
	
	Результат = Новый Структура;
	Результат.Вставить("endpoint", "");
	Результат.Вставить("number", "");
	Результат.Вставить("regDate", "");
...
Функция ПараметрыКомандыСервисДоступен()
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Адрес = СтрШаблон("%1/ping", ПрефиксАдресаЗапроса());
	ПараметрыКоманды.Аутентификация = Истина;
	ПараметрыКоманды.ТипАутентификации = "";
	ПараметрыКоманды.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	ПараметрыКоманды.ПараметрыСоединенияССервисом = ПараметрыСоединенияССервисомАккредитации();
	ПараметрыКоманды.Обработка = "ИстинаПоКоду200";
	ПараметрыКоманды.Наименование = НСтр("ru = 'Получение данных сервиса ФСА'");
	ПараметрыКоманды.Таймаут = 5;
	ПараметрыКоманды.КодСостояния = 0;
	
	ПараметрыКоманды.ПараметрыПовтора.КоличествоПовторов = 0;
	
	Возврат ПараметрыКоманды;
	
КонецФункции

Функция ПараметрыКомандыЗапросаДанныхДокументаАккредитации(ПараметрыЗапроса)
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.КодСостояния = 400;
	ПараметрыКоманды.Адрес = СтрШаблон("%1/%2?number=%3&regDate=%4", ПрефиксАдресаЗапроса(), 
		ПараметрыЗапроса.endpoint, ПараметрыЗапроса.number, ПараметрыЗапроса.regDate);
	Если Не ПустаяСтрока(ПараметрыЗапроса.endDate) Тогда
		ПараметрыКоманды.Адрес = ПараметрыКоманды.Адрес + СтрШаблон("&endDate=%1", ПараметрыЗапроса.endDate);
	КонецЕсли;
	...
	ТипВозвращаемогоКонтента = СтрШаблон("application/vnd-1cbn-%1-v%2+json",
		ПараметрыЗапроса.endpoint,
		АктуальныйВнутреннийФормат());
	Заголовки = Новый Соответствие();
	Заголовки.Вставить(ИмяЗаголовкаТипВозвращаемогоКонтента(), ТипВозвращаемогоКонтента);
...
Функция ПараметрыСоединенияССервисомАккредитации()

	ПараметрыСоединения = Новый Структура;
	
	ПараметрыСоединения.Вставить("ИмяСервера", "fsa.1c.ru");
	ПараметрыСоединения.Вставить("Порт",       443);
	ПараметрыСоединения.Вставить("Таймаут",    30);
	ПараметрыСоединения.Вставить("Прокси",     ПолучениеФайловИзИнтернета.ПолучитьПрокси("https"));
	ПараметрыСоединения.Вставить("ЗащищенноеСоединение",
...
Функция ПараметрыПовтораЗапросовАккредитации()
	
	ПараметрыПовтора = НовыеПараметрыПовтора();
	ПараметрыПовтора.КоличествоПовторов = 10;
	ПараметрыПовтора.ВремяОжидания = 1;
	ПараметрыПовтора.ПриращениеТаймаута = 1.4;
	ПараметрыПовтора.ПриращениеСуммированием = Ложь;
	
	Возврат ПараметрыПовтора;
	
КонецФункции

Функция ЗапросКСервисуАккредитацииВПовторе(ПараметрыКоманды)
	
	ДанныеДокумента = ПараметрыКоманды.ДанныеДокумента;
	Результат = НовыйРезультатДействия();
	ПолучитьДанныеДокументаАккредитацииИзСервиса(ДанныеДокумента, ПараметрыКоманды.ПараметрыДокумента);
	Если ДанныеДокумента.Статус <> СтатусПринятоВОбработку() Тогда
		Результат.Результат = ДанныеДокумента.Статус; 
	КонецЕсли;
...
Процедура ПолучитьДанныеДокументаАккредитацииИзСервиса(ДанныеДокумента, ПараметрыДокумента)
	...
	ПараметрыКоманды     = ПараметрыКомандыЗапросаДанныхДокументаАккредитации(ПараметрыЗапроса);
	Отказ                = Ложь;
	ПодробныйТекстОшибки = НСтр("ru = 'Ошибка при выполнении команды сервиса'");
	ОтветСервиса         = ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ);
	
	Если ПараметрыКоманды.КодСостояния = КодСостоянияВыполнено() 
		И Не Отказ 
		И ТипЗнч(ОтветСервиса) = Тип("Строка")
		И Не ПустаяСтрока(ОтветСервиса) Тогда
		ЗаполнитьДанныеДокументаПоОтветуСервиса(ДанныеДокумента, ОтветСервиса, ПараметрыДокумента, Отказ);
		ПодробныйТекстОшибки = НСтр("ru = 'Ошибка при получении данных из файла'") + " " + ОтветСервиса;
	ИначеЕсли ПараметрыКоманды.КодСостояния = КодСостоянияОжидание() Тогда
		ДанныеДокумента.Статус = СтатусПринятоВОбработку();
	Иначе
		Отказ = Истина;
		ПодробныйТекстОшибки = НСтр("ru = 'Код состояния:'") + " " + ПараметрыКоманды.КодСостояния + Символы.ПС 
			+ ПолучитьСтрокуURLЗапроса(ПараметрыКоманды.СоединениеССервисом, ПараметрыКоманды.Адрес)
			+ Символы.ПС + ПараметрыКоманды.ТекстОшибки;
	КонецЕсли;
	
	Если Отказ Тогда
		ДанныеДокумента.Статус = РаботаСНоменклатуройСлужебныйКлиентСервер.СтатусОшибка();
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(ПараметрыКоманды.Наименование), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробныйТекстОшибки);
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(Наименование)
	Возврат НСтр("ru = 'Работа с номенклатурой.'", ОбщегоНазначения.КодОсновногоЯзыка())
		+ Наименование;
КонецФункции

Процедура ЗаполнитьПараметрыЗапроса(ПараметрыЗапроса, ПараметрыДокумента)
	
	ВидДокумента          = ПараметрыДокумента.ВидДокумента;
	Номер                 = ПараметрыДокумента.Номер;
	ДатаРегистрации       = ПараметрыДокумента.ДатаРегистрации;
	ДатаОкончанияДействия = ПараметрыДокумента.ДатаОкончанияДействия;
	
	Если РаботаСНоменклатуройСлужебныйКлиентСервер.ЭтоСертификат(ВидДокумента) Тогда
		ПараметрыЗапроса.endpoint = ВидДокументаСервисаСертификат();
	ИначеЕсли РаботаСНоменклатуройСлужебныйКлиентСервер.ЭтоДекларация(ВидДокумента) Тогда
		ПараметрыЗапроса.endpoint = ВидДокументаСервисаДекларация();
	КонецЕсли;
	ПараметрыЗапроса.number = Номер;
	ПараметрыЗапроса.regDate = Формат(ДатаРегистрации, "ДФ=yyyyMMdd;");
	Если ТипЗнч(ДатаОкончанияДействия) = Тип("Дата") И ЗначениеЗаполнено(ДатаОкончанияДействия) Тогда
		ПараметрыЗапроса.endDate = Формат(ДатаОкончанияДействия, "ДФ=yyyyMMdd;");
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДанныеДокументаПоОтветуСервиса(ДанныеДокумента, АдресДанных, ПараметрыДокумента, Отказ)
	
	ДанныеВАрхиве = ДвоичныеДанныеПоАдресуURL(АдресДанных);
	Если ДанныеВАрхиве = Неопределено Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	ДанныеСервиса = ДанныеАрхиваJSON(ДанныеВАрхиве);
	Если ТипЗнч(ДанныеСервиса) <> Тип("Массив") Или ДанныеСервиса.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось получить данные документа аккредитации'");
	КонецЕсли;
	
	СтатусЗапроса = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеСервиса[0], "requestStatus");
	
	Если СтатусЗапроса <> "1" Тогда
		ДанныеДокумента.Статус = СтатусНеНайдено();
	ИначеЕсли РаботаСНоменклатуройСлужебныйКлиентСервер.ЭтоСертификат(ПараметрыДокумента.ВидДокумента) Тогда
		ЗаполнитьДанныеСертификата(ДанныеДокумента.Данные, ДанныеСервиса[0]);
		ДанныеДокумента.Статус = СтатусВыполнено();
	ИначеЕсли РаботаСНоменклатуройСлужебныйКлиентСервер.ЭтоДекларация(ПараметрыДокумента.ВидДокумента) Тогда
		ЗаполнитьДанныеДекларации(ДанныеДокумента.Данные, ДанныеСервиса[0]);
		ДанныеДокумента.Статус = СтатусВыполнено();
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДанныеСертификата(Приемник, Источник)
	
	Приемник.Номер = Источник.certNumber;
	Приемник.ДатаРегистрации = XMLЗначение(Тип("Дата"), Источник.certDate);
	Приемник.Статус = Источник.certStatus;
	Приемник.ДатаПоследнегоИзменения = XMLЗначение(Тип("Дата"), Источник.certModified);
	
	Если Источник.Свойство("certExpirationDate") И Не ПустаяСтрока(Источник.certExpirationDate) Тогда
		Приемник.ДатаОкончанияДействия = XMLЗначение(Тип("Дата"), Источник.certExpirationDate);
	КонецЕсли;
	Приемник.НаименованиеЗаявителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certApplicantName",
		Приемник.НаименованиеЗаявителя);
	Приемник.ИНН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certINN",
		Приемник.ИНН);
	Приемник.ОГРН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certOGRN",
		Приемник.ОГРН);
	Приемник.НаименованиеИзготовителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certManufactureName",
		Приемник.НаименованиеИзготовителя);
	Приемник.НаименованиеПродукции = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certProductName",
		Приемник.НаименованиеПродукции);
	Приемник.Гиперссылка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certEntry",
		Приемник.Гиперссылка);
	Приемник.ТНВЭД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certTNVED",
		Приемник.ТНВЭД);
	Приемник.ОКПД2 = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certOKPD2",
		Приемник.ОКПД2);
	Приемник.ТехническиеРегламенты = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certTechReg",
		Приемник.ТехническиеРегламенты);
	Приемник.Штрихкоды = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "certProviderGTINs",
		Приемник.Штрихкоды);
	
КонецПроцедуры

Процедура ЗаполнитьДанныеДекларации(Приемник, Источник)
	
	Приемник.Номер = Источник.declNumber;
	Приемник.ДатаРегистрации = XMLЗначение(Тип("Дата"), Источник.declDate);
	Приемник.Статус = Источник.declStatus;
	Приемник.ДатаПоследнегоИзменения = XMLЗначение(Тип("Дата"), Источник.declModified);
	
	Если Источник.Свойство("declExpirationDate") И Не ПустаяСтрока(Источник.declExpirationDate) Тогда
		Приемник.ДатаОкончанияДействия = XMLЗначение(Тип("Дата"), Источник.declExpirationDate);
	КонецЕсли;
	
	Приемник.НаименованиеЗаявителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declApplicantName",
		Приемник.НаименованиеЗаявителя);
	Приемник.ИНН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declINN",
		Приемник.ИНН);
	Приемник.ОГРН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declOGRN",
		Приемник.ОГРН);
	Приемник.НаименованиеИзготовителя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declManufactureName",
		Приемник.НаименованиеИзготовителя);
	Приемник.НаименованиеПродукции = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declProductName",
		Приемник.НаименованиеПродукции);
	Приемник.Гиперссылка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declEntry",
		Приемник.Гиперссылка);
	Приемник.ТНВЭД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declTNVED",
		Приемник.ТНВЭД);
	Приемник.ОКПД2 = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declOKPD2",
		Приемник.ОКПД2);
	Приемник.ТехническиеРегламенты = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declTechReg",
		Приемник.ТехническиеРегламенты);
	Приемник.Штрихкоды = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник, "declProviderGTINs",
		Приемник.Штрихкоды);
	
КонецПроцедуры

Функция ПолучитьСтрокуURLЗапроса(Соединение, Адрес)
	
	Порт = Соединение.Порт;
	ЗащищенноеСоединение = Соединение.ЗащищенноеСоединение;
	СтрокаПорт = ?(Не ЗначениеЗаполнено(Порт) Или Порт = 80 Или Порт = 443, "", ":" + Формат(Порт, "ЧГ="));
	Результат = ?(ЗащищенноеСоединение = Неопределено Или ЗащищенноеСоединение = Ложь, "http://", "https://")
		+ Соединение.Сервер + СтрокаПорт + ?(Лев(Адрес, 1) = "/", "", "/") + Адрес;
