...
Функция ИндексКартинкиПоВидуУпаковкиИВидуПродукции(ВидУпаковки, ВидПродукции) Экспорт
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.СоковаяПродукция Тогда
...
Функция ДополнитьДеревоУпаковокДаннымиТоваров(ДеревоУпаковок, ИсточникДанных, ИменаКолонокДляЗаполнения, СообщениеJSON)
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			...
			КонецЦикла;
		Иначе
			ЕстьОшибки = ЕстьОшибки Или ДополнитьДеревоУпаковокДаннымиТоваров(СтрокаДерева, ИсточникДанных, ИменаКолонокДляЗаполнения, СообщениеJSON);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ЕстьОшибки Тогда
		...
		Если ДанныеШапки.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
	...
	Если ОперацияПодтверждениеПриемки Тогда
		...
		Иначе
			...
			Если Не ЕстьОшибки
				...
				ЕстьОшибки = ДополнитьДеревоУпаковокДаннымиТоваров(ДеревоУпаковокПоДаннымДокумента, Товары, ИменаКолонокДляЗаполнения, СообщениеJSON);
...
Процедура ЗаполнитьРезультатПроверкиПриемкиТоваровИСМППриОСУ(РезультатПроверки, Товары, ШтрихкодыУпаковок, ПараметрыСканирования)
	...
	Для Каждого СтрокаТаблицы Из Товары Цикл
		...
		Если СтрокиПоEAN.Количество() = 1
			...
			СтрокаJSON.weight = СтрокаТаблицы.Количество * 1000; // Указывается общий вес всей позиции в граммах
		КонецЕсли;
...
Функция СообщениеМожноЗагружать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости = Неопределено, ДатаСледующейИтерации = Неопределено)
	...
	Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.СтанцияУправленияЗаказами Тогда
		...
		Если НастройкаОбменаСУЗ <> Неопределено Тогда
			...
			ВремяОжидания = ОбщегоНазначенияИС.ВремяОжиданияСледующегоЗапросаСУЗ(НастройкаОбменаСУЗ);
			Если ВремяОжидания > 0 Тогда
