...
Функция ОпределитьИдентификаторЗаказаНаОплату(
	...
	Иначе
		...
		ИсторическиеПараметрыНастройкиПодключения = СистемаБыстрыхПлатежейСлужебный.ПараметрыНастройкиПодключения(
			АнализИсторическихДанных.НастройкаПодключения);
...
Процедура ОбработатьСтатусыСтатусыОплат(ДанныеОпераций, ОбработанныеОперации)
	...
	Для Каждого ОтложеннаяОперация Из ДанныеОпераций Цикл
		...
		НастройкиПодключения.Добавить(ОтложеннаяОперация.НастройкаПодключения);
		
	КонецЦикла;
	
	ПараметрыНастроекПодключения = СистемаБыстрыхПлатежейСлужебный.ПараметрыНастроекПодключения(
	...
	Для Каждого ОтложеннаяОперация Из ДанныеОпераций Цикл
		...
		РезультатПроверки = ОпределитьСтатусОплаты(
			ОтложеннаяОперация,
			ПараметрыНастроекПодключения.Получить(ОтложеннаяОперация.НастройкаПодключения),
			ИдентификаторОперацииПроверкиДлительнойОплаты(),
			Ложь,
			"Идентификатор");
