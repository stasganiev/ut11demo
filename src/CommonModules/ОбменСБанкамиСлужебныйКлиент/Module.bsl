...
Процедура ОбработатьРезультатОтправкиДокументовПоНастройкеСбербанк(РезультатОтправки, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.МассивСообщенийТребующихПодтверждение.Количество() Тогда
		...
		Если НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) >= 
			ОбменСБанкамиСлужебныйКлиентСервер.ДатаОтключенияПодписанияSMSСбербанк() Тогда
			// С 01.06.2023 Сбербанк отключил возможность подписания по sms, поэтому всегда считаем,
			// что отправлять документы без подписи.
			РеквизитыНастройкиОбмена.ОтправлятьДокументыБезПодписиSMS = Истина;
		КонецЕсли;
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		...
		Иначе
			...
			Иначе
				...
				ЭлементКоллекцииОшибка = НовыйЭлементКоллекцииОшибка();
				...
				ДополнитьЭлементКоллекцииОшибка(
					ЭлементКоллекцииОшибка, ДополнительныеПараметры.ОповещениеПослеБазовойАутентификации.ДополнительныеПараметры, 
						ДополнительныеПараметры.НастройкаОбмена);
				...
				Если НЕ ОповещениеПослеОтправки = Неопределено Тогда	
				...
				Иначе
					...
					ПоказатьОшибкуСетевогоДоступа(КоллекцияОшибок, 
...
Процедура ПослеПолученияВыписки(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Для Каждого СообщениеПользователю Из Результат.СообщенияПользователю Цикл
			ФормаВладелец = Неопределено;
			Если ДополнительныеПараметры.Свойство("ФормаВладелец", ФормаВладелец)
				И ТипЗнч(ДополнительныеПараметры.ФормаВладелец) = Тип("ФормаКлиентскогоПриложения") Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СообщениеПользователю.Текст;
				Сообщение.ИдентификаторНазначения = ДополнительныеПараметры.ФормаВладелец.УникальныйИдентификатор;
				Сообщение.Сообщить();
			Иначе
...
Процедура ОтправкаСообщенийОбмена(Параметры)
	...
	Если НЕ Параметры.Свойство("КоллекцияОшибок") Тогда
		Параметры.Вставить("КоллекцияОшибок", Новый Массив);
	КонецЕсли;
	...
	Если НЕ Параметры.Свойство("СоотвНастроекОбменаИСтруктурСертификатов") Тогда
...
Процедура ПолучитьМаркерБанкаПоЛогинуИПаролю(
	...
	Если Результат.ТребуетсяSMSАвторизация Тогда
		...
		ОписаниеОповещенияОдноразовыйПароль = Новый ОписаниеОповещения("ОтправитьОдноразовыйПарольSMSВБанк",
			ЭтотОбъект, Параметры);
		ОткрытьФорму("Обработка.ОбменСБанками.Форма.ЗапросОдноразовогоПароля", ПараметрыФормы, , , , ,
			ОписаниеОповещенияОдноразовыйПароль);
	Иначе
		Если ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ТипОшибкиИнтернетСоединения) Тогда
				ОбработатьОшибку(ВидОперации, Результат.ТекстОшибки,,НастройкаОбмена);
			...
			Иначе
				ЭлементКоллекцииОшибка = НовыйЭлементКоллекцииОшибка(); 	
				ЭлементКоллекцииОшибка.КраткоеПредставлениеОшибки = Результат.КраткоеПредставлениеОшибки;
				ЭлементКоллекцииОшибка.ТипОшибки = 	ТипОшибкиИнтернетСоединения;                             
				ЭлементКоллекцииОшибка.НастройкаОбмена = НастройкаОбмена;
				
				ДополнитьЭлементКоллекцииОшибка(ЭлементКоллекцииОшибка, Обработчик.ДополнительныеПараметры, НастройкаОбмена);
															
				Если Обработчик.ИмяПроцедуры = "ОтправитьСообщениеВБанкПослеПолученияМаркера" Тогда
					// При отправке сообщений. В этом сценарии вызов происходит в псевдоцикле для каждой учетной записи.
					// В этом случае накапливаем ошибки, и покажем позже все сразу, по окончании всего процесса отправки.
					Обработчик.ДополнительныеПараметры.КоллекцияОшибок.Добавить(ЭлементКоллекцииОшибка);
					ВыполнитьОбработкуОповещения(Обработчик, Результат.ИдентификаторСессии);
				Иначе 
					// Для остальных случаев, когда ошибка должна быть показана сразу.
					КоллекцияОшибок = Новый Массив;
					КоллекцияОшибок.Добавить(ЭлементКоллекцииОшибка);
					ПоказатьОшибкуСетевогоДоступа(КоллекцияОшибок, Обработчик, Результат.ИдентификаторСессии);
				КонецЕсли;
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершитьОтправку", ЭтотОбъект, Параметры);
	Если НЕ Параметры.КоллекцияОшибок.Количество() Тогда
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	Иначе
		ПоказатьОшибкуСетевогоДоступа(Параметры.КоллекцияОшибок, ОписаниеОповещения);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗавершитьОтправку(Результат, Параметры)Экспорт
	
	Оповестить("ОбновитьСостояниеОбменСБанками");
	
	// Определим, нет ли запроса на отзыв ЭД. Если есть, то он уже подписан, а другие документы подписывать по SMS в
	// этот момент уже не надо, так как они уже отправлены и подписаны.
	ЗапросНаОтзывЭД = НайтиЗапросНаОтзыв(Параметры);
	
	Если НЕ ЗначениеЗаполнено(ЗапросНаОтзывЭД) Тогда
		
		НастройкаОбмена = Неопределено;
		Параметры.Свойство("НастройкаОбмена", НастройкаОбмена);
		МассивСсылокНаОбъект = Неопределено;
		Параметры.Свойство("МассивСсылокНаОбъект", МассивСсылокНаОбъект);
		ДополнительныеПараметры = ОбменСБанкамиСлужебныйВызовСервера.ПодготовитьПараметрыКПодписаниюПоSMS(
			МассивСсылокНаОбъект, НастройкаОбмена);
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Параметры, ДополнительныеПараметры, Истина);
		СообщенияОбменаКПодписаниюКодомИзSMS = Неопределено;
		Если Параметры.Свойство("СообщенияОбменаКПодписаниюКодомИзSMS", СообщенияОбменаКПодписаниюКодомИзSMS)
			И СообщенияОбменаКПодписаниюКодомИзSMS <> Неопределено 
			И ТипЗнч(СообщенияОбменаКПодписаниюКодомИзSMS) = Тип("Массив") Тогда
			
			Если СообщенияОбменаКПодписаниюКодомИзSMS.Количество() <> 0 Тогда
				
				ПодписатьДокументыКодомИзSMS(Неопределено, Параметры);
...
Функция НайтиЗапросНаОтзыв(Параметры)
	
	ЗапросНаОтзывЭД = Неопределено;
	Если НЕ Параметры.Свойство("СообщениеОбмена", ЗапросНаОтзывЭД)
		ИЛИ НЕ ЗначениеЗаполнено(ЗапросНаОтзывЭД) Тогда
		
		Возврат Неопределено;
		
	КонецЕсли;
	
	Если ОбменСБанкамиСлужебныйВызовСервера.ЭтоЗапросНаОтзывЭД(ЗапросНаОтзывЭД) Тогда
		
		Возврат ЗапросНаОтзывЭД;
...
Процедура ОкноОповещенияОбменСБанками(ДополнительныеПараметры)
	
	ОкноОповещенияОбОперацияхКлючУникальности = Неопределено;
	ДополнительныеПараметры.Свойство("ОкноОповещенияОбОперацияхКлючУникальности", ОкноОповещенияОбОперацияхКлючУникальности);
	ОкноОповещенияОбОперацияхДействие = Неопределено;
	ДополнительныеПараметры.Свойство("ОкноОповещенияОбОперацияхДействие", ОкноОповещенияОбОперацияхДействие);
	ОкноОповещенияОбОперацияхЕстьОшибка = Неопределено;
	ДополнительныеПараметры.Свойство("ОкноОповещенияОбОперацияхЕстьОшибка", ОкноОповещенияОбОперацияхЕстьОшибка);
	
	// Если есть ошибка при выполнении фоновых заданий, то оповестим об этом окно обработки 
	// ОбменСБанками.ОповещенияОбОперациях и выйдем из процедуры
	Если ОкноОповещенияОбОперацияхЕстьОшибка = Истина Тогда
		Оповестить("ОбменСБанкамиОкноОповещенияОбОперацияхЕстьОшибка", ОкноОповещенияОбОперацияхКлючУникальности);
		Возврат;
	КонецЕсли;
	
	// Если нужно открыть окно, то явным образом откроем окно.
	// Если нужно зарыть окно, то пошлем оповещение окну обработки ОбменСБанками.ОповещенияОбОперациях,
	// и оно само себя закроет (т.к. для формы, открытой в режиме "РежимОткрытияОкнаФормы.БлокироватьОкноВладельца",
	// не выполняется поиск по уникальному идентификатору
	Если ОкноОповещенияОбОперацияхДействие = "Открыть" Тогда
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("ТекстСообщения", ДополнительныеПараметры.ОкноОповещенияОбОперацияхТекстСообщения);
		ОткрытьФорму("Обработка.ОбменСБанками.Форма.ОповещенияОбОперациях", ПараметрыФормы, , ОкноОповещенияОбОперацияхКлючУникальности);
	Иначе
		Оповестить("ОбменСБанкамиОкноОповещенияОбОперациях", ОкноОповещенияОбОперацияхКлючУникальности);
	КонецЕсли;
...
Процедура ПоказатьОшибкуСетевогоДоступа(КоллекцияОшибок, ОбработкаОповещения, ПараметрОповещенияЗакрытия = Неопределено)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("КоллекцияОшибок", КоллекцияОшибок);
	Если ПараметрОповещенияЗакрытия <> Неопределено Тогда
		ПараметрыФормы.Вставить("ПараметрОповещенияЗакрытия", ПараметрОповещенияЗакрытия);
	КонецЕсли;

	ОткрытьФорму("Обработка.ОбменСБанками.Форма.ОшибкаСетевогоДоступа",ПараметрыФормы,,,,,ОбработкаОповещения);
...
Процедура ПослеОтправкиПлатежныхДокументовВБанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		НужноВыполнитьОбработкуОповещения = Истина;
		...
		КонецЕсли;
		ДополнительныеПараметрыОповещение = Неопределено;
		Если Оповещение <> Неопределено Тогда
			
			ДополнительныеПараметрыОповещение = ?(ЗначениеЗаполнено(Оповещение.ДополнительныеПараметры),
				Оповещение.ДополнительныеПараметры, Неопределено);
			
		КонецЕсли;
		Если ДополнительныеПараметрыОповещение <> Неопределено Тогда
			
			НастройкаОбмена = Неопределено;
			ДополнительныеПараметрыОповещение.Свойство("НастройкаОбмена", НастройкаОбмена);
			
		КонецЕсли;
		
		ЭтоАсинхронныйОбмен = Ложь;
		Если НастройкаОбмена <> Неопределено Тогда  
			...
			ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(
				НастройкаОбмена, РеквизитыНастройкиОбмена);
			ЭтоАсинхронныйОбмен = ?(
				РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен"),
				Истина,
				ЭтоАсинхронныйОбмен);
			
		КонецЕсли;
		
		Если ЭтоАсинхронныйОбмен Тогда
			
			СообщениеОбмена = Неопределено;
			ДополнительныеПараметрыОповещение.Свойство("СообщениеОбмена", СообщениеОбмена);
			МассивСсылокНаОбъект = Неопределено;
			ДополнительныеПараметрыОповещение.Свойство("МассивСсылокНаОбъект", МассивСсылокНаОбъект);
			ДополнительныеПараметрыПодписаниеПоSMS = ОбменСБанкамиСлужебныйВызовСервера.ВозможноПодписаниеПоSMS(
				РезультатОперации, МассивСсылокНаОбъект, ДополнительныеПараметры.Оповещение.ИмяПроцедуры, СообщениеОбмена);
			Если ДополнительныеПараметрыПодписаниеПоSMS.ВозможноПодписаниеПоSMS Тогда
				
				НужноВыполнитьОбработкуОповещения = Ложь;
				ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
					ДополнительныеПараметрыОповещение, ДополнительныеПараметрыПодписаниеПоSMS, Истина);
				ПодписатьДокументыКодомИзSMS(Неопределено, ДополнительныеПараметрыОповещение);
		...
		Если НужноВыполнитьОбработкуОповещения Тогда
...
//
// Параметры:
// СсылкаНаПисьмо - ДокументСсылка.ПисьмоОбменСбанками - пересылаемое письмо
//
Процедура ОткрытьФормуПересылкиЭлектронногоПисьма(СсылкаНаПисьмо) Экспорт 
	
	ПараметрыОткрытия = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыПересылкиНовогоЭлектронногоПисьма(СсылкаНаПисьмо);  		
	ОбменСБанкамиКлиентПереопределяемый.ОткрытьНовоеЭлектронноеПисьмо(ПараметрыОткрытия);	
...
//
Процедура ОбработатьОшибку(ВидОперации, Знач ПодробныйТекстОшибки, Знач ТекстСообщения = "", СсылкаНаОбъект = Неопределено) Экспорт
...
Функция НовыйЭлементКоллекцииОшибка()
...
Функция ДополнитьЭлементКоллекцииОшибка(ЭлементКоллекцииОшибка, Источник, НастройкаОбмена)
...
//
// Возвращаемое значение:
//  Булево, Неопределено - принимает следующие значения:
...
#Область ПодтверждениеДокументовКодомИзSMS

// Заполняет необходымые параметры для запуска процесса подписания документов кодом из SMS
//
// Параметры:
// 	НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
// 	МассивДокументов - Массив Из ДокументСсылка.СообщениеОбменСБанками - массив документов, которые необходимо подписать.
// 	ДополнительныеПараметры - Структура:
// 		* НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
// 		* СообщенияОбменаВСтатусеНеПодтвержден - Массив Из ДокументСсылка.СообщениеОбменСБанками - массив документов
// 			в стаусе "НеПодтвержден"
// 		* СообщенияОбменаДоСтатусаНеПодтвержден - Массив Из ДокументСсылка.СообщениеОбменСБанками - массив документов
// 			до стауса "НеПодтвержден"
// 		* СообщенияОбменаВСтатусеПодтвержден - Массив Из ДокументСсылка.СообщениеОбменСБанками - массив документов
// 			в стаусе "Подтвержден"
...
//
Процедура ПодготовитьОкружениеИПодписатьДокументыКодомИзSMS(НастройкаОбмена, МассивДокументов = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ДополнительныеПараметры = Неопределено Тогда 
		
		ДополнительныеПараметры = Новый Структура();
		ДополнительныеПараметры.Вставить("НастройкаОбмена", НастройкаОбмена);
		ДополнительныеПараметры.Вставить("СообщенияОбменаВСтатусеНеПодтвержден", МассивДокументов);
		ДополнительныеПараметры.Вставить("СообщенияОбменаДоСтатусаНеПодтвержден", Новый Массив);
		ДополнительныеПараметры.Вставить("СообщенияОбменаВСтатусеПодтвержден", Новый Массив);
		ДополнительныеПараметры.Вставить("ДокументыОтклоненныеБанком", Новый Массив);
		
	КонецЕсли;
	Если НЕ ДополнительныеПараметры.Свойство("РеквизитыНастройкиОбмена") Тогда
		
		РеквизитыНастройкиОбмена = РеквизитыНастройкиОбмена();
		ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(НастройкаОбмена, РеквизитыНастройкиОбмена);
		ДополнительныеПараметры.Вставить("РеквизитыНастройкиОбмена", РеквизитыНастройкиОбмена);
		
	КонецЕсли;
	Оповещение = Новый ОписаниеОповещения("ОтправитьЗапросКодаИзSMSВБанк", ЭтотОбъект, ДополнительныеПараметры);
	ПараметрыАвторизации = Неопределено;
	ИдентификаторСессии = Неопределено;
	ДополнительныеПараметры.Свойство("ИдентификаторСессии", ИдентификаторСессии);
	Если ИдентификаторСессии <> Неопределено Тогда
		
		ОповещениеОтправитьЗапросКодаИзSMS = Новый ОписаниеОповещения("ОтправитьЗапросКодаИзSMSВБанкПослеПолученияМаркера", ЭтотОбъект, ДополнительныеПараметры);
		ВыполнитьОбработкуОповещения(ОповещениеОтправитьЗапросКодаИзSMS, ИдентификаторСессии);
		
	Иначе
		
		Если ПолученыДанныеАвторизации(НастройкаОбмена, ПараметрыАвторизации) Тогда
			
			ВыполнитьОбработкуОповещения(Оповещение, ПараметрыАвторизации);
...
Процедура ОтправитьЗапросКодаИзSMSВБанк(ДанныеАутентификации, ДополнительныеПараметры) Экспорт

	Если ДанныеАутентификации = Неопределено Тогда
		
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
		
	КонецЕсли;
	ОкноОповещенияОбОперацияхДействие = Неопределено;
	ДополнительныеПараметры.Свойство("ОкноОповещенияОбОперацияхДействие", ОкноОповещенияОбОперацияхДействие);
	Если ОкноОповещенияОбОперацияхДействие = Неопределено Тогда
		
		ДополнительныеПараметры.Вставить("ОкноОповещенияОбОперацияхТекстСообщения", НСтр("ru = 'Ожидание документов из банка для подписания по SMS'"));
		...
		ОкноОповещенияОбменСБанками(ДополнительныеПараметры);
		
	КонецЕсли;
	РеквизитыНастройкиОбмена = ДополнительныеПараметры.РеквизитыНастройкиОбмена;
	Обработчик = Новый ОписаниеОповещения(
		"ОтправитьЗапросКодаИзSMSВБанкПослеПолученияМаркера", ЭтотОбъект, ДополнительныеПараметры);
	ПолучитьМаркерБанкаПоЛогинуИПаролю(Обработчик, РеквизитыНастройкиОбмена.АдресСервера,
		РеквизитыНастройкиОбмена.ИдентификаторОрганизации, ДанныеАутентификации,
		РеквизитыНастройкиОбмена.ВерсияФормата, ДополнительныеПараметры.НастройкаОбмена);
	
КонецПроцедуры

Процедура ОтправитьЗапросКодаИзSMSВБанкПослеПолученияМаркера(Маркер, ДополнительныеПараметры) Экспорт
	...
	ПараметрыАутентификации = Новый Структура;
	ПараметрыАутентификации.Вставить("ИдентификаторСессии", Маркер);
	СоотвНастроекОбменаИСтруктурСертификатов = Новый Соответствие();
	СоотвНастроекОбменаИСтруктурСертификатов.Вставить(ДополнительныеПараметры.НастройкаОбмена, ПараметрыАутентификации);
	ДополнительныеПараметры.Вставить("СоотвНастроекОбменаИСтруктурСертификатов", СоотвНастроекОбменаИСтруктурСертификатов);
	ДополнительныеПараметры.Вставить("ВидОперации", НСтр("ru = 'Формирование GenSMSSign'")); 
	УникальныйИдентификаторЗадания = Новый УникальныйИдентификатор();
	
	Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияГенерацииЗапросаПодписанияПоSMS(ДополнительныеПараметры, УникальныйИдентификаторЗадания); 
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	Оповещение = Новый ОписаниеОповещения("ПослеГенерацииЗапросаПодписанияПоSMS", ЭтотОбъект, ДополнительныеПараметры);

	ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
	
КонецПроцедуры

// Запускает каскад процедур для подписания платежных документов с помошью кода из SMS
//
// Параметры:
// 	Результат - Неопределено - фактически всегда пустой параметр, иногда используемый для вызова из ОписаниеОповещения
// 	ДополнительныеПараметры - Структура:
// 		* НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
// 		* СообщенияОбменаВСтатусеНеПодтвержден - Массив Из ДокументСсылка.СообщениеОбменСБанками - массив документов
// 			в стаусе "НеПодтвержден"
...
// 		* ИдентификаторСессии - Строка - идентификатор установленной сессии.   
//
Процедура ПодписатьДокументыКодомИзSMS(Результат, ДополнительныеПараметры) Экспорт
	
	// Этапы:
	//  1 - формируем запросы текущих состояний документов, отправляем в банк
	//  2 - дожидаемся когда все документы станут В Статусе = НеПодтвержден
	//  3 - Формируем запрос "GenSMSSign"
	//  4 - Отправляем в банк "GenSMSSign", получаем назад из Банка "GenSMSSignResponse" - синхронный метод
	//  5 - Открываем форму показа страницы банка для ввода SMS-кода. После успешного ввода SMS-кода,
	//       в банке должны подписаться документы
	//  6 - Отправляем запрос в банк о новых документах для 1С и получаем их из банка
	//  7 - Ждем когда в 1С вернутся новые статусы, что документы подписаны/частично подписаны (т.е. приняты банком)
	//  8 - Закрываем форму показа страницы подписания банка
	
	ВыполняетсяЗапросСтатусов = Ложь;
	
	// Если есть документы, у которых должен обновится статус, то запросим их статус
	Если ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Количество() <> 0 Тогда
		...
		Если НЕ ДополнительныеПараметры.Свойство("ОкноОповещенияОбОперацияхДействие") Тогда
			...
			ДополнительныеПараметры.Вставить("ОкноОповещенияОбОперацияхТекстСообщения",
				НСтр("ru = 'Ожидание документов из банка для подписания по SMS'"));
			ДополнительныеПараметры.Вставить("ОкноОповещенияОбОперацияхКлючУникальности", Новый УникальныйИдентификатор());
			ДополнительныеПараметры.Вставить("ОкноОповещенияОбОперацияхДействие", "Открыть");
			ОкноОповещенияОбменСБанками(ДополнительныеПараметры);
		...
		СоотвНастроекОбменаИСтруктурСертификатов = Неопределено;
		ДополнительныеПараметры.Свойство("СоотвНастроекОбменаИСтруктурСертификатов", СоотвНастроекОбменаИСтруктурСертификатов);
		Если СоотвНастроекОбменаИСтруктурСертификатов <> Неопределено Тогда
			
			СтруктураПараметров = СоотвНастроекОбменаИСтруктурСертификатов.Получить(ДополнительныеПараметры.НастройкаОбмена);
			Если СтруктураПараметров <> Неопределено Тогда  
				
				ИдентификаторСессии = СтруктураПараметров.ИдентификаторСессии;
				ДополнительныеПараметры.Вставить("ИдентификаторСессии", ИдентификаторСессии);
				ДополнительныеПараметры.Вставить("ВидОперации", Нстр("ru = 'Формирование запросов состояний документов'"));	
				ДополнительныеПараметры.Вставить("СообщенияОбменаДокументы",
					ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден);
				УникальныйИдентификаторЗадания = Новый УникальныйИдентификатор(); 
				
				Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияСформироватьЗапросыСостоянийЭД(
					ДополнительныеПараметры, УникальныйИдентификаторЗадания);
				
				ИдентификаторЗадания = Результат.ИдентификаторЗадания;
				Оповестить("ОбменСБанкамиИдентификаторДлительнойОперации", ИдентификаторЗадания);
				
				ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
				ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
				Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатФормированияЗапросовСтатусовЭД", ЭтотОбъект,
					ДополнительныеПараметры);
				ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
				ВыполняетсяЗапросСтатусов = Истина; 
	...
	// Если все нужные документы перешли в статус "НеПодтвержден" то продолжим подписание по SMS
	Если ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Количество() = 0
		И ДополнительныеПараметры.СообщенияОбменаВСтатусеНеПодтвержден.Количество() <> 0
		И НЕ ВыполняетсяЗапросСтатусов Тогда
		
		Если НЕ ДополнительныеПараметры.Свойство("РеквизитыНастройкиОбмена") Тогда
			
			РеквизитыНастройкиОбмена = РеквизитыНастройкиОбмена();
			ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(ДополнительныеПараметры.НастройкаОбмена, 
				РеквизитыНастройкиОбмена);
			ДополнительныеПараметры.Вставить("РеквизитыНастройкиОбмена", РеквизитыНастройкиОбмена);
			
		КонецЕсли;
		ПодготовитьОкружениеИПодписатьДокументыКодомИзSMS(ДополнительныеПараметры.НастройкаОбмена, ,
			ДополнительныеПараметры);
		
	КонецЕсли; 
		
	// Если есть документы отклоненные банком, то выведем их пользователю и закроем окно фонового задания.
	ВывестиСообщениеОбОтклоненныхДокументах(ДополнительныеПараметры);
	
КонецПроцедуры

Функция РеквизитыНастройкиОбмена()
...
Процедура ВывестиСообщениеОбОтклоненныхДокументах(ДополнительныеПараметры) Экспорт
	
	Если НЕ ДополнительныеПараметры.Свойство("ДокументыОтклоненныеБанком") Тогда
		Возврат;
	КонецЕсли;
	
	Если ДополнительныеПараметры.ДокументыОтклоненныеБанком.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Документ Из ДополнительныеПараметры.ДокументыОтклоненныеБанком Цикл
		МассивСообщений.Добавить(Строка(Документ));
	КонецЦикла;
	
	ТекстСообщения = СтрСоединить(МассивСообщений, Символы.ПС);
	ТекстСообщения = СокрЛП(ТекстСообщения); 
	
	ДополнительныеПараметры.Вставить("ОкноОповещенияОбОперацияхЕстьОшибка", Истина);
	ОкноОповещенияОбменСБанками(ДополнительныеПараметры);
	
	ШаблонТекста = НСтр("ru = 'Данные документы были отклонены банком: 
							 |%1'");
	ТекстСообщения = СтрШаблон(ШаблонТекста, ТекстСообщения);
	ВидОперации = НСтр("ru = 'Подписание платежных документов по коду из SMS-сообщения'");
	ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения, ДополнительныеПараметры.НастройкаОбмена);
	
	ДополнительныеПараметры.ДокументыОтклоненныеБанком.Очистить();
	
КонецПроцедуры

Процедура ОбработатьРезультатФормированияЗапросовСтатусовЭД(РезультатЗадания, ДополнительныеПараметры) Экспорт
	
	Если РезультатЗадания = Неопределено Тогда // задание было отменено
		
		ВидОперации = НСтр("ru = 'Формирование запросов состояний электронных документов.'");
		ОбработатьОшибку(ВидОперации, НСтр("ru = 'Задание было отменено'"), НСтр("ru = 'Задание было отменено'"),
			ДополнительныеПараметры.НастройкаОбмена);
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
	...
	Иначе // выполнено
		...
		ЗапросыСостоянийЭД = ПолучитьИзВременногоХранилища(РезультатЗадания.АдресРезультата);
		ДополнительныеПараметры.Вставить("ЗапросыСостоянийЭД", ЗапросыСостоянийЭД);
		ДополнительныеПараметры.Вставить("ВидОперации", НСтр("ru = 'Отправка запросов состояний электронных документов'"));
		УникальныйИдентификаторЗадания = Новый УникальныйИдентификатор();
		
		Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияОтправитьЗапросыОСостоянияхЭД(ДополнительныеПараметры, УникальныйИдентификаторЗадания);
		...
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатОтправкиЗапросовСтатусовЭД", ЭтотОбъект, ДополнительныеПараметры);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
...
Процедура ОбработатьРезультатОтправкиЗапросовСтатусовЭД(РезультатЗадания, ДополнительныеПараметры) Экспорт
	
	Если РезультатЗадания = Неопределено Тогда // задание было отменено
		
		ВидОперации = НСтр("ru = 'Отправка запросов состояний электронных документов.'");
		ОбработатьОшибку(ВидОперации, НСтр("ru = 'Задание было отменено'"), НСтр("ru = 'Задание было отменено'"),
			ДополнительныеПараметры.НастройкаОбмена);
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
	...
	Иначе // выполнено
		...
		Если ВыполнитьПовторнуюАутентификацию(Результат, ДополнительныеПараметры) Тогда 
		...
		ДополнительныеПараметры.Вставить("ВидОперации",
			Нстр("ru = 'Получение извещений о состояниях электронных документов'"));
		
		Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПолученияИзвещениийОСостоянийЭД(
			ДополнительныеПараметры, УникальныйИдентификаторЗадания);  
		...
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		Оповещение = Новый ОписаниеОповещения("ПослеПолученияИзвещенийОСостоянияхЭД", ЭтотОбъект, ДополнительныеПараметры);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
...
Процедура ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры)
...
Процедура ПослеПолученияИзвещенийОСостоянияхЭД(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = Неопределено Тогда
		
		ВидОперации = НСтр("ru = 'Получение извещений о состояниях документов из банка'");
		ОбработатьОшибку(ВидОперации, НСтр("ru = 'Задание было отменено'"), НСтр("ru = 'Задание было отменено'"),
			ДополнительныеПараметры.НастройкаОбмена);
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
	...
	Если ВыполнитьПовторнуюАутентификацию(СтруктураВозврата, ДополнительныеПараметры) Тогда
	...
	Для Каждого ЭлементСоответствия Из СтруктураВозврата.ПараметрОповещения Цикл
	...
	// Дополним массив документами, которые ожадют изменения статуса, но не вернулись из фонового задания.
	// Пока известно, что это может быть "Запрос на отзыв ЭД".
	ЗапросНаОтзывЭД = Неопределено;
	Если ДополнительныеПараметры.Свойство("ЗапросНаОтзывЭД", ЗапросНаОтзывЭД) Тогда
		
		МассивДополнительныхДокументов = Новый Массив;
		МассивДополнительныхДокументов.Добавить(ЗапросНаОтзывЭД);
		РеквизитыДополнительныхЭлектронныхДокументов = ОбменСБанкамиСлужебныйВызовСервера.РеквизитыЭлектронныхДокументов(
			МассивДополнительныхДокументов, Ложь);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РеквизитыЭлектронныхДокументов, РеквизитыДополнительныхЭлектронныхДокументов);
		
	КонецЕсли;
	
	Для Каждого РеквизитЭлектронногоДокумента Из РеквизитыЭлектронныхДокументов Цикл 
		
		Если РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ТребуетсяПодтверждение") Тогда
			
			ИндексДокумента = ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Найти(РеквизитЭлектронногоДокумента.СообщениеОбмена);
			Если ИндексДокумента <> Неопределено Тогда
				
				ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Удалить(ИндексДокумента);
				ДополнительныеПараметры.СообщенияОбменаВСтатусеНеПодтвержден.Добавить(РеквизитЭлектронногоДокумента.СообщениеОбмена);
				
			КонецЕсли;
			
		ИначеЕсли РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ОжидаетсяИсполнение")
			ИЛИ РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ЧастичноПодтвержден")
			ИЛИ РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ОжидаетсяВыписка") 
			ИЛИ РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ПлатежИсполнен") Тогда
			
			ИндексДокумента = ДополнительныеПараметры.СообщенияОбменаВСтатусеНеПодтвержден.Найти(РеквизитЭлектронногоДокумента.СообщениеОбмена);
			Если ИндексДокумента <> Неопределено Тогда
				
				ДополнительныеПараметры.СообщенияОбменаВСтатусеНеПодтвержден.Удалить(ИндексДокумента);
				ДополнительныеПараметры.СообщенияОбменаВСтатусеПодтвержден.Добавить(РеквизитЭлектронногоДокумента.СообщениеОбмена);
				
			КонецЕсли;
			
		// Если документ отклонен банком, то исключаем его из списка
		// и потом выведем список отклоненных документов в ПодписатьДокументыКодомИзSMS()
		ИначеЕсли РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.Отклонен")
			ИЛИ РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.Аннулирован")
			ИЛИ РеквизитЭлектронногоДокумента.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ОшибкаПередачи") Тогда
			
			ИндексДокумента = ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Найти(РеквизитЭлектронногоДокумента.СообщениеОбмена);
			Если ИндексДокумента <> Неопределено Тогда
				
				ДополнительныеПараметры.СообщенияОбменаДоСтатусаНеПодтвержден.Удалить(ИндексДокумента);
				Если ДополнительныеПараметры.ДокументыОтклоненныеБанком.Найти(РеквизитЭлектронногоДокумента.СсылкаНаОбъект) = Неопределено Тогда
					ДополнительныеПараметры.ДокументыОтклоненныеБанком.Добавить(РеквизитЭлектронногоДокумента.СсылкаНаОбъект);
				КонецЕсли;
	...
	// Попробуем разорвать стек вызовов, во избежания переполнения, которое возможно при длительном ожидании
	// новых статусов из банка. Для этого передадим управление в форму "Обработки.ОбменСБанками.ОповещенияОбОперациях",
	// если она открыта, там вызовем обработчик ожидания. И затем вернем процесс сюда в процедуру 
	// "ПодписатьДокументыКодомИзSMS". Если форма по каким-то причинам не будет найдена, то напрямую вызовем процедуру
	// "ПодписатьДокументыКодомИзSMS"
	Результат = Неопределено;
	ОкноОповещенияОбОперациях = ОкноОповещенияОбОперациях(ДополнительныеПараметры.ОкноОповещенияОбОперацияхКлючУникальности);
	Если ОкноОповещенияОбОперациях <> Неопределено Тогда
		
		ПродолжитьВызов = Новый ОписаниеОповещения(
			"ПодписатьДокументыКодомИзSMS_ПродолжитьВызов", ОкноОповещенияОбОперациях, ДополнительныеПараметры);
		ВыполнитьОбработкуОповещения(ПродолжитьВызов, Результат);
		
	Иначе
		
		ПодписатьДокументыКодомИзSMS = Новый ОписаниеОповещения(
			"ПодписатьДокументыКодомИзSMS", ЭтотОбъект, ДополнительныеПараметры);
...
Функция ОкноОповещенияОбОперациях(КлючУникальности)
	
	ОкноОповещенияОбОперациях = Неопределено;
	Окна = ПолучитьОкна();
	
	Если Окна = Неопределено Тогда
		Возврат ОкноОповещенияОбОперациях;
	КонецЕсли;
	...
	Для Каждого Окно Из Окна Цикл
		Если ТипЗнч(Окно) = Тип("ОкноКлиентскогоПриложения") 
			И Окно.Содержимое.Количество() = 1
			И Окно.Содержимое[0].ИмяФормы = "Обработка.ОбменСБанками.Форма.ОповещенияОбОперациях" 
			И Окно.Содержимое[0].КлючУникальности = КлючУникальности Тогда
			
			ОкноОповещенияОбОперациях = Окно.Содержимое[0]; 
			Прервать;
...
Процедура ПослеГенерацииЗапросаПодписанияПоSMS(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда // задание было отменено
		
		ВидОперации = НСтр("ru = 'Формирование GenSMSSign.'");
		ОбработатьОшибку(ВидОперации, НСтр("ru = 'Задание было отменено'"), НСтр("ru = 'Задание было отменено'"),
			ДополнительныеПараметры.НастройкаОбмена);
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
	...
	Иначе // выполнено
		...
		УникальныйИдентификаторЗадания = Новый УникальныйИдентификатор();
		ФайлGenSMSSign = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		ДополнительныеПараметры.Вставить("ФайлGenSMSSign", ФайлGenSMSSign);
		// Для отправки данных методом ОбменСБанкамиСлужебный.ОтправитьВБанк (будет использован в
		//  ОбменСБанкамиСлужебный.ОтправитьЗапросИПолучитьСсылкуДляПодписанияПоSMS)
		//  нужно использовать Соответствие "ИдентификаторыСессий". Подготовим его.
		ИдентификаторыСессий = Новый Соответствие;
		ИдентификаторСессииСтруктура = Новый Структура("ИдентификаторСессии", ДополнительныеПараметры.ИдентификаторСессии);
		ИдентификаторыСессий.Вставить(ДополнительныеПараметры.НастройкаОбмена, ИдентификаторСессииСтруктура);
		ДополнительныеПараметры.Вставить("ИдентификаторыСессий", ИдентификаторыСессий);
		ДополнительныеПараметры.Вставить("ВидОперации", НСтр("ru = 'Получение GenSMSSignResponse'"));
		РезультатЗадания = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияОтправкиЗапросаИПолученияСсылкиДляПодписанияПоSMS(
			ДополнительныеПараметры, УникальныйИдентификаторЗадания);
		...
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		Оповещение = Новый ОписаниеОповещения("ПослеПолученияСсылкиДляПодписанияПоSMS", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ПослеПолученияСсылкиДляПодписанияПоSMS(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда // задание было отменено
		
		ВидОперации = НСтр("ru = 'Получение GenSMSSignResponse.'");
		ОбработатьОшибку(ВидОперации, НСтр("ru = 'Задание было отменено'"), НСтр("ru = 'Задание было отменено'"),
			ДополнительныеПараметры.НастройкаОбмена);
		ЗакрытьОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
	...
	Иначе // выполнено
		...
		ПараметрыФормы = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		
		Если ВыполнитьПовторнуюАутентификацию(ПараметрыФормы, ДополнительныеПараметры) Тогда
		...
		ПараметрыФормы.Вставить("НастройкаОбмена", ДополнительныеПараметры.НастройкаОбмена);
		ПараметрыФормы.Вставить("ИдентификаторСессии", ДополнительныеПараметры.ИдентификаторСессии);
		ПараметрыФормы.Вставить("СообщенияОбменаВСтатусеНеПодтвержден", ДополнительныеПараметры.СообщенияОбменаВСтатусеНеПодтвержден);
		Если ДополнительныеПараметры.Свойство("ИмяПроцедурыПослеПодтвержденияКодомИзSMS") Тогда
			ПараметрыФормы.Вставить("ИмяПроцедурыПослеПодтвержденияКодомИзSMS", ДополнительныеПараметры.ИмяПроцедурыПослеПодтвержденияКодомИзSMS);
			ПараметрыФормы.Вставить("ДополнительныеПараметрыФоновыхЗаданий", ДополнительныеПараметры);
		КонецЕсли;
		Если ДополнительныеПараметры.Свойство("РезультатОперацииПослеПодтвержденияКодомИзSMS") Тогда
			ПараметрыФормы.Вставить("РезультатОперацииПослеПодтвержденияКодомИзSMS", ДополнительныеПараметры.РезультатОперацииПослеПодтвержденияКодомИзSMS);
		КонецЕсли;
		
		ОткрытьФорму(
			"Обработка.ОбменСБанками.Форма.ВводКодаИзSMS",
			ПараметрыФормы,
			,
			КлючУникальности,
			,
...
Процедура ОбработатьПолучениеДанныхАутентификации(ДанныеАутентификации, ДополнительныеПараметры) Экспорт

	Если ДанныеАутентификации = Неопределено Тогда
		
		ВидОперации = ДополнительныеПараметры.ВидОперации;
		ПредставлениеОшибки = Нстр("ru = 'Не удалось получить данные авторизации'");
		ОбработатьОшибку(ВидОперации, ПредставлениеОшибки, ПредставлениеОшибки, ДополнительныеПараметры.НастройкаОбмена);
	...
	ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(
		ДополнительныеПараметры.НастройкаОбмена, РеквизитыНастройкиОбмена);
	ОбработчикПослеАутентификации = Неопределено;
	ДополнительныеПараметры.Свойство("ОбработчикПослеАутентификации", ОбработчикПослеАутентификации);
	Если ОбработчикПослеАутентификации = Неопределено Тогда
		ОбработчикПослеАутентификации = Новый ОписаниеОповещения("ПродолжитьВзаимодействиеПослеПолученияМаркераБанка", ЭтотОбъект, ДополнительныеПараметры);
	КонецЕсли;
	
	ПолучитьМаркерБанкаПоЛогинуИПаролю(ОбработчикПослеАутентификации, РеквизитыНастройкиОбмена.АдресСервера,
...
Процедура ПродолжитьВзаимодействиеПослеПолученияМаркераБанка(Маркер, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИдентификаторСессии", Маркер);
	Если ДополнительныеПараметры.ВидОперации = НСтр("ru = 'Отправка запросов состояний электронных документов'") Тогда
		
		УникальныйИдентификаторЗадания = Новый УникальныйИдентификатор();
		Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияОтправитьЗапросыОСостоянияхЭД(ДополнительныеПараметры,
		...
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
		Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатОтправкиЗапросовСтатусовЭД", ЭтотОбъект,
			ДополнительныеПараметры);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
		
	КонецЕсли;
	Если ДополнительныеПараметры.ВидОперации = НСтр("ru = 'Получение извещений о состояниях электронных документов'") Тогда
	...
	КонецЕсли;
	Если ДополнительныеПараметры.ВидОперации = НСтр("ru = 'Получение GenSMSSignResponse'") Тогда
...
Функция ВыполнитьПовторнуюАутентификацию(РезультатФоновогоЗадания, ДополнительныеПараметры) Экспорт
	
	ТребуетсяПовторнаяАутентификация = ТребуетсяПовторнаяАутентификация(РезультатФоновогоЗадания);
	
	Если ТребуетсяПовторнаяАутентификация Тогда
		
		ПараметрыАвторизации = Новый Структура;
		Если ПолученыДанныеАвторизации(ДополнительныеПараметры.НастройкаОбмена, ПараметрыАвторизации) Тогда
			
			ОбработатьПолучениеДанныхАутентификации(ПараметрыАвторизации, ДополнительныеПараметры);
			
		Иначе 
			
			Оповещение = Новый ОписаниеОповещения(
				"ОбработатьПолучениеДанныхАутентификации", ЭтотОбъект, ДополнительныеПараметры);
			ПолучитьДанныеАутентификации(Оповещение, ДополнительныеПараметры.НастройкаОбмена);
	...
	Возврат ТребуетсяПовторнаяАутентификация;
	
КонецФункции

Функция ТребуетсяПовторнаяАутентификация(РезультатФоновогоЗадания)
	
	ПолученныйТекстОшибки = "";
	РезультатФоновогоЗадания.Свойство("ТекстОшибки", ПолученныйТекстОшибки);
	Если ЗначениеЗаполнено(ПолученныйТекстОшибки) Тогда
		
		ТекстОшибки = Сред(ПолученныйТекстОшибки, 1);
		ТекстОшибки = Нрег(СтрЗаменить(ТекстОшибки, " ",""));
		Если СтрНайти(ТекстОшибки, "кодошибки1006") Тогда
	...
	КонецЕсли;
	СвойствоТребуетсяПовторнаяАутентификация = Неопределено;
	РезультатФоновогоЗадания.Свойство("ТребуетсяПовторнаяАутентификация", СвойствоТребуетсяПовторнаяАутентификация);
	Если СвойствоТребуетсяПовторнаяАутентификация = Истина Тогда
