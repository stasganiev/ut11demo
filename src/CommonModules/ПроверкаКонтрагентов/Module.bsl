...
Процедура ПроверитьКонтрагента(Параметры, АдресХранилища) Экспорт 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		НоваяСтрока.ОбластьДанныхВспомогательныеДанные = ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса();
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Параметры);
	
	ПараметрыПроверки = Новый Структура(
		"ЭтоПроверкаПоКнопке, ВыполнятьПредварительнуюПроверкуКонтрагента",
		Ложь, Ложь);
	
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, Параметры);
	
	ДополнительныеПараметры = Новый Структура(
		"СохранятьРезультатСразуПослеПроверки,
			|ВыполнятьПредварительнуюПроверкуКонтрагента,
			|ЭтоПроверкаПоКнопке",
		Ложь,
		Ложь,
		Ложь);
...
Процедура ПроверитьВебСервисомФНС(ДанныеКонтрагентов, ДополнительныеПараметры) Экспорт
	...
	СлужебнаяПроверитьВебСервисомФНС(ДанныеКонтрагентов, ДополнительныеПараметры, Истина);
...
Процедура ВыполнитьПроверкуКонтрагентов(
	...
	Знач ВидПроверки = "Смешанный", 
	Знач ДополнительныеПараметры = Неопределено) Экспорт
	
	// Проверка корректности переданного параметра.
	Если ВидПроверки <> "ИзКэша"
		И ВидПроверки <> "АктуальныеДанные"
		И ВидПроверки <> "Смешанный" Тогда
	...
	Если ПроверяемыеКонтрагенты.Колонки.Найти("Состояние") = Неопределено Тогда
		ПроверяемыеКонтрагенты.Колонки.Добавить(
			"Состояние",
	...
	ПараметрыПроверки = Новый Структура("ЭтоПроверкаДокумента", Ложь);
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ДополнительныеПараметры);
	
	Если ПараметрыПроверки.ЭтоПроверкаДокумента Тогда
		ЗаполнитьСостоянияВДокументе(ПроверяемыеКонтрагенты, ДанныеКонтрагентов);
	Иначе
...
Процедура ПроверитьКонтрагентовВДокументеФоновоеЗадание(Форма, ПараметрыФоновогоЗадания) Экспорт
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		ПараметрыЗапуска = Новый Структура(
			"СохранятьРезультатСразуПослеПроверки",
			Ложь);
		ЗаполнитьЗначенияСвойств(ПараметрыЗапуска, ПараметрыФоновогоЗадания);
...
Процедура ОтобразитьРезультатПроверкиКонтрагентаВДокументе(Форма) Экспорт
	...
	ОтобразитьРезультатПроверкиКонтрагентаВДокументеСлужебная(Форма);
...
Функция ОписаниеДокументов(Форма) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоДокументСКонтрагентомВШапке Тогда
		...
		Для Каждого СтрокаОписания Из Описание Цикл
			...
			СтрокаОписания.ПричинаОтключенияПроверкиФНС =
				Форма.РеквизитыПроверкиКонтрагентов.ПричинаОтключенияПроверкиФНС;
		КонецЦикла;
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоДокументСоСчетомФактуройВПодвале Тогда
		...
		// Дополняем описание состоянием.
		Для Каждого СтрокаОписания Из Описание Цикл
			СтрокаОписания.СостояниеКонтрагента =
				Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагентовВСчетеФактуре;
			СтрокаОписания.ПричинаОтключенияПроверкиФНС =
				Форма.РеквизитыПроверкиКонтрагентов.ПричиныОтключенияПроверкиФНСВСчетеФактуре;
		КонецЦикла;
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоСчетФактура Тогда
		...
		Если Форма.РеквизитыПроверкиКонтрагентов <> Неопределено Тогда
			Для Каждого СтрокаОписания Из Описание Цикл
				...
				СтрокаОписания.ПричинаОтключенияПроверкиФНС = Форма.РеквизитыПроверкиКонтрагентов[
					ИмяРеквизитаПричиныОтключенияПроверкиФНСВСчетФактуре(ИмяРеквизитаСчетаФактуры)];
...
Функция ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеКонтрагентов, Отбор = Неопределено) Экспорт
	...
	Если ДанныеПоКонтрагентам.Количество() = 1 Тогда
		...
		ПричинаОтключенияПроверкиФНС = ДанныеПоКонтрагенту.ПричинаОтключенияПроверкиФНС;
		КонтрагентовНесколько        = Ложь;
	...
	Иначе
		...
		ПричиныОтключенияПроверкиФНС        = Новый Массив();
		...
		Для каждого ДанныеПоКонтрагенту Из ДанныеПоКонтрагентам Цикл
			...
			ИначеЕсли ДанныеПоКонтрагенту.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентНеПодлежитПроверке") Тогда
				...
				Если ЗначениеЗаполнено(ДанныеПоКонтрагенту.ПричинаОтключенияПроверкиФНС)
					И ПричиныОтключенияПроверкиФНС.Найти(ДанныеПоКонтрагенту.ПричинаОтключенияПроверкиФНС) = Неопределено Тогда
					ПричиныОтключенияПроверкиФНС.Добавить(ДанныеПоКонтрагенту.ПричинаОтключенияПроверкиФНС);
				КонецЕсли;
		...
		ИначеЕсли ЕстьНедействующиеКонтрагенты Тогда
			СостояниеКонтрагента =
				ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентОтсутствуетВБазеФНС");
		ИначеЕсли ЕстьКонтрагентыСОшибками Тогда
			СостояниеКонтрагента =
				ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных");
		ИначеЕсли ЕстьКонтрагентыНеПодлежащиеПроверке Тогда
			СостояниеКонтрагента         =
				ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентНеПодлежитПроверке");
			ПричинаОтключенияПроверкиФНС = СтрСоединить(ПричиныОтключенияПроверкиФНС, ", ");
		ИначеЕсли ЕстьКонтрагентыСПустымИННКПП Тогда
			СостояниеКонтрагента =
				ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.ПустойИННИлиКПП");
		Иначе
			СостояниеКонтрагента =
				ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентЕстьВБазеФНС");
		КонецЕсли;
	...
	Результат.Вставить("ПричинаОтключенияПроверкиФНС", ПричинаОтключенияПроверкиФНС);
...
Процедура ПередФормированиемОтчета(Форма) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется Тогда
		...
		// Есть вероятность получения данных из кэша, поэтому перед началом
		// формирования отчета проверка доступности сервисе не выполняется.
...
Процедура ЗапомнитьРезультатФормированияОтчета(Форма, РезультатВыполнения, АдресХранилища = Неопределено) Экспорт
	...
	РезультатПроверки = Новый Структура("ДанныеДляПроверкиКонтрагентов");
	ЗаполнитьЗначенияСвойств(РезультатПроверки, РезультатВыполнения);
	
	Если Не Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется
		Или РезультатПроверки.ДанныеДляПроверкиКонтрагентов = Неопределено Тогда
			Возврат;
	КонецЕсли;
	
	ДанныеДляПроверкиКонтрагентов = РезультатПроверки.ДанныеДляПроверкиКонтрагентов;
	
	ОтчетПроверкиКонтрагентов = Новый Структура("ПроверкаКонтрагентовАдресДанныхОтчета");
	ЗаполнитьЗначенияСвойств(ОтчетПроверкиКонтрагентов, ДанныеДляПроверкиКонтрагентов);
	
	Форма.РеквизитыПроверкиКонтрагентов.АдресДанныхОтчета =
		ОтчетПроверкиКонтрагентов.ПроверкаКонтрагентовАдресДанныхОтчета;
	...
	Форма.РеквизитыПроверкиКонтрагентов.НедействующиеКонтрагентыКоличество =
		ДанныеДляПроверкиКонтрагентов.НедействующиеКонтрагенты.Количество();
	Форма.РеквизитыПроверкиКонтрагентов.КонтрагентыСПустымСостояниемКоличество =
		Форма.РеквизитыПроверкиКонтрагентов.НедействующиеКонтрагентыКоличество;
		
	Если Не Форма.РеквизитыПроверкиКонтрагентов.ОтчетСРазделами Тогда
		Форма.ЗначениеВРеквизитФормы(
			ДанныеДляПроверкиКонтрагентов.НедействующиеКонтрагенты,
			"ПроверкаКонтрагентовНедействующиеКонтрагенты");
	КонецЕсли;
...
Процедура ОтобразитьРезультатПроверкиКонтрагентаВОтчете(Форма, ПолеТабличногоДокумента, РезультатФормированияОтчета, ТекущийНомерРаздела = Неопределено) Экспорт
	...
	Если РезультатПроверки = Неопределено Или РезультатПроверки.Свойство("НетДоступаКВебСервисуФНС") Тогда
...
Процедура ПриСозданииНаСервереКонтрагент(Форма, Параметры) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется Тогда
		...
		ПроверкаНеТребуется      =
			СостояниеКонтрагента = Перечисления.СостоянияСуществованияКонтрагента.КонтрагентНеПодлежитПроверке;
		ЭтоИностранныйКонтрагент = Форма.РеквизитыПроверкиКонтрагентов.ЭтоИностранныйКонтрагент;
		ОтключитьПроверкуФНС     = Форма.РеквизитыПроверкиКонтрагентов.ОтключитьПроверкуФНС;
		
		Если (ЭтоИностранныйКонтрагент Или ОтключитьПроверкуФНС) И ПроверкаНеТребуется 
			Или Не ЭтоИностранныйКонтрагент И Не ОтключитьПроверкуФНС И ЗначениеЗаполнено(СостояниеКонтрагента) Тогда
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Попытка
		...
		Запрос.Текст = 
			...
			|					КОГДА &ЭтоМодельСервисов
			|							И СостоянияКонтрагентов.Состояние = &КонтрагентНеПодлежитПроверке
			|						ТОГДА СостоянияКонтрагентов.ПричинаОтключения
			|					ИНАЧЕ """"
			|				КОНЕЦ
			|		ИНАЧЕ ВЫБОР
			|				КОГДА СостоянияКонтрагентов.Контрагент В (&Контрагенты)
			...
			|							ИНАЧЕ """"
			|						КОНЕЦ
			|				ИНАЧЕ СостоянияКонтрагентов.ПричинаОтключения
			|			КОНЕЦ
			|	КОНЕЦ КАК ПричинаОтключения,
			|	СостоянияКонтрагентов.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанныхВспомогательныеДанные,
			|	СостоянияКонтрагентов.ИНН КАК ИНН,
			|	СостоянияКонтрагентов.КПП КАК КПП
			|ИЗ
...
// Результат проверки доступа к сервисам 1С:Контрагенты.
//
// Возвращаемое значение:
//  Структура - информация о результатах проверки:
//    * ПроверкаДоступаКСервисуФНС - Строка - содержит ошибки при проверке доступа к сервису ФНС,
//      пустая строка, если проверка подключения успешна.
//    * ПроверкаДоступаСервиса1СКонтрагент - Строка - содержит ошибки при проверке доступа к сервису 1С:Контрагент,
//      пустая строка, если проверка подключения успешна.
//    * ПроверкаДоступаСервиса1СКонтрагентREST - Строка - содержит ошибки при проверке доступа 
//      к сервису 1С:Контрагент (REST), пустая строка, если проверка подключения успешна.
//
Функция ПроверкаДоступностиСервисов() Экспорт
	
	Результат = Новый Структура;
	
	// Проверка доступа к сервису Проверка контрагентов ФНС
	ДлительныеОперации.СообщитьПрогресс(
		0,
		НСтр("ru = 'Проверка наличия доступа к веб-сервису ФНС.'")); 
		
	ОписаниеОшибкиФНС = ПроверкаКонтрагентовФНС.ПроверитьДоступностьСервиса();
	Если Не ПустаяСтрока(ОписаниеОшибкиФНС) Тогда
		ОписаниеОшибкиФНС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проверка доступа к сервису ""Проверка контрагентов ФНС"".
					|%1'"),
				ОписаниеОшибкиФНС);
	КонецЕсли;
	
	Результат.Вставить("ПроверкаДоступаКСервисуФНС", ОписаниеОшибкиФНС);
	
	// Проверка доступа к сервису 1С:Контрагент
	МассивОшибокСервисаКонтрагент = Новый Массив;
	
	ДлительныеОперации.СообщитьПрогресс(
		33,
		НСтр("ru = 'Проверка наличия доступа к веб-сервису 1С:Контрагент.'"));
	ОписаниеОшибкиSOAP = "";
	РаботаСКонтрагентами.ПроверитьДоступностьСервисаКонтрагент(ОписаниеОшибкиSOAP);
	Если Не ПустаяСтрока(ОписаниеОшибкиSOAP) Тогда
		МассивОшибокСервисаКонтрагент.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проверка доступа к сервису ""1С:Контрагент"".
					|%1'"),
				ОписаниеОшибкиSOAP));
	КонецЕсли;
	
		
	// Проверка доступа к сервису 1С:Контрагент(REST)
	ДлительныеОперации.СообщитьПрогресс(
		66,
		НСтр("ru = 'Проверка наличия доступа к веб-сервису 1С:Контрагент (REST).'"));
	
	ОписаниеОшибкиREST = "";
	URLОперацииPing = РаботаСКонтрагентами.URLRESTОперацииPing();
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		ОписаниеОшибкиREST = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		МассивОшибокСервисаКонтрагент.Добавить(
			"Проверка доступа к сервису ""1С:Контрагент (REST)""" + Символы.ПС + ОписаниеОшибкиREST);
	КонецЕсли;
	
	Результат.Вставить(
		"ПроверкаДоступаСервиса1СКонтрагент",
		СтрСоединить(МассивОшибокСервисаКонтрагент, Символы.ПС + Символы.ПС));
	
	ДлительныеОперации.СообщитьПрогресс(
		100,
		НСтр("ru = 'Обработка результатов проверки.'")); 
...
// Определяет области данных, которые используются и в которых неактуальные состояния контрагентов.
//
// Возвращаемое значение:
//  Массив - содержит номера областей, которые хранят неактуальные состояния контрагентов.
//
Функция ОбластиДанныхПроверкиСостояний()
	
	ОбластиДанных = Новый Массив;
	ДатаОбновления = НачалоДня(ТекущаяУниверсальнаяДата());
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатаОбновленияСостоянийКонтрагентов КАК ДатаОбновленияСостоянийКонтрагентов
		|		ПО ОбластиДанных.ОбластьДанныхВспомогательныеДанные = ДатаОбновленияСостоянийКонтрагентов.ОбластьДанныхВспомогательныеДанные
		|ГДЕ
		|	ОбластиДанных.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбластейДанных.Используется)
		|	И ЕСТЬNULL(НАЧАЛОПЕРИОДА(ДатаОбновленияСостоянийКонтрагентов.ДатаОбновления, ДЕНЬ), ДАТАВРЕМЯ(1, 1, 1)) <> &ДатаОбновления
		|
		|УПОРЯДОЧИТЬ ПО
		|	ОбластьДанных";
...
//
Процедура ВыполнитьПроверкуКонтрагентовРегламентноеЗадание(ОбластьДанных = Неопределено)
	
	ДатаОбновленияСостояний = ТекущаяУниверсальнаяДата();
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	СостоянияКонтрагентов.Контрагент КАК Контрагент,
	...
	|	И СостоянияКонтрагентов.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных";
	...
	СлужебнаяПроверитьВебСервисомФНС(ДанныеКонтрагентов, ДополнительныеПараметры, Ложь);
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		УстановитьДатуОбновленияСостоянийКонтрагентов(ДатаОбновленияСостояний, ОбластьДанных);
	КонецЕсли;
...
//		в режиме ЭтоПроверкаСправочника = Истина, который показывает, что предварительная проверка ИНН, КПП
//		и Даты уже была выполнена ранее и здесь проверку можно пропустить.
Процедура СлужебнаяПроверитьВебСервисомФНС(ДанныеКонтрагентов, ДополнительныеПараметры, ОбновитьКэш)
	
	БратьСостоянияТолькоИзКэша = (ДополнительныеПараметры.Свойство("ИзКэша")
		И ДополнительныеПараметры.ИзКэша);
	
	// Перед выполнением запроса к сервису нет необходимости проверять доступность,
	// т.к. существует вероятность получения данных из кэша.
	ДополнительныеПараметры.Вставить("ЕстьДоступКВебСервисуФНС",  Истина);
	ДополнительныеПараметры.Вставить("ТребуетсяПроверкаСервисом", Истина);
	
	// Проверяем данные в контрагентах, справочниках, документах предварительными проверками 
	// даже при отсутствующем доступе к веб-сервису и выполняем приведение данных к нужному формату,
	// выявление записей с ошибками.
	ПодготовитьДанныеИВыполнитьПредварительнуюПроверку(ДанныеКонтрагентов, ДополнительныеПараметры);
	
	Если НЕ БратьСостоянияТолькоИзКэша Тогда
		ПроверкаКонтрагентовФНС.СостоянияКонтрагентовФНС(
			ДанныеКонтрагентов,
			ОбновитьКэш,
			ДополнительныеПараметры.ЕстьДоступКВебСервисуФНС);
	КонецЕсли;
...
// Устанавливает новую дату обновления состояний контрагентов.
//
// Параметры:
//  ДатаОбновления - Дата - дата обновлений состояний контрагента
//  ОбластьДанных - Число - номер области данных, по которой устанавливается дата.
//
Процедура УстановитьДатуОбновленияСостоянийКонтрагентов(ДатаОбновления, ОбластьДанных)
	
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат;
	КонецЕсли;
	
	НаборЗаписей = РегистрыСведений.ДатаОбновленияСостоянийКонтрагентов.СоздатьНаборЗаписей();
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Попытка
		...
		Запись.ПричинаОтключения = ДанныеКонтрагента.ПричинаОтключенияПроверкиФНС;
...
Функция ЭтоДокументСОшибкамиПроверкиКонтрагентов(ДокументСсылка)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ДокументыСОшибкамиПроверкиКонтрагентов.Документ КАК Документ
		|ИЗ
...
Процедура ПодготовитьДанныеИВыполнитьПредварительнуюПроверку(ДанныеКонтрагентовДляПроверки, ДополнительныеПараметры)
...
Процедура ДополнитьДанныеКонтрагентов(ДанныеКонтрагентовДляПроверки, ДанныеКонтрагентов)
	...
	ЕстьКолонкаЭтоЮридическоеЛицо 		=
		ДанныеКонтрагентовДляПроверки.Колонки.Найти("ЭтоЮридическоеЛицо") <> Неопределено;
	ЕстьКолонкаЭтоИностранныйКонтрагент =
		ДанныеКонтрагентовДляПроверки.Колонки.Найти("ЭтоИностранныйКонтрагент") <> Неопределено;
	...
	Если ДанныеКонтрагентов.Количество() = 1
		...
		И ЗначениеЗаполнено(ДанныеКонтрагентов[0].ЭтоИностранныйКонтрагент)
		И ДанныеКонтрагентов[0].ОтключитьПроверкуФНС = Истина Тогда
	...
	Иначе
		...
		Если СтандартнаяОбработка Тогда
			...
			Для каждого ДанныеКонтрагента Из ДанныеКонтрагентов Цикл
				...
				Если ДанныеКонтрагентовДляПроверки.Колонки.Найти("ОтключитьПроверкуФНС") <> Неопределено
					И Не ЗначениеЗаполнено(ДанныеКонтрагента.ОтключитьПроверкуФНС) Тогда
					РаботаСКонтрагентамиПереопределяемый.ПриОпределенииСтатусаПроверкиКонтрагентаВБазеФНС(
						ДанныеКонтрагента,
						ДанныеКонтрагента.ОтключитьПроверкуФНС,
						ДанныеКонтрагента.ПричинаОтключенияПроверкиФНС);
				КонецЕсли;
...
Процедура СвернутьДанныеКонтрагентов(ДанныеКонтрагентов, ДополнительныеКолонкиГруппировок = "")
	...
	КолонкиГруппировок =
		"Контрагент, ИНН, КПП, Дата, Состояние, ЭтоЮридическоеЛицо, ЭтоИностранныйКонтрагент, ОтключитьПроверкуФНС, ПричинаОтключенияПроверкиФНС"
...
Процедура ОставитьРазрешенныхКонтрагентов(ДанныеКонтрагентов)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Запрос.Текст = 
			...
			|	ДанныеКонтрагентов.ОтключитьПроверкуФНС,
			|	ДанныеКонтрагентов.ПричинаОтключенияПроверкиФНС,
			...
			|	ВсеДанныеКонтрагентов.ОтключитьПроверкуФНС,
			|	ВсеДанныеКонтрагентов.ПричинаОтключенияПроверкиФНС,
...
Процедура ВыполнитьПредварительнуюПроверкуДанныхПоКаждомуКонтрагенту(ДанныеКонтрагента, ДополнительныеПараметрыПроверки = Неопределено)
	...
	Если ДанныеКонтрагента.ЭтоИностранныйКонтрагент = Истина
		Или ДанныеКонтрагента.ОтключитьПроверкуФНС = Истина Тогда // Может равняться Неопределено.
	...
	Иначе
		...
		ПроверкаКонтрагентовКлиентСервер.НайденыОшибкиВДанных(
			ДанныеКонтрагента,
			ТекущаяДатаСеанса(),
...
Процедура ВзятьСостоянияИзКэша(ДанныеКонтрагентов, ДополнительныеПараметрыПроверки)
	...
	Если ДополнительныеПараметрыПроверки <> Неопределено
		...
		Если ЭтоПроверкаСправочника
			...
			ПолучитьСостоянияКонтрагентовИзКэша(
				ДанныеКонтрагентов,
				ДополнительныеПараметрыПроверки,
				ПолучатьВсеСостоянияИзРегистра,
				ЭтоПроверкаДокумента);
...
Процедура ПолучитьСостоянияКонтрагентовИзКэша(
		ДанныеКонтрагентов,
		ДополнительныеПараметрыПроверки,
		ПолучатьВсеСостоянияИзРегистра,
		ЭтоПроверкаДокумента)
	...
	Если ЭтоПроверкаДокумента Тогда
		...
		// Сначала будет проверены состояния в РС КэшСостоянийКонтрагентовФНС
		// для контрагентов, по которым не удалось получить состояния, будет дополнительно
		// выполнена проверка в РС СостоянияКонтрагентов.
		...
		Запрос.Текст = 
			...
			|	&ДанныеКонтрагентов КАК ДанныеКонтрагентов
			|ГДЕ
			|	ДанныеКонтрагентов.Дата >= &ГраницаАктуальности
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	КэшСостоянийКонтрагентовФНС.ПричинаОтключения КАК ПричинаОтключенияПроверкиФНС
			|ИЗ
			|	РегистрСведений.КэшСостоянийКонтрагентовФНС КАК КэшСостоянийКонтрагентовФНС
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеКонтрагентов КАК ДанныеКонтрагентов
			|		ПО КэшСостоянийКонтрагентовФНС.ИНН = ДанныеКонтрагентов.ИНН
			|			И КэшСостоянийКонтрагентовФНС.КПП = ДанныеКонтрагентов.КПП
			|			И КэшСостоянийКонтрагентовФНС.ДатаПроверки = ДанныеКонтрагентов.Дата
			|			И (КэшСостоянийКонтрагентовФНС.Проверен)
			|			И КэшСостоянийКонтрагентовФНС.ОбластьДанныхВспомогательныеДанные = ДанныеКонтрагентов.ОбластьДанныхВспомогательныеДанные";
		
		Запрос.УстановитьПараметр("ДанныеКонтрагентов",  ДанныеКонтрагентов);
		Запрос.УстановитьПараметр("ГраницаАктуальности", ПроверкаКонтрагентовФНС.ГраницаАктуальностиКэшаСостояний());
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ОтборСтрок.Вставить("ИНН",  ВыборкаДетальныеЗаписи.ИНН);
			ОтборСтрок.Вставить("КПП",  ВыборкаДетальныеЗаписи.КПП);
			ОтборСтрок.Вставить("Дата", ВыборкаДетальныеЗаписи.Дата);
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				ОтборСтрок.Вставить("ОбластьДанныхВспомогательныеДанные", ВыборкаДетальныеЗаписи.ОбластьДанныхВспомогательныеДанные);
			КонецЕсли;
			
			НайденныеСтроки = ДанныеКонтрагентов.НайтиСтроки(ОтборСтрок);
			Для Каждого ОписаниеКонтрагента Из НайденныеСтроки Цикл
				ОписаниеКонтрагента.Состояние = ВыборкаДетальныеЗаписи.Состояние;
				ОписаниеКонтрагента.ПричинаОтключенияПроверкиФНС = ВыборкаДетальныеЗаписи.ПричинаОтключенияПроверкиФНС;
			КонецЦикла;
		КонецЦикла;
		
		ОтборСтрок = Новый Структура;
		ОтборСтрок.Вставить("Состояние", Перечисления.СостоянияСуществованияКонтрагента.ПустаяСсылка());
		НайденныеСтроки = ДанныеКонтрагентов.НайтиСтроки(ОтборСтрок);
		
		Если НайденныеСтроки.Количество() = 0 Тогда
			// Вся информация получена из кэша состояний,
			// выполнять вызов сервиса ФНС не требуется.
			ДополнительныеПараметрыПроверки.ТребуетсяПроверкаСервисом = Ложь;
			Возврат;
		КонецЕсли;
		
		ДанныеКонтрагентовТекущиеДанные = ДанныеКонтрагентов.СкопироватьКолонки();
		Для каждого ОписаниеКонтрагента Из НайденныеСтроки Цикл
			ЗаполнитьЗначенияСвойств(
				ДанныеКонтрагентовТекущиеДанные.Добавить(),
				ОписаниеКонтрагента);
			ДанныеКонтрагентов.Удалить(ОписаниеКонтрагента); // Данные будут получены из РС СостоянияКонтрагентов.
		КонецЦикла;
		
		ПолучитьСостоянияКонтрагентовИзКэшаТекущееСостояние(
			ДанныеКонтрагентовТекущиеДанные,
			ПолучатьВсеСостоянияИзРегистра);
...
Процедура ПолучитьСостоянияКонтрагентовИзКэшаТекущееСостояние(ДанныеКонтрагентов, ПолучатьВсеСостоянияИзРегистра)
	...
	Запрос.Текст =	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              	...
	              	|	ВЫБОР
					|		КОГДА ДанныеКонтрагентов.ПричинаОтключенияПроверкиФНС = """"
					|			ТОГДА СостоянияКонтрагентов.ПричинаОтключения
					|		ИНАЧЕ ДанныеКонтрагентов.ПричинаОтключенияПроверкиФНС
...
Процедура ДобавитьТекстПодзапросаНепроверенныеКонтрагенты(
	...
	ЧастиЗапроса.Добавить("ЛОЖЬ" + ?(ЭтоПерваяТаблица, " КАК ОтключитьПроверкуФНС,", ","));
...
Процедура ЗаполнитьСостоянияВДокументе(ПроверяемыеКонтрагенты, ДанныеКонтрагентов)
	...
	ДанныеКонтрагентов.Индексы.Добавить("Контрагент, ИНН, КПП");
	...
	Если ПроверяемыеКонтрагенты.Колонки.Найти("ПричинаОтключенияПроверкиФНС") <> Неопределено Тогда
		ПроверяемыеКонтрагенты.ЗаполнитьЗначения("", "ПричинаОтключенияПроверкиФНС");
	КонецЕсли;
	...
	Для каждого ПроверяемыйКонтрагент Из ПроверяемыеКонтрагенты Цикл
		...
		Для каждого СтрокаДанныхКонтрагента Из ДанныеКонтрагента Цикл
			...
			Если ДатыСовпадают ИЛИ ДатаПустая ИЛИ ДатаДокументаБольшеТекущей Тогда
				...
				ПроверяемыйКонтрагент.ПричинаОтключенияПроверкиФНС =
					СтрокаДанныхКонтрагента.ПричинаОтключенияПроверкиФНС;
				Прервать;
			КонецЕсли;
...
Функция ОпределитьНедействующихКонтрагентовВОтчете(ПроверяемыеКонтрагенты)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		...
		ДанныеКонтрагентов.ЗаполнитьЗначения(
			ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса(),
			"ОбластьДанныхВспомогательныеДанные");
	КонецЕсли;
...
Функция ШаблонТаблицыДанныеКонтрагента()
	...
	ДанныеКонтрагентов.Колонки.Добавить("ОтключитьПроверкуФНС",     ТипБулевоИНеопределено);
	ДанныеКонтрагентов.Колонки.Добавить("ПричинаОтключенияПроверкиФНС", ТипСтрока100);
...
Функция ШаблонТаблицыДанныеКонтрагентаДляДокумента()
	...
	ДанныеКонтрагентов.Индексы.Добавить("Контрагент, Дата");
	ДанныеКонтрагентов.Индексы.Добавить("Ссылка, Контрагент, Дата");
...
Функция ШаблонТаблицыОписания()
	...
	Описание.Колонки.Добавить("ПричинаОтключенияПроверкиФНС");
...
Функция ШаблонТаблицыОписанияКонтрагентовВСчетеФактуре()
	...
	Описание.Колонки.Добавить("ИмяРеквизитаСПричинойОтключенияПроверкиФНС");
...
Процедура ДополнитьОписаниеКонтрагентовВСчетеФактуре(
		...
		ИмяРеквизитаСПричинойОтключенияПроверкиФНС)
	...
	СтрокаОписания.ИмяРеквизитаСПричинойОтключенияПроверкиФНС = ИмяРеквизитаСПричинойОтключенияПроверкиФНС;
...
Функция ОписаниеКонтрагентовВСчетеФактуре(Тип)
	...
	Для каждого ИмяРеквизитаКонтрагента Из ИменаРеквизитовКонтрагентов Цикл
		ДополнитьОписаниеКонтрагентовВСчетеФактуре(
			...
			ИмяРеквизитаПричиныОтключенияПроверкиФНСВСчетФактуре(ИмяРеквизитаКонтрагента));
...
Процедура ИнициализироватьРеквизитыПроверкиКонтрагентовВДокументе(Форма, ЭтоПринудительнаяИнициализация = Ложь)
	...
	// Реквизит, предназначенный для определения необходимости запуска проверки
	// сервисом ФНС.
	ДобавитьРеквизитПроверкиКонтрагентов(Форма, "ТребуетсяПроверкаСервисом", Истина);
	...
	ДобавитьРеквизитПроверкиКонтрагентов(Форма, "ПричиныОтключенияПроверкиФНС", "");
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется
		...
		Если ЭтоДокументСКонтрагентомВШапке Тогда
			...
			ДобавитьРеквизитПроверкиКонтрагентов(Форма, "ПричинаОтключенияПроверкиФНС", "");
		...
		Если ЭтоДокументСоСчетомФактуройВПодвале ИЛИ ЭтоСчетФактура Тогда
			...
			ДобавитьРеквизитПроверкиКонтрагентов(Форма, "ПричиныОтключенияПроверкиФНСВСчетеФактуре", "");
		...
		Если ЭтоСчетФактура Тогда
			...
			Для Каждого СтрокаОписания Из Описание Цикл
				...
				Форма.РеквизитыПроверкиКонтрагентов.Вставить(Свойство,
					Перечисления.СостоянияПроверкиКонтрагентов.ПустаяСсылка());
				Форма.РеквизитыПроверкиКонтрагентов.Вставить(
					ИмяРеквизитаПричиныОтключенияПроверкиФНСВСчетФактуре(ИмяРеквизитаСчетаФактуры),
					"");
...
Процедура ЗапомнитьРезультатПроверкиКонтрагентовВДокументе(Форма, ДанныеКонтрагентов)
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоДокументСКонтрагентомВШапке Тогда
		...
		Для каждого СтрокаОписания Из СтрокиПоДокументу Цикл
			...
			Форма.РеквизитыПроверкиКонтрагентов.ПричинаОтключенияПроверкиФНС =
				ИтоговоеСостояние.ПричинаОтключенияПроверкиФНС;
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоДокументСоСчетомФактуройВПодвале Тогда
		...
		Если СтандартнаяОбработка Тогда
			...
			ИтоговоеСостояние = ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеКонтрагентов, Отбор);
			Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагентовВСчетеФактуре =
				ИтоговоеСостояние.СостояниеКонтрагента;
			Форма.РеквизитыПроверкиКонтрагентов.ПричиныОтключенияПроверкиФНСВСчетеФактуре =
				ИтоговоеСостояние.ПричинаОтключенияПроверкиФНС;
	...
	Если ЭтоДокументСКонтрагентомВШапке
		...
		Для каждого СтрокаОписания Из Описание Цикл
			...
			Если Элемент = Неопределено ИЛИ Элемент = СтрокаОписания.Элемент Тогда
				...
				ПараметрыПрорисовки.Вставить("ПричинаОтключенияПроверкиФНС",
					СтрокаОписания.ПричинаОтключенияПроверкиФНС);
...
Процедура СохранитьРезультатПроверкиКонтрагентов(Форма, ТекущийОбъект, ЭтоОтображениеИзКэшаПриОткрытии = Ложь)
	
	Если Форма.РеквизитыПроверкиКонтрагентов.ИнициализацияВыполненаПолностью Тогда
		
		СохраняемоеЗначение = РезультатПроверкиКонтрагентовВДокументе(Форма, ТекущийОбъект);
		
		// При открытии документа не сохранять состояние документа, если оно
		// не изменилось в результате проверки по данным из кэша.
		Если Не ЭтоОтображениеИзКэшаПриОткрытии
			Или ЭтоОтображениеИзКэшаПриОткрытии
			И СохраняемоеЗначение[ТекущийОбъект.Ссылка] <> Форма.РеквизитыПроверкиКонтрагентов.ЭтотДокументБылСОшибкой Тогда
...
Процедура ПоказатьПредварительныйРезультатИзКэшаПриОткрытииДокумента(Форма)
	...
	Форма.РеквизитыПроверкиКонтрагентов.ТребуетсяПроверкаСервисом = 
		(ДополнительныеПараметры.Свойство("ТребуетсяПроверкаСервисом")
		И ДополнительныеПараметры.ТребуетсяПроверкаСервисом);
	
	ОтобразитьРезультатПроверкиКонтрагентаВДокументеСлужебная(Форма, Истина);
...
Процедура ОтобразитьРезультатПроверкиКонтрагентаВДокументеСлужебная(Форма, ЭтоОтображениеИзКэшаПриОткрытии = Ложь)
		...
		Если ЗначениеЗаполнено(Форма.РеквизитыПроверкиКонтрагентов.АдресХранилища) 
			...
			Если РезультатРаботыФоновогоЗадания <> Неопределено Тогда
				...
				Если РезультатРаботыФоновогоЗадания.СохранятьРезультатСразуПослеПроверки Тогда
					...
					СохранитьРезультатПроверкиКонтрагентов(Форма, ДокументОбъект, ЭтоОтображениеИзКэшаПриОткрытии);
...
Функция ПолеКонтрагентаСкрытоФункциональнойОпцией(Форма, ОписаниеТабличнойЧасти)
	...
	ИскомыйОбъект = ДокументСсылка.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Реквизиты[ИмяКолонки];
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ЭтоСчетФактура Тогда
		...
		Для каждого СтрокаОписания Из Описание Цикл
			...
			Если ЗначениеЗаполнено(Контрагент) Тогда
				...
				ИтоговоеСостояние = ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеКонтрагентов, Отбор);
				Форма.РеквизитыПроверкиКонтрагентов[СтрокаОписания.ИмяРеквизитаССостоянием] =
					ИтоговоеСостояние.СостояниеКонтрагента;
				Форма.РеквизитыПроверкиКонтрагентов[СтрокаОписания.ИмяРеквизитаСПричинойОтключенияПроверкиФНС] =
					ИтоговоеСостояние.ПричинаОтключенияПроверкиФНС;
			КонецЕсли;
...
Функция ИмяРеквизитаПричиныОтключенияПроверкиФНСВСчетФактуре(ИмяРеквизитаКонтрагента)
	
	Возврат "ПричинаОтключенияПроверкиФНС" + ИмяРеквизитаКонтрагента;
...
Процедура СохранитьРезультатПроверкиКонтрагентаПриЗаписи(КонтрагентОбъект, Отказ) Экспорт 
	...
	ДанныеКонтрагента.Вставить("ПричинаОтключенияПроверкиФНС", "");
	...
	УстановитьПривилегированныйРежим(Истина);
	Если Не ЗначениеЗаполнено(СостояниеКонтрагента)
		И Не ИзменилисьСвойстваКонтрагента(ДанныеКонтрагента) Тогда
		Возврат;
	КонецЕсли;
	СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента);
...
// Проверяет изменение свойств справочника по регистру Состояния контрагентов.
//
// Параметры:
//  ДанныеКонтрагента - Структура - Новые свойства контрагента.
// 
// Возвращаемое значение:
//  Булево - Признак изменения свойств.
//
Функция ИзменилисьСвойстваКонтрагента(ДанныеКонтрагента)
...
Процедура СохранитьРезультатыПроверкиСправочника(
	...
	Иначе
		...
		ОставитьКонтрагентовСНовымиДанными(ДанныеКонтрагентов);
		СохранитьРезультатыПроверкиКонтрагентовВРегистр(ДанныеКонтрагентов);
...
Процедура ОставитьКонтрагентовСНовымиДанными(ДанныеКонтрагентов)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияКонтрагентов КАК СостоянияКонтрагентов
	|		ПО ДанныеКонтрагентов.Контрагент = СостоянияКонтрагентов.Контрагент
	|			И ДанныеКонтрагентов.Состояние = СостоянияКонтрагентов.Состояние
	|			И ДанныеКонтрагентов.ИНН = СостоянияКонтрагентов.ИНН
	|			И ДанныеКонтрагентов.КПП = СостоянияКонтрагентов.КПП
	|			И ДанныеКонтрагентов.ОбластьДанныхВспомогательныеДанные = СостоянияКонтрагентов.ОбластьДанныхВспомогательныеДанные
	|ГДЕ
	|	СостоянияКонтрагентов.Контрагент ЕСТЬ NULL";
...
Процедура ЗапомнитьРезультатыПроверкиСправочника(
	ДанныеКонтрагентов,
	...
	ПараметрыПроверки = Новый Структура(
		"ЭтоПроверкаСправочника,СохранятьРезультатСразуПослеПроверки,АдресХранилища",
		Ложь,
		Ложь,
		Неопределено);
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ДополнительныеПараметры);
	
	Если Не ПараметрыПроверки.ЭтоПроверкаСправочника Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	СохранитьРезультатыПроверкиСправочника(
		ДанныеКонтрагентов,
		ПараметрыПроверки.АдресХранилища,
		ПараметрыПроверки.СохранятьРезультатСразуПослеПроверки);
