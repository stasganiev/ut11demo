...
Процедура ПриОпределенииНастроек(Параметры) Экспорт
	...
	Объекты.Добавить(Метаданные.Справочники.ВидыИнцидентов);
	Объекты.Добавить(Метаданные.Справочники.Контрагенты);
	Объекты.Добавить(Метаданные.Справочники.КритичностьИнцидентов);
	Объекты.Добавить(Метаданные.Справочники.НаборыУпаковок);
	...
	Объекты.Добавить(Метаданные.Справочники.ПолитикиУчетаСерий);
	Объекты.Добавить(Метаданные.Справочники.Приоритеты);
	Объекты.Добавить(Метаданные.Справочники.ПриоритетыОплаты);
	Объекты.Добавить(Метаданные.Справочники.ПричиныОтменыЗаказовКлиентов);
	Объекты.Добавить(Метаданные.Справочники.ПричиныОтменыЗаказовПоставщикам);
	Объекты.Добавить(Метаданные.Справочники.ПричиныПроигрышаСделок);
	Объекты.Добавить(Метаданные.Справочники.РолиПартнеровВСделкахИПроектах);
	Объекты.Добавить(Метаданные.Справочники.СезонныеГруппыБизнесРегионов);
	Объекты.Добавить(Метаданные.Справочники.СкидкиНаценки);
	Объекты.Добавить(Метаданные.Справочники.СостоянияПроцессов);
	Объекты.Добавить(Метаданные.Справочники.СтавкиНДС);
	Объекты.Добавить(Метаданные.Справочники.СтатьиДвиженияДенежныхСредств);
	Объекты.Добавить(Метаданные.Справочники.ВидыДокументовФизическихЛиц);
...
Процедура ПередОбновлениемИнформационнойБазы() Экспорт
	...
	Если ВерсииПодсистем.Количество() > 0 Тогда
		...
		Если ВерсииПодсистем.Найти("Локализация", "ИмяПодсистемы") = Неопределено Тогда
			...
			ОбновлениеИнформационнойБазыЛокализация.ПриДобавленииПодсистемы(ОписаниеПодсистемы);
			
			ОбновлениеИнформационнойБазы.ЗарегистрироватьНовуюПодсистему(ОписаниеПодсистемы.Имя, "2.5.1.0");
...
Процедура ПередФормированиеСпискаОтложенныхОбработчиков(ИтерацииОбновления) Экспорт
	...
	//++ НЕ ГОСИС
	ОбновлениеИнформационнойБазыУТ.ПроверитьНомераВерсий(ИтерацииОбновления);
	//-- НЕ ГОСИС
...
Процедура ПриФормированииСпискаРазрабатываемыхПодсистем(РазрабатываемыеПодсистемы) Экспорт
	...
	//++ НЕ ГОСИС
	РазрабатываемыеПодсистемы.Добавить(Метаданные.Имя);
	РазрабатываемыеПодсистемы.Добавить("УправлениеПредприятием");
	РазрабатываемыеПодсистемы.Добавить("КомплекснаяАвтоматизация");
	РазрабатываемыеПодсистемы.Добавить("УправлениеТорговлей" + ?(СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации(), "Базовая", ""));
	РазрабатываемыеПодсистемы.Добавить("Локализация" + ?(СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации(), "Базовая", ""));
	//-- НЕ ГОСИС
...
Процедура ПриНачальномЗаполненииЭлементов(ПолноеИмяОбъекта, КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	//++ НЕ ГОСИС
	Если ПолноеИмяОбъекта = Метаданные.Справочники.ПрофилиГруппДоступа.ПолноеИмя() Тогда
		УправлениеДоступомУТ.УстановитьНаименованиеГруппПрофилейГруппДоступа(Элементы);
	КонецЕсли;
	//-- НЕ ГОСИС
