		...
		Если РезультатВыполнения.Свойство("НаборДанных") Тогда
			...
			Для Каждого ПодписываемыеДанные Из РезультатВыполнения.НаборДанных Цикл
				...
				Если ЗначениеЗаполнено(Доверенность) Тогда
					...
					ИННДоверителя = "";
					Если ЗначениеЗаполнено(Контекст.ДанныеПакета.РеквизитыПакета.ИННДоверителя) Тогда
						ИННДоверителя = Контекст.ДанныеПакета.РеквизитыПакета.ИННДоверителя;
					Иначе
						ИННДоверителя = Контекст.ДанныеПакета.РеквизитыПакета.ИННОрганизации;
					КонецЕсли;
					ПараметрыПроверкиМЧД.ИННДоверителя = ИННДоверителя;
...
Функция РезультатОперацииВСервисе1СЭДО(Контекст) Экспорт
	...
	ИдентификаторЗаявки = "";
	Если ЗначениеЗаполнено(Контекст.ДанныеПакета) Тогда
		ИдентификаторЗаявки = Контекст.ДанныеПакета.РеквизитыПакета.УникальныйИдентификаторЗаявки1СЭДО;
	КонецЕсли;
	
	РезультатОперации = Новый Структура;
	РезультатОперации.Вставить("Успех", Ложь);
	РезультатОперации.Вставить("УникальныйИдентификаторЗаявки1СЭДО", ИдентификаторЗаявки);
...
// Параметры:
//  ПараметрыСоздания - см. УчетныеЗаписиЭДОКлиент.НовыеПараметрыСозданияУчетнойЗаписи
Процедура СоздатьУчетнуюЗапись(ПараметрыСоздания) Экспорт

	Если ПараметрыСоздания.ОперацияЭДО <> Неопределено 
		И ПараметрыСоздания.ОперацияЭДО.СлужебныеПараметры.ПаролиСертификатов <> Неопределено Тогда
		ПараметрыСоздания.ОперацияЭДО.СлужебныеПараметры.ПаролиСертификатов.Очистить();
	КонецЕсли;
	...
	ПараметрыФормы.Вставить("ЧерезОблачныйЭДО", ПараметрыСоздания.ЧерезОблачныйЭДО);
