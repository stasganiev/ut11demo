...
Функция ТаблицыРасхожденийПоДаннымЭДО(ДеревоУпаковокФакт, ДеревоУпаковокПлан, ШтрихкодыБрак, ПараметрыСверки) Экспорт
	...
	Если ПараметрыСверки.ЭтоПродукцияМОТП Тогда
		Результат = ТаблицыТОРГ2ПланФактПоДеревуУпаковокМОТП(
			ДеревоУпаковокПлан,
			ШтрихкодыБрак,
			Истина,
	...
	Иначе
		Результат = ТаблицыТОРГ2ПланФактПоДеревуУпаковокИСМП(
			ДеревоУпаковокПлан,
			ШтрихкодыБрак,
			Истина,
			ПараметрыСверки.ТоварыПринятыеБезРасхождений,
...
Функция ТаблицаРасхожденийШтриховыхКодовПолная(ПараметрыЗаполнения) Экспорт
	...
	Запрос.УстановитьПараметр("БезРасхождений",           ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("ТипРасхожденияИСМП"));
		...
		Если НайденнаяСтрокаДерева2 = Неопределено Тогда
			...
			Иначе
				...
				Если СтрокаДерева1.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Потребительская
					И СтрокаДерева1.ВидУпаковки <> Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
...
Функция ТаблицыТОРГ2ПланФактПоДеревуУпаковокИСМП(ДеревоМаркированнойПродукции, НедопустимыеШтрихкоды,
	ПерваяИтерация = Ложь, ТоварыПринятыеБезРасхождений = Неопределено, Детализация) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.Строки Цикл
		...
		Иначе
			...
			Иначе
				...
				ВложенныйРезультат = ТаблицыТОРГ2ПланФактПоДеревуУпаковокИСМП(СтрокаДерева, НедопустимыеШтрихкоды, ,ТоварыПринятыеБезРасхождений, Детализация);
...
Процедура ДополнитьТаблицыТОРГ2(ТаблицаПриемник, СтрокаИсточник, ЗначениеШтрихкода, Принята, Излишек, Брак = Ложь)
	...
	Количество = Макс(СтрокаИсточник.КоличествоПотребительскихУпаковок, 1);
...
Процедура ДополнитьТаблицыТОРГ2МОТП(ТаблицаПриемник, СтрокаИсточник, ЗначениеШтрихкода, Принята, Излишек, Брак = Ложь)
	...
	Количество = СтрокаИсточник.КоличествоПотребительскихУпаковок;
	Если Принята Тогда
		СтрокаПриемник.КоличествоПринято = Количество;
	ИначеЕсли Излишек Тогда
		СтрокаПриемник.КоличествоИзлишек = Количество;
	ИначеЕсли Брак Или Количество = 0 Тогда
		Если Количество = 0 И СтрокаИсточник.Коэффициент <> Неопределено Тогда
			Количество = СтрокаИсточник.КоличествоГрупповыхУпаковок * СтрокаИсточник.Коэффициент;
		КонецЕсли;
		СтрокаПриемник.КоличествоБрак = Количество;
		СтрокаПриемник.Брак = Истина;
	Иначе
