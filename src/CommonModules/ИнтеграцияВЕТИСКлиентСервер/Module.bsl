...
Функция ПараметрыЗаполненияТабличнойЧасти() Экспорт
	...
	ПараметрыЗаполнения.Вставить("ЗаполнитьПризнакСкоропортящаясяПродукция", Ложь);
...
Функция СтруктураДанныхШтрихкодыУпаковок() Экспорт
	...
	Данные.Вставить("ШтрихкодУпаковки");
...
Процедура УстановитьОтборОрганизацияПредприятиеВЕТИС(Список, ДеревоОтбора, Префиксы, ИмяПоляОтбора = "ХозяйствующийСубъект") Экспорт
	...
	ЭлементыГруппаОрганизацияПредприятие = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(
		Список.Отбор,,"ГруппаОрганизацияПредприятие");
	Для Каждого ЭлементОтбора Из ЭлементыГруппаОрганизацияПредприятие Цикл
...
Функция ПодсказкаКСостояниюОбмена(ИдентификаторПроблемы, Элемент = Неопределено) Экспорт
	...
	ИначеЕсли ИдентификаторПроблемы = "НеЗаполненыСлужебныеРегистры" Тогда
		
		СтрокаЗаголовка = Новый Массив;
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru='Не заполнен регистр сведений ""Виды продукции по группам ВетИС""
				         |или регистр сведений ""Допустимые цели по группам ВетИС"".
				         |При оформлении исходящих транспортных операций ВетИС недоступны цели.'")));
		СтрокаЗаголовка.Добавить("");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru='Заполнить'"),,,, "ЗаполнитьСлужебныеРегистры"));
		СтрокаЗаголовка.Добавить(".");
