...
// Формирует настройки подключения для выполнения запросов к Системе быстрых платежей.
//
// Параметры:
...
//    * Ключ - СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей -
//      настройка выполнения оплаты.
//    * Значение - Структура - настройки выполнения запросов:
//       ** ИдентификаторМерчанта    - Строка - идентификатор торговой точки в Системе быстрых платежей;
//       ** ИдентификаторУчастника - Строка - идентификатор участника Системы быстрых платежей;
//       ** Используется - Булево - признак активности настройки;
//       ** ТипАутентификации - ПеречислениеСсылка.ТипыАутентификацииСБП - способ авторизации в сервисе участника СБП.
//
Функция ПараметрыНастроекПодключения(НастройкиПодключения) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО НастройкиПодключенияКСистемеБыстрыхПлатежей.ИдентификаторУчастника = НастройкиУчастниковСБП.Идентификатор
		|ГДЕ
		|	НастройкиПодключенияКСистемеБыстрыхПлатежей.Ссылка В(&НастройкиПодключения)";
	
	Запрос.УстановитьПараметр("НастройкиПодключения", НастройкиПодключения);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Результат.Вставить(ВыборкаДетальныеЗаписи.Ссылка, ПараметрыНастройкиПодключения);
...
#Область НастройкаФормыПодключения
...
Процедура СохранитьНастройкиАутентификации(
	...
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
		НастройкаПодключения,
		ПреобразоватьНастройкиАутентификации(
			ИдентификаторУчастника,
			ПараметрыАутентификации,
			Истина));
