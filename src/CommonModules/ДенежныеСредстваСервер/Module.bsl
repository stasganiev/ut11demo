...
Процедура УстановитьПараметрыВыбораЗаявки(Объект, ЭлементЗаявка) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета
		...
		МассивПараметров.Добавить(Новый СвязьПараметраВыбора("Контрагент", "Объект.Контрагент"));
	КонецЕсли;
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	Если ДанныеЗаполнения.ПлатежСКонвертацией Тогда
		Если ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчетКонтрагента) Тогда
			ДанныеЗаполнения.Валюта = ДанныеЗаполнения.БанковскийСчетКонтрагентаВалюта;
		ИначеЕсли ЗначениеЗаполнено(ДанныеЗаполнения.БанковскийСчетПолучатель) Тогда
			ДанныеЗаполнения.Валюта = ДанныеЗаполнения.БанковскийСчетПолучательВалюта;
		КонецЕсли;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Сумма > 0 Тогда
			...
			//Если Не ЗначениеЗаполнено(НоваяСтрока.СуммаВзаиморасчетов) Тогда
			//	НоваяСтрока.СуммаВзаиморасчетов = НоваяСтрока.Сумма;
			//КонецЕсли;
			//
			//Если Не ЗначениеЗаполнено(НоваяСтрока.ВалютаВзаиморасчетов) Тогда
...
Функция ТекстыЗапросаПроверкаКурсовЗачетаВАвансовыхОтчетах()
	...
	|		ТИП(Документ.Бронирование)
	|	)
	|	И Данные.СуммаНачальныйОстаток > 0
	|	И Данные.СуммаРеглНачальныйОстаток / Данные.СуммаНачальныйОстаток <> 1
	|	И ВЫРАЗИТЬ(Данные.СуммаРеглНачальныйОстаток - Данные.СуммаРеглКонечныйОстаток КАК Число(31)) <>
	...
	|	ВЫРАЗИТЬ(Данные.СуммаРеглНачальныйОстаток - Данные.СуммаРеглКонечныйОстаток КАК Число(31))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ВЫРАЗИТЬ(Данные.СуммаУпрНачальныйОстаток - Данные.СуммаУпрКонечныйОстаток КАК Число(31)) <>
	...
	|	ВЫРАЗИТЬ(Данные.СуммаУпрНачальныйОстаток - Данные.СуммаУпрКонечныйОстаток КАК Число(31))
	|";
