...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПредприятия2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	|ИЗ
	|	ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(&ДатаДокумента, (Номенклатура, ХарактеристикаЦО,
...
Процедура ВыполнитьОбновлениеЦен(ВидыЦен, ПараметрыОбновленияЦен) Экспорт
	...
	Если  ИспользоватьИнтеграциюСЯндексМаркет Тогда 
		...
		Если МассивВидовЦенЯндексМаркет.Количество() Тогда
			...
			Для Каждого ЭлементКоллекции Из ВидыЦен Цикл
			...
			ИнтеграцияСМаркетплейсамиСерверЛокализация.ЗагрузитьРекомендованныеЦеныЯндексМаркет(Неопределено, ПараметрыЗадания);
		КонецЕсли;
...
Функция ДобавитьТоварыПредприятия2_5(Форма, Знач ТаблицаТовары, КэшДанных)
	...
	|	Таблица.СерияЦО КАК СерияЦО,
	|	Таблица.УпаковкаЦО КАК УпаковкаЦО
	...
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ) КАК СерииИспользуются,
	|	(ВЫБОР
	|		КОГДА ВидыНоменклатуры.НастройкиКлючаЦенПоУпаковке = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ) КАК УпаковкиИспользуются
	|ПОМЕСТИТЬ ТаблицаНоменклатура
	|ИЗ
	...
	|	ТаблицаНоменклатура.Индекс
	|ИТОГИ
	|	МИНИМУМ(ХарактеристикиИспользуются) КАК ХарактеристикиИспользуются,
	|	МИНИМУМ(СерииИспользуются) КАК СерииИспользуются,
	|	МИНИМУМ(СерияЦОДляСвязи) КАК СерияЦОДляСвязи,
	|	МИНИМУМ(УпаковкиИспользуются) КАК УпаковкиИспользуются,
	|	МИНИМУМ(УпаковкаЦОДляСвязи) КАК УпаковкаЦОДляСвязи
	|ПО
	|	Номенклатура,
	|	ХарактеристикаЦО,
	|	СерияЦО,
	...
	|	ЦеныНоменклатуры.ВидЦены          КАК ВидЦены
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(&ДатаДокумента, (Номенклатура, ХарактеристикаЦО, СерияЦО, УпаковкаЦО) В (ВЫБРАТЬ Т.Номенклатура, Т.ХарактеристикаЦО, Т.СерияЦО, Т.УпаковкаЦО ИЗ ТаблицаНоменклатура КАК Т) И ВидЦены В (&ВидыЦен)) КАК ЦеныНоменклатуры
	|";
