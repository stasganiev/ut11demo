...
Процедура ПриЗаполненииХостовСервисовИнтернетПоддержки(ХостыСервисовИнтернетПоддержки) Экспорт
	...
	СтруктураURL = ОбщегоНазначенияКлиентСервер.СтруктураURI(ХостСервисаИнтеграцииСОнлайнОплатами(Истина));
	ХостыСервисовИнтернетПоддержки.Вставить(
		СтруктураURL.Хост,
		НСтр("ru = 'Интеграция с ЮKassa: Заказы.'"));
	СтруктураURL = ОбщегоНазначенияКлиентСервер.СтруктураURI(ХостСервисаИнтеграцииСОнлайнОплатами(Ложь));
	ХостыСервисовИнтернетПоддержки.Вставить(
		СтруктураURL.Хост,
		НСтр("ru = 'Интеграция с ЮKassa: Заказы.'"));
	...
	ХостыСервисовИнтернетПоддержки.Вставить(
		"money.yandex.ru",
		НСтр("ru = 'Интеграция с ЮKassa: Оплаты.'"));
...
Процедура УстановитьСтатусОбменаСОнлайнОплатами(Знач НастройкаОнлайнОплаты, Знач Статус) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка установки статуса обмена'");
...
Функция СоздатьПредопределенныеШаблоныСообщений(СоздатьШаблоныПисем, СоздатьШаблоныСМС) Экспорт
	...
	Для Каждого Шаблон Из МассивПредопределенныхШаблоновСообщений Цикл 
		...
		Если Не Шаблон.Свойство("ТипШаблона") Тогда // АПК:1416 Поддержка старых шаблонов БЭД.
			Продолжить;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	ЗапросРазрешений = МодульРаботаВБезопасномРежиме.ЗапросНаИспользованиеВнешнихРесурсов(
		Разрешения,
		ВладелецРазрешения,
		Истина);
...
Функция ОснованиеПлатежаИзменено(Знач ВходящиеПараметры) Экспорт
	...
	ДанныеВСервисе  = ВходящиеПараметры.ДанныеОснованияПлатежаВСервисе;
	
	Если ВходящиеПараметры.НастройкаОнлайнОплаты.ОтправкаЧековЧерезСервис Тогда
		ДанныеОснования.Номенклатура =
			ПровестиФорматноЛогическийКонтроль(ДанныеОснования.Номенклатура);
	КонецЕсли;
	
	Если ДанныеВСервисе.СрокЖизниСсылки = Неопределено
		Или ДанныеОснования.СрокЖизниСсылки = Неопределено
		Или ДанныеВСервисе.СрокЖизниСсылки = ДанныеОснования.СрокЖизниСсылки Тогда
		СрокЖизниСсылкиСовпадает = Истина;
	Иначе
		СрокЖизниСсылкиСовпадает = Ложь;
	КонецЕсли;
	
	ДанныеДляСервиса = ДанныеОснованияПлатежаДляСервиса(ДанныеОснования);
	
	Если ДанныеДляСервиса.checksum = ДанныеВСервисе.ХешСумма
		И СрокЖизниСсылкиСовпадает Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ПлатежнаяСсылка(Знач ВходящиеПараметры) Экспорт 
	...
	Если СтруктураПараметровОнлайнОплаты.ИспользоватьНовыйПротокол Тогда
...
// Выполняет отмену платежной ссылки в сервисе.
//
// Параметры:
//  ВходящиеПараметры - Структура - параметры метода:
//    * ОснованиеПлатежа - Произвольный - основание платежа, для которого нужно получить ссылку на оплату.
...
//       пользователю необходимо выдать доступ к онлайн оплате, 
//       перейдя на страницу по адресу ОнлайнОплаты.АдресСтраницыПредоставленияДоступа().
//   * Статус - Строка - статус платежной ссылки. Может принимать значение:
//     Ошибка, Отменен, Оплачен, ОплаченОжиданиеЧека.
//
Функция ОтменаПлатежнойСсылки(Знач ВходящиеПараметры) Экспорт 
	
	// Инициализация команды
	ИмяКоманды = "ОтменитьЗаказ";
	...
	Если Отказ Тогда
		...
		Результат.Вставить("ОтсутствуетДоступ", Ложь);
		Результат.Вставить("Статус",            ИдентификаторСтатусаОшибка());
	...
	Если СтруктураПараметровОнлайнОплаты.ИспользоватьНовыйПротокол Тогда
		Возврат ОтменаПлатежнойСсылкиНовыйПротокол(ПараметрыСервиса, СтруктураПараметровОнлайнОплаты, ВходящиеПараметры, Отказ);
	Иначе
		
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Операция не поддерживается в варианте настройки без договора.'"));
...
//    * ИдентификаторОснованияПлатежа - Строка, Неопределено - идентификатор основания платежа, по которому выполянется запрос.
//
// Возвращаемое значение:
...
//
Функция ДанныеОснованияПлатежаВСервисе(Знач ВходящиеПараметры, ИдентификаторОснованияПлатежа = Неопределено) Экспорт
	...
	Результат.Вставить("ОтсутствуетДоступ", Ложь);
	Результат.Вставить("СрокЖизниСсылки", Неопределено);
...
Функция ОперацииОнлайнОплат(Знач ВходящиеПараметры) Экспорт
	...
	Для каждого ПараметрыПолучения Из ПараметрыПолученияОпераций Цикл
		...
		// Обновление настроек
		Если РезультатРазбора.Свойство("Настройки") // АПК:1416 Не обязательное свойство ответа сервиса.
...
//  Структура - результат длительной операции. См. ДлительныеОперации.ВыполнитьФункцию.
//
Функция ОтменаПлатежнойСсылкиВСервисеВФоне(Знач ВходящиеПараметры, Знач ИдентификаторФормы) Экспорт

	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru = 'Отмена ссылки для оплаты через ЮKassa.'");
		
	Результат = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполнения,
		"ОнлайнОплатыСлужебный.ОтменаПлатежнойСсылки",
		ВходящиеПараметры);
...
Процедура ОбновитьНастройкуОнлайнОплаты(НастройкаСсылка, ЗначенияРеквизитов, Отказ)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка обновления настройки онлайн оплаты'");
...
Функция ИдентификаторОснованияПлатежа(Знач ОснованиеПлатежа)
	
	Идентификатор = "";
	
	ИнтеграцияПодсистемБИП.ЗаполнитьИдентификаторОснованияПлатежа(ОснованиеПлатежа, Идентификатор);
...
Функция ДанныеОснованияПлатежа(Знач ОснованиеПлатежа)
	...
	ТаблицаНоменклатуры.Колонки.Добавить("ПризнакСпособаРасчета");
	...
	ТаблицаНоменклатуры.Колонки.Добавить("СуммаАкциза");
	...
	Данные.Вставить("СрокЖизниСсылки", Неопределено);
...
Функция ДанныеОснованияПлатежаТоварыЧек(Знач ДанныеОснованияПлатежа)
	...
	СоответствиеПолей.Вставить("НомерТаможеннойДекларации",    "customsDeclarationNumber");
	СоответствиеПолей.Вставить("СуммаАкциза",                  "excise");
	СоответствиеПолей.Вставить("СпособРасчетаСтрокой",         "paymentMode");
...
Функция ПровестиФорматноЛогическийКонтроль(ПозицииЧека)
	
	ДопустимоеРасхождение = 0;
	ИндексНачальнойСтроки = ПозицииЧека.Количество() - 1;
	НовыеПозицииЧека = ПозицииЧека.СкопироватьКолонки();
	
	Для ИндексМассива = 0 По ИндексНачальнойСтроки Цикл
		ТекущаяПозиция = ПозицииЧека[ИндексМассива];
		Количество = ?(ТекущаяПозиция.Количество = 0, 1, ТекущаяПозиция.Количество);
		Если Количество = 1 Тогда
			НоваяПозицияЧека = НовыеПозицииЧека.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяПозицияЧека, ТекущаяПозиция);
			Продолжить;
		КонецЕсли;
		НоваяСуммаБезОкругления = Количество * ТекущаяПозиция.Цена;
		...
		Если Окр(ТекущаяПозиция.Цена* Количество, 2, 1) = ТекущаяПозиция.Сумма Тогда
		...
		ИначеЕсли РазницаСуммДляПроверки >= -ДопустимоеРасхождение И РазницаСуммДляПроверки <= ДопустимоеРасхождение Тогда
			...
			НоваяПозицияЧека = НовыеПозицииЧека.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяПозицияЧека, ТекущаяПозиция);
			НоваяПозицияЧека.Цена = РасчетнаяЦена;
			Продолжить;
		ИначеЕсли РазницаСумм = 0 Тогда
			// Если разница неокругленная есть, а округленной нет - оставляем одну строку.
			// Но подменяем расчетную цену.
			НоваяПозицияЧека = НовыеПозицииЧека.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяПозицияЧека, ТекущаяПозиция);
			НоваяПозицияЧека.Цена = РасчетнаяЦена;
...
Процедура РазделитьФискальнуюСтроку(ТекущаяПозиция, НовыеПозицииЧека, РасчетнаяЦена, РазницаСумм) 
	...
	Если ЗначениеЗаполнено(ТекущаяПозиция.СуммаАкциза) Тогда
		СуммаАкцизаНачальная = ТекущаяПозиция.СуммаАкциза;
	Иначе
		СуммаАкцизаНачальная = Неопределено;
	КонецЕсли;
	...
	Если СуммаАкцизаНачальная <> Неопределено Тогда
		Если СуммаПереоценки > СуммаРасчетная Тогда
			СуммаАкцизаПереоценки = Окр(СуммаАкцизаНачальная * СуммаПереоценки / СуммаНачальная, 2, 1);
			СуммаАкцизаРасчетная = СуммаАкцизаНачальная - СуммаАкцизаПереоценки;
		Иначе
			СуммаАкцизаРасчетная = Окр(СуммаАкцизаНачальная * СуммаРасчетная / СуммаНачальная, 2, 1);
			СуммаАкцизаПереоценки = СуммаАкцизаНачальная - СуммаАкцизаРасчетная;
		КонецЕсли;
	КонецЕсли;
	...
	НоваяПозицияЧека = НовыеПозицииЧека.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяПозицияЧека, ТекущаяПозиция);
	НоваяПозицияЧека.Количество = КоличествоРасчетное;
	НоваяПозицияЧека.Цена = РасчетнаяЦена;
	НоваяПозицияЧека.Сумма = СуммаРасчетная;
	Если СуммаАкцизаНачальная <> Неопределено Тогда
		НоваяПозицияЧека.СуммаАкциза = СуммаАкцизаРасчетная;
	КонецЕсли;
	...
	НоваяПозицияЧека = НовыеПозицииЧека.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяПозицияЧека, ТекущаяПозиция);
	НоваяПозицияЧека.Количество = КоличествоПереоценки;
	НоваяПозицияЧека.Цена = ЦенаПереоценки;
	НоваяПозицияЧека.Сумма = СуммаПереоценки;
	Если СуммаАкцизаНачальная <> Неопределено Тогда
		НоваяПозицияЧека.СуммаАкциза = СуммаАкцизаПереоценки;
	КонецЕсли;
	
КонецПроцедуры

Функция ПроверитьЗаполнениеОснованияПлатежа(Знач ОснованиеПлатежа) Экспорт
...
Функция ДанныеОснованияПлатежаВСервисеСтарыйПротокол(
	...
	ИначеЕсли РезультатРазбора.ДанныеXDTO.Status = 0 Тогда
		...
		Если РезультатРазбора.ДанныеXDTO.OrderState < 2 Тогда
			Результат.Статус = ИдентификаторСтатусаНеОплачен();
		ИначеЕсли РезультатРазбора.ДанныеXDTO.OrderState = 2 Тогда
			Результат.Статус = ИдентификаторСтатусаОплачен();
		ИначеЕсли РезультатРазбора.ДанныеXDTO.OrderState = 3 Тогда
			Результат.Статус = ИдентификаторСтатусаОплаченОжиданиеЧека();
		КонецЕсли;
...
Функция ОтменаПлатежнойСсылкиНовыйПротокол(ПараметрыСервиса, СтруктураПараметровОнлайнОплаты, ВходящиеПараметры, Отказ)
	
	ИмяКоманды = "ОтменитьПлатежнуюСсылку";
	
	Результат = Новый Структура;
	Результат.Вставить("ОтсутствуетДоступ", Ложь);
	Результат.Вставить("Статус",            "");
	
	ПараметрыКоманды = ПараметрыКомандыОтменитьЗаказНовыйПротокол(
		ПараметрыСервиса,
		СтруктураПараметровОнлайнОплаты,
		ВходящиеПараметры,
		Отказ);
	...
	Если Отказ Тогда
		Результат.Статус = ИдентификаторСтатусаОшибка();
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Отказ Тогда
		Результат.Статус = "Отменен";
		Возврат Результат;
	КонецЕсли;
	
	Если РезультатКоманды.ТипОшибки = ИдентификаторОшибкиНачатаОплата() Тогда
		
		// Кэшируем сообщения, чтобы не вывести раньше времени
		БуферОбмена = ОбщегоНазначения.СтрокиИзБуфераОбмена();
		
		Если БуферОбмена.Источник = ИдентификаторИсточникаОшибок()
			И ТипЗнч(БуферОбмена.Данные) = Тип("Соответствие") Тогда
			СообщенияПользователю = БуферОбмена.Данные;
			ОчиститьПереченьОшибок();
		Иначе
			СообщенияПользователю = Новый Соответствие;
		КонецЕсли;
		
		РезультатОперации = ДанныеОснованияПлатежаВСервисе(
			ВходящиеПараметры,
			ПараметрыКоманды.ИдентификаторОснованияПлатежа);
		
		Если РезультатОперации.Статус = ИдентификаторСтатусаОплачен()
			Или РезультатОперации.Статус = ИдентификаторСтатусаОтменен()
			Или РезультатОперации.Статус = ИдентификаторСтатусаОплаченОжиданиеЧека() Тогда
			ОчиститьПереченьОшибок();
			Результат.Статус = РезультатОперации.Статус;
		Иначе
			ВывестиСообщенияПользователю(СообщенияПользователю);
			Результат.Статус = ИдентификаторСтатусаОшибка();
		КонецЕсли;
	Иначе
		ВывестиОшибкиПользователю();
...
Функция ПараметрыОнлайнОплаты(Знач ИмяКоманды, Знач ВходящиеПараметры, Отказ = Ложь)
	...
	ДанныеПоиска.Вставить("НастройкаОнлайнОплаты", Неопределено);
	ДанныеПоиска.Вставить("ОснованиеПлатежа",      Неопределено);
	ДанныеПоиска.Вставить("Организация",           Неопределено);
	
	ЗаполнитьЗначенияСвойств(ДанныеПоиска, ВходящиеПараметры);
	
	// Подготовим настройки Онлайн оплаты
	Если ДанныеПоиска.НастройкаОнлайнОплаты <> Неопределено Тогда
		НастройкаОнлайнОплаты = ДанныеПоиска.НастройкаОнлайнОплаты;
	ИначеЕсли ДанныеПоиска.ОснованиеПлатежа <> Неопределено Тогда
		НастройкаОнлайнОплаты = НастройкаОнлайнОплатыОснованияПлатежа(ДанныеПоиска.ОснованиеПлатежа);
	ИначеЕсли ДанныеПоиска.Организация <> Неопределено Тогда
		НастройкаОнлайнОплаты = НастройкаОнлайнОплатыОрганизации(ДанныеПоиска.Организация);
	Иначе
		ПараметрыСообщения = Новый Структура();
		ПараметрыСообщения.Вставить("ИмяКоманды", ИмяКоманды);
...
Функция ПараметрыКомандыПолучитьДанныеПоЗаказу(
	...
	Если ИдентификаторОснованияПлатежа = Неопределено
		И ПараметрыОнлайнОплаты.Настройка.СДоговором Тогда
		ДанныеОснованияПлатежа = ДанныеОснованияПлатежа(ВходящиеПараметры.ОснованиеПлатежа);
		ИдентификаторОснованияПлатежа = ДанныеОснованияПлатежа.Идентификатор;
	КонецЕсли;
	...
	Если ПараметрыОнлайнОплаты.Настройка.СДоговором Тогда
		Параметры.Вставить("orderNumber", Строка(ИдентификаторОснованияПлатежа));
	Иначе
...
Функция ПараметрыКомандыСоздатьОбновитьЗаказ(
	...
	ДанныеОснованияПлатежа = ДанныеОснованияПлатежа(ВходящиеПараметры.ОснованиеПлатежа);
	Если ЗначениеЗаполнено(ВходящиеПараметры.КонтактныеДанныеЧека) Тогда
...
Функция ПараметрыКомандыСоздатьОбновитьЗаказНовыйПротокол(
	...
	ПараметрыЗапроса.Вставить("paymentCapabilityTtl", ДанныеОснованияПлатежа.СрокЖизниСсылки);
	
	// receipt
	Если НастройкаОнлайнОплаты.ОтправкаЧековЧерезСервис Тогда
		
		ДанныеОснованияПлатежа.Номенклатура = ПровестиФорматноЛогическийКонтроль(ДанныеОснованияПлатежа.Номенклатура);
...
Функция ПараметрыКомандыОтменитьЗаказНовыйПротокол(
		Знач ПараметрыСервиса,
		Знач ПараметрыОнлайнОплаты,
		Знач ВходящиеПараметры,
		Отказ = Ложь)
	...
	ИдентификаторОснованияПлатежа = ИдентификаторОснованияПлатежа(ВходящиеПараметры.ОснованиеПлатежа);
	
	Если ПустаяСтрока(ИдентификаторОснованияПлатежа) Тогда
		ДанныеОснованияПлатежа = ДанныеОснованияПлатежа(ВходящиеПараметры.ОснованиеПлатежа);
		ИдентификаторОснованияПлатежа = ДанныеОснованияПлатежа.Идентификатор;
	КонецЕсли;
	...
	КодировкаURL = СпособКодированияСтроки.КодировкаURL;
	ИдентификаторПлатежа = КодироватьСтроку(Строка(ИдентификаторОснованияПлатежа), КодировкаURL);
	...
	Параметры.Вставить("ИдентификаторМагазина", Формат(НастройкаОнлайнОплаты.ИдентификаторМагазина,"ЧГ="));
	Параметры.Вставить("ИдентификаторПлатежа", ИдентификаторПлатежа);
	Параметры.Вставить("ИдентификаторОснованияПлатежа", Строка(ИдентификаторОснованияПлатежа));
...
Процедура ПроверитьПараметрыВызоваКомандыСервиса(
	...
	ИначеЕсли ИмяКоманды = "ОбновитьДанныеЗаказа_НовыйПротокол" Тогда
		...
		Если ПараметрыКоманды.Свойство("ПараметрыЗапроса") Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыЗапроса.paymentCapabilityTtl) Тогда
				
				СтруктураПроверки = Новый Структура;
				СтруктураПроверки.Вставить("paymentCapabilityTtl", "Число");
				...
				Если ПараметрыЗапроса.paymentCapabilityTtl < 1
					Или ПараметрыЗапроса.paymentCapabilityTtl > 129600 Тогда
					
					СформироватьПараметрыОшибкиЗначенияПараметровКоманды(
						ИмяКоманды,
						"paymentCapabilityTtl",
						Отказ);
	...
	ИначеЕсли ИмяКоманды = "ОтменитьПлатежнуюСсылку" Тогда
...
Процедура ПроверитьСтруктуру(ИмяКоманды, ПараметрыПроверки, ПроверяемыеДанные, Отказ)
	...
	Для Каждого ПараметрПроверки Из ПараметрыПроверки Цикл
		...
		Если Не ПроверяемыеДанные.Свойство(ПроверяемоеСвойство) // АПК:1415 Проверка свойств прикладной логики.
...
Процедура ПроверитьСоответствиеСумм(ИмяКоманды, СуммаЗаказа, МассивТоваров, Отказ)
	...
	Для Каждого Товар Из МассивТоваров Цикл
		Если Товар.Свойство("sum") Тогда // АПК:1416 Проверка свойств прикладной логики..
...
Процедура ПреобразоватьСтрокуАутентификации(ПараметрыКоманды, ИспользоватьНовыйПротокол)
	...
	Если Не ПараметрыКоманды.Свойство("ДанныеАутентификации") Тогда // АПК:1415 Поддержка старых сценариев БЭД.
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьКомандуСервиса(Знач ИмяКоманды, ПараметрыКоманды, ИспользоватьНовыйПротокол, Результат, Отказ)
	...
	Если ЗначениеЗаполнено(HTTPОтвет.СообщениеОбОшибке) Тогда
		...
		Если ИспользоватьНовыйПротокол Тогда
			РезультатРазбора = РазобратьОшибкуПоДаннымОтвета(
				HTTPОтвет.КодСостояния,
				HTTPОтвет.Содержимое);
			Результат.СообщениеОбОшибке = РезультатРазбора.ПредставлениеОшибки;
			Результат.ТипОшибки = РезультатРазбора.ТипОшибки;
		КонецЕсли;
		...
		ПараметрыСообщения.Вставить("ТекстСообщения", ?(
			ЗначениеЗаполнено(Результат.СообщениеОбОшибке),
...
Функция ПараметрыКомандыПолученияДанныхЭквайера(
	...
	Если ВходящиеПараметры.Свойство("Период") И ЗначениеЗаполнено(ВходящиеПараметры.Период) Тогда  // АПК:1415 Поддержка старых сценариев БЭД.
...
Процедура РазобратьДанныеСервисаJSON(ИмяКоманды, РезультатКоманды, РезультатРазбора, Отказ)
	...
	ИначеЕсли ИмяКоманды = "ДанныеЭквайера" Тогда
		...
		РезультатРазбора.Вставить("НаименованиеПолное",   ДанныеСервиса.fullName);
		...
		РезультатРазбора.Вставить("КодПоОКПО",            ДанныеСервиса.okpo);
	...
	ИначеЕсли ИмяКоманды = "ПолучитьДанныеПоЗаказу_НовыйПротокол" Тогда
		...
		РезультатРазбора.Вставить("СрокЖизниСсылки",           ДанныеСервиса.paymentCapabilityTtl);
		РезультатРазбора.Вставить(
			"Статус",
			ЗначениеСтатусаЗаказаИзЗначенияСервиса(ДанныеСервиса.orderState));
...
Функция ЗначениеСтатусаЗаказаИзЗначенияСервиса(ЗначениеСервиса)
	...
	Если ЗначениеСервиса = "NEW"
		Или ЗначениеСервиса = "UPDATED" Тогда
		Результат = ИдентификаторСтатусаНеОплачен();
	ИначеЕсли ЗначениеСервиса = "PAID" Тогда
		Результат = ИдентификаторСтатусаОплачен();
	ИначеЕсли ЗначениеСервиса = "PAID_WAITING_FOR_RECEIPT" Тогда
		Результат = ИдентификаторСтатусаОплаченОжиданиеЧека();
	ИначеЕсли ЗначениеСервиса = "CANCELED" Тогда
		Результат = ИдентификаторСтатусаОтменен();
	КонецЕсли;
...
Функция ИдентификаторСтатусаОплачен()
	
	Возврат "Оплачен";
	
КонецФункции

Функция ИдентификаторСтатусаОплаченОжиданиеЧека()
	
	Возврат "ОплаченОжиданиеЧека";
	
КонецФункции

Функция ИдентификаторСтатусаОтменен()
...
Процедура СформироватьПараметрыОшибкиЗначенияПараметровКоманды(ИмяКоманды, ОбязательныйПараметр, Отказ)
	...
	ПараметрыСообщения.Вставить("ИмяКоманды", ИмяКоманды);
	ПараметрыСообщения.Вставить("Параметр", ОбязательныйПараметр);
	ОбработатьОшибку("ФормированиеСсылкиОшибкаЗначенияПараметра", ПараметрыСообщения, Отказ);
...
//    * "ФормированиеСсылкиОшибкаЗначенияПараметра".
...
Процедура ОбработатьОшибку(ИдентификаторОшибки, ДополнительныеПараметры = Неопределено, Отказ = Ложь)
	...
	СообщенияПользователю.Вставить(ИдентификаторОшибки, Сообщение.ТекстСообщения);
	ОбщегоНазначения.СкопироватьВБуферОбмена(СообщенияПользователю, ИдентификаторИсточникаОшибок());
...
Функция ТекстыСообщенийОбОшибках(ИдентификаторОшибки, ДополнительныеПараметры = Неопределено)
	...
	ИначеЕсли ИдентификаторОшибки = "ФормированиеСсылкиОшибкаЗначенияПараметра" Тогда
		ОбработатьПараметрыСообщенияОбОшибке(ИдентификаторОшибки, "ИмяКоманды, Параметр", ДополнительныеПараметры);
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			НСтр("ru = 'При формировании запроса обнаружена ошибка в значении параметра.'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ВывестиСообщенияПользователю(СообщенияПользователю)
	
	Для Каждого Сообщение Из СообщенияПользователю Цикл
		
		ТекстСообщения = Сообщение.Значение;
		
		Если ТипЗнч(ТекстСообщения) <> Тип("Строка")
			Или ПустаяСтрока(ТекстСообщения) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОчиститьПереченьОшибок()
	
	ОбщегоНазначения.СкопироватьВБуферОбмена(
		Неопределено,
		ИдентификаторИсточникаОшибок());
	
КонецПроцедуры

Функция РазобратьОшибкуПоДаннымОтвета(КодСостояния, Содержимое)
	
	РезультатОперации = Новый Структура;
	РезультатОперации.Вставить("ПредставлениеОшибки", "");
	РезультатОперации.Вставить("ТипОшибки", "");
	
	Если КодСостояния = 400 Тогда
		РезультатОперации.ПредставлениеОшибки = НСтр("ru = 'Некорректные данные запроса'");
	ИначеЕсли КодСостояния = 401 Тогда
		РезультатОперации.ПредставлениеОшибки = НСтр("ru = 'Ошибка авторизации.'");
	ИначеЕсли КодСостояния = 403 Тогда
		Если СтрНайти(Содержимое, "NO_SHOP_PERMISSION") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'У текущего пользователя Портала нет прав для работы с указанным магазином.'");
		ИначеЕсли СтрНайти(Содержимое, "ACCOUNT_MISMATCH") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'Для указанного идентификатора магазина счета созданы с использованием другой учетной записи Портала 1С:ИТС.'");
		Иначе
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'Недостаточно прав у текущего пользователя Портала.'");
		КонецЕсли;
	ИначеЕсли КодСостояния = 404 И СтрНайти(Содержимое, "ORDER_NOT_FOUND") > 0 Тогда
		РезультатОперации.ПредставлениеОшибки =
			НСтр("ru = 'Счет с указанным идентификатором не найден.'");
	ИначеЕсли КодСостояния = 422 Тогда
		Если СтрНайти(Содержимое, "NO_SUBSCRIBER_FOUND") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'Не удалось определить абонента.'");
		ИначеЕсли СтрНайти(Содержимое, "UNMODIFIABLE_ORDER_PAYMENT_ALREADY_CREATED") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'По счету начат процесс оплаты, изменение невозможно.'");
			РезультатОперации.ТипОшибки = ИдентификаторОшибкиНачатаОплата();
		ИначеЕсли СтрНайти(Содержимое, " ACCOUNT_MISMATCH") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'Счет создан с использованием другой учетной записи Портала,
					|чтение и внесение изменений невозможно.'");
		ИначеЕсли СтрНайти(Содержимое, "ENVIRONMENT_MISMATCH") > 0 Тогда
			РезультатОперации.ПредставлениеОшибки =
				НСтр("ru = 'Несоответствие окружения.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ИдентификаторОшибкиНачатаОплата()
	
	Возврат "НачатаОплата"
	
КонецФункции

Функция ИдентификаторИсточникаОшибок()
	
	Возврат "СообщенияПользователюМодуляОнлайнОплатыСлужебный";
...
Процедура ОбработатьПараметрыСообщенияОбОшибке(Знач ИдентификаторОшибки, Знач ОбязательныеПараметры, Параметры) 
	...
	Для каждого Имя Из МассивОбязательныхПараметров Цикл
		...
		Если Не Параметры.Свойство(Имя) Тогда // АПК:1414 Проверка свойств прикладной логики.
...
Процедура ДополнитьДанныеПоНоменклатуре(ДанныеПоНоменклатуре)
	...
	ВидыСпособаРасчетов = ВидыСпособаРасчетов();
	...
	Для каждого ЭлементКоллекции Из ДанныеПоНоменклатуре Цикл
		...
		ЭлементКоллекции.ПредметРасчетаСтрокой = ?(
			ПредметРасчета = Неопределено,
			"commodity",
			ПредметРасчета);
		
		ПризнакСпособаРасчета = ВидыСпособаРасчетов.Получить(ЭлементКоллекции.ПризнакСпособаРасчета);
		ЭлементКоллекции.СпособРасчетаСтрокой = ?(
...
Функция ВидыСпособаРасчетов()
	
	Результат = Новый Соответствие;
	
	Результат.Вставить(1,  "full_prepayment");
	Результат.Вставить(2,  "full_payment");
