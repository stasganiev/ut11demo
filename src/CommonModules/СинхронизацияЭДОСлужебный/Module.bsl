...
Процедура ПолучитьНовыеЭлектронныеДокументыПоРасписанию() Экспорт
	...
	Попытка
		...
		РезультатЗагрузкиОбработки = ЭлектронныеДокументыЭДО.ЗагрузитьОбработатьДанныеОбъектовКонтейнеров(Результат.ДанныеКонтейнеров,
		...
		Если ЗначениеЗаполнено(РезультатЗагрузкиОбработки.ДокументыДляОбработкиИзвещений) Тогда
			...
			ПараметрыВыполнения.РасшифрованныеМаркеры = РасшифрованныеМаркерыИзКлючейСинхронизации(ДанныеДляСинхронизации.КлючиСинхронизации);
			
			НаборДействий = ПараметрыВыполнения.НаборДействий;
			ИнтерфейсДокументовЭДОКлиентСервер.ДобавитьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Подписать);
			ИнтерфейсДокументовЭДОКлиентСервер.ДобавитьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ПодготовитьКОтправке);
			ИнтерфейсДокументовЭДОКлиентСервер.ДобавитьДействие(НаборДействий, Перечисления.ДействияПоЭДО.Отправить);
			
			ЭлектронныеДокументыЭДО.ВыполнитьДействияЭДО(ПараметрыВыполнения);
		КонецЕсли;
...
Функция РасшифрованныеМаркерыИзКлючейСинхронизации(КлючиСинхронизации)
	
	РасшифрованныеМаркеры = КриптографияБЭДКлиентСервер.НовыйНаборРасшифрованныхДанных();
	
	Если Не ЗначениеЗаполнено(КлючиСинхронизации) Тогда
		Возврат РасшифрованныеМаркеры;
	КонецЕсли;
	
	Для Каждого КлючСинхронизации Из КлючиСинхронизации Цикл
		Сертификат = КлючСинхронизации.Значение.ВыбранныйСертификат;
		ДанныеМаркера = КлючСинхронизации.Значение.МаркерРасшифрованный;
		Если Не ЗначениеЗаполнено(Сертификат)
			Или ДанныеМаркера = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		РасшифрованныйМаркер = КриптографияБЭДКлиентСервер.НовыеДанныеРасшифровки();
		РасшифрованныйМаркер.Сертификат = Сертификат;
		РасшифрованныйМаркер.Данные = ДанныеМаркера;
		РасшифрованныеМаркеры.Вставить(КлючСинхронизации.Ключ, РасшифрованныйМаркер);
	КонецЦикла;
...
Процедура ПодготовитьКонтейнеры(КлючиСинхронизации, КонтекстДиагностики)
	...
	ЭлектронныеДокументыЭДО.ВыполнитьДействияЭДО(ПараметрыДействийПоЭДО, КонтекстДиагностики);
