...
Функция ПараметрыСвязиИдентификаторов(ПараметрыПолучения, КонтекстДиагностики = Неопределено,
	...
	СоставнойЗапрос = Новый Запрос(
		...
		|	ЕСТЬNULL(ПриглашенияНаИдентификаторы.Статус, ЕСТЬNULL(ПриглашенияШироковещательные.Статус,
		|		ЕСТЬNULL(ВТ_ПриглашенияПоОператорам.Статус, &СтатусПриглашенияПустаяСсылка))) КАК СтатусПриглашения,
