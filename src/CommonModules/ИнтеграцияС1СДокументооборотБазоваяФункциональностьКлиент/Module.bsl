...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	НастройкиАвторизацииИзБазы = Новый Структура;
	НастройкиАвторизацииИзБазы.Вставить("ИмяПользователя", Неопределено);
	НастройкиАвторизацииИзБазы.Вставить("ПарольСохранен", Неопределено);
	НастройкиАвторизацииИзБазы.Вставить("Пароль", Неопределено);
	НастройкиАвторизацииИзБазы.Вставить("ИспользуетсяАутентификацияОССтароеЗначение", Неопределено);
	НастройкиАвторизацииИзБазы.Вставить("ИспользуетсяАутентификацияJWTСтароеЗначение", Неопределено);
...
//   Отбор - Произвольный - отбор для выбора из списка.
//
Процедура ВыбратьЗначениеРеквизитаДО(Форма, Элемент, Типы, Представление, ЗначениеРеквизита,
		ЗначениеРеквизитаID, ЗначениеРеквизитаТип, Оповещение, Отбор = Неопределено) Экспорт
	...
	ПараметрыВыбратьЗначение.Вставить("Отбор", Отбор);
...
Процедура ПередВыполнениемКоманды(Источник) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ФормаКлиентскогоПриложения")
			...
			И Источник.ОписаниеОповещенияОЗакрытии.ИмяПроцедуры =
				"СоздатьИнтегрированныйОбъектЗавершение" Тогда
...
Процедура ПроверитьПодключение(ОписаниеОповещения, Владелец, ПовторноПоказыватьОкноАвторизации = Ложь,
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ИмяПользователя", Неопределено);
	ПараметрыОповещения.Вставить("ПарольСохранен", Неопределено);
	ПараметрыОповещения.Вставить("Пароль", Неопределено);
	ПараметрыОповещения.Вставить("ИспользуетсяАутентификацияОССтароеЗначение", Неопределено);
	ПараметрыОповещения.Вставить("ИспользуетсяАутентификацияJWTСтароеЗначение", Неопределено);
	ПрочитатьНастройкиАвторизации(
...
Процедура ЗакончитьРедактированиеФайла(ID, Наименование, Расширение, УникальныйИдентификаторФормы,
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗакончитьРедактированиеСРасширениемПослеВопросаОбОсвобождении",
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент,
		Параметры);
...
Процедура ОткрытьПрисоединенныеФайлы(ВладелецФайла, ТекущийФайл = Неопределено, ТолькоПросмотр = Ложь,
	...
	Параметры.Вставить("ПредставлениеВладельца", ПредставлениеВладельца);
...
Процедура СохранитьИзмененияРедактируемогоФайла(ID, Наименование, Расширение,
	...
	Если Не Файл.Существует() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"СохранитьИзмененияСРасширениемПослеВопросаОбОсвобождении",
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент,
			Параметры);
		ПоказатьВопросДаНет(
			ОписаниеОповещения,
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Если НастройкиЭП.ОписанияПрограмм.Количество() > 1 И ЭтоАдресВременногоХранилища(ДанныеСтроки.АдресПодписи) Тогда
			...
			Если ТипЗнч(ДанныеПодписи) = Тип("ДвоичныеДанные") Тогда
				АлгоритмПодписи = ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмСформированнойПодписи(ДанныеПодписи);
...
//     * УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ДобавитьФайлСДискаКОбъектуИСПослеПоискаОбъектаДО(ОбъектДО, Параметры) Экспорт
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОбъектДО", ОбъектДО);
	ПараметрыОповещения.Вставить("ВладелецФайла", Параметры.ВладелецФайла);
	ПараметрыОповещения.Вставить("УникальныйИдентификаторФормы", Параметры.УникальныйИдентификаторФормы);
	
	Оповещение = Новый ОписаниеОповещения(
		"ДобавитьФайлСДискаКОбъектуИСПослеПроверкиДобавленияСканКопии",
		ЭтотОбъект,
		ПараметрыОповещения);
...
// Продолжает добавление файла с диска после проверки на добавление скан-копии.
//
// Параметры:
//   Результат - КодВозвратаДиалога - ответ пользователя,
...
//       ** СостояниеРазрешаетУдалениеФайла - Булево
//
Процедура ДобавитьФайлСДискаКОбъектуИСПослеПроверкиДобавленияСканКопии(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	
	ЯвляетсяСканКопией = (Параметры.ОбъектДО.СостояниеРазрешаетДобавлениеСканКопии
		И Не Параметры.ОбъектДО.СостояниеРазрешаетДобавлениеФайла);
	
	СоздатьФайлСДиска(
		Параметры.ОбъектДО.ID,
		Параметры.ОбъектДО.type,
		Параметры.УникальныйИдентификаторФормы,
		Строка(Параметры.ВладелецФайла),
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура ЗакончитьРедактированиеСРасширениемПослеВопросаОбОсвобождении(Результат, Параметры) Экспорт
...
Процедура ОткрытьПрисоединенныеФайлыЗавершение(ВладелецФайла, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.Уникальность) Тогда
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура СохранитьИзмененияСРасширениемПослеВопросаОбОсвобождении(Результат, Параметры) Экспорт
...
//   ДополнительныеПараметры - Структура:
//     * ФормаОбъектаИС - РасширениеФормыКлиентскогоПриложенияДляОбъектов - форма, из которой вызвана команда.
//     * ОповещениеПродолжения - ОписаниеОповещения - действие, вызываемое после проверки.
//
...
Процедура ВыбратьЗначениеРеквизитаДОЗавершение(ЗначениеТипа, ПараметрыВыбратьЗначение) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ЗначениеТипа) Тогда
		...
		КонецЕсли;
		Если ПараметрыВыбратьЗначение.Отбор = Неопределено Тогда
			ПараметрыФормы.Вставить("Отбор", Новый Структура);
		Иначе
			ПараметрыФормы.Вставить("Отбор", ПараметрыВыбратьЗначение.Отбор);
		КонецЕсли;
		...
		Если ЗначениеТипа = "DMDocument" Или ЗначениеТипа = "DMCorrespondent" Или ЗначениеТипа = "DMMeeting" Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыВыбратьЗначение.ЗначениеРеквизита) Тогда
				ПараметрыФормы.Отбор.Вставить("name", СокрЛП(ПараметрыВыбратьЗначение.ЗначениеРеквизита));
