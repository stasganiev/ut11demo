...
Функция ДанныеМонитора() Экспорт
	...
	ПодсистемаОбновленияСуществует = ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы");
	
	Если ПодсистемаОбновленияСуществует Тогда
...
Функция НовыйТелоЗапросаСодержимоеМонитора(
	...
	ДанныеЗапроса.Вставить(
		"getUpdateAvailabilityStatus",
		ПараметрыОперации.ПараметрыОбновления <> Неопределено);
	
	Если ПараметрыОперации.ПараметрыОбновления <> Неопределено Тогда
		
		ПараметрыОбновления = ПараметрыОперации.ПараметрыОбновления;
		ДанныеЗапроса.Вставить(
			"getOnlyPlatformUpdateAvailabilityStatus",
			Не ПараметрыОбновления.ПолучатьОбновленияКонфигурации);
		ДанныеЗапроса.Вставить(
			"checkSupportStatus",
			ПараметрыОбновления.ИнформироватьОВыходеВерсийДлительнойПоддержки);
...
Процедура ЗаписатьПараметрыСервисов(ДанныеЗапроса)
	...
	Для Каждого ПараметрыСервиса Из ПараметрыСервисов Цикл
		ДанныеЗапроса.applicationParams.Добавить(Новый Структура(
			...
			ЗначениеВМассив(ПараметрыСервиса.Значение)));
	КонецЦикла;
