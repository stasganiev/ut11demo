...
Процедура ДополнитьПараметрыФормы(ПараметрыФормы, ИсточникПараметров, ИмяОткрываемойФормы) Экспорт
	...
	ДанныеТорговойПлощадки          = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ИсточникПараметров, "ДанныеТорговойПлощадки", Неопределено);
	...
	Если ИмяОткрываемойФормы = "Обработка.ЖурналДокументовПродажи.Форма.СписокДокументов" Тогда
		...
		Если КлючНазначенияФормы = "РаботаСПоставкамиOzon" Тогда
			КлючНазначенияОткрываемойФормы = "РаботаСПоставкамиOzon";
			ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документы поставки <%1> (все)'"), ДанныеТорговойПлощадки.ПостфиксЗаголовкаФормы);
				...
				КонецЕсли;
		ИначеЕсли КлючНазначенияФормы = "РаботаСПродажамиOzon" Тогда
			КлючНазначенияОткрываемойФормы = "РаботаСПродажамиOzon";
			ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документы продажи <%1> (все)'"), ДанныеТорговойПлощадки.ПостфиксЗаголовкаФормы);
		Иначе
			Возврат;
		КонецЕсли;
		...
		Если КлючНазначенияОткрываемойФормы = "РаботаСПоставкамиOzon" Тогда
			Отбор.Вставить("Договор", ДанныеТорговойПлощадки.ДоговорПродажиЧерезСкладыТорговойПлощадки);
		ИначеЕсли КлючНазначенияОткрываемойФормы = "ДокументыПродажиЧерезOzon" Тогда
		...
		Если КлючНазначенияОткрываемойФормы = "РаботаСПоставкамиOzon" Тогда
			ПараметрыФормы.Вставить("ИспользуемыеХозОперации", ИспользуемыеХозОперации.Поставки);
		ИначеЕсли КлючНазначенияОткрываемойФормы = "РаботаСПродажамиOzon" Тогда
			ПараметрыФормы.Вставить("ИспользуемыеХозОперации", ИспользуемыеХозОперации.Продажи);
		КонецЕсли;
	...
	ИначеЕсли ИмяОткрываемойФормы = "Документ.ЗаказКлиента.Форма.ФормаСпискаДокументов" Тогда
		...
		Если КлючНазначенияОткрываемойФормы = "РаботаСПоставкамиOzon" Тогда
			Отбор.Вставить("Договор", ДанныеТорговойПлощадки.ДоговорПродажиЧерезСкладыТорговойПлощадки);
		ИначеЕсли КлючНазначенияОткрываемойФормы = "РаботаСПродажамиOzon" Тогда
			Отбор.Вставить("Договор", ДанныеТорговойПлощадки.ДоговорПродажиЧерезСкладыСобственные);
		КонецЕсли;
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.ЖурналДокументовОтчетыКомиссионеров.Форма.КОформлениюОтчетовКомиссионеров" Тогда
		...
		Если КлючНазначенияФормы = "РаботаСПродажамиOzon"
				Или КлючНазначенияФормы = "ДокументыПоставкиЧерезOzon" Тогда
			КлючНазначенияОткрываемойФормы = "РаботаСПродажамиOzon";
		Иначе
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ИмяОткрываемойФормы = "Документ.ТранспортнаяНакладная.Форма.ФормаСписка" Тогда
		...
		Если КлючНазначенияФормы = "РаботаСПоставкамиOzon" Или КлючНазначенияФормы = "РаботаСПродажамиOzon" Тогда
		...
		Если БыстрыйОтбор.Свойство("Организация") И ЗначениеЗаполнено(БыстрыйОтбор.Организация) Тогда
			ДанныеТорговойПлощадки.Вставить("Организация", БыстрыйОтбор.Организация);
		КонецЕсли;
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.ЖурналДокументовНДС.Форма.СписокДокументов" Тогда
		...
		Если Не БыстрыйОтбор.Свойство("Контрагент") Тогда
			БыстрыйОтбор.Вставить("Контрагент", ДанныеТорговойПлощадки.Контрагент);
		КонецЕсли;
	...
	ИначеЕсли ИмяОткрываемойФормы = "Документ.ПриобретениеУслугПрочихАктивов.ФормаСписка" Тогда
		...
		Если КлючНазначенияФормы = "ВзаиморасчетыOzon" Тогда
			КлючНазначенияОткрываемойФормы = "ВзаиморасчетыOzon";
			ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	ИначеЕсли ИмяОткрываемойФормы = "Документ.ВзаимозачетЗадолженности.ФормаСписка" Тогда
		...
		БыстрыйОтбор.Вставить("КонтрагентДебитор",  ДанныеТорговойПлощадки.Контрагент);
		БыстрыйОтбор.Вставить("КонтрагентКредитор", ДанныеТорговойПлощадки.Контрагент);

		БыстрыйОтборВДинамическийСписок = Истина;

		Отбор.Вставить("КонтрагентДебитор",  ДанныеТорговойПлощадки.Контрагент);
		Отбор.Вставить("КонтрагентКредитор", ДанныеТорговойПлощадки.Контрагент);
	...
	ИначеЕсли ИмяОткрываемойФормы = "Документ.СверкаВзаиморасчетов.ФормаСписка" Тогда
...
//     * ПодсказкаОткрытияФормыСопоставленияДанных  - Строка - имя формы, открываемой для сопоставления загружаемых данных.
...
//
Процедура ОткрытьФормуВыгрузкиЗагрузкиДанных(ПараметрыФормы) Экспорт
