...
// 
// Параметры:
//  ИмяФайлаНаКлиенте - Строка - Имя файла на клиенте
//  ИмяФайлаНаСервере - Строка - Имя файла на сервере
// 
// Возвращаемое значение:
//  Обещание - Поместить файл на сервер
Асинх Функция ПоместитьФайлНаСервер(ИмяФайлаНаКлиенте, ИмяФайлаНаСервере) Экспорт
	...
	ЭтоФайл = Ждать ОбъектФС.ЭтоФайлАсинх();
	
	Если Не ЭтоФайл Тогда
		Возврат Ложь;
	КонецЕсли;
	
	РазмерФайла = Ждать ОбъектФС.РазмерАсинх();
	Позиция = 0;
	
	Поток = Ждать ФайловыеПотоки.ОткрытьАсинх(ИмяФайлаНаКлиенте, РежимОткрытияФайла.Открыть, ДоступКФайлу.Чтение);
	Буфер = Новый БуферДвоичныхДанных(ФайлыБТСКлиентСервер.РазмерПорцииОбработки(РазмерФайла));
	
	Пока Позиция < РазмерФайла Цикл
		
		КоличествоПрочитанныхБайт = Ждать Поток.ПрочитатьАсинх(Буфер, 0, Буфер.Размер);
		Позиция = Поток.ТекущаяПозиция();
		Буфер = ?(Буфер.Размер = КоличествоПрочитанныхБайт, Буфер, Буфер.Прочитать(0, КоличествоПрочитанныхБайт));
		
		ФайлыБТСВызовСервера.ОтправитьОчереднуюПорцию(
			ИмяФайлаНаСервере,
			ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер),
			Позиция - КоличествоПрочитанныхБайт);
