...
Функция СменаАктивна(КассоваяСмена) Экспорт
	...
	ИначеЕсли ОписаниеСмены.Статус = Перечисления.СтатусыКассовойСмены.Открыта
		И ОписаниеСмены.ДатаИстеченияСрокаДействия > ОбщегоНазначенияБПО.ДатаСеанса() Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция СтатусПоследнейСмены(ФискальноеУстройство) Экспорт
	...
	Если Не (ОписаниеПоследнейСмены = Неопределено) Тогда
		...
		РезультатОперации.Активна = РезультатОперации.Открыта И ОписаниеПоследнейСмены.ДатаИстеченияСрокаДействия > ОбщегоНазначенияБПО.ДатаСеанса();
		
		ФискальноеУстройствоТипОборудования = ОбщегоНазначенияБПО.ЗначениеРеквизитаОбъекта(ФискальноеУстройство, "ТипОборудования");
		Если ФискальноеУстройствоТипОборудования <> Перечисления.ТипыПодключаемогоОборудования.ККТ Тогда
			РезультатОперации.ТекущийНомерЧека = ПолучитьТекущийНомерЧекаККТ(ФискальноеУстройство, ОписаниеПоследнейСмены.КассоваяСмена);
		КонецЕсли;
		КассоваяСменаСсылка = ОписаниеПоследнейСмены.КассоваяСмена;
		КассоваяСменаРеквизиты = ОбщегоНазначенияБПО.ЗначенияРеквизитовОбъекта(КассоваяСменаСсылка, "Номер, НомерСменыККТ");
		РезультатОперации.КассоваяСмена = КассоваяСменаСсылка;
		РезультатОперации.НомерСмены = КассоваяСменаРеквизиты.Номер;
		РезультатОперации.НомерСменыККТ = КассоваяСменаРеквизиты.НомерСменыККТ;
...
Функция ОписаниеПоследнейКассовойСмены(ФискальноеУстройство) Экспорт
	...
	Если ТипЗнч(ФискальноеУстройство) = Тип("СправочникСсылка.ПодключаемоеОборудование") Тогда
		Запрос.УстановитьПараметр("ФискальноеУстройство", ФискальноеУстройство);
	Иначе
		Запрос.УстановитьПараметр("КассаККМ", ФискальноеУстройство);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ФискальноеУстройство", "КассаККМ");
	КонецЕсли;
	Результат = Запрос.Выполнить().Выгрузить();
...
Функция НоваяКассоваяСмена(ФискальноеУстройство, ПараметрыКоманды) Экспорт
	...
	КассоваяСменаОбъект.Дата = ОбщегоНазначенияБПО.ДатаСеанса();
	...
	Если ТипЗнч(ФискальноеУстройство) = Тип("СправочникСсылка.ПодключаемоеОборудование") Тогда
		КассоваяСменаОбъект.Организация = ОбщегоНазначенияБПО.ЗначениеРеквизитаОбъекта(ФискальноеУстройство, "Организация");
	Иначе
...
Процедура ЗакрытьКассовуюСмену(ПараметрыКоманды) Экспорт
	...
	КассоваяСменаОбъект = ПараметрыКоманды.КассоваяСмена.ПолучитьОбъект(); // ДокументОбъект.КассоваяСмена
	КассоваяСменаОбъект.Статус = Перечисления.СтатусыКассовойСмены.Закрыта;
	КассоваяСменаОбъект.ОкончаниеКассовойСмены = ОбщегоНазначенияБПО.ДатаСеанса();
