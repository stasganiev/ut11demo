...
Процедура ОбслуживаниеОбластейДанных() Экспорт
	...
	РегистрыСведений.ДанныеОчищаемойОбласти.ВосстановитьИнформациюОбУдаленныхОбластях();
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭтоОчисткаОбласти = Менеджер.Статус = Перечисления.СтатусыОбластейДанных.КУдалению;
		
		Если ЭтоОчисткаОбласти Тогда 
			ИмяМетода = ИмяМетодаОчиститьОбластьДанных();
		ИначеЕсли Менеджер.Статус = Перечисления.СтатусыОбластейДанных.Новая Тогда
		...
		Если Менеджер.Повтор < МаксимальноеКоличествоПовторов Тогда
			...
			Если ЭтоОчисткаОбласти Тогда
				...
				СоздатьЗаданиеОчисткиОбластиДанных(Выборка.ОбластьДанных);
...
Функция АдресПровайдераАутентификации() Экспорт
	...
	Возврат РаботаВМоделиСервисаБТСПовтИсп.АдресПровайдераАутентификации();
...
Функция АутентификацияТокеномДоступаВнутреннихВызововПрограммногоИнтерфейса() Экспорт
	...
	Возврат РаботаВМоделиСервисаБТСПовтИсп.АутентификацияТокеномДоступаВнутреннихВызововПрограммногоИнтерфейса();
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "АдресПровайдераАутентификации");
	ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров, "АутентификацияТокеномДоступаВнутреннихВызововПрограммногоИнтерфейса");
	...
	ДобавитьКонстантуВТаблицуПараметровИБ(
		ТаблицаПараметров,
		"НачалоИнтервалаВремениПриоритетаОбработкиДанных");
	ДобавитьКонстантуВТаблицуПараметровИБ(
		ТаблицаПараметров,
		"ОкончаниеИнтервалаВремениПриоритетаОбработкиДанных");
	
	ИмяКонстантыКоличествоПотоковОбновления = "КоличествоПотоковОбновленияИнформационнойБазы";
	Если Метаданные.Константы.Найти(ИмяКонстантыКоличествоПотоковОбновления) <> Неопределено Тогда
		ДобавитьКонстантуВТаблицуПараметровИБ(
			ТаблицаПараметров,
			ИмяКонстантыКоличествоПотоковОбновления);
	КонецЕсли;
...
Процедура ПриЗаполненииТиповИсключаемыхИзПроверкиУстраненияПроблемы(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.Справочники.ВерсииПоставляемыхРасширений);
	...
	Типы.Добавить(Метаданные.Справочники.ОбработчикиОчередиЗаданий);
	Типы.Добавить(Метаданные.Справочники.ОбщиеДанныеИнформационногоЦентра);
	Типы.Добавить(Метаданные.Справочники.ОповещенияПользователей);
	...
	Типы.Добавить(Метаданные.Справочники.ПоставляемыеРасширения);
		...
		Исключение
			...
			Если ТекущаяПопытка = КоличествоПопыток Тогда
				...
				ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонКомментария, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Удаляет все данные области данных, устанавливает для области данных статус Удалена,
// отправляет в менеджер сервиса сообщение с информацией об изменении статуса области.
// После этого область данных будет непригодна к использованию.
//
...
//
Процедура ОчиститьОбластьДанных(ОбластьДанных) Экспорт
	...
	Попытка
		...
		МенеджерЗаписи = ПолучитьМенеджерЗаписиОбластиДанных(
			ОбластьДанных,
			Перечисления.СтатусыОбластейДанных.КУдалению);
...
Процедура ОчиститьДанныеОбласти() Экспорт
	
	Если РазделениеВключено() Тогда
		
		УдалитьДанныеПриложения();
		
	Иначе
		
		ОчиститьДанныеИнформационнойБазы(Истина);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ОчиститьДанныеИнформационнойБазы(УдалятьРасширения, УдалятьТекущегоПользователя = Истина) Экспорт
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение  НСтр("ru = 'В информационной базе включено разделение, допустимо использование метода РаботаВМоделиСервиса.ОчиститьДанныеОбласти'");	
	КонецЕсли;
...
Процедура СоздатьЗаданиеОчисткиОбластиДанных(ОбластьДанных) Экспорт
	
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ОбластьДанных);
	
	ПараметрыЗадания = Новый Структура();
	ПараметрыЗадания.Вставить("ИмяМетода", ИмяМетодаОчиститьОбластьДанных());
	ПараметрыЗадания.Вставить("Параметры", ПараметрыМетода);
	ПараметрыЗадания.Вставить("Ключ", "1");
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(ИмяМетодаОчиститьОбластьДанных());
...
Процедура ПриОпределенииИсключенийНеразделенныхДанных(Исключения) Экспорт
	...
	Исключения.Добавить(Метаданные.РегистрыСведений.ДанныеОчищаемойОбласти);
...
Функция ИмяМетодаОчиститьОбластьДанных() Экспорт
	
	Возврат "РаботаВМоделиСервиса.ОчиститьОбластьДанных";
...
Процедура УдалитьПользователейОбласти(УдалятьТекущегоПользователя = Истина) Экспорт
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Если ПервыйАдминистратор = Неопределено
			И Пользователи.ЭтоПолноправныйПользователь(ПользовательИБ, Истина, Ложь) Тогда
...
//   ИДФайлаВыгрузки - УникальныйИдентификатор - ид файла начальных данных.
//   Вариант - Строка - вариант начальных данных.
//   СообщениеОФатальнойОшибке - Строка - возращаемый параметр.
//
// Возвращаемое значение:
//  Строка - один из вариантов "Успех", "ФатальнаяОшибка".
//
...
Процедура УдалитьДанныеПриложения(ОбластьДанных = Неопределено)
	
	Если Не ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru = 'Удаление данных приложения может быть выполнено только при включенном использовании разделителей'");
	КонецЕсли;
	...
	УстановленВнешнийМонопольныйРежим = МонопольныйРежим();
	МонопольныйРежимУстановлен = Ложь;
	
	Попытка
	
		Если Не УстановленВнешнийМонопольныйРежим Тогда
			УстановитьМонопольныйРежим(Истина);
			МонопольныйРежимУстановлен = Истина;
		КонецЕсли;
		
		ПередУдалениемДанныхПриложения(ОбластьДанных, СохраняемыеДанные);
		
		Если РасширенияВМоделиСервиса.ЕстьУстановленныеРасширенияИзменяющиеСтруктуруДанных() Тогда
			
			// Отдельное удаление расширение и последующий старт фонового для очистки данных приложения
			// необходим для обхода ошибки платформы из-за которой не очищаются данные расширенных таблиц
			УдалитьРасширенияОбласти();
			УдалитьДанныеПриложенияВФоне(ОбластьДанных);
			
		Иначе
			
			ВыполнитьУдалениеДанныхПриложения();
			
		КонецЕсли;
		
		ПослеУдаленияДанныхПриложения(ОбластьДанных, СохраняемыеДанные);
...
Процедура УдалитьДанныеПриложенияВФоне(ОбластьДанных)
	
	ОбластьДанныхСтрокой = Формат(ОбластьДанных, "ЧГ=0;");
	Задание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений(
		"РаботаВМоделиСервиса.ВыполнитьУдалениеДанныхПриложения",
		,
		ОбластьДанныхСтрокой,
		СтрШаблон(НСтр("ru = 'Очистка данных приложения %1'"), ОбластьДанныхСтрокой));
	Задание = Задание.ОжидатьЗавершенияВыполнения();
	
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение НСтр("ru = 'Задание очистки данных приложения отменено'");
...
Процедура ВыполнитьУдалениеДанныхПриложения() Экспорт
	
	УдалитьДанныеИнформационнойБазы();
	УдалитьПользователейОбласти();
	
КонецПроцедуры

Процедура ПередУдалениемДанныхПриложения(ОбластьДанных, СохраняемыеДанные)
	
	Если Не РегистрыСведений.ДанныеОчищаемойОбласти.ДанныеОбластиСохранены(ОбластьДанных) Тогда
		РегистрыСведений.ДанныеОчищаемойОбласти.СохранитьДанныеОбласти(ОбластьДанных);
	КонецЕсли;
	
	СохраняемыеДанные.Вставить("КлючОбластиДанных", Константы.КлючОбластиДанных.Получить());
	
КонецПроцедуры

Процедура ПослеУдаленияДанныхПриложения(ОбластьДанных, СохраняемыеДанные)
	
	Константы.КлючОбластиДанных.Установить(СохраняемыеДанные.КлючОбластиДанных);
	
	МодельДанных = РаботаВМоделиСервисаПовтИсп.ПолучитьМодельДанныхОбласти();
	
	Для Каждого ЭлементМодели Из МодельДанных Цикл
		
		Если ЭтоПолноеИмяОбъектаСПредопределеннымиДанными(ЭлементМодели.Ключ) Тогда
			ОтключитьИнициализациюПредопределенныхДанных(ЭлементМодели.Ключ);
		КонецЕсли;
		
	КонецЦикла;
	
	РегистрыСведений.ДанныеОчищаемойОбласти.ВосстановитьДанныеОбласти(ОбластьДанных);
	
КонецПроцедуры

Процедура ОтключитьИнициализациюПредопределенныхДанных(ПолноеИмяОбъектаМетаданных)
