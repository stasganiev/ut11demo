...
// 	см. СервисЭДОКлиентСервер.НовыеПараметрыРегистрацииВСервисе1СЭДО
Функция НовыеПараметрыРегистрацииВСервисе1СЭДО() Экспорт
	Возврат СервисЭДОКлиентСервер.НовыеПараметрыРегистрацииВСервисе1СЭДО();
КонецФункции
...
Процедура ЗаполнитьПодписантаДанныхДляОператораЭДО(РезультатВыбораСертификата, ПараметрыРегистрации) Экспорт
	...
	ПараметрыРегистрации.СертификатПодписанта = РезультатВыбораСертификата.ОписаниеДанных.ВыбранныйСертификат.Ссылка;
	...
	РезультатВыбораСертификата.ОписаниеДанных.ТекущийЭлементНабораДанных.ДанныеПакетаСЗаполненнымПодписантом =
		...
		Если ДанныеДляРегистрационногоПакета.Свойство("ТекстОшибки") И ЗначениеЗаполнено(ДанныеДляРегистрационногоПакета.ТекстОшибки) Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru ='При формировании данных регистрационного пакета возникла ошибка: %1'"),
				ДанныеДляРегистрационногоПакета.ТекстОшибки);
			ОбработкаНеисправностейБЭДВызовСервера.ОбработатьОшибку(НСтр("ru ='Подготовка регистрационного пакета'"),
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().РаботаСФайлами, ТекстОшибки);
		ОписаниеДанных = Новый Структура("ОписаниеОшибки", ТекстОшибки);
	Иначе
		ОписаниеДанных = Новый Структура("Данные", ДанныеДляРегистрационногоПакета.ДвоичныеДанныеДляОператораЭДО);
	КонецЕсли;	
