...
Процедура ЗаполнитьНаименованиеРабочегоМеста(Объект, ИмяПользователя) Экспорт
	...
	Если ПустаяСтрока(Объект.Наименование) Тогда
		...
		ИмяПользователя = ?(ПустаяСтрока(ИмяПользователя), ИмяПустойПользователь, Строка(ИмяПользователя));
		РабочееМестоИмя = ?(ПустаяСтрока(Объект.ИмяКомпьютера), Объект.Код, Объект.ИмяКомпьютера);
		Наименование = СтрШаблон(НСтр("ru = '%1(%2)'"), ИмяПользователя, РабочееМестоИмя);
...
//   * ПодключенноеУстройство - Неопределено, см. МенеджерОборудованияКлиент.ПараметрыПодключенияУстройства.
...
Функция ПараметрыВыполненияОперацииНаОборудовании(Результат = Ложь, ОписаниеОшибки = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт; 
	...
	РезультатВыполнения.Вставить("ИдентификаторОбъекта"   , "");
...
Функция ПараметрыПодключения(ДанныеДрайвера) Экспорт
	...
	Подключение.Вставить("ТипыОборудования"        , ДанныеДрайвера.ТипыОборудования);   
	Подключение.Вставить("ТипОборудования"         , ДанныеДрайвера.ТипОборудования);
	Подключение.Вставить("ТипОборудованияИмя"      , ДанныеДрайвера.ТипОборудованияИмя);
...
Функция МеткиRFID(ДанныеМеток) Экспорт
	...
	Возврат МенеджерОборудованияВызовСервера.МеткиRFID(ДанныеМеток);
	#Иначе
	
	Результат = Новый Массив();
	Если Не ПустаяСтрока(ДанныеМеток) Тогда
...
//
// См. ОбщегоНазначенияБПО.ОбщийМодуль
//
// Параметры:
...
//
// Пример:
//	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
//		МодульОбновлениеКонфигурации = ОбщегоНазначенияБПО.ОбщийМодуль("ОбновлениеКонфигурации");
//		МодульОбновлениеКонфигурации.<Имя метода>();
//	КонецЕсли;
//
//	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.ПолнотекстовыйПоиск") Тогда
//		МодульПолнотекстовыйПоискСервер = ОбщегоНазначенияБПО.ОбщийМодуль("ПолнотекстовыйПоискСервер");
...
Функция ДатаСеанса() Экспорт       
	...
	Возврат ОбщегоНазначенияБПОКлиентСервер.ДатаСеанса();   
