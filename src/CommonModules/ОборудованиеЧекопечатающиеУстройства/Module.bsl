...
Функция ПараметрыРегистрацииУстройства(Идентификатор) Экспорт
	...
	Если ЕстьДанные Тогда
		...
		Если ДанныеХранения.Свойство("ПризнакАвтоматическогоТорговогоАвтомата") И НЕ ПустаяСтрока(ДанныеХранения.ПризнакАвтоматическогоТорговогоАвтомата) Тогда
			РезультатОперации.ПризнакАвтоматическогоТорговогоАвтомата = ПреобразоватьВБулево(ДанныеХранения.ПризнакАвтоматическогоТорговогоАвтомата); 
		КонецЕсли;
		Если ДанныеХранения.Свойство("ПризнакОказанииУслугОбщественногоПитания") И НЕ ПустаяСтрока(ДанныеХранения.ПризнакОказанииУслугОбщественногоПитания) Тогда
			РезультатОперации.ПризнакОказанииУслугОбщественногоПитания = ПреобразоватьВБулево(ДанныеХранения.ПризнакОказанииУслугОбщественногоПитания); 
		КонецЕсли;
		Если ДанныеХранения.Свойство("ПризнакОптовойТорговлиСОрганизациями") И НЕ ПустаяСтрока(ДанныеХранения.ПризнакОптовойТорговлиСОрганизациями) Тогда
			РезультатОперации.ПризнакОптовойТорговлиСОрганизациями = ПреобразоватьВБулево(ДанныеХранения.ПризнакОптовойТорговлиСОрганизациями); 
		КонецЕсли;
...
Процедура ЗаписатьФискальнуюОперацию(ПараметрыФискализации) Экспорт
	...
	Если ПараметрыФискализации.Свойство("ЗаписьОперацииКМ") Тогда
		...
		Если ПараметрыФискализации.ЗаписьОперацииКМ.ВыполнялосьПодтврждениеПокупки Тогда
				...
				Если ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					Пока ЧтениеXML.Прочитать() И НЕ (ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента) Цикл
						...
						Если ЧтениеXML.Имя = "IndustryAttribute" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда   
							Если ФискальнаяСтрока <> Неопределено Тогда
								ФискальнаяСтрока.ОтраслевойРеквизит.ИдентификаторФОИВ = ЗначениеXMLАтрибута(ЧтениеXML, "IdentifierFOIV");
...
Функция СтатусыЧекаВОчереди() Экспорт
	...
	Соответствие.Вставить("Фискализирован" , ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Фискализирован"));
	Соответствие.Вставить("Ошибка"         , ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Ошибка"));
	Соответствие.Вставить("Новый"          , ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Новый"));
	Соответствие.Вставить("Фискализируется", ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Фискализируется"));
	Соответствие.Вставить("Отменен"        , ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Отменен"));
...
// Возвращает название организации по сслыке типа ОпределяемыеТипы.Организация
//
// Параметры:
...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.ВерсияДрайвера = "4.2.3.5";
	...
	Драйвер.ВерсияДрайвера = "1.1.1.0";             
	...
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ;
	Драйвер.ИмяДрайвера  = "ДрайверШтрихМККТ54ФЗ4ФФД12NG";
	Драйвер.Наименование = НСтр("ru = 'ШТРИХ-М:ККТ с передачей данных в ОФД (ФФД 1.2) NG'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "ShtrihM_fr_drv_ng_V1"; 
	...
	Драйвер.Наименование = НСтр("ru = 'ККС:ККТ с передачей данных в ОФД (ФФД 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.ВерсияДрайвера = "1.6.3.0"; 
	...
	Драйвер.ВерсияДрайвера = "2.2.0.4";
	...
	Драйвер.Наименование = НСтр("ru = 'Инитпро:ККТ с передачей данных в ОФД (ФФД 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Эвотор:ККТ с передачей данных в ОФД (ФФД 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Инкотекс:ККТ с передачей данных в ОФД (ФФД 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "INCOTEX_KKT_FFD12";   
	
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ;
	Драйвер.ИмяДрайвера  = "ДрайверНЕВА54ФЗ4ФФД12";
	Драйвер.Наименование = НСтр("ru = 'НЕВА:ККТ с передачей данных в ОФД (ФФД 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "NevaFiscalCoreCEL";        
	Драйвер.ВерсияДрайвера = "11.2.38.1015";
	// -- НеМобильноеПриложение                  
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "CheckFiscalization" Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыОперации.УстройствоПечати) Тогда
	...
	ИначеЕсли Команда = "CheckKM" Тогда
		...
		Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
			...
			Для Каждого Операция Из ДанныеОперации.ЗапросыКМ Цикл
				...
				Операция.Вставить("ПараметрыXML", МодульМенеджерОборудованияМаркировка.СформироватьXMLДляЗапросаКМ(Операция));
...
Процедура ОбработатьДанныеОперации(ПараметрыПодключения, Команда, РезультатВыполнения, ДанныеОперации) Экспорт
	...
	ИначеЕсли Команда = "RequestKM" Тогда
		...
		Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
			МодульМенеджерОборудованияМаркировка = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОборудованияМаркировка");
			ДанныеСтруктура = МодульМенеджерОборудованияМаркировка.ПолучитьРезультатыЗапросаКМИзXMLПакета(РезультатВыполнения.РезультатXML);
...
Процедура ИнкрементироватьТекущийНомерЧекаККТ(ФискальноеУстройство, КассоваяСмена) 
	...
	Если ИспользуетсяКассовыеСмены() Тогда
		МодульКассовыеСмены = ОбщегоНазначенияБПО.ОбщийМодуль("КассовыеСмены");
		МодульКассовыеСмены.ИнкрементироватьТекущийНомерЧекаККТ(ФискальноеУстройство, КассоваяСмена);
	КонецЕсли;
...
Процедура ВывестиШапкуЧека(Контекст, ОбщиеПараметры, ТипДокумента)
	...
	ДатаВремя = ?(ПустаяСтрока(ОбщиеПараметры.ДатаВремя), 
		ОбщегоНазначенияБПО.ДатаСеанса(),
	...
	Для Каждого ТекущаяПозицияЧека Из ОбщиеПараметры.ПозицииЧека Цикл
		...
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			ИначеЕсли ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.Свойство("РеквизитКодаТовара") Тогда 
				...
				Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
					МодульМенеджерОборудованияМаркировка = ОбщегоНазначенияБПО.ОбщийМодуль("МенеджерОборудованияМаркировка");
					КодТовараИдентифицируетЭкземпляр = МодульМенеджерОборудованияМаркировка.КодТовараИдентифицируетЭкземпляр(ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.РеквизитКодаТовара, Штрихкод);
...
Процедура ВывестиТаблицуОплат(Контекст, ОбщиеПараметры)
	...
	// АПК: 1299-выкл особенность форматирования в чеках
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 20%'"),     Итоги.СуммаПозицийНДС20,  Итоги.СуммаНДС20, 20/120);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 18%'"),     Итоги.СуммаПозицийНДС18,  Итоги.СуммаНДС18, 18/118);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 10%'"),     Итоги.СуммаПозицийНДС10,  Итоги.СуммаНДС10, 10/110);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 20/120'"), Итоги.СуммаПозицийНДС120, Итоги.СуммаНДС120, 20/120);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 18/118'"), Итоги.СуммаПозицийНДС118, Итоги.СуммаНДС118, 18/118);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru=' СУММА НДС 10/110'"), Итоги.СуммаПозицийНДС110, Итоги.СуммаНДС110, 10/110);
