...
// Открывает форму вывода форматированной строки.
// 
// Параметры:
//  ФорматированнаяСтрока - ФорматированнаяСтрока - строка, которую необходимо вывести на форму
//  ЗаголовокФормы - Строка - заголовок, который необходимо задать форме
//  ОбработчикОповещения - Неопределено, ОписаниеОповещения - оповещение, которой необходимо вызвать 
//   при закрытии формы
//
Процедура ПоказатьФорматированнуюСтроку(Знач ФорматированнаяСтрока, Знач ЗаголовокФормы,
	ОбработчикОповещения = Неопределено) Экспорт
	
	ПараметрыФормы = Новый Структура("ФорматированнаяСтрока, Заголовок", ФорматированнаяСтрока, ЗаголовокФормы);
	ОткрытьФорму("Обработка.ОбщегоНазначенияБЭД.Форма.ВыводФорматированнойСтроки", ПараметрыФормы,,,,, 
...
// * ФункцияПроверкиВвода - Строка - Функция, которая будет вызываться для проверки введенного значения. Пример:
...
//			
//		КонецФункции
// * КомментарийОшибкиПроверкиВвода - Строка - если указан, будет отображен пользователю при отрицательном результате
// 		проверки ввода. Работает только при использовании функции проверки ввода.
Функция ПараметрыВводаСтроки() Экспорт
	...
	Параметры.Вставить("ФункцияПроверкиВвода", "");
	Параметры.Вставить("КомментарийОшибкиПроверкиВвода", "");
...
// Возвращает массив открытых форм окна клиентского приложения по имени, при условии, что форма открыта
// 
// Параметры:
//  ИмяФормы - Строка - Путь к форме, поиск которой производится среди открытых окон приложения
// 
// Возвращаемое значение:
//  Массив Из ФормаКлиентскогоПриложения
Функция ОткрытаяФормаОкнаПриложения(ИмяФормы) Экспорт
	...
	Для Каждого ОкноПриложения Из ПолучитьОкна() Цикл
		Для Каждого ФормаОкна Из ОкноПриложения.Содержимое Цикл
			Если ТипЗнч(ФормаОкна) = Тип("ФормаКлиентскогоПриложения")
				И ФормаОкна.ИмяФормы = ИмяФормы И ФормаОкна.Открыта() Тогда
				ОткрытыеФормы.Добавить(ФормаОкна);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Возврат ОткрытыеФормы;
...
// Обрабатывает оповещение через интервал.
// 
// Параметры:
//  Интервал   - Число
...
//             - Произвольный
Процедура ОбработатьОповещениеЧерезИнтервал(Интервал, Оповещение, Результат = Неопределено) Экспорт
	
	АктивныеОповещения = ПараметрыПриложения[ИмяГлобальногоПараметраАктивныеОповещения()];
	Если АктивныеОповещения = Неопределено Тогда
		АктивныеОповещения = Новый Массив;
		ПараметрыПриложения.Вставить(ИмяГлобальногоПараметраАктивныеОповещения(), АктивныеОповещения);
	КонецЕсли;
	
	ПараметрыОбработки = НовыеПараметрыОбработкиОповещенияЧерезИнтервал();
	ПараметрыОбработки.Начало = Интервал * 1000 + ТекущаяУниверсальнаяДатаВМиллисекундах();
	ПараметрыОбработки.Оповещение = Оповещение;
	ПараметрыОбработки.Результат = Результат;
	
	АктивныеОповещения.Добавить(ПараметрыОбработки);
	
	ПодключитьОбработчикОжидания("КонтрольОбработкиОповещенийЧерезИнтервал", Интервал, Истина);
	
КонецПроцедуры

// Обработывает активные оповещения, добавленные методом см. ОбработатьОповещениеЧерезИнтервал
Процедура ОбработатьАктивныеОповещения() Экспорт
	
	АктивныеОповещения = ПараметрыПриложения[ИмяГлобальногоПараметраАктивныеОповещения()]; // Массив из см. НовыеПараметрыОбработкиОповещенияЧерезИнтервал
	Если Не ЗначениеЗаполнено(АктивныеОповещения) Тогда
		Возврат;
	КонецЕсли;
	
	Количество = АктивныеОповещения.Количество();
	Обработано = 0;
	Индекс = 0;
	
	Пока Обработано < Количество Цикл
		Обработано = Обработано + 1;
		
		ПараметрыОбработки = АктивныеОповещения[Индекс];
		Если ПараметрыОбработки.Начало <= ТекущаяУниверсальнаяДатаВМиллисекундах() Тогда
			ВыполнитьОбработкуОповещения(ПараметрыОбработки.Оповещение, ПараметрыОбработки.Результат);
			АктивныеОповещения.Удалить(Индекс);
		Иначе
			Индекс = Индекс + 1;
		КонецЕсли;
...
// Возвращает пустые параметры обработки оповещения через интервал.
// 
// Возвращаемое значение:
//  Структура:
//  * Начало - Число
...
//  * Результат - Неопределено
//             - Произвольный
Функция НовыеПараметрыОбработкиОповещенияЧерезИнтервал()
	Параметры = Новый Структура;
	Параметры.Вставить("Начало", 0);
	Параметры.Вставить("Оповещение", Новый ОписаниеОповещения);
	Параметры.Вставить("Результат", Неопределено);
	Возврат Параметры;
КонецФункции

// Возвращает имя глобального параметра набора активных оповещений для обработки через интервал.
// 
// Возвращаемое значение:
//  Строка
Функция ИмяГлобальногоПараметраАктивныеОповещения()
	Возврат "ЭлектронноеВзаимодействие.БазоваяФункциональность.АктивныеОповещения";
КонецФункции
