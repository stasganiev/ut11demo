...
Функция ТекстНачальныеОстаткиПартийТоваров() // использует ЕстьДекларации, ЕстьУведомленияОВвозе
	Возврат 
		"ВЫБРАТЬ
...
Функция ТекстКонечныеОстаткиПартийТоваров() // использует ЕстьДекларации, ЕстьУведомленияОВвозе
	Возврат 
		"ВЫБРАТЬ
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ВозвратныеОтходы
	Возврат 
		...
		|		КОГДА Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
		|		КОГДА Т.ВидДеятельностиНДСДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
		...
		|			ТОГДА Т.КорАналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС
		|		КОГДА Т.КорРазделУчета В (&ЗабалансовыеРазделыУчета)
		|		  И НЕ ЕСТЬNULL(Т.КорВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) В
...
Функция ТекстВыручкаСохраняемыеДвижения()
	Возврат 
		...
		|		ИЛИ Т.Регистратор ССЫЛКА Документ.ВводОстатковОПродажахЗаПрошлыеПериоды
		|		ИЛИ Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОприходованиеПоВозврату))
		|	И Т.Активность
		|	И НЕ Т.РасчетПартий
		|	И НЕ Т.РасчетСебестоимости
		|";
