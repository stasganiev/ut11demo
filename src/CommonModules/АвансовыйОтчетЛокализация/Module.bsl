...
Функция ПолучитьДанныеДляПечатнойФормыИзвещения(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Запрос.Текст = ТекстЗапросаПечати(Ложь) + "
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаПечати(ИспользуетсяРеглУчет)
	...
	|	ВЫБОР
	|		КОГДА ДанныеРегистра.Регистратор ССЫЛКА Документ.АвансовыйОтчет
	|			ТОГДА ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.АвансовыйОтчет).МоментВремени
	|		КОГДА ДанныеРегистра.Регистратор ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|			ТОГДА ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.ПриобретениеТоваровУслуг).АвансовыйОтчет.МоментВремени
	|		КОГДА ДанныеРегистра.Регистратор ССЫЛКА Документ.Бронирование
	|			ТОГДА ВЫРАЗИТЬ(ДанныеРегистра.Регистратор КАК Документ.Бронирование).АвансовыйОтчет.МоментВремени
	...
	|	ВТПериоды.Ссылка,
	|	ВТПериоды.МоментВремени КАК МоментВремениТекущийОтчет,
	...
	|			И (Документ.МоментВремени < ВТПериоды.МоментВремени)
	|			И Документ.Проведен
	|СГРУППИРОВАТЬ ПО
	|	ВТПериоды.Организация,
	|	ВТПериоды.ПодотчетноеЛицо,
	...
	|	ВложенныйЗапрос.МоментВремениТекущийОтчет КАК МоментВремениТекущийОтчет,
	|	ВложенныйЗапрос.ПредыдущаяСсылка.МоментВремени КАК МоментВремениПредыдущийОтчет
	...
	|		ВТПериоды.Подразделение,
	|		ВТПериоды.Ссылка,
	|		ВТПериоды.МоментВремениТекущийОтчет) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ // 3
	...
	|	СУММА(ВЫБОР
	|		КОГДА ДенежныеСредстваВыдано.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			И ДенежныеСредстваВыдано.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КонвертацияВалютыПодотчетнымЛицом) ТОГДА
	|			ЕСТЬNULL(ДенежныеСредстваВыдано.Сумма, 0)
	|		ИНАЧЕ
	|			0
	...
	|	Израсходовано.ЦельВыдачи
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ // 5 КонечныеОстатки
	|	АвансовыйОтчетНаПечатьВТ3.Ссылка,
	...
	|	ДенежныеСредства.Валюта,
	|	СУММА(ВЫБОР
	|		КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ЕСТЬNULL(ДенежныеСредства.Сумма, 0) > 0 
	|				ИЛИ ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И ЕСТЬNULL(ДенежныеСредства.Сумма, 0) < 0 ТОГДА
	|			ДенежныеСредства.Сумма
	|		ИНАЧЕ
	|			-ДенежныеСредства.Сумма
	|	КОНЕЦ) КАК СуммаКонечныйОстаток,
	|	СУММА(ВЫБОР
	|		КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ЕСТЬNULL(ДенежныеСредства.СуммаРегл, 0) > 0 
	|				ИЛИ ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И ЕСТЬNULL(ДенежныеСредства.СуммаРегл, 0) < 0 ТОГДА
	|			ДенежныеСредства.СуммаРегл
	|		ИНАЧЕ
	|			-ДенежныеСредства.СуммаРегл
	|	КОНЕЦ) КАК СуммаРеглКонечныйОстаток
	|ПОМЕСТИТЬ КонечныеОстатки
	|ИЗ
	|	АвансовыйОтчетНаПечатьВТ3
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		И ДенежныеСредства.Подразделение = АвансовыйОтчетНаПечатьВТ3.Подразделение
	|		И ДенежныеСредства.ПодотчетноеЛицо = АвансовыйОтчетНаПечатьВТ3.ПодотчетноеЛицо
	|		И (ДенежныеСредства.МоментВремени < АвансовыйОтчетНаПечатьВТ3.МоментВремениТекущийОтчет
	...
	|
	|ВЫБРАТЬ // 6 
	|	КонечныеОстатки.Ссылка,
	|	КонечныеОстатки.Организация,
	|	КонечныеОстатки.ПодотчетноеЛицо,
	|	КонечныеОстатки.Валюта,
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(КонечныеОстатки.СуммаКонечныйОстаток, 0) > 0 ТОГДА
	|			КонечныеОстатки.СуммаКонечныйОстаток
	|		ИНАЧЕ
	|			0
	...
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(КонечныеОстатки.СуммаРеглКонечныйОстаток, 0) > 0 ТОГДА
	|			КонечныеОстатки.СуммаРеглКонечныйОстаток
	|		ИНАЧЕ
	|			0
	...
	|ИЗ
	|	КонечныеОстатки КАК КонечныеОстатки
	|СГРУППИРОВАТЬ ПО
	|	КонечныеОстатки.Ссылка,
	|	КонечныеОстатки.Организация,
	|	КонечныеОстатки.ПодотчетноеЛицо,
	|	КонечныеОстатки.ЦельВыдачи,
	|	КонечныеОстатки.Валюта
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	...
	|	ВЫБОР
	|		КОГДА СУММА(ДанныеШапки.КонечныйОстаток
	|				- ДанныеШапки.КонечныйПерерасход
	|				+ ДанныеШапки.СуммаИзрасходовано
	|				- ДанныеШапки.СуммаВыданоНаличными
	|				- ДанныеШапки.СуммаВыданоКартой
	|				- ДанныеШапки.СуммаКонвертация
	|				+ ДанныеШапки.СуммаВозвращено) > 0 ТОГДА
	|			СУММА(ДанныеШапки.КонечныйОстаток
	|				- ДанныеШапки.КонечныйПерерасход
	|				+ ДанныеШапки.СуммаИзрасходовано
	|				- ДанныеШапки.СуммаВыданоНаличными
	|				- ДанныеШапки.СуммаВыданоКартой
	|				- ДанныеШапки.СуммаКонвертация
	|				+ ДанныеШапки.СуммаВозвращено)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НачальныйОстаток,
	|	ВЫБОР
	|		КОГДА СУММА(ДанныеШапки.КонечныйПерерасход) - СУММА(ДанныеШапки.СуммаПерерасход) > 0 
	|			ТОГДА СУММА(ДанныеШапки.КонечныйПерерасход) - СУММА(ДанныеШапки.СуммаПерерасход)
	|		ИНАЧЕ 0
	...
	|	ВЫБОР
	|		КОГДА СУММА(ДанныеШапки.КонечныйОстатокРегл 
	|				- ДанныеШапки.КонечныйПерерасходРегл
	|				+ ДанныеШапки.СуммаИзрасходованоРегл 
	|				- ДанныеШапки.СуммаВыданоНаличнымиРегл 
	|				- ДанныеШапки.СуммаВыданоКартойРегл
	|				- ДанныеШапки.СуммаКонвертацияРегл
	|				+ ДанныеШапки.СуммаВозвращеноРегл)> 0 ТОГДА
	|			СУММА(ДанныеШапки.КонечныйОстатокРегл 
	|				- ДанныеШапки.КонечныйПерерасходРегл 
	|				+ ДанныеШапки.СуммаИзрасходованоРегл 
	|				- ДанныеШапки.СуммаВыданоНаличнымиРегл 
	|				- ДанныеШапки.СуммаВыданоКартойРегл
	|				- ДанныеШапки.СуммаКонвертацияРегл
	|				+ ДанныеШапки.СуммаВозвращеноРегл)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НачальныйОстатокРегл,
	|	ВЫБОР
	|		КОГДА СУММА(ДанныеШапки.КонечныйПерерасходРегл) - СУММА(ДанныеШапки.СуммаПерерасходРегл) > 0 
	|			ТОГДА СУММА(ДанныеШапки.КонечныйПерерасходРегл) - СУММА(ДанныеШапки.СуммаПерерасходРегл)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НачальныйПерерасходРегл
	|ПОМЕСТИТЬ СуммыАвансовыхОтчетов
	|ИЗ
	|(ВЫБРАТЬ
	...
	|	Выдано
	|ГДЕ
	|	Выдано.СуммаВыданоНаличными <> 0 ИЛИ Выдано.СуммаВыданоКартой <> 0 ИЛИ Выдано.СуммаКонвертация <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0 КАК СуммаКонвертация,
	|	0 КАК СуммаВозвращено,
	|	ВЫБОР КОГДА СУММА(ВложенныйЗапрос.Сумма) < 0
	...
	|	0 КАК СуммаКонвертацияРегл,
	|	0 КАК СуммаВозвращеноРегл,
	|	ВЫБОР КОГДА СУММА(ВложенныйЗапрос.СуммаРегл) < 0
	|		ТОГДА -СУММА(ВложенныйЗапрос.СуммаРегл)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаПерерасходРегл,
	|	0 КАК КонечныйОстатокРегл,
	|	0 КАК КонечныйПерерасходРегл,
	|	0 КАК СуммаИзрасходованоРегл
	|ИЗ
	...
	|		Израсходовано.Валюта КАК Валюта,
	|		Израсходовано.ЦельВыдачи КАК ЦельВыдачи,
	|		-Израсходовано.СуммаИзрасходовано КАК Сумма,
	|		-Израсходовано.СуммаИзрасходованоРегл КАК СуммаРегл
	|	ИЗ
	...
	|		Выдано.СуммаВыданоНаличными + Выдано.СуммаВыданоКартой + Выдано.СуммаКонвертация - Выдано.СуммаВозвращено КАК Сумма,
	|		Выдано.СуммаВыданоНаличнымиРегл + Выдано.СуммаВыданоКартойРегл + Выдано.СуммаКонвертацияРегл - Выдано.СуммаВозвращеноРегл КАК СуммаРегл
	|	ИЗ
	|		Выдано КАК Выдано) КАК ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Ссылка,
	|	ВложенныйЗапрос.ЦельВыдачи,
	...
	|	ДанныеШапки.СуммаИзрасходовано КАК СуммаИзрасходовано,
	...
	|	ДанныеШапки.СуммаИзрасходованоРегл КАК СуммаИзрасходованоРегл,
	...
	|			ВЫБОР КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ДенежныеСредства.Сумма > 0
	|					ИЛИ ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И ДенежныеСредства.Сумма < 0
