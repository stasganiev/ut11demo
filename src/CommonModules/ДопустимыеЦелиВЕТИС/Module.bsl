...
Функция ПересечениеДопустимыхЦелейПоПродукциям(МассивПродукции, НизкокачественнаяПродукция) Экспорт
	...
	Иначе
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПродукцияВЕТИС КАК ПродукцияВЕТИС
		|		ПО (ПродукцияВЕТИС.Родитель.Идентификатор = ВидыПродукцииПоГруппамПриказаВЕТИС.ВидПродукцииИдентификатор)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРолиПользователяВЕТИС КАК ПолныеПрава
		|		ПО (ПолныеПрава.РольПользователя = ЗНАЧЕНИЕ(Перечисление.РолиПользователейВЕТИС.ГосударственныйВетеринарныйВрач))
		|ГДЕ
		|	ПолныеПрава.РольПользователя ЕСТЬ NULL
		|	И ПродукцияВЕТИС.Ссылка В(&МассивПродукции)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРолиПользователяВЕТИС КАК РолиПользователяВЕТИС
		|		ПО РолиПользователяВЕТИС.РольПользователя = ЗНАЧЕНИЕ(Перечисление.РолиПользователейВЕТИС.ГосударственныйВетеринарныйВрач)
		|
		|СГРУППИРОВАТЬ ПО
		|	ЦелиВЕТИС.Идентификатор
		|
		...
		|		ПО (РолиПользователяВЕТИС.РольПользователя = ВидыПродукцииПоГруппамПриказаВЕТИС.РольПользователя)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПродукцияВЕТИС КАК ПродукцияВЕТИС
		|			ПО ПродукцияВЕТИС.Ссылка В(&МассивПродукции)
		|			И (ПродукцияВЕТИС.Родитель.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВерсионныхОбъектовВЕТИС.Удален)
		|			ИЛИ ПродукцияВЕТИС.ВидПродукции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВерсионныхОбъектовВЕТИС.Удален)
		|			ИЛИ ПродукцияВЕТИС.Продукция.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВерсионныхОбъектовВЕТИС.Удален))
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРолиПользователяВЕТИС КАК ПолныеПрава
		|		ПО (ПолныеПрава.РольПользователя = ЗНАЧЕНИЕ(Перечисление.РолиПользователейВЕТИС.ГосударственныйВетеринарныйВрач))
		|ГДЕ
		|	ПолныеПрава.РольПользователя ЕСТЬ NULL
...
Процедура ЗаполнитьДанныеВРегистрахВидыПродукцииДопустимыеЦелиПоГруппамПриказаВЕТИС(ИсходныеДанныеКлассификатора, РольПользователя) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ПреобразованныеДанные = ПреобразованныеДанныеКлассификатора(ИсходныеДанныеКлассификатора, РольПользователя);
	
	ЗаполнитьВидыПродукцииПоГруппамПриказаВЕТИС(ПреобразованныеДанные.ВидыПродукцииПоГруппамВЕТИС, РольПользователя);
	ЗаполнитьДопустимыеЦелиПоГруппамПриказаВЕТИС(ПреобразованныеДанные.ДопустимыеЦелиПоГруппамВЕТИС);
	
КонецПроцедуры

Функция ПреобразованныеДанныеКлассификатора(ИсходныеДанные, РольПользователя, ПроверятьНаличиеГруппы = Ложь) Экспорт
	
	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("ВидыПродукцииПоГруппамВЕТИС");
	ВозвращаемоеЗначение.Вставить("ДопустимыеЦелиПоГруппамВЕТИС");
	
	Если РольПользователя = Перечисления.РолиПользователейВЕТИС.УполномоченноеЛицо Тогда
		
		ГруппыПродукции = ГруппыПродукцииУполномоченныхЛиц();
		ВозвращаемоеЗначение.ВидыПродукцииПоГруппамВЕТИС = ПереченьПродукцииУполномоченныхЛиц(
			ГруппыПродукции,
			ИсходныеДанные.Продукция,
			ПроверятьНаличиеГруппы);
		ВозвращаемоеЗначение.ДопустимыеЦелиПоГруппамВЕТИС = ЦелиУполномоченныхЛиц(
			ГруппыПродукции,
			ИсходныеДанные.Цели,
			ПроверятьНаличиеГруппы);
		
	ИначеЕсли РольПользователя = Перечисления.РолиПользователейВЕТИС.АттестованныйСпециалист Тогда
		
		ГруппыПродукции = ГруппыПродукцииАттестованныхСпециалистов();
		ВозвращаемоеЗначение.ВидыПродукцииПоГруппамВЕТИС = ПереченьПродукцииАттестованныхСпециалистов(
			ГруппыПродукции,
			ИсходныеДанные.Продукция,
			ПроверятьНаличиеГруппы);
		ВозвращаемоеЗначение.ДопустимыеЦелиПоГруппамВЕТИС = ЦелиАттестованныхСпециалистов(
			ГруппыПродукции,
			ИсходныеДанные.Цели,
			ПроверятьНаличиеГруппы);
...
Функция ЦелиУполномоченныхЛиц(ГруппыПродукции, Источник, ПроверятьНаличиеГруппы)
	...
	ИмяМетаданных = Метаданные.РегистрыСведений.ДопустимыеЦелиПоГруппамВЕТИС.ПолноеИмя();
	КолонкиДанных = ИнтеграцияВЕТИС.ПоляСтрокиДанныхКлассификатора(ИмяМетаданных);
	...
	Для Каждого СтрокаДанных Из Источник Цикл
		...
		СтрГруппаПриказа                          = СтрокаДанных[КолонкиДанных.ГруппаПриказа];
		СтруктураПроверкиДублей.ЦельНаименование  = СтрокаДанных[КолонкиДанных.ЦельНаименование];
		СтруктураПроверкиДублей.ЦельИдентификатор = СтрокаДанных[КолонкиДанных.ЦельИдентификатор];
		...
		Если ГруппаПриказа <> СтрГруппаПриказа Тогда
			...
			Если ГруппаПродукции = Неопределено Тогда
				Если ПроверятьНаличиеГруппы Тогда
...
Функция ЦелиАттестованныхСпециалистов(ГруппыПродукции, Источник, ПроверятьНаличиеГруппы)
	...
	Для Каждого СтрокаДанных Из Источник Цикл
		...
		ЧастиСтрокиГруппыПриказа = СтрРазделить(СтрокаДанных[КолонкиДанных.ГруппаПриказа], ".");
...
Функция ПереченьПродукцииУполномоченныхЛиц(ГруппыПродукции, Источник, ПроверятьНаличиеГруппы)
	...
	ИмяМетаданных = Метаданные.РегистрыСведений.ВидыПродукцииПоГруппамВЕТИС.ПолноеИмя();
	КолонкиДанных = ИнтеграцияВЕТИС.ПоляСтрокиДанныхКлассификатора(ИмяМетаданных);
	...
	Для Каждого СтрокаДанных Из Источник Цикл
		...
		ГруппаПриказаСтрока       = СтрокаДанных[КолонкиДанных.ГруппаПриказа];
		ВидПродукцииИдентификатор = СтрокаДанных[КолонкиДанных.ВидПродукцииИдентификатор];
		ВидПродукцииНаименование  = СтрокаДанных[КолонкиДанных.ВидПродукцииНаименование];
...
Функция ПереченьПродукцииАттестованныхСпециалистов(ГруппыПродукции, Источник, ПроверятьНаличиеГруппы)
...
Функция ГруппыПродукцииУполномоченныхЛиц() Экспорт
...
Функция ГруппыПродукцииАттестованныхСпециалистов() Экспорт
