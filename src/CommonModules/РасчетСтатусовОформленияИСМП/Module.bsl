...
Процедура РассчитатьСтатусОформленияИСМППриЗаписиДокументаОбработчик(Источник, Отказ) Экспорт
	...
	// Для версии АПИ 2 процедура расчета статусов в ОбменСКонтрагентамиПереопределяемый.ПослеФормированияЭлектронногоДокумента
	Если ЭлектронноеВзаимодействиеИСМП.ВерсияАПИ() < 2 Тогда
		
		ИмяТипаЭлектронныйДокументИсходящий = "ДокументОбъект.ЭлектронныйДокументИсходящий";
		
		Если ТипЗнч(Источник) = Тип(ИмяТипаЭлектронныйДокументИсходящий) Тогда
...
Функция РеквизитыДляРасчета(МетаданныеОснования, МетаданныеДокументаИСМП) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ВетИС") Тогда
		...
		Модуль.ПриОпределенииИменРеквизитовДляРасчетаСтатусаОформления(МетаданныеОснования, МетаданныеДокументаИСМП, Реквизиты);
	КонецЕсли;
...
Процедура ПриОпределенииЗапросаТоварыДокументаОснования(МетаданныеОснования, МетаданныеДокументаИСМП,
	...
	ИначеЕсли МетаданныеОснования = Метаданные.Документы.МаркировкаТоваровИСМП Тогда
		...
		ПодзапросКОформлению = "
		|ВЫБРАТЬ
		|	ТаблицаТовары.Ссылка         КАК Ссылка,
		|	ТаблицаТовары.Номенклатура   КАК Номенклатура,
		|	ТаблицаТовары.Характеристика КАК Характеристика,
		|	ТаблицаТовары.Серия          КАК Серия,
		|	ТаблицаТовары.Количество / ЕСТЬNULL(ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке,1) КАК Количество
		|ПОМЕСТИТЬ МаркировкаКОформлению
		|ИЗ
		...
		|ГДЕ
		|	ТаблицаТовары.Ссылка В (&МассивДокументов)
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ПодтверждениеПоступленияКИЗ)
		|	И ТаблицаТовары.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.Агрегация)
		|	И Не ТаблицаТовары.Ссылка.ОперацияНанесенияТолькоДляНаборов
		|	И Не (&ТребуетВзвешивания)";
		ОбщегоНазначенияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ПодзапросКОформлению, "ОписаниеНоменклатурыИС", "ТаблицаТовары.Номенклатура");
		МассивТекстовЗапроса.Добавить(ПодзапросКОформлению);
		ПодзапросУжеМаркировано = "
		|ВЫБРАТЬ
		...
		|	ТаблицаТовары.Серия          КАК Серия,
		|	-ВЫБОР 
		|		КОГДА &ПроизвольнаяЕдиницаУчета
		|			ТОГДА ТаблицаТовары.КоличествоПотребительскихУпаковок
		|		КОГДА ТаблицаТовары.Количество <> 0
		|			ТОГДА ТаблицаТовары.Количество
		|	ИНАЧЕ 1 КОНЕЦ КАК Количество
		|ПОМЕСТИТЬ МаркировкаУжеМаркировано
		|ИЗ
		|	Документ.МаркировкаТоваровИСМП.ШтрихкодыУпаковок КАК ШтрихкодыУпаковок
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров КАК ТаблицаТовары
		|	ПО ТаблицаТовары.Ссылка = ШтрихкодыУпаковок.ШтрихкодУпаковки
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеНоменклатурыИС
		|		ПО &ТоварыОписаниеНоменклатурыИС
		|ГДЕ
		|	ШтрихкодыУпаковок.Ссылка В (&МассивДокументов)";
		ОбщегоНазначенияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ПодзапросУжеМаркировано, "ОписаниеНоменклатурыИС", "ТаблицаТовары.Номенклатура");
		МассивТекстовЗапроса.Добавить(ПодзапросУжеМаркировано);
		...
		|ИЗ
		|	МаркировкаКОформлению КАК КОформлению
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	УжеМаркировано.Ссылка КАК Ссылка,
		|	УжеМаркировано.Номенклатура   КАК Номенклатура,
		|	УжеМаркировано.Характеристика КАК Характеристика,
		|	УжеМаркировано.Серия          КАК Серия,
		|	УжеМаркировано.Количество     КАК Количество
		|ИЗ
		|	МаркировкаУжеМаркировано КАК УжеМаркировано
		|) КАК ВложенныйЗапрос
		|СГРУППИРОВАТЬ ПО
		...
		|
		|");
		ТекстЗапроса = СтрСоединить(МассивТекстовЗапроса, ИнтеграцияИС.РазделительЗапросовВПакете());
		
	
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ВетИС") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИСМПВЕТИС");
		Модуль.ПриОпределенииТекстаЗапросаДляРасчетаСтатусаОформления(
...
Функция ОпределитьСтатусыОформленияДокументов(МассивДокументов, МетаданныеДокументаИСМП, МенеджерВТ) Экспорт
	...
	ШаблонЗапросаВТОформленныеДокументы = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	И НЕ Статусы.Статус В (&КонечныеСтатусыМаркировкаТоваровИСМП)
		|	И ТаблицаДокументы.Операция <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.Агрегация)
		|	И (ТаблицаДокументы.ВидПродукции В(
		...
		|	ИЛИ ТаблицаДокументы.Операция В(
		...
		|		ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.Агрегация)))
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка";
	...
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.АнтисептикиГрупповаяУпаковкаСтандартныйКМ)
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.БАДыГрупповаяУпаковка)
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.БАДыГрупповаяУпаковкаСтандартныйКМ)
	...
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.РадиоэлектроннаяПродукцияГрупповаяУпаковка)
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.РадиоэлектроннаяПродукцияГрупповаяУпаковкаСтандартныйКМ)
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.ОптоволокноИОптоволоконнаяПродукцияГрупповаяУпаковка)
	|	И ОформленныеТовары.Шаблон <> ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.ОптоволокноИОптоволоконнаяПродукцияГрупповаяУпаковкаСтандартныйКМ)
	|
	...
	ШаблонЗапросаВозвратВОборот = "
		...
		|		0                                КАК План,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ОформленныеТовары.КодМаркировки.Количество, 0) = 0 ТОГДА 1
		|			ИНАЧЕ ОформленныеТовары.КодМаркировки.Количество
		|		КОНЕЦ КАК Факт
		|	ИЗ
		|		Документ.%1.%2 КАК ОформленныеТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОформленныеДокументы%1 КАК ОформленныеДокументы
