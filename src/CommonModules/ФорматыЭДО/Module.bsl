...
//     *** ИнформацияПокупателя - Строка - идентификатор формата информации покупателя.
//     *** ДополнительныеСведения - Строка - идентификатор формата информации покупателя.
...
//   * ДокументПредприятия - Строка - идентификатор формата документ предприятия.
//
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.ФНС.АктОРасхождениях.Вставить("ДополнительныеСведения", ФорматыФНС.АктОРасхождениях.ДополнительныеСведения);
	...
	Форматы.ФНС.Вставить("ИзвещениеОПолучении0201", ФорматыФНС.ИзвещениеОПолучении0201); 
	...
	Форматы.ФНС.Вставить("УведомлениеОбУточнении0201", ФорматыФНС.УведомлениеОбУточнении0201); 
	...
	Форматы.ФНС.Вставить("МашиночитаемаяДоверенность2022_2", ФорматыФНС.МашиночитаемаяДоверенность2022_2);
	Форматы.ФНС.Вставить("МашиночитаемаяДоверенность003", ФорматыФНС.МашиночитаемаяДоверенность003);
	...
	Форматы.ФНС.Вставить("ДоговорныйДокумент101", ФорматыФНС.ДоговорныйДокумент101);
	
	Форматы.ФНС.Вставить("ДоговорныйДокументXML", Новый Структура);
	Форматы.ФНС.ДоговорныйДокументXML.Вставить("ИнформацияОтправителя",
		ФорматыФНС.ДоговорныйДокументXML.ИнформацияОтправителя);
	...
	Форматы.КорректировочныйДокумент1С.Вставить("ИнформацияПродавца", "OKORDOC");
	Форматы.КорректировочныйДокумент1С.Вставить("ИнформацияПокупателя", "PKORDOC");
	...
	ИмяФормата = Обработки.ФорматДокументПредприятия.ИмяФормата();
	Форматы.Вставить("ДокументПредприятия", ИмяФормата);
	
	Форматы.Вставить("Прикладной", "Прикладной");
...
Процедура ОбновитьФорматыЭлектронныхДокументов(Форматы, ДатаИзменения) Экспорт
	...
	Если Форматы.ДатаПоследнегоИзменения <> ДатаИзменения ИЛИ Не ЕстьЗаписи Тогда
		...
		НеподдерживаемыеФорматыЭлектронныхДокументов = НеподдерживаемыеФорматыЭлектронныхДокументов();
		
		Для Каждого Формат Из ПоддерживаемыеФорматы Цикл
			
			Если НеподдерживаемыеФорматыЭлектронныхДокументов.Найти(Формат) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
// Версия программы 1С для заполнения ВерсПрог.
// 
// Возвращаемое значение:
//  Строка - Версия программы 1С
Функция ВерсияПрограммы() Экспорт
	Возврат "1С:Предприятие 8";
КонецФункции

// Возвращает признак, что файл сформирован в программе 1С:Предприятие 8,
// если данного признака у файла нет возвращает Неопределено.
// Параметры:
//  ОписаниеФайла - см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
// 
// Возвращаемое значение:
//  Неопределено, Булево - Признак, что файл сформирован в 1С:Предприятие 8
Функция ФайлСформированВ1С(ОписаниеФайла) Экспорт
	ОписаниеОшибки = "";
	ДанныеФайлаЭД = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Возврат Неопределено;
	КонецЕсли;
	ВерсПрог = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсПрог");
	Если ВерсПрог = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат ВерсПрог = ВерсияПрограммы();
КонецФункции
...
Функция КНДПоСвойствамДокумента(СвойстваДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях Тогда
		...
		КНД = "1175014";
		Если СтрНайти(ИмяФайла, "DP_PRIRASXSVED") > 0 Тогда
			КНД = "1175015";
		КонецЕсли;
...
Функция ЭтоМЧД(ОписаниеФайла) Экспорт
	...
	Если ОписаниеФайла.ДвоичныеДанные <> Неопределено Тогда
		ДанныеXDTO = ДанныеФайлаЭД(ОписаниеФайла, "", Неопределено, , Истина);
		
		ВерсияФорматаМЧД = МашиночитаемыеДоверенности.ВерсияФорматаОбъектаМЧД(ДанныеXDTO);
		
		Возврат ЗначениеЗаполнено(ВерсияФорматаМЧД);
...
//  - ОбработкаМенеджер.ФорматДоговорныйДокумент101
//
...
// Определяет, что объект является ОбработкойОбъектом или ОбработкойМенеджером формата ЭД.
// 
// Параметры:
//  Объект - Произвольный
// 
...
// См. ЭлектронныеДокументыЭДО.ПодключитьИспользованиеФорматаДоговорныхДокументовXML
Процедура ПодключитьИспользованиеФорматаДоговорныхДокументовXML() Экспорт
	
	Формат = Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПространствоИмен();
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ФорматыЭлектронныхДокументов");
		ЭлементБлокировкиДанных.УстановитьЗначение("Формат", Формат);
		ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
		БлокировкаДанных.Заблокировать();
		
		Набор = РегистрыСведений.ФорматыЭлектронныхДокументов.СоздатьНаборЗаписей();
		Набор.Отбор.Формат.Установить(Формат);
		Набор.Прочитать();
		
		Если Набор.Количество() > 0 Тогда
			
			Для Каждого СтрокаНабора Из Набор Цикл
				СтрокаНабора.ПоддерживаетсяКонфигурацией = Истина;
			КонецЦикла;
	...
	Исключение
		...
		ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
		ТекстСообщения = НСтр("ru = 'Не удалось установить использование формата договорных документов xml.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
			УровеньЖурналаРегистрации.Ошибка);
...
// 
// Параметры:
// 	ОператорОтправителя - Строка - идентификатор оператора отправителя
// 	ОператорПолучателя - Строка - идентификатор оператора получателя
// 	Формат - Строка - формат электронного документа
// 	ТипРегламента - ПеречислениеСсылка.ТипыРегламентовЭДО - регламент электронного документа
// 	
// Возвращаемое значение:
// 	- Неопределено
// 	- Структура:
...
//
Функция ФорматПоддержанОператорами(ОператорОтправителя, ОператорПолучателя, Формат, ТипРегламента) Экспорт

	Если ПустаяСтрока(Формат)
		Или (ПустаяСтрока(ОператорПолучателя)
		Или ПустаяСтрока(ОператорОтправителя)) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	НеподдерживаемыеФорматы = СервисНастроекЭДО.НеподдерживаемыеОператорамиФорматы();
	
	НеподдерживаемыеФорматыОтправителя = НеподдерживаемыеФорматы.Получить(ОператорОтправителя);
	Если ЗначениеЗаполнено(НеподдерживаемыеФорматыОтправителя) Тогда
		НеподдерживаемыеРегламентыОтправителя = НеподдерживаемыеФорматыОтправителя.Получить(Формат);
		Если НеподдерживаемыеРегламентыОтправителя <> Неопределено Тогда
			// Если для формата не указаны неподдерживаемые регламенты, то он полностью неподдержан оператором.
			Если НеподдерживаемыеРегламентыОтправителя.Количество() = 0 Тогда
				Результат.ФорматПоддержанОператоромОтправителя = Ложь;
			Иначе
				Для Каждого Регламент Из НеподдерживаемыеРегламентыОтправителя Цикл
					ТипРегламентаЭДО = ТипРегламентаПоЗначениюСервиса(Регламент);
					Если ЗначениеЗаполнено(ТипРегламентаЭДО)
						И ТипРегламентаЭДО = ТипРегламента Тогда
						Результат.ФорматПоддержанОператоромОтправителя = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
	...
	НеподдерживаемыеФорматыПолучателя = НеподдерживаемыеФорматы.Получить(ОператорПолучателя);
	Если ЗначениеЗаполнено(НеподдерживаемыеФорматыПолучателя) Тогда
		НеподдерживаемыеРегламентыПолучателя = НеподдерживаемыеФорматыПолучателя.Получить(Формат);
		Если НеподдерживаемыеРегламентыПолучателя <> Неопределено Тогда
			// Если для формата не указаны неподдерживаемые регламенты, то он полностью неподдержан оператором.
			Если НеподдерживаемыеРегламентыПолучателя.Количество() = 0 Тогда
				Результат.ФорматПоддержанОператоромПолучателя = Ложь;
			Иначе
				Для Каждого Регламент Из НеподдерживаемыеРегламентыПолучателя Цикл
					ТипРегламентаЭДО = ТипРегламентаПоЗначениюСервиса(Регламент);
					Если ЗначениеЗаполнено(ТипРегламентаЭДО)
						И ТипРегламентаЭДО = ТипРегламента Тогда
						Результат.ФорматПоддержанОператоромПолучателя = Ложь;
	...
	Если Не Результат.ФорматПоддержанОператоромОтправителя Или Не Результат.ФорматПоддержанОператоромПолучателя Тогда
		
		СведенияОФормате = ПолучитьСведенияОФормате(Формат);
		Если ЗначениеЗаполнено(СведенияОФормате) Тогда
			Результат.Формат = СведенияОФормате.ПредставлениеФормата;
		Иначе
			Результат.Формат = Формат;
		КонецЕсли;
		
		МассивОператоров = Новый Массив;
		МассивОператоров.Добавить(ОператорОтправителя);
		МассивОператоров.Добавить(ОператорПолучателя);
		ОператорыЭДО = СервисНастроекЭДО.НайтиОператоровЭДОПоИдентификаторам(МассивОператоров);
		
		ДанныеОператораОтправителя = ОператорыЭДО.Найти(ОператорОтправителя, "ИдентификаторЭДО");
		Если ЗначениеЗаполнено(ДанныеОператораОтправителя) Тогда
			Результат.ОператорОтправителя = ДанныеОператораОтправителя.ПредставлениеОператора;
		КонецЕсли;
			
		ДанныеОператораПолучателя = ОператорыЭДО.Найти(ОператорПолучателя, "ИдентификаторЭДО");
		Если ЗначениеЗаполнено(ДанныеОператораПолучателя) Тогда
			Результат.ОператорПолучателя = ДанныеОператораПолучателя.ПредставлениеОператора;
		КонецЕсли;
...
// 
// Параметры:
//  ОписаниеФайла - См. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла - описание файла для добавления данных МЧД.
...
//
// Возвращаемое значение:
//  См. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла - описание файла после добавления данных МЧД.
//
Функция ЗаполнитьДанныеДоверенностейДокумента(ОписаниеФайла, Организация, Доверенности, СодержаниеДокумента) Экспорт
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		...
		Если Формат = ПоддерживаемыеФорматы.ФНС.УведомлениеОбУточнении
			Или Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении Тогда 
			ЗаполнитьДанныеДоверенностейЭД_УОУ_ИОП(ДанныеФайлаЭД, Доверенности, ПространствоИмен);
		Иначе
			ПутиКПолямФормата = ПутиКИнформационнымПолямФорматаЭДДляМЧД(ДанныеФайлаЭД, Формат, ПоддерживаемыеФорматы);
			ЗаполнитьДанныеДоверенностейВСодержимомЭД(ДанныеФайлаЭД, Доверенности, ПространствоИмен, ПутиКПолямФормата);
		КонецЕсли;
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	Если (Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Или Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения)
		И ФайлСформированВ1С(ОписаниеФайла) = Ложь Тогда
		Подписант = СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки);

		Если Не ПодписантЗаполнен(Подписант, Организация, ОписаниеОшибки) Или Не ВУчетнойЗаписиЕстьСертификатПодписанта(
			Подписант, ОписаниеОшибки) Тогда

			СброситьСведенияОПодписанте(ОписаниеФайла);
		КонецЕсли;
		Если ПустаяСтрока(ОписаниеОшибки) Тогда
			ИндексПодписанта = ДанныеФайлаЭД.Документ.Подписант.Количество() - 1;
			Подписант = ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта];
			Если Подписант.ИП <> Неопределено Или Подписант.ЮЛ <> Неопределено Тогда
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.УведомлениеОбУточнении 
		...
		Если ЗначениеЗаполнено(ОтчествоСертификата) Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, , Ошибки);
		КонецЕсли;
		Если Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении Или Формат = ПоддерживаемыеФорматы.ФНС.УведомлениеОбУточнении Тогда
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях Тогда
		...
		Если Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения Тогда
			...
			// Лицо, в полномочия которого входит утверждение документа,
			// оформляющего событие (факт хозяйственной жизни).
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "ОблПолн", "21", Истина, Ошибки); 
		...
		Иначе
			...
			// Лицо, ответственное за оформление события (операции).
...
Функция ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	...
	Если ТипЗнч(Обработка) = Тип("Неопределено") Тогда
		Возврат Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
			...
			Или Формат = ПоддерживаемыеФорматы.ФНС.УведомлениеОбУточнении0201    						
			Или Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении 
			Или Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении0201 
...
// * Отчество - Строка
// * Вид - Строка
Функция СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторОрганизации", "");
	Результат.Вставить("Фамилия", "");
	Результат.Вставить("Имя", "");
	Результат.Вставить("Отчество", "");
	Результат.Вставить("Вид", "ФЛ");
	ДанныеФайлаЭД = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Возврат Результат;
	КонецЕсли;
	Результат.ИдентификаторОрганизации = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "СвУчДокОбор.ИдОтпр", "");
	
	Если ТипЗнч(ДанныеФайлаЭД.Документ.Подписант) = Тип("СписокXDTO") Тогда
		ИндексПодписанта = ДанныеФайлаЭД.Документ.Подписант.Количество() - 1;
		Подписант = ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта];
	Иначе
		Подписант = ДанныеФайлаЭД.Документ.Подписант;
	КонецЕсли;

	Если Подписант.ИП <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ИП.ФИО);
		Результат.Вид = "ИП";
	ИначеЕсли Подписант.ЮЛ <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ЮЛ.ФИО);
		Результат.Вид = "ЮЛ";
	ИначеЕсли Подписант.ФЛ <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ФЛ.ФИО);
		Результат.Вид = "ФЛ";
	Иначе 
		Результат.Вид = "";
	КонецЕсли;
...
// Подписант заполнен и размещен в правильном разделе.
// 
// Параметры:
//  Подписант - см. ФорматыЭДО.СведенияОПодписанте
//  Организация - ОпределяемыйТип.Организация - Организация
//  ОписаниеОшибки - Строка
// 
// Возвращаемое значение:
//  Булево - Подписант заполнен
Функция ПодписантЗаполнен(Подписант, Организация, ОписаниеОшибки = "") Экспорт
	Если ПустаяСтрока(Подписант.Имя) Или ПустаяСтрока(Подписант.Фамилия) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ИнтеграцияЭДО.ЭтоФизЛицо(Организация) Тогда
		Если Подписант.Вид = "ЮЛ" Тогда
			ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Подписант %1 %2 %3 должен быть заполнен в разделе <ИП>. Данные подписанта будут заполнены при подписании.'"),
				Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
		КонецЕсли;
	Иначе
		Если Подписант.Вид = "ИП" Тогда
			ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Подписант %1 %2 %3 должен быть заполнен в разделе <ЮЛ>. Данные подписанта будут заполнены при подписании.'"),
				Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
		КонецЕсли;
	КонецЕсли;
	Возврат Не (ЗначениеЗаполнено(ОписаниеОшибки) Или Подписант.Вид = "ФЛ");
КонецФункции

// В учетной записи есть сертификат подписанта.
// 
// Параметры:
//  Подписант - см. ФорматыЭДО.СведенияОПодписанте
//  ОписаниеОшибки - Строка
// 
// Возвращаемое значение:
//  Булево - В учетной записи есть сертификат подписанта
Функция ВУчетнойЗаписиЕстьСертификатПодписанта(Подписант, ОписаниеОшибки = "") Экспорт
	Запрос = Новый Запрос;
	...
	Запрос.УстановитьПараметр("ИдентификаторЭДО", Подписант.ИдентификаторОрганизации);
	УстановитьПривилегированныйРежим(Истина);
	Результат = Запрос.Выполнить();
	Сертификаты = Результат.Выгрузить().ВыгрузитьКолонку("Сертификат");
	СвойстваСубъектов = КриптографияБЭД.СвойстваСубъектаСертификатовПоСсылке(Сертификаты);
	Для Каждого СвойстваСубъекта Из СвойстваСубъектов Цикл
		Если СвойстваСубъекта.Значение.Фамилия = Подписант.Фамилия 
			И СвойстваСубъекта.Значение.Имя = Подписант.Имя
			И (Не ЗначениеЗаполнено(СвойстваСубъекта.Значение.Отчество) 
			Или СвойстваСубъекта.Значение.Отчество = Подписант.Отчество) Тогда
		Возврат Истина;
		КонецЕсли;
	КонецЦикла;
	ОписаниеОшибки = СтрШаблон(НСтр(
			"ru = 'Для подписанта %1 %2 %3 нет сертификата, раздел подписанта будет заполнен при подписании.'"),
			Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
	Возврат Ложь;
КонецФункции

// Сбросить сведения о подписанте в xml формализованного формата.
// 
// Параметры:
//  ОписаниеФайла - см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
//  ОписаниеОшибки - Строка
Процедура СброситьСведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	ДанныеФайлаЭД = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Возврат;
	КонецЕсли;
	...
	Ошибки = Новый Массив;
	Если Подписант.ИП <> Неопределено Тогда
		РаботаСФайламиБЭД.СброситьСвойствоXDTO(Подписант, "ИП", Ошибки);
	КонецЕсли;
	Если Подписант.ЮЛ <> Неопределено Тогда
		РаботаСФайламиБЭД.СброситьСвойствоXDTO(Подписант, "ЮЛ", Ошибки);
	КонецЕсли;
	...
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", "-");
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Имя", "-");
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФЛ, "ФИО", ФИО, Истина);
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "ФЛ", ФЛ, Истина);
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД, Ложь);
		ДвоичныеДанные = ОбщегоНазначенияБЭД.УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИмен);
	Иначе
...
#Область ЗаполнениеДанныхПодписей

// Возвращает признак доступности заполнения данных электронных подписей в документе
// 
// Параметры:
// 	СодержаниеДокумента - см. НовоеОписаниеФайлаДокумента
//
...
//  Булево
//
Функция ЗаполнениеЭлектронныхПодписейДоступно(СодержаниеДокумента) Экспорт
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	Возврат СодержаниеДокумента.Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения;
...
//  ЗаписываемыеПодписи - Массив из см. КриптографияБЭДКлиентСервер.НовыеСвойстваПодписи
//
// Возвращаемое значение:
//  Структура - см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
//
Функция ЗаполнитьЭлектронныеПодписи(ОписаниеФайла, ЗаписываемыеПодписи) Экспорт
	...
	ЛокальноеИмя = "Файл";
	ПространствоИменСхемы = ДанныеФайлаЭД.Тип().URIПространстваИмен;
	ДанныеФайлаЭД.Документ.ИдИнфПок.ЭП.Очистить();
	
	Для Каждого ЗаписываемаяПодпись Из ЗаписываемыеПодписи Цикл
		
		ПодписьСтрокой = КриптографияБЭД.ДанныеПодписиВСтрокуБезПереносов(ЗаписываемаяПодпись.Подпись);
		ДанныеФайлаЭД.Документ.ИдИнфПок.ЭП.Добавить(ПодписьСтрокой);
...
//   * ВерсияПрограммы - Строка - версия программы, с помощью которой сформирован файл. По умолчанию см. ФорматыЭДО.ВерсияПрограммы
//   * УникальныйИдентификатор - Строка- 36 символьный глобально уникальный идентификатор.
//   * ДанныеДокумента - ДеревоЗначений - данные документа.
...
Функция НовыеДанныеДляФормированияОсновногоТитула() Экспорт
	...
	Данные.Вставить("ВерсияПрограммы", ВерсияПрограммы());
...
// Формирует электронный документ дополнительного титула по учету.
//
// Параметры:
//  Настройки - см. НовыеНастройкиФормированияДокументаПоОбъектуУчета
//  Данные - см. НовыеДанныеДляФормированияОсновногоТитула
...
//  см. НовыйРезультатФормированияДокументаПоУчету
//
Функция СформироватьДанныеДополнительногоТитулаПоДаннымОбъектаУчета(Настройки, Данные) Экспорт
...
// Выполняет проверку возможности формирования ответного титула без документа учета.
// 
// Параметры:
// 	ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО - тип проверяемого документа.
// Возвращаемое значение:
// 	Булево - Истина если подписание без документа учета возможно и Ложь в обратном случае.
Функция ВозможноФормированиеОтветногоТитулаБезУчетногоДокумента(ТипДокумента) Экспорт

	ТипыДокументов = ТипыДокументовНеПоддерживающиеФормированиеОтветногоТитулаБезДокументаУчета();
...
// Проверить загружаемый файл исходящего документа при повторной загрузке.
// 
// Параметры:
//  Организация - ОпределяемыйТип.Организация
//  ЗагружаемыйФайл - Структура - загружаемый файл:
//                           * Хранение  - Строка - расположение данных во временном хранилище.
//                           * Имя       - Строка - Путь к файлу
//  ЗагруженныйФайл - СправочникСсылка.СообщениеЭДОПрисоединенныеФайлы
//  ИмяФайла - Строка
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//		*Ключ - Строка - текст ошибки, имя атрибута
//		*Значение - Булево, Структура - Ложь, если принудительная загрузка невозможна, если загрузка 
//										возможна, тогда Структура:
//											**ВИсходномДокументе - Строка -значение в загруженном файле
//											**ВЗагружаемомДокументе - Строка -значение в загружаемом файле
Функция ЗагружаемыйФайлСоответсвуетЗагруженномуФайлу(Организация, ЗагружаемыйФайл, ЗагруженныйФайл, ИмяФайла) Экспорт
	Ошибки = Новый Соответствие;
	ОписаниеЗагружаемогоФайла = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
	ОписаниеЗагружаемогоФайла.ИмяФайла = ЗагружаемыйФайл.Имя;
	ОписаниеЗагружаемогоФайла.ДвоичныеДанные = ПолучитьИзВременногоХранилища(ЗагружаемыйФайл.Хранение);
	ОписаниеОшибки = "";
	ЗагруженныйФайлРеквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗагруженныйФайл, "Расширение, ПолноеИмяФайла", Истина);
	
	Попытка
		ЗагружаемыеДанные = ДанныеФайлаЭД(ОписаниеЗагружаемогоФайла, ОписаниеОшибки);
	Исключение
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Документ не загружен, так как файл %1 содержит ошибки (см. Журнал регистрации).
			|Выберите другой файл и повторите загрузку.'"), 
			ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ЗагружаемыйФайл.Имя).Имя);
	КонецПопытки;
	
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Ошибки.Вставить(ОписаниеОшибки, Ложь);
		Возврат Ошибки;
	КонецЕсли;
	
	ОписаниеФайла = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
	ОписаниеФайла.ИмяФайла = ЗагруженныйФайлРеквизиты.ПолноеИмяФайла;
	ОписаниеФайла.ДвоичныеДанные = РаботаСФайлами.ДвоичныеДанныеФайла(ЗагруженныйФайл);
	ЗагруженныеДанные = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	...
	Если ТипЗнч(ЗагружаемыеДанные) = Тип("ОбъектXDTO") 
		И ЗагружаемыеДанные.Тип().URIПространстваИмен <> ЗагруженныеДанные.Тип().URIПространстваИмен Тогда
		Если ФорматыНеСовместимы(ЗагружаемыеДанные.Тип().URIПространстваИмен, ЗагруженныеДанные.Тип().URIПространстваИмен) Тогда
					ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Документ не загружен, так как формат файла %1 нарушает регламент текущего учетного документа.
			|Загрузка приведет к ошибкам со стороны оператора.
			|Выберите другой файл и повторите загрузку.'"), 
			ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ЗагружаемыйФайл.Имя).Имя);
			Ошибки.Вставить(ОписаниеОшибки, Ложь);
			Возврат Ошибки;
		Иначе
			...
			ДанныеСравнения.Вставить("ВИсходномДокументе", ЗагруженныеДанные.Тип().URIПространстваИмен);
			ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ЗагружаемыеДанные.Тип().URIПространстваИмен);
			Ошибки.Вставить(НСтр("ru = 'Формат файла'"), ДанныеСравнения);
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "ИдФайл") Или Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "СвУчДокОбор") Тогда
		Ошибки.Очистить();
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Документ не загружен, так как файл %1 не является формализованным документом ЭДО.
			|Выберите другой файл и повторите загрузку.'"), 
			ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ЗагружаемыйФайл.Имя).Имя);
		Ошибки.Вставить(ОписаниеОшибки, Ложь);
		Возврат Ошибки;
	...
	ИмяФайла = ЗагружаемыеДанные.ИдФайл;
	
	Если ЗагружаемыеДанные.СвУчДокОбор.ИдОтпр <> ЗагруженныеДанные.СвУчДокОбор.ИдОтпр Тогда
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", ЗагруженныеДанные.СвУчДокОбор.ИдОтпр);
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ЗагружаемыеДанные.СвУчДокОбор.ИдОтпр);
		Ошибки.Вставить("ИдОтпр", ДанныеСравнения);
	КонецЕсли;
	
	Если ЕстьСвойствоXDTO(ЗагружаемыеДанные.СвУчДокОбор, "ИдПол") Тогда
		ИдПолЗагружаемый = ЗагружаемыеДанные.СвУчДокОбор.ИдПол;
	ИначеЕсли ЕстьСвойствоXDTO(ЗагружаемыеДанные.СвУчДокОбор, "ИдПок") Тогда
		ИдПолЗагружаемый = ЗагружаемыеДанные.СвУчДокОбор.ИдПок;
	Иначе
		ИдПолЗагружаемый = "";
	КонецЕсли;
	
	Если ЕстьСвойствоXDTO(ЗагруженныеДанные.СвУчДокОбор, "ИдПол") Тогда
		ИдПолИсходный = ЗагруженныеДанные.СвУчДокОбор.ИдПол;
	ИначеЕсли ЕстьСвойствоXDTO(ЗагруженныеДанные.СвУчДокОбор, "ИдПок") Тогда
		ИдПолИсходный = ЗагруженныеДанные.СвУчДокОбор.ИдПок;
	Иначе
		ИдПолИсходный = "";
	КонецЕсли;
	
	Если ИдПолЗагружаемый <> ИдПолИсходный Тогда
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", ИдПолИсходный);
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ИдПолЗагружаемый);
		Ошибки.Вставить("ИдПол", ДанныеСравнения);
	КонецЕсли;

	
	ЗагружаемыйДокумент = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыеДанные, "Документ", Неопределено);
	ТекущийДокумент = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагруженныеДанные, "Документ", Неопределено);
	
	Если ЗагружаемыйДокумент = Неопределено И ТекущийДокумент = Неопределено Тогда
		Возврат Ошибки;
	ИначеЕсли ЗагружаемыйДокумент = Неопределено Тогда 
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", 
			СтрШаблон(НСтр("ru = 'данные формата &1'"), ЗагруженныеДанные.Тип().URIПространстваИмен));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", НСтр("ru = 'нет данных'"));
		Ошибки.Вставить("Документ", ДанныеСравнения);
		Возврат Ошибки;
	КонецЕсли;
	
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
			ТекущийДокумент, "Функция") <> РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "Функция") Тогда
			
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "Функция"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "Функция"));
		Ошибки.Вставить("Функция", ДанныеСравнения);
			
	КонецЕсли;
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
			ТекущийДокумент, "ПоФактХЖ") <> РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "ПоФактХЖ") Тогда
				
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "ПоФактХЖ"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "ПоФактХЖ"));
		Ошибки.Вставить("ПоФактХЖ", ДанныеСравнения);
			
	КонецЕсли;
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
			ТекущийДокумент, "НаимДокОпр") <> РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимДокОпр") Тогда
				
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимДокОпр"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимДокОпр"));
		Ошибки.Вставить("НаимДокОпр", ДанныеСравнения);
			
	КонецЕсли;
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
			ТекущийДокумент, "НаимЭконСубСост") <> РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимЭконСубСост") Тогда
				
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимЭконСубСост"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимЭконСубСост"));
		Ошибки.Вставить("НаимЭконСубСост", ДанныеСравнения);
			
	КонецЕсли;
	РаспознаниеТекущего = Новый Структура;
	РаспознаниеТекущего.Вставить("НомерДокумента", "");
	РаспознаниеТекущего.Вставить("ДатаДокумента", Дата(1,1,1));
	РаспознаниеТекущего.Вставить("СуммаДокумента", 0); 
	ЗаполнитьЗначенияСвойств(РаспознаниеТекущего, 
		КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(ОписаниеФайла.ДвоичныеДанные,, Истина));
	
	РаспознаниеЗагружаемого = Новый Структура;
	РаспознаниеЗагружаемого.Вставить("НомерДокумента", "");
	РаспознаниеЗагружаемого.Вставить("ДатаДокумента", Дата(1,1,1));
	РаспознаниеЗагружаемого.Вставить("СуммаДокумента", 0);
	ЗаполнитьЗначенияСвойств(РаспознаниеЗагружаемого, 
		КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(ОписаниеЗагружаемогоФайла.ДвоичныеДанные,, Истина));
	Если РаспознаниеТекущего.НомерДокумента <> РаспознаниеЗагружаемого.НомерДокумента Тогда
		
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", РаспознаниеТекущего.НомерДокумента);
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", РаспознаниеЗагружаемого.НомерДокумента);
		Ошибки.Вставить("Номер документа", ДанныеСравнения);
			
	КонецЕсли;

	Если РаспознаниеТекущего.ДатаДокумента <> РаспознаниеЗагружаемого.ДатаДокумента Тогда
		
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", Формат(РаспознаниеТекущего.ДатаДокумента, "ДЛФ=Д"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", Формат(РаспознаниеЗагружаемого.ДатаДокумента, "ДЛФ=Д"));
		Ошибки.Вставить("Дата документа", ДанныеСравнения);
			
	КонецЕсли;
	
	Если РаспознаниеТекущего.СуммаДокумента <> РаспознаниеЗагружаемого.СуммаДокумента Тогда
		
		ДанныеСравнения = Новый Структура;
		ДанныеСравнения.Вставить("ВИсходномДокументе", Формат(РаспознаниеТекущего.СуммаДокумента, "ЧЦ=10; ЧДЦ=2"));
		ДанныеСравнения.Вставить("ВЗагружаемомДокументе", Формат(РаспознаниеЗагружаемого.СуммаДокумента, "ЧЦ=10; ЧДЦ=2"));
		Ошибки.Вставить("Сумма документа", ДанныеСравнения);
			
	КонецЕсли;
	
	Если ФайлСформированВ1С(ОписаниеЗагружаемогоФайла) = Ложь Тогда
		Подписант = СведенияОПодписанте(ОписаниеЗагружаемогоФайла, ОписаниеОшибки);
		Если Не ПодписантЗаполнен(Подписант, Организация, ОписаниеОшибки) 
			Или Не ВУчетнойЗаписиЕстьСертификатПодписанта(Подписант, ОписаниеОшибки) Тогда
    		СброситьСведенияОПодписанте(ОписаниеЗагружаемогоФайла);
		КонецЕсли;
		Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			ЗагружаемыйФайл.Хранение = ПоместитьВоВременноеХранилище(ОписаниеЗагружаемогоФайла.ДвоичныеДанные,
				ЗагружаемыйФайл.Хранение);
			ДанныеСравнения = Новый Структура;
			ДанныеСравнения.Вставить("ВИсходномДокументе", "");
			ДанныеСравнения.Вставить("ВЗагружаемомДокументе", НСтр("ru = 'нет сертификата'"));
			Ошибки.Вставить("Подписант", ДанныеСравнения);
		КонецЕсли;
	КонецЕсли;
	Возврат Ошибки;
КонецФункции
...
//  ОбъектXDTO.
//
Функция ДанныеФайлаЭД(ДанныеФайла, ОписаниеОшибки, ПространствоИменФНС = Неопределено, ИмяКорневогоУзла = "", МягкоеЧтение = Ложь) Экспорт
	...
	ИначеЕсли НРег(Прав(ДанныеФайла.ИмяФайла, 4)) = ".pdf" Тогда
		ДвоичныеДанныеФайла = Обработки.ФорматДоговорныйДокумент101.ДанныеДокументаИзФайлаОбмена(ДанныеФайла);
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Чтение ЭД.'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ОписаниеОшибки);
	...
	Если ЗначениеЗаполнено(ИдФайл) И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не МягкоеЧтение Тогда
			...
			ИначеЕсли СтрНайти(ИдФайл, "DP_IZVPOL") И ВерсФорм = "1.03" Тогда
				URIПространстваИмен = "DP_IZVPOL_1_982_00_01_03_01";
			...
			ИначеЕсли СтрНайти(ИдФайл, "DP_UVUTOCH") Тогда
				...
				ИначеЕсли ВерсФорм = "1.03" Тогда
					URIПространстваИмен = "DP_UVUTOCH_1_03";
				Иначе
			...
			ИначеЕсли СтрНайти(ИдФайл, "DP_PRIRASXSVED") Тогда
				URIПространстваИмен = "DP_PRIRASXDOP_5_01";
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
			МодульЭлектронноеАктированиеЕИС = ОбщегоНазначения.ОбщийМодуль("ЭлектронноеАктированиеЕИС");
			ДанныеФайлаЕИС =  МодульЭлектронноеАктированиеЕИС.ПриФормированияДанныхФайлаЭД(
				ДвоичныеДанныеФайла,
				ПотокДанныхФайла,
				ПространствоИменФайла,
				URIПространстваИмен,
				ИдФайл);
			Если ДанныеФайлаЕИС <> Неопределено Тогда
				Возврат ДанныеФайлаЕИС;
			КонецЕсли; 
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПространствоИменФайла) И ЗначениеЗаполнено(URIПространстваИмен) Тогда
			ДвоичныеДанныеФайла = ОбщегоНазначенияБЭД.ДобавитьПространствоИмен(ПотокДанныхФайла, URIПространстваИмен,
				Кодировка);
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			Если ИмяКорневогоУзла = "CommerceOfferRequest" Или ИмяКорневогоУзла = "CommerceOffer" Тогда
				ДанныеФайлаЭД = ФорматыЭДОСобытия.ДанныеФайлаФорматаBNCommerceOffering(ДвоичныеДанныеФайла);
			Иначе
				ТипКорневогоУзла = ТипКорневогоУзла(URIПространстваИмен);
				Если ТипКорневогоУзла <> Неопределено Тогда
					УдаляемыеАтрибуты = Новый Массив;
					УдаляемыеАтрибуты.Добавить("xsi:type");
					ДвоичныеДанныеФайла = УдалитьАтрибутыУзлов(ДвоичныеДанныеФайла, Кодировка, УдаляемыеАтрибуты);
				КонецЕсли;
				ЧтениеXML.ОткрытьПоток(ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения());
				ДанныеФайлаЭД = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ТипКорневогоУзла);
			КонецЕсли; 
		Исключение
			ШаблонСообщения = НСтр("ru = 'Ошибка при чтении данных из файла:
								   |%1'");
...
// Данные о идентификаторах, номере и дате документа по данным файла.
// 
// Параметры:
//  ОписаниеФайла - см. РаботаСФайламиБЭД.НовоеОписаниеФайла
//  ОписаниеОшибки - Строка
//  
// Возвращаемое значение:
//  Неопределено, Структура - Настройка отправки по данным файла:
// * ИдентификаторОрганизации - Строка
// * ИдентификаторКонтрагента - Строка
// * ДатаДокумента - Строка
// * НомерДокумента - Строка
Функция ИдентификаторыДатаНомерДокументаПоДаннымФайла(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	НастройкаОтправки = Новый Структура;
	
	Попытка
		ДанныеФайлаЭД = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	Исключение
		Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			ШаблонСообщения = НСтр("ru = 'Ошибка: %1 
									|при чтении данных из файла:
									|%2'");
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ОписаниеФайла.ИмяФайла, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдОтпр") Тогда
		НастройкаОтправки.Вставить("ИдентификаторОрганизации", 
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "СвУчДокОбор.ИдОтпр"));
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	Если ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдПол") Тогда 
		НастройкаОтправки.Вставить("ИдентификаторКонтрагента", 
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "СвУчДокОбор.ИдПол"));
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдПок") Тогда
		НастройкаОтправки.Вставить("ИдентификаторКонтрагента",
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "СвУчДокОбор.ИдПок"));
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	РаспознаниеТекущего = КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(ОписаниеФайла.ДвоичныеДанные,, Истина);
	
	Если РаспознаниеТекущего.Свойство("НомерДокумента") Тогда
		НастройкаОтправки.Вставить("НомерДокумента", РаспознаниеТекущего.НомерДокумента);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Если РаспознаниеТекущего.Свойство("ДатаДокумента") Тогда
		НастройкаОтправки.Вставить("ДатаДокумента", РаспознаниеТекущего.ДатаДокумента);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
// * СвойстваДокументовПоФорматам - см. НовоеОписаниеСвойствДокументовПоФорматам
//
Функция НовоеОписаниеФайлаДокумента() Экспорт
	...
	Содержание.Вставить("СвойстваДокументовПоФорматам",
		НовоеОписаниеСвойствДокументовПоФорматам());
		
	Возврат Содержание;
	
КонецФункции

// Новое описание свойств файлов документов по форматам.
// 
// Возвращаемое значение:
//  Структура:
//   * АктОРасхождениях - Структура:
//    ** ФормируютсяИтогиПриемки - Булево - признак того, что для ЭД формируются итоги приемки.
//    ** ВариантФормированияИтоговПриемки - Строка - значение варианта формирования итогов приемки.
Функция НовоеОписаниеСвойствДокументовПоФорматам() Экспорт
	
	Результат = Новый Структура;	
	
	АктОРасхождениях = Новый Структура;
	АктОРасхождениях.Вставить("ФормируютсяИтогиПриемки", Ложь);
	АктОРасхождениях.Вставить("ВариантФормированияИтоговПриемки", "");
...
// Возвращаемое значение:
//  Структура - параметры:
//   * ОсновнойФайл - см. РаботаСФайламиБЭД.НовоеОписаниеФайла
//   * ДополнительныйФайл - см. РаботаСФайламиБЭД.НовоеОписаниеФайла
...
// Возвращаемое значение:
//  - Неопределено
//  - См. ДанныеЭлектронногоДокументаОсновныхТипов
...
// Извлекает ИНН доверителя машиночитаемой доверенности из файла формализованного электронного документа 
//
// Параметры:
//  ОписаниеФайла - см. РаботаСФайламиБЭД.НовоеОписаниеФайла
//  Формат - Строка
...
//
Функция ИННДоверителяМашиночитаемойДоверенности(ОписаниеФайла, Формат) Экспорт
	
	Результат = Новый Соответствие();
	
	Если ЗаполнениеДанныхДоверенностиДоступноПоФормату(Формат) Тогда
		ФорматыЭДО_ФНС.ЗаполнитьИННДоверителейМашиночитаемыхДоверенностей(Результат, ОписаниеФайла, Формат);
	КонецЕсли;
...
//
// Параметры:
//  Параметры - см. НовыеПараметрыПолученияДанныхДокумента
//  ИменаДопКолонок - Строка - имена дополнительных колонок через запятую.
// 
// Возвращаемое значение:
//  - Неопределено
//  - См. НоваяТаблицаТоваров
//
...
// * Артикул - Строка
// * СтавкаНДС - Строка
// * СодержитИзменяемыеДанные - Строка
...
Функция НовыеДанныеДляСопоставленияТоваров() Экспорт
	...
	Сопоставление.Вставить("ТипНоменклатурыФНС"    , Неопределено);
...
Функция РасшифровкаВариантовЗаполненияФорматовЭДО() Экспорт
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = Обработки.ФорматДоговорныйДокумент101.ПространствоИмен();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент;
	НоваяСтрока.ВариантЗаполнения = "ДоговорныйДокумент101";
	///////////////////////////////////////////////////////////////////////////////////
	
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПространствоИмен();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент;
	НоваяСтрока.ВариантЗаполнения = "ДоговорныйДокументXML";
	///////////////////////////////////////////////////////////////////////////////////
	
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = Обработки.ФорматДокументПредприятия.ПространствоИмен();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДокументПредприятия;
	НоваяСтрока.ВариантЗаполнения = "ДокументПредприятия";
	///////////////////////////////////////////////////////////////////////////////////
...
// Возвращает структуру с вариантами формирования итогов приемки.
// 
// Возвращаемое значение:
//   ФиксированнаяСтруктура:
//     * НеФормируются - Строка - 1.
//     * ПокупателемФормируютсяДопСведенияОбОприходованииИмЦенностей - Строка - 2.
//     * ПокупателемФормируютсяДопСведенияОУтвержденииДокумента - Строка - 3.
//     * ПокупателемФормируютсяДопСведенияИные - Строка - 4.
//     * ПродавцомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки - Строка - 5.
//     * ПродавцомФормируютсяДопСведенияИные - Строка - 6.
//     * ПеревозчикомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки - Строка - 7.
//     * ПеревозчикомФормируютсяДопСведенияИные - Строка - 8.
//
Функция ВариантыФормированияИтоговПриемки() Экспорт
	
	Варианты = Новый Структура();
	
	Варианты.Вставить("НеФормируются", "1");
	Варианты.Вставить("ПокупателемФормируютсяДопСведенияОбОприходованииИмЦенностей", "2");
	Варианты.Вставить("ПокупателемФормируютсяДопСведенияОУтвержденииДокумента", "3");
	Варианты.Вставить("ПокупателемФормируютсяДопСведенияИные", "4");
	Варианты.Вставить("ПродавцомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки", "5");
	Варианты.Вставить("ПродавцомФормируютсяДопСведенияИные", "6");
	Варианты.Вставить("ПеревозчикомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки", "7");
	Варианты.Вставить("ПеревозчикомФормируютсяДопСведенияИные", "8");
...
#Область ПравилаПреобразованияФорматов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Читаемые.Добавить(Метаданные.Константы.ДатаЗагрузкиОписанияФорматовДляПреобразования.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");

	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ФорматыДляПреобразования.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ПравилаПреобразованияФорматов.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.Константы.ДатаЗагрузкиОписанияФорматовДляПреобразования.ПолноеИмя());
	...
	Обработчик.Комментарий = НСтр(
		"ru = '1С:Обмен с контрагентами: обновление правил преобразования форматов (начальное заполнение).'");
...
Функция ВычислитьЗначенияДополнительныхПолейШапки(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	Исключение
		ОбработатьОшибкуВыполненияЗапросаКонструктораДополнительныхПолей(
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПараметрыЗаполнения);
		Возврат ДополнительныеДанные;
	КонецПопытки;
...
Функция СодержаниеДокументаПоПравиламКонвертации(ОписаниеФайла)
	...
	ИначеЕсли НРег(Файл.Расширение) = ".pdf" Тогда
		ДвоичныеДанные = Обработки.ФорматДоговорныйДокумент101.ДанныеДокументаИзФайлаОбмена(ОписаниеФайла);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Если СодержаниеДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность 
		И НЕ СтрЗаканчиваетсяНа(НРег(ОписаниеФайла.ИмяФайла), ".zip") Тогда
		СодержаниеДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.Прочее;
	КонецЕсли;
...
Функция ТипРегламентаПоЗначениюСервиса(Значение)
	...
	ИначеЕсли Значение = "АктОРасхождениях" Тогда
		ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях;
	КонецЕсли;
...
//
// Параметры:
//  ПараметрыПолученияДанных - см. ДанныеЭлектронногоДокумента.ПараметрыПолученияДанных
// 
// Возвращаемое значение:
//  - Неопределено
//  - Структура:
...
Функция ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанных)
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда //чтение файлов без namespace по форматам ФНС
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "DP_PRIRASXSVED") > 0 Тогда
			ФорматыЭДО_ФНС.ПрочитатьДополнительныеСведенияАктОРасхожденияхXDTO(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
			НовыйЭД.ВерсияФормата = "DP_PRIRASXDOP_1_994_02_05_01_01";
...
Функция ОпределитьПараметрыФайлаПрикладногоЭлектронногоДокумента(ПараметрыПолученияДанных, РасширениеФайла)
	...
	Содержание.Формат = ПоддерживаемыеФорматы().Прикладной;
...
// Удаляет свойства в узлах XML
//
// Параметры:
//  ДвоичныеДанныеФайла	 - ДвоичныеДанные 
//  Кодировка			 - Строка
//  УдаляемыеАтрибуты	 - Массив Из Строка
//
// Возвращаемое значение:
//  ДвоичныеДанные
//
Функция УдалитьАтрибутыУзлов(Знач ДвоичныеДанныеФайла, Знач Кодировка, Знач УдаляемыеАтрибуты)
	
	Если УдаляемыеАтрибуты.Количество() = 0 Тогда
		Возврат ДвоичныеДанныеФайла;
	КонецЕсли;
	
	ЧтениеXML = Новый ЧтениеXML;
	ПотокДляЧтения = ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения();
	ЧтениеXML.ОткрытьПоток(ПотокДляЧтения, , , Кодировка);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
				...
				Если УдаляемыеАтрибуты.Найти(ЧтениеXML.Имя) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
...
// 
// Возвращаемое значение:
//  - Неопределено - если тип не удалось определить или пространство имен не определено
//  - ТипЗначенияXDTO
//  - ТипОбъектаXDTO
Функция ТипКорневогоУзла(URIПространстваИмен)

	ТипКорневогоУзла = Неопределено;
	Если URIПространстваИмен = "ON_AKTSVEROTP_1_972_01_05_01_01" Или URIПространстваИмен = "ON_AKTSVERPOL_1_972_02_05_01_01"
		Или URIПространстваИмен = "ON_AKTREZRABP_1_971_01_01_00_01" Или URIПространстваИмен = "ON_AKTREZRABZ_1_971_02_01_00_01"
		Или URIПространстваИмен = Обработки.ФорматДоговорныйДокумент101.ПространствоИмен() Или URIПространстваИмен
		= Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПространствоИмен() Тогда
		...
		СвойствоФайл = Пакет.КорневыеСвойства.Получить("Файл");
		ТипКорневогоУзла = СвойствоФайл.Тип;
	ИначеЕсли URIПространстваИмен = "ON_CHETOP_1_998_01_01_01" Тогда
		ВерсияСхемы = "4.01";
		ТипКорневогоУзла = ПолучитьТипОбъектаCML(СтрШаблон("{%1}.Файл", URIПространстваИмен), ВерсияСхемы);
	Иначе
		ТипКорневогоУзла = ПолучитьТипЗначенияCML("Файл", URIПространстваИмен);
	КонецЕсли;
	Возврат ТипКорневогоУзла;
...
Функция ПространствоИменАкта(ДанныеФайлаЭД, ИдентификаторЭД)
	...
	Иначе
		ФлагФНС = 2;
	КонецЕсли;
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "СвУчДокОбор.СвОЭДОтпрСФ") <> Неопределено Тогда
		ФлагФНС = 3;
	КонецЕсли;
...
// Возврашает массив путей к информационным полям формата, предназначенным для размещения данных по МЧД.
// 
// Параметры:
//  ДанныеФайлаЭД - См. ДанныеФайлаЭД
//  Формат - Строка
//  ПоддерживаемыеФорматы - См. ПоддерживаемыеФорматы
// 
// Возвращаемое значение:
//  Массив Из Строка
//
Функция ПутиКИнформационнымПолямФорматаЭДДляМЧД(ДанныеФайлаЭД, Формат, ПоддерживаемыеФорматы)
	
	ПутиКИнформационнымПолям = Новый Массив;
	ФункцияДокумента = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ.Функция");
	
	Если Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПродавца
		И (ФункцияДокумента = "СЧФДОП" Или ФункцияДокумента = "ДОП") Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СвСчФакт.ИнфПолФХЖ1");
		ПутиКИнформационнымПолям.Добавить("Документ.СвПродПер.ИнфПолФХЖ3");
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПокупателя Тогда
		ПутиКИнформационнымПолям.Добавить("ИнфПок.СодФХЖ4.ИнфПолФХЖ4");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПродавца Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СвДокПТПрКроме.СвДокПТПр.СодФХЖ1.ИнфПолФХЖ1");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПокупателя Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СодФХЖ4.ИнфПолФХЖ4");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПокупателя Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СодФХЖ3.ИнфПолФХЖ3");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПродавца Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СвДокПРУ.СодФХЖ1.ИнфПолФХЖ1");
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СвКСчФ.ИнфПолФХЖ1");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ИнформацияПокупателя Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СодФХЖ1.ИнфПолФХЖ1");
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения Тогда
		ПутиКИнформационнымПолям.Добавить("Документ.СвИтПрием.ИнфПол");
	КонецЕсли;
	
	Возврат ПутиКИнформационнымПолям;
	
КонецФункции

// Заполняет данные доверенностей в файле электронного документа по переданным путям к полям формата.
// 
// Параметры:
//  ДанныеФайлаЭД - См. ДанныеФайлаЭД
...
//  ПутиКПолямФормата - Массив Из Строка
//
Процедура ЗаполнитьДанныеДоверенностейВСодержимомЭД(ДанныеФайлаЭД, Доверенности, ПространствоИмен, ПутиКПолямФормата)

	Для Каждого ПутьКПолю Из ПутиКПолямФормата Цикл
		
		ПолныйПутьКПолю = СтрШаблон("Файл.%1", ПутьКПолю);
		НовоеПоле = ПолучитьОбъектТипаCML(ПолныйПутьКПолю, ПространствоИмен);
		ПолеФайлаЭД = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, ПутьКПолю, НовоеПоле);
		
		ЗаполнитьДанныеДоверенностейВИнформационномПолеЭД(Доверенности, ПолеФайлаЭД, ПространствоИмен);
		
		ПозицияПоследнегоРазделителя = СтрНайти(ПутьКПолю, ".", НаправлениеПоиска.СКонца);
		ИмяПоля = Прав(ПутьКПолю, СтрДлина(ПутьКПолю) - ПозицияПоследнегоРазделителя);
		ПутьБезИмениПоля = Лев(ПутьКПолю, ПозицияПоследнегоРазделителя - 1);
		ТекущийДокумент = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, ПутьБезИмениПоля);
		
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекущийДокумент, ИмяПоля, ПолеФайлаЭД, Истина);
		
	КонецЦикла;

КонецПроцедуры

// Заполняет данные МЧД подписанта в файле электронного документа УОУ, ИОП.
// 
// Параметры:
//  ДанныеФайлаЭД - см. ДанныеФайлаЭД
//  Доверенности - Массив из СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  ПространствоИмен - Строка - пространство имен схемы документа УОУ, ИОП.
//
Процедура ЗаполнитьДанныеДоверенностейЭД_УОУ_ИОП(ДанныеФайлаЭД, Доверенности, ПространствоИмен) 
	
	Если Не Доверенности.Количество() Тогда
		// Лицо, имеющее полномочия на подписание документа без доверенности.
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант, "СтатПодп", "1", Истина); 
		Возврат;
	КонецЕсли;
	Доверенность = Доверенности[0];
	
	// Лицо, имеющее полномочия на подписание документа на основании доверенности в электронной форме.
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант, "СтатПодп", "2", Истина);
	
	Если ЗначениеЗаполнено(Доверенность) Тогда
		// Сведения о доверенности.
		СвДоверЭл = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML("Файл.Документ.Подписант.СвДоверЭл", ПространствоИмен);  					
		
		ДанныеДоверенности = МашиночитаемыеДоверенности.ДанныеДоверенностиДляКонтейнера(Доверенность);
		
		НомерДоверенности = МашиночитаемыеДоверенности.ПолучитьИдентификаторМЧД(Доверенность);

		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "НомДовер", НомерДоверенности, Истина);
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ДатаВыдДовер", Формат(ДанныеДоверенности.ДатаВыдачи, "ДФ=dd.MM.yyyy"), Истина);
		
		Если ДанныеДоверенности.ЭтоФайловаяДоверенность Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "СпособПредставл", "1", Истина);
		Иначе
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "СпособПредставл", "2", Истина);
			ИмяСвойства = "ИдСистХран";
			Если Найти(ДанныеФайлаЭД.ИдФайл, "DP_UVUTOCH") > 0 Тогда
			    ИмяСвойства = "СвСистХран";
			КонецЕсли;
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, ИмяСвойства, ДанныеДоверенности.СсылкаНаРеестр, Истина);
		КонецЕсли;
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант, "СвДоверЭл", СвДоверЭл, Истина);	
	КонецЕсли; 

КонецПроцедуры

// Заполняет данные доверенностей в информационном поле файла электронного документа.
// 
// Параметры:
//  Доверенности - Массив Из ОпределяемыйТип.МашиночитаемаяДоверенность
//  ИнформационноеПоле - СписокXDTO - Элемент файла обмена, содержаший произвольные сведения в текстовом формате
//  ПространствоИмен - Строка - Пространство имен схемы документа
Процедура ЗаполнитьДанныеДоверенностейВИнформационномПолеЭД(Доверенности, ИнформационноеПоле, ПространствоИмен)
	...
	Для Каждого Доверенность Из Доверенности Цикл
		...
		Если МассивМЧД.Найти(ДанныеДоверенности.НомерДоверенности) = Неопределено Тогда
			ТекстИнфТип = ПолучитьОбъектТипаCML(ИнформационноеПоле.ТекстИнф.ВладеющееСвойство.Тип, ПространствоИмен);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Идентиф", "МЧД", Истина);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Значен", ДанныеДоверенности.НомерДоверенности, Истина);
			ТекстИнф.Добавить(ТекстИнфТип);
			
			ТекстИнфТип = ПолучитьОбъектТипаCML(ИнформационноеПоле.ТекстИнф.ВладеющееСвойство.Тип, ПространствоИмен);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Идентиф", "Дата совершения МЧД", Истина);
			...
			Если ЗначениеЗаполнено(ДанныеДоверенности.СсылкаНаРеестр) Тогда
				ТекстИнфТип = ПолучитьОбъектТипаCML(ИнформационноеПоле.ТекстИнф.ВладеющееСвойство.Тип, ПространствоИмен);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Идентиф", "Сведения об информационной системе", Истина);
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Значен", ДанныеДоверенности.СсылкаНаРеестр, Истина);
				ТекстИнф.Добавить(ТекстИнфТип);
			КонецЕсли;
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Значен", ДанныеДоверенности.ДоверительИНН, Истина);
			ТекстИнф.Добавить(ТекстИнфТип);
		КонецЕсли;
	КонецЦикла;
...
Функция СоответствиеТитулов()
	...
	СоответствиеТитулов.Вставить(Форматы.ФНС.АктОРасхождениях.ИнформацияПокупателя,
		Форматы.ФНС.АктОРасхождениях.ДополнительныеСведения);
...
Функция РазобратьУниверсальныйДопФайл(ДанныеФайла)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			ДанныеФайла.ИмяФайла, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Чтение ЭД.'"),
...
Функция МетаданныеОбработокФорматов()
	...
	МассивМетаданных.Добавить(Метаданные.Обработки.ФорматДоговорныйДокумент101);
	МассивМетаданных.Добавить(Метаданные.Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО);
	МассивМетаданных.Добавить(Метаданные.Обработки.ФорматДокументПредприятия);
	Возврат МассивМетаданных;
КонецФункции

// Возвращает массив типов документов для которых невозможно формирование ответного титула без учетных документов.
// 
// Возвращаемое значение:
// 	Массив из ПеречислениеСсылка.ТипыДокументовЭДО
Функция ТипыДокументовНеПоддерживающиеФормированиеОтветногоТитулаБезДокументаУчета()

	МассивТиповДокументов = Новый Массив;
	МассивТиповДокументов.Добавить(
		ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.АктСверкиВзаиморасчетов"));
...
// Возвращаемое значение:
//  см. ОбработкаФормата
Функция НайтиОбработкуФорматаПоФайлуОбмена(ИмяФайлаОбмена)
	Для Каждого Обработка Из ОбработкиФорматов() Цикл
		Попытка
...
// Это единица измерения классификатора.
// 
// Параметры:
//  Код - Строка - Код единицы измерения
//  КлассификаторЕдиницИзмерения - См. ПолучитьКлассификаторЕдиницИзмерения
// 
// Возвращаемое значение:
//  Булево - Это единица измерения классификатора
Функция ЭтоЕдиницаИзмеренияКлассификатора(Код, КлассификаторЕдиницИзмерения) Экспорт
	
	ЯчейкаСКодом = КлассификаторЕдиницИзмерения.НайтиТекст(Код, , КлассификаторЕдиницИзмерения.Области.КодЧисловой,
		Истина, Истина, Истина, Истина);
	Возврат ЯчейкаСКодом <> Неопределено;
	
КонецФункции

// Возвращаемое значение:
//  ТабличныйДокумент:
//  * Области - КоллекцияОбластейТабличногоДокумента:
//  ** КодЧисловой - ОбластьЯчеекТабличногоДокумента
Функция КлассификаторЕдиницИзмерения() Экспорт
	Возврат Обработки.ОбменСКонтрагентами.ПолучитьМакет("КлассификаторЕдиницИзмерения");
КонецФункции

// Форматы, которые нужно исключить из поддержки
//
// Возвращаемое значение:
//  Массив Из Строка - форматы, которые требуется исключить
//
Функция НеподдерживаемыеФорматыЭлектронныхДокументов()
	
	Массив = Новый Массив;
	
	ИспользуетсяФорматДоговорногоДокументаXML = Ложь;
	
	ОбменСКонтрагентамиПереопределяемый.УстановитьПризнакИспользованияФорматаДоговорногоДокументаXML(
		ИспользуетсяФорматДоговорногоДокументаXML);
	
	Если Не ИспользуетсяФорматДоговорногоДокументаXML Тогда
		Массив.Добавить(Обработки.ФорматДоговорныйДокументИнформацияОтправителяЭДО.ПространствоИмен());
	КонецЕсли;
...
//  Неопределено
Функция ПолучитьСведенияОФормате(Формат)
...
// Проверяет совместимость форматов по UriПространствоИмен
// 
// Параметры:
//  UriФорматаЗагружаемый - Строка - UriПространствоИмен загружаемый
//  UriФорматаЗагруженный - Строка - UriПространствоИмен загруженный
// 
// Возвращаемое значение:
// Булево - форматы не совместимы
Функция ФорматыНеСовместимы(UriФорматаЗагружаемый, UriФорматаЗагруженный)
	Запрос = Новый Запрос;
	
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СвязьВидовИФорматовДокументовЭДО.Формат КАК Формат,
		|	СвязьВидовИФорматовДокументовЭДО.ВидДокумента КАК ВидДокумента
		|ПОМЕСТИТЬ АктуальныеФорматы
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗагружаемыйФормат.ВидДокумента КАК ВидДокумента
		|ИЗ
		|	АктуальныеФорматы КАК ЗагружаемыйФормат
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АктуальныеФорматы КАК ЗагруженныйФормат
		|		ПО ЗагружаемыйФормат.ВидДокумента = ЗагруженныйФормат.ВидДокумента
		|			И (ЗагружаемыйФормат.Формат = &ЗагружаемыйФормат)
		|			И (ЗагруженныйФормат.Формат = &ЗагруженныйФормат)";
	

	Запрос.УстановитьПараметр("ЗагружаемыйФормат", UriФорматаЗагружаемый);
	Запрос.УстановитьПараметр("ЗагруженныйФормат", UriФорматаЗагруженный);
	УстановитьПривилегированныйРежим(Истина);
