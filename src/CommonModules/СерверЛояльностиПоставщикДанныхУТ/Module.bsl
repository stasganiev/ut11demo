...
Функция ПолучитьТаблицуОстатков(ПараметрыПолученияОстатков) Экспорт
	...
	|	ОстаткиТоваров.ВНаличии КАК Остаток,
	|	ОстаткиТоваров.Резерв КАК ВРезерве,
	|	ОстаткиТоваров.Свободно КАК Свободно
	|ПОМЕСТИТЬ ВТОстатки
	|ИЗ
	|	РегистрСведений.РаспределениеЗапасов КАК ОстаткиТоваров
	|ГДЕ
	...
	|	СУММА(ВТОстатки.Остаток / ЕСТЬNULL(ЕдиницыИзмерения.Числитель, 1)) КАК Остаток,
	|	СУММА(ВТОстатки.ВРезерве / ЕСТЬNULL(ЕдиницыИзмерения.Числитель, 1)) КАК ВРезерве,
	|	СУММА(ВТОстатки.Свободно / ЕСТЬNULL(ЕдиницыИзмерения.Числитель, 1)) КАК Свободно
	|ИЗ
