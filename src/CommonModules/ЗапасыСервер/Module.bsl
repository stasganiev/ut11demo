...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект,
	...
	ЗаписыватьРезервыТоваровОрганизацийВместеСоВсеми = Не ((СозданиеСторноДокумента
...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект,
	...
	|	ВЫБОР
	|		КОГДА Настройка.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)
	...
	|ИЗ
	|	РегистрСведений.НастройкиСистемыНалогообложения.СрезПоследних(&Дата, НЕ &ПоВсемНалогообложениямНДС) КАК Настройка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК СпрОрганизации
	|		ПО Настройка.Организация = СпрОрганизации.ГоловнаяОрганизация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПроверитьЗаполнениеНомеровГТД(ДокументОбъект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	|			И НЕ ЕСТЬNULL(ТаблицаТовары.Номенклатура.ПрослеживаемыйТовар, ЛОЖЬ))
	|		ИЛИ (ЕСТЬNULL(ТаблицаТовары.Номенклатура.ПрослеживаемыйТовар, ЛОЖЬ)
	|			И НАЧАЛОПЕРИОДА(&ДатаДокумента, МЕСЯЦ) >= &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			И НЕ &ЭтоЗакупкаВСтранахЕАЭС))";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяПоляНоменклатура", "ИсходнаяТаблица." + ИмяПоляНоменклатура);
	...
	Запрос.УстановитьПараметр("ДатаДокумента",	ПараметрыДокумента.Дата);
	Запрос.УстановитьПараметр("ТаблицаТовары",	ТаблицаТовары);
...
Функция СторнироватьДвиженияТоварыОрганизацийВПути(Знач ШаблонПериод = "&КонецПериода", Знач ИсправленияРазвернутогоСальдо = Ложь) Экспорт
	...
	ТекстЗапроса = 
		...
		|		И ДанныеДокумента.Договор.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваров)
		...
		|	ВидЗапасовСобственныйТовар.Ссылка				КАК ВидЗапасов,
		|	ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)		КАК НомерГТД,
		|	ВЫБОР
		|		КОГДА ТаблицаРасхождений.ВидРасхождений = ЗНАЧЕНИЕ(Перечисление.ВидыРасхожденийПоступленийПриобретений.Недопоставка)
		|			ТОГДА 1
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеРасхожденийПоступлениеПриобретение КАК ДанныеДокумента
		|		ПО ДанныеДокумента.Ссылка = ТаблицаРасхождений.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидЗапасовСобственныйТовар
		|		ПО ТаблицаРасхождений.ВидЗапасов.Организация = ВидЗапасовСобственныйТовар.Организация
		|			И ВидЗапасовСобственныйТовар.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
		|			И ТаблицаРасхождений.ВидЗапасов.ГруппаФинансовогоУчета = ВидЗапасовСобственныйТовар.ГруппаФинансовогоУчета
		|			И ТаблицаРасхождений.ВидЗапасов.ГруппаПродукции = ВидЗапасовСобственныйТовар.ГруппаПродукции
		|			И ТаблицаРасхождений.ВидЗапасов.НалогообложениеНДС = ВидЗапасовСобственныйТовар.НалогообложениеНДС
		|ГДЕ
		|	ДанныеДокумента.Проведен
		|	И ДанныеДокумента.Дата <= &ШаблонПериод  
		|	И НЕ ДанныеДокумента.Ссылка В 
		...
		|			(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
		|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
		|	И (ТаблицаРасхождений.Номенклатура.ПрослеживаемыйТовар
		|		ИЛИ ТаблицаРасхождений.Номенклатура.ВестиУчетПоГТД)
		|	И ВЫБОР 
		|		КОГДА ДанныеДокумента.Распоряжение ССЫЛКА Документ.ЗаказПоставщику 
		|			ТОГДА ВЫРАЗИТЬ(ДанныеДокумента.Распоряжение КАК Документ.ЗаказПоставщику).ХозяйственнаяОперация
		|		КОГДА ДанныеДокумента.Распоряжение ССЫЛКА Документ.ПриобретениеТоваровУслуг 
...
Процедура СформироватьВТТоварыОрганизацийИзменения(ДокументОбъект,
	...
	|	ГДЕ
	|		ТоварыОрганизаций.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|		И НЕ (ТоварыОрганизаций.Сторно
	|			И &ИсключатьДвиженияСторно)
	|		
	...
	Запрос.УстановитьПараметр("ИсключатьДвиженияСторно",		ИсключатьДвиженияСторно);
...
Процедура СформироватьВтОтборыОстатковПоТаблицеТоваров(МенеджерВременныхТаблиц, ДобавлятьАналитикиИзДвиженийПередЗаписью, ПодбиратьЗапасыРазныхПериодов = Ложь)
	...
	ТекстЧастиЗапроса =
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	Товары.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))";
