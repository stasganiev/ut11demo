...
//
Функция НовыйОбъектДО(Прокси, Тип, Предмет = Неопределено, Колонки = Неопределено,
...
// Определяет поддерживает ли БИД объект метаданных.
//
// Параметры:
...
Функция ПодходящийТип(ТипыКандидаты, ЗначениеСвойстваXDTO = Неопределено, ТипТекущегоЗначения = Неопределено) Экспорт
	...
	Иначе
		...
		ТипПоЗначениюСвойстваXDTO = Неопределено;
		Если ТипЗнч(ЗначениеСвойстваXDTO) = Тип("ОбъектXDTO") Тогда
			...
			КонецЕсли;
			ТипПоЗначениюСвойстваXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ТипИС(
				ТипОбъектаXDTO);
		КонецЕсли;
		
		Если ТипыКандидаты.Найти(ТипПоЗначениюСвойстваXDTO) <> Неопределено Тогда
			ПодходящийТип = ТипПоЗначениюСвойстваXDTO;
...
Процедура ПриОпределенииЗначенияРеквизитаПоОбъектуXDTO(Результат, ЗначениеЗаполнения, ТипРеквизита, ИмяРеквизита,
	...
	Если Результат = Неопределено Тогда
		...
		Если ТипРеквизита = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипПользователи()
		...
		ИначеЕсли ТипРеквизита = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипПользователи()
				И ТипЗнч(ЗначениеЗаполнения) = Тип("ОбъектXDTO")
				И СвойствоУстановлено(ЗначениеЗаполнения, "objectID")
				И (ЗначениеЗаполнения.objectID.type = "DMParty"
					Или ЗначениеЗаполнения.objectID.type = "DMPartyRowContact"
					Или ЗначениеЗаполнения.Тип().Имя = "DMParty"
					Или ЗначениеЗаполнения.Тип().Имя = "DMPartyRowContact") Тогда
		...
		ИначеЕсли ТипРеквизита = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипВалюты()
				И ТипЗнч(ЗначениеЗаполнения) = Тип("ОбъектXDTO")
				И СвойствоУстановлено(ЗначениеЗаполнения, "objectID")
		...
		ИначеЕсли ТипРеквизита = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипКлассификаторБанков()
				И ТипЗнч(ЗначениеЗаполнения) = Тип("ОбъектXDTO")
				И СвойствоУстановлено(ЗначениеЗаполнения, "objectID")
...
Функция РеквизитыОбъектаИС(ТипОбъектаИС, ДляВыражения = Ложь) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьПереопределяемый.ПолучитьРеквизитыОбъектаИС(
		ТипОбъектаИС,
		Реквизиты);
...
Функция ДанныеФайловОбъектаИС(Источник, ИдентификаторФормы, Знач СписокРасширений = Неопределено) Экспорт
	...
	Для Каждого ФайлXDTO Из НужныеФайлыXDTO Цикл
		...
		ДанныеФайла.СсылкаНаДвоичныеДанныеФайла = ПоместитьВоВременноеХранилище(ФайлXDTO.binaryData, ИдентификаторФормы);
...
Функция ПараметрыПечатиВСтроку(ПараметрыПечати) Экспорт
	...
	СтруктураДляОбработки = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ПараметрыПечати);
...
Процедура ДобавитьКомандуФайлы(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		...
		ВерсияБСП = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ВерсияБСП();
...
Функция ПроверкаПодключенияПоЛогинуПаролю(АдресВебСервисаДокументооборота, ДанныеСервиса, ИмяПользователя, Пароль,
	...
	Если ЗначениеЗаполнено(ИмяПользователя) Тогда
		...
		Исключение
			...
			ТекстСообщенияОбОшибке = ТекстСообщенияОбОшибке
				+ ?(ТекстСообщенияОбОшибке = "", "", РазделительТекстаОшибки)
				+ СокрЛП(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
