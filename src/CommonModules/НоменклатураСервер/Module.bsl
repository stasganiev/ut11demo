...
Процедура УстановитьУсловноеОформлениеЕдиницИзмерения(Форма,
		...
		ПутьКПолюПризнакаИспользованияАдресногоХранения = "") Экспорт
	...
	Если ЗначениеЗаполнено(ПутьКПолюПризнакаИспользованияАдресногоХранения) Тогда
		...
		ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПутьКПолюПризнакаИспользованияАдресногоХранения);
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура УстановитьУсловноеОформлениеНомераГТД(Форма,
												...
												ПутьКПолюОтбора = "Объект.Товары.ВедетсяУчетПоГТД",
												ПутьКПолюПризнакаОтбораПоРНПТ = "") Экспорт
...
Функция НомераСтрокСНекорректнымОкруглениемКоличества(Объект, ПараметрыПроверки = Неопределено) Экспорт
	...
	КонецЦикла;
	Таблица.Скопировать(Новый Структура("ОкруглениеВыполненоКорректно", Ложь), "НомерСтроки");
	Возврат Таблица;
...
Функция ЕстьВидНоменклатурыТребующийОтключенияОрдернойСхемыПриПоступленииИОтгрузки(Склад) Экспорт
	...
	|	НЕ ВидыНоменклатуры.ЭтоГруппа
	|	И ВидыНоменклатуры.ИспользоватьСерии
	|	И (ВидыНоменклатуры.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС)
	...
	|	И (ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий ЕСТЬ NULL
	|				И ВидыНоменклатуры.ПолитикаУчетаСерий <> ЗНАЧЕНИЕ(Справочник.ПолитикиУчетаСерий.СерииНеИспользуются)
	|				И НЕ ВидыНоменклатуры.ПолитикаУчетаСерий.УчитыватьСебестоимостьПоСериям
	|			ИЛИ НЕ ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий ЕСТЬ NULL
	|				И ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий <> ЗНАЧЕНИЕ(Справочник.ПолитикиУчетаСерий.СерииНеИспользуются)
	|				И НЕ ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий.УчитыватьСебестоимостьПоСериям)";
