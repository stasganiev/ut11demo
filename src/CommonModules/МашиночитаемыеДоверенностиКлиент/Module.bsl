...
Процедура ЗарегистрироватьМЧДПослеПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЗарегистрироватьВРеестреФНС Тогда
		...
		Иначе
			...
			Если СтрЧислоВхождений(РезультатРегистрации.ТекстОтвета, "В данный момент система не поддерживает работу") Тогда // АПК:1297 не выводится пользователю
				ТекстСообщения = ТекстСообщения 
					+ Символы.ПС 
					+ НСтр("ru = 'На данный момент ФНС не поддерживает регистрацию такого типа доверенности'");	
			КонецЕсли;
			Если СтрЧислоВхождений(РезультатРегистрации.ТекстОтвета, "ТипПред, равным '1'") Тогда // АПК:1297 не выводится пользователю
