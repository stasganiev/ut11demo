...
//
Функция ЗаписатьОбъектАсинхронно(УникальныйИдентификатор, ОбъектСтрока) Экспорт
...
//   см. ДлительныеОперации.ВыполнитьВФоне
//
Функция ПолучитьОбъектАсинхронно(УникальныйИдентификатор, Тип, ID, Колонки = Неопределено,
...
Функция ДанныеЗаполненияИнтегрируемогоОбъекта(УИДПравила, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	Объект = ИнтеграцияС1СДокументооборотБазоваяФункциональность.НовыйОбъектИС(ТипОбъектаИС);
	...
	РеквизитыОбъекта = ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитыОбъектаИС(ТипОбъектаИС);
...
Функция ДанныеОбъектаДОПоВнешнемуОбъекту(СсылкаНаВнешнийОбъект, ПолучитьИзДО = Ложь, ПолучатьНДПС = Ложь) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Или ПолучитьИзДО Тогда
		...
		Если Результат.documents.Количество() > 0 Тогда
			...
			ДополнитьНДПС(ДанныеВозврата, Объект);
	...
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		...
		Если ДанныеВозврата <> Неопределено Тогда
			...
			ДанныеВозврата.Вставить("СостояниеРазрешаетДобавлениеФайла", Истина);
			ДанныеВозврата.Вставить("СостояниеРазрешаетУдалениеФайла", Истина);
			ДанныеВозврата.Вставить("СостояниеРазрешаетРедактированиеФайла", Истина);
			ДанныеВозврата.Вставить("СостояниеРазрешаетДобавлениеСканКопии", Истина);
			ДанныеВозврата.Вставить("СостояниеРазрешаетИзменениеЭП", Истина);
			
			МодульИнтеграцияС1СДокументооборот3 = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияС1СДокументооборот3");
			Если ПолучатьНДПС И МодульИнтеграцияС1СДокументооборот3.ЭтоДокументДО3(ДанныеВозврата.type) Тогда
				
				Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси();
				Документ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(
					Прокси,
					ДанныеВозврата.type,
					ДанныеВозврата.ID,
					"enabledProperties");
				
				ДополнитьНДПС(ДанныеВозврата, Документ);
...
Функция СоздатьОбъектДОПоПравилу(ОбъектИС, Правило) Экспорт
	...
	ДополнитьНДПС(Результат, ОбъектДОИлиСообщение);
...
Процедура ПроверитьПодписи(ТаблицаПодписей, УникальныйИдентификатор,
		ВыделенныеПодписи = Неопределено, АдресСлепкаДокумента = Неопределено, Программа = Неопределено) Экспорт
...
Процедура ДополнитьНДПС(СтруктураСостояний, Объект)
	
	СтруктураСостояний.Вставить("СостояниеРазрешаетДобавлениеФайла", Истина);
	СтруктураСостояний.Вставить("СостояниеРазрешаетУдалениеФайла", Истина);
	СтруктураСостояний.Вставить("СостояниеРазрешаетРедактированиеФайла", Истина);
	СтруктураСостояний.Вставить("СостояниеРазрешаетДобавлениеСканКопии", Истина);
	СтруктураСостояний.Вставить("СостояниеРазрешаетИзменениеЭП", Истина);
	
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Объект, "enabledProperties")
			И Объект.enabledProperties.Количество() <> 0 Тогда
		
		СтруктураСостояний.СостояниеРазрешаетДобавлениеФайла = Ложь;
		СтруктураСостояний.СостояниеРазрешаетУдалениеФайла = Ложь;
		СтруктураСостояний.СостояниеРазрешаетРедактированиеФайла = Ложь;
		СтруктураСостояний.СостояниеРазрешаетДобавлениеСканКопии = Ложь;
		СтруктураСостояний.СостояниеРазрешаетИзменениеЭП = Ложь;
		
		Для Каждого ДоступноеПоле Из Объект.enabledProperties Цикл
			
			Если НРег(ДоступноеПоле) = "addfile" Тогда
				СтруктураСостояний.СостояниеРазрешаетДобавлениеФайла = Истина;
				
			ИначеЕсли НРег(ДоступноеПоле) = "deletefile" Тогда
				СтруктураСостояний.СостояниеРазрешаетУдалениеФайла = Истина;
				
			ИначеЕсли НРег(ДоступноеПоле) = "editfile" Тогда
				СтруктураСостояний.СостояниеРазрешаетРедактированиеФайла = Истина;
				
			ИначеЕсли НРег(ДоступноеПоле) = "addscannedcopy" Тогда
				СтруктураСостояний.СостояниеРазрешаетДобавлениеСканКопии = Истина;
				
			ИначеЕсли НРег(ДоступноеПоле) = "signatures" Тогда
				СтруктураСостояний.СостояниеРазрешаетИзменениеЭП = Истина;
...
Функция ПолучитьМенеджерКриптографии(Операция = "", Программа = Неопределено)
	...
	Возврат ЭлектроннаяПодпись.МенеджерКриптографии(Операция,,, Программа);
