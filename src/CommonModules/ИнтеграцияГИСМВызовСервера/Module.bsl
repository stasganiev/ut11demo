...
Функция ТекстГиперссылкиВДокументеОсновании(Ссылка, ТипНадписи) Экспорт
	...
	Если ИнтеграцияГИСМ.ПодсистемаНеИспользуется() Тогда
		Возврат "";
	ИначеЕсли ТипНадписи = "УведомлениеОбОтгрузке" Тогда
		Возврат ТекстУведомлениеОбОтгрузкеМаркированнойПродукции(Ссылка);
	ИначеЕсли ТипНадписи = "УведомлениеОСписании" Тогда
		Возврат ТекстУведомлениеОСписанииКиЗ(Ссылка);
	ИначеЕсли ТипНадписи = "Маркировка" Тогда
		Возврат ТекстМаркировкаТоваров(Ссылка);
	ИначеЕсли ТипНадписи = "ЗаявкаНаВыпускКиЗГИСМ" Тогда
		Возврат ТекстЗаявкаНаВыпускКиЗГИСМ(Ссылка);
	ИначеЕсли ТипНадписи = "ЗакупкаПоИмпорту" Тогда
		Возврат ТекстУведомлениеОбИмпорте(Ссылка);
	ИначеЕсли ТипНадписи = "ЗакупкаВЕАЭС" Тогда
		Возврат ТекстУведомлениеОВвозе(Ссылка);
	КонецЕсли;
	Возврат "";
...
Функция ТекстСтатусДокумента(Ссылка) Экспорт
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		|ГДЕ
		|	СтатусыИнформированияГИСМ.Документ = &Ссылка");
		...
		Если Выборка.Следующий() Тогда
			...
			Если ИнтеграцияГИСМ.НеотображаемыеВДокументахДальнейшиеДействия().Найти(Выборка.ДальнейшееДействие) = Неопределено Тогда
	...
	Возврат ИнтеграцияГИСМ.ПредставлениеСтатусаГИСМ(
		СтатусГИСМ,
		ДальнейшееДействие,
...
Процедура АрхивироватьДокументы(ДокументыКАрхивированию) Экспорт
	...
	ТипДокументов = ТипЗнч(ДокументыКАрхивированию[0]);
	Если ТипДокументов = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
		РегистрыСведений.СтатусыЗаявокНаВыпускКиЗГИСМ.Архивировать(ДокументыКАрхивированию);
	ИначеЕсли ТипДокументов = Тип("ДокументСсылка.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ") Тогда
		РегистрыСведений.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.Архивировать(ДокументыКАрхивированию);
	ИначеЕсли ТипДокументов = Тип("ДокументСсылка.УведомлениеОПоступленииМаркированныхТоваровГИСМ") Тогда
		РегистрыСведений.СтатусыУведомленийОПоступленииМаркированныхТоваровГИСМ.Архивировать(ДокументыКАрхивированию);
	Иначе
		РегистрыСведений.СтатусыИнформированияГИСМ.Архивировать(ДокументыКАрхивированию);
	КонецЕсли;
...
Процедура АрхивироватьРаспоряженияКОформлению(Распоряжения, Документ) Экспорт
	...
	ТипДокументов = ТипЗнч(Документ);
	Если ТипДокументов = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
		РегистрыСведений.СтатусыЗаявокНаВыпускКиЗГИСМ.АрхивироватьРаспоряженияКОформлению(Распоряжения);
	ИначеЕсли ТипДокументов = Тип("ДокументСсылка.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ") Тогда
		РегистрыСведений.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.АрхивироватьРаспоряженияКОформлению(Распоряжения);
	Иначе
		РегистрыСведений.СтатусыИнформированияГИСМ.АрхивироватьРаспоряженияКОформлению(Распоряжения);
	КонецЕсли;
...
Функция АктуальнаяВерсияСхемОбмена() Экспорт
	...
	ВерсияСервера = ВерсияСервераГИСМ();
...
Функция ВерсияСервераГИСМ() Экспорт
	...
	РезультатОтправкиЗапроса = ИнтеграцияГИСМ.ОтправитьЗапрос(
		КонвертSOAP,
		ПредопределенноеЗначение("Перечисление.ВариантыЗапросовГИСМ.ПередачаДанных"));
		
	Попытка
		
		ТекстСообщенияXML = ТекстВходящегоСообщенияXML(РезультатОтправкиЗапроса.ТекстВходящегоСообщенияXML);
...
Функция СообщениеЗапросаGLN(Организация, ИНН, КПП) Экспорт
	...
	СообщениеXML.КонвертSOAP = ПоместитьТекстСообщенияXMLВКонвертSOAP(ТекстСообщенияXML);
...
Функция ОбработатьПодписанныеСообщенияЗапросыСписковВходящихДокументовИКвитанций(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если Категории.ПолучениеСпискаВходящихДокументов.Получить(Сообщение.Операция) <> Неопределено Тогда
			...
			РезультатПередачиИОбработки = ПолучитьВходящиеДокументы(КонвертSOAP, Сообщение.GLN);
...
Функция ОбработатьПодписанныеСообщенияПолучитьВходящиеДокументы(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		РезультатПередачиИОбработки = ПолучитьДокумент(КонвертSOAP, Сообщение.ДанныеДляПолученияДокумента);
...
Функция ПередатьДанные(ИсходящееСообщение, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	ТекстИсходящегоСообщенияXML = КонвертSOAPИзПротокола(ИсходящееСообщение);
		...
		Попытка
			...
			Если ДанныеОбработки.СтатусОбработки <> Перечисления.СтатусыОбработкиСообщенийГИСМ.Обрабатывается
				...
				Если ОбновитьСостояниеПодтверждения Тогда
					...
					МенеджерОбъекта.ОбновитьСостояниеПодтверждения(ДокументОбъект, Операция, ИсходящееСообщение, ДанныеОбработки.СтатусОбработки);
		...
		Попытка
			...
			Если ДанныеОбработки.СтатусОбработки <> Перечисления.СтатусыОбработкиСообщенийГИСМ.Обрабатывается
				...
				Если ОбновитьНомерГИСМ ИЛИ ОбновитьСостояниеПодтверждения Тогда
					...
					Если ОбновитьСостояниеПодтверждения Тогда
						МенеджерОбъекта.ОбновитьСостояниеПодтверждения(
							ДокументОбъект,
							Операция,
							ИсходящееСообщение,
...
Функция ПолучитьВходящиеДокументы(ТекстИсходящегоСообщенияSOAP, GLN) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Вставить("ТекстСообщенияXML", ЗапросДокументаXML(Выборка.GLN, Выборка.output_id));
		...
		Иначе
			...
			Если ДанныеОбработки.Результат.unreceived_count > 0 Тогда
				...
				Для Каждого ЭлементДанных Из ДанныеОбработки.Результат.list.items Цикл
					...
					НоваяСтрока.Вставить("ТекстСообщенияXML", ЗапросДокументаXML(GLN, ЭлементДанных.output_id));
...
Функция РеквизитыКонтрагента(Контрагент) Экспорт
	...
	Реквизиты.Вставить("Страна", "");
	Реквизиты.Вставить("ЭтоРозничныйПокупатель", Ложь);
	Реквизиты.Вставить("ЭтоФизическоеЛицо", Ложь);
...
Функция ЗапросКвитанцииОФиксацииПоСсылкеXML(Ссылка, Операция) Экспорт
	...
	ДанныеДляПолученияКвитанцииИзОчереди = ДанныеЗапросаКвитанцииОФиксацииИзОчереди(Ссылка, Операция);
	Если ДанныеДляПолученияКвитанцииИзОчереди = Неопределено Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ДанныеЗапросаКвитанцииОФиксации = ДанныеЗапросаКвитанцииОФиксацииИзПротокола(Выборка.Сообщение);
			Если ДанныеЗапросаКвитанцииОФиксации = Неопределено Тогда
				Возврат Неопределено;
			КонецЕсли;
			ДанныеЗапросаКвитанцииОФиксации.Вставить("GLN", Выборка.GLN);
			...
			ДанныеДляПолученияКвитанцииИзОчереди.Вставить("Данные",      ДанныеЗапросаКвитанцииОФиксации);
...
Функция ДанныеЗапросаКвитанцииОФиксацииИзПротокола(Сообщение) Экспорт
	...
	ДанныеЗапросаКвитанцииОФиксации = ДанныеЗапросаКвитанцииОФиксацииИзXML(ТекстСообщенияXML);
	
	Возврат ДанныеЗапросаКвитанцииОФиксации;
...
Процедура ДобавитьДанныеЗапросаКвитанцииОФиксацииВОчередь(ТекстСообщенияXML, Реквизиты, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	ДанныеЗапросаКвитанцииОФиксации = ДанныеЗапросаКвитанцииОФиксацииИзXML(ТекстСообщенияXML);
	ДанныеЗапросаКвитанцииОФиксации.Вставить("GLN", GLN);
	...
	Если СертификатыДляПодписанияНаСервере <> Неопределено Тогда
		...
		Если СтрокаСертификата <> Неопределено Тогда
			...
			РезультатПодписания = ИнтеграцияГИСМ.Подписать(
				ЗапросКвитанцииОФиксацииXML(ДанныеЗапросаКвитанцииОФиксации),
	...
	Если Не СообщениеДобавленоВОчередь Тогда
		...
		НоваяЗапись.Данные    = Новый ХранилищеЗначения(ДанныеЗапросаКвитанцииОФиксации);
		НоваяЗапись.GLN       = GLN;
...
Функция ПодготовитьСообщениеКПередаче(ТекстСообщенияXML, Реквизиты) Экспорт
	...
	Попытка
		...
		Если ОбновитьСостояниеПодтверждения Тогда
			...
			МенеджерОбъекта.ОбновитьСостояниеПодтверждения(ДокументОбъект, Реквизиты.Операция, ИсходящееСообщение, СтатусОбработки);
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись, РежимПроведенияДокумента.Неоперативный);
			ДокументОбъект.Разблокировать();
...
#Область ГиперссылкиВФормахОснований

Функция ТекстУведомлениеОбОтгрузкеМаркированнойПродукции(Ссылка)
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ";
	Иначе
	...
	Если Выборка.Следующий() Тогда
		...
		ИмяКоманды   = "#ГИСМ#ОткрытьПротоколОбмена";
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяКоманды) Тогда
		Возврат Новый ФорматированнаяСтрока(ТекстНадписи,,ЦветаСтиля.ЦветГиперссылкиГосИС,,ИмяКоманды);
	Иначе
		Возврат Новый ФорматированнаяСтрока(ТекстНадписи);
	КонецЕсли;
	
КонецФункции

Функция ТекстУведомлениеОСписанииКиЗ(Ссылка)
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#УведомлениеОСписанииКиЗГИСМ";
	Иначе
...
Функция ТекстМаркировкаТоваров(Ссылка)
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#МаркировкаТоваровГИСМ";
	Иначе
	...
	|	ЕСТЬNULL(СтатусыИнформированияГИСМ.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыИнформированияГИСМ.Черновик)) КАК Статус
	|ИЗ
	...
	Запрос.УстановитьПараметр("ОснованиеСсылка", Ссылка);
...
Функция ТекстЗаявкаНаВыпускКиЗГИСМ(Ссылка)
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#ЗаявкаНаВыпускКиЗГИСМ";
	Иначе
	...
	|ГДЕ
	|	СтатусыЗаявокНаВыпускКиЗГИСМ.Документ = &Основание
...
Функция ТекстУведомлениеОбИмпорте(Ссылка)
	
	МетаданныеУведомление = Метаданные.Документы.УведомлениеОбИмпортеМаркированныхТоваровГИСМ;
	ПравоДобавления       = ПравоДоступа("Добавление", МетаданныеУведомление);
	ПравоЧтения           = ПравоДоступа("Чтение", МетаданныеУведомление);
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#УведомлениеОбИмпортеМаркированныхТоваровГИСМ";
	Иначе
...
Функция ТекстУведомлениеОВвозе(Ссылка)
	
	МетаданныеУведомление  = Метаданные.Документы.УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ;
	ПравоДобавления        = ПравоДоступа("Добавление", МетаданныеУведомление);
	ПравоЧтения            = ПравоДоступа("Чтение", МетаданныеУведомление);
	...
	Если ПравоДобавления Тогда
		...
		ИмяКоманды   = "#ГИСМ#УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ";
	Иначе
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки) Экспорт
	...
	ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
		ТекстОшибки,
		НСтр("ru = 'ГИСМ'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПрочитатьПервичныеДанныеВходящегоСообщенияXML(ТекстСообщенияXML) Экспорт
	...
	ОбъектXTDOДляПолученияВерсии = РаботаСXMLИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ТекстСообщенияXML);
	...
	Попытка
		...
		Иначе
			...
			Иначе
				...
				ВозвращаемоеЗначение.Результат = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.result.success);
...
Функция ОбработатьОтветНаПередачуДанных(ТекстВходящегоСообщенияSOAP) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ВозвращаемоеЗначение.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято;
			ВозвращаемоеЗначение.Результат = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.success);
	...
	Попытка
		...
		Если ОбъектXDTO.item_body = Неопределено Тогда
			...
			Иначе
				...
				ОписаниеПоддерживаемогоТипа = ИнтеграцияГИСМ.ПоддерживаемыеТипыДокументов().Получить(ДанныеДляПолученияДокумента.action_id);
				Если ОписаниеПоддерживаемогоТипа <> Неопределено Тогда
					НаименованиеДокумента = ОписаниеПоддерживаемогоТипа.Описание;
				Иначе
		...
		Иначе
			...
			Если ОбъектXDTO.item_body.sender_info <> Неопределено Тогда
				...
				Результат.ДанныеОтправителя = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.item_body.sender_info);
			...
			Для Каждого Свойство Из ОбъектXDTO.item_body.Свойства() Цикл
				...
				Результат.ДанныеДокумента = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ЗначениеСвойства);
...
//
// Параметры:
//  ТекстВходящегоСообщенияSOAP - Строка - конверт SOAP, содержащий ответ на запрос о получении списка новых документов
//                                         для загрузки.
// 
// Возвращаемое значение:
//  Структура:
...
Функция ОбработатьОтветНаЗапросПолученияДокументов(ТекстВходящегоСообщенияSOAP) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ВозвращаемоеЗначение.Результат = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.item_list);
	...
	Попытка
		...
		Иначе
			...
			Если ОбъектXDTO.success.item_list <> Неопределено Тогда
				ВозвращаемоеЗначение.Результат = РаботаСXMLИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.success.item_list);
			Иначе
...
Функция ИдентификаторОрганизацииГИСМ(Ссылка)
	...
	Если ОбщегоНазначенияИС.ИспользоватьПодразделения() Тогда
		...
		МенеджерОбъектаПодразделение = ОбщегоНазначенияИС.МенеджерОбъектаПоПолномуИмени(ПолноеИмяПодразделение);
...
Функция ПоместитьТекстСообщенияXMLВКонвертSOAP(ТекстСообщенияXML) Экспорт
	...
	ТекстСообщенияXMLБезОтступов = РаботаСXMLИС.ФорматироватьXML(ТекстСообщенияXML, Ложь);
...
Функция ОтправитьЗапросДляПередачиДанных(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	Возврат ИнтеграцияГИСМ.ОтправитьЗапросДляПередачиДанных(ТекстПодписанногоИсходящегоСообщенияXML);
...
Функция ОтправитьЗапросДляПолученияДанных(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	Возврат ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияДанных(ТекстПодписанногоИсходящегоСообщенияXML);
...
Функция ОтправитьЗапросДляПолученияРеестров(ТекстПодписанногоИсходящегоСообщенияXML) Экспорт
	...
	Возврат ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияРеестров(ТекстПодписанногоИсходящегоСообщенияXML);
