...
Функция ВариантыСопоставленияНоменклатуры(НаборНоменклатурыКонтрагентов, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСопоставления = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовыеПараметрыСопоставленияНоменклатуры();
...
#Область ПоискПоНатуральнымКлючамНоменклатурыКонтрагента

// Возвращает структуру, содержащую MD5 хеши от натуральных ключей номенклатуры. 
// Если натуральный ключ пустой, то значением соответствующего ключа структуры будет пустая строка.
// 
// Параметры:
//  Наименование - Строка - наименование номенклатуры
//  Артикул - Строка - артикул номенклатуры
...
// Возвращаемое значение:
//  Структура:
//   * ХешНаименования - Строка - хеш MD5 от наименования
//   * ХешАртикула - Строка - хеш MD5 от артикула
//   * ХешКодаНоменклатуры - Строка - хеш MD5 от кода
//   * ХешШтрихкода - Строка - хеш MD5 от штрихкода
//
Функция ХешиНатуральныхКлючейНоменклатуры(Знач Наименование, Знач Артикул, Знач КодНоменклатуры,
	Знач Штрихкод) Экспорт
	
	Ключи = Новый Структура;
	Ключи.Вставить("ХешНаименования", "");
	Ключи.Вставить("ХешАртикула", "");
	Ключи.Вставить("ХешКодаНоменклатуры", "");
	Ключи.Вставить("ХешШтрихкода", "");
	
	Если Не ПустаяСтрока(Наименование) Тогда
		Ключи.ХешНаименования = ХешПоСтроке(Наименование);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Артикул) Тогда
		Ключи.ХешАртикула = ХешПоСтроке(Артикул);
	КонецЕсли;
	
	Если Не ПустаяСтрока(КодНоменклатуры) Тогда
		Ключи.ХешКодаНоменклатуры = ХешПоСтроке(КодНоменклатуры);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Штрихкод) Тогда
		Ключи.ХешШтрихкода = ХешПоСтроке(Штрихкод);
	КонецЕсли;
...
//
// Возвращаемое значение:
//  - Неопределено - в случае незаполненного набора номенклатуры возвращается Неопределено.
...
//  ПараметрыСопоставления            - см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовыеПараметрыСопоставленияНоменклатуры
// 
// Возвращаемое значение:
...
Функция ПодобратьВариантыСопоставленияНоменклатуры(Знач ТаблицаНоменклатурыКонтрагентов, ПараметрыСопоставления)
	...
	Если ПараметрыСопоставления.ИспользоватьПоискПоНоменклатуреКонтрагента Тогда
		ПодобратьВариантыСопоставленияПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов, 
			ПараметрыСопоставления.ОграничениеТипаНоменклатуры);
	КонецЕсли;
...
Функция НормализоватьСтроковоеЗначение(Знач Параметр)
	
	Возврат ВРег(СокрЛП(Параметр));
	
КонецФункции

Функция ХешПоСтроке(Знач Параметр)
	
	Возврат ОбщегоНазначения.КонтрольнаяСуммаСтрокой(НормализоватьСтроковоеЗначение(Параметр));
	
КонецФункции

Функция ДанныеНоменклатурыДляПоискаПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов)
	
	ТипСтрокаХеша       = ОбщегоНазначения.ОписаниеТипаСтрока(32);
	ДанныеНоменклатуры = Новый ТаблицаЗначений();
	ДанныеНоменклатуры.Колонки.Добавить("Владелец", ТаблицаНоменклатурыКонтрагентов.Колонки.Владелец.ТипЗначения);
	ДанныеНоменклатуры.Колонки.Добавить("Идентификатор", 
		ТаблицаНоменклатурыКонтрагентов.Колонки.Идентификатор.ТипЗначения);
	ДанныеНоменклатуры.Колонки.Добавить("ХешНаименования", ТипСтрокаХеша);
	ДанныеНоменклатуры.Колонки.Добавить("ХешАртикула", ТипСтрокаХеша);
	ДанныеНоменклатуры.Колонки.Добавить("ХешКодаНоменклатуры", ТипСтрокаХеша);
	ДанныеНоменклатуры.Колонки.Добавить("ХешШтрихкода", ТипСтрокаХеша);
	
	Для Каждого НоменклатураКонтрагента Из ТаблицаНоменклатурыКонтрагентов Цикл
		
		Если ЗначениеЗаполнено(НоменклатураКонтрагента.НоменклатураИБ) Тогда
			Продолжить;
		КонецЕсли;
		
		НовыеДанныеНоменклатуры = ДанныеНоменклатуры.Добавить();
		
		Ключи = ХешиНатуральныхКлючейНоменклатуры(
			НоменклатураКонтрагента.Наименование,
			НоменклатураКонтрагента.Артикул,
			НоменклатураКонтрагента.КодНоменклатуры,
			НоменклатураКонтрагента.ШтрихкодКомбинации);
		
		ЗаполнитьЗначенияСвойств(НовыеДанныеНоменклатуры, НоменклатураКонтрагента);
		ЗаполнитьЗначенияСвойств(НовыеДанныеНоменклатуры, Ключи);
...
Функция ТекстЗапросаПоискаПоНоменклатуреКонтрагента()
	
	Возврат "ВЫБРАТЬ
	|	ДанныеНоменклатуры.Владелец КАК Владелец,
	|	ДанныеНоменклатуры.Идентификатор КАК Идентификатор,
	|	ДанныеНоменклатуры.ХешНаименования КАК ХешНаименования,
	|	ДанныеНоменклатуры.ХешАртикула КАК ХешАртикула,
	|	ДанныеНоменклатуры.ХешКодаНоменклатуры КАК ХешКодаНоменклатуры,
	|	ДанныеНоменклатуры.ХешШтрихкода КАК ХешШтрихкода
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПодЗапрос.Владелец КАК Владелец,
	|	ПодЗапрос.Идентификатор КАК Идентификатор,
	|	ПодЗапрос.Номенклатура КАК Номенклатура,
	|	СУММА(ПодЗапрос.Вес) КАК Вес
	|ПОМЕСТИТЬ втВарианты
	|ИЗ
	|	(ВЫБРАТЬ
	|		втДанныеНоменклатуры.Владелец КАК Владелец,
	|		втДанныеНоменклатуры.Идентификатор КАК Идентификатор,
	|		НоменклатураКонтрагентовБЭД.Номенклатура КАК Номенклатура,
	|		1 КАК Вес
	|	ИЗ
	|		втДанныеНоменклатуры КАК втДанныеНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураКонтрагентовБЭД КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.Владелец
	|			И втДанныеНоменклатуры.ХешНаименования = НоменклатураКонтрагентовБЭД.ХешНаименования
	|			И &УсловиеПроверкиПустойНоменклатуры
	|	ГДЕ
	|		втДанныеНоменклатуры.ХешНаименования <> """"
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		втДанныеНоменклатуры.Владелец,
	|		втДанныеНоменклатуры.Идентификатор,
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураКонтрагентовБЭД КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.Владелец
	|			И втДанныеНоменклатуры.ХешАртикула = НоменклатураКонтрагентовБЭД.ХешАртикула
	|			И &УсловиеПроверкиПустойНоменклатуры
	|	ГДЕ
	|		втДанныеНоменклатуры.ХешАртикула <> """"
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураКонтрагентовБЭД КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.Владелец
	|			И втДанныеНоменклатуры.ХешКодаНоменклатуры = НоменклатураКонтрагентовБЭД.ХешКодаНоменклатуры
	|			И &УсловиеПроверкиПустойНоменклатуры
	|	ГДЕ
	|		втДанныеНоменклатуры.ХешКодаНоменклатуры <> """"
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураКонтрагентовБЭД КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.Владелец
	|			И втДанныеНоменклатуры.ХешШтрихкода = НоменклатураКонтрагентовБЭД.ХешШтрихкода
	|			И &УсловиеПроверкиПустойНоменклатуры
	|	ГДЕ
	|		втДанныеНоменклатуры.ХешШтрихкода <> """") КАК ПодЗапрос
	|СГРУППИРОВАТЬ ПО
	|	ПодЗапрос.Владелец,
	|	ПодЗапрос.Идентификатор,
	|	ПодЗапрос.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втВарианты.Владелец КАК Владелец,
	|	втВарианты.Идентификатор КАК Идентификатор,
	|	втВарианты.Номенклатура КАК Номенклатура
	|ИЗ
	|	втВарианты КАК втВарианты
	|ГДЕ
	|	втВарианты.Номенклатура В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			Сортировка.Номенклатура
	|		ИЗ
	|			втВарианты КАК Сортировка
	|		ГДЕ
	|			Сортировка.Владелец = втВарианты.Владелец
	|			И Сортировка.Идентификатор = втВарианты.Идентификатор
	|
	|		УПОРЯДОЧИТЬ ПО
	|			Сортировка.Вес УБЫВ)";
...
// Выполняет поиск по ранее сопоставленной номенклатуре контрагента (регистру НоменклатураКонтрагентовБЭД)
// 
// Параметры:
...
// 
Процедура ПодобратьВариантыСопоставленияПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов,
	ОграничениеТипаНоменклатуры)
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапросаПоискаПоНоменклатуреКонтрагента(),
		"&УсловиеПроверкиПустойНоменклатуры",
		ТекстУсловияПроверкиПустойНоменклатуры(ОграничениеТипаНоменклатуры));
	
	ДанныеНоменклатуры = ДанныеНоменклатурыДляПоискаПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ДанныеНоменклатуры", ДанныеНоменклатуры);
	
	НайденныеВарианты = Запрос.Выполнить().Выгрузить();
	
	ДобавитьИндекс = ТаблицаНоменклатурыКонтрагентов.Количество() > 1000;
	Если ДобавитьИндекс Тогда
		ТаблицаНоменклатурыКонтрагентов.Индексы.Добавить("Владелец, Идентификатор");
	КонецЕсли;
	
	ШаблонИсключения = НСтр("ru = 'Номенклатура контрагента %1 с идентификатором %2 передана в поиск более 1 раза'");
	Для Каждого Вариант Из НайденныеВарианты Цикл
		
		Если Не ЗначениеЗаполнено(Вариант.Номенклатура) Тогда
			Продолжить;
		КонецЕсли;
		
		Отбор               = Новый Структура("Владелец, Идентификатор", Вариант.Владелец, Вариант.Идентификатор);
		СтрокиДляЗаполнения = ТаблицаНоменклатурыКонтрагентов.НайтиСтроки(Отбор);
		
		ОбщегоНазначенияКлиентСервер.Проверить(СтрокиДляЗаполнения.Количество() = 1, 
			СтрШаблон(ШаблонИсключения, Вариант.Владелец, Вариант.Идентификатор));
		
		СтрокиДляЗаполнения[0].НоменклатураИБ = Вариант.Номенклатура;
		
	КонецЦикла;
	
КонецПроцедуры

Функция ТекстУсловияПроверкиПустойНоменклатуры(ОграничениеТипаНоменклатуры)
	
	УсловияПроверки  = Новый Массив;
	ШаблонУсловия    = "НоменклатураКонтрагентовБЭД.Номенклатура <> %1";
	НаборТипов       = Метаданные.ОпределяемыеТипы.НоменклатураБЭД.Тип.Типы();
	Разделитель      = Символы.ПС + "И ";
	ШаблонИсключения = НСтр("ru = 'Ошибка встраивания БЭД: определяемый тип НоменклатураБЭД содержит тип %1'");
	
	Для Каждого ТипСсылки Из НаборТипов Цикл
		
		МетаданныеСсылки = Метаданные.НайтиПоТипу(ТипСсылки);
		ОбщегоНазначенияКлиентСервер.Проверить(МетаданныеСсылки <> Неопределено, 
			СтрШаблон(ШаблонИсключения, Строка(ТипСсылки)));
		
		Если ТипЗнч(ОграничениеТипаНоменклатуры) = Тип("ОписаниеТипов") 
			И Не ОграничениеТипаНоменклатуры.СодержитТип(ТипСсылки) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаПустаяСсылка = СтрШаблон("ЗНАЧЕНИЕ(%1.ПустаяСсылка)", МетаданныеСсылки.ПолноеИмя());
		УсловияПроверки.Добавить(СтрШаблон(ШаблонУсловия, СтрокаПустаяСсылка));
		
	КонецЦикла;
	
	Если НаборТипов.Количество() > 1 И ТипЗнч(ОграничениеТипаНоменклатуры) = Тип("ОписаниеТипов") Тогда
		УсловияПроверки.Добавить(СтрШаблон("(%1)", ТекстУсловияПроверкиТипаНоменклатуры(ОграничениеТипаНоменклатуры)));
	ИначеЕсли УсловияПроверки.Количество() > 1 Тогда
		УсловияПроверки.Добавить(СтрШаблон(ШаблонУсловия, "НЕОПРЕДЕЛЕНО"));
	КонецЕсли;
	
	Возврат СтрСоединить(УсловияПроверки, Разделитель);
	
КонецФункции

Функция ТекстУсловияПроверкиТипаНоменклатуры(ОграничениеТипаНоменклатуры)
	
	УсловияПроверки = Новый Массив;
	ШаблонУсловия   = "ТИПЗНАЧЕНИЯ(НоменклатураКонтрагентовБЭД.Номенклатура) = ТИП(%1)";
	Разделитель     = Символы.ПС + "ИЛИ ";
	
	Для Каждого ТипСсылки Из ОграничениеТипаНоменклатуры.Типы() Цикл
		...
		УсловияПроверки.Добавить(СтрШаблон(ШаблонУсловия, МетаданныеСсылки.ПолноеИмя()));
		
	КонецЦикла;
	
	Если УсловияПроверки.Количество() = 0 Тогда
		УсловияПроверки.Добавить("ИСТИНА");
	КонецЕсли;
...
//
Функция Хеш_MD5_64(Знач Данные)
