...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИзображенияСлужебный.ПрайсЛист КАК ПрайсЛист,
	|	ИзображенияСлужебный.Номенклатура КАК Номенклатура,
	|	ИзображенияСлужебный.Характеристика КАК Характеристика,
	|	МИНИМУМ(ИзображенияСлужебный.ФайлИзображения) КАК ФайлИзображения
	|ПОМЕСТИТЬ ИзображенияСлужебныйСГруппировкой
	|ИЗ
	|	ИзображенияСлужебный
	|СГРУППИРОВАТЬ ПО
	|	ИзображенияСлужебный.ПрайсЛист,
	|	ИзображенияСлужебный.Номенклатура,
	|	ИзображенияСлужебный.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ИзображенияСлужебныйСГруппировкой.ФайлИзображения КАК ФайлИзображения
	|ПОМЕСТИТЬ ТоварыСлужебныйСИзображениями
	|ИЗ
	|	ТоварыСлужебный КАК ТоварыСлужебный
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИзображенияСлужебныйСГруппировкой КАК ИзображенияСлужебныйСГруппировкой
	|		ПО ТоварыСлужебный.ПрайсЛист = ИзображенияСлужебныйСГруппировкой.ПрайсЛист
	|		И ТоварыСлужебный.Номенклатура = ИзображенияСлужебныйСГруппировкой.Номенклатура
	|		И ТоварыСлужебный.Характеристика = ИзображенияСлужебныйСГруппировкой.Характеристика";
	...
	Для каждого Ошибка Из ТаблицаОшибок Цикл
		...
		Если ЗначениеЗаполнено(Ошибка.ДополнительныйРеквизит) Тогда
			ПолноеИмяРеквизита = СтрШаблон(
				...
				Строка(Ошибка.ДополнительныйРеквизит.УникальныйИдентификатор()));
		Иначе
...
Процедура ОпределитьВладельцевНезаполненныхРеквизитов(
	...
	ИначеЕсли Не СтрНайти(ПроверяемыйРеквизит, "|") = 0 Тогда
		...
		ТорговыеПредложенияПереопределяемый.ОпределитьВладельцевНезаполненныхРеквизитов(ЧастиРеквизита[0], 
			СтрокаТорговогоПредложения, ЧастиРеквизита[1], СсылкаНаОбъект, ПутьКДанным, ТекстСообщения);
