...
Функция ПреобразоватьКодКладрДляЕИС(Знач КодКладрБСП) Экспорт
	...
	ДлинаКодаСКодомУлицы = 15;
	ДлинаКодаРДИК_ИК_1030 = 13;
	Если СтрДлина(КодКладрБСП) > ДлинаКодаСКодомУлицы И Прав(КодКладрБСП, 2) = "00" Тогда
		КодКЛАДР = Лев(КодКладрБСП, СтрДлина(КодКладрБСП) - 2);
	ИначеЕсли СтрДлина(КодКладрБСП) = ДлинаКодаРДИК_ИК_1030 Тогда
		// Требование контроля РДИК_ИК_1030.
		КодКЛАДР = Лев(КодКладрБСП, СтрДлина(КодКладрБСП) - 2);
	Иначе
...
Функция ДанныеГосконтрактаПоВладельцу(ВладелецКонтракта) Экспорт
	...
	|	ВЫБОР
	|		КОГДА СпрНоменклатура.ТипНоменклатуры =	ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга) ТОГДА
	|			ИСТИНА
	|		ИНАЧЕ
	|		 	ЛОЖЬ
	|	КОНЕЦ КАК Услуга, 	
...
Функция ДанныеГосударственногоКонтракта(СсылкаНаКонтракт, ВозвратитьКонтрактВВидеСтроки=Ложь) Экспорт
	...
	Иначе
		ДанныеКонтракта = РазобратьФайлКонтракта(ДвоичныеДанныеКонтракта, , Истина);
	КонецЕсли;
...
Процедура ПриВыполненииДействияЗагрузить(ТранспортныеКонтейнеры, ТребуетсяЗагрузкаИзЕИС) Экспорт
	
	ТребуетсяЗагрузкаИзЕИС = ЕстьВозможностьОтправкиПолученияЭД() И НЕ ЗначениеЗаполнено(ТранспортныеКонтейнеры);
...
Процедура ПроверитьОснованиеОтгрузкиТоваров(ДеревоДанных) Экспорт
	...
	Реквизит = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
		ДеревоДанных, "ОснованиеОтгрузкиТоваров");
	Если НЕ (ТипЗнч(Реквизит) = Тип("Число") И Реквизит > 0) Тогда
		Возврат;
	КонецЕсли;
	...
	// ДокументНаименование, атрибут НаимОсн.
	Основание = НСтр("ru = 'Контракт'");
	// ДокументНомер, атрибут НомОсн.
	ОснованиеНомер = ДанныеКонтракта.Номер;
	// ДокументДата, атрибут ДатаОсн
	ОснованиеДата = ДанныеКонтракта.ДатаЗаключенияКонтракта;
	// ДокументДопСведения, атрибут ДопСвОсн.
	ДополнительныеСведения = ДополнительныеСведенияОснованияПоДаннымКонтракта(ДанныеКонтракта);
	
	ОснованиеОтгрузкиТоваров = ПодготовитьТаблицуДанныхОснованийУПД(
		Основание,
		ОснованиеНомер,
		ОснованиеДата,
...
Функция ПередФормированиемЭлектронногоДокумента(ДеревоДанных, ПространствоИмен) Экспорт
	...
	Если СтрНайти(ПространствоИмен, ПространствоИменУПД) > 0 Тогда
		...
		ПроверитьОснованиеОтгрузкиТоваров(ДеревоДанных);
	КонецЕсли;
...
Процедура ПриФормированииДвоичныхДанныхЭД(ВидЭД, ОбъектФайла, ДеревоДанных) Экспорт
	...
	УбратьКоличествоПриТекстовомВыражениОбъема(ВидЭД, ОбъектФайла, ДеревоДанных);
...
// Определяет является ли тип документа ЭДО типом документа электронного актирования.
// 
// Параметры:
//  ТипДокумента - Перечисления.ТипыДокументовЭДО
// 
// Возвращаемое значение:
//  Булево -- Истина, если это тип документа электронного актирования.
Функция ЭтоТипДокументаАктирования(ТипДокумента) Экспорт
...
Функция ЕстьВозможностьОтправкиПолученияЭД() Экспорт
	...
	ТекстЗапроса = ТекстЗапросаЕстьВозможностьОтправкиПолученияЭД();
...
// При формировании идентификаторов учетных записей
// для отправки получения документов ЭДО
// 
// Параметры:
//  ИдентификаторыЭДО - Массив из Строка
Процедура ПриФормированииИдентификаторовУчетныхЗаписейДляОтправкиПолученияДокументовЭДО(ИдентификаторыЭДО) Экспорт
	...
	Для Каждого Строка Из НастройкиПоставщиков Цикл
		Если НЕ ПустаяСтрока(Строка.ИдентификаторУчастникаДокументооборота)
			И НЕ ПустаяСтрока(Строка.НаименованиеОрганизации)
			И СтрДлина(Строка.ТокенАвторизации) = ДлинаИдентификатора Тогда
			ИдентификаторыЭДО.Добавить(Строка.ИдентификаторУчастникаДокументооборота);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьОписаниеОбъектаУчета(ИсточникДанныхСсылка, Описание) Экспорт
	...
	Если ТипЗнч(ИсточникДанныхСсылка) = ТипСправочникаГосударственныхКонтрактов() Тогда
		...
		СтрокаОписания.Договор = ЭлектронноеАктированиеЕИСУТ.ПолучитьДоговорГосконтракта(ИсточникДанныхСсылка);
...
Функция ТекстЗапросаНастроекОтправкиОбъектовУчета() Экспорт
	...
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ВерсияФормата, НастройкиОтправкиБезДоговора.ВерсияФормата)
	|		ИНАЧЕ НастройкиОтправкиЕИС.Формат
	|	КОНЕЦ КАК Формат,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.МаршрутПодписания, НастройкиОтправкиБезДоговора.МаршрутПодписания)
	|		ИНАЧЕ НастройкиОтправкиЕИС.МаршрутПодписания
	|	КОНЕЦ КАК МаршрутПодписания,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.СпособОбменаЭД, НастройкиОтправкиБезДоговора.СпособОбменаЭД)
	|		ИНАЧЕ НастройкиОтправкиЕИС.СпособОбмена
	|	КОНЕЦ КАК СпособОбмена,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ИдентификаторОтправителя, НастройкиОтправкиБезДоговора.ИдентификаторОтправителя)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ИдентификаторОтправителя
	|	КОНЕЦ КАК ИдентификаторОтправителя,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ИдентификаторПолучателя, НастройкиОтправкиБезДоговора.ИдентификаторПолучателя)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ИдентификаторПолучателя
	|	КОНЕЦ КАК ИдентификаторПолучателя,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ТребуетсяОтветнаяПодпись, НастройкиОтправкиБезДоговора.ТребуетсяОтветнаяПодпись)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ТребуетсяОтветнаяПодпись
	|	КОНЕЦ КАК ТребуетсяОтветнаяПодпись,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ТребуетсяИзвещениеОПолучении, НастройкиОтправкиБезДоговора.ТребуетсяИзвещениеОПолучении)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ТребуетсяИзвещениеОПолучении
	|	КОНЕЦ КАК ТребуетсяИзвещениеОПолучении,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ВыгружатьДополнительныеСведения, НастройкиОтправкиБезДоговора.ВыгружатьДополнительныеСведения)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ВыгружатьДополнительныеСведения
	|	КОНЕЦ КАК ВыгружатьДополнительныеСведения,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ОбменБезПодписи, НастройкиОтправкиБезДоговора.ОбменБезПодписи)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ОбменБезПодписи
	|	КОНЕЦ КАК ОбменБезПодписи,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.Формировать, НастройкиОтправкиБезДоговора.Формировать)
	|		ИНАЧЕ НастройкиОтправкиЕИС.Формировать
	|	КОНЕЦ КАК Формировать,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ЗаполнениеКодаТовара, НастройкиОтправкиБезДоговора.ЗаполнениеКодаТовара)
...
Функция СформироватьТранспортныйПакетДляЕИС(ТранспортныйКонтейнер, ДанныеОбъекта) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТипДокумента) Тогда
		...
		ВызватьИсключение(ОписаниеОшибки);
	КонецЕсли;
	Контракт = ГосударственныйКонтрактДоговора(ДанныеОбъекта.ДоговорКонтрагента);
	Если Контракт.ЭтоЗакрытыйКонтракт Тогда
		// Требование РДИК_ИК_0055. Для контракта, к которому принимается документ, установлен признак сокрытия,
		// поэтому значение в атрибуте "Тип приложенного документа" (ФайлПакет/@ТипПрилож) транспортного
		// пакета должно содержать постфикс "Закрыт".
		ТипДокумента = ТипДокумента + "Закрыт";
	КонецЕсли;
...
// Возвращает сформированные данные файла если для их формирования используется особая схема ЕИС  
// 
// Параметры:
//  ДвоичныеДанныеФайла - ДвоичныеДанные
//  ПотокДанныхФайла - Поток
//  ПространствоИменФайла - Строка - Пространство имен взятое из файла
//  URIПространстваИмен - Строка - Пространство имен, которое было определено для чтения файла
//  ИмяФайла - Строка
// 
// Возвращаемое значение:
//  Неопределено, ОбъектXDTO - Если данные файла по схеме ЕИС не формируются тогда Неопределено иначе ОбъектXDTO
Функция ПриФормированияДанныхФайлаЭД(
	ДвоичныеДанныеФайла,
	ПотокДанныхФайла,
	...
	ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС = ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС(ИмяФайла);
	Если Не ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПространствоИменЕИС = ПространствоИменЕИСПоИмениФайла(ИмяФайла);
	Если ПространствоИменЕИС = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПространствоИменФайла) Тогда
		ДокументDOM.ЭлементДокумента.УстановитьСоответствиеПространстваИмен("", ПространствоИменЕИС);
	Иначе
		Атрибут = ДокументDOM.ЭлементДокумента.Атрибуты.ПолучитьИменованныйЭлемент(
			"http://www.w3.org/2000/xmlns/",
			"xmlns");
		Атрибут.Значение = ПространствоИменЕИС;
	КонецЕсли;
	...
	// Заменяются двоичные данные и поток в методе вызвавшем данную функцию
	ДвоичныеДанныеФайла = ПотокЗаписи.ЗакрытьИПолучитьДвоичныеДанные();
	ПотокДанныхФайла = ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения();
	
	// Заменяются значения пространств имен на пространство имен пакета ЕИС в методе вызвавшем данную функцию
	ПространствоИменФайла = ПространствоИменЕИС;
	URIПространстваИмен = ПространствоИменЕИС;
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'ЭлектронноеАктированиеЕИС.ПриФормированияДанныхФайлаЭД'",
			КодОсновногоЯзыка);
...
// При определении доступных действий по ЭДО.
// 
// Параметры:
//  Действия - Соответствие - Набор действий по состоянию ЭДО.
...
Процедура ПриОпределенииДоступныхДействийПоЭДО(Действия, СостояниеДокумента, СвойстваДокумента) Экспорт
	
	Если ЭтоОбменЧерезСерверЕИС(СвойстваДокумента.СпособОбмена) Тогда
		// Аннулирование документа производится в личном кабинете заказчика.
...
Функция СведенияОПоставщикеИЕгоПодписантах(Организация, Настройки = Неопределено) Экспорт
	...
	Исключение
		Шаблон = НСтр("ru = 'Ошибка при разборе ответа сервера ЕИС с данными подписантов:'")
			+ Символы.ПС + "%1" + Символы.ПС + НСтр("ru='Подробности см. в журнале регистрации.'");
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		КраткоеПредставление = ОбработкаОшибок().КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ПолноеПредставление = ОбработкаОшибок().ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		Результат.ОписаниеОшибки = СтрШаблон(Шаблон, КраткоеПредставление);
		Шаблон = Шаблон + Символы.ПС + НСтр("ru = 'Текст ответа:'") + Символы.ПС + "%2";
		ОписаниеОшибкиДляЖурнала = СтрШаблон(Шаблон, ПолноеПредставление, Ответ);
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибкиДляЖурнала, , , УровеньЖурналаРегистрации.Ошибка);
		Возврат Результат;
	КонецПопытки;
...
Функция РезультатОбработкиТранспортногоПакета(Организация,
	...
	Если НЕ Результат.ДанныеОбрабатываются Тогда
		...
		Если РезультатРазбора.ОшибкаВЗапросе Тогда
			...
			// Запись в ЖР ошибки и текстов запроса и ответов.
			ВариантСобытия = НСтр("ru = 'Ошибка в запросе к сервису ЕИС или ошибка сервера ЕИС'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			Шаблон = НСтр("ru = 'Ошибка при получении статуса пакета %1 организации %2:'");
			ОписаниеОшибки = СтрШаблон(Шаблон, ИдентификаторПакета, Организация) + Символы.ПС
				+ Результат.ОписаниеОшибки + Символы.ПС
				+ НСтр("ru = 'Запрос к серверу ЕИС:'") + Символы.ПС
				+ ТекстЗапроса + Символы.ПС
				+ НСтр("ru = 'Ответ сервера ЕИС:'") + Символы.ПС
				+ Ответ;
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ВариантСобытия, , УровеньЖурналаРегистрации.Ошибка);
...
Функция ПоддерживаемыеВерсииСхем() Экспорт
	...
	// Версией по умолчанию считается версия, добавленная последней.
	Версии = Новый Массив;
	Если МосковскоеВремя < Дата(2023, 7, 1, 0, 0, 0) Тогда
		Версии.Добавить("13.2");
...
//
// Параметры:
//  ДвоичныеДанныеИлиАдрес - ДвоичныеДанные - адрес ДвоичныхДанных во временном хранилище или непосредственно ДвоичныеДанные.
//
// Возвращаемое значение:
//  Строка, КодировкаТекста - "windows-1251"/КодировкаТекста.UTF8/КодировкаТекста.UTF16/"cp866"
//
Функция ОпределитьКодировку(Знач ДвоичныеДанныеИлиАдрес) Экспорт
	
	КодировкаПоУмолчанию = "windows-1251";
	Если ТипЗнч(ДвоичныеДанныеИлиАдрес) = Тип("Строка") Тогда 
		Если ЭтоАдресВременногоХранилища(ДвоичныеДанныеИлиАдрес) Тогда 
			Данные = ПолучитьИзВременногоХранилища(ДвоичныеДанныеИлиАдрес);
		Иначе
			Возврат КодировкаПоУмолчанию;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДвоичныеДанныеИлиАдрес) = Тип("ДвоичныеДанные") Тогда 
		Данные = ДвоичныеДанныеИлиАдрес;
	Иначе
		Возврат КодировкаПоУмолчанию;
	КонецЕсли;
	
	Стр = Лев(ПолучитьHexСтрокуИзДвоичныхДанных(Данные), 512);
	...
	СтрокаДекларации = "3C3F786D6C";
	СтрокаКода = "656E636F64696E67"; // encoding word
	Строка1251 = "31323531";
	ДлинаВыборкиXML = 150;
	Если КодировкаПоBOM = КодировкаТекста.UTF16 Тогда
		СтрМакс = СтрДлина(СтрокаДекларации) / 2;
		Для ш = 1 по СтрМакс Цикл 
			СтрокаДекларации = Лев(СтрокаДекларации, ш * 2 + (ш-1) * 2) + "00" + Прав(СтрокаДекларации, СтрМакс * 2 - ш * 2);
		КонецЦикла;
		СтрМакс = СтрДлина(СтрокаКода) / 2;
		Для ш = 1 по СтрМакс Цикл 
			СтрокаКода = Лев(СтрокаКода, ш * 2 + (ш-1) * 2) + "00" + Прав(СтрокаКода, СтрМакс * 2 - ш * 2);
		КонецЦикла;
		СтрМакс = СтрДлина(Строка1251) / 2;
		Для ш = 1 по СтрМакс Цикл 
			Строка1251 = Лев(Строка1251, ш * 2 + (ш-1) * 2) + "00" + Прав(Строка1251, СтрМакс * 2 - ш * 2);
		КонецЦикла;
		ДлинаВыборкиXML = 200;
	КонецЕсли;
	...
	Если ИндексXML > 0 и ИндексXML < 15 Тогда //xml declaration должно быть в самом начале файла
		...
		Иначе
			...
			Если Открывающая <> 0 И Закрывающая <> 0 Тогда 
				ТекстКодировки = Сред(XMLpfx, Открывающая + 2, Закрывающая - Открывающая - 2); //Без обрамляющих кавычек
				Если СтрНайти(ТекстКодировки, Строка1251) > 0 Тогда //1251
					КодировкаПоXML = КодировкаПоУмолчанию;
	...
	Пока СтрДлина(Стр) > 2 Цикл
		...
		Иначе
			...
			Иначе
				...
				КонецЕсли;
				Если (Дес<32 И Дес <> 10 И Дес <> 13) Тогда // Control chars
					Служебных = Служебных + 1;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	ПроцентСлужебных = ((Служебных / Всего) * 100);	//Процент служебных
	
	Процент00 = (((СтаршихUtf16) / Всего) * 100);	//Процент старших нулевых байтов Utf16
	
	Если (ПроцентСлужебных > 15 И 
		ПроцентСлужебных > Процент8 И 
		ПроцентСлужебных > Процент16 И 
		ПроцентСлужебных > ПроцентAnsi И 
		ПроцентСлужебных > ПроцентOem) Тогда
		
		Возврат КодировкаПоУмолчанию; // Двоичные данные
	КонецЕсли;
	...
	Если (Процент8 > ПроцентAnsi И Процент8 > ПроцентOem)
		Или (Процент8 >= ПроцентAnsi И Процент8 >= ПроцентOem
		И (КодировкаПоBOM = КодировкаТекста.UTF8 Или КодировкаПоXML = КодировкаТекста.UTF8)) Тогда
	...
	Если (КодировкаПоЧастоте = Неопределено И ПроцентAnsi > Процент8 И ПроцентAnsi > ПроцентOem)
		Или (ПроцентAnsi >= Процент8 И ПроцентAnsi >= ПроцентOem И (КодировкаПоXML = КодировкаПоУмолчанию)) Тогда
		КодировкаПоЧастоте = КодировкаПоУмолчанию;
	КонецЕсли;
...
// Гиперссылка на личный кабинет поставщика.
// 
// Возвращаемое значение:
//  Строка - Гиперссылка на личный кабинет поставщика
...
// 
// Возвращаемое значение:
//  Массив, ТаблицаЗначений, СписокXDTO
...
Функция ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеДокумента, ПроверятьКодировку = Ложь) Экспорт
	
	Кодировка = Неопределено;
	Если ПроверятьКодировку Тогда 
		Кодировка = ОпределитьКодировку(ДвоичныеДанныеДокумента);
	КонецЕсли;
	
	Чтение = Новый ЧтениеXML;
	Если ПроверятьКодировку Тогда
		ТекстПакета = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанныеДокумента,
			 Кодировка);
	...
	КонецЕсли;
	Документ = ФабрикаXDTO.ПрочитатьXML(Чтение);
	Чтение.Закрыть();
...
Функция РазобратьФайлКонтракта(ПутьККонтрактуИлиДвоичныеДанныеИлиОбъектXDTO,
	...
	Если ТипЗнч(ДанныеКонтракта) = Тип("СписокXDTO") Тогда
		ОписаниеОшибки = НСтр("ru = 'В файле обнаружено несколько контрактов, будет загружен только первый контракт.'");
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
		ДанныеКонтракта = ДанныеКонтракта[0];
	КонецЕсли;
	...
	ИначеЕсли ЕстьСвойство(ОснованиеЗаключения, ИмяУзла44ФЗ) Тогда
		...
		ИначеЕсли ЕстьСвойство(Узел44ФЗ, ИмяУзлаЗакупкаУОдногоПоставщика) Тогда
			...
			Результат.ЗакупкаУЕдинственногоПоставщика = Истина;
	...
	// Если не заполнен ИдентификаторУчастникаДокументооборотаЗаказчика, он будет определен из данных контракта
	Если Не ЗначениеЗаполнено(Результат.ИдентификаторУчастникаДокументооборотаЗаказчика) Тогда
			
		КодПоСПЗ = Результат.СведенияОЗаказчике.КодыОрганизации.КодПоСПЗ;
		ИдентификаторЗаказчика = ПолучитьИдентификаторЗаказчика(КодПоСПЗ);
		Результат.ИдентификаторУчастникаДокументооборотаЗаказчика = ИдентификаторЗаказчика;
...
Процедура ЗаполнитьОграничениеТипаНоменклатуры(ЭлементФормы, ТипНоменклатура, ТипОбъектаЗакупки) Экспорт
	...
	ЭлектронноеАктированиеЕИСПереопределяемый.ДополнитьОграничениеТипаНоменклатурыКонтракта(
		Типы, ТипОбъектаЗакупки);
	
	ЭлементФормы.ВыбиратьТип = Типы.Количество() > 1;
...
Функция УстановитьСоединениеССерверомИнтернета(URLСервера, ОписаниеОшибки = "", Таймаут = 60)
	...
	Попытка
		Соединение = Новый HTTPСоединение(
			...
			Прокси,
			Таймаут,
			?(НРег(Схема) = "http", Неопределено, ЗащищенноеСоединение));
	Исключение
...
Функция ПроверитьПараметрыНаСервереИнтернетПоддержки(Логин = "", Пароль = "")
	...
	Пароль = ЭкранироватьСимволыJSON(Пароль);
...
Процедура ЗаполнитьДанныеПриложенияПродавца(ДанныеПриложения, ДвоичныеДанныеПриложения, ТипПриложения)
	...
	ДанныеФайлаПриложения = ОбъектXDTOИзДвоичныхДанных(ДвоичныеДанныеПриложения);
	...
	Для Каждого СтрокаСведенийОТоварах Из СведенияОТоварах Цикл
		...
		ДанныеТРУ = ТаблицаНоменклатуры.Добавить();
		...
		Если ЕстьСвойство(СтрокаСведенийОТоварах, "НедеталТРУ") ИЛИ ЭтоКорректировка Тогда
			...
			Если ЭтоКорректировка Тогда
				...
				ДанныеТРУ.КодТовараДляЕИС = ДанныеТовараБезДетализации.КодТов;
			Иначе
				ДанныеТовараБезДетализации = СтрокаСведенийОТоварах.НедеталТРУ;
			КонецЕсли;
			
			ЗаполнитьИдентификаторыНоменклатуры(ДанныеТРУ, ДанныеТовараБезДетализации);
			ДанныеТРУ.Наименование = ДанныеТовараБезДетализации.НаимТовИсх;
			
			Если ЕстьСвойство(ДанныеТовараБезДетализации, "НеЛПСвед") Тогда
				
				НеЛПСвед = ДанныеТовараБезДетализации.НеЛПСвед;
				
				Для каждого СтрокаТовара Из ЗначениеВКоллекцию(НеЛПСвед) Цикл
					
					НоваяСтрокаТовара = ТаблицаНоменклатуры.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаТовара, ДанныеТРУ);
					
					НоваяСтрокаТовара.НомерСтроки = СтрокаТовара.НомСтр;
					
					Если НЕ ЭтоКорректировка Тогда
						
						НоваяСтрокаТовара.ОбъемРаботыУслуги = ЗначениеНеобязательногоСвойства(
							СтрокаТовара, "ОбРабУсл");
						
					Иначе
						
						Если ЕстьСвойство(СтрокаТовара, "ОбРабУсл") Тогда
							НоваяСтрокаТовара.ОбъемРаботыУслуги = СтрокаТовара.ОбРабУсл.ДоИзм;
							НоваяСтрокаТовара.ОбъемРаботыУслугиПослеИзменения = СтрокаТовара.ОбРабУсл.ПослеИзм;
						КонецЕсли;
				...
				ТаблицаНоменклатуры.Удалить(ДанныеТРУ);
		...
		Иначе
			...
			Если ЭтоКорректировка Тогда
				...
				ЗаполнитьИдентификаторыНоменклатуры(ДанныеТРУ, ДанныеТовараСДетализацией);
				ДанныеТРУ.КодТовараДляЕИС = ДанныеТовараСДетализацией.КодТов;
				ДанныеТРУ.НомерСтроки = ДанныеТовараСДетализацией.НомСтр;
				ДанныеТРУ.Наименование = ДанныеТовараСДетализацией.НаимТовИсх;
			Иначе
				...
				Для Каждого СведенияДетализации Из ЗначениеВКоллекцию(ДанныеТовараСДетализацией.СведДетал) Цикл
					ДанныеТРУ = ТаблицаНоменклатуры.Добавить();
					Если НЕ ЭтоКорректировка Тогда
						ДанныеТРУ.НомерСтроки = СведенияДетализации.НомСтр;
						ДанныеТРУ.ОбъемРаботыУслуги = ЗначениеНеобязательногоСвойства(
							СведенияДетализации, "ОбРабУсл");
						ЗаполнитьИдентификаторыНоменклатуры(ДанныеТРУ, СведенияРодителя);
						ДанныеТРУ.Наименование = СведенияРодителя.НаимТов;
						ДанныеТРУ.КодТовараДляЕИС = СведенияРодителя.КодТов;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ТекстЗапросаЕстьВозможностьОтправкиПолученияЭД()
...
Функция НовыеДанныеКонтракта()
	...
	// Флаг устанавливается в Истина, если в файле контракта присутствует узел singleCustomer.
	Результат.Вставить("ЗакупкаУЕдинственногоПоставщика", Ложь);
...
Функция ОпределитьСтатусДокумента(СтроковоеПредставлениеСтатуса)
	...
	Статусы.Вставить("RETURN", Перечисления.СтатусыДокументовЕИС.Возврат);
...
Функция РазобратьДанныеОбъекта(Данные, ЭтоЗаказчик = Ложь)
	...
	Если ЕстьСвойство(Данные.Body[ИмяУзлаОбъекта], "ФайлПакет") Тогда
		...
		Содержимое = ПерекодироватьДокументыПакета(Содержимое);
...
Функция РазобратьСписокОбъектов(Данные)
	...
	Для каждого ДанныеОбъекта Из ДанныеОбъектов Цикл
		...
		Объект.ВидДокумента = ДанныеОбъекта.documentKind;
		Объект.ДатаДокумента = XMLЗначение(Тип("Дата"), ДанныеОбъекта.documentDate);
		// Обязательный реквизит по схеме, но может отсутствовать, например, у DP_KVITIZMSTATUS.
		Объект.ВерсияСхемы = ЗначениеНеобязательногоСвойства(ДанныеОбъекта, "schemeVersion");
		Объект.НомерВерсии = ЗначениеНеобязательногоСвойства(ДанныеОбъекта, "versionNumber", "Число");
		Статус = ЗначениеНеобязательногоСвойства(ДанныеОбъекта, "status");
		Если ЗначениеЗаполнено(Статус) Тогда
			Объект.Статус = ОпределитьСтатусДокумента(Статус);
...
Функция РазобратьСведенияОПоставщикеИЕгоПодписантах(Данные)
	...
	Для каждого Подписант Из ЗначениеВКоллекцию(УзелИнформации.signersInfo.signerInfo) Цикл
		...
		Если ЕстьСвойство(Подписант, "authoritysInfo")
			// Узел authoritysInfo обязателен по схеме, но может отсутствовать в ответе сервера ЕИС.
...
Функция ПроверитьДоступностьСервераSOAP(АдресСервера, АдресРесурса)
	...
	КлючДатаПроверкиДоступности = "Обмен.ДатаПроверкиДоступностиСервиса";
	ДатаПроверкиДоступности = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		КлючОбъектаОбщихНастроек(),
		КлючДатаПроверкиДоступности, Дата(1, 1, 1));
	
	Час = 3600;
	Если ТекущаяУниверсальнаяДата() - ДатаПроверкиДоступности < Час Тогда
		Результат.Выполнено = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибкиОтправки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
Функция ОшибкаСервисаЗагрузкиПакетов(ТекстОтвета)
	...
	Если Результат.ОшибкаВЗапросе Тогда
		...
		ВариантСобытия = НСтр("ru = 'Ошибка в запросе к сервису ЕИС или ошибка сервера'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС
			 + НСтр("ru = 'Ответ сервера ЕИС:'") + Символы.ПС
			 + ТекстОтвета;
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ВариантСобытия, , УровеньЖурналаРегистрации.Ошибка);
...
Функция ПодготовитьТаблицуДанныхОснованийУПД(Знач Наименование, Знач Номер, Знач Дата, Знач ДопСведения)
...
Процедура ЗаполнитьИдентификаторИсходногоФайлаПриложения(ДанныеПриложения)
	...
	Если ЗначениеЗаполнено(ДанныеПриложения.ДокументОснование) Тогда
		...
		Если ЗначениеЗаполнено(ДокументОснование) Тогда
			...
			ИмяФайла = ДанныеФайла.ИмяБезРасширения;
			// Основанием документа ЕИС не может быть "Акт приемки‑сдачи работ (услуг) по приказу 552".
			АктПриемки552 = "DP_REZRUISP";
			Если СтрНайти(ИмяФайла, АктПриемки552) = 0 Тогда
				ДанныеПриложения.ИдентификаторИсходногоФайла = ИмяФайла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПерекодироватьДокументыПакета(Знач ТекстПакета)
	
	ТегНачалоКонтента = "<Контент>";
	ТегОкончаниеКонтента = "</Контент>";
	ДлинаТегаКонтент = СтрДлина(ТегНачалоКонтента);
	НомерНачало = ДлинаТегаКонтент + 1;
	Пока НомерНачало > ДлинаТегаКонтент Цикл
		НомерНачало = СтрНайти(ТекстПакета, ТегНачалоКонтента, , НомерНачало);
		Если НомерНачало > 0 Тогда
			НомерТегаОкончания = СтрНайти(ТекстПакета, ТегОкончаниеКонтента, , НомерНачало);
			Содержимое = Сред(ТекстПакета, НомерНачало + ДлинаТегаКонтент, НомерТегаОкончания - НомерНачало - ДлинаТегаКонтент);
			ДвоичныеДанные = Base64Значение(Содержимое);
			Кодировка = ОпределитьКодировку(ДвоичныеДанные);
			Если НЕ Кодировка = КодировкаТекстаСервисаЕИС() Тогда
				// Может приходить документ в кодировке UTF-8.
				ТекстДокумента = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанные,
					Кодировка);
				ДвоичныеДанные = ПолучитьДвоичныеДанныеИзСтроки(ТекстДокумента,
					КодировкаТекстаСервисаЕИС());
				Содержимое = Base64Строка(ДвоичныеДанные);
				НомерКонец = НомерТегаОкончания + ДлинаТегаКонтент + 1;
				ТекстПакета = Лев(ТекстПакета, НомерНачало - 1) + ТегНачалоКонтента + Содержимое + ТегОкончаниеКонтента
					+ Прав(ТекстПакета, СтрДлина(ТекстПакета) - НомерКонец + 1);
			КонецЕсли;
		КонецЕсли;
		НомерНачало = НомерНачало + ДлинаТегаКонтент;
	КонецЦикла;
	Возврат ТекстПакета;
...
Функция НовыеДанныеТранспортногоПакета()
	...
	// РеестрНомКонт, Строка 19, необязательный 
	Результат.Вставить("РеестровыйНомерКонтракта", "");
...
Функция ПространствоИменВерсииСхем()
	...
	ПространстваВерсий.Вставить("13.2", "DP_PAKET_EIS_01_00_13_2");
...
Функция ДополнительныеСведенияОснованияПоДаннымКонтракта(ДанныеКонтракта)
	
	Возврат НСтр("ru = 'Реестровый номер в реестре контрактов:'")
		+ " " + ДанныеКонтракта.Код;
	
КонецФункции

Функция ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС(Знач ИмяФайла)
	
	ИмяФайла = ВРег(ИмяФайла);
	ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС = СтрНайти(ИмяФайла, "2ZK-CUS") <> 0 И СтрНайти(ИмяФайла, "2ZK-SUP") <> 0;
	Возврат ЭтоИмяФайлаИспользуетсяПриОбменеСЕИС;
	 
КонецФункции

Функция ПространствоИменЕИСПоИмениФайла(Знач ИмяФайла)
	
	СтруктураИмен = Новый Структура;
	СтруктураИмен.Вставить("ON_AKTREZRABP", "ON_AKTREZRABP_1_971_01_01_00_01_EIS");
	СтруктураИмен.Вставить("ON_NSCHFDOPPR", "ON_NSCHFDOPPR_1_997_01_05_01_02_EIS");
	
	ИмяФайла = ВРег(ИмяФайла);
	
	Для Каждого КлючЗначение Из СтруктураИмен Цикл
		
		Если СтрНачинаетсяС(ИмяФайла, КлючЗначение.Ключ) Тогда
...
Функция ПолучитьИдентификаторЗаказчика(Знач КодПоСПЗ)
	
	Шаблон = "00000000000%1";
	ПодготовленныйКод = СтрШаблон(Шаблон, КодПоСПЗ);
	Код = Прав(ПодготовленныйКод, 11);
	
	Оператор = ОператорЭДОФедеральногоКазначейства();
	Шаблон = "%1-CUS-%2";
	Возврат СтрШаблон(Шаблон, Оператор.Идентификатор, Код);
	
КонецФункции

Процедура УбратьКоличествоПриТекстовомВыражениОбъема(ВидДокумента, Файл, ДеревоДанных)
	
	Если ВидДокумента = "УПДПрод" Тогда 
		ДанныеБД = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
						ДеревоДанных, "ДанныеЭлектронногоАктированияЕИС.ДанныеБД");
		ДанныеПриложения = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
						ДеревоДанных, "ДанныеЭлектронногоАктированияЕИС.ДанныеПриложения");
		Контракт = ДанныеБД.ДанныеГосконтракта[0].Ссылка;
		ДанныеКонтракта = ДанныеГосударственногоКонтракта(Контракт);
		
		ТаблСчФакт = ЗначениеНеобязательногоСвойства(Файл.Документ, "ТаблСчФакт");
		Если ТаблСчФакт = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		СведенияОТоварах = Файл.Документ.ТаблСчФакт.СведТов;
		Для Каждого СведениеОТоваре Из СведенияОТоварах Цикл
			НомерСтроки = СведениеОТоваре.НомСтр;
			ЕстьУказаниеОбъемаВТекстовомВыражении =
				ЕстьУказаниеОбъемаВТекстовомВыражении(ДанныеПриложения, ДанныеКонтракта, НомерСтроки);
				
			Если ЕстьУказаниеОбъемаВТекстовомВыражении Тогда
				СведениеОТоваре.Сбросить("КолТов");
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ЕстьУказаниеОбъемаВТекстовомВыражении(ДанныеПриложения, ДанныеКонтракта, НомерСтроки)
	
	Если ТипЗнч(ДанныеПриложения.СведенияОТоварах) <> Тип("Структура") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	МассивТоваров = ДанныеПриложения.СведенияОТоварах.ТоварыРаботыУслуги;
	
	Для Каждого СтруктураСведенийОТоваре Из МассивТоваров Цикл
		
		Если СтруктураСведенийОТоваре.НомерСтрокиТаблицы = НомерСтроки Тогда
			
			Для Каждого ОбъектЗакупки Из ДанныеКонтракта.ОбъектыЗакупки Цикл
				
				Если ОбъектЗакупки.Идентификатор = СтруктураСведенийОТоваре.Идентификатор Тогда
					Если ОбъектЗакупки.СпособУказанияОбъемаРаботыУслуги = "VTF" Тогда
						Возврат Истина;
					КонецЕсли;
