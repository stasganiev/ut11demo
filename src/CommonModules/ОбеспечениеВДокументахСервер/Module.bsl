...
Функция ПараметрыВыбораОбеспечения(Форма, ТекущаяСтрока, Режим, Склады = Неопределено) Экспорт
	...
	КорректировкаДвижений = ?(ПараметрыВстраивания.АдресКорректировкиДвижений = Неопределено,
		Неопределено,
		ПолучитьИзВременногоХранилища(Форма[ПараметрыВстраивания.АдресКорректировкиДвижений]));
	ДобавитьВременныеТаблицыДляПолученияОстатков(Запрос, Истина, ТаблицаОформлено, КорректировкаДвижений, Истина, Истина);
...
Функция РаспределениеЗапасовДляРедактируемогоДокумента(ВызовИзФормы, ИсточникДанных, ОтборСостояний = Неопределено) Экспорт
	...
	Регистраторы = Запрос.МенеджерВременныхТаблиц.Таблицы[0].ПолучитьДанные().Выгрузить().ВыгрузитьКолонку("Регистратор");
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Заказы, Регистраторы); // для этапов перерабатываемых на стороне.
...
Функция ВыгрузитьТаблицуИзмененийДокумента(МенеджерВременныхТаблиц, Форма)
	...
	ТекстЗапроса =
		...
		|	ВЫБОР
		|		КОГДА СУММА(Корректировка.Отгрузить + Корректировка.Резервировать) > 0
		|			ТОГДА СУММА(Корректировка.Отгрузить + Корректировка.Резервировать)
		|		ИНАЧЕ 0
		|	КОНЕЦ <
		|	ВЫБОР
		|		КОГДА СУММА(Корректировка.РезервироватьПоМереПоступления) < 0
		|			ТОГДА -СУММА(Корректировка.РезервироватьПоМереПоступления)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ОбновлятьКолонкуДоступноПослеРаспределенияЗапасов
		|ИЗ(
		|	ВЫБРАТЬ
		|		Движения.Номенклатура    КАК Номенклатура,
		|		Движения.Характеристика  КАК Характеристика,
		...
		|	ГДЕ
		|		Движения.ВНаличии <> 0 ИЛИ Движения.РезервироватьНаСкладе <> 0
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		// то колонку Доступно нужно перезаполнять в форме документ после выполения заданий распределения запасов по заказу,
