...
Процедура ЗаполнитьСтатическиеИнформационныеСсылки(Форма, МассивЭлементов, ЭлементВсеСсылки = Неопределено, 
	...
	Попытка
		...
		Если ТипЗнч(ЭлементВсеСсылки) = Тип("ДекорацияФормы") Тогда
...
Функция УстановленаИнтеграцияСоСлужбойПоддержки() Экспорт
	...
	Возврат НастроенаИнтеграцияСИнформационнымЦентром();
...
//
Функция ШаблонТекстаВТехПоддержку(ТекстСообщения = "") Экспорт
	...
	Шаблон = НСтр("ru = 'Здравствуйте.
		...
		|<p/>
		|С уважением, %3.'");
	Шаблон = СтрШаблон(Шаблон, ТекстСообщения, "ПозицияКурсора", Пользователи.ТекущийПользователь().ПолноеНаименование());
...
Функция ЗапроситьКодыПользователей(ДанныеПользователей) Экспорт
	...
	Результат.Вставить("АдресаНетПользователяУАбонента", Неопределено);
	Результат.Вставить("ЕстьСтраницаСЗащитой", Ложь);
	Результат.Вставить("ТекстСообщения", "");
	
	ИмяМетода = "RequestLocalUserCodesStart";
	
	АдресСервиса = АдресУСП + "/v1/"+ИмяМетода;
	...
	Попытка
		...
		СведенияОбИБДляИнтеграции = СведенияОбИБДляИнтеграции();
		
		ДанныеЗапроса = Новый Структура;
		ДанныеЗапроса.Вставить("method_name", ИмяМетода);
		ДанныеЗапроса.Вставить("users_data", ДанныеПользователей);
		ДанныеЗапроса.Вставить("ibid", СведенияОбИБДляИнтеграции.ИдентификаторИБ);
		ДанныеЗапроса.Вставить("register_code", КодРегистрацииВУСП());
		...
		Результат.ЗапросОтправлен = Истина;
		
		Если ДанныеОтвета.Свойство("request_id") Тогда
			Результат.ИдентификаторЗапроса = ДанныеОтвета.request_id;
		КонецЕсли;
		
		Если ДанныеОтвета.Свойство("captcha")
			И ДанныеОтвета.captcha Тогда
			Результат.ЕстьСтраницаСЗащитой = Истина;
		Иначе
			Результат.ЕстьСтраницаСЗащитой = Ложь;
		КонецЕсли;
		
		Если ДанныеОтвета.Свойство("letters_sent_count") Тогда
			Результат.КоличествоОтправленных = ДанныеОтвета.letters_sent_count;
		КонецЕсли;
		
		Если ДанныеОтвета.Свойство("no_abonent_users") Тогда
			Результат.АдресаНетПользователяУАбонента = ДанныеОтвета.no_abonent_users;
		КонецЕсли;
...
Функция РасшифроватьТокенДляРегистрацииВСлужбеПоддержки(Токен) Экспорт
	...
	Попытка
		...
		ДвоичныеДанные = base64UrlDecode(Токен);
