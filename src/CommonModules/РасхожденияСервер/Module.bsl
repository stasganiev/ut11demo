...
Процедура ЗаполнитьПоОснованиям(Форма, ВызыватьУправлениеДоступностью = Истина) Экспорт
	...
	Иначе
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Форма.Объект.ТипОснованияАктаОРасхождении)
			...
			|	ТоварыПередачи.УдалитьСуммаНДС       КАК СуммаНДС,
			|	ТоварыПередачи.УдалитьСуммаСНДС      КАК СуммаСНДС,
			...
			|	ТоварыПередачи.УдалитьСуммаНДС       КАК СуммаНДСПоДокументу,
			|	ТоварыПередачи.УдалитьСуммаСНДС      КАК СуммаСНДСПоДокументу,
		...
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(Форма.Объект.ТипОснованияАктаОРасхождении)
			...
			|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка) КАК СтавкаНДС,
			|	ТаблицаТовары.УдалитьСуммаНДС     КАК СуммаНДС,
			|	ТаблицаТовары.УдалитьСуммаСНДС    КАК СуммаСНДС,
			|	ТаблицаТовары.УдалитьСуммаНДС     КАК СуммаНДСПоДокументу,
			|	ТаблицаТовары.УдалитьСуммаСНДС    КАК СуммаСНДСПоДокументу,
...
Функция ТекстЗапросаТабличныеЧастиАктаОРасхожденияхПослеПриемкиИПриемкиТоваровНаХранение(АктОРасхождениях, Основание, СтруктураВозврата, ИзменениеДокумента)
	...
	|	АктОРасхожденияхПослеПриемкиТовары.КодСтроки                            КАК КодСтроки,
	|	ВЫБОР
	|		КОГДА АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику
	|	КОНЕЦ                                                                   КАК ЗаказПоставщику,
	|	АктОРасхожденияхПослеПриемкиТовары.Склад                                КАК Склад,
	...
	|	ПриемкаТоваровНаХранениеТовары.КодСтроки                 КАК КодСтроки,
	|	ВЫБОР
	|		КОГДА ПриемкаТоваровНаХранениеТовары.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ПриемкаТоваровНаХранениеТовары.ЗаказПоставщику
	|	КОНЕЦ                                                    КАК ЗаказПоставщику,
	|	ПриемкаТоваровНаХранениеТовары.Склад                     КАК Склад,
...
//
Процедура ОбработатьУказаниеНомераГТДСервер(Форма,
											ИдентификаторСтроки,
											РезультатПодбора,
											ЗаполненыНомераГТД,
											НомерГТД) Экспорт
