...
Процедура ПроверитьКорректностьКомплекта(Объект, Отказ, ПараметрыПроверки) Экспорт
	...
	Если ОшибкиПроверки.Следующий() Тогда
		...
		Если ОшибкиПроверки.НепрослеживаемыйКомплект Тогда
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНепрослеживаемогоКомплекта(ДанныеКомплекта.ТипНоменклатуры),
										Номенклатура,
...
Процедура ПроверитьКорректностьПрослеживаемыхНомеровГТД(Объект, Отказ, ПараметрыПроверки) Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаДокумента", ТабличнаяЧастьОбъекта.Выгрузить());
...
Процедура УстановитьУсловноеОформлениеНомераГТД(Форма,
												...
												ПутьКПолюДатаДокумента = "Объект.Дата",
												ПутьКПолюНомерГТД = "Объект.Товары.НомерГТД") Экспорт
...
//	Форма - ДанныеФормыСтруктура, ФормаКлиентскогоПриложения - содержит данные формы, которая содержит:
//
Процедура УстановитьУсловноеОформлениеУчетаПоГТДВДокументахКомиссионера(Форма) Экспорт
	...
	ГруппаИли = Элемент.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
...
Процедура ПроверитьДанныеПрослеживаемостиНомеровГТД(Объект,
	...
	Запрос.Текст = 
		...
		|		ПО ТаблицаТоваров.НомерГТД = НомераГТДПрослеживаемыеКомплектующие.Ссылка
		|ГДЕ
		|	НомераГТДПрослеживаемыеКомплектующие.Ссылка.ПометкаУдаления = ЛОЖЬ
		|	И ВЫРАЗИТЬ(ТаблицаТоваров.НомерГТД КАК
		|		Справочник.НомераГТД).ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТКомплекта)
	...
	Если ИмяТабличнойЧасти = "ВидыЗапасов" Тогда
		...
		ТекстОшибки = НСтр("ru='Документ содержит номера ГТД, в которых некорректно заполнены стоимостные показатели комплектующих.
									|Для уточнения информации откройте форму редактирования видов запасов документа.'");
...
Функция ТекстЗапросаВТДанныеКомплекта()
	...
	|	ВЫБОР
	|		КОГДА Товары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Набор)
	|			ТОГДА ЕСТЬNULL(Классификатор.ПрослеживаемыйТовар, ЛОЖЬ)
	|		ИНАЧЕ Товары.ПрослеживаемыйТовар
...
Функция ТекстЗапросаПроверокКомплекта()
	...
	ТекстыПроверок.Добавить(ТекстЗапросаПроверкиНепрослеживаемогоКомплекта());
	ТекстыПроверок.Добавить(ТекстЗапросаПроверкиОтличияКодовТНВЭДСборки());
	ТекстыПроверок.Добавить(ТекстЗапросаПроверкиОтличияКодовТНВЭДРазборки());
...
Функция ТекстЗапросаПроверкиНепрослеживаемогоКомплекта()
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИСТИНА	КАК НепрослеживаемыйКомплект
	|ИЗ
	|	ДанныеКомплекта КАК ДанныеКомплекта
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПрослеживаемыеКомплектующие КАК Комплектующие
	|		ПО ИСТИНА
	|
	|ГДЕ
	|	НЕ ДанныеКомплекта.ПрослеживаемыйТовар
	|	И НЕ Комплектующие.Номенклатура ЕСТЬ NULL";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаПроверкиОтличияКодовТНВЭДСборки()
	...
	|ГДЕ
	|	&СборкаКомплекта
	|	И ДанныеКомплекта.ПрослеживаемыйТовар
	|	И НЕ ДанныеКомплекта.КодТНВЭД В
	|		(ВЫБРАТЬ
	|			Комплектующие.КодТНВЭД КАК КодТНВЭД
	|		ИЗ
	|			ПрослеживаемыеКомплектующие КАК Комплектующие
	|		ГДЕ
	|			Комплектующие.ОсновнаяКомплектующая)";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаПроверкиОтличияКодовТНВЭДРазборки()
	...
	|			ИСТИНА
	|		ИЗ
	|			ПрослеживаемыеКомплектующиеСгруппированная КАК Комплектующие
	|		ИМЕЮЩИЕ
	|			КОЛИЧЕСТВО(*) = 1)";
...
Функция ШаблонСообщенияНепрослеживаемогоКомплекта(ТипНоменклатуры)
	
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		ТекстШаблона = НСтр("ru='Непрослеживаемый по РНПТ набор ""%1"" не должен содержать прослеживаемые комплектующие:
							|%2.'");
	Иначе
		ТекстШаблона = НСтр("ru='Непрослеживаемый по РНПТ комплект ""%1"" не должен содержать прослеживаемые комплектующие:
							|%2.'");
	КонецЕсли;
