...
Функция ОбщиеНастройкиРазбораКодаМаркировки(ТолькоСервер = Ложь, ВидыПродукции = Неопределено, ТолькоУчитываемыеВидыПродукции = Истина) Экспорт
	...
	Если ТолькоУчитываемыеВидыПродукции И ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС("БазоваяФункциональность") Тогда
		УчитываемыеВидыМаркируемойПродукции = ПолучитьИспользуемыеВидыПродукцииРасширеннойВерсии();
	Иначе
		УчитываемыеВидыМаркируемойПродукции = ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииИСМП(Истина);
		...
		Если ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная Тогда
			...
			Если МодулиПодсистемыЕГАИС = Неопределено Тогда
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЕГАИС") Тогда
					МодульНастройки = "РазборКодаМаркировкиЕГАИСОбщегоНазначения";
					МодульРазбораКМ = "РазборКодаМаркировкиЕГАИСОбщегоНазначенияКлиентСервер";
...
#Область НастройкиШаблоновКодовМаркировки
...
Функция НастройкиОписанияШаблонаКодаМаркировкиВидаПродукции() Экспорт
	...
	СтруктураОписания.Вставить("ВидПродукции");
	СтруктураОписания.Вставить("ТипШтрихкодаИВидУпаковки");
	СтруктураОписания.Вставить("СоставКодаМаркировки");
	СтруктураОписания.Вставить("СписокШаблонов");
	СтруктураОписания.Вставить("ДанныеОбщегоМодуля");
...
#Область НастройкиРазбораКодаМаркировкиСлужебные
...
// Конструктор настроек разбора кода маркировки.
//
// Возвращаемое значение:
//    Структура:
...
//     * ШаблоныКодовМаркировкиПоВидамПродукции - см. ИнициализацияШаблоновКодовМаркировки
//     * ШаблоныКодовМаркировки - см. ИнициализацияШаблоновКодовМаркировки
...
Функция ИнициализацияНастроекРазбораКодаМаркировки()
	...
	ШаблоныКодовМаркировки = ИнициализацияШаблоновКодовМаркировки();
...
// Конструктор шаблонов кодов маркировки.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//    * Шаблон - Строка
...
//    * ВключаетИдентификаторыПрименения - Булево
//    * Длина - Число
//    * ДлинаСоСкобкой - Число
//    * ДлинаСРазделителемGS - Число
//    * КодПервогоЭлемента - Строка
Функция ИнициализацияШаблоновКодовМаркировки() Экспорт
	...
	Возврат ШаблоныКодовМаркировки;
...
Функция ПолучитьИспользуемыеВидыПродукцииРасширеннойВерсии()
	
	//@skip-check wrong-string-literal-content
	Модуль = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИС");
	Возврат Модуль.УчитываемыеВидыМаркируемойПродукции();
