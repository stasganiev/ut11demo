...
// * РезультатПодготовкиДанныхОблачногоЭДО - Неопределено - если подготовка не выполнялась.
//                               - Структура:
//                                 ** АдресДанных - Строка
Функция НовыеПараметрыПредварительногоПросмотраЭлектронногоДокумента() Экспорт
	Параметры = Новый Структура;
	...
	Параметры.Вставить("РезультатПодготовкиДанныхОблачногоЭДО", Неопределено);
	Возврат Параметры;
КонецФункции
...
Функция ПроверитьГотовностьОбъектаУчетаКДокументообороту(ОбъектУчета) Экспорт 
		...
		Если ПараметрыСоставаПечати.ПечататьТехнологическиеКвитанции Тогда
			...
			Для каждого ТехническоеСообщение Из ТехническиеСообщения Цикл
				...
				Если ПараметрыСоставаПечати.ДополнятьСинонимМакетаВидомПечатнойФормы Тогда
					...
					Если ЭлементРегламента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ДопСведенияПоРезультатамПриемки Тогда
...
// Параметры:
// 	СостояниеДокумента - см. ДействияПоСостояниюДокумента.СостояниеДокумента
// 	СвойстваДокумента - см. ДействияПоСостояниюДокумента.СвойстваДокумента
// 
// Возвращаемое значение:
//  - Неопределено
//  - ПеречислениеСсылка.ДействияПоЭДО
Функция ОжидаемоеДействиеСтатусаЭлектронногоДокумента(СостояниеДокумента, СвойстваДокумента) Экспорт
	
	ДоступныеДействия = ДействияПоСостояниюДокумента(СостояниеДокумента, СвойстваДокумента);
...
Функция НовоеОписаниеПодписанногоСообщения() Экспорт
	...
	ОписаниеДанных.Вставить("Организация", Неопределено);
...
Функция ПараметрыОтправкиПолученияДокументовЭДО() Экспорт
...
// Возвращаемое значение:
//  Массив из Строка
Функция ИдентификаторыУчетныхЗаписейДляОтправкиПолученияДокументовЭДО(ОтборУчетныхЗаписей = Неопределено) Экспорт
...
Функция ПараметрыПросмотраЭлектронногоДокументаПоОбъектуУчета(ОбъектУчета, ПараметрыОткрытия) Экспорт
	...
	Если ИспользоватьИнтеграциюОблачногоЭДО() Тогда
		ОбщийМодульИнтеграцияОблачногоЭДО = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияОблачногоЭДО");
		ПараметрыПросмотра = ОбщийМодульИнтеграцияОблачногоЭДО.ПараметрыПросмотраЭлектронногоДокументаПоОбъектуУчета(
...
// Возвращаемое значение:
//  Структура:
//  * Ошибка    - Булево - признак успешного формирования данных для просмотра.
...
//  * АдресаОписанийСообщений - Строка - адреса временных хранилищ данных для просмотра.
...
Функция ПодготовитьДанныеПредварительногоПросмотра(ОписаниеОбъектаУчета, НастройкиФормирования, ПараметрыОткрытия) Экспорт
	...
	РезультатПодготовкиДанных.Вставить("АдресаОписанийСообщений", "");
	РезультатПодготовкиДанных.Вставить("АдресСведенийОбОшибках", "");
	РезультатПодготовкиДанных.Вставить("ИдентификаторОсновногоФайла", "");
	...
	АдресаОписанийСообщений = Новый Массив;
	Для Каждого ОписаниеСообщения Из ОписанияСообщений Цикл
		...
		Если ОписаниеСообщения.ТипЭлементаРегламента =
			...
			ДлинаРасширенияСТочкой = СтрДлина(РасширениеФайла) + 1;
			ИмяФайлаБезРасширения = Лев(ИмяФайла, СтрДлина(ИмяФайла) - ДлинаРасширенияСТочкой);
            РезультатПодготовкиДанных.ИдентификаторОсновногоФайла = ИмяФайлаБезРасширения;
		КонецЕсли;
		
		АдресаОписанийСообщений.Добавить(
			ПоместитьВоВременноеХранилище(ОписаниеСообщения, Новый УникальныйИдентификатор));
	КонецЦикла;
...
// Возвращаемое значение:
//  Булево
Функция ИспользоватьИнтеграциюВстроенногоЭДО() Экспорт
