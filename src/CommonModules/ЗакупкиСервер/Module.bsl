...
Процедура ЗаполнитьНомераГТДВТабличнойЧасти(Товары,
	...
	ИмпортныеТовары = РезультатПоНоменклатуре.Выгрузить().ВыгрузитьКолонку("Ссылка");
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если ИмпортныеТовары.Найти(СтрокаТаблицы[ИмяПоляНоменклатура]) <> Неопределено Тогда
...
Функция НайтиИлиСоздатьСоставныеПрослеживаемыеНомераГТД(Товары,
	...
	|			ТОГДА
	|				Товары.Номенклатура.КодТНВЭД.ЕдиницаИзмерения
	|			ИНАЧЕ
	...
	|	СоставнойНомер.ЕдиницаИзмеренияТНВЭД КАК ЕдиницаИзмеренияТНВЭД,
	// Номенклатура, характеристика и серия является справочной информацией, и не является ключом при поиске/создании прослеживаемых комплектов.
...
Функция РазобратьСоставныеПрослеживаемыеНомераГТД(ТоварыКСписанию, ТоварыКОприходованию, ПоляОбъектные = Неопределено, ПоляЧисловые = Неопределено, Отказ = Ложь) Экспорт
	...
	|	&СуммаПоляЧисловые,
	|	Товары.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.ЕдиницаИзмерения КАК ЕдиницаИзмеренияТНВЭД,
