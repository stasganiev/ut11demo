...
Процедура ОткрытьСписокСправок1СПАРКРиски(
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ПоказыватьОтбор     = ДополнительныеПараметры.ПоказыватьОтбор;
		Источник            = ДополнительныеПараметры.Источник;
		Уникальность        = ДополнительныеПараметры.Уникальность;
...
Функция ИндексыСПАРККонтрагента(Форма, Контрагент, ВидКонтрагента, ПризнакЗагрузкиДанных = Истина) Экспорт
	...
	Если ТипЗнч(КэшИндексов) = ТипСоответствие Тогда
		...
		Если НайденноеЗначениеДляКонтрагента <> Неопределено
			...
			Если НайденноеЗначениеДляКонтрагента.ДатаЗаполненияКлиентскогоКэша >= (ОбщегоНазначенияКлиент.ДатаСеанса() - 3600) Тогда
...
Процедура ОбработкаНавигационнойСсылки(
	...
	Если СтандартнаяОбработкаБиблиотекой = Истина Тогда
		...
		ИначеЕсли ВРег(НавигационнаяСсылка) = ВРег(Настройки.ОписаниеСтатусаНавигационнаяСсылкаПодсказки) Тогда // Открыть описание статуса
			...
			ОткрытьФорму("Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАРКРиски", ПараметрыФормы, Форма);
		...
		ИначеЕсли ВРег(НавигационнаяСсылка) = ВРег(Настройки.НетИнформацииНавигационнаяСсылкаПодсказки) Тогда // Причины отсутствия информации о контрагенте.
			...
			ОткрытьФорму("Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАРКРиски", ПараметрыФормы);
		...
		ИначеЕсли ВРег(НавигационнаяСсылка) = ВРег(Настройки.ОткрытьУведомление) Тогда
			СтандартнаяОбработкаФормой = Ложь;
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторПортала1СИТС") Тогда
				...
				МодульМониторПортала1СИТСКлиент.ОткрытьСообщениеОбОпцияхИнтернетПоддержки(
					Форма.ИндексыСПАРКРиски.Уведомление);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
				...
				Если Источник = Форма.УникальныйИдентификатор Тогда
					...
					Если Параметр.Статус = "Выполнено" Тогда
						Если НЕ ПустаяСтрока(Параметр.АдресХранилищаФоновогоЗадания) Тогда
							...
							// Там есть таблица значений, которую необходимо преобразовать в структуру
							// вида СПАРКРискиКлиентСервер.НовыйДанныеИндексов.
...
//
Процедура ПерейтиВВебВерсиюСПАРК(
		Контрагент,
		ОписаниеОповещения = Неопределено,
		ПодключитьТестовыйПериод = Истина) Экспорт
	
	ВыполнитьОповещение = Истина;
	СообщениеОбОшибке   = "";
	
	Если ИспользованиеРазрешено() Тогда
		Результат = СПАРКРискиВызовСервера.URLВебВерсииСПАРК(Контрагент);
	Иначе
	...
	// Стандартная обработка перехода в веб-версию СПАРК.
	Если ОписаниеОповещения = Неопределено Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ОбработатьПереходВВебВерсиюСПАРК",
			СПАРКРискиКлиент);
	КонецЕсли;
	
	Если Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Не удалось подключиться к сервису 1СПАРК Риски </b>
				|
				|Не удалось подключиться к сервису 1СПАРК Риски. Сервис временно недоступен. Повторите попытку подключения позже.'"));
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				|
				|Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.'"));
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ОшибкаПодключения") Тогда
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена")
		И Не ИнтернетПоддержкаПользователейВызовСервера.ДоступноПодключениеИнтернетПоддержки() Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				|
				|Для работы с сервисом 1СПАРК Риски необходимо подключить Интернет-поддержку пользователей. Обратитесь к администратору.'"));
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена") Тогда
		
		ПараметрыПерехода = Новый Структура;
		ПараметрыПерехода.Вставить("Контрагент",               Контрагент);
		ПараметрыПерехода.Вставить("ОписаниеОповещения",       ОписаниеОповещения);
		ПараметрыПерехода.Вставить("ПодключитьТестовыйПериод", ПодключитьТестовыйПериод);
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ПараметрыПерехода",   ПараметрыПерехода);
		ДополнительныеПараметры.Вставить("ИдентификаторВызова", "ПереходВВебВерсию");
		
		ПоказатьВопрос(
			Новый ОписаниеОповещения(
				"ПослеОтветаНаВопросПодключенияИнтернетПоддержкиПользователей",
				ЭтотОбъект,
				ДополнительныеПараметры),
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит") Тогда
		
		Если ПодключитьТестовыйПериод
			И ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодключениеСервисовСопровождения") Тогда
			
			ПараметрыПерехода = Новый Структура;
			ПараметрыПерехода.Вставить("Контрагент",               Контрагент);
			ПараметрыПерехода.Вставить("ОписаниеОповещения",       ОписаниеОповещения);
			ПараметрыПерехода.Вставить("ПодключитьТестовыйПериод", Ложь);
			
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("ПараметрыПерехода",   ПараметрыПерехода);
			ДополнительныеПараметры.Вставить("ИдентификаторВызова", "ПереходВВебВерсию");

			ОписаниеОповещенияТестовыйПериод = Новый ОписаниеОповещения(
				"ПодключитьТестовыйПериодЗавершение",
				ЭтотОбъект,
				ДополнительныеПараметры);
			...
			МодульПодключениеСервисовСопровожденияКлиент.ПодключитьТестовыйПериод(
				СПАРКРискиКлиентСервер.ИдентификаторСервиса(),
				ЭтотОбъект,
				ОписаниеОповещенияТестовыйПериод);
		...
		Иначе
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
					|
					|Недостаточно лимита для выполнения перехода.
					|Для пополнения лимита необходимо <a href=""%1"">приобрести тариф</a>,
					|который включает в себя возможность перехода в веб-версию СПАРК.'"),
				СПАРКРискиКлиентСервер.АдресСтраницыТарифовСервисаНаПортале1С());
			
		КонецЕсли;
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеПодлежитПроверке") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				|
				|Контрагент не подлежит проверке, переход в веб-версию невозможен.'"));
		
	ИначеЕсли ЗначениеЗаполнено(Результат.ВидОшибки) Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Не удалось перейти в веб-версию СПАРК </b>
				|
				|При получении ссылки для перехода возникли ошибки. Обратитесь к администратору.'"));
...
Процедура ПереходыВВебВерсиюСПАРК() Экспорт
	...
	ОткрытьФорму("РегистрСведений.ПереходыВВебВерсиюСПАРК.ФормаСписка");
...
Процедура ВыполнитьПодключаемуюКоманду(Команда, Форма, Источник, ДополнительныеПараметры = Неопределено) Экспорт
		...
		Если Источник.Ссылка.Пустая() Или Форма.Модифицированность Тогда
			...
			ИначеЕсли Команда.Имя = "СПАРКРиски_ВключитьМониторингСобытий" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
					...
					|Данные будут записаны.'");
			ИначеЕсли Команда.Имя = "СПАРКРиски_ПерейтиВСПАРК" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
	...
	ИначеЕсли Команда.Имя = "СПАРКРиски_ВключитьМониторингСобытий" Тогда
		...
		СПАРКРискиВызовСервера.ВключитьОтключитьМониторингСобытийКонтрагента(
			КонтрагентСсылка,
			ВключитьМониторинг);
	ИначеЕсли Команда.Имя = "СПАРКРиски_ПерейтиВСПАРК" Тогда
...
Процедура ПриОбработкеВыбораТабличногоДокумента(ФормаОтчета, Элемент, Область, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Область.Расшифровка = "ПодключитьИнтернетПоддержкуПользователей" Тогда
		...
		ПараметрыПерехода = Новый Структура;
		ПараметрыПерехода.Вставить("ФормаОтчета", ФормаОтчета);
	...
	ИначеЕсли Область.Текст = "Подробнее"
		И ТипЗнч(Область.Расшифровка) = Тип("Структура") Тогда
		
		СтандартнаяОбработка = Ложь;
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторПортала1СИТС") Тогда
			МодульМониторПортала1СИТСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("МониторПортала1СИТСКлиент");
			МодульМониторПортала1СИТСКлиент.ОткрытьСообщениеОбОпцияхИнтернетПоддержки(Область.Расшифровка);
		КонецЕсли;
...
Функция АдресСтраницыПереходовВВебВерсиюСПАРК() Экспорт
	
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
		"/application/list/1C-Spark-risks/received-cards-info?needAccessToken=true");
...
Процедура ОбработатьПереходВВебВерсиюСПАРК(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат.ВидОшибки) Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
			Результат.URL);
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Переход в веб-версию СПАРК'"),
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выполнен переход в веб-версию СПАРК для ИНН %1'"),
				Результат.ИНН));
	Иначе
...
Процедура ПодключитьТестовыйПериодЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Результат = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен")
		...
		ИначеЕсли ДополнительныеПараметры.ИдентификаторВызова = "ПереходВВебВерсию" Тогда
			ПерейтиВВебВерсиюСПАРК(
				ДополнительныеПараметры.ПараметрыПерехода.Контрагент,
				ДополнительныеПараметры.ПараметрыПерехода.ОписаниеОповещения,
				ДополнительныеПараметры.ПараметрыПерехода.ПодключитьТестовыйПериод);
		КонецЕсли;
...
// Обрабатывает ответ и открывает форму ввода данных аутентификации.
//
// Параметры:
//  Результат - КодВозвратаДиалога - ответ пользователя;
//  ДополнительныеПараметры - Структура - дополнительные параметры.
//
Процедура ПослеОтветаНаВопросПодключенияИнтернетПоддержкиПользователей(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		Если ДополнительныеПараметры.ИдентификаторВызова = "Отчет" Тогда
			ОтчетыКлиент.СформироватьОтчет(
				ДополнительныеПараметры.ПараметрыПерехода.ФормаОтчета);
		ИначеЕсли ДополнительныеПараметры.ИдентификаторВызова = "ПереходВВебВерсию" Тогда
			ПерейтиВВебВерсиюСПАРК(
				ДополнительныеПараметры.ПараметрыПерехода.Контрагент,
				ДополнительныеПараметры.ПараметрыПерехода.ОписаниеОповещения,
