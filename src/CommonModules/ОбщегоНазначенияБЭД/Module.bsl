...
// 
// Параметры:
// 	Форма - ФормаКлиентскогоПриложения - форма, положение и размер которой нужно сбросить.
...
// Формирует массив пустых значения для типов, описанных объектом ОписаниеТипов
// 
// Параметры:
...
//
Функция ПустыеЗначенияОписанияТипов(ОписаниеТипов) Экспорт
	...
	Для Каждого Тип Из Типы Цикл
		Типизатор = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип));
		ПустоеЗначение = Типизатор.ПривестиЗначение();
		Если ПустыеЗначения.Найти(ПустоеЗначение) = Неопределено Тогда
			ПустыеЗначения.Добавить(ПустоеЗначение);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьСлужебноеПолеВДинамическийСписок(ДинамическийСписок, ИмяПоля) Экспорт
	...
	Если ПолеУжеЕстьВЗапросе Тогда
		Возврат;
	КонецЕсли;
	...
	ПолеСписка = ДинамическийСписок.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
	ПолеСписка.ПутьКДанным = СтрШаблон("%1.%2", Оператор.Источники[0].Источник.Псевдоним, ИмяПоля);
	ПолеСписка.ОграничениеИспользования.Группировка = Ложь;
	ПолеСписка.ОграничениеИспользования.Порядок = Ложь;
	ПолеСписка.ОграничениеИспользования.Условие = Ложь;
	ПолеСписка.ОграничениеИспользования.Поле = Ложь;
	ПолеСписка.ОграничениеИспользованияРеквизитов.Группировка = Ложь;
	ПолеСписка.ОграничениеИспользованияРеквизитов.Порядок = Ложь;
	ПолеСписка.ОграничениеИспользованияРеквизитов.Условие = Ложь;
	ПолеСписка.ОграничениеИспользованияРеквизитов.Поле = Ложь;
КонецПроцедуры

// Возвращаемое значение:
//  Булево
Функция ЭтоРежимСервисаОблачногоЭДО() Экспорт
	Возврат ОбщегоНазначения.ПодсистемаСуществует("ОблачныйЭДО");
КонецФункции

// Выводит область ячеек в HTML - документ.
//
// Параметры:
//  СтрокаHTML        - Строка - код страницы HTML.
//  ТабличныйДокумент - ТабличныйДокумент - табличный документ для вывода.
//
Процедура ВывестиТабличныйДокументВДокументHTML(СтрокаHTML, ТабличныйДокумент) Экспорт
	...
	СтрокаHTMLТабличногоДокумента = ТекстовыйДокумент.ПолучитьТекст();
	
	СтрокаHTMLТабличногоДокумента = HTMLСтрокаСПреобразованиемСтилей(СтрокаHTMLТабличногоДокумента);
	
	СтрокаHTML = СтрокаHTML + Символы.ПС + СтрокаHTMLТабличногоДокумента;
	
	ФайловаяСистема.УдалитьВременныйФайл(ИмяВременногоФайла);
	
КонецПроцедуры

// Приводит все пустые значения составных измерений к значению Неопределено.
// 
// Анализирует метаданные регистра сведений на наличие измерений составного типа, которые не обязательны для заполнения,
// и приводит к одному значению Неопределено.
// 
// Данную нормализацию рекомендуется приводить для регистров сведений, в которых конкретный тип пустого значения
// не имеет отдельного прикладного смысла, т.е. все пустые значения равносильны.
// 
// Нормализация проводится с целью сократить вероятность возникновения дублей записей (записей, у которых отличаются
// пустые значения пустых измерений).
// 
// Параметры:
//  НаборЗаписей - РегистрСведенийНаборЗаписей
Процедура НормализоватьПустыеЗначенияСоставныхИзмеренийНабораЗаписей(НаборЗаписей) Экспорт
	МетаданныеРегистра = НаборЗаписей.Метаданные();
	ПроверяемыеИзмерения = СоставныеИзмеренияРегистраНеобязательныеДляЗаполнения(МетаданныеРегистра);
	Для Каждого Измерение Из ПроверяемыеИзмерения Цикл
		Для Каждого Запись Из НаборЗаписей Цикл
			Если Не ЗначениеЗаполнено(Запись[Измерение]) Тогда
				Запись[Измерение] = Неопределено;
			КонецЕсли;
		КонецЦикла;
		ОтборПоИзмерению = НаборЗаписей.Отбор.Найти(Измерение);
		Если ОтборУстановленИПустой(ОтборПоИзмерению) Тогда
			ОтборПоИзмерению.Значение = Неопределено;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
// По переданному КПП определяет КПП головной организации
// 
// Параметры:
//  КПП - Строка
// 
// Возвращаемое значение:
//  Строка
Функция КППГоловнойОрганизации(КПП) Экспорт
	Возврат Лев(КПП, 4) + "01" + Сред(КПП, 7);
КонецФункции

// Проверяет по КПП отношение кода к подразделению, представительству или филиалу
// 
// Параметры:
//  КПП - Строка
// 
// Возвращаемое значение:
//  Булево
Функция ЭтоПодразделениеФилиалИлиПредставительство(КПП) Экспорт
	...
	Если КодыКПППодразделений().Найти(КодПричиныПостановкиНаУчет) <> Неопределено
		Или КодыКППФилиалов().Найти(КодПричиныПостановкиНаУчет) <> Неопределено
		Или КодыКПППредставительств().Найти(КодПричиныПостановкиНаУчет) <> Неопределено Тогда
...
Функция ЗначениеФункциональнойОпции(НаименованиеФО) Экспорт
	...
	СоответствиеФО.Вставить("ИспользоватьВнутренниеДокументыЭДО", "ИспользоватьВнутренниеДокументыЭДО");
	СоответствиеФО.Вставить("ИспользоватьИнтеграциюСКлиентомЭДО", "ИспользоватьИнтеграциюСКлиентомЭДО");
	СоответствиеФО.Вставить("ИспользоватьСервис1СШтамп", "ИспользоватьСервис1СШтамп");
...
//  ВызыватьИсключение - Булево - Вызвать исключение, если перечисление не найдено. По умолчанию Истина
// 
// Возвращаемое значение:
//  ПеречислениеСсылка
//
Функция ЗначениеПеречисленияПоИмени(ИмяЗначения, МетаданныеПеречисления, ВызыватьИсключение = Истина) Экспорт
...
//
// Параметры:
//  НаборСсылок - Массив из ЛюбаяСсылка
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - ЛюбаяСсылка
//  * Значение - см. ОбщегоНазначения.ИдентификаторОбъектаМетаданных
Функция ИдентификаторыОбъектовМетаданных(НаборСсылок) Экспорт
	
	Результат = Новый Соответствие;
	
	ПолныеИменаОбъектов = Новый Соответствие;
	УникальныеИменаОбъектов = Новый Массив;
	Для Каждого СсылкаНаОбъект Из НаборСсылок Цикл
		ПолноеИмяОбъекта = СсылкаНаОбъект.Метаданные().ПолноеИмя();
		ПолныеИменаОбъектов.Вставить(СсылкаНаОбъект, ПолноеИмяОбъекта);
		Если УникальныеИменаОбъектов.Найти(ПолноеИмяОбъекта) = Неопределено Тогда
			УникальныеИменаОбъектов.Добавить(ПолноеИмяОбъекта);
		КонецЕсли;
	КонецЦикла;
	
	ИдентификаторыОбъектовМетаданных = ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(УникальныеИменаОбъектов);
	
	Для Каждого ПолноеИмяПоОбъекту Из ПолныеИменаОбъектов Цикл
		ИдентификаторОбъектаМетаданных = ИдентификаторыОбъектовМетаданных[ПолноеИмяПоОбъекту.Значение];
		Результат.Вставить(ПолноеИмяПоОбъекту.Ключ, ИдентификаторОбъектаМетаданных);
	КонецЦикла;
...
// Производит преобразование строки HTML, получаемой при сохранении табличного документа в HTML документ.
// Добавляются необходимые блоки и уникальные классы для применения стилей.
// 
// Параметры:
//  СтрокаHTML - Строка - код страницы HTML.
//
Функция HTMLСтрокаСПреобразованиемСтилей(СтрокаHTML)
	
	УИДКласса = "sign" + СтрЗаменить(Новый УникальныйИдентификатор(), "-", "");
	
	СтрокаHTML = СтрЗаменить(СтрокаHTML, "</BODY>", "</DIV></BODY>");
	
	ЗаменаСтилейЗапущена = Ложь;
	ЗаменаСтилейЗавершена = Ложь;
	ТегНачалаТелаЗаменен = Ложь;
	ИтоговыйHTML = "";
	
	Для Счетчик = 1 По СтрЧислоСтрок(СтрокаHTML) Цикл
		
		ТекСтрока = СтрПолучитьСтроку(СтрокаHTML, Счетчик);
		Если Не ЗаменаСтилейЗавершена И СтрНайти(ТекСтрока, "<STYLE") > 0 Тогда
			
			ЗаменаСтилейЗапущена = Истина;

		ИначеЕсли Не ТегНачалаТелаЗаменен И СтрНайти(ТекСтрока, "<BODY") > 0 Тогда
			
			ТегНачалаТелаЗаменен = Истина;
			ИтоговыйHTML = ИтоговыйHTML + "<BODY><DIV CLASS=""" + УИДКласса + """>" + Символы.ПС;
			Продолжить;
		
		ИначеЕсли ЗаменаСтилейЗапущена И НЕ ЗаменаСтилейЗавершена Тогда
			
			Если СтрНачинаетсяС(ТекСтрока, "body") Тогда
				
				ТекСтрока = СтрЗаменить(ТекСтрока, "body", "." + УИДКласса);
				
			ИначеЕсли СтрНачинаетсяС(ТекСтрока, "</STYLE>") Тогда
				
				ЗаменаСтилейЗавершена = Истина;
				
			Иначе
				
				ТекСтрока = "." + УИДКласса + " " + ТекСтрока;
				
			КонецЕсли;
			
		КонецЕсли;
		
		ИтоговыйHTML = ИтоговыйHTML + ТекСтрока + Символы.ПС;
...
// Параметры:
//  ИмяФайлаHTML - Строка - имя временного файла формата HTML.
//
...
// Коллекция имен измерений, которые могут быть пустыми.
// 
// Параметры:
...
// Возвращаемое значение:
//  Массив Из Строка
Функция СоставныеИзмеренияРегистраНеобязательныеДляЗаполнения(МетаданныеРегистра)
	Измерения = Новый Массив;
	Для Каждого ОписаниеИзмерения Из МетаданныеРегистра.Измерения Цикл
		Если Не ОписаниеИзмерения.ЗапрещатьНезаполненныеЗначения И ОписаниеИзмерения.Тип.Типы().Количество() > 1 Тогда
			Измерения.Добавить(ОписаниеИзмерения.Имя);
		КонецЕсли;
	КонецЦикла;
	Возврат Измерения;
КонецФункции
...
// Параметры:
//  ОтборПоИзмерению - ЭлементОтбора, Неопределено -
// 
// Возвращаемое значение:
//  Булево
Функция ОтборУстановленИПустой(ОтборПоИзмерению)
	Возврат ОтборПоИзмерению <> Неопределено И ОтборПоИзмерению.Использование И Не ЗначениеЗаполнено(
		ОтборПоИзмерению.Значение);
КонецФункции

#Область СпискиКодовКПП

// Возвращаемое значение:
//  Массив Из Строка
Функция КодыКПППодразделений()
	
	Результат = Новый Массив; // Массив из Строка
	Результат.Добавить("45");
	Результат.Добавить("31");
	Результат.Добавить("32");
...
// Возвращаемое значение:
//  Массив Из Строка
Функция КодыКППФилиалов()
	
	Результат = Новый Массив; // Массив из Строка
	Результат.Добавить("44");
	Результат.Добавить("04");
	Результат.Добавить("05");
...
// Возвращаемое значение:
//  Массив Из Строка
Функция КодыКПППредставительств()
	
	Результат = Новый Массив; // Массив из Строка
	Результат.Добавить("43");
	Результат.Добавить("02");
