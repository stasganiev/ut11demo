...
Функция ДанныеАбонентовИзСервисаНастроек(Отбор, КонтекстДиагностики, Отказ = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Абоненты) Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ОбработкаОшибок.ПодробноеПредставлениеОшибки(Ошибка));
			Отказ = Истина;
		КонецПопытки;
...
Функция РейтингУчетныхЗаписей(Знач ИдентификаторыОтправителя, Знач ИдентификаторыПолучателя) Экспорт
	...
	ОператорыПолучателя = СервисНастроекЭДО.НайтиОператоровЭДОПоИдентификаторам(ИдентификаторыПолучателя);
	
	Для Каждого Идентификатор Из ИдентификаторыПолучателя Цикл
		
		НоваяСтрока = ТаблицаИдентификаторыПолучателя.Добавить();
		НоваяСтрока.Идентификатор = Идентификатор;
		ОператорПолучателя = ОператорыПолучателя.Найти(Идентификатор, "ИдентификаторЭДО");
		Если ОператорПолучателя <> Неопределено Тогда
			НоваяСтрока.ОператорЭДО = ОператорПолучателя.ИдентификаторОператора;
		КонецЕсли;
...
Процедура ЗаписатьДанныеКонтрагентаДляПроверки(Объект, Отказ) Экспорт
	...
	ДанныеКонтрагента.Вставить("ОбластьДанныхВспомогательныеДанные", ЗначениеРазделителяСеанса());
...
Процедура ПроверитьКонтрагента(Параметры) Экспорт
	...
	КонтрагентДляПроверки.ОбластьДанныхВспомогательныеДанные = ЗначениеРазделителяСеанса();
...
// 
// Параметры:
//  ДанныеКонтрагентовДляПроверки - см.ТаблицаДанныхКонтрагентовДляПередачи
...
Функция ДанныеКонтрагентовТипизированныеИАктуализированные(ДанныеКонтрагентовДляПроверки)
	...
	Для каждого ДанныеКонтрагентаДляПроверки Из ДанныеКонтрагентовДляПроверки Цикл
		...
		СписокСвойств = "Контрагент, ИНН, КПП, Состояние, СостояниеПоАбонентам, СостояниеПоПриглашениям, Дата, ОбластьДанныхВспомогательныеДанные";
...
// Свернуть данные контрагентов.
// 
// Параметры:
//  ДанныеКонтрагентов - ТаблицаЗначений - Данные контрагентов:
// * Контрагент - ОпределяемыйТип.КонтрагентБЭД
// * ИНН - Строка
// * КПП - Строка
// * Дата - Дата - Дата получения состояния 
// * Состояние - ПеречислениеСсылка.СостоянияКонтрагентаБЭД
// * СостояниеПоАбонентам - ПеречислениеСсылка.СостоянияКонтрагентаБЭД
// * СостояниеПоПриглашениям - ПеречислениеСсылка.СостоянияКонтрагентаБЭД
// * ОбластьДанныхВспомогательныеДанные - Число - номер области данных
...
// 
// Параметры:
//  ДанныеКонтрагентов - см. ТаблицаДанныхКонтрагентовДляПередачи
...
Процедура ПроверитьСостояниеКонтрагентовРегламент()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ТекстПоляОбластиДанных = "СостоянияКонтрагентов.ОбластьДанныхВспомогательныеДанные";
	Иначе
		ТекстПоляОбластиДанных = "0";
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОбластьДанныхВспомогательныеДанные", ТекстПоляОбластиДанных);
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений - см. ТаблицаДанныхКонтрагентовДляПередачи
Функция ПолучитьСостоянияКонтрагентовИзКэша(ДанныеКонтрагентов)	
	...
	Запрос.Текст =	
		...
		|		И &УсловиеСоединенияОбластьДанныхВспомогательныеДанные
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ТекстУсловияВспомогательныхДанных = "ДанныеКонтрагентов.ОбластьДанныхВспомогательныеДанные = СостоянияКонтрагентовБЭД.ОбластьДанныхВспомогательныеДанные";
	Иначе
		ТекстУсловияВспомогательныхДанных = "ИСТИНА"
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеСоединенияОбластьДанныхВспомогательныеДанные", ТекстУсловияВспомогательныхДанных);
...
// Определяет значение разделителя с учетом наличия подсистемы РаботВМоделиСервиса
// 
// Возвращаемое значение:
//  Число - Значение разделителя сеанса
Функция ЗначениеРазделителяСеанса()
	Результат = 0;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
		Результат = МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса();
	КонецЕсли;
