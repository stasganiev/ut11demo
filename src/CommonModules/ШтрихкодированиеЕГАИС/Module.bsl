...
Процедура НормализоватьДанныеВложенныхШтрихкодов(ВложенныеШтрихкоды, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено, ТребуетсяЧастичноеВыбытие = Неопределено) Экспорт
	...
	Для Каждого СтрокаДанных Из ВложенныеШтрихкоды.ДеревоУпаковок.Строки Цикл
		СтрокаДанных.Количество = СтрокаДанных.КоличествоПоДаннымИБ;
	КонецЦикла;
	
	Для Каждого СтрокаДанных Из ВложенныеШтрихкоды.МаркированныеТовары Цикл
		СтрокаДанных.Количество = СтрокаДанных.КоличествоПоДаннымИБ;
	КонецЦикла;
...
Процедура ЗаполнитьСтрокуДереваУпаковок(СтрокаДерева, Выборка, Уровень, МаркированныеТовары, ДополнительныеПараметры)
	...
	Если СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
		...
		Если ЗначениеЗаполнено(Выборка.ЧастичноеВыбытиеВариантУчета) Тогда
			...
			СтрокаДерева.ЧастичноеВыбытиеКоличество     = Выборка.ЧастичноеВыбытиеКоличество;
			...
			КонецЕсли;
			СтрокаДерева.Количество      = Выборка.ЧастичноеВыбытиеКоличество;
...
Процедура ВыполнитьПроверкуНаОшибкиКонтрольАкцизныхМарок(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	Если ДанныеПоШтрихкодам.ВложенныеШтрихкоды = Неопределено Тогда
		...
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			Если СтрокаДанных.ЭтоШтрихкодНоменклатуры Тогда
				Продолжить;
			ИначеЕсли Не ПараметрыСканирования.ОбрабатыватьПивоВЕГАИС И СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
				Продолжить;
			КонецЕсли;
	...
	Иначе
		...
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			Если Не ПараметрыСканирования.ОбрабатыватьПивоВЕГАИС И СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьТаблицуДанныеКодовМаркировкиОписаниемНоменклатуры(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	|ВЫБРАТЬ
	|	Таблица.ИндексСтроки КАК ИндексСтроки,
	|	ЕСТЬNULL(ОписаниеНоменклатуры.ВариантЧастичногоВыбытия, НЕОПРЕДЕЛЕНО) КАК ВариантЧастичногоВыбытия,
	|	ЕСТЬNULL(ОписаниеНоменклатуры.ЕмкостьПотребительскойУпаковки, 0)      КАК ЕмкостьПотребительскойУпаковки,
	|	ЕСТЬNULL(ОписаниеНоменклатуры.КоличествоВПотребительскойУпаковке, 0)  КАК КоличествоВПотребительскойУпаковке,
	|	ЕСТЬNULL(НастройкиЧастичногоВыбытия.НоменклатураЧастичногоВыбытия, НЕОПРЕДЕЛЕНО)   КАК НоменклатураЧастичногоВыбытия,
	|	ЕСТЬNULL(НастройкиЧастичногоВыбытия.ХарактеристикаЧастичногоВыбытия, НЕОПРЕДЕЛЕНО) КАК ХарактеристикаЧастичногоВыбытия
	|ИЗ
	|	Таблица КАК Таблица
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиЧастичногоВыбытияПродукцииИС КАК НастройкиЧастичногоВыбытия
	|		ПО Таблица.Номенклатура = НастройкиЧастичногоВыбытия.Номенклатура
	|		И Таблица.Характеристика = НастройкиЧастичногоВыбытия.Характеристика";

	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаДанных = ДанныеКодовМаркировки[Выборка.ИндексСтроки];
		СтрокаДанных.Коэффициент = Выборка.КоличествоВПотребительскойУпаковке;
		Если ЧастичноеВыбытиеИспользуется Тогда
			СтрокаДанных.ЧастичноеВыбытиеВариантУчета   = Выборка.ВариантЧастичногоВыбытия;
			СтрокаДанных.ЕмкостьПотребительскойУпаковки = Выборка.ЕмкостьПотребительскойУпаковки;
			СтрокаДанных.ЧастичноеВыбытиеНоменклатура   = Выборка.НоменклатураЧастичногоВыбытия;
			СтрокаДанных.ЧастичноеВыбытиеХарактеристика = Выборка.ХарактеристикаЧастичногоВыбытия;
			СтрокаДанных.ЧастичноеВыбытие               = СтрокаДанных.ЧастичноеВыбытиеВыведено > 0;
			СтрокаДанных.ЧастичноеВыбытиеОстаток        = Макс(СтрокаДанных.ЕмкостьПотребительскойУпаковки - СтрокаДанных.ЧастичноеВыбытиеВыведено, 0);
		КонецЕсли;
	КонецЦикла;
