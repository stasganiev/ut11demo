...
Функция ТекстНадписиПоляИнтеграцииВФормеДокументаОснования(Знач ДокументОснование) Экспорт
	...
	ДокументыВЕТИС = ДокументыВЕТИСПоДокументуОснованию(ДокументОснование);
...
Функция ОшибкиСервисаПоДокументу(ДокументСсылка) Экспорт
	...
	Если ДанныеСообщенияСОшибкой <> Неопределено Тогда
		ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(ДанныеСообщенияСОшибкой.Сообщение);
...
Процедура ДополнитьОтборПоПрикладнымРеквизитам(Отбор, ИсточникДанных) Экспорт
	...
	Если ЗначениеЗаполнено(ИсточникДанных.Номенклатура) Тогда
		Продукция = ИнтеграцияВЕТИС.СопоставленнаяПродукция(ИсточникДанных.Номенклатура, ИсточникДанных.Характеристика, ИсточникДанных.Серия);
		Если ЗначениеЗаполнено(Продукция) Тогда
			Отбор.Вставить("Продукция", Продукция);
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИсточникДанных.Организация) Тогда
		ХозяйствующийСубъект = ХозяйствующиеСубъектыПоОрганизации(ИсточникДанных.Организация);
		Если ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
			Отбор.Вставить("ГрузоотправительХозяйствующийСубъект", ХозяйствующийСубъект);
			Если Не ИсточникДанных.Производство Тогда
				Отбор.Вставить("ГрузополучательХозяйствующийСубъект", ХозяйствующийСубъект);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// Коэффициент пересчета между единицами ВЕТИС.
// 
// Параметры:
//  Источник - СправочникСсылка.ЕдиницыИзмеренияВЕТИС - Источник
//  Приемник - СправочникСсылка.ЕдиницыИзмеренияВЕТИС - Приемник
// 
// Возвращаемое значение:
//  Число -  Коэффициент пересчета между единицами ВЕТИС
Функция КоэффициентПересчетаМеждуЕдиницамиВЕТИС(Знач Источник, Знач Приемник) Экспорт
	
	Массив = Новый Массив;
	Массив.Добавить(Источник);
	Массив.Добавить(Приемник);
	Реквизиты = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Массив, "Коэффициент");
	Если Не ЗначениеЗаполнено(Реквизиты[Источник]) Или Не ЗначениеЗаполнено(Реквизиты[Приемник]) Тогда
		Возврат 1;
	КонецЕсли;
	Возврат Реквизиты[Источник] / Реквизиты[Приемник];
...
Функция ОтправитьДляОбработки(ТелоЗапроса, Порт, АдресРесурса, SOAPAction, НастройкаОбмена)
	...
	ПараметрыЛогированияЗапросов = ЛогированиеЗапросовВЕТИС.ПараметрыЛогированияЗапросов();
	...
	ДанныеЗаписи.АдресРесурса    = URLЗапроса(HTTPЗапрос, Соединение, ДанныеЗаписи.HTTPМетод);
...
Функция URLЗапроса(HTTPЗапрос, Соединение, HTTPМетод)
...
Функция ХозяйствующиеСубъектыПоОрганизации(Организация) Экспорт
	...
	Возврат Справочники.ХозяйствующиеСубъектыВЕТИС.ХозяйствующиеСубъектыПоОрганизации(Организация);
...
Процедура ИзвлечьЛогЗапросовИзРезультатаОбмена(РезультатОбмена) Экспорт
	...
	ЛогированиеЗапросовИС.ДописатьВТекущийЛогДанныеИзФоновогоЗадания(
		ДанныеДляЗаписи,
		ЛогированиеЗапросовВЕТИС.ПараметрыЛогированияЗапросов());
...
Процедура ЗаписатьЛогЗапросовВоВременноеХранилищеВозвращаемогоЗначения(ПараметрыОбмена, Приемник) Экспорт
	...
	Если ОбщегоНазначенияИСВызовСервера.ЭтоФоновоеЗаданиеНаСервере() Тогда
		ПараметрыЛогирования = ЛогированиеЗапросовВЕТИС.ПараметрыЛогированияЗапросов();
...
// Параметры:
//  ДокументСсылка - ДокументСсылка - документ, по которому требуется отменить передачу данных.
//  ДанныеПоследнегоСообщения - Неопределено,
//                   СтрокаТаблицыЗначений с колонками:
//                     ФорматОбмена - ПеречислениеСсылка.ФорматыОбменаВЕТИС - формат.
//                     Операция - ПеречислениеСсылка.ВидыОперацийВЕТИС - операция.
//                     ХозяйствующийСубъект - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - ХозяйствующийСубъект.
//  ТекстОшибки - Строка.
