...
Функция НастройкиИнтеграции() Экспорт
	...
	Иначе
		...
		Если НастройкиХранилище <> Неопределено
			...
			Если НастройкиХранилище.Свойство("ВидимостьКнопки1СБухфон") Тогда //АПК:1416 хранилище может не содержать значение.
				НастройкиПользователя.ОтображатьКнопкуЗапуска = НастройкиХранилище.ВидимостьКнопки1СБухфон;
			ИначеЕсли НастройкиХранилище.Свойство("ВидимостьКнопкиВызовОнлайнПоддержки") Тогда //АПК:1416 хранилище может не содержать значение.
...
// Возвращаемое значение:
//  Структура:
//    * ДоступнаИнтеграцияСКоннект - Булево
//    * ИспользоватьКоннект - Неопределено - интеграции с коннект не доступна.
//                          - Булево
//
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы() Экспорт
	
	ДоступнаИнтеграцияСКоннект = (ДоступнаИнтеграцияСКоннект()
		И ПравоДоступа("Просмотр", Метаданные.ОбщиеФормы.НастройкаПодключенияКоннект));
		
	Результат = Новый Структура();
	Результат.Вставить("ДоступнаИнтеграцияСКоннект", ДоступнаИнтеграцияСКоннект);
	Результат.Вставить("ИспользоватьКоннект"       , Неопределено);
	
	Если Не ДоступнаИнтеграцияСКоннект Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ИспользоватьКоннект = Константы.ИспользоватьКоннект.Получить();
