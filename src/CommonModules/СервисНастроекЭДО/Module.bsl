...
Функция ОператорыЭлектронногоДокументооборота() Экспорт
	...
	Запрос.Текст = 
		...
		|	ОператорыЭДО.ПоддерживаемыеПриРегистрацииПредставителяТипыМЧД КАК ТипыМЧДПоддерживаемыеПриРегистрацииПредставителя
		|ИЗ
		|	РегистрСведений.ОператорыЭДО КАК ОператорыЭДО
		|ГДЕ
		|	ОператорыЭДО.ДоступноПодключениеЧерез1С
	...
	Результат.Колонки.Добавить("ПоддерживаемыеПриРегистрацииПредставителяТипыМЧД", Новый ОписаниеТипов("СписокЗначений"));
	
	СоответствиеТиповМЧД = МашиночитаемыеДоверенности.СоответствиеДанныхСервисаНастроекЭДОИТиповМЧД();
	
	Для Каждого ТекущаяСтрока Из Результат Цикл
		
		Если ПустаяСтрока(ТекущаяСтрока.ТипыМЧДПоддерживаемыеПриРегистрацииПредставителя) Тогда
			Продолжить;
		КонецЕсли;
		
		ПоддерживаемыеТипыМЧД = СтрРазделить(ТекущаяСтрока.ТипыМЧДПоддерживаемыеПриРегистрацииПредставителя, ";");
		
		Для Каждого ТекущийТип Из ПоддерживаемыеТипыМЧД Цикл
			ТипМЧД = СоответствиеТиповМЧД.Получить(ТекущийТип);
			Если ТипМЧД <> Неопределено Тогда
				ТекущаяСтрока.ПоддерживаемыеПриРегистрацииПредставителяТипыМЧД.Добавить(ТипМЧД, ТекущийТип);
			КонецЕсли;
		КонецЦикла;
		
	КонецЦикла;
	
	Результат.Колонки.Удалить("ТипыМЧДПоддерживаемыеПриРегистрацииПредставителя");
...
Функция СписокОператоровЭДО() Экспорт
	...
	СписокОператоровЭДО = Новый СписокЗначений;
	
	ЗапросОператоровЭДО = ЗапросОператоровЭДО();
...
//  Структура:
//	* ОператорЭДО - Строка
//	* МаксимальноеКоличествоЭДВПакете - Число
//	* МаксимальныйРазмерПакетаЭД - Число
Функция НовыеОграниченияОбъемаПакетаДокументов() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ОператорЭДО", "");
	Результат.Вставить("МаксимальноеКоличествоЭДВПакете", 0);
	Результат.Вставить("МаксимальныйРазмерПакетаЭД", 0);
	Возврат Результат;
	
КонецФункции

// Параметры:
//  ИдентифакторОператора - Строка
//
// Возвращаемое значение:
//	см. НовыеОграниченияОбъемаПакетаДокументов
Функция ОграниченияОбъемаПакетаДокументов(ИдентифакторОператора) Экспорт
	
	Результат = НовыеОграниченияОбъемаПакетаДокументов();
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаОграниченияОбъемаПакетаДокументов();
	Запрос.УстановитьПараметр("ИдентификаторОператора", ИдентифакторОператора);
...
Функция ОбработкаРезультатаКлассификаторПолномочийФНС_МЧД003(Знач ПолученныеДанные, Отказ = Ложь,
	...
	Полномочия.Колонки.Добавить("Родитель",          ТипСтрока150);
...
//
Функция ПолучитьКлассификаторПолномочийФНС_МЧД002(КонтекстДиагностики, ПолучатьНеобработанныеДанные = Ложь) Экспорт
	...
	ДанныеФайла = ДанныеФайлаОблачногоХранилищаНастроек(Путь, ПараметрыОперации.Наименование, КонтекстДиагностики);
	...
	Результат = ОбработкаРезультатаКлассификаторПолномочийФНС_МЧД002(ПолученныеДанные, Отказ, КонтекстДиагностики);
...
//
Функция ПолучитьКлассификаторПолномочийФНС_МЧД003(КонтекстДиагностики, ПолучатьНеобработанныеДанные = Ложь) Экспорт
	...
	Результат = ОбработкаРезультатаКлассификаторПолномочийФНС_МЧД003(ПолученныеДанные, Отказ, КонтекстДиагностики);
...
Функция ТипДокументаПоЗначениюСервиса(Значение) Экспорт
	...
	ТипыДокументов.Вставить("ДоговорнойДокумент", Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент);
	ТипыДокументов.Вставить("ДоговорныйДокумент", Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент);
	ТипыДокументов.Вставить("ДокументПредприятия", Перечисления.ТипыДокументовЭДО.ДокументПредприятия);
...
// См. СервисНастроекЭДОПовтИсп.НеподдерживаемыеОператорамиФорматы
Функция НеподдерживаемыеОператорамиФорматы() Экспорт
	
	Возврат СервисНастроекЭДОПовтИсп.НеподдерживаемыеОператорамиФорматы();
...
Функция ПараметрыПолученияДанныхФорматовЭДО(ПараметрыЗапроса)
	...
	ПараметрыКоманды.ФорматОтвета        = ФорматыОтветаСервиса().JSON;
	ПараметрыКоманды.Данные              = ОбщиеПараметрыКомандыСервиса(ПараметрыЗапроса);
...
Функция ОбщиеПараметрыКомандыСервиса(ПараметрыЗапроса)
	...
	Возврат ОбщегоНазначенияБЭД.JSONСтрока(СтруктураJSON);
...
// Параметры:
//  ПараметрыКоманды	- Структура - параметры команды.
//  ДанныеСервиса		- HTTPОтвет - возвращаемые данные сервиса.
//  Отказ				- Булево - результат проверки.
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
...
Функция ОбработатьДанныеОтветаСервиса(Знач ПараметрыКоманды, ДанныеСервиса, Отказ, КонтекстДиагностики)
	...
	Если ЗначениеЗаполнено(ПараметрыКоманды.ОбработкаРезультата) Тогда
		...
		ИначеЕсли ПараметрыКоманды.ОбработкаРезультата =
			"ПолучениеДанныхОрганизацийСОбязательнойОтправкойЗаявкиНаРоуминг" Тогда
			
			Результат = ОбработкаРезультатаПолученияДанныхОрганизацийСОбязательнойОтправкойЗаявкиНаРоуминг(
				ДанныеСервиса, Отказ);
...
Функция ОбъектИзJSON(ПараметрыКоманды, Знач ДвоичныеДанные, Отказ, КонтекстДиагностики = Неопределено,
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось прочитать ответ от сервиса по причине:'") + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		...
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(НаименованиеКоманды,
			ОбработкаНеисправностейБЭДКлиентСервер.ВидОшибкиНеизвестнаяОшибка(), ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
			ТекстСообщения);
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
	КонецПопытки;
...
Функция ОбработкаРезультатаПолученияДанныхАбонентов(Знач ПолученныеДанные, Отказ)
	...
	ИдентификаторыАбонентов = Новый Массив;
	Для Каждого Абонент Из ПолученныеДанные.identifierInfos Цикл
		ИдентификаторыАбонентов.Добавить(Абонент.id);
	КонецЦикла;
	ОператорыАбонентов = НайтиОператоровЭДОПоИдентификаторам(ИдентификаторыАбонентов);
	...
	Для Каждого Абонент Из ПолученныеДанные.identifierInfos Цикл
		...
		ОператорАбонента = ОператорыАбонентов.Найти(Абонент.id, "ИдентификаторЭДО");
		Если ОператорАбонента <> Неопределено Тогда
			НоваяСтрока.ОператорЭДО = ОператорАбонента.ИдентификаторОператора;
		КонецЕсли;
...
//    ** ОднородныйОтветНаВсеДокументыПакета - Булево
//    ** ПриРегистрацииПредставителяТребуетсяМЧД - Строка
//    ** ПоддерживаемыеПриРегистрацииПредставителяТипыМЧД - Строка
//    ** НеподдерживаемыеФорматы - Строка
//
Функция ОбработкаРезультатаКаталогОператоров(ПолученныеДанные, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ОператорЭДО Из ПолученныеДанные.operators Цикл
		...
		НоваяСтрока.НеподдерживаемыеФорматы = ОбщегоНазначения.ЗначениеВJSON(ОператорЭДО.unsupportedFormats);
		НоваяСтрока.МаксимальноеКоличествоЭДВПакете = ОператорЭДО.maximumQuantityDocumentsInPacket;
		НоваяСтрока.МаксимальныйРазмерПакетаЭД = ОператорЭДО.maximumPacketSize;
...
// Возвращаемое значение:
//  Строка
Функция ТекстЗапросаОграниченияОбъемаПакетаДокументов()
	
	Возврат
		"ВЫБРАТЬ
		|	ОператорыЭДО.МаксимальноеКоличествоЭДВПакете КАК МаксимальноеКоличествоЭДВПакете,
		|	ОператорыЭДО.МаксимальныйРазмерПакетаЭД КАК МаксимальныйРазмерПакетаЭД,
