...
Функция ИдентификаторНоменклатурыКонтрагентаПоНатуральнымКлючам(Знач НоменклатураКонтрагента, 
	...
	Возврат СтрШаблон("%1%2", Префикс, ОбщегоНазначения.КонтрольнаяСуммаСтрокой(Ключ));
...
Функция НайтиСоответствиеНоменклатуры(Знач Отбор = Неопределено, Знач ТолькоСопоставленные = Ложь) Экспорт
	...
	ДобавитьИндексы = (ДанныеНоменклатурыКонтрагентов.Количество() >= 10000);
	
	Запрос = ЗапросСоответствиеНоменклатуры(Отбор, ДобавитьИндексы, ДанныеНоменклатурыКонтрагентов);
	...
	Если ДобавитьИндексы Тогда
		ДанныеНоменклатурыКонтрагентов.Индексы.Добавить("Владелец, Идентификатор, ДополнительныйИдентификатор");
	КонецЕсли;
	Пока Выборка.Следующий() Цикл
		...
		Соответствие    = СопоставлениеНоменклатурыКонтрагентовСлужебный.НовоеСоответствиеНоменклатуры(ДанныеБазы,
			НоменклатураИБ);
		ПараметрыОтбора = Новый Структура("Владелец, Идентификатор, ДополнительныйИдентификатор", 
			Выборка.Владелец, Выборка.Идентификатор, "");
		
		НайденоПоИдентификаторуДокумента = ПустаяСтрока(Выборка.ОсновнойИдентификатор);
		...
		Если Не НайденоПоИдентификаторуДокумента Тогда
			ПараметрыОтбора.ДополнительныйИдентификатор = ПараметрыОтбора.Идентификатор;
			ПараметрыОтбора.Идентификатор = Выборка.ОсновнойИдентификатор;
			ДанныеБазы.Идентификатор = Выборка.ОсновнойИдентификатор;
		КонецЕсли;
...
#Область РегистрыСведений_НоменклатураКонтрагентовБЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|* [1.8.1] заполняются идентификаторы номенклатуры, характеристики и упаковки, создается элемент справочника НоменклатураКонтрагентов в случае отсутствия;
	|* [1.9.11] заполняются хеши натуральных ключей.'");
...
Функция ТекстЗапросаСозданиеВременнойТаблицыНоменклатурыИБ()
...
Функция ТекстЗапросаСозданиеВременнойТаблицыНоменклатурыКонтрагентов(ДобавитьИндексы)
...
Функция ТекстЗапросаКРегистру()
	...
	|	"""" КАК ОсновнойИдентификатор
	|ИЗ
	|	РегистрСведений.НоменклатураКонтрагентовБЭД КАК Сопоставление";
	
КонецФункции

Функция ЗапросСоответствиеНоменклатуры(Отбор, ДобавитьИндексы, ДанныеНоменклатурыКонтрагентов)
	...
	Если Отбор.Свойство("НоменклатураИБ") Тогда
		ТекстыПакетаЗапросов.Добавить(ТекстЗапросаСозданиеВременнойТаблицыНоменклатурыИБ());
		ТекстыУсловий.Добавить("(Сопоставление.Номенклатура, Сопоставление.Характеристика, Сопоставление.Упаковка) В
			|(ВЫБРАТЬ вт.Номенклатура, вт.Характеристика, вт.Упаковка ИЗ втТаблицаНоменклатурыИБ КАК вт)");
		ТаблицаНоменклатурыИБ = НоваяТаблицаНоменклатурыИнформационнойБазы(Отбор.НоменклатураИБ);
		Запрос.УстановитьПараметр("ТаблицаНоменклатурыИБ", ТаблицаНоменклатурыИБ);
	КонецЕсли;
	
	Если Отбор.Свойство("Владелец") Тогда
		ТекстыУсловий.Добавить("Сопоставление.Владелец = &Владелец");
		Запрос.УстановитьПараметр("Владелец", Отбор.Владелец);
	КонецЕсли;
	
	ТекстыОсновногоЗапроса = Новый Массив();
	ТекстыОсновногоЗапроса.Добавить(ТекстЗапросаКРегистру());
	ЕстьТекстыУсловий = ТекстыУсловий.Количество();
	Если ЕстьТекстыУсловий Тогда
		ТекстыОсновногоЗапроса.Добавить("ГДЕ");
		ТекстыОсновногоЗапроса.Добавить(СтрСоединить(ТекстыУсловий, Символы.ПС + "И "));
	КонецЕсли;
	
	Если Отбор.Свойство("НоменклатураКонтрагента") Тогда
		
		Если ЕстьТекстыУсловий Тогда
			ТекстыОсновногоЗапроса.Добавить("И ");
		Иначе
			ТекстыОсновногоЗапроса.Добавить("ГДЕ");
		КонецЕсли;
		ТекстыОсновногоЗапроса.Добавить("	(Сопоставление.Владелец, Сопоставление.Идентификатор) В
			|		(ВЫБРАТЬ вт.Владелец, вт.Идентификатор ИЗ втТаблицаНоменклатурыКонтрагентов КАК вт)");
		
		
		ТекстыОсновногоЗапроса.Добавить("ОБЪЕДИНИТЬ ВСЕ");
		
		ВторойЗапрос = ТекстЗапросаКРегистру();
		ВторойЗапрос = СтрЗаменить(ВторойЗапрос, "	"""" КАК ОсновнойИдентификатор", "	Идентификаторы.Идентификатор");
		ТекстыОсновногоЗапроса.Добавить(ВторойЗапрос);
		ТекстыОсновногоЗапроса.Добавить("ВНУТРЕННЕЕ СОЕДИНЕНИЕ втТаблицаНоменклатурыКонтрагентов КАК Идентификаторы
			|		ПО Сопоставление.Владелец = Идентификаторы.Владелец
			|			И Сопоставление.Идентификатор = Идентификаторы.ДополнительныйИдентификатор");
		Если ЕстьТекстыУсловий Тогда
			ТекстыОсновногоЗапроса.Добавить("ГДЕ");
			ТекстыОсновногоЗапроса.Добавить(СтрСоединить(ТекстыУсловий, Символы.ПС + "И "));
		КонецЕсли;
		
		ТекстыПакетаЗапросов.Добавить(ТекстЗапросаСозданиеВременнойТаблицыНоменклатурыКонтрагентов(ДобавитьИндексы));
		Запрос.УстановитьПараметр("ТаблицаНоменклатурыКонтрагентов", ДанныеНоменклатурыКонтрагентов);
		
	КонецЕсли;
	
	ТекстыПакетаЗапросов.Добавить(СтрСоединить(ТекстыОсновногоЗапроса, Символы.ПС));
	
	Запрос.Текст = СтрСоединить(ТекстыПакетаЗапросов, Символы.ПС + ";" + Символы.ПС);
...
Функция СоответствиеАктуально(Соответствие, ДанныеДокумента)
	...
	СоставКлюча = НатуральныеКлючиНоменклатурыКонтрагента();
	ВсегоПолей = СоставКлюча.Количество();
	Для ОбратныйИндекс = 1 По ВсегоПолей Цикл
		ТекущееПоле = СоставКлюча[ВсегоПолей - ОбратныйИндекс];
		Если Не ЗначениеЗаполнено(ДанныеБазы[ТекущееПоле]) Тогда
			СоставКлюча.Удалить(ВсегоПолей - ОбратныйИндекс);
		КонецЕсли;
	КонецЦикла;
	
	КлючПоБазе      = ИдентификаторНоменклатурыКонтрагентаПоНатуральнымКлючам(ДанныеБазы, СоставКлюча);
	КлючПоДокументу = ИдентификаторНоменклатурыКонтрагентаПоНатуральнымКлючам(ДанныеДокумента, СоставКлюча);
