...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоПиво Тогда
		...
		МассивНепроверяемыхРеквизитов.Добавить("СрокРеализации");
	КонецЕсли;
...
Процедура ПолучитьКодФИАС(Команда)
	...
	ИнтерфейсИСМПОбщегоНазначенияКлиент.МестаОсуществленияДеятельности(ПараметрыЗапросаДанных);
...
Процедура НастроитьЭлементыФормы()
	...
	Если ЗначениеЗаполнено(ВидПродукции) Тогда
		Элементы.Остаток.РасширеннаяПодсказка.Заголовок = ИнтеграцияИСКлиентСервер.ПредставлениеЕдиницыЧастичногоВыбытия(
			ВидПродукции);
		Элементы.ОбъемСлива.РасширеннаяПодсказка.Заголовок = ИнтеграцияИСКлиентСервер.ПредставлениеЕдиницыЧастичногоВыбытия(
			ВидПродукции);
	Иначе
		Элементы.Остаток.РасширеннаяПодсказка.Заголовок = ИнтеграцияИСКлиентСервер.ПредставлениеЕдиницыЧастичногоВыбытия(
...
Процедура ПриИзмененииВидаПродукции()
	...
	Если ИмяПодсистемы = "ИСМП" И ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
