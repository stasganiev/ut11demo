...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не УказатьСрокГодности
		Или ИнтеграцияИСМПКлиентСервер.ЭтоВидПродукцииСоСрокамиГодностиБезОбязательногоЗаполнения(ВидПродукции) Тогда
	...
	Если ЭтоЧастичноеВыбытие(ЭтотОбъект) Тогда
		...
		ИначеЕсли ЧастичноеВыбытиеКоличество = ЕмкостьПотребительскойУпаковки
			И ВидПродукции <> Перечисления.ВидыПродукцииИС.Пиво Тогда
			ЕстьОшибкаКоличества = Истина;
		КонецЕсли;
...
Процедура ОбновитьВидимостьДоступностьЭлементов()
	...
	Если УказатьСрокГодности Тогда
		...
		Если ИнтеграцияИСМПКлиентСервер.ЭтоВидПродукцииСоСрокамиГодностиБезОбязательногоЗаполнения(ВидПродукции) Тогда
...
Процедура НастроитьЭлементыФормыЧастичногоВыбытия(Форма)
	...
	Элементы.РежимВыбытия.ТолькоПросмотр = Форма.ЧастичноеВыбытиеВыведено > 0 Или Форма.КоличествоВскрытыхПотребительскихУпаковок > 0 Или ЭтоВыборКодаМаркировкиВскрытойПотребительскойУпаковки(Форма);
	...
	Если ЭтоЧастичноеВыбытие Тогда
		...
		Если Форма.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво") Тогда
