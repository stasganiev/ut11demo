...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ВариантНастройки = "ОбменУПРМК" И Не ЗначениеЗаполнено(ЭтотОбъект.КассаККМ) Тогда
		
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Необходимо заполнить кассу ККМ.'"), ,
			"КассаККМ");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОткрытьСписокОтправляемыхЭквайринговыхТерминалов(Команда)
	...
	Если Объект.Организации.Количество() > 0 Тогда
	     НакладываемыеФильтры.Вставить("ЗначениеПараметра", Объект.Организации[0].Организация);
	Иначе	
...
#Область ГруппаЭквайринговыйТерминалПоУмолчанию
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		(Объект.ВариантНастройки = "ОбменУПРМК" ИЛИ Объект.ВариантНастройки = "ОбменМК"  ИЛИ Объект.ВариантНастройки = "ОбменУПБП" ИЛИ Объект.ВариантНастройки = "ОбменУниверсальный"));
#КонецОбласти  
...
Процедура ОбновитьЗависимыеОтборы()
	...
	Если Объект.Организации.Количество() > 0 Тогда
		Организация = Объект.Организации[0].Организация;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(КассаККМ) Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(КассаККМ, КлючиРеквизитов);
