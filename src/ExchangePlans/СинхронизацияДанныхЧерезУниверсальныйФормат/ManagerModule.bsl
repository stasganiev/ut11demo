...
Процедура АктуализацияРеквизитовУзлаПланаОбмена(УзелПланаОбмена) Экспорт
	...
	Если ВариантНастройки = "ОбменУПРМК" И ИспользоватьОтборПоКассамККМ И КоличествоВыбранныхКассККМ <> 0 Тогда
		...
		КлючиРеквизитов = Новый Структура(
			"Владелец, Склад, СкладРозничныйВидЦены",
			"Владелец", "Склад", "Склад.РозничныйВидЦены");
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УзелПланаОбмена.КассыККМ[0].КассаККМ, КлючиРеквизитов);
		
		УзелПланаОбмена.Организации.Очистить();
		НоваяСтрока = УзелПланаОбмена.Организации.Добавить();
		НоваяСтрока.Организация = ЗначенияРеквизитов.Владелец;
		УзелПланаОбмена.ИспользоватьОтборПоОрганизациям = Истина;
		
		УзелПланаОбмена.Склады.Очистить();
		НоваяСтрока = УзелПланаОбмена.Склады.Добавить();
		НоваяСтрока.Склад = ЗначенияРеквизитов.Склад;
		УзелПланаОбмена.ИспользоватьОтборПоСкладам = Истина;
		
		УзелПланаОбмена.ВидыЦенНоменклатуры.Очистить();
		НоваяСтрока = УзелПланаОбмена.ВидыЦенНоменклатуры.Добавить();
		НоваяСтрока.ВидЦенНоменклатуры = ЗначенияРеквизитов.СкладРозничныйВидЦены;
		УзелПланаОбмена.ВыгружатьЦеныНоменклатуры = Истина;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.16.19";  
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5331c173-9096-11eb-a3f8-4cedfb67472c"); 
	Обработчик.Комментарий = НСтр("ru = 'Заполняет отбор по документам в плане обмена через универсальный формат, если ранее был установлен фильтр по разделам учета.'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры = Неопределено) Экспорт
	...
	|	И НЕ СинхронизацияДанныхЧерезУниверсальныйФорматРазделыУчета.Ссылка.ЭтотУзел
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
