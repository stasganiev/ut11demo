...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.Ссылка.ЭтотУзел <> Истина Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения = Неопределено Тогда
		...
		Если ОбменДаннымиПовтИспУТУП.ЭтоПодчиненныйУзелРИБСОтбором() Тогда
			...
			ИспользоватьОтборПоВидамЦен = НастройкиЦентальногоУзлаРИБ.ИспользоватьОтборПоВидамЦен;
			ИспользоватьОтборПоОрганизациям = НастройкиЦентальногоУзлаРИБ.ИспользоватьОтборПоОрганизациям;
			ВыгружатьИнформативныеОстатки = НастройкиЦентальногоУзлаРИБ.ВыгружатьИнформативныеОстатки;
			
			Подразделения.Загрузить(НастройкиЦентальногоУзлаРИБ.Подразделения.Выгрузить());
			Организации.Загрузить(НастройкиЦентальногоУзлаРИБ.Организации.Выгрузить());
			ВидыЦен.Загрузить(НастройкиЦентальногоУзлаРИБ.ВидыЦен.Выгрузить());
			ИнформативныеОстаткиПоСкладам.Загрузить(НастройкиЦентальногоУзлаРИБ.ИнформативныеОстаткиПоСкладам.Выгрузить());
...
Функция ПолучитьРеглЗаданияЭтойИБ(ЭлементДанных, ИмяРеквизита)
	...
	ИдентификаторСуществующегоРегламентногоЗадания(РеглЗаданияЭтойИБ);
...
Процедура ПереопределитьНаборЗаписей(ЭлементДанных, УзелПланаОбмена)
	...
	Пока КоличествоЗаписейВНаборе >= 0 Цикл
		...
		Если ТипЗнч(ЭлементДанных) = Тип("РегистрНакопленияНаборЗаписей.ПартииТоваровОрганизаций") 
			И ЭтоПодчиненныйУзел
			И Не ЗаписьНабора.Первичное Тогда
			ЭлементДанных.Удалить(ЗаписьНабора);
		ИначеЕсли ОбменДаннымиУТУП.ОбъектНеПроходитФильтр(ЗаписьНабора, УзелПланаОбмена) Тогда
			ЭлементДанных.Удалить(ЗаписьНабора);
		КонецЕсли;
