...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ВключатьПланПоставок = Ложь;
	ВключатьПланОплат = Ложь;
	Для Каждого ВыбранноеПоле Из МассивВыбранныхПолей Цикл 
		Если Строка(ВыбранноеПоле.Поле) = "КПоступлению" Тогда
			ВключатьПланПоставок = Истина;
		КонецЕсли;
		Если Строка(ВыбранноеПоле.Поле) = "НашДолг"
			Или Строка(ВыбранноеПоле.Поле) = "НашДолгПросрочено" Тогда
			ВключатьПланОплат = Истина;
		КонецЕсли;
	КонецЦикла;
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(НастройкиОтчета, "ВключатьПланПоставок", ВключатьПланПоставок);
...
Функция ТекстЗапроса()
	...
	|				КОНЕЦ
	|		ИНАЧЕ 0 
	|	КОНЕЦ                                                          КАК ДолгПоставщикаНеКонтролируется,
	|	
	|	0                                                              КАК КПоступлению
	...
	|	0                                        КАК ДолгПоставщикаНеКонтролируется,
	|	
	|	0                                        КАК КПоступлению
	|ИЗ
	...
	|	ТаблицаЗадолженностей.ДолгПоставщикаНеКонтролируется           КАК ДолгПоставщикаНеКонтролируется,
	...
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ТаблицаЗадолженностей.ДолгПоставщика = 0 ИЛИ ТаблицаЗадолженностей.ДатаПлановогоПогашения = ДАТАВРЕМЯ(1, 1, 1)
	|					ТОГДА 0
	|				ИНАЧЕ ВЫБОР
