								...
								Пока ГруппировкаПоПризнакуСторно.Следующий() Цикл
									...
									Пока СчетаФактурыЗаписи.Следующий() Цикл
										Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, СчетаФактурыЗаписи) Тогда
											...
											Если КоличествоСгруппированных > 1 
												...
												Иначе
													Если НомерСтрокиВГруппировке < КоличествоСгруппированных Тогда
														...
														ИмяОбластиЧасть1 = "ОбластьЗаписьОсновнаяЧасть1Многострочная2";
													Иначе
														// Последняя строка
														...
														Иначе
															...
															ИмяОбластиЧасть1 = "ОбластьЗаписьОсновнаяЧасть1Многострочная3";
														КонецЕсли;
													КонецЕсли;
												КонецЕсли;
											...
											ВывестиГраницуСтроки(СтруктураПараметров, Часть1Строка, ИмяОбластиЧасть1, НомерСтрокиВГруппировке = КоличествоСгруппированных);
											...
											ЗаполнитьПодстрочникРНПТ(
												...
												ТабличныйДокумент,
												"ОбластьЗаписьЧасть1",
												НомерСтрокиВГруппировке = КоличествоСгруппированных);
											...
											Если ВыводитьОстатокПоСчетуФактуре Тогда 
												...
												ВывестиГраницуСтроки(СтруктураПараметров, СтрокаОстаток, "Часть1Строка_Многострочная3", НомерСтрокиВГруппировке = КоличествоСгруппированных);
									...
									Если СтруктураПараметров.ЗаполнениеДекларации Тогда
										...
										Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
										 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1" Тогда
											СтруктураСвПосрДеят.Вставить("СвРегНом", Новый Массив());
										КонецЕсли;
...
Функция ПолучитьЗаписиЖурналаСчетовФактур(СтруктураПараметров) Экспорт
	...
	Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов()
		+ УчетНДСПереопределяемый.ТекстЗапросаПодготовкаСведенийРНПТЖурнал();
	...
	|	ЖурналУчетаСчетовФактур.СчетФактураКомитентаПродавца КАК СчетФактураКомитентаПродавца,
	...
	|	ЖурналУчетаСчетовФактур.Период,
	|	ЖурналУчетаСчетовФактур.СчетФактураПолученныйОтПродавца,
	|	ЖурналУчетаСчетовФактур.СчетФактураКомитентаПродавца,
	...
	|	ЗаписиРегистраЖурналУчетаСчетовФактур.СчетФактураКомитентаПродавца КАК СчетФактураКомитентаПродавца,
...
Процедура ЗаполнитьПодстрочникРНПТ(ЗаписьОтчета, ПараметрыСтрокиРНПТ, СтруктураПараметров, СекцияСтрокаРНПТ, ТабличныйДокумент, ИмяОбласти, ПоследняяСтрокаГруппировки = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.МассивСвРегНом)
		...
		Для ЭлементСвРегНом = 1 По СтруктураПараметров.МассивСвРегНом.Количество() - 1 Цикл
			...
			Если ЭлементСвРегНом = СтруктураПараметров.МассивСвРегНом.Количество() - 1
			   И ПоследняяСтрокаГруппировки Тогда
			...
			Иначе
				ОбластьТабличногоДокумента = ТабличныйДокумент.Вывести(СекцияСтрокаРНПТ);
...
Процедура ВывестиГраницуСтроки(СтруктураПараметров, СекцияСтрока, ИмяОбласти, ПоследняяСтрокаГруппировки = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтруктураПараметров.МассивСвРегНом)
		Или (СтруктураПараметров.МассивСвРегНом.Количество() = 1 И ПоследняяСтрокаГруппировки) Тогда
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(
	ПараметрыСтроки, 
	ЗаписьЖурналаУчетаСчетовФактур, 
	СтруктураПараметров, 
	ДополнительныеСвойства = Неопределено, 
	НомерПП = 1, 
	ЕстьПодстроки = Ложь) Экспорт
	...
	Если (СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1")
		...
		Если СтрокаСведенийОРНПТ <> Неопределено
			...
			Если ЗаписьЖурналаУчетаСчетовФактур.ЧастьЖурнала = Перечисления.ЧастиЖурналаУчетаСчетовФактур.ВыставленныеСчетаФактуры
				И ЗначениеЗаполнено(ЗаписьЖурналаУчетаСчетовФактур.СчетФактураКомитентаПродавца) Тогда
				СтрокаСведенийОРНПТСчетаФактурыКомитента = СтруктураПараметров.СведенияОРНПТ.Найти(ЗаписьЖурналаУчетаСчетовФактур.СчетФактураКомитентаПродавца, "Ссылка");
				// Возьмем только те РНПТ, которые были в выставленном счете-фактуре, т.к. полученный счет-фактура может быть сводным.
				Если СтрокаСведенийОРНПТСчетаФактурыКомитента <> Неопределено
					И ЗначениеЗаполнено(СтрокаСведенийОРНПТСчетаФактурыКомитента.СвРегНом) Тогда
					СведенияРНПТВыставленногоСчетаФактуры = МассивСтруктурВТаблицуЗначений(СтрокаСведенийОРНПТ.СвРегНом);
					Для Каждого СтрокаСРегНомером ИЗ СтрокаСведенийОРНПТСчетаФактурыКомитента.СвРегНом Цикл
						Если СведенияРНПТВыставленногоСчетаФактуры.Найти(СтрокаСРегНомером.РегНомПросл, "РегНомПросл") <> Неопределено Тогда
							МассивСвРегНом.Добавить(СтрокаСРегНомером);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			Иначе
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента 
		...
		Если ЗаписьЖурналаУчетаСчетовФактур.ЧастьЖурнала = Перечисления.ЧастиЖурналаУчетаСчетовФактур.ВыставленныеСчетаФактуры Тогда
			...
			УмСт = ПреобразоватьСтрокуВФорматДекларации(СоставСтроки.СуммаДокументаРазницаУменьшение, ТекстБезНДС, ТекстПустаяСтрока);
			Если УмСт <> 0 Тогда 
				РазнСтоимКСчФ.Вставить("УмСт", УмСт);
			КонецЕсли;
			УвелСт = ПреобразоватьСтрокуВФорматДекларации(СоставСтроки.СуммаДокументаРазницаУвеличение, ТекстБезНДС, ТекстПустаяСтрока);
			Если УвелСт <> 0 Тогда 
				РазнСтоимКСчФ.Вставить("УвелСт", УвелСт);
			КонецЕсли;
			
			Если РазнСтоимКСчФ.Свойство("УмСт")
				ИЛИ РазнСтоимКСчФ.Свойство("УвелСт") Тогда 
				СоставСтроки.Вставить("РазнСтоимКСчФ", РазнСтоимКСчФ);
			КонецЕсли;
			
			РазнНДСКСчФ = Новый Структура();
			УмНДС = ПреобразоватьСтрокуВФорматДекларации(СоставСтроки.СуммаНДСРазницаУменьшение, ТекстБезНДС, ТекстПустаяСтрока);
			Если УмНДС <> 0 Тогда 
				РазнНДСКСчФ.Вставить("УмНДС", УмНДС);
			КонецЕсли;
			УвелНДС = ПреобразоватьСтрокуВФорматДекларации(СоставСтроки.СуммаНДСРазницаУвеличение, ТекстБезНДС, ТекстПустаяСтрока);
			Если УвелНДС <> 0 Тогда 
				РазнНДСКСчФ.Вставить("УвелНДС", УвелНДС);
			КонецЕсли;
			
			Если РазнНДСКСчФ.Свойство("УмНДС")
				ИЛИ РазнНДСКСчФ.Свойство("УвелНДС") Тогда 
...
Функция МассивСтруктурВТаблицуЗначений(МассивСтруктур)
	...
	Если Не ЗначениеЗаполнено(МассивСтруктур)
		ИЛИ МассивСтруктур.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Для Каждого ЭлементКоллекции ИЗ МассивСтруктур[0] Цикл
		Результат.Колонки.Добавить(ЭлементКоллекции.Ключ);
	КонецЦикла;
...
Функция ТекстЗапросаЖурналСчетовФактур981()
	...
	|	ЕСТЬNULL(ВТ_СчетаФактурыОтКомитента.СчетФактура, НЕОПРЕДЕЛЕНО) КАК СчетФактураКомитентаПродавца,
	|	ЖурналУчетаСчетовФактурСрезПоследних.Сторно КАК Сторно,
...
Функция ТекстЗапросаЖурналСчетовФактур1137()
	...
	|	ЕСТЬNULL(ВТ_СчетаФактурыОтКомитента.СчетФактура, НЕОПРЕДЕЛЕНО),
