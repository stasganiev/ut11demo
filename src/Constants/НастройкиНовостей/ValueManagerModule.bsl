...
Процедура ПриЗаписи(Отказ)
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Если ТипЗнч(ПараметрыОкруженияБИП_Новости) = ТипСтруктура Тогда
			ПараметрыОкруженияБИП_НовостиЭталон = Новый Структура;
				ПараметрыОкруженияБИП_НовостиЭталон.Вставить("СостояниеПриНачалеРаботыСистемы", Ложь);
			ЗаполнитьЗначенияСвойств(ПараметрыОкруженияБИП_НовостиЭталон, ПараметрыОкруженияБИП_Новости);
			Если ПараметрыОкруженияБИП_НовостиЭталон.СостояниеПриНачалеРаботыСистемы = Истина Тогда
	...
	Если ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации() Тогда
		...
		ПредыдущееЗначениеСтрока = "Неопределено";
		ДополнительныеСвойстваЭталон = Новый Структура;
			ДополнительныеСвойстваЭталон.Вставить("ЗначениеПередЗаписью", Неопределено);
		ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ДополнительныеСвойства);
		Если ТипЗнч(ДополнительныеСвойстваЭталон.ЗначениеПередЗаписью) = ТипХранилищеЗначения Тогда
			ПредыдущееЗначениеСтруктура = ДополнительныеСвойстваЭталон.ЗначениеПередЗаписью.Получить();
			Если ТипЗнч(ПредыдущееЗначениеСтруктура) = ТипСтруктура Тогда
				ПредыдущееЗначениеСтрока = ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(
					ПредыдущееЗначениеСтруктура,
					": ",
					Символы.ПС);
			КонецЕсли;
		КонецЕсли;

		НовоеЗначениеСтрока = "Неопределено";
		Если ТипЗнч(Значение) = ТипХранилищеЗначения Тогда
			Если ТипЗнч(Значение.Получить()) = ТипСтруктура Тогда
				НовоеЗначениеСтрока = ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(
					Значение.Получить(),
					": ",
					Символы.ПС);
			КонецЕсли;
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредыдущееЗначение%", ПредыдущееЗначениеСтрока);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НовоеЗначение%"     , НовоеЗначениеСтрока);
		// Запись в журнал регистрации
