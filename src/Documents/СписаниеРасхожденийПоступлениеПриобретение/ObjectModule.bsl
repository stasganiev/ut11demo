...
Функция ВременныеТаблицыДанныхДокумента(ВидРасхождений = Неопределено, ДляТоваровУПартнеров = Ложь, ТолькоИмпортныеТовары = Ложь)
	Запрос = Новый Запрос;
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	&Дата КАК Дата,
	...
	|ГДЕ
	|	ВЫРАЗИТЬ(ТаблицаТоваров.Номенклатура КАК
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		МенеджерВременныхТаблицНедостачи = ВременныеТаблицыДанныхДокумента(ВидРасхожденийНедопоставка, Ложь, Истина);
		...
		Если Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки Тогда 
			...
			ПараметрыЗаполнения = ПараметрыЗаполненияВидовЗапасов(Истина, Истина);
...
Функция ПараметрыЗаполненияВидовЗапасов(ПоПоложительномуОстатку, ПоСобственномуТипуЗапасов)
	ПараметрыЗаполнения = ЗапасыСервер.ПараметрыЗаполненияВидовЗапасов();
	
	ПараметрыЗаполнения.ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию = Истина;
	ПараметрыЗаполнения.ТаблицаРеквизитовВидовЗапасовПоУмолчанию = ТаблицаРеквизитовВидовЗапасовПоУмолчанию();
	ПараметрыЗаполнения.ИмяТаблицыОстатков = ?(ПоПоложительномуОстатку, "СписаниеРасхожденийПоложительныйОстаток", "СписаниеРасхожденийОтрицательныйОстаток");
	ПараметрыЗаполнения.БезОтбораПоНомерамГТД = Истина;
	ПараметрыЗаполнения.ОтборыВидовЗапасов.ТипЗапасов.Очистить();
	Если ПоСобственномуТипуЗапасов Тогда
