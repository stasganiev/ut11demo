...
Процедура ОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС") Тогда
		...
		ПриВводеИВыбореВСД(НовыйОбъект, Неопределено);
	КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	УстановитьПризнакИсключенияПродукцииТретьегоУровня(ЭтотОбъект, Объект.Дата);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	НоменклатураПриИзменении(Истина);
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыНоменклатуры = ИнтеграцияВЕТИСВызовСервера.ПараметрыСозданияНоменклатуры(
		ТекущиеДанные.Продукция, ТекущиеДанные.ЕдиницаИзмеренияВЕТИС);
...
Процедура ТоварыНадписьУпаковкиШтрихкодыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияУпаковокШтрихкодовПоСтроке(ЭтотОбъект,
		Объект,
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ЗаполнитьПредставленияТабличныхЧастейПоСтроке(ЭтотОбъект, ТекущиеДанные);
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ОчиститьДанныеТабличныхЧастейПоСтроке(ЭтотОбъект, ТекущиеДанные);
...
Процедура ТоварыСерияПриИзменении(Элемент)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Серия) Тогда
			...
			ЗаполнитьПредставлениеТабличнойЧастиПоСтроке(ЭтотОбъект, "ПроизводственныеПартии", ТекущиеДанные);
...
Процедура ТоварыЗаписьСкладскогоЖурналаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИнтеграцияВЕТИСКлиент.ЭтоВыборЗаписиЖурнала(ВыбранноеЗначение) Тогда
		...
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		Если ТекущиеДанные.Операция <> ПредопределенноеЗначение("Перечисление.ОперацииИнвентаризацииПродукцииВЕТИС.Изменение")
				Или ТекущиеДанные.КоличествоИзменение >= 0 Тогда
		...
		ВыбранноеЗначение.Суффикс = "Изменение";
		Если ТекущиеДанные.КоличествоИзменениеВЕТИС * КоэффициентПересчета > ВыбранноеЗначение.КоличествоВЕТИС Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьВыборЗаписиЖурнала", ЭтотОбъект, ТекущиеДанные);
...
&НаКлиенте
Процедура ТоварыВетеринарноСопроводительныйДокументСоздание(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ОткрытьФормуРегистрацииВСД();
...
Процедура ТоварыВетеринарноСопроводительныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Продукция) Тогда
		ТекстСообщения = НСтр("ru = 'Укажите продукцию'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ТекстСообщения,,
			"Объект.Товары" + ТекущиеДанные.ПолучитьИдентификатор()+".Продукция");
		Возврат;
	КонецЕсли;
	
	Отбор = Новый Структура;
	Отбор.Вставить("ГрузополучательХозяйствующийСубъект", Объект.ХозяйствующийСубъект);
	Отбор.Вставить("ГрузополучательПредприятие", Объект.Предприятие);
	Отбор.Вставить("Продукция", ТекущиеДанные.Продукция);
	
	ПараметрыФормыПодбора = Новый Структура;
	ПараметрыФормыПодбора.Вставить("МножественныйВыбор", Ложь);
	ПараметрыФормыПодбора.Вставить("ДоступныВсеВСД");
	ПараметрыФормыПодбора.Вставить("ПодобранныеВДокументСтроки", Новый Массив);
	ПараметрыФормыПодбора.Вставить("Отбор", Отбор);
	
	Оповещение = Новый ОписаниеОповещения("ПриВводеИВыбореВСД", ЭтотОбъект);
	
	ОткрытьФорму("Справочник.ВетеринарноСопроводительныйДокументВЕТИС.Форма.ПодборВСД",
...
Процедура ЗаписатьДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИнвентаризацияПродукцииВЕТИС.Форма.ФормаДокумента.Записать");
...
Процедура ПровестиДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИнвентаризацияПродукцииВЕТИС.Форма.ФормаДокумента.Провести");
...
Процедура ПровестиИЗакрыть(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИнвентаризацияПродукцииВЕТИС.Форма.ФормаДокумента.ПровестиИЗакрыть");
...
Процедура СгенерироватьСерии(Команда)
	...
	Если СтрокиДляГенерацииСерий.Количество() Тогда
		...
		ИнтеграцияИСКлиент.ОповеститьОбОкончанииЗаполненияСерийВДокументе(
...
Процедура РазбитьСтроку(Команда)
	...
	ПараметрыРазбиенияСтроки = СобытияФормИСКлиент.ПараметрыРазбиенияСтроки(ТекущиеДанные.КоличествоИзменение);
	ПараметрыРазбиенияСтроки.ИмяПоляКоличество = "КоличествоИзменение";
...
Процедура УстановитьПризнакИсключенияПродукцииТретьегоУровня(Форма, ДатаДокумента)
	
	Форма.ИсключитьПродукциюТретьегоУровня = ИнтеграцияВЕТИСВызовСервера.ИспользуетсяОграничениеВыбораПродукцииТретьегоУровня(ДатаДокумента);
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтеграцияВЕТИС.УстановитьУсловноеОформлениеПоляПродукция(
		ЭтотОбъект,
		Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ВыполненЧерезWeb);
...
Процедура ПриСозданииЧтенииНаСервере(ОбновитьКомандыОснования = Истина)
	...
	УстановитьПризнакИсключенияПродукцииТретьегоУровня(ЭтотОбъект, ТекущаяДатаСеанса());
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	Для каждого Строка Из Объект.Товары Цикл
		...
		ЗаполнитьПредставленияТабличныхЧастейПоСтроке(ЭтотОбъект, Строка);
...
Процедура ПриИзмененииПродукцииНаСервере(ИдентификаторСтроки, Пересчет = Истина)
	...
	ОбновитьСвязанныеТабличныеЧастиПоПродукции(СтрокаТабличнойЧасти);
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСвязанныеТабличныеЧастиПоПродукции(СтрокаТовары)
	
	СтрокиУпаковки = Объект.УпаковкиВЕТИС.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТовары", СтрокаТовары.ИдентификаторСтроки));
	СтрокиШтрихкоды = Объект.ШтрихкодыУпаковок.НайтиСтроки(Новый Структура("ИдентификаторСтроки", СтрокаТовары.ИдентификаторСтроки));
	СтрокиПроизводители = Объект.Производители.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТовары", СтрокаТовары.ИдентификаторСтроки));
	СтрокиПартии = Объект.ПроизводственныеПартии.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТовары", СтрокаТовары.ИдентификаторСтроки));
	
	Строк = СтрокиУпаковки.ВГраница();
	Для Индекс = 0 По Строк Цикл
		Объект.УпаковкиВЕТИС.Удалить(СтрокиУпаковки[Строк-Индекс]);
	КонецЦикла;
	
	Строк = СтрокиШтрихкоды.ВГраница();
	Для Индекс = 0 По Строк Цикл
		Объект.ШтрихкодыУпаковок.Удалить(СтрокиШтрихкоды[Строк-Индекс]);
	КонецЦикла;
	
	Строк = СтрокиПроизводители.ВГраница();
	Для Индекс = 0 По Строк Цикл
		Объект.Производители.Удалить(СтрокиПроизводители[Строк-Индекс]);
	КонецЦикла;
	
	Строк = СтрокиПартии.ВГраница();
	Для Индекс = 0 По Строк Цикл
		Объект.ПроизводственныеПартии.Удалить(СтрокиПартии[Строк-Индекс]);
	КонецЦикла;
	
	ПроизводителиПродукции = Справочники.ПродукцияВЕТИС.Производители(СтрокаТовары.Продукция);
	Если ПроизводителиПродукции.Количество() = 1 Тогда
		
		НоваяСтрокаПроизводители = Объект.Производители.Добавить();
		НоваяСтрокаПроизводители.ИдентификаторСтрокиТовары = СтрокаТовары.ИдентификаторСтроки;
		НоваяСтрокаПроизводители.Производитель = ПроизводителиПродукции[0];
		НоваяСтрокаПроизводители.РольПредприятия = Перечисления.РолиПредприятийВЕТИС.Производитель;
		Справочники.ПредприятияВЕТИС.ЗаполнитьНомера(Объект.Производители, НоваяСтрокаПроизводители.Производитель);

	КонецЕсли;
	
	ЗаполнитьПредставленияТабличныхЧастейПоСтроке(ЭтотОбъект, СтрокаТовары);
...
Процедура ПриИзмененииЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти, ПерезаполнитьСтроку = Ложь)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЗаписьСкладскогоЖурнала) Тогда
		...
		ПриИзмененииЗаписиСкладскогоЖурналаНаСервере(СтрокаТабличнойЧасти.ПолучитьИдентификатор(),
			ПерезаполнитьСтроку, ДанныеВыбранногоСкладскогоЖурнала);
		...
		Если СтрокаТабличнойЧасти.КоличествоИзменение <> 0 Тогда
			...
			Если СтрокаТабличнойЧасти.Операция = ПредопределенноеЗначение("Перечисление.ОперацииИнвентаризацииПродукцииВЕТИС.Изменение") Тогда
				...
				КоличествоИзменениеПриИзменении(СтрокаТабличнойЧасти);
				
				Если ДанныеВыбранногоСкладскогоЖурнала = Неопределено
					Или (СтрокаТабличнойЧасти.КоличествоИзменениеВЕТИС < 0
					И ДанныеВыбранногоСкладскогоЖурнала.КоличествоВЕТИС < -СтрокаТабличнойЧасти.КоличествоИзменениеВЕТИС) Тогда
					
					ТекстСообщения = НСтр( "ru = 'Остатка продукции по записи складского журнала %1 не достаточно для оформления ВСД.'");
					ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаТабличнойЧасти.ЗаписьСкладскогоЖурнала);
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						ТекстСообщения,
						Объект.Ссылка,
						"Объект.Товары[" + СтрокаТабличнойЧасти.ПолучитьИдентификатор() + "].КоличествоИзменениеВЕТИС");
	...
	Иначе
		...
		ОчиститьДанныеТабличныхЧастейПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
		
		ЗаполнитьПредставлениеПериодовПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
		ЗаполнитьПредставленияТабличныхЧастейПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
...
&НаСервере
Процедура ПриИзмененииЗаписиСкладскогоЖурналаНаСервере(ИдентификаторСтроки, ПерезаполнитьСтроку = Ложь, ДанныеСкладскогоЖурнала = Неопределено)
	...
	ЗаполнитьЗначенияСвойств(ДанныеСкладскогоЖурнала, ДанныеЗаписиЖурнала);
...
Функция ОбновитьДанныеЗаписейСкладскогоЖурнала()
	...
	Если СтатусВЕТИС = Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.Выполнен
		ИЛИ СтатусВЕТИС = Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ВыполненЧерезWeb Тогда
...
Процедура ОбновитьКонтрольныеРеквизитыСравненияПоСтрокеНаСервере(ИдентификаторСтроки, ИзменяемыеПоля = "")
	...
	ЗаписьСкладскогоЖурнала = ЗаписьСкладскогоЖурналаПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
	...
	Для каждого Поле Из ПоляПроверки Цикл
		...
		ИначеЕсли Поле = "УпаковкиШтрихкоды" Тогда
			...
			СтруктураРеквизитовШтрихкоды.Удалить("ШтрихкодУпаковки");
...
Процедура НадписьУпаковкиШтрихкодыНажатиеЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ЗаполнитьПредставлениеТабличнойЧастиПоСтроке(ЭтотОбъект, "УпаковкиВЕТИС", СтрокаТабличнойЧасти);
...
Процедура ОткрытьФормуРедактированияТабличнойЧастиПоСтрокеЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ОчиститьДанныеТабличнойЧастиПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти, ИмяТЧ);
		...
		ЗаполнитьПредставлениеТабличнойЧастиПоСтроке(ЭтотОбъект, ИмяТЧ, СтрокаТабличнойЧасти);
...
Процедура НастроитьВидимостьДоступностьЭлементовПоСтроке(ТекущиеДанные)
	...
	ЭлементыТолькоПросмотр = (ТекущиеДанные.Операция = Удаление)
		...
		ИЛИ СтатусВЕТИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.Выполнен")
		ИЛИ СтатусВЕТИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ВыполненЧерезWeb");
...
Процедура ОбработкаВыбораНоменклатурыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	НоменклатураБылаУказана = ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура);
	СтрокаТабличнойЧасти.Номенклатура = Результат;
	
	НоменклатураПриИзменении(НоменклатураБылаУказана);
...
Процедура ЗаполнитьКоличествоСтароеВЕТИСПоСтроке(ИдентификаторСтроки, Знач ДанныеЗаписиЖурнала = Неопределено)
	...
	Если Коэффициент = Неопределено Тогда
		...
		Коэффициент = Справочники.ЕдиницыИзмеренияВЕТИС.КоэффициентПересчета(
...
Функция ЗаписьСкладскогоЖурналаПоСтроке(Форма, СтрокаТабличнойЧасти)
	...
	Если Форма.СтатусВЕТИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.Выполнен")
		ИЛИ Форма.СтатусВЕТИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ВыполненЧерезWeb") Тогда
...
Процедура ОбновитьСтатусВЕТИС()
	...
	Если СтатусВЕТИС <> Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ВыполненЧерезWeb Тогда
	...
	РедактированиеФормыНеДоступно = СтатусВЕТИС <> Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.Черновик
	                              И СтатусВЕТИС <> Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ОтклоненВЕТИС
	                              И СтатусВЕТИС <> Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ОшибкаПередачи;
	...
	#Область ОшибкиВетИС
	Если СтатусВЕТИС = Перечисления.СтатусыОбработкиИнвентаризацийПродукцииВЕТИС.ОтклоненВЕТИС Тогда
...
Процедура ОткрытьПодборСерий(Текст, СтандартнаяОбработка)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		ИнтеграцияИСКлиент.ОткрытьПодборСерий(
			ЭтотОбъект, ПараметрыУказанияСерий, Текст, СтандартнаяОбработка);
...
Процедура ПриВыбореНоменклатуры(Номенклатура, ДополнительныеПараметры) Экспорт
	...
	НоменклатураБылаУказана = ЗначениеЗаполнено(Номенклатура);
...
&НаКлиенте
Процедура НоменклатураПриИзменении(НоменклатураБылаУказана)
	...
	Если РедактированиеФормыНедоступно
		ИЛИ ЗначениеЗаполнено(ТекущиеДанные.ЗаписьСкладскогоЖурнала)
		ИЛИ (Не НоменклатураБылаУказана И ЗначениеЗаполнено(ТекущиеДанные.Продукция)) Тогда
		ПараметрыЗаполнения.ПересчитатьКоличествоЕдиницПоВЕТИС = Истина;
	Иначе
		ПараметрыЗаполнения.ПересчитатьКоличествоЕдиницВЕТИС = Истина;
		ПараметрыЗаполнения.ЗаполнитьПродукциюВЕТИС = Истина;
		ПараметрыЗаполнения.ЗаполнитьПризнакСкоропортящаясяПродукция = ТекущиеДанные.Операция = ПредопределенноеЗначение("Перечисление.ОперацииИнвентаризацииПродукцииВЕТИС.Добавление");
	КонецЕсли;
...
Процедура ЗаполнитьОтборыПоОснованию(ОбновитьКомандыОснования = Истина)
	...
	ОтборыПоОснованию = ИнтеграцияВЕТИСВызовСервера.ОтборыДляРеквизитовДокументаПоОснованию(Тип("ДокументСсылка.ИнвентаризацияПродукцииВЕТИС"), 
...
Процедура ПерезаполнитьПоОснованиюСервер()
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("НеЗаполнятьШапку");
...
Функция ЗаполнитьЗаписиСкладскогоЖурналаНаСервере()
	...
	Если РезультатЗаполнения.ИзмененныеСтроки.Количество() Тогда
		...
		Для каждого Строка Из РезультатЗаполнения.ИзмененныеСтроки Цикл
			...
			ОчиститьДанныеТабличныхЧастейПоСтроке(ЭтотОбъект, Строка);
...
&НаКлиенте
Процедура ОткрытьФормуРегистрацииВСД()
	...
	Отбор.Вставить("ГрузополучательХозяйствующийСубъект", Объект.ХозяйствующийСубъект);
	Отбор.Вставить("ГрузополучательПредприятие", Объект.Предприятие);
	Отбор.Вставить("Тип", ПредопределенноеЗначение("Перечисление.ТипыВетеринарныхДокументовВЕТИС.Инвентаризационный"));
	Отбор.Вставить("Продукция", ТекущиеДанные.Продукция);
	Отбор.Вставить("Статус",ПредопределенноеЗначение("Перечисление.СтатусыВетеринарныхДокументовВЕТИС.Закрыт"));
		
	ОткрытьФорму("Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ФормаОбъекта",
		Новый Структура("Основание", Отбор),
		ЭтотОбъект,,,,,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ПриВводеИВыбореВСД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		Иначе
			Возврат;
		КонецЕсли;
	ИначеЕсли ТипЗнч(Результат) = Тип("СправочникСсылка.ВетеринарноСопроводительныйДокументВЕТИС") Тогда
	...
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	ТекущиеДанные.ВетеринарноСопроводительныйДокумент = ВСД;
	
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Продукция) Тогда
	
		ОчиститьСообщения();
		ПриИзмененииВСДНаСервере(ТекущиеДанные.ПолучитьИдентификатор());
		НастроитьВидимостьДоступностьЭлементовПоСтроке(ТекущиеДанные);
...
&НаСервере
Процедура ПриИзмененииВСДНаСервере(ИдентификаторСтроки)
	
	СтрокаТабличнойЧасти = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", СтрокаТабличнойЧасти.ВетеринарноСопроводительныйДокумент);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВетеринарноСопроводительныйДокументВЕТИС.Продукция,
	|	ВетеринарноСопроводительныйДокументВЕТИС.ЕдиницаИзмеренияВЕТИС,
	|	ВетеринарноСопроводительныйДокументВЕТИС.СкоропортящаясяПродукция,
	|	ВетеринарноСопроводительныйДокументВЕТИС.НизкокачественнаяПродукция,
	|	ВетеринарноСопроводительныйДокументВЕТИС.ДатаПроизводстваСтрока,
	|	ВетеринарноСопроводительныйДокументВЕТИС.ДатаПроизводстваТочностьЗаполнения,
	|	ВетеринарноСопроводительныйДокументВЕТИС.ДатаПроизводстваНачалоПериода,
	|	ВетеринарноСопроводительныйДокументВЕТИС.ДатаПроизводстваКонецПериода,
	|	ВетеринарноСопроводительныйДокументВЕТИС.СрокГодностиСтрока,
	|	ВетеринарноСопроводительныйДокументВЕТИС.СрокГодностиТочностьЗаполнения,
	|	ВетеринарноСопроводительныйДокументВЕТИС.СрокГодностиНачалоПериода,
	|	ВетеринарноСопроводительныйДокументВЕТИС.СрокГодностиКонецПериода,
	...
	ПриИзмененииПродукцииНаСервере(ИдентификаторСтроки);
	
	Производители = Выборка.Производители.Выбрать();
	Пока Производители.Следующий() Цикл
		НоваяСтрока = Объект.Производители.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Производители);
		НоваяСтрока.ИдентификаторСтрокиТовары = СтрокаТабличнойЧасти.ИдентификаторСтроки;
	КонецЦикла;
	Партии = Выборка.ПроизводственныеПартии.Выбрать();
	Пока Партии.Следующий() Цикл
		НоваяСтрока = Объект.ПроизводственныеПартии.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Партии);
		НоваяСтрока.ИдентификаторСтрокиТовары = СтрокаТабличнойЧасти.ИдентификаторСтроки;
	КонецЦикла;
	
	ЗаполнитьПредставлениеТабличнойЧастиПоСтроке(ЭтотОбъект, "Производители", СтрокаТабличнойЧасти);
	ЗаполнитьПредставлениеТабличнойЧастиПоСтроке(ЭтотОбъект, "ПроизводственныеПартии", СтрокаТабличнойЧасти);
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		КоличествоИзменениеПриИзменении(НоваяСтрока);
		ОбновитьКонтрольныеРеквизитыСравненияПоСтроке(НоваяСтрока, "КоличествоВЕТИС");
