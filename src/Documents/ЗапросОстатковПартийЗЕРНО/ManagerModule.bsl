...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросПартий
		...
		ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа(ПараметрыЗагрузкиПартии, ПараметрыОбмена);
...
Процедура ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа(ПараметрыЗагрузкиПартии, ПараметрыОбмена)
	
	ТаблицаСсылкиПроверкиПолей = Новый ТаблицаЗначений();
	ТаблицаСсылкиПроверкиПолей.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.КлючиРеквизитовОрганизацийЗЕРНО"));
	
	Для Каждого КлючИЗначение Из ПараметрыЗагрузкиПартии.ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа Цикл
		...
		Если ДанныеПоСвойству.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ЮридическоеЛицо
			...
			КонецЕсли;
			Если ДанныеПоСвойству.Данные.Address <> Неопределено
				И ЗначениеЗаполнено(ДанныеПоСвойству.Данные.Address.PostalCode) Тогда
		...
		ПараметрыЗагрузкиПартии.ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа[КлючИЗначение.Ключ] = ЗначенияПолей;
		Если ЗначенияПолей.Количество() Тогда
			НоваяСтрока = ТаблицаСсылкиПроверкиПолей.Добавить();
			НоваяСтрока.Ссылка = КлючИЗначение.Ключ;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ТаблицаСсылкиПроверкиПолей.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	КлючиРеквизитовОрганизацийЗЕРНО.Ссылка В (&Ссылка)
	...
	Запрос.УстановитьПараметр("Ссылка", ТаблицаСсылкиПроверкиПолей.ВыгрузитьКолонку("Ссылка"));
	...
	ЭлементБлокировки.ИсточникДанных = ТаблицаСсылкиПроверкиПолей;
	...
	Попытка
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			КлассификаторОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(
				КлассификаторОбъект,
				ПараметрыЗагрузкиПартии.ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа[ВыборкаДетальныеЗаписи.Ссылка]);
	...
	ПараметрыЗагрузкиПартии.ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа.Очистить();
