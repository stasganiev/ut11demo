...
Процедура ТоварыБезРазбиенияРезультатИсследованияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы = ПараметрыФормыРезультатыИсследований();
...
Процедура ПрименитьУсловноеОформлениеХарактеристикаСерия(СписокРеквизитов = "")
	...
	Если Не ЗначениеЗаполнено(СписокРеквизитов) Тогда
		СписокРеквизитов = "ТоварыБезРазбиенияХарактеристика, ТоварыБезРазбиенияСерия";
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("Операция")
		...
		ЭтоФормированиеПартииПоРезультатамГосмонтиринга = Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗЕРНО.ФормированиеПартииПоРезультатамГосмониторинга");
		ЭтоСборУрожая = Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗЕРНО.ФормированиеПартииПриСбореУрожая");
		ЭтоФормированиеПартииИмпорт = Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗЕРНО.ФормированиеПартииИмпорт");
...
Процедура ИнициализироватьПоляКонтактнойИнформации()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		СтрокаТовары.КомментарийМестоположение = ИнтеграцияИС.КомментарийКонтактнойИнформации(
			СтрокаТовары.Местоположение);
		
		Если Объект.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииИмпорт Тогда
			СтрокаТовары.КомментарийПроисхождение = ИнтеграцияИС.КомментарийКонтактнойИнформации(
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения)
	...
	Если ИмяОбновляемогоПоля = "РезультатИсследования" Или ИмяОбновляемогоПоля = "ТоварыРезультатИсследования" Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.ОКПД2) Тогда
			...
			Если ДанныеРезультатовИсследования.ОКПД2 <> ТекущаяСтрока.ОКПД2 Тогда
				
				ТекущаяСтрока.ОКПД2              = ДанныеРезультатовИсследования.ОКПД2; 
				ТекущаяСтрока.ОКПД2Представление = ДанныеРезультатовИсследования.ОКПД2Представление;
		...
		ЗаполнитьПотребительскиеСвойстваПродукцииПоДаннымИсточнка(ДанныеРезультатовИсследования.ПотребительскиеСвойства, ТекущаяСтрока.Идентификатор);
...
&НаСервере
Процедура ЗаполнитьПотребительскиеСвойстваПродукцииПоДаннымИсточнка(ИсточникДанных, ИдентификаторСтроки)
...
&НаКлиенте
Функция ПараметрыФормыРезультатыИсследований()
...
Процедура СкопироватьПотребительскиеСвойстваПартииНаСервере(Результат, ИдентификаторСтроки)
	...
	ЗаполнитьПотребительскиеСвойстваПродукцииПоДаннымИсточнка(СвойстваПартии, ИдентификаторСтроки);
