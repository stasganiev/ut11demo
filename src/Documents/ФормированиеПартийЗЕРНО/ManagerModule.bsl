	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииИмпорт
		...
		Попытка
			...
			Для Каждого СтрокаДанных Из ДанныеДляОбработки Цикл
				...
				Если ЗначениеЗаполнено(ПараметрыЗапроса.СкладКонтрагент)
					И ЗначениеЗаполнено(ПараметрыЗапроса.МестоположениеСтрокой) Тогда
					ИсходныеДанныеАдреса = Справочники.КлючиАдресовЗЕРНО.ИсходныеДанныеАдреса(
						ПараметрыЗапроса.МестоположениеСтрокой, ПараметрыЗапроса.Индекс);
					Справочники.КлючиАдресовЗЕРНО.КлючАдреса(
						ИсходныеДанныеАдреса, ПараметрыОбмена, ПараметрыЗапроса.СкладКонтрагент);
				КонецЕсли;
...
Функция ФормированиеПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	|	Товары.РезультатИсследования.Идентификатор КАК НомерИсследования,
	|
	...
	Если ЭтоПереработчик Тогда
		...
		Для Каждого СтрокаПартии Из Товары Цикл
			...
			ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииИмпорт Тогда
				...
				ПроисхождениеXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(
					ПространствоИмен, "PlaceOfOrigin", ЭлементПартииXDTO);
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(
					ПроисхождениеXDTO, "Address", СтрокаПартии.ПроисхождениеСтрокой,, АбстрактноеСообщениеXML);
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(
			...
			//AddressRFType
			МестоположениеXDTO = ИнтеграцияЗЕРНОСлужебный.АдресРФXDTO(СтрокаПартии.Местоположение, СтрокаПартии.МестоположениеСтрокой, АбстрактноеСообщениеXML);
			...
			Если Шапка.НазначениеПартии = Справочники.КлассификаторНСИЗЕРНО.НазначениеПартииВывозСТерриторииРФ Тогда
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСтрануНазначения(
					ФормированиеПартииXDTO, Шапка.СтранаНазначенияКодАльфа3, ПространствоИмен, АбстрактноеСообщениеXML);
			КонецЕсли;
			...
			ПараметрыПреобразования = ИнтеграцияЗЕРНОСлужебный.ПараметрыПреобразования();
	...
	Иначе
		...
		Для Каждого СтрокаПартии Из Товары Цикл
			...
			ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииПоРезультатамГосмониторинга Тогда
				...
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЭлементПартииXDTO, "amount", СтрокаПартии.КоличествоЗЕРНО, АбстрактноеСообщениеXML);
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЭлементПартииXDTO, "reserchNumber", СтрокаПартии.НомерИсследования, АбстрактноеСообщениеXML);
				ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ФормированиеПартииXDTO, "LotFromField", ЭлементПартииXDTO, АбстрактноеСообщениеXML);
