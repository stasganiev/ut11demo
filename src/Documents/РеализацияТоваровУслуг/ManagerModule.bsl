...
Функция ТекстЗапросаВтПодходящиеСоглашения(Запрос, ТекстыЗапроса)
	...
	|	ЕСТЬNULL(Услуги.Номенклатура, НЕОПРЕДЕЛЕНО) КАК Номенклатура,
	|	ЕСТЬNULL(Услуги.Характеристика, НЕОПРЕДЕЛЕНО) КАК Характеристика
	|ПОМЕСТИТЬ ВтПодходящиеСоглашения
	|ИЗ
	|	ДанныеДокументаТаблицаТовары КАК КОформлению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками КАК СоглашениеСПоставщиком
	|	ПО СоглашениеСПоставщиком.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
	|		И СоглашениеСПоставщиком.Организация = &Организация
	|		И СоглашениеСПоставщиком.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОказаниеАгентскихУслуг)
	|		И СоглашениеСПоставщиком.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Действует),
	|			ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Закрыто))
	|		И &Период >= СоглашениеСПоставщиком.ДатаНачалаДействия
	|		И (&Период <= СоглашениеСПоставщиком.ДатаОкончанияДействия
	|			ИЛИ СоглашениеСПоставщиком.ДатаОкончанияДействия = ДАТАВРЕМЯ(1,1,1))
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СоглашенияСПоставщиками.АгентскиеУслуги КАК Услуги
	|	ПО СоглашениеСПоставщиком.Ссылка = Услуги.Ссылка
...
Функция ТекстЗапросаВтАгентскиеУслуги(Запрос, ТекстыЗапроса)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтПодходящиеСоглашения", ТекстыЗапроса) Тогда
		ТекстЗапросаВтПодходящиеСоглашения(Запрос, ТекстыЗапроса);
	КонецЕсли;
	
	ТекстЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА НЕ УслугиПоДействующимСоглашениям.Ссылка ЕСТЬ NULL
	|		ТОГДА УслугиПоДействующимСоглашениям.Ссылка
	|		КОГДА НЕ ДействующиеСоглашения.Ссылка ЕСТЬ NULL
	|		ТОГДА ДействующиеСоглашения.Ссылка
	|		КОГДА НЕ УслугиПоЗакрытымСоглашениям.Ссылка ЕСТЬ NULL
	|		ТОГДА УслугиПоЗакрытымСоглашениям.Ссылка
	|		ИНАЧЕ ЕСТЬNULL(ЗакрытыеСоглашения.Ссылка, НЕОПРЕДЕЛЕНО)
	|	КОНЕЦ КАК Соглашение,
	|	ВЫБОР
	|		КОГДА НЕ УслугиПоДействующимСоглашениям.Ссылка ЕСТЬ NULL
	|		ТОГДА УслугиПоДействующимСоглашениям.Валюта
	|		КОГДА НЕ ДействующиеСоглашения.Ссылка ЕСТЬ NULL
	|		ТОГДА ДействующиеСоглашения.Валюта
	|		КОГДА НЕ УслугиПоЗакрытымСоглашениям.Ссылка ЕСТЬ NULL
	|		ТОГДА УслугиПоЗакрытымСоглашениям.Валюта
	|		ИНАЧЕ ЕСТЬNULL(ЗакрытыеСоглашения.Валюта, &Валюта)
	|	КОНЕЦ КАК Валюта,
	...
	|	ДанныеДокументаТаблицаТовары КАК КОформлению
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПодходящиеСоглашения КАК УслугиПоДействующимСоглашениям
	|	ПО УслугиПоДействующимСоглашениям.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
	|		И УслугиПоДействующимСоглашениям.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Действует)
	|		И КОформлению.Номенклатура = УслугиПоДействующимСоглашениям.Номенклатура
	|		И КОформлению.Характеристика = УслугиПоДействующимСоглашениям.Характеристика
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПодходящиеСоглашения КАК ДействующиеСоглашения
	|	ПО ДействующиеСоглашения.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
	|		И ДействующиеСоглашения.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Действует)
	|		И УслугиПоДействующимСоглашениям.Ссылка ЕСТЬ NULL
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПодходящиеСоглашения КАК УслугиПоЗакрытымСоглашениям
	|	ПО УслугиПоЗакрытымСоглашениям.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
	|		И УслугиПоЗакрытымСоглашениям.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Закрыто)
	|		И КОформлению.Номенклатура = УслугиПоЗакрытымСоглашениям.Номенклатура
	|		И КОформлению.Характеристика = УслугиПоЗакрытымСоглашениям.Характеристика
	|		И УслугиПоДействующимСоглашениям.Ссылка ЕСТЬ NULL
	|		И ДействующиеСоглашения.Ссылка ЕСТЬ NULL
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПодходящиеСоглашения КАК ЗакрытыеСоглашения
	|	ПО ЗакрытыеСоглашения.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
	|		И ЗакрытыеСоглашения.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Закрыто)
	|		И УслугиПоДействующимСоглашениям.Ссылка ЕСТЬ NULL
	|		И ДействующиеСоглашения.Ссылка ЕСТЬ NULL
	|		И УслугиПоЗакрытымСоглашениям.Ссылка ЕСТЬ NULL
	|ГДЕ
	|	КОформлению.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|	И ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал) <> &Организация
	|	И ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал) <> НЕОПРЕДЕЛЕНО
...
Функция ТекстЗапросаВтРаботыУслуги(Запрос, ТекстыЗапроса, ДокументОбъект)
	...
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ТаблицаТовары.НоменклатураТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))";
