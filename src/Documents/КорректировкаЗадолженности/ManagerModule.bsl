...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f712570d-6db6-46e9-b56b-7bf66fc5ed0b");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Старый вариант формирования проводок"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	НЕ КорректировкаЗадолженности.СтарыйВариантПроводок
	|	И (НЕ КорректировкаЗадолженности.УдалитьСтатьяРасходов В (&ПустыеСтатьи)
	|	ИЛИ НЕ КорректировкаЗадолженности.УдалитьСтатьяДоходов В (&ПустыеСтатьи))";
	
	
	ПустыеСтатьи = Новый СписокЗначений();
	ПустыеСтатьи.Добавить(ПланыВидовХарактеристик.СтатьиРасходов.ПустаяСсылка());
	ПустыеСтатьи.Добавить(ПланыВидовХарактеристик.СтатьиДоходов.ПустаяСсылка());
	ПустыеСтатьи.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка());
	ПустыеСтатьи.Добавить(НЕОПРЕДЕЛЕНО);
	
	Запрос.УстановитьПараметр("ПустыеСтатьи", ПустыеСтатьи);
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если НЕ ДокументОбъект.СтарыйВариантПроводок и (ЗначениеЗаполнено(ДокументОбъект.УдалитьСтатьяРасходов) ИЛИ ЗначениеЗаполнено(ДокументОбъект.УдалитьСтатьяДоходов)) Тогда
					ДокументОбъект.СтарыйВариантПроводок = ИСТИНА;
				КонецЕсли;
