...
Функция ТекстЗапросаТаблицаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТекстыЗапроса, Регистры)
	...
	|			&Период
	|		ИНАЧЕ
	|			КОНЕЦПЕРИОДА(ДанныеДокумента.Дата, ДЕНЬ)
	|	КОНЕЦ КАК Период,
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	#Область ЗадолженностьПеревозчикаПоБилетуЗадолженностьПеревозчикуАгенту
	...
	ТекстЗакупка = "
		...
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.Ссылка.ДатаПрибытия < ДанныеДокумента.Дата
		|			ТОГДА ДанныеДокумента.Дата
		...
		|	ДанныеДокумента.Ссылка.Перевозчик                                   КАК Партнер,
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.Дата < ДанныеДокумента.Ссылка.ДатаПрибытия ТОГДА
		|			КОНЕЦПЕРИОДА(ДанныеДокумента.Ссылка.ДатаПрибытия, ДЕНЬ)
		|		ИНАЧЕ
		|			ДанныеДокумента.Дата
		|	КОНЕЦ                                                               КАК ДатаРегистратора,
		|	ДанныеДокумента.Ссылка.Номер                                        КАК НомерРегистратора,
...
Функция ТекстЗапросаТаблицаВтСуммыДокументовВВалютахУчета(Запрос, ТекстыЗапроса)
	...
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНеОблагаемаяНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК БазаНДСРегл,
	...
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНеОблагаемаяНДС - ТаблицаДокумента.СуммаНДС) * КоэффициентыПересчетаВалют.КоэффициентПересчетаРегл КАК БазаНДСРегл,
	|	ТаблицаДокумента.Ссылка.Валюта КАК ВалютаВзаиморасчетов,
	|	ТаблицаДокумента.Сумма КАК СуммаВзаиморасчетов,
	|	0 КАК СуммаНДСВзаиморасчетов
	|ИЗ
	...
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	(ТаблицаДокумента.СуммаДокумента - ТаблицаДокумента.СуммаНеОблагаемаяНДС - ТаблицаДокумента.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК БазаНДСРегл,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("34db78d0-0d6c-4ae2-929e-38ab74fc2694");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Сумма итого"" табличной части ""Операции"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.Бронирование.Операции КАК Документ
	|ГДЕ
	|	Документ.СуммаИтог = 0
	|	И Документ.Ссылка.Проведен
	|	И Документ.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийСБилетами.Покупка)
	|";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				Для Каждого Стр Из ДокументОбъект.Операции Цикл
					Если Стр.ТипОперации = Перечисления.ТипыОперацийСБилетами.Покупка 
						И Стр.СуммаИтог = 0 Тогда
						Стр.СуммаИтог = Стр.Сумма;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
