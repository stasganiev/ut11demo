...
&НаКлиенте
Процедура РасчетыСКредиторомПриИзменении(Элемент)

	ПриИзмененииФинИнструментов();

КонецПроцедуры

&НаКлиенте
Процедура РасчетыСДебиторомПриИзменении(Элемент)
...
&НаКлиенте
Процедура РасчетыСАрендодателемПриИзменении(Элемент)
...
Процедура ЗаполнитьТаблицуРасчетовСервер()
	...
	Иначе
		...
		Документы.СверкаВзаиморасчетов.ИнициализироватьКомпоновщикНастроек(КомпоновщикОтборФинИнструменты, УникальныйИдентификатор);
		...
		ТаблицаРезультатаСКД = Документы.СверкаВзаиморасчетов.ПолучитьВзаиморасчеты(КомпоновщикОтборФинИнструменты);
	КонецЕсли;
...
Процедура СоздатьВФоновомЗаданииНаСервере()
	...
	Если ИспользуютсяФинИнструменты Или Не НоваяАрхитектураВзаиморасчетов Тогда
		ИмяПроцедуры = "Документы.СверкаВзаиморасчетов.СоздатьДокументы";
	Иначе
		ИмяПроцедуры = "Документы.СверкаВзаиморасчетов2_5_11.СоздатьДокументы";
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовНаСервере()
	...
	Элементы.ГруппаКомментарийНастройкиСверкиЭДО.Видимость = Ложь;
	...
	Если ДоступныРасчетыПоФинансовымИнструментам Тогда
		Если ДоступныДоговорыКредитовИДепозитов Тогда
			//++ Локализация
			Элементы.ГруппаКомментарийНастройкиСверкиЭДО.Видимость = Истина;
			//-- Локализация
	...
	Элементы.ГруппаКомментарийНастройкиСверкиЭДО.Видимость = НЕ ЭтоБазовая;
...
&НаСервере
Процедура ПриИзмененииФинИнструментов()

	ИспользуютсяФинИнструменты = РасчетыСАрендодателем ИЛИ РасчетыСДебитором ИЛИ РасчетыСКредитором;
	Элементы.РежимСверкиИтоговВзаиморасчетов.Доступность = НЕ ИспользуютсяФинИнструменты;
	Элементы.СверкаВВалюте.Доступность = НЕ ИспользуютсяФинИнструменты;
