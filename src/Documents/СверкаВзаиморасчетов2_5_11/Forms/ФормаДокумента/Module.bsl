...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ПоказатьИтоговыеЗаписи Тогда
		РассчитатьСуммовыеПоказателиИтоговыхЗаписей();
	КонецЕсли;
...
Процедура ИтоговыеЗаписиОбъектРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтрокаТаблицы = Элементы.ИтоговыеЗаписи.ТекущиеДанные;
...
Процедура ИтоговыеЗаписиПередУдалением(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные = Неопределено Или Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ТекущиеДанные, ПоляОтбора);
...
&НаКлиенте
Процедура ИтоговыеЗаписиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ИтоговыеЗаписиПередОкончаниемРедактированияЗавершение",
		ЭтотОбъект,
		Новый Структура("НаборСтрок", НаборСтрок));
...
&НаКлиенте
Процедура ИтоговыеЗаписиПередОкончаниемРедактированияЗавершение(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = КодВозвратаДиалога.Да Тогда	// Удалить строки
		Для каждого ДетальнаяЗапись Из ДополнительныеПараметры.НаборСтрок Цикл
...
Процедура РазъединитьСтрокуДетальныеЗаписи(Команда)
	...
	ЗаполнитьЗначенияСвойств(СтрокаКонтрагента, СтрокаОрганизации, "ВалютаДокумента," + СписокСвойств);
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ДетальныеЗаписиВалютаДокумента");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	...
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "НомерДокумента", Истина);
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "ДатаДокумента", Истина);
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "НаименованиеДокумента", Истина);
...
&НаСервере
Процедура УстановитьПроверкуЗаполненияАналитики(ТипЗаписей, ИмяПоля, ПоДаннымКонтрагента);

	Элемент = УсловноеОформление.Элементы.Добавить();

	Если ПоДаннымКонтрагента Тогда
		Постфикс = "Контрагент";
		ИмяПоляСтороныПроверки = "ЕстьДанныеКонтрагента";
	Иначе
		Постфикс = "";
		ИмяПоляСтороныПроверки = "ЕстьДанныеОрганизации";
	КонецЕсли;
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Объект.%1.%2", ТипЗаписей, ИмяПоляСтороныПроверки));
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;

	ОтборЭлемента = ГруппаОтбораЭлемента.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Объект.%1.%2%3", ТипЗаписей, ИмяПоля, Постфикс));
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
Процедура УстановитьВидимость()
	...
	Элементы.ИтоговыеЗаписиДоговор.Видимость = ВедетсяУчетПоДоговорам;
	Элементы.ДетальныеЗаписиДоговор.Видимость = ВедетсяУчетПоДоговорам;

	СверкаПоДоговорам = Объект.РежимСверкиИтоговВзаиморасчетов = ПредопределенноеЗначение("Перечисление.РежимСверкиИтоговВзаиморасчетов.ПоДоговорам");
	Элементы.ДетальныеЗаписиОбъектРасчетов.Видимость = Не СверкаПоДоговорам;
	Элементы.ДетальныеЗаписиОбъектРасчетов.Видимость = Не СверкаПоДоговорам;
...
Процедура УстановитьВидимостьОбъектаРасчетов()
	...
	ВидимотьОбъектаРасчтеов = Объект.РежимСверкиИтоговВзаиморасчетов = Перечисления.РежимСверкиИтоговВзаиморасчетов.ПоОбъектамРасчетов;
	Элементы.ИтоговыеЗаписиОбъектРасчетов.Видимость = ВидимотьОбъектаРасчтеов;
...
Процедура УправлениеЭлементамиФормы()
	...
	Если ЭтоСверкаМеждуОрганизациями Тогда
		...
		Элементы.ИтоговыеЗаписиДоговор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыМеждуОрганизациями");
		Элементы.ДетальныеЗаписиДоговор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыМеждуОрганизациями");
	Иначе
		...
		Элементы.ИтоговыеЗаписиДоговор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов");
		Элементы.ДетальныеЗаписиДоговор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов");
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьСтатусСверки(ИмяСтатуса)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Статус);
	ПоказатьВопрос(Новый ОписаниеОповещения("УстановитьСтатусСверкиЗавершение", ЭтотОбъект, Новый Структура("Статус", Статус)), ТекстВопроса, РежимДиалогаВопрос.ДаНет);

КонецПроцедуры

&НаКлиенте
Процедура УстановитьСтатусСверкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура РассчитатьСуммовыеПоказателиИтоговыхЗаписей()
...
&НаКлиенте
Процедура КлючевойРеквизитПриИзменении(ИмяРеквизита)
	...
	Если НЕ ПустаяСтрока(ДополнительныеПараметры.ТекстВопроса) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("КлючевойРеквизитПриИзмененииЗавершение",
		ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОписаниеОповещения, ДополнительныеПараметры.ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		КлючевойРеквизитПриИзмененииЗавершение(КодВозвратаДиалога.Да, ДополнительныеПараметры);
	КонецЕсли; 

КонецПроцедуры

&НаКлиенте
Процедура КлючевойРеквизитПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Отказ Тогда
		Объект[ДополнительныеПараметры.ИмяРеквизита] = ПредыдущиеЗначения[ДополнительныеПараметры.ИмяРеквизита];
		Элементы.Валюта.ТолькоПросмотр = НЕ Объект.СверкаВВалюте;
	Иначе
		ПредыдущиеЗначения.Вставить(ДополнительныеПараметры.ИмяРеквизита, Объект[ДополнительныеПараметры.ИмяРеквизита]);
		Модифицированность = Истина;
	КонецЕсли;
...
Функция ПараметрыПриИзмененииКлючевыхРеквизитов(ИмяРеквизита)
	...
	Если НужноРаспровести И НужноПерезаполнитьТЧ Тогда
		ТекстВопроса = НСтр("ru = 'Заполнение табличной части ""Взаиморасчеты"" возможно только в непроведенном документе.
			|Табличная часть будет перезаполнена. Перезаполнить и отменить проведение?'");
