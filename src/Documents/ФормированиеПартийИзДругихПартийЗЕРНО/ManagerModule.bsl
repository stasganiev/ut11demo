...
Функция ФормированиеПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоПереработчик Тогда
		...
		ЗаполнитьМестоположение(ФормированиеПартииXDTO, Шапка, СообщениеXML);
		...
		Если Шапка.НазначениеПартии = Справочники.КлассификаторНСИЗЕРНО.НазначениеПартииВывозСТерриторииРФ Тогда
			ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСтрануНазначения(ФормированиеПартииXDTO, Шапка.СтранаНазначенияКодАльфа3, ПространствоИмен, СообщениеXML);
		КонецЕсли;
...
Процедура ЗаполнитьМестоположение(ОбъектXDTOРодитель, ДанныеДокумента, СообщениеXML)
	...
	Если ДанныеДокумента.Операция = Перечисления.ВидыОперацийЗЕРНО.ФормированиеПартииНаОсновеБумажногоСДИЗ Тогда
		...
		МестоположениеXDTO = ИнтеграцияЗЕРНОСлужебный.АдресРФXDTO(
			ДанныеДокумента.Местоположение, ДанныеДокумента.МестоположениеСтрокой, СообщениеXML);
	Иначе
		МестоположениеXDTO = ИнтеграцияЗЕРНОСлужебный.АдресРФПоКлючуАдресовXDTO(
			ДанныеДокумента.МестоположениеКлючАдресаСтрокой, ДанныеДокумента.Индекс, СообщениеXML);
	КонецЕсли;
