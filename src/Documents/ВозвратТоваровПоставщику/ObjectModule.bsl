...
Функция ИмеютсяЗаписиТаблицыТоваровВМенеджереВременныхТаблиц(МенеджерВременныхТаблиц)
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		МенеджерВременныхТаблицПоДокументуПоступления = ВременныеТаблицыДанныхДокумента(Истина);
		МенеджерВременныхТаблицБезДокументаПоступления = ВременныеТаблицыДанныхДокумента(Ложь);
		
		ВидыЗапасовПромежуточная = ВидыЗапасов.Выгрузить(Новый Массив);
		
		// 1. Возвраты по документу поступления обрабатываются в первую очередь, с извлечением остатков по документу поступления.
		Если ИмеютсяЗаписиТаблицыТоваровВМенеджереВременныхТаблиц(МенеджерВременныхТаблицПоДокументуПоступления) Тогда
			...
			ВидыЗапасов.Очистить();
			ВидыЗапасовДляЗаполнения = ВидыЗапасов.Выгрузить();
			ВидыЗапасовДляЗаполнения.Колонки.Добавить("ДокументОприходования", 
				Новый ОписаниеТипов("ДокументСсылка.ПриобретениеТоваровУслуг"));
			
			ПараметрыЗаполнения.ТаблицаВидыЗапасов = ВидыЗапасовДляЗаполнения;
			
			ЗапасыСервер.ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ЭтотОбъект,
																	МенеджерВременныхТаблицПоДокументуПоступления,
																	Отказ,
																	ПараметрыЗаполнения);
			ВидыЗапасовДляЗаполнения.Колонки.ДокументПоступления.Имя = "ДокументПоступленияКУдалению";
			ВидыЗапасовДляЗаполнения.Колонки.ДокументОприходования.Имя = "ДокументПоступления";
			
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ВидыЗапасовДляЗаполнения, ВидыЗапасовПромежуточная);
		КонецЕсли;
		
		// 2. Возвраты без указания документа закупки обрабатываются во вторую очередь, по общим остаткам Товаров организаций.
		Если ИмеютсяЗаписиТаблицыТоваровВМенеджереВременныхТаблиц(МенеджерВременныхТаблицБезДокументаПоступления) Тогда
			ПараметрыЗаполнения = ПараметрыЗаполненияВидовЗапасов();
			
			ВидыЗапасов.Очистить();
			ЗапасыСервер.ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ЭтотОбъект,
																	МенеджерВременныхТаблицБезДокументаПоступления,
																	Отказ,
																	ПараметрыЗаполнения);
		...
		ВидыЗапасов.Загрузить(ВидыЗапасовПромежуточная);
		
		// 3. Постобработка.
		ВидыЗапасов.Свернуть("ДокументПоступления, АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД, СтавкаНДС",
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов() Экспорт
	...
	СтруктураПоиска = Новый Структура("ДокументПоступления, АналитикаУчетаНоменклатуры");
