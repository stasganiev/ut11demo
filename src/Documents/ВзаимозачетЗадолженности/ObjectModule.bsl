...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Бартер
		...
		Если ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаОрганизацияКонтрагент
			...
			ИЛИ ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ОтражениеОплатыЧерезКомиссионера Тогда
			Источник = КредиторскаяЗадолженность;
			Приемник = ДебиторскаяЗадолженность;
		КонецЕсли;
		СуммаВзаиморасчетов = Источник.Итог("СуммаВзаиморасчетов");
		СуммаРегл = Источник.Итог("СуммаРегл");
		СуммаУпр = Источник.Итог("СуммаУпр");
		Если Приемник.Количество() > 0 И СуммаВзаиморасчетов <> 0 Тогда
			ИтогиРеглНеРавны = Источник.Итог("СуммаРегл") <> Приемник.Итог("СуммаРегл");
			ИтогиУпрНеРавны = Источник.Итог("СуммаУпр") <> Приемник.Итог("СуммаУпр");
			Если ИтогиРеглНеРавны ИЛИ ИтогиУпрНеРавны Тогда
				СтрокиПриемника = Приемник.Выгрузить();
				РезультатПриемник = Приемник.ВыгрузитьКолонки();
				Для Каждого СтрокаПриемник Из Приемник Цикл
					СтрокаПриемник.СуммаРегл = 0;
					СтрокаПриемник.СуммаУпр = 0;
				КонецЦикла;
				ПоследнийИндекс = Приемник.Количество()-1;
				Для Каждого Отправитель Из Источник Цикл
					Индекс = -1;
					СуммаОтправитель = Отправитель.СуммаВзаиморасчетов;
					КурсРегл = Отправитель.СуммаРегл/Отправитель.СуммаВзаиморасчетов;
					КурсУпр = Отправитель.СуммаУпр/Отправитель.СуммаВзаиморасчетов;
					Для Каждого Получатель Из СтрокиПриемника Цикл
						Индекс = Индекс + 1;
						Если Получатель.СуммаВзаиморасчетов = 0 Тогда
							Продолжить;
						КонецЕсли;
						Если СуммаОтправитель = 0 Тогда
							Прервать;
						КонецЕсли;
						ПоследнийПлатеж = Индекс = ПоследнийИндекс;
						СуммаВзаиморасчетов = ВзаиморасчетыСервер.СписатьСумму(СуммаОтправитель, Получатель.СуммаВзаиморасчетов, ПоследнийПлатеж);
						НоваяСтрока = РезультатПриемник.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Получатель);
						НоваяСтрока.СуммаВзаиморасчетов = СуммаВзаиморасчетов;
						НоваяСтрока.СуммаРегл = СуммаВзаиморасчетов * КурсРегл;
						НоваяСтрока.СуммаУпр = СуммаВзаиморасчетов * КурсУпр;
					КонецЦикла;
				КонецЦикла;
				Для Каждого СтрокаПриемник Из Приемник Цикл
					Отбор = Новый Структура("ИдентификаторСтроки", СтрокаПриемник.ИдентификаторСтроки);
					НайденныеСтроки = РезультатПриемник.НайтиСтроки(Отбор);
					Для Каждого Строка Из НайденныеСтроки Цикл
						СтрокаПриемник.СуммаРегл = СтрокаПриемник.СуммаРегл + Строка.СуммаРегл;
						СтрокаПриемник.СуммаУпр = СтрокаПриемник.СуммаУпр + Строка.СуммаУпр;
					КонецЦикла;
				КонецЦикла;
