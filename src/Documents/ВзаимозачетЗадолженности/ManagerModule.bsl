...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	МассивОпераций.Добавить(Перечисления.ВидыОперацийВзаимозачетаЗадолженности.УдалитьКлиентаМеждуОрганизациями);
	МассивОпераций.Добавить(Перечисления.ВидыОперацийВзаимозачетаЗадолженности.УдалитьПоставщикаМеждуОрганизациями);
...
Процедура РаспределитьТабличныеЧасти(Запрос)
	...
	|	И (&ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.Произвольный)
	|		ИЛИ &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.Бартер)
	|		ИЛИ &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.БартерМеждуОрганизациями))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	СуммыУказаныВДокументе = Запрос.Параметры.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Бартер
	...
	Для Каждого Отправитель Из ОтправительЗадолженности Цикл
		...
		Для Каждого Получатель Из ПолучательЗадолженности Цикл
			...
			// распределим несколько валют отправителя на несколько валют получателя
			СуммаОтправитель = СуммаВзаиморасчетов(ОтправленоВВалюте[Отправитель.ВалютаВзаиморасчетов], СуммаРегл);
			НоваяСуммаВзаиморасчетов = Отправитель.СуммаВзаиморасчетов - СуммаОтправитель;
			Если НоваяСуммаВзаиморасчетов < 0 Тогда // скорректируем пересписание
				СуммаОтправитель = СуммаОтправитель + НоваяСуммаВзаиморасчетов;
				ОтправленоВВалюте[Отправитель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов = 
					ОтправленоВВалюте[Отправитель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов - НоваяСуммаВзаиморасчетов;
				ПолученоВВалюте[Получатель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов = 
					ПолученоВВалюте[Получатель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов - НоваяСуммаВзаиморасчетов;
			КонецЕсли;
			...
			НоваяСуммаВзаиморасчетов = Получатель.СуммаВзаиморасчетов - СуммаПолучатель;
			Если НоваяСуммаВзаиморасчетов < 0 Тогда // скорректируем пересписание
				СуммаПолучатель = СуммаПолучатель + НоваяСуммаВзаиморасчетов;
				СуммаОтправитель = СуммаПолучатель;
				Отправитель.СуммаВзаиморасчетов = Отправитель.СуммаВзаиморасчетов - НоваяСуммаВзаиморасчетов;
				ОтправленоВВалюте[Отправитель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов = 
					ОтправленоВВалюте[Отправитель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов - НоваяСуммаВзаиморасчетов;
				ПолученоВВалюте[Получатель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов = 
					ПолученоВВалюте[Получатель.ВалютаВзаиморасчетов].СуммаВзаиморасчетов - НоваяСуммаВзаиморасчетов;
			...
			Если СуммыУказаныВДокументе Тогда
			...
			Если НоваяАрхитектураВзаиморасчетов И НЕ СуммыУказаныВДокументе Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.16.40";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("cf6385b8-276a-4059-be04-848695e1d3f6");
	...
	Обработчик.Комментарий = НСтр("ru = 'Корректирует операцию взаимозачета задолженности'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ВзаимозачетЗадолженности";
	...
	"ВЫБРАТЬ
	|	ДанныеДокумента.Ссылка КАК Ссылка,
	|	ЗадолженностьДт.ТипРасчетов КАК ТипРасчетовДт,
	|	КОЛИЧЕСТВО(ЗадолженностьДт.НомерСтроки) КАК КоличествоДт,
	|	ЗадолженностьКт.ТипРасчетов КАК ТипРасчетовКт,
	|	КОЛИЧЕСТВО(ЗадолженностьКт.НомерСтроки) КАК КоличествоКт
	|ИЗ
	|	Документ.ВзаимозачетЗадолженности КАК ДанныеДокумента
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВзаимозачетЗадолженности.ДебиторскаяЗадолженность КАК ЗадолженностьДт
	|		ПО ДанныеДокумента.Ссылка = ЗадолженностьДт.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВзаимозачетЗадолженности.КредиторскаяЗадолженность КАК ЗадолженностьКт
	|		ПО ДанныеДокумента.Ссылка = ЗадолженностьКт.Ссылка
	|ГДЕ
	|	ДанныеДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачетаЗадолженности.Произвольный)
	|	И ДанныеДокумента.ОбъектРасчетовДебиторКредитор = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|	И ТИПЗНАЧЕНИЯ(ДанныеДокумента.КонтрагентДебитор) = ТИП(Справочник.Контрагенты)
	|	И ТИПЗНАЧЕНИЯ(ДанныеДокумента.КонтрагентКредитор) = ТИП(Справочник.Контрагенты)
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.Ссылка,
	|	ЗадолженностьДт.ТипРасчетов,
	|	ЗадолженностьКт.ТипРасчетов
	|
	|ИМЕЮЩИЕ
	|	ЗадолженностьДт.ТипРасчетов = ЗадолженностьКт.ТипРасчетов
	|	И (КОЛИЧЕСТВО(ЗадолженностьДт.НомерСтроки) = КОЛИЧЕСТВО(ЗадолженностьКт.НомерСтроки)
	|	
	|		ИЛИ КОЛИЧЕСТВО(ЗадолженностьДт.НомерСтроки) > 1
	|			И КОЛИЧЕСТВО(ЗадолженностьКт.НомерСтроки) = 1
	|	
	|		ИЛИ КОЛИЧЕСТВО(ЗадолженностьДт.НомерСтроки) = 1
	|			И КОЛИЧЕСТВО(ЗадолженностьКт.НомерСтроки) > 1)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТипРасчетовСКлиентом = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом;
	Для Каждого ДанныеДокумента Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			КоличествоДт = ДокументОбъект.ДебиторскаяЗадолженность.Количество();
			ТипРасчетовДт = Неопределено;
			Если КоличествоДт > 0 Тогда
				ТипРасчетовДт = ДокументОбъект.ДебиторскаяЗадолженность[0].ТипРасчетов;
			Иначе
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ДанныеДокумента.Ссылка);
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			КоличествоКт = ДокументОбъект.КредиторскаяЗадолженность.Количество();
			
			Если ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Произвольный
				И НЕ ЗначениеЗаполнено(ДокументОбъект.ОбъектРасчетовДебиторКредитор) Тогда
			
				Если ЗначениеЗаполнено(ДокументОбъект.ОбъектРасчетовИнтеркампани) Тогда
					
					Если КоличествоДт = КоличествоКт Тогда
						Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуОрганизациями;
						Иначе
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаПоставщикуМеждуОрганизациями;
						КонецЕсли;
						
					ИначеЕсли КоличествоДт > 1 И КоличествоКт = 1 Тогда
						Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносДолгаКлиентаМеждуОрганизациями;
						Иначе
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаПоставщикуМеждуОрганизациями;
						КонецЕсли;
					...
					ИначеЕсли КоличествоДт = 1 И КоличествоКт > 1 Тогда
						Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуОрганизациями;
						Иначе
							ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносДолгаПоставщикуМеждуОрганизациями;
						КонецЕсли;
					КонецЕсли;
					
				Иначе
					Если ДокументОбъект.КонтрагентДебитор = ДокументОбъект.КонтрагентКредитор Тогда
						Если КоличествоДт = КоличествоКт Тогда
							Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
						...
						ИначеЕсли КоличествоДт > 1 И КоличествоКт = 1 Тогда
							Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносДолгаКлиентаОрганизацияКонтрагент;
							Иначе
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаПоставщикуОрганизацияКонтрагент;
							КонецЕсли;
						...
						ИначеЕсли КоличествоДт = 1 И КоличествоКт > 1 Тогда
							Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаОрганизацияКонтрагент;
							Иначе
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносДолгаПоставщикуОрганизацияКонтрагент;
							КонецЕсли;
						КонецЕсли;
					Иначе
						Если КоличествоДт = КоличествоКт Тогда
							Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаМеждуКонтрагентами;
							Иначе
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаПоставщикуМеждуКонтрагентами;
							КонецЕсли;
							
						ИначеЕсли КоличествоДт > 1 И КоличествоКт = 1 Тогда
							Если ТипРасчетовДт = ТипРасчетовСКлиентом Тогда
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносДолгаКлиентаМеждуКонтрагентами;
							Иначе
								ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаПоставщикуМеждуКонтрагентами;
							КонецЕсли;
				...
				КонецЕсли; // ЗначениеЗаполнено(ДокументОбъект.ОбъектРасчетовИнтеркампани)
			КонецЕсли; // ЭтоПроизвольный И НЕ ЗначениеЗаполнено(ДокументОбъект.ОбъектРасчетовДебиторКредитор)
