...
Процедура ПодтвердитьПлатеж(Команда)
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		МассивСообщенийОбмена = Новый Массив;
		МассивСообщенийОбмена.Добавить(Объект.Ссылка);
		ОбменСБанкамиСлужебныйКлиент.ПодготовитьОкружениеИПодписатьДокументыКодомИзSMS(Объект.НастройкаОбмена, МассивСообщенийОбмена);
...
Процедура ИзменитьВидимостьДоступность()
	...
	ДоступноПодписаниеПоSMS = ОбменСБанкамиСлужебныйВызовСервера.ДоступноПодписаниеПоSMS(Объект.ВидЭД, Объект.НастройкаОбмена);
	...
	Элементы.КомандаПодтвердитьПлатеж.Видимость = Объект.Статус = Перечисления.СтатусыОбменСБанками.НеПодтвержден
		И ((ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн
			И ТекущаяДатаСеанса() < ОбменСБанкамиСлужебныйКлиентСервер.ДатаОтключенияПодписанияSMSСбербанк())
			...
			ИЛИ (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			И ДоступноПодписаниеПоSMS));
...
Функция МожноЗапроситьСостояние()
	...
	НастройкиДокумента = ОбменСБанкамиСлужебный.НастройкиДокумента(Объект.ВидЭД, Объект.НастройкаОбмена);
	Возврат (Объект.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросНаОтзывЭД
			...
			И (Объект.Статус = Перечисления.СтатусыОбменСБанками.Принят
				...
				ИЛИ (Объект.Статус = Перечисления.СтатусыОбменСБанками.ЧастичноПодписан
					И НастройкиДокумента.СпособПодтвержденияДокумента = Перечисления.СпособыПодтвержденияДокументовОбменСБанками.SMS))
...
Процедура ЗаполнитьКартуСтатусовЭД(КартаСтатусовЭД)
	...
	ПараметрыЭД.Вставить("ВидЭД", Перечисления.ВидыЭДОбменСБанками.ПустаяСсылка());
	ПараметрыЭД.Вставить("Направление", Перечисления.НаправленияЭДО.ПустаяСсылка());
	ПараметрыЭД.Вставить("Организация", Неопределено);
	ПараметрыЭД.Вставить("Банк", Неопределено);
	ПараметрыЭД.Вставить("НастройкаОбмена", Справочники.НастройкиОбменСБанками.ПустаяСсылка());
	ПараметрыЭД.Вставить("Статус", Перечисления.СтатусыОбменСБанками.ПустаяСсылка());
	
	ЗаполнитьЗначенияСвойств(ПараметрыЭД, Объект);
	
	Настройки = Новый Структура();
	Настройки.Вставить("Направление", Перечисления.НаправленияЭДО.ПустаяСсылка());
	Настройки.Вставить("ВидЭД", Перечисления.ВидыЭДОбменСБанками.ПустаяСсылка());
	Настройки.Вставить("ИспользоватьПодпись", Ложь);
	Настройки.Вставить("ПрограммаБанка", Перечисления.ПрограммыБанка.ПустаяСсылка());
	Настройки.Вставить("Статус", Перечисления.СтатусыОбменСБанками.ПустаяСсылка());
	Настройки.Вставить("СпособПодтвержденияДокумента",
	...
	Настройки.Вставить("СпособПодтвержденияДокумента", НастройкиДокумента.СпособПодтвержденияДокумента);
