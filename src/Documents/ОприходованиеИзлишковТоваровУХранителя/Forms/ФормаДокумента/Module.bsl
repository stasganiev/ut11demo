...
Процедура ЗаполнитьВидыЦен()
	...
	ПараметрыОтбора = ПолучитьПараметрыОтбораВидаЦен();
	СписокВидовЦен = Справочники.ВидыЦен.ДоступныеВидыЦен(ПараметрыОтбора);
	Элементы.ВидЦены.СписокВыбора.ЗагрузитьЗначения(СписокВидовЦен);
	Элементы.ВидЦены.СписокВыбора.Добавить(Справочники.ВидыЦен.ПустаяСсылка(), НСтр("ru='<произвольный>'"));
