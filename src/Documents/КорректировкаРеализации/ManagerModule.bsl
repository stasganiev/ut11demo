...
//
// Возвращаемое значение:
//  КлючевыеРеквизиты - Строка - список реквизитов связи.
//
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		КОГДА НЕ ВидыЗапасов.СписатьНаРасходы
	...
	|
	|	ВидыЗапасов.СтатьяРасходовСписание КАК СтатьяРасходовСписания,
	|	Неопределено КАК СтатьяДоходов,
	|	ВидыЗапасов.АналитикаРасходовСписание КАК АналитикаРасходов,
	...
	|		ИНАЧЕ ВидыЗапасов.СуммаБезНДСУпр КОНЕЦ) КАК СтоимостьУпр,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	0 КАК НДСРегл,
	|	0 КАК НДСУпр,
	...
	|	КОНЕЦ КАК ЗаказКлиента,
	|
	|	ВидыЗапасов.СтатьяРасходовОприходование КАК СтатьяРасходовСписания,
	|	Неопределено КАК СтатьяДоходов,
	|	ВидыЗапасов.АналитикаРасходовОприходование КАК АналитикаРасходов,
	|	Неопределено КАК АналитикаДоходов,
	|
	...
	|		  ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
	|		  И ТаблицаВидыЗапасов.КорВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		ТОГДА ТаблицаВидыЗапасов.СтатьяРасходовОприходование
	|		ИНАЧЕ Неопределено
	...
	|		  ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
	|		  И ТаблицаВидыЗапасов.КорВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		ТОГДА ТаблицаВидыЗапасов.АналитикаРасходовОприходование
	|		ИНАЧЕ Неопределено
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Перемещение с реализации на корректировку - оприходование, и корректировка движения по давальческой продукции.
	|ВЫБРАТЬ
	|	5 КАК Порядок,
	|	ТаблицаВидыЗапасов.НомерСтроки КАК НомерСтрокиДокумента,
	|	ВЫБОР КОГДА ТаблицаВидыЗапасов.ТипЗапасов В
	...
	|		  И ТаблицаВидыЗапасов.КорВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиента)
	|	КОНЕЦ КАК ХозяйственнаяОперация,
	|	НЕОПРЕДЕЛЕНО КАК КорРазделУчета,
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ТипЗапасов В
	...
	|	НЕОПРЕДЕЛЕНО КАК ЗаказКлиента,
	|
	|	ТаблицаВидыЗапасов.СтатьяРасходовОприходование КАК СтатьяРасходовСписания,
	|	Неопределено КАК СтатьяДоходов,
	|	ТаблицаВидыЗапасов.АналитикаРасходовОприходование КАК АналитикаРасходов,
	|	Неопределено КАК АналитикаДоходов,
	|
	|	ДатаВремя(1,1,1) КАК ПериодПродажи,
	|	НЕОПРЕДЕЛЕНО КАК КорАналитикаФинансовогоУчета,
	...
	|		  И ТаблицаВидыЗапасов.КорВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		ТОГДА 
	|			ВЫБОР КОГДА &КорректировкаТекущегоПериода
	...
	|	
	|	ТаблицаВидыЗапасов.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	// Для движения по давальческой продукции настройка хозяйственной операции не заполняется, т.к. у давальческой продукции нет балансовой стоимости.
	|	(ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ТипЗапасов В
	|			(ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
	...
	|	И ((ТаблицаВидыЗапасов.ВидЗапасовОтгрузки <> ТаблицаВидыЗапасов.ВидЗапасов
	|		И &КорректировкаТекущегоПериода)
	|		ИЛИ (ТаблицаВидыЗапасов.ТипЗапасов В
	|		 (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца),
	|		  ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
	|		  И ТаблицаВидыЗапасов.КорВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)))
	|	И НЕ ТаблицаВидыЗапасов.СпособОпределенияСебестоимости В (
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Оприходование давальческой продукции
	|ВЫБРАТЬ
	|	6 КАК Порядок,
	|	ТаблицаВидыЗапасов.НомерСтроки КАК НомерСтрокиДокумента,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	...
	|	0 КАК НДСУпр,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5) КАК ХозяйственнаяОперация,
	|	НЕОПРЕДЕЛЕНО КАК КорРазделУчета,
	|	НЕОПРЕДЕЛЕНО КАК КорВидЗапасов,
	|	НЕОПРЕДЕЛЕНО КАК КорАналитикаУчетаНоменклатуры,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаПоПартнерам,
	...
	|	НЕОПРЕДЕЛЕНО КАК ЗаказКлиента,
	|
	|	ТаблицаВидыЗапасов.СтатьяРасходовСписание КАК СтатьяРасходовСписания,
	|	Неопределено КАК СтатьяДоходов,
	|	ТаблицаВидыЗапасов.АналитикаРасходовСписание КАК АналитикаРасходов,
	|	Неопределено КАК АналитикаДоходов,
	|
	|	ДатаВремя(1,1,1) КАК ПериодПродажи,
	|	НЕОПРЕДЕЛЕНО КАК КорАналитикаФинансовогоУчета,
	...
	|	
	|	ТаблицаВидыЗапасов.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	// Для прихода давальческой продукции настройку хозяйственной операции не заполняем, т.к. у давальческой продукции нет балансовой стоимости.  
	...
	|
	|	(ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22 И &ФИФОСкользящаяОценка ТОГДА
	|			&Ссылка
	...
	|	(ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22 И НЕ &ФИФОСкользящаяОценка
	|			ТОГДА ВидыЗапасов.АналитикаУчетаПартий
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ)									КАК КорАналитикаУчетаПартий,
	...
	|		ИНАЧЕ ВидыЗапасов.СуммаБезНДСРегл КОНЕЦ) КАК СтоимостьРегл,
	|	(ВЫБОР
	|		КОГДА НЕ &УправленческийУчетОрганизаций ТОГДА 0
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА ВидыЗапасов.СебестоимостьБезНДС
	...
	|	(ВЫБОР
	|		КОГДА НЕ &УправленческийУчетОрганизаций ТОГДА 0
	|		ИНАЧЕ ВидыЗапасов.НДСУпр КОНЕЦ) КАК НДСУпр,
	|
	...
	|	КОНЕЦ КАК ЗаказКлиента,
	|
	|	ТаблицаРасхождения.СтатьяРасходов КАК СтатьяРасходовСписания,
	|	Неопределено КАК СтатьяДоходов,
	|	ТаблицаРасхождения.АналитикаРасходов КАК АналитикаРасходов,
	|	Неопределено КАК АналитикаДоходов,
	|
	...
	|	-ТаблицаРасхождения.Количество КАК Количество,
	|	(ВЫБОР
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА ТаблицаРасхождения.Себестоимость
	|		ИНАЧЕ -ТаблицаРасхождения.СуммаСНДСУпр КОНЕЦ) КАК Стоимость,
	|	(ВЫБОР
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА ТаблицаРасхождения.СебестоимостьБезНДС
	|		ИНАЧЕ -ТаблицаРасхождения.СуммаБезНДСУпр КОНЕЦ) КАК СтоимостьБезНДС,
	|	(ВЫБОР
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	...
	|		ИНАЧЕ -ТаблицаРасхождения.СуммаБезНДСУпр КОНЕЦ) КАК СтоимостьУпр,
	|	(ВЫБОР
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	...
	|	-(ВЫБОР
	|		КОГДА НЕ &УправленческийУчетОрганизаций ТОГДА 0
	|		ИНАЧЕ ТаблицаРасхождения.НДСУпр КОНЕЦ) КАК НДСУпр,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаДоВводаОстатков) КАК ХозяйственнаяОперация,
	|	НЕОПРЕДЕЛЕНО КАК КорРазделУчета,
	|	НЕОПРЕДЕЛЕНО КАК КорВидЗапасов,
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия) КАК ТипЗаписи,
	|	
	// Поля ХозяйственнаяОперация, ИдентификаторФинЗаписи и НастройкаХозяйственнойОперации должны одинаково заполняться
	// в себестоимости товаров и в выручке и себестоимости продаж. Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж().
	|	ТаблицаРасхождения.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.КорректировкаДоВводаОстатков) КАК НастройкаХозяйственнойОперации
	|ИЗ
	...
	|	КОНЕЦ КАК ЗаказКлиента,
	|
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ВидыЗапасов.СтатьяРасходовСписание) = ТИП(ПланВидовХарактеристик.СтатьиРасходов)
	|			ТОГДА ВидыЗапасов.СтатьяРасходовСписание
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СтатьяРасходовСписания,
	|	НЕОПРЕДЕЛЕНО КАК СтатьяДоходов,
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ВидыЗапасов.СтатьяРасходовСписание) = ТИП(ПланВидовХарактеристик.СтатьиРасходов)
	|			ТОГДА ВидыЗапасов.АналитикаРасходовСписание
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК АналитикаРасходов,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаДоходов,
	|
	...
	|	КОНЕЦ КАК ЗаказКлиента,
	|
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ВидыЗапасов.СтатьяРасходовОприходование) = ТИП(ПланВидовХарактеристик.СтатьиРасходов)
	|			ТОГДА ВидыЗапасов.СтатьяРасходовОприходование
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СтатьяРасходовСписания,
	|	НЕОПРЕДЕЛЕНО КАК СтатьяДоходов,
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ВидыЗапасов.СтатьяРасходовОприходование) = ТИП(ПланВидовХарактеристик.СтатьиРасходов)
	|			ТОГДА ВидыЗапасов.АналитикаРасходовОприходование
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК АналитикаРасходов,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаДоходов,
	|
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА -ТаблицаРасхождения.Себестоимость
	|		КОГДА ТаблицаРасхождения.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента)
	|			ТОГДА ТаблицаРасхождения.СуммаСНДСУпр
	|		ИНАЧЕ 0 КОНЕЦ) КАК Стоимость,
	|	(ВЫБОР
	|		КОГДА ТаблицаРасхождения.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
...
Функция ТекстЗапросаПроверкиСпособовОпределенияСебестоимости()
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И ТаблицаТоваров.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ПустаяСсылка)";
