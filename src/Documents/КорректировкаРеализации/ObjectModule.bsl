		...
		Если Не Проведен
			...
			Если СтрокиОтразитьНаПрочихДоходах.Количество() Тогда
				...
				Для Каждого Стр Из СтрокиОтразитьНаПрочихДоходах Цикл
					ТипНоменклатуры = СоответствиеНоменклатурыТипу.Получить(Стр.Номенклатура);
					
					Если ТипыНоменклатурыТовар.Найти(ТипНоменклатуры) <> Неопределено Тогда
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если ЗаполнятьСебестоимость
				И СтрокаТоваров.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.Вручную Тогда
...
Процедура ЗаполнитьДопКолонкиВидовЗапасовОприходования(ВидЗапасов)
	...
	Для Каждого СтрокаРасхождений Из Расхождения Цикл
		ТипНоменклатуры = СоответствиеНоменклатурыТипу.Получить(СтрокаРасхождений.Номенклатура);
		
		Если ВариантыОтражения.Найти(СтрокаРасхождений.ВариантОтражения) = Неопределено
			Или ТипыНоменклатурыТовар.Найти(ТипНоменклатуры) = Неопределено Тогда
...
Функция ВременныеТаблицыДанныхДокументаОприходование(МассивВариантовОтражения)
	...
	|	ВЫБОР
	|		КОГДА &ТипКорректировки = ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.ОбычнаяКорректировка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПродажи)
...
Процедура ЗаполнитьВидыЗапасовКорректировкаВыручкиКорректировкиДоВводаОстатков(МенеджерВременныхТаблиц)
	...
	Для Каждого СтрокаОприходования Из ТаблицаТоваровОприходования Цикл
		НоваяСтрока = ВидыЗапасовКорректировкаВыручки.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОприходования);
		
		НоваяСтрока.ВидЗапасов = ВидЗапасаОприходования;
	КонецЦикла;
