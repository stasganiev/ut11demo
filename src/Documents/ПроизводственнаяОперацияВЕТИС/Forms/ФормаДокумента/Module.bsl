...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьПредупреждениеРедактированияШапки(ЭтотОбъект);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИнтеграцияВЕТИСКлиент.ЭтоВыборЗаписиЖурнала(ВыбранноеЗначение) Тогда
		...
		НайденныеСтроки = Объект.Сырье.НайтиСтроки(Новый Структура("ЗаписьСкладскогоЖурнала", ВыбранноеЗначение.ЗаписьСкладскогоЖурнала));
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ЗаполнитьПредставлениеУпаковокПоСтроке(ЭтотОбъект, ТекущиеДанные);
...
Процедура СырьеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СырьеНадписьУпаковкиШтрихкоды" Тогда
		...
		ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияУпаковокШтрихкодовПоСтроке(ЭтотОбъект, Объект, ТекущиеДанные, ОписаниеОповещения);
...
Процедура СырьеЗаписьСкладскогоЖурналаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы = ИнтеграцияВЕТИСКлиент.ПараметрыФормыПодбораВыбораЗаписейЖурнала(
		Объект.ХозяйствующийСубъект, Объект.Предприятие, ТекущиеДанные);
...
&НаКлиенте
Процедура СырьеЗаписьСкладскогоЖурналаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
...
Процедура ОбработкаПодбораНоменклатурыЗавершениеНаСервере(ВыбранноеЗначение, ИмяТЧ)
	...
	Для каждого Строка Из ПараметрыЗаполнения.ДобавленныеСтроки Цикл
		...
		ЗаполнитьПредставлениеУпаковокПоСтроке(ЭтотОбъект, Строка);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИдентификаторПроизводственнойТранзакции");
	...
	СобытияФормИС.УстановитьУсловноеОформлениеПоляНоменклатура(ЭтотОбъект, "СырьеНоменклатура", "Сырье");
	СобытияФормИС.УстановитьУсловноеОформлениеПоляХарактеристика(ЭтотОбъект, "СырьеХарактеристика", "Сырье");
...
Процедура ОбработатьВводПериода(ВидПериода)
	...
	Если ВидПериода = "СрокГодности" Тогда
		ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияСрокаГодности(ЭтотОбъект, СтрокаТабличнойЧасти, ОбработчикОповещения, РедактированиеФормыНеДоступно, СписокДоступнойТочности);
	Иначе
		ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияДатыПроизводства(ЭтотОбъект, СтрокаТабличнойЧасти, ОбработчикОповещения, РедактированиеФормыНеДоступно, СписокДоступнойТочности);
	КонецЕсли;
...
Процедура ПродукцияПриИзмененииНаСервере(ИмяТЧ, ТекущаяСтрока, Пересчет = Истина)
	...
	СтрокаТабличнойЧасти = Объект[ИмяТЧ].НайтиПоИдентификатору(ТекущаяСтрока);
...
Процедура НоменклатураПриИзменении(ИмяТЧ, ТекущиеДанные)
	...
	Иначе
		...
		Если ИмяТЧ = "Товары" Тогда
			...
			Если Не ЗначениеЗаполнено(ТекущиеДанные.ЗаписьСкладскогоЖурнала) Тогда
				ПараметрыЗаполнения.ЗаполнитьПризнакСкоропортящаясяПродукция = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти = Неопределено)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЗаписьСкладскогоЖурнала) Тогда
		...
		ПриИзмененииЗаписиСкладскогоЖурналаНаСервере(СтрокаТабличнойЧасти.ПолучитьИдентификатор(), ДанныеВыбранногоСкладскогоЖурнала);
		...
		Иначе
			...
			КоличествоПриИзменении(СтрокаТабличнойЧасти);
			
			Если ДанныеВыбранногоСкладскогоЖурнала = Неопределено 
				Или ДанныеВыбранногоСкладскогоЖурнала.КоличествоВЕТИС < СтрокаТабличнойЧасти.КоличествоВЕТИС Тогда
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					ТекстСообщения,
					Объект.Ссылка,
					"Объект.Сырье[" + СтрокаТабличнойЧасти.ПолучитьИдентификатор() + "].КоличествоВЕТИС");
...
&НаСервере
Процедура ПриИзмененииЗаписиСкладскогоЖурналаНаСервере(ИдентификаторСтроки, ДанныеСкладскогоЖурнала = Неопределено)
	...
	ЗаполнитьСтрокуПоДаннымЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти, ДанныеСкладскогоЖурнала);
	
	ЗаполнитьПредставлениеУпаковокПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
...
Процедура ЗаполнитьСтрокуПоДаннымЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти, ДанныеСкладскогоЖурнала = Неопределено)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		ИнтеграцияИСПереопределяемый.ЗаполнитьСтатусыУказанияСерий(
			Объект,
			ПараметрыУказанияСерий.Сырье);
	КонецЕсли;
...
Процедура ОбновитьСтатусВЕТИС()
	...
	ИнтеграцияИСКлиентСервер.УстановитьДоступностьЭлементовГруппыФормыРекурсивно(Элементы.СтраницаСырье,
		НЕ РедактированиеФормыНеДоступно, Исключения);
...
Процедура ОткрытьПодборСерий(Текст, ИмяТЧ, СтандартнаяОбработка)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		ИнтеграцияИСКлиент.ОткрытьПодборСерий(ЭтотОбъект, ПараметрыУказанияСерий[ИмяТЧ], Текст, СтандартнаяОбработка);
...
Процедура ПодготовитьЗаполнитьУстановитьВидимостьСерий()
	...
	ПараметрыУказанияСерий = ИнтеграцияИС.ПараметрыУказанияСерийФормыОбъекта(Объект, Документы.ПроизводственнаяОперацияВЕТИС);
...
Процедура ЗаполнитьСтатусыУказанияСерийСервер()
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		ИнтеграцияИСПереопределяемый.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Сырье);
	КонецЕсли;
...
Процедура ЗаполнитьСтатусыУказанияСерийПриОкончанииРедактированияСтрокиТЧ(ТекущаяСтрокаИдентификатор,
																		ПараметрыУказанияСерийТабличнойЧасти,
																		КэшированныеЗначения)
...
Процедура ЗаполнитьСпискиВыбораНоменклатуры(ТекущаяСтрока, ИмяЭлемента, ИмяЭлементаВыбора = "НоменклатураДляВыбора")
	...
	НайденныеСтроки = ЭтотОбъект[ИмяЭлементаВыбора].НайтиСтроки(СтруктураПоиска);
...
Процедура ОткрытьФормуПодбораТоваров(ИмяТЧ)
	...
	ПараметрыПодбора = ИнтеграцияВЕТИСКлиентСервер.ПараметрыФормыПодбораТоваров(ЭтотОбъект, ПараметрыСерий);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьПредупреждениеРедактированияШапки(Форма)
	
	Авто       = ОтображениеПредупрежденияПриРедактировании.Авто;
	Отображать = ОтображениеПредупрежденияПриРедактировании.Отображать;
	
	Если Форма.Объект.Товары.Количество() И ЗначениеЗаполнено(Форма.Объект.ХозяйствующийСубъект) Тогда
		Форма.Элементы.ХозяйствующийСубъект.ОтображениеПредупрежденияПриРедактировании = Отображать;
	Иначе
		Форма.Элементы.ХозяйствующийСубъект.ОтображениеПредупрежденияПриРедактировании = Авто;
	КонецЕсли;
	
	Если Форма.Объект.Товары.Количество() И ЗначениеЗаполнено(Форма.Объект.Предприятие) Тогда
		Форма.Элементы.Предприятие.ОтображениеПредупрежденияПриРедактировании = Отображать;
	Иначе
		Форма.Элементы.Предприятие.ОтображениеПредупрежденияПриРедактировании = Авто;
	КонецЕсли;
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		КоличествоПриИзменении(НоваяСтрока);
