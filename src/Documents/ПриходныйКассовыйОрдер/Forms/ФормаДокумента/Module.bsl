...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.Свойство("Основание") Тогда
			ИсключаемыеТипыПриЗаполненииРеквизита = Новый Массив;
			ИсключаемыеТипыПриЗаполненииРеквизита.Добавить(Тип("Структура"));
			ИсключаемыеТипыПриЗаполненииРеквизита.Добавить(Тип("СправочникСсылка.ПодарочныеСертификаты"));
			Если ЗначениеЗаполнено(Параметры.Основание) И ИсключаемыеТипыПриЗаполненииРеквизита.Найти(ТипЗнч(Параметры.Основание)) = Неопределено Тогда
				Объект.Приложение = ОбщегоНазначенияУТ.ПолучитьПредставлениеДокумента(Параметры.Основание,
	...
	Элементы.РасшифровкаБезРазбиенияДатаПогашения.ТолькоПросмотр = Не ПравоДоступа(
		"Редактирование",
		Метаданные.Документы.ПриходныйКассовыйОрдер.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.ДатаПогашения);
...
Процедура КассаПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Объект.Валюта) Тогда
		Объект.Валюта = СтруктураРеквизитовКассы.Валюта;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущаяКасса) Тогда
		...
		Если ИзмениласьОрганизация Тогда
			...
			Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
				ТипРасчетов = ?(Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика")
...
Процедура КонтрагентПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ПринятоОт) Тогда
			...
			Объект.ПринятоОт = ДанныеКонтрагента.Представление;
...
Процедура ПодборПоОстаткам(Команда)
	...
	Если ЭтоПоступлениеИзДругойОрганизации Тогда
		ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("КассаОтправитель", НСтр("ru = 'Отправитель'"));
	Иначе
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.КассаОтправитель.Доступность = НЕ Объект.Исправление;
	Элементы.БанковскийСчет.Доступность = НЕ Объект.Исправление;
	Элементы.КассаККМ.Доступность = НЕ Объект.Исправление;
	Элементы.ПодотчетноеЛицо.Доступность = НЕ Объект.Исправление;
	...
	ДенежныеСредстваСервер.УстановитьПараметрыВыбораДоговораКредитаДепозита(Объект, Элементы.РасшифровкаПлатежаДоговорКредитаДепозита);
	ДенежныеСредстваСервер.УстановитьПараметрыВыбораДоговораКредитаДепозита(Объект, Элементы.РасшифровкаБезРазбиенияДоговорКредитаДепозита);
	УстановитьПараметрыВыбораРаспоряжениеНаПеремещениеДенежныхСредств();
...
&НаСервере
Процедура УстановитьПараметрыВыбораРаспоряжениеНаПеремещениеДенежныхСредств()

	МассивПараметровВыбора = Новый Массив;
	
	МассивПараметровВыбора.Добавить(
			Новый ПараметрВыбора("Отбор.Статус", Перечисления.СтатусыРаспоряженийНаПеремещениеДенежныхСредств.КОплате));
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнкассацияДенежныхСредствИзБанка Тогда
		МассивПараметровВыбора.Добавить(
			Новый ПараметрВыбора("Отбор.ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.СнятиеНаличныхДенежныхСредств));
	Иначе
		МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.ХозяйственнаяОперация", Объект.ХозяйственнаяОперация));
	КонецЕсли;
	
	Элементы.РаспоряжениеНаПеремещениеДенежныхСредств.ПараметрыВыбора =
...
Процедура Подключаемый_ДатаПриИзмененииЛокализация(Элемент)
	...
	ДатаПриИзмененииЛокализацияСервер();
	
КонецПроцедуры

&НаСервере
Процедура ДатаПриИзмененииЛокализацияСервер()
...
Процедура КонтрагентПриИзмененииСерверЛокализация()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		ТекущийПринятоОт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийКонтрагент, "НаименованиеПолное");
		Если Не ЗначениеЗаполнено(Объект.ПринятоОт) Или Объект.ПринятоОт = ТекущийПринятоОт Тогда
...
Процедура РаспределитьСуммуНаСервере()
	...
	СтрокиОсновнойДолг = РасшифровкаПлатежа.НайтиСтроки(
		Новый Структура("ТипСуммыКредитаДепозита", Перечисления.ТипыСуммГрафикаКредитовИДепозитов.ОсновнойДолг));
	...
	СтрокиПроценты = РасшифровкаПлатежа.НайтиСтроки(
		Новый Структура("ТипСуммыКредитаДепозита", Перечисления.ТипыСуммГрафикаКредитовИДепозитов.Проценты));
