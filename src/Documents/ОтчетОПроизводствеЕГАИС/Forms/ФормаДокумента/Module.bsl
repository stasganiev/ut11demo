...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, Перечисления.ВидыПродукцииИС.Пиво);
	
	ДопустимыеВидыПродукцииСырья = Новый Массив;
	ДопустимыеВидыПродукцииСырья.Добавить(Перечисления.ВидыПродукцииИС.Алкогольная);
	ДопустимыеВидыПродукцииСырья.Добавить(Перечисления.ВидыПродукцииИС.Пиво);
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, ДопустимыеВидыПродукцииСырья, "СырьеНоменклатура");
	...
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтотОбъект,   "СырьеХарактеристика", "Элементы.Сырье.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтотОбъект,   "СырьеУпаковка",       "Элементы.Сырье.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтотОбъект,   "СырьеСерия",          "Элементы.Сырье.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСХарактеристикой(ЭтотОбъект, "СырьеСерия", "Элементы.сырье.ТекущиеДанные.Характеристика");
	
	// Режим отладки
	ДоступенРежимОтладки = ОбщегоНазначения.РежимОтладки() И Пользователи.ЭтоПолноправныйПользователь();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПредварительноеСохранениеРезультатовСканированияАлкогольнойПродукции"
		И ТипЗнч(Параметр) = Тип("ДокументСсылка.ОтчетОПроизводствеЕГАИС")
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	|	1
	|ИЗ
	|	Документ.ОтчетОПроизводствеЕГАИС.Товары КАК СтарыеТовары
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ИнтеграцияЕГАИС.ЗаполнитьАлкогольнуюПродукцию(Объект.Сырье);
	Если ПрименитьРезультатСопоставленияДляШтрихкодовУпаковок Тогда
		Документы.ОтчетОПроизводствеЕГАИС.ПрименитьРезультатСопоставленияДляШтрихкодовУпаковок(ТекущийОбъект);
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ОтчетОПроизводствеЕГАИС", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура ДекорацияОтсканироватьПроверитьАлкогольнуюПродукциюНажатие(Элемент)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Сканирование алкогольной продукции возможно только в записанном ""Отчете о производстве ЕГАИС"". Записать?'");
	...
	ИначеЕсли Модифицированность Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru = '""Отчет о производстве ЕГАИС"" был изменен. %1?'"), ТекстПровестиЗаписать);
		ТребуетсяВопрос = Истина;
...
&НаКлиенте
Процедура ДекорацияОтсканироватьПроверитьПивоНажатие(Элемент)
	
	ИнтеграцияЕГАИСИСМПКлиент.ОткрытьФормуПроверкиИПодбораПиваИСМП(ЭтотОбъект, ПараметрыУказанияСерий);
...
Процедура ШапкаНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект, ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво"), СтандартнаяОбработка,,
...
Процедура ШапкаСерияПриИзменении(Элемент)
	...
	ПараметрыЗаполнения.ЗаполнитьАлкогольнуюПродукцию = Истина;
	ПараметрыЗаполнения.ЗаполнитьКрепость             = Истина;
	ПараметрыЗаполнения.ИмпортПроизводство            = Истина;
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	УстановитьОтборСырьяТекущейСтрокиТовары();
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент = Элементы.ТоварыАлкогольнаяПродукция Тогда
		...
		ЗаполнитьСписокВыбораАлкогольнойПродукции(ТекущиеДанные, Элемент.ТекущийЭлемент.Имя);
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	Если ДанныеУдаляемыхСтрок <> Неопределено Тогда
		ТоварыПослеУдаленияНаСервере(ДанныеУдаляемыхСтрок);
	КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		...
		ОбновитьПредставлениеСырья(ТекущиеДанные);
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект, ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво"), СтандартнаяОбработка,
		Новый ОписаниеОповещения("ПриВыбореНоменклатурыТовары", ЭтотОбъект),
		ИнтеграцияЕГАИСВызовСервера.РеквизитыАлкогольнойПродукцииДляСозданияНоменклатуры(ТекущиеДанные.АлкогольнаяПродукция));
...
Процедура СырьеПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент = Элементы.СырьеАлкогольнаяПродукция Тогда
...
Процедура СырьеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	СырьеТекущиеДанные = Элементы.Сырье.ТекущиеДанные;
	
	Если НоваяСтрока И Не Копирование Тогда
		ТоварыТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		Если ТоварыТекущиеДанные <> Неопределено Тогда
			СырьеТекущиеДанные.ИдентификаторСтрокиТовары = ТоварыТекущиеДанные.ИдентификаторСтроки;
		КонецЕсли;
	КонецЕсли;
	
	Если НоваяСтрока Тогда
		СырьеТекущиеДанные.ИдентификаторСтроки = Строка(Новый УникальныйИдентификатор);
		СырьеТекущиеДанные.КоличествоАкцизныхМарок = 0;
		АкцизныеМаркиКлиентСервер.ЗаполнитьИндексАкцизнойМарки(СырьеТекущиеДанные);
	КонецЕсли;
	
	ЗаполнитьСписокВыбораПродукции(ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура СырьеИдентификаторСтрокиТоварыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.Сырье.ТекущиеДанные;
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		ТекущиеДанные.ПредставлениеСтрокиТовары = Элементы.СырьеИдентификаторСтрокиТовары.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение).Представление;
	Иначе
		ТекущиеДанные.ПредставлениеСтрокиТовары = "";
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СырьеИдентификаторСтрокиТоварыОчистка(Элемент, СтандартнаяОбработка)
...
Процедура СырьеНоменклатураПриИзменении(Элемент)
	...
	Идентификатор = Элементы.Сырье.ТекущиеДанные.ПолучитьИдентификатор();
	
	ПриИзмененииНоменклатурыСырье(Идентификатор, КэшированныеЗначения);
...
Процедура СырьеНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		...
		Новый ОписаниеОповещения("ПриВыбореНоменклатурыСырье", ЭтотОбъект),
		ИнтеграцияЕГАИСВызовСервера.РеквизитыАлкогольнойПродукцииДляСозданияНоменклатуры(ТекущиеДанные.АлкогольнаяПродукция));
...
Процедура СырьеХарактеристикаПриИзменении(Элемент)
	...
	ПриИзмененииХарактеристикиСырье(Идентификатор, КэшированныеЗначения);
...
&НаКлиенте
Процедура ОтобразитьСырьеПоТекущейСтрокеТовары(Команда)
	
	СырьеТекущейСтроки = Не СырьеТекущейСтроки;
	Элементы.СырьеОтобразитьСырьеПоТекущейСтрокеТовары.Пометка = СырьеТекущейСтроки;
	Элементы.СырьеИдентификаторСтрокиТовары.Видимость = Не СырьеТекущейСтроки;
	УстановитьОтборСырьяТекущейСтрокиТовары();
...
Процедура ОткрытьПодбор(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ОтчетОПроизводствеЕГАИС.ФормаДокумента.Команда.ОткрытьПодбор");
...
Процедура ЗаписатьДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента.Записать");
...
Процедура ПровестиДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента.Провести");
...
Процедура ПровестиИЗакрыть(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента.ПровестиИЗакрыть");
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормЕГАИС.УстановитьУсловноеОформлениеПоляКоличество(ЭтотОбъект,
		"Объект.Сырье.НеупакованнаяПродукция",
		"СырьеКоличествоЕГАИС");
	
	СобытияФормЕГАИС.УстановитьУсловноеОформлениеПоляЕдиницаИзмеренияЕГАИС(ЭтотОбъект, "Сырье");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СырьеАлкогольнаяПродукция.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Сырье.Номенклатура");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СырьеИдентификаторСтрокиТовары.Имя);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.Сырье.ПредставлениеСтрокиТовары"));
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ПравоИзменения = ПравоДоступа("Изменение", Метаданные.Документы.ОтчетОПроизводствеЕГАИС);
	...
	ПараметрыУказанияСерий = ИнтеграцияИС.ПараметрыУказанияСерийФормыОбъекта(Объект, Документы.ОтчетОПроизводствеЕГАИС);
	...
	АкцизныеМаркиЕГАИС.ЗаполнитьСлужебныеРеквизиты(ЭтотОбъект,,"Сырье");
...
Процедура ОбновитьСтатус(ОбновитьСтатусПроверкиИПодбора = Истина)
	...
	Если Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется
		Или (СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется
			И Не Объект.ОтчетПроизводственнойЛинии)Тогда
...
Процедура ОткрытьПодборСерий(Текст, ИмяТЧ, СтандартнаяОбработка)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда
		...
		ИнтеграцияИСКлиент.ОткрытьПодборСерий(ЭтотОбъект, ПараметрыУказанияСерий, Текст, СтандартнаяОбработка, Элементы[ИмяТЧ].ТекущиеДанные);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ЗаполнитьСписокВыбораПродукции(Форма);
	ЗаполнитьПредставлениеСырья(Форма);
	...
	Форма.Элементы.СтраницаАкцизныеМарки.Видимость = Форма.ДоступенРежимОтладки И Не Форма.Объект.ОтчетПроизводственнойЛинии;
	Форма.Элементы.СтраницаОтчетПроизводственнойЛинии.Видимость = Форма.Объект.ОтчетПроизводственнойЛинии;
	Форма.Элементы.СырьеОтобразитьСырьеПоТекущейСтрокеТовары.Видимость = Не Форма.Объект.ОтчетПроизводственнойЛинии;
	Форма.Элементы.СырьеИдентификаторСтрокиТовары.Видимость = Не Форма.СырьеТекущейСтроки И Не Форма.Объект.ОтчетПроизводственнойЛинии;
...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	ОграничениеПоСтатусуЕГАИС = (Форма.СтатусЕГАИС <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик")
	                             И Форма.СтатусЕГАИС <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.ОшибкаПередачи"));
	...
	ЭтоЧерновик = Форма.СтатусЕГАИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик");
...
Процедура ОбновитьИнформациюОткрытияФормыСканирования()
	...
	Если Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.ПустаяСсылка() Тогда
		Элементы.ГруппаСканированиеИПроверкаАлкогольнойПродукцииПиво.Видимость = Ложь;
		Элементы.ГруппаСканированиеИПроверкаАлкогольнойПродукцииСырье.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаСканированиеИПроверкаАлкогольнойПродукцииПиво.Видимость = Истина;
	Элементы.ГруппаСканированиеИПроверкаАлкогольнойПродукцииСырье.Видимость = Истина;
	...
	Если СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		...
		КонецЕсли;
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик
		Или СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.ОшибкаПередачи) Тогда
		Если ПравоИзменения Тогда
	...
	Элементы.ДекорацияОтсканироватьПроверитьПиво.Заголовок = СтрокаОткрытиеФормыСканирования;
	Элементы.ГруппаИнформацияОСканированииВДругойФормеПиво.Видимость = ПравоИзменения
	...
	Если Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Продолжить подбор и проверку сырья'");
		Иначе
			ТекстНадписи = НСтр("ru = 'Промежуточные результаты подбора сырья'");
		КонецЕсли;
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик
		Или СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.ОшибкаПередачи) Тогда
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Подобрать и проверить сырье'");
		ИначеЕсли Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Завершено Тогда
			ТекстНадписи = НСтр("ru = 'Результаты подбора сырья'");
		КонецЕсли;
	Иначе
		ТекстНадписи = НСтр("ru = 'Результаты подбора сырья'");
	КонецЕсли;
	СтрокаОткрытиеФормыСканирования = Новый ФорматированнаяСтрока(ТекстНадписи,
	                                                              ИнтеграцияИСКлиентСервер.ШрифтГиперссылки(),
	                                                              ЦветаСтиля.ЦветГиперссылкиГосИС,,
	                                                              "ПроверитьКодыМаркировкиСырья");
	
	Элементы.ДекорацияОтсканироватьПроверитьАлкогольнуюПродукцию.Заголовок = СтрокаОткрытиеФормыСканирования;
	Элементы.ГруппаИнформацияОСканированииВДругойФормеСырье.Видимость = ПравоИзменения
		И Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется;
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента.ОткрытьФормуСканированияАлкогольнойПродукции");
...
&НаКлиенте
Процедура ЗаполнитьСписокВыбораАлкогольнойПродукции(ТекущаяСтрока, ИмяПоля)
	
	СписокВыбораНоменклатура = Элементы[ИмяПоля].СписокВыбора;
...
Процедура ВопросПриИзмененииОтчетПроизводственнойЛинииЗавершение(РезультатВопроса, Действия) Экспорт
	...
	Если Объект.ОтчетПроизводственнойЛинии Тогда
		...
		КонецЕсли;
		Если Не СырьеТекущейСтроки Тогда
			СырьеТекущейСтроки = Истина;
			Элементы.СырьеОтобразитьСырьеПоТекущейСтрокеТовары.Пометка = Истина;
			Элементы.СырьеИдентификаторСтрокиТовары.Видимость = Ложь;
			УстановитьОтборСырьяТекущейСтрокиТовары();
		КонецЕсли;
		ПрименитьУсловноеОформлениеСправка2();
...
Функция ОбработатьУпаковкиНаСервере(ВыделенныеСтроки, ПараметрыОбработки)
	...
	РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
		"Документы.ОтчетОПроизводствеЕГАИС.ПодготовитьДанныеЗаполненияОтчетПроизводственноЛинии",
...
&НаКлиенте
Процедура УстановитьОтборСырьяТекущейСтрокиТовары()
	
	Если Не СырьеТекущейСтроки Тогда
		Элементы.Сырье.ОтборСтрок = Неопределено;
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Отбор = Новый Структура("ИдентификаторСтрокиТовары", " ");
	Если ТекущиеДанные <> Неопределено Тогда
		Отбор.ИдентификаторСтрокиТовары = ТекущиеДанные.ИдентификаторСтроки;
	КонецЕсли;
	Элементы.Сырье.ОтборСтрок = Новый ФиксированнаяСтруктура(Отбор);
	
КонецПроцедуры

&НаСервере
Процедура УдалитьСырье(УдалениеТовары)
	
	Справки2 = Новый Массив;
	
	ТаблицаСырье = Объект.Сырье.Выгрузить();
	ТаблицаСырье.Колонки.Добавить("Удалить", Новый ОписаниеТипов("Булево"));
	ТаблицаСырье.Индексы.Добавить("ИдентификаторСтроки");
	Для Каждого Идентификатор Из УдалениеТовары.ИдентификаторыСтрок Цикл
		Для Каждого СтрокаСырье Из ТаблицаСырье.НайтиСтроки(Новый Структура("ИдентификаторСтроки", Идентификатор)) Цикл
			СтрокаСырье.Удалить = Истина;
			Если ЗначениеЗаполнено(СтрокаСырье.Справка2) Тогда
				Справки2.Добавить(СтрокаСырье.Справка2);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	ТаблицаСырье = ТаблицаСырье.Скопировать(Новый Структура("Удалить", Ложь));
	Объект.Сырье.Загрузить(ТаблицаСырье);
	
	Если Справки2.Количество() Тогда
		АкцизныеМаркиЕГАИС.УдалитьАкцизныеМаркиПоСправкам2(
			ЭтотОбъект,
			Справки2);
	КонецЕсли;
...
&НаСервере
Процедура ТоварыПослеУдаленияНаСервере(ДанныеУдаляемыхСтрок)
	УдалитьАкцизныеМарки(ДанныеУдаляемыхСтрок);
	УдалитьСырье(ДанныеУдаляемыхСтрок);
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПредставлениеСырья(ТекущиеДанные)
	
	ЭлементыПредставления = Новый Массив;
	ЭлементыПредставления.Добавить(ТекущиеДанные.Номенклатура);
	ЭлементыПредставления.Добавить(ТекущиеДанные.Характеристика);
	ЭлементыПредставления.Добавить(ТекущиеДанные.Серия);
	ЭлементыПредставления.Добавить(ТекущиеДанные.Количество);
	ПредставлениеСтроки = СтрСоединить(ЭлементыПредставления, ",");
	ТекущиеДанные.ПредставлениеСтрокиТовары = ПредставлениеСтроки;
	Для Каждого СтрокаСырье Из Объект.Сырье Цикл
		Если СтрокаСырье.ИдентификаторСтрокиТовары = ТекущиеДанные.ИдентификаторСтроки Тогда
			СтрокаСырье.ПредставлениеСтрокиТовары = ПредставлениеСтроки;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеСырья(Форма)
	
	ПредставленияСоответствие = Новый Соответствие;
	Для Каждого СтрокаТЧ Из Форма.Объект.Товары Цикл
		ЭлементыПредставления = Новый Массив;
		ЭлементыПредставления.Добавить(СтрокаТЧ.Номенклатура);
		ЭлементыПредставления.Добавить(СтрокаТЧ.Характеристика);
		ЭлементыПредставления.Добавить(СтрокаТЧ.Серия);
		ЭлементыПредставления.Добавить(СтрокаТЧ.Количество);
		ПредставлениеСтроки = СтрСоединить(ЭлементыПредставления, ",");
		СтрокаТЧ.ПредставлениеСтрокиТовары = ПредставлениеСтроки;
		ПредставленияСоответствие.Вставить(СтрокаТЧ.ИдентификаторСтроки, ПредставлениеСтроки);
	КонецЦикла;
	
	Для Каждого СтрокаТЧ Из Форма.Объект.Сырье Цикл
		СтрокаТЧ.ПредставлениеСтрокиТовары = ПредставленияСоответствие.Получить(СтрокаТЧ.ИдентификаторСтрокиТовары);
	КонецЦикла;
...
&НаКлиенте
Процедура ПриВыбореНоменклатурыТовары(Результат, ДополнительныеПараметры) Экспорт
...
#Область ОбработчикиСобытийЭлементовТаблицыСырьеСлужебные

&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСписокВыбораПродукции(Форма)
	
	Если Форма.СырьеТекущейСтроки Тогда
		Возврат;
	КонецЕсли;
	
	Форма.Элементы.СырьеИдентификаторСтрокиТовары.СписокВыбора.Очистить();
	Для Каждого СтрокаТовары Из Форма.Объект.Товары Цикл
		Форма.Элементы.СырьеИдентификаторСтрокиТовары.СписокВыбора.Добавить(СтрокаТовары.ИдентификаторСтроки, СтрокаТовары.ПредставлениеСтрокиТовары);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ПриИзмененииНоменклатурыСырье(Идентификатор, КэшированныеЗначения)
	
	ТекущаяСтрока = Объект.Сырье.НайтиПоИдентификатору(Идентификатор);
...
&НаКлиенте
Процедура ПриВыбореНоменклатурыСырье(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ПриИзмененииХарактеристикиСырье(Идентификатор, КэшированныеЗначения)
