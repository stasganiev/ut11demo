...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ПараметрыВидаПлана = ПолучитьПараметрыВидаПлана(Объект.ВидПлана, ОбновлятьПравило, 
														АдресПравилаЗаполнения, АдресПользовательскихНастроек);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ПланВнутреннихПотреблений.ФормаДокумента.Событие.ПередЗаписью");
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Планирование.ПроверитьСуществованиеУжеСозданныхПланов(Объект, "ПланВнутреннихПотреблений");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		СтруктураДействий.Вставить("ПоляГруппировки", "Номенклатура, Характеристика, Назначение, Упаковка, Склад");
		
		Планирование.ОбработкаПроверкиЗаполненияВФорме(ТоварыПоПериодам, СтруктураДействий, Отказ, 
			ПроверяемыеРеквизиты, Объект.КроссТаблица);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ПланВнутреннихПотреблений", , Объект.Ссылка);
...
Процедура ТоварыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ПланВнутреннихПотребленийПрисоединенныеФайлы") Тогда
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакХарактеристикиИспользуются", Новый Структура(
		"Номенклатура", "ХарактеристикиИспользуются"));
	СтруктураДействий.Вставить("ПроверитьХарактеристикуПоВладельцу",         ТекущаяСтрока.Характеристика);
	СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул",                    Новый Структура(
		"Номенклатура", "Артикул"));
	СтруктураДействий.Вставить("ЗаполнитьПризнакТипНоменклатуры",            Новый Структура(
		"Номенклатура", "ТипНоменклатуры"));
	СтруктураДействий.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу",      ТекущаяСтрока.Упаковка);

	СтруктураДействий.Вставить("НоменклатураПриИзмененииПереопределяемый", Новый Структура(
		"ИмяФормы, ИмяТабличнойЧасти", ИмяФормы, "Товары"));
...
Процедура ТоварыХарактеристикаПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ХарактеристикаПриИзмененииПереопределяемый", Новый Структура(
		"ИмяФормы, ИмяТабличнойЧасти", ИмяФормы, "Товары"));
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.Свойство("Отклонение") И ЗначениеЗаполнено(ТекущаяСтрока.Формула) Тогда
		РезультатВычисления = ПланированиеКлиент.ВычислитьПоФормуле(
			ТекущаяСтрока.Формула, ТекущаяСтрока, Неопределено, Неопределено);
...
Процедура ТоварыДатаОтгрузкиПриИзменении(Элемент)
	Если Объект.Замещающий
		...
		ДатаПоступленияНачалоПериода = ПланированиеКлиентСерверПовтИсп.РассчитатьДатуНачалаПериода(
			ТекущиеДанные.ДатаОтгрузки, Объект.Периодичность);
...
Процедура ТоварыПоПериодамНоменклатураПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("НоменклатураПриИзмененииПереопределяемый", Новый Структура(
		"ИмяФормы, ИмяТабличнойЧасти", ИмяФормы, "ТоварыПоПериодам"));
...
Процедура ПодобратьТовары(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ПланВнутреннихПотреблений.ФормаДокумента.Команда.ПодобратьТовары");
	...
	ОтборПоТипуНоменклатуры.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа"));
...
Процедура СкопироватьСтроки(Команда)
	...
	Если РаботаСТабличнымиЧастямиКлиент.ВыбранаСтрокаДляВыполненияКоманды(ТаблицаФормы) Тогда
		...
		РаботаСТабличнымиЧастямиКлиент.ОповеститьПользователяОКопированииСтрок(
			ТаблицаФормы.ВыделенныеСтроки.Количество());
	КонецЕсли;
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ДополнительныеПараметры.ТипПлана               = "ПланВнутреннихПотреблений";
	...
	ДополнительныеПараметры.ДоступныеТипыНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Ложь);
...
Процедура ЗаполнитьТовары(Команда)
	...
	ОтборПоказатели = ПланированиеКлиентСервер.ОтборДляЗаполненияПоказателей(
		ТипПлана, ПараметрыОтбора, ПараметрыВидаПлана);
		...
		Если Объект.КроссТаблица И АктивныеПериоды.Количество() > 1 Тогда
			...
			Если ТекущийЭлементИмя = "ТоварыПоПериодам" + "Количество_"+Период.ИмяКолонки Тогда
				...
				ОписаниеОповещения = Новый ОписаниеОповещения(
					"ПослеВопросаЗадатьПроизвольнуюФормулу", ЭтотОбъект, ДополнительныеПараметры);
		...
		Для каждого СтрокаТЧ Из ТоварыПоПериодам Цикл
			Для каждого Период Из ЭтотОбъект["Периоды"] Цикл
				Если НЕ Период.Активная Тогда
					Продолжить;
				КонецЕсли;
				СтрокаТЧ["КартинкаКомментарий_"+ Период.ИмяКолонки] = ЗначениеЗаполнено(
					СтрокаТЧ["Комментарий_"+ Период.ИмяКолонки]);
			КонецЦикла;
		КонецЦикла;
...
Процедура ПриИзмененииПериодовНаСервере(КорректироватьНачалоОкончаниеПериода = Ложь)
	...
	КонецЕсли;
	Планирование.ЗаполнитьТаблицуПериодов(ТаблицаПериоды, Объект.Периодичность, Объект.НачалоПериода,
		Объект.ОкончаниеПериода, ОтображатьНомерПериода, ЗамещенныеПериоды);
...
Процедура СценарийПриИзмененииСервер()
	...
	Если ОбновлятьВидПлана Тогда
		...
		Объект.ВидПлана = Планирование.ПолучитьВидПланаПоУмолчанию(
			Перечисления.ТипыПланов.ПланВнутреннихПотреблений, Объект.Сценарий);
...
Процедура ВидПланаПриИзмененииСервер(ОбновлятьПравило = Ложь)
	...
	ПараметрыВидаПлана = ПолучитьПараметрыВидаПлана(Объект.ВидПлана, 
		ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек);
...
Процедура УстановитьВидимость()
	...
	Элементы.ТоварыПоПериодамЗаполнитьКолонки.Видимость = ЗаполнятьСкладВТЧ ИЛИ ЗаполнятьНазначениеВТЧ;
	...
	Элементы.ТоварыНазначение.Видимость = ПланированиеПоНазначениям 
		И ЗаполнятьНазначениеВТЧ 
		И ИспользоватьОбособленноеОбеспечениеЗаказов;
	Элементы.ТоварыПоПериодамНазначение.Видимость = ПланированиеПоНазначениям 
		И ЗаполнятьНазначениеВТЧ 
		И ИспользоватьОбособленноеОбеспечениеЗаказов;
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьПодразделение, ЗаполнятьСклад, ЗаполнятьСкладВТЧ, 
		|ЗапретитьРедактированиеПравила, ЗаполнятьПоФормуле, Замещающий, 
	...
	Если ОбновлятьПравило Тогда
		АдресПравилаЗаполнения = Планирование.ПолучитьАдресПравилаЗаполненияПоУмолчанию(
			Перечисления.ТипыПланов.ПланВнутреннихПотреблений, ВидПлана, АдресПравилаЗаполнения);
		АдресПользовательскихНастроек = Планирование.ПолучитьАдресПользовательскихНастроекПоУмолчанию(
			Перечисления.ТипыПланов.ПланВнутреннихПотреблений, ВидПлана, АдресПользовательскихНастроек);
		СтруктураНастроек = Планирование.ПолучитьНастройкиПоУмолчанию(
			Перечисления.ТипыПланов.ПланВнутреннихПотреблений, ВидПлана);
...
Процедура СкопироватьВКроссТаблицу(КэшированныеЗначения)
	...
	Если Объект.КроссТаблица
		...
		ПланированиеКлиентСервер.ЗаполнитьКроссТаблицуИзТаблицы(ТоварыПоПериодам, 
			Объект.Товары, ПараметрыПреобразования);
...
Функция ЗагрузитьИзExcelНаСервере(МассивСтруктурТовары)
	...
	Для Каждого СтруктураТовар Из МассивСтруктурТовары Цикл
		...
		СтруктураТовар.Вставить("Номенклатура",   Справочники.Номенклатура.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторНоменклатура)));
		СтруктураТовар.Вставить("Характеристика", Справочники.ХарактеристикиНоменклатуры.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторХарактеристика)));
		СтруктураТовар.Вставить("Упаковка",       Справочники.УпаковкиЕдиницыИзмерения.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторУпаковка)));
		СтруктураТовар.Вставить("Склад",          Справочники.Склады.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторСклад)));
		СтруктураТовар.Вставить("Назначение",     Справочники.Назначения.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураТовар.ИдентификаторНазначение)));
...
Функция ЗагрузитьИзExcelНаКлиентеПродолжение(ДанныеЗагрузки)
	...
	Если ЕстьДобавленныеСтроки Тогда
		...
		ПараметрыЗагрузки.ПланироватьПоСумме = Ложь;
		...
		ПараметрыЗагрузки.ПараметрыОтбора.Вставить("ТипНоменклатуры", ОтборПоТипуНоменклатуры);
...
Процедура СкопироватьСтрокиНаСервере()
	...
	Если Объект.КроссТаблица Тогда
		РаботаСТабличнымиЧастями.СкопироватьСтрокиВБуферОбмена(ТоварыПоПериодам, 
			Элементы.ТоварыПоПериодам.ВыделенныеСтроки);
	Иначе
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	ПараметрыОтбора.Вставить(
		"Номенклатура.ТипНоменклатуры", ОтборПоТипуНоменклатуры);
	
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Количество,ДатаОтгрузки,Склад";
...
Процедура ИнициализироватьНастроекПриЧтенииНаСервере(ДокументОбъект)
	...
	Планирование.ИнициализироватьДопПараметры(ДополнительныеПараметрыТовары, 
		ДополнительныеПараметрыТоварыПоПериодам, ПараметрыИнициализации);
...
Процедура УстановитьЗначениеНастройки(ИмяНастройки, Настройка)
	...
	ЗначениеНастройкиИзХранилища = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиРаботыПользователя" 
		+ ТипПлана, "СтруктураНастроек", Настройка);
...
Процедура ПослеВопросаЗадатьПроизвольнуюФормулу(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	СтруктураДанных = Новый Структура(
		"Формула,ФормулаПредставление,Заголовок,ВключитьЗначение,МассивСтрок,ПериодИзменения");
...
Процедура УстановитьПроизвольнуюФормулу(СтруктураДанных)
	...
	ПараметрыОтбора = ПараметрыОтбора(ЭтаФорма);
	ПараметрыВидаПлана = ПараметрыВидаПлана(ЭтаФорма);
	ОтборПоказатели = ПланированиеКлиентСервер.ОтборДляЗаполненияПоказателей(ТипПлана, 
		ПараметрыОтбора, ПараметрыВидаПлана);
	...
	ПараметрыФормы.Вставить("ЗаголовокЗначения", ""+СтруктураДанных.ПараметрыРасшифровки.Отбор.Номенклатура
		+ ?(ЗначениеЗаполнено(СтруктураДанных.ПараметрыРасшифровки.Отбор.Характеристика)," ("
		+ СтруктураДанных.ПараметрыРасшифровки.Отбор.Характеристика + ")",""));
	ПараметрыФормы.Вставить("ПараметрыРасшифровкиОперативногоПланирования", СтруктураДанных.ПараметрыРасшифровки);
	ПараметрыФормы.Вставить("Формула",
		?(СтруктураДанных.Формула = ПланированиеКлиентСервер.ТекстУстановкиНовойФормулы(),"",
		СтруктураДанных.Формула));
	ПараметрыФормы.Вставить("Представление",
		?(СтруктураДанных.ФормулаПредставление = ПланированиеКлиентСервер.ТекстУстановкиНовойФормулы(),"",
		СтруктураДанных.ФормулаПредставление));
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПослеОткрытияКонструктораУстановитьПроизвольнуюФормулу", ЭтотОбъект, СтруктураДанных);
...
Процедура Подключаемый_УстановитьПроизвольнуюФормулу(Элемент, СтандартнаяОбработка)
	...
	СтруктураДанных = Новый Структура("Формула,ФормулаПредставление,ПараметрыРасшифровки,
		|ВключитьЗначение,МассивСтрок,ПериодИзменения");
	...
	Если Объект.КроссТаблица Тогда
		...
		Если АктивныеПериоды.Количество() > 1 Тогда
			...
			Если СтрНайти(ТекущийЭлементИмя, "ТоварыПоПериодам" + "Количество_") = 0
				И СтрНайти(ТекущийЭлементИмя, "ТоварыПоПериодамКартинкаКомментарий_") = 0 Тогда
				ПоказатьОповещениеПользователя(НСтр("ru = 'Активируйте колонку с периодом'"));
				Возврат;
			Иначе
				Для каждого Период Из АктивныеПериоды Цикл
...
Процедура ПослеОткрытияКонструктораУстановитьПроизвольнуюФормулу(Результат, СтруктураДанных) Экспорт
			...
			Для каждого Период Из ЭтаФорма.Периоды.НайтиСтроки(Новый Структура("Активная", Истина)) Цикл
				...
				Если ИмяПериода <> "ВсеПериоды" Тогда
					...
					Если ИмяПериода = Период.ИмяКолонки Тогда
						...
						РезультатВычисления = ПланированиеКлиент.ВычислитьПоФормуле(Результат.Формула, СтрокаТоваров,
							ЭтаФорма.Периоды, Период, Результат.Представление);
						...
						ПланированиеКлиентСервер.ОтобразитьЗначениеДополнительныхПараметров(ЭтотОбъект, 
							ПолучитьДопРеквизиты(ЭтотОбъект), "ТоварыПоПериодам");
						
					ИначеЕсли ЗначениеЗаполнено(СтрокаТоваров["Формула_" + Период.ИмяКолонки]) 
												И СтрокаТоваров["Отклонение_" + Период.ИмяКолонки] = 0 Тогда
						
						РезультатВычисления = ПланированиеКлиент.ВычислитьПоФормуле(
							СтрокаТоваров["Формула_" + Период.ИмяКолонки],
							СтрокаТоваров, ЭтаФорма.Периоды, Период, Результат.Представление);
						СтрокаТоваров["ФормулаПредставление_" + Период.ИмяКолонки] = РезультатВычисления.Представление;
						СтрокаТоваров["ФормулаВычисление_" + Период.ИмяКолонки] = РезультатВычисления.Вычисление;
						СтрокаТоваров["Количество_" + Период.ИмяКолонки] = РезультатВычисления.Результат;
	...
	Иначе
		...
		Для каждого ИДСтроки Из МассивСтрок Цикл
			...
			РезультатВычисления = ПланированиеКлиент.ВычислитьПоФормуле(Результат.Формула, СтрокаТоваров, Неопределено,
				Неопределено, Результат.Представление);
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	КонструкцияДляВставки = "[ПланыВнутреннихПотреблений] * 1.15";
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'План внутренних потреблений +15%'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);

	КонструкцияДляВставки = "Макс([ФактыВнутреннихПотреблений], [ПланыВнутреннихПотреблений])"
		+ ?(ИспользоватьПланыОстатков," - [ПланыОстатков]",
			?(ПланированиеПоНазначениям," - [ФактическиеНачальныеОстатки]"," - [СвободныеОстатки]"));
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'План внутренних потреблений с учетом корректировки по факту внутренних потреблений'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);

	КонструкцияДляВставки = "[ФактыВнутреннихПотреблений] / [ДоляДнейНаличияТовараНаОстатках]";
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Факт внутренних потреблений с учетом наличия товаров на остатках'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
...
Процедура ПриИзмененииКоличестваОтклонение(СтрокаТоваров)
	...
	Для каждого Период Из ЭтаФорма.Периоды.НайтиСтроки(Новый Структура("Активная", Истина)) Цикл
		...
		РезультатВычисления = ПланированиеКлиент.ВычислитьПоФормуле(СтрокаТоваров["Формула_" + Период.ИмяКолонки],
																	СтрокаТоваров, ЭтаФорма.Периоды, Период);
		
		Если Элементы.ТоварыПоПериодам.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "Количество_" + Период.ИмяКолонки Тогда
			СтрокаТоваров["Отклонение_" + Период.ИмяКолонки] = РезультатВычисления.Результат 
				- СтрокаТоваров["Количество_" + Период.ИмяКолонки];
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьСклад,
...
Функция ПараметрыОтбора(Форма)
	...
	ПараметрыОтбора = Новый Структура("Склад, Подразделение, ПланированиеПоНазначениям, Назначение");
...
Процедура ЗаполнитьДокумент(Настройки)
	...
	ДлительнаяОперация    = ЗаполнитьДокументНаСервере(Настройки, КэшированныеЗначения);
	...
	Для каждого Период Из ТаблицаПериоды Цикл
		Для Каждого СтрокаТоваров Из Форма.ТоварыПоПериодам Цикл
			...
			Если Форма.ИспользуютсяПланируемыеОстатки Тогда
				...
				Планирование.ВычислитьРассчитываемыеОперанды(СтрокаТоваров, Период, ПредыдущийПериод,
					НовыйПланОстатков, "_", ОсновнойОперандПлана, ЗнакКорректировки);
				Планирование.ВычислитьРассчитываемыеОперанды(СтрокаТоваров, Период, ПредыдущийПериод,
					НовыйПланОстатковСОтбором, "СОтбором_", ОсновнойОперандПланаСОтбором, ЗнакКорректировки);
			КонецЕсли;
			
			Если Период.Активная Тогда
				
				РезультатВычисления = Планирование.ВычислитьПоФормуле(Форма.СтруктураНастроек.Формула, СтрокаТоваров,
					Форма.Периоды, Период, Форма.СтруктураНастроек.ФормулаПредставление);
...
Функция ЗаполнитьДокументНаСервере(Знач ПараметрыЗадания, КэшированныеЗначения)
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"Документы.ПланВнутреннихПотреблений.ЗаполнитьДокумент",
...
Функция ПодготовитьПараметрыЗаполненияДокументаДляЗамещения(ПараметрыЗадания, КэшированныеЗначения)
	...
	ТипПлана = Перечисления.ТипыПланов.ПланВнутреннихПотреблений;
	...
	ПараметрыЗадания.Вставить("КлючОбщихНастроек",       "Документ.ПланВнутреннихПотреблений");
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(ПараметрыЗадания.КлючОбщихНастроек,
		"НастройкиФоновогоЗадания_"+ПараметрыЗадания.УникальныйИдентификатор, Настройки);
...
Функция ПолучитьРезультатЗаполненияНаСервере(КэшированныеЗначения)
	...
	Замер = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(
		"Документ.ПланВнутреннихПотреблений.Формы.ФормаДокумента.ПолучитьРезультатЗаполненияНаСервере");
...
Процедура ТоварыКомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения(
		"ТоварыКомментарийНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(
		Оповещение, 
		Элемент.ТекстРедактирования, 
		НСтр("ru = 'Комментарий ячейки плана'"));
		...
		Для каждого Период Из ЭтаФорма.Периоды.НайтиСтроки(Новый Структура("Активная", Истина)) Цикл
			Если Элементы.ТоварыПоПериодам.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "Комментарий_"+Период.ИмяКолонки Тогда
				...
				ДополнительныеПараметры = Новый Структура("ТекущаяСтрока, ИмяКолонки", 
					...
					Период.ИмяКолонки);
				Оповещение = Новый ОписаниеОповещения(
					"Подключаемый_НачалоВыбораКомментарияЗавершение", ЭтотОбъект, ДополнительныеПараметры);
				ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(
					Оповещение, 
					КомментарийКЯчейке, 
					НСтр("ru = 'Комментарий ячейки плана'"));
...
Процедура Подключаемый_НачалоВыбораКомментарияЗавершение(Знач ВведенныйТекст, Знач ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные["КартинкаКомментарий_"+ ДополнительныеПараметры.ИмяКолонки] = ЗначениеЗаполнено(
		ТекущиеДанные["Комментарий_"+ ДополнительныеПараметры.ИмяКолонки]);
	Модифицированность = Истина;
...
Процедура ТоварыПоПериодамПриАктивизацииЯчейкиНаКлиенте()
	...
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		Для каждого Период Из АктивныеПериоды Цикл
			Если Элемент.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "Количество_" + Период.ИмяКолонки Тогда
				ЯчейкаОтменена = ТекущиеДанные["Отменено_"+ Период.ИмяКолонки];
				КомментарийКЯчейке = ТекущиеДанные["Комментарий_"+ Период.ИмяКолонки];
...
Процедура ОбновитьПризнакЗамещения()
	...
	|		ПО ЗамещенныеПериоды.ЗамещенныйПериод = ПланВнутреннихПотребленийТовары.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	ПланВнутреннихПотребленийТовары.ДатаОтгрузки";
	
	Запрос.УстановитьПараметр("ЗамещенныеПериоды", ЗамещенныеПериоды.Выгрузить());
	Запрос.УстановитьПараметр("ПланВнутреннихПотребленийТовары", Объект.Товары.Выгрузить());
...
Функция ЗаполнитьЗамещенныеПериоды()
	...
	Результат = Планирование.ПолучитьЗамещенныеПериоды(ПараметрыЗамещения, "ПланВнутреннихПотреблений");
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ПланВнутреннихПотреблений.Товары";
