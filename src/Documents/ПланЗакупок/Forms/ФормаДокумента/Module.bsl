...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ЗаполнитьЗначенияСвойств(Объект, ПараметрыВидаПлана,"ЗаполнятьПланОплат,ЗаполнятьПоФормуле,Замещающий,ЗаполнятьПоДефициту");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Запрос.Текст =  
		...
		|	Товары.ДатаЗаказа      КАК ДатаЗаказа,
		...
		|	ДвиженияДокумента.ДатаЗаказа     КАК ДатаЗаказа,
		...
		|		ПО ДвиженияДокумента.Номенклатура = СпрНоменклатура.Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДвиженияДокумента.Период,
		|	ДвиженияДокумента.ДатаЗаказа,
		...
		|			И ДвиженияДокумента.ДатаЗаказа = ПланыЗакупок.ДатаЗаказа
		...
		|			ПО ПланыЗакупок.ПланЗакупок = ЗамещениеПланов.ЗамещающийПлан
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ЗамещениеПланов.ЗамещающийПлан ЕСТЬ NULL
		|				ТОГДА ПланыЗакупок.Регистратор <> &Ссылка
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
	...
	Запрос.УстановитьПараметр("Сценарий", ТекущийОбъект.Сценарий);
	Запрос.УстановитьПараметр("Товары", ТекущийОбъект.Товары.Выгрузить(, "ДатаПоступления, ДатаЗаказа, Номенклатура, Характеристика, Партнер, 
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, ,, Отказ);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		СтруктураДействий.Вставить("ПоляГруппировки", "Номенклатура, Характеристика, Назначение, Упаковка, Склад, Партнер, Соглашение, ВидЦеныПоставщика");
...
Процедура ТоварыДатаПоступленияПриИзменении(Элемент)
	...
	ТекущиеДанные.ДатаЗаказа = ТоварыСрокИсполненияЗаказаПриИзмененииНаСервере(ТекущиеДанные.ДатаПоступления,
																			ТекущиеДанные.СрокИсполненияЗаказа);
КонецПроцедуры

&НаКлиенте
Процедура ТоварыСрокИсполненияЗаказаПриИзменении(Элемент)
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
...
Функция ТоварыСрокИсполненияЗаказаПриИзмененииНаСервере(ДатаПоступления, СрокИсполненияЗаказа)
	Возврат Справочники.СпособыОбеспеченияПотребностей.ОпределитьДатуЗаказаПоДатеПоставки(ДатаПоступления,
																						СрокИсполненияЗаказа);
КонецФункции

&НаКлиенте
Процедура ТоварыДатаЗаказаПриИзменении(Элемент)
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	ТекущиеДанные.СрокИсполненияЗаказа = 0;
КонецПроцедуры
...
Процедура ПодобратьТовары(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ПланЗакупок.ФормаДокумента.Команда.ПодобратьТовары");
	...
	Если Объект.ПланироватьПоСумме 
		...
		ОчиститьСообщения();
		Если ИспользоватьСоглашенияСПоставщиками И Не ЗначениеЗаполнено(Объект.Соглашение) Тогда
...
&НаКлиенте
Процедура ЗаполнитьПоставщикаПоЗначению(Команда)
	
	ВыделенныеСтроки = ?(Объект.КроссТаблица, Элементы.ТоварыПоПериодам.ВыделенныеСтроки,Элементы.Товары.ВыделенныеСтроки);
	ТаблицаТовары = ?(Объект.КроссТаблица, ТоварыПоПериодам, Объект.Товары);
	
	СтруктураОтбора = Новый Структура("ЭтоГруппа, Поставщик", Ложь, Истина);
	СтруктураПараметров = Новый Структура("Отбор, ВыборГруппИЭлементов", СтруктураОтбора, ИспользованиеГруппИЭлементов.Элементы);
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ДополнительныеПараметры.ТипПлана               = "ПланЗакупок";
	...
	ДополнительныеПараметры.ИспользуютсяВидыЦен    = ЗаполнятьПартнера ИЛИ ЗаполнятьПартнераВТЧ;
	...
	ДополнительныеПараметры.Партнер = ?(ЗаполнятьПартнера, Объект.Партнер, Неопределено);
...
Процедура УстановитьВидимость()
	...
	Элементы.ТоварыЗаполнитьПоставщикаПоЗначению.Видимость = ЗаполнятьПартнераВТЧ;
	Элементы.ТоварыПоПериодамЗаполнитьПоставщикаПоЗначению.Видимость = ЗаполнятьПартнераВТЧ;
	...
	Элементы.ТоварыЗаполнитьДатыЗаказа.Видимость = Не Объект.ЗаполнятьПоДефициту;
...
Процедура ДобавитьТовары(ТаблицаТоваров, КэшированныеЗначения = Неопределено, ЗагрузкаИзФайла = Ложь)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(ТекущаяСтрока.ДатаПоступления) Тогда
				ТекущаяСтрока.ДатаПоступления = Объект.НачалоПериода;
			КонецЕсли;
...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения, ЭтотОбъект)
	...
	Если ЭтотОбъект.КроссТаблица
		...
		ЗаполнитьДатыЗаказаНаСервере(ЭтотОбъект);
...
Функция ПараметрыПреобразованияКроссТаблицы()
	...
	Если ЗаполнятьПартнераВТЧ Тогда
		ПоляГруппировки = ПоляГруппировки + ", Партнер, ВидЦеныПоставщика";
	КонецЕсли;
	...
	Иначе
		...
		Если Объект.КроссТаблица Тогда
			...
			Для каждого Период Из ТаблицаПериоды Цикл
				...
				ЭтапОплаты.ВариантОплаты = ПредопределенноеЗначение("Перечисление.ВариантыКонтроляОплатыПоставщику.ПредоплатаДоПоступления");
...
Процедура ЗаполнитьЭтапыОплатыПоШаблонуСервер(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	Если Объект.КроссТаблица Тогда
		...
		Для каждого Период Из ТаблицаПериоды Цикл
			...
			Для каждого ЭтапОплаты Из ПланОплатыКопия Цикл
				...
				НовСтр.ДатаПоступления = Период.ДатаНачала;
	...
	Иначе
		...
		СуммыПоДатам = Объект.Товары.Выгрузить(Новый Структура("Отменено", Ложь),"ДатаПоступления, ДатаЗаказа, Замещен, Сумма");
		СуммыПоДатам.Свернуть("ДатаПоступления, ДатаЗаказа, Замещен", "Сумма");
		СуммыПоДатам.Сортировать("ДатаПоступления");
		...
		Для каждого СуммаНаДату Из СуммыПоДатам Цикл
			...
			ПараметрыЗаполнения.ЕстьДатаПереходаПраваСобственности = Ложь;
			ПараметрыЗаполнения.Вставить("ДатаОтгрузки", СуммаНаДату.ДатаПоступления);
			ПараметрыЗаполнения.Вставить("ДатаЗаказа", СуммаНаДату.ДатаЗаказа);
			ПараметрыЗаполнения.Вставить("ШаблонГрафика", ШаблонГрафика);
			ПараметрыЗаполнения.Вставить("Календарь", Календарь);
			ПараметрыЗаполнения.Вставить("СуммаОплаты", СуммаОплатыКраспределению);
			...
			Для каждого ЭтапОплаты Из ПланОплатыКопия Цикл
				...
				НовСтр.Замещен = СуммаНаДату.Замещен;
				НовСтр.ДатаПоступления = СуммаНаДату.ДатаПоступления;
...
Процедура ДобавитьЭтапОплатыПоУмолчаниюСервер()
	...
	Для каждого СуммаНаДату Из СуммыПоДатам Цикл
		...
		ЭтапОплаты = Объект.ПланОплаты.Добавить();
		ЭтапОплаты.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыПоставщику.ПредоплатаДоПоступления;
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	Иначе
		...
		КолонкиПоиска.Добавить(НСтр("ru='Дата заказа'"),"ДатаЗаказа");
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Количество,Цена,
		|ДатаПоступления,Партнер,Соглашение,Склад";
...
&НаКлиенте
Процедура ЗаполнитьДатыЗаказа(Команда)
	ЗаполнитьДатыЗаказаНаСервере();
КонецПроцедуры
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	КонструкцияДляВставки = "[ПланыЗакупок] * 1.15";
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'План закупок +15%'");
	...
	КонструкцияДляВставки = ?(ИспользоватьПланыПродаж, "[ПланыПродаж]", "[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]")
		+ ?(ИспользоватьСборкиРазрборки,
			" + " + ?(ИспользоватьПланыСборки, "[ПланыСборкиКомплектующие]", "[ФактыРазборкиКомплектующие]"), "")
		+ ?(ИспользоватьСборкиРазрборки,
			" + " + ?(ИспользоватьПланыСборки, "[ПланыРазборкиКомплекты]", "[ФактыСборкиКомплекты]"), "")
		+ ?(ИспользоватьПланыОстатков, " - [ПланОстатков]",
			?(ПланированиеПоНазначениям, " - [ФактическиеНачальныеОстатки]", " - [СвободныеОстатки]"));
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Минимальное обеспечение планов'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	
	КонструкцияДляВставки = "Макс([ФактыЗакупок], [ПланыЗакупок])"
		+ ?(ИспользоватьПланыОстатков, " - [ПланОстатков]",
			?(ПланированиеПоНазначениям, " - [ФактическиеНачальныеОстатки]", " - [СвободныеОстатки]"));
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Закупки прошедшего периода с учетом остатка'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	
	КонструкцияДляВставки = ?(ИспользоватьПланыОстатков, "?([ПланОстатков] = 0 И ",
		?(ПланированиеПоНазначениям,"?([ФактическиеНачальныеОстатки] = 0 И ","?([СвободныеОстатки] = 0 И "))
		+ ?(ИспользоватьПланыПродаж,"[ПланыПродаж]","[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]") + "> 0, "
		+ ?(ИспользоватьПланыПродаж,"[ПланыПродаж]","[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]") + ", 0)";
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Пополнение остатка'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	
	КонструкцияДляВставки = "(" + ?(ИспользоватьПланыПродаж, "[ПланыПродаж]","[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]")
		+ ?(ИспользоватьПланыОстатков, " - [ПланОстатков]) * 1.15",
			?(ПланированиеПоНазначениям, " - [ФактическиеНачальныеОстатки]) * 1.15", " - [СвободныеОстатки]) * 1.15"));
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Продажи за вычетом остатка +15%'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	
	КонструкцияДляВставки = ?(ИспользоватьПланыПродаж, "[ПланыПродаж]", "[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]") + " * 1.15";
	Идентификатор = КонструкцияДляВставки;
	Представление = НСтр("ru = 'Увеличение продаж прошлых периодов на 15%'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
...
Процедура УстановитьПериодЗапроса(ПараметрыЗапроса)
	...
	Иначе
		...
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		ТаблицаПериоды = ЭтаФорма.Периоды; // см. Планирование.ТаблицаПериоды
		МассивСтрок = ТаблицаПериоды.НайтиСтроки(Новый Структура("ДатаНачала, Активная", ТекущиеДанные.ДатаПоступления, Истина));
	...
	Иначе
		...
		Если ДополнительныеПоля <>  Неопределено Тогда
			...
			Если Не РаботаСФормуламиПроверитьФормулу Тогда
				...
				СтруктураНастроек.Вставить("Формула","[ПланыЗакупок] * 1.15");
				СтруктураНастроек.Вставить("ФормулаПредставление",НСтр("ru='Увеличение планов закупок прошлых периодов на 15%'"));
				СтруктураНастроек.Вставить("ФильтроватьНезаполненныеСтроки", Ложь);
...
&НаСервере
Процедура ЗаполнитьДатыЗаказаНаСервере(ЭтотОбъект = Неопределено)
	...
	|ВЫБРАТЬ
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(Соглашения.СрокПоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа) КАК СрокИсполненияЗаказа,
	|
	|	ВЫБОР
	|		КОГДА НЕ ДатаЗаказа.ДатаГрафика ЕСТЬ NULL
	|			ТОГДА ДатаЗаказа.ДатаГрафика
	|		КОГДА НЕ ДатаЗаказаВПрошломГоду.ДатаГрафика ЕСТЬ NULL
	|			ТОГДА ДатаЗаказаВПрошломГоду.ДатаГрафика
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(Товары.ДатаПоступления, ДЕНЬ, - ЕСТЬNULL(Соглашения.СрокПоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа))
	...
	|		ПО ДатаПоставки.Календарь = &Календарь
	|		 И ДатаПоставки.ДатаГрафика = Товары.ДатаПоступления
	|		 И ДатаПоставки.Год = ГОД(Товары.ДатаПоступления)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КалендарныеГрафики КАК ДатаЗаказа
	|		ПО ДатаЗаказа.Календарь = &Календарь
	|		 И ДатаЗаказа.Год = ГОД(Товары.ДатаПоступления)
	|		 И ДатаЗаказа.КоличествоДнейВГрафикеСНачалаГода = ДатаПоставки.КоличествоДнейВГрафикеСНачалаГода -
	|			ЕСТЬNULL(Соглашения.СрокПоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа)
	...
	|		 И КонецПрошлогоГода.Год = ГОД(Товары.ДатаПоступления) - 1
	|		 И КонецПрошлогоГода.ДатаГрафика = ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(Товары.ДатаПоступления, ГОД), ДЕНЬ, -1)
	...
	|		 И ДатаЗаказаВПрошломГоду.Год = ГОД(Товары.ДатаПоступления) - 1
	|		 И ДатаЗаказаВПрошломГоду.КоличествоДнейВГрафикеСНачалаГода = КонецПрошлогоГода.КоличествоДнейВГрафикеСНачалаГода -
	|			ЕСТЬNULL(Соглашения.СрокПоставки, СпрСпособОбеспечения.СрокИсполненияЗаказа) + ДатаПоставки.КоличествоДнейВГрафикеСНачалаГода
	...
	Если ЗаполнятьСкладВТЧ Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Склад", "Товары.Склад");
	...
	Если ЗаполнятьСоглашениеВТЧ Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Соглашение", "Товары.Соглашение");
	ИначеЕсли ЗаполнятьСоглашение Тогда 
		Запрос.УстановитьПараметр("Соглашение", ЭтотОбъект.Соглашение);
	Иначе
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(Товары[Выборка.НомерСтроки - 1], Выборка, "СрокИсполненияЗаказа,ДатаЗаказа");
	КонецЦикла;
	
	ЭтотОбъект.Товары.Загрузить(Товары);
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад, 
		|ПланированиеПоНазначениям, ЗаполнятьНазначениеВТЧ, ЗаполнятьПартнераВТЧ, ЗаполнятьСоглашениеВТЧ, ЗаполнятьСкладВТЧ");
...
Процедура ПодготовитьПараметрыЗаполненияДокументаПоФормуле(ПараметрыЗадания, КэшированныеЗначения)
	...
	Если Объект.ПланироватьПоСумме 
		...
		Если СтруктураНастроек.ВариантЗаполненияЦен = "ЦеныНоменклатурыПоставщиков" Тогда
		...
		ИначеЕсли (СтруктураНастроек.ВариантЗаполненияЦен = "ЦеныНоменклатуры" 
	...
	Если ИспользуютсяПланируемыеОстатки Тогда
		...
		Для Каждого Строка Из Таблица Цикл
			Если Строка.ПланируемыеНачальныеОстатки<>0 Тогда
				Если ПериодыПлановОстатка.НайтиПоЗначению(Строка.ДатаПоступления) = Неопределено Тогда
					ПериодыПлановОстатка.Добавить(Строка.ДатаПоступления);
				КонецЕсли;
			КонецЕсли;
			
			Если ИспользуютсяПланируемыеОстаткиСОтбором
				И Строка.ПланируемыеНачальныеОстаткиСОтбором<>0 Тогда
				Если ПериодыПлановОстаткаСОтбором.НайтиПоЗначению(Строка.ДатаПоступления) = Неопределено Тогда
					ПериодыПлановОстаткаСОтбором.Добавить(Строка.ДатаПоступления);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		Для каждого Период Из АктивныеПериоды Цикл
			Если Элемент.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "Цена_"+Период.ИмяКолонки 
				...
				ИЛИ Элемент.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "КартинкаКомментарий_"+Период.ИмяКолонки 
...
Процедура СкорректироватьПотребностьЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.НоменклатураКорректировка) 
		...
		Иначе
			...
			НоваяСтрока.ДатаПоступления = ДополнительныеПараметры.Период;
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ПланЗакупок.Товары";
