...
Функция ОбработатьОтветНаЗапросВозможностиПеревозкиПартииВРамкахРегионализации(ДанныеДокумента, ХозяйствующийСубъект, Предприятие, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатДобавленияЗаписей.ВходящееСообщение.НовоеСообщение Тогда
		...
		ЕстьОшибкиВСДДолженБытьВСостоянииОформлен = Ложь;
		Если ЕстьОшибкиВСДДолженБытьВСостоянииОформлен(ДанныеДокумента.Ошибки) Тогда
			ЕстьОшибкиВСДДолженБытьВСостоянииОформлен = Истина;
			ПодготовитьЗапросНаОбновлениеВСД(ДанныеДокумента.Документ);
		КонецЕсли;
		ПараметрыОбновленияСтатуса.ЕстьОшибкиВСДДолженБытьВСостоянииОформлен = ЕстьОшибкиВСДДолженБытьВСостоянииОформлен;
...
//  Операция - ПеречислениеСсылка.ВидыОперацийВЕТИС - Операция обмена с ВЕТИС
//  ДополнительныеПараметры - см. ИнтеграцияВЕТИС.ПараметрыОбновленияСтатуса.
// 
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * НовыйСтатус - ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - Новый статус.
...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийВЕТИС.ОтветНаЗапросОформленияВходящейПартии Тогда
		...
		Если Не ИнтеграцияВЕТИС.ОшибкаПараметраОбновленияСтатуса(ДополнительныеПараметры,"ЕстьОшибкиВСДДолженБытьВСостоянииОформлен") Тогда
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - Новый статус.
//
...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	// Запросы восстановления
	ПротоколОбменаИС.ДобавитьОперациюВПоследовательность(Таблица, -211,
		Исходящий,
		Перечисления.ВидыОперацийВЕТИС.ЗапросИзмененныхВСД);
	ПротоколОбменаИС.ДобавитьОперациюВПоследовательность(Таблица, -211,
		Входящий,
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - Статус по-умолчанию.
//
...
Функция КонечныеСтатусы(ТребуетсяПовторноеОформление = Истина) Экспорт
	...
	Если Не ТребуетсяПовторноеОформление Тогда
		Статусы.Добавить(Перечисления.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС.ВСДПогашены);
		Статусы.Добавить(Перечисления.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС.ВСДПогашеныЧерезWeb);
	КонецЕсли;
...
Функция ЗапросПроверкиВозможностиПеревозкиВРамкахРегионализацииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Пока Маршруты.Следующий() Цикл
		...
		Иначе
			routePoint.location = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(routePoint, "location");
...
//
// Возвращаемое значение:
//   Неопределено, Структура - См. ИнтеграцияВЕТИСКлиентСервер.СтруктураИзменения
...
Процедура ОбработатьВСДПакетом(СообщенияXML, ДокументСсылка, ДанныеДокумента, Знач НастройкаОбмена, Знач ВспомогательныеДанные)
	...
	Для каждого НомерПакета Из НомераПакетов Цикл
		...
		Если ТекущийНомерПакета <> НомерПакета Тогда
			...
			ДокументXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениСвойства(
...
Функция ЗапросНаОформлениеВходящейПартииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Для каждого ДанныеСтроки Из ТоварыОбрабатываемыеПоОтдельности Цикл
		...
		Если ЗначениеЗаполнено(Шапка.НомерАктаНесоответствия) Тогда
			...
			АктНесоответствия = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "discrepancyReport");
			...
			АктНесоответствия.reason = ПричинаНесоответствия;
...
Функция ДанныеВходящейПартии(ДокументСсылка, РазделитьПоПакетам = Ложь)
	...
	|	ТаблицаДокумента.ВетеринарноСопроводительныйДокумент.МестоДляКарантинирования                           КАК ВСД_МестоДляКарантинирования,
	|	ТаблицаДокумента.ВетеринарноСопроводительныйДокумент.КоличествоДнейДляКарантинирования                  КАК ВСД_КоличествоДнейДляКарантинирования,
...
Функция ЕстьОшибкиВСДДолженБытьВСостоянииОформлен(Ошибки)
	
	Если Ошибки = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	КодОшибки_MERC72268 = "MERC72268"; // Ветеринарно-сопроводительный документ должен быть в состоянии "оформлен" (т.е. не погашен)
	КодОшибки_MERC72269 = "MERC72269"; // Ветеринарно-сопроводительный документ должен быть в состоянии "оформлен" (т.е. не аннулирован).
	КодОшибки_MERC72270 = "MERC72270"; // Ветеринарно-сопроводительный документ должен быть в состоянии "оформлен"
	
	Для Каждого Ошибка Из Ошибки Цикл
		Если Ошибка.Код = КодОшибки_MERC72268
			Или Ошибка.Код = КодОшибки_MERC72269
			Или Ошибка.Код = КодОшибки_MERC72270 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовитьЗапросНаОбновлениеВСД(ДокументСсылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТЧ.ВетеринарноСопроводительныйДокумент КАК ВетеринарноСопроводительныйДокумент,
	|	ТЧ.ВетеринарноСопроводительныйДокумент.Идентификатор КАК Идентификатор,
	|	ТЧ.ВетеринарноСопроводительныйДокумент.ГрузоотправительХозяйствующийСубъект КАК ГрузоотправительХозяйствующийСубъект,
	|	ТЧ.ВетеринарноСопроводительныйДокумент.ГрузополучательПредприятие КАК ГрузополучательПредприятие,
	|	ТЧ.ВетеринарноСопроводительныйДокумент.ГрузополучательХозяйствующийСубъект КАК ГрузополучательХозяйствующийСубъект,
	|	ТЧ.ВетеринарноСопроводительныйДокумент.ГрузоотправительПредприятие КАК ГрузоотправительПредприятие
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ТЧ
	|ГДЕ
	|	ТЧ.ВетеринарноСопроводительныйДокумент <> ЗНАЧЕНИЕ(Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ПустаяСсылка)
	|	И ТЧ.Ссылка = &Ссылка";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ИнтеграцияВЕТИС.ПодготовитьЗапросНаОбновлениеВетеринарноСопроводительногоДокумента(
			ВыборкаДетальныеЗаписи, Новый УникальныйИдентификатор);
