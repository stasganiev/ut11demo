...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьОбщуюФорму") Тогда
		...
		ИначеЕсли ИмяТабличнойЧасти = "МаршрутВозврата" Тогда
			...
			ПараметрыФормы.ИмяПоляВидПродукции = "ВидПродукцииВСД";
			...
			ПараметрыФормы.ХозяйствующийСубъект = Объект.ГрузополучательХозяйствующийСубъект;
			ПараметрыФормы.ЭтоМаршрутВозврата = Истина;
...
Функция НажатиеНавигационнойСсылкиТребуетЗаписиДокумента(НавигационнаяСсылкаФорматированнойСтроки)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьФормуРегионализации"
		...
		ИЛИ НавигационнаяСсылкаФорматированнойСтроки = "УказатьНомераТранспортныхСредств"
		ИЛИ НавигационнаяСсылкаФорматированнойСтроки = "ПоказатьПричинуОшибки" Тогда
...
Процедура ОбновитьСтатусВЕТИС(Форма, УстановитьПараметрыОбновленияСтатуса = Ложь)
	...
	Иначе
		Элементы.ТоварыГруппаОтображениеРегионализации.Видимость = Форма.ТребуетсяПроверкаРегионализации;
		...
		Элементы.ТоварыБезРазбиенияРезультатПроверкиПравилРегионализации.Видимость = Форма.ТребуетсяПроверкаРегионализации;
	КонецЕсли;
	
	Если Форма.ТребуетсяПроверкаРегионализации Тогда
...
Функция ПараметрыДополненияСтатусаВЕТИС(Форма)
	...
	Форма.ТребуетсяПроверкаРегионализации = ЕстьПеремещениеМеждуРегионами;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Для Каждого РеквизитТТН Из РеквизитыТТН Цикл
		КлючевыеПоляОчисткиВСД.Удалить(РеквизитТТН.Ключ);
	КонецЦикла;
	...
	Если СтруктураРеквизитов.Свойство("ПереключательТТН")
		...
		Если Не ЗначениеЗаполнено(Объект.ТипТТН) И Объект.ТТНУказан Тогда
			Объект.ТипТТН = ПредопределенноеЗначение("Перечисление.ТипыДокументовВЕТИС.ТТН");
		ИначеЕсли ЗначениеЗаполнено(Объект.ТипТТН) И Не Объект.ТТНУказан Тогда
	...
	Если СтруктураРеквизитов.Свойство("Регионализация")
		...
		РеквизитПоказываем = Объект.ГрузополучательПредприятие <> Объект.ГрузоотправительПредприятие
			И Не Объект.ГашениеНаСВХ И Не ЭтоИмпорт И Объект.Товары.Итог("ВозвращаемоеКоличествоВЕТИС") <> 0
			И Форма.ТребуетсяПроверкаРегионализации;
	...
	Если Инициализация Или Не ЗначениеЗаполнено(Объект.НомерАктаНесоответствия) И СтруктураРеквизитов.Свойство("ЕстьОтклонения") Тогда
		...
		Для каждого Стр Из Объект.Товары Цикл
			...
			Если Стр.ДанныеВСД <> Неопределено Тогда
				Для Каждого РеквизитТТН Из РеквизитыТТН Цикл
					Стр.ЕстьОтклонения = Стр.ЕстьОтклонения Или Стр.ДанныеВСД[РеквизитТТН.Ключ]<>Форма.Объект[РеквизитТТН.Ключ];
					ЕстьОтклонения = ЕстьОтклонения Или Стр.ЕстьОтклонения;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	//Для случая уточнения ВСД на несколько характеристик, в форме списка
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ВыбраноХарактеристика");
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения)
	...
	РеквизитыТТН = РеквизитыТТН();
	Для Каждого РеквизитТТН Из РеквизитыТТН Цикл
		РеквизитыВлияющиеНаВСД.Удалить(РеквизитТТН.Ключ);
	КонецЦикла;
	...
	Если РеквизитыТТН.Свойство(ИмяОбновляемогоПоля) Тогда
		МассивПолейДляНастройки.Добавить("ЕстьОтклонения");
	КонецЕсли;
...
Функция ДанныеВетеринарноСопроводительногоДокумента(ИсходныеДанныеВСД, УникальныйИдентификатор)
	...
	| СерияТТН, ДатаТТН, НомерТТН, ТипТТН";
...
Процедура ЗаполнитьМаршрутВозврата()
	...
	Если Объект.Товары.Итог("ВозвращаемоеКоличествоВЕТИС") > 0
		И Объект.МаршрутВозврата.Количество() = 0
		И Объект.Маршрут.Количество() > 0 Тогда
		
		ПунктМаршрутаВозврата = Объект.МаршрутВозврата.Добавить();
		
		//Заполним прибывшее транспортное средство
		Для каждого ПунктМаршрута Из Объект.Маршрут Цикл
			Если ПунктМаршрута.СПерегрузкой Тогда
				ЗаполнитьЗначенияСвойств(ПунктМаршрутаВозврата, ПунктМаршрута);
			КонецЕсли;
		КонецЦикла;
		
		//Заполним отправителя
		ПунктМаршрутаВозврата.Предприятие           = Объект.ГрузополучательПредприятие;
		ПунктМаршрутаВозврата.СПерегрузкой          = Истина;
		ПунктМаршрутаВозврата.Адрес                 = "";
		ПунктМаршрутаВозврата.АдресПредставление    = "";
		ПунктМаршрутаВозврата.ДанныеАдресаСтруктура = Неопределено;
...
Процедура ЗаполнитьСлужебныеРеквизитыТоваров(ТекущаяСтрока, ОбновляемыеПоля = Неопределено, ДополнительныеПараметры = Неопределено)
	...
	Если Инициализация Или СтруктураПолей.Свойство("Характеристика") Тогда
		...
		Если ТекущаяСтрока.ВыбраноХарактеристика > 1 Тогда
			ТекущаяСтрока.СопоставлениеХарактеристика = СтрШаблон(НСтр("ru = '<Выбрано (%1)>'"), ТекущаяСтрока.ВыбраноХарактеристика);
		ИначеЕсли ТекущаяСтрока.СопоставленоКоличество = 1 Тогда
...
Функция КоэффициентЕдиницыИзмеренияВЕТИС(БазоваяЕдиницаИзмеренияВЕТИС, ЕдиницаИзмеренияВЕТИС)
	...
	Возврат Справочники.ЕдиницыИзмеренияВЕТИС.КоэффициентПересчета(БазоваяЕдиницаИзмеренияВЕТИС, ЕдиницаИзмеренияВЕТИС);
