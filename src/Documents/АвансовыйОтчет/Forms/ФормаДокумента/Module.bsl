...
Процедура РассчитатьСуммуДокумента(Форма)
	...
	Если Не Объект.Мультивалютный Тогда
		...
		Для каждого Расход Из Объект.ПрочиеРасходы Цикл
			Если Расход.Отменено Тогда
				Объект.СуммаОтклонено = Объект.СуммаОтклонено + Расход.СуммаСНДС;
			Иначе
				Объект.СуммаИзрасходовано = Объект.СуммаИзрасходовано + Расход.СуммаСНДС;
			КонецЕсли;
		КонецЦикла;
		...
		Если Форма.СуммаПолученоИтог < Форма.СуммаИзрасходовано + Форма.СуммаВозвращено Тогда
			Форма.ОстатокПерерасход = Форма.СуммаИзрасходовано - Форма.СуммаПолученоИтог + Форма.СуммаВозвращено;
			Форма.Элементы.ОстатокПерерасход.Заголовок = НСтр("ru = 'Перерасход'");
		Иначе
			Форма.ОстатокПерерасход = Форма.СуммаПолученоИтог - Форма.СуммаИзрасходовано - Форма.СуммаВозвращено;
	...
	Иначе
		...
		Для каждого СтрокаТЧ Из Форма.ТаблицаИтоговПоВалютам Цикл
			...
			КонецЕсли;
			ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам, СтрокаТЧ.Валюта, СтрокаТЧ.Получено, "ПолученоИтог");
		...
		Для каждого КлючИЗначение Из ИтогиПоВалютам Цикл
			...
			Если СтрокаИтогов.ПолученоИтог + СтрокаИтогов.Конвертировано - СтрокаИтогов.Возвращено <= СтрокаИтогов.Израсходовано Тогда
				СтрокаИтогов.Перерасход = СтрокаИтогов.Израсходовано - СтрокаИтогов.ПолученоИтог - СтрокаИтогов.Конвертировано + СтрокаИтогов.Возвращено;
				СтрокаИтогов.Остаток = 0;
			Иначе
				СтрокаИтогов.Перерасход = 0;
				СтрокаИтогов.Остаток = СтрокаИтогов.ПолученоИтог + СтрокаИтогов.Конвертировано - СтрокаИтогов.Израсходовано - СтрокаИтогов.Возвращено;
			КонецЕсли;
...
Процедура ЗаполнитьСуммуПолучено()
	...
	Если Не Объект.Мультивалютный Тогда
		Если ВыборкаПолучено.Следующий() Тогда
			...
			СуммаВозвращено = ВыборкаПолучено.Возвращено;
		Иначе
			СуммаПолучено = 0;
			СуммаВозвращено = 0;
		КонецЕсли;
	Иначе
		...
		КонецЦикла;
		Таблица.Свернуть("Валюта, Курс, Растяжка", "Получено, Возвращено, ПолученоИтог, Израсходовано, Перерасход, Отклонено, Остаток, Конвертировано, СуммаЗакупок");
...
Функция ИнициализироватьИтогПоВалюте()
	...
	ИтогПоВалюте.Вставить("ПолученоИтог", 0);
...
Функция ВыборкаПолучено()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			Документ.АвансовыйОтчет.КонвертацияВалюты КАК ТЧКонвертацияВалюты
	|		ПО
	|			ТЧКонвертацияВалюты.Ссылка = ДанныеДокумента.Ссылка
	|	ГДЕ
	...
	Запрос.УстановитьПараметр("НачальнаяДатаОборотов", ДатаОтчетаНачальная);
	Запрос.УстановитьПараметр("КонечнаяДатаОборотов", ДатаОтчетаКонечная);
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	СУММА(ДенежныеСредства.Получено) КАК Получено,
	|	СУММА(ДенежныеСредства.Возвращено) КАК Возвращено,
	...
	|			И НЕ ДенежныеСредства.Регистратор ССЫЛКА Документ.Бронирование
	|			И НЕ ДенежныеСредства.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КонвертацияВалютыПодотчетнымЛицом)
	|		ТОГДА ДенежныеСредства.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Получено,
	|	ВЫБОР КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			И ДенежныеСредства.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПодотчетника)
	|		ТОГДА ДенежныеСредства.Сумма
	|		ИНАЧЕ 0
	...
	|ГДЕ
	|	ДенежныеСредства.Период >=&НачальнаяДатаОборотов
	|	И ДенежныеСредства.Период <=&КонечнаяДатаОборотов
	...
	|	ДенежныеСредства.Валюта
	|ИМЕЮЩИЕ 
	|	СУММА(ДенежныеСредства.Получено) > 0
	|";
