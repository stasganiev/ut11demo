...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	СтруктураДействий.Вставить("СкорректироватьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект, Ложь, ВозвратПереданнойМногооборотнойТары));
		...
		Если ЕстьПоДокументамПродажи Тогда
			...
			Если Не Проведен
				...
				Для Каждого СтрокаТаблицы из ВидыЗапасов Цикл
					СтрокаТаблицы.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПродажи;
				КонецЦикла;
