...
Функция ПараметрыЗаполненияДокумента() Экспорт
	...
	ПараметрыЗаполнения.Вставить("ЗаполнятьДанныеПоМаркам", Ложь);
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.ЗаполнятьПоОрдеру Тогда
		...
		Если ПараметрыЗаполнения.Свойство("ЗаполнятьДанныеПоМаркам") Тогда
			ЗаполнятьДанныеПоМаркам = ПараметрыЗаполнения.ЗаполнятьДанныеПоМаркам;
		Иначе
			ЗаполнятьДанныеПоМаркам = Ложь;
		КонецЕсли;
	Иначе
	...
	Если ЗаполнятьДанныеПоМаркам И ТаблицаШтрихкодыУпаковок.Количество() > 0 Тогда
