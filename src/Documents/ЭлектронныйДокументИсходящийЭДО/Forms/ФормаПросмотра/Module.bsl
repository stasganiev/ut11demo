...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДействияПередОткрытиемФормы") Тогда
		ОбработатьДействияПередОткрытиемФормы(Параметры.ДействияПередОткрытиемФормы);
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОшибкаЧтенияXML Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Файл содержит ошибки. Выберите другой файл'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаписьДокументаПередПодписанием(Оповещение)
	...
	Иначе
		...
		Если Не ОкноОтправкиВФормализованномВидеПоказано
			...
			ПоказатьОкноПредложенияОтправкиВФормализованномВиде(Оповещение);
			Возврат;
		КонецЕсли;
...
&НаСервере
Процедура ОбработатьДействияПередОткрытиемФормы(Действия)
	Действие = Неопределено;
	Если Действия.Свойство("ОбернутьДанныеСообщенияВПДФ", Действие) Тогда
		ОбернутьДанныеСообщенияВПДФ(Действие);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ОбернутьДанныеСообщенияВПДФ(Действие)
	
	АдресПДФКонтейнера = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Действие, "АдресПДФКонтейнера", "");
	
	Если ЭтоАдресВременногоХранилища(АдресПДФКонтейнера) Тогда
		
		ДвоичныеДанныеФайлаПДФ = ПолучитьИзВременногоХранилища(АдресПДФКонтейнера);
		АдресаОписанийСообщений = ПолучитьИзВременногоХранилища(
			Параметры.РезультатПодготовкиДанных.АдресаОписанийСообщений);
		...
		Для Каждого АдресОписанияСообщения Из АдресаОписанийСообщений Цикл
			...
			Если ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя 
				Или (МодульОбменСГИСЭПД <> Неопределено И МодульОбменСГИСЭПД.ЭтоТитулОтправителя(ОписаниеСообщения.ТипЭлементаРегламента)) Тогда
				Обработки.ФорматДоговорныйДокумент101.ПоместитьФайлЭДВФайлПДФ(ДвоичныеДанныеФайлаПДФ,
					ОписаниеСообщения.Данные.Документ);
				ПоместитьВоВременноеХранилище(ОписаниеСообщения, АдресОписанияСообщения);
			КонецЕсли;
...
Процедура НачатьЗаписьДокумента(ОповещениеОЗавершении)
	...
	Если РезультатЗаписи.ТребуетсяФормализованныйФормат
		...
		ПоказатьОкноПредложенияОтправкиВФормализованномВиде(ОповещениеОЗавершении);
		Возврат;
	КонецЕсли;
...
Процедура ПроверкаСуществующегоДокумента()
	Если ЗначениеЗаполнено(СуществующийДокумент) Тогда
		ОписаниеРезультатаВопроса = Новый ОписаниеОповещения("ОбработкаОтветаОткрытияСуществующегоДокумента", ЭтотОбъект);
		ТекстВопроса = СтрШаблон(НСтр("ru =  'Такой документ уже был загружен ранее. %1Открыть его?'"), Символы.ПС);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ru =  'Да, открыть'"));
		Кнопки.Добавить("Отмена", НСтр("ru =  'Нет, отменить загрузку'"));
		ПоказатьВопрос(ОписаниеРезультатаВопроса, ТекстВопроса, Кнопки,, "Открыть");
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
...
Функция ЗагрузкаИзФайлаНаСервереУспешна(ВыбранныйФайл, ТаблицаОшибок, ЗагрузитьПринудительно = Ложь)
	...
	Если ТаблицаОшибок.Количество() = 0 Или ЗагрузитьПринудительно Тогда
		СвойстваФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ВыбранныйФайл.Имя);
		СвойстваФайла.Расширение = СтрЗаменить(СвойстваФайла.Расширение, ".", "");
		Инфо = Новый Структура;
		Инфо.Вставить("АдресФайлаВоВременномХранилище", ВыбранныйФайл.Хранение); 
		Инфо.Вставить("ИмяБезРасширения", ИмяФайлаБезРасширения);
		РаботаСФайлами.ОбновитьФайл(ОсновнойФайл, Инфо);
		ФайлОбъект = ОсновнойФайл.ПолучитьОбъект();
		ФайлОбъект.ПолноеИмяФайла = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ИмяФайлаБезРасширения, СвойстваФайла.Расширение);
		ФайлОбъект.Записать();
		ОбновитьПредставлениеЭлектронногоДокумента();
		ПараметрыЗаписиЖР = ОбщегоНазначенияБЭД.НовыеПараметрыЗаписиВЖурналРегистрации(); 
		ПараметрыЗаписиЖР.Данные = ФайлОбъект.Ссылка;
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'В исходящем документе %1 был загружен сторонний файл. Имя файла %2'"),
			Объект.Ссылка, ВыбранныйФайл.Имя);
...
&НаКлиенте
Процедура ОбработкаОтветаОткрытияСуществующегоДокумента(Ответ, ДополнительныеПараметры) Экспорт
...
Функция ТребуетсяФормализованныйФормат(Знач ТипРегламента, Знач ВидДокумента)
	...
	ТребуетсяПоказатьОкно = ТребуетсяПоказыватьОкноПредложенияОтправкиВФормализованномВиде();
...
&НаСервереБезКонтекста
Функция ТребуетсяПоказыватьОкноПредложенияОтправкиВФормализованномВиде()
...
&НаКлиенте
Процедура ЗапомнитьНеПоказыватьОкноПредложенияОтправкиВФормализованномВиде()
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить(
		"НастройкиБольшеНеПоказывать", "ПоказыватьОкноПредложенияОтправкиВФормализованномВиде", Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьОкноПредложенияОтправкиВФормализованномВиде(ОповещениеОЗавершении)

	ДопПараметры = Новый Структура("Оповещение", ОповещениеОЗавершении);
	Оповещение = Новый ОписаниеОповещения("ТребуетсяФормализованныйФорматПослеОтветаНаВопрос", 
		ЭтотОбъект, ДопПараметры);
	
	ШаблонВопроса = НСтр("ru='Для документа ""%1"" предусмотрен утвержденный ФНС формат.
		|Отправить в формализованном виде?'");
	ТекстВопроса = СтрШаблон(ШаблонВопроса, Объект.ВидДокумента);
	...
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
		Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, ПараметрыВопроса);
	
КонецПроцедуры

&НаКлиенте
Процедура ТребуетсяФормализованныйФорматПослеОтветаНаВопрос(Результат, ДопПараметры) Экспорт
	...
	Если Результат.БольшеНеЗадаватьЭтотВопрос Тогда
		ЗапомнитьНеПоказыватьОкноПредложенияОтправкиВФормализованномВиде();
	КонецЕсли;
	
	Если Результат.Значение = КодВозвратаДиалога.Да 
		ИЛИ Результат.Значение = КодВозвратаДиалога.Нет Тогда
	...
	Если Результат.Значение = КодВозвратаДиалога.Нет Тогда
		НачатьЗаписьДокумента(ДопПараметры.Оповещение);
	КонецЕсли;
