...
//   * НовыйСтатус - ПеречислениеСсылка.СтатусыОбработкиОбъединенийЗаписейСкладскогоЖурналаВЕТИС - Новый статус.
...
Функция КонечныеСтатусы(ТребуетсяПовторноеОформление = Истина) Экспорт
	...
	Если Не ТребуетсяПовторноеОформление Тогда
		Статусы.Добавить(Перечисления.СтатусыОбработкиОбъединенийЗаписейСкладскогоЖурналаВЕТИС.Выполнен);
		Статусы.Добавить(Перечисления.СтатусыОбработкиОбъединенийЗаписейСкладскогоЖурналаВЕТИС.ВыполненЧерезWeb);
	КонецЕсли;
...
Функция ОбработатьРезультатЗапросаНаОбъединениеЗаписейСкладскогоЖурнала(ДанныеДокумента, ХозяйствующийСубъект, Предприятие, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатДобавленияЗаписей.ВходящееСообщение.НовоеСообщение Тогда
		...
		Если ДанныеДокумента.Объект <> Неопределено Тогда
			...
			ВсеИзмененныеЗаписиЖурнала = Новый Массив;
			...
			Для Каждого ЭлементДанных Из ДанныеДокумента.Объект.stockEntryList.stockEntry Цикл
				...
				Если ДанныеЗаписиСкладскогоЖурнала <> Неопределено Тогда
					ВсеИзмененныеЗаписиЖурнала.Добавить(ДанныеЗаписиСкладскогоЖурнала.Ссылка);
				КонецЕсли;
				
			КонецЦикла;
			
			// Информация о ВСД не приходит в элементе-результате объединения
			ПеренестиВСДВНовуюЗаписьСкладскогоЖурнала(ДокументОбъект.ЗаписьСкладскогоЖурнала, ВсеИзмененныеЗаписиЖурнала);
...
Функция ЗапросНаОбъединениеЗаписейСкладскогоЖурналаXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Если Шапка.СпособОбъединения = Перечисления.СпособыОбъединенияЗаписейСкладскогоЖурналаВЕТИС.Присоединить Тогда
		...
		ЗаписьЖурнала = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъединяемыеЗаписи, "sourceStockEntry");
	...
	Если Шапка.СпособОбъединения = Перечисления.СпособыОбъединенияЗаписейСкладскогоЖурналаВЕТИС.Объединить Тогда
		...
		НоваяЗаписьЖурнала = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ОбъединяемыеЗаписи, "resultStockEntry");
...
Функция ДанныеДляОбъединенияЗаписейСкладскогоЖурнала(ДокументСсылка, Операция)
	...
	|		КОГДА ДанныеДокумента.Продукция.ПолноеНаименование = """" ТОГДА
	|			ДанныеДокумента.Продукция.Наименование
	|		ИНАЧЕ ДанныеДокумента.Продукция.ПолноеНаименование
	...
	|	
	|	ВЫБОР КОГДА ТаблицаДокумента.ЗаписьСкладскогоЖурнала.Продукция.ПолноеНаименование = """" ТОГДА
	|		ТаблицаДокумента.ЗаписьСкладскогоЖурнала.Продукция.Наименование
	|	ИНАЧЕ
	|		ТаблицаДокумента.ЗаписьСкладскогоЖурнала.Продукция.ПолноеНаименование
...
Процедура ПеренестиВСДВНовуюЗаписьСкладскогоЖурнала(РезультатОбъединения, ИзмененныеЗаписи)
	
	УстановитьПривилегированныйРежим(Истина);
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ИзмененныеЗаписи);
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗаписиСкладскогоЖурналаВЕТИС.ВетеринарноСопроводительныйДокумент КАК ВетеринарноСопроводительныйДокумент
	|ИЗ
	|	Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ВетеринарноСопроводительныеДокументы КАК ЗаписиСкладскогоЖурналаВЕТИС
	|ГДЕ
	|	ЗаписиСкладскогоЖурналаВЕТИС.Ссылка В (&Ссылка)";
	ЗаписьЖурналаОбъект = РезультатОбъединения.ПолучитьОбъект();
	ЗаписьЖурналаОбъект.ВетеринарноСопроводительныеДокументы.Загрузить(Запрос.Выполнить().Выгрузить());
	ЗаписьЖурналаОбъект.Записать();
