...
Процедура ПодобратьДокументЗавершить(Знач Результат, Знач СпособОбработки) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ОповеститьВладельца();
		УправлениеДоступностью();
		ИнтерфейсДокументовЭДОКлиент.ПослеПодбораУчетногоДокумента(Документооборот, Результат);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения) Тогда
		Попытка
			...
			Если Не ЗначениеЗаполнено(ОбъектыУчета) Тогда 
				ИнтеграцияЭДО.ОтразитьДанныеЭлектронногоДокументаСКонтролем(ПараметрыЗаполнения[0], СпособОбработки);
			КонецЕсли;
...
Процедура ПерезаполнитьОбъектУчета(ОбъектУчета, Документооборот, СпособОбработки)
	...
	ИнтеграцияЭДО.ПерезаполнитьОбъектУчетаПоЭлектронномуДокументу(
		ОбъектУчета, Документооборот, СпособОбработки);
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если ПараметрыПроизвольногоДокумента <> Неопределено
		...
		СписокТиповДокументов = ИнтеграцияЭДО.СписокОперацийВидаДокумента(ВидДляОтраженияВУчете);
	ИначеЕсли Не ДокументРаспознан Тогда
		СписокТиповДокументов = ИнтеграцияЭДО.СписокОперацийВсехТиповДокумента();
	Иначе
		СписокТиповДокументов = ИнтеграцияЭДО.СписокОперацийВидаДокумента(ВидДокумента);
	КонецЕсли;
	
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность Тогда
		МашиночитаемыеДоверенности.ОграничитьВариантыОтраженияВУчетеПоДаннымЭлектронногоДокумента(Документооборот, 
