...
Процедура ОформитьИзлишкиНедостачиНаСкладе(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЛОЖЬ КАК РазрешеноОтражатьОстатокПоОрдерномуСкладу
	|ИЗ
	|	Документ.КорректировкаПриобретения.Расхождения КАК ТаблицаРасхождения
	|ГДЕ
	|	ТаблицаРасхождения.Ссылка В(&Ссылка)
...
Процедура СформироватьВременнуюТаблицуИсходныхДанных(МенеджерВременныхТаблиц, ДокументОснование, ДокументСсылка = Неопределено) Экспорт
	...
	ТекстЗапросаПоОснованию = ?(ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПервичныйДокумент"), "",
		ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстЗапросаПоОснованию);
...
Функция ТекстЗапросаВТПроверкиДанныхПоПоследнейКорректировке()
	...
	|	МАКСИМУМ(КорректировкаТовары.Ссылка.Дата) КАК Дата,
	|	КорректировкаТовары.Номенклатура КАК Номенклатура,
	|	КорректировкаТовары.Характеристика КАК Характеристика,
	|	КорректировкаТовары.Содержание КАК Содержание,
	|	КорректировкаТовары.Упаковка КАК Упаковка,
	|	КорректировкаТовары.СтатьяРасходов КАК СтатьяРасходов,
	|	КорректировкаТовары.АналитикаРасходов КАК АналитикаРасходов,
	|	КорректировкаТовары.Серия КАК Серия,
	|	КорректировкаТовары.Склад КАК Склад,
	|	КорректировкаТовары.НомерГТД КАК НомерГТД,
	|	КорректировкаТовары.КодТНВЭД КАК КодТНВЭД,
	|	КорректировкаТовары.Подразделение КАК Подразделение
	|ПОМЕСТИТЬ ВТПоследняяКорректировка
	|ИЗ
	|	Документ.КорректировкаПриобретения.Товары КАК КорректировкаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоваровДляЗапроса КАК ВТРасхождения
	|		ПО (ВЫБОР
	|				КОГДА &ЭтоКорректировкаДоВводаОстатковТоваровИУслуг
	|					ТОГДА ВТРасхождения.Номенклатура = КорректировкаТовары.Номенклатура
	|							И ВТРасхождения.Характеристика = КорректировкаТовары.Характеристика
	|							И ВТРасхождения.Серия = КорректировкаТовары.Серия
	|							И ВТРасхождения.Упаковка = КорректировкаТовары.Упаковка
	|							И ВТРасхождения.Склад = КорректировкаТовары.Склад
	|							И ВТРасхождения.НомерГТД = КорректировкаТовары.НомерГТД
	|							И ВТРасхождения.КодТНВЭД = КорректировкаТовары.КодТНВЭД
	|							И ВТРасхождения.СтатьяРасходов = КорректировкаТовары.СтатьяРасходов
	|							И ВТРасхождения.АналитикаРасходов = КорректировкаТовары.АналитикаРасходов
	|				ИНАЧЕ ВТРасхождения.Содержание = КорректировкаТовары.Содержание
	|						И ВТРасхождения.Подразделение = КорректировкаТовары.Подразделение
	|						И ВТРасхождения.СтатьяРасходов = КорректировкаТовары.СтатьяРасходов
	|						И ВТРасхождения.АналитикаРасходов = КорректировкаТовары.АналитикаРасходов
	|						И ВТРасхождения.НомерГТД = КорректировкаТовары.НомерГТД
	|						И ВТРасхождения.КодТНВЭД = КорректировкаТовары.КодТНВЭД
	|			КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовСторноИсправление
	|		ПО КорректировкаТовары.Ссылка = РеестрДокументовСторноИсправление.СторнируемыйДокумент
	|			И (РеестрДокументовСторноИсправление.Проведен)
	|			И (НЕ РеестрДокументовСторноИсправление.ДополнительнаяЗапись)
	|ГДЕ
	|	КорректировкаТовары.Ссылка.ДокументОснование = &ДокументОснование
	|	И КорректировкаТовары.Ссылка <> &Ссылка
	|	И КорректировкаТовары.Ссылка.Проведен
	|	И КорректировкаТовары.Ссылка.ТипКорректировки = &ТипКорректировки
	|	И КорректировкаТовары.Ссылка.Дата < &Дата
	|	И РеестрДокументовСторноИсправление.Ссылка ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаТовары.СтатьяРасходов,
	|	КорректировкаТовары.АналитикаРасходов,
	|	КорректировкаТовары.Подразделение,
	|	КорректировкаТовары.Содержание,
	|	КорректировкаТовары.Номенклатура,
	|	КорректировкаТовары.Характеристика,
	|	КорректировкаТовары.Упаковка,
	|	КорректировкаТовары.Серия,
	|	КорректировкаТовары.Склад,
	|	КорректировкаТовары.НомерГТД,
	|	КорректировкаТовары.КодТНВЭД
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КорректировкаТовары.НомерГТД КАК НомерГТД,
	|	КорректировкаТовары.КодТНВЭД КАК КодТНВЭД,
	|	КорректировкаТовары.СтавкаНДС КАК СтавкаНДС,
	|	КорректировкаТовары.КоличествоУпаковок КАК КоличествоУпаковок,
	|	КорректировкаТовары.Количество КАК Количество,
	|	КорректировкаТовары.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
	|	КорректировкаТовары.Сумма КАК Сумма,
	|	КорректировкаТовары.СуммаНДС КАК СуммаНДС,
	|	КорректировкаТовары.СуммаСНДС КАК СуммаСНДС
	|ПОМЕСТИТЬ ВТДанныеПоследнейКорректировки
	|ИЗ
	|	ВТПоследняяКорректировка КАК ПоследняяКорректировка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения.Товары КАК КорректировкаТовары
	|		ПО (ВЫБОР
	|				КОГДА &ЭтоКорректировкаДоВводаОстатковТоваровИУслуг
	|					ТОГДА ПоследняяКорректировка.Номенклатура = КорректировкаТовары.Номенклатура
	|							И ПоследняяКорректировка.Характеристика = КорректировкаТовары.Характеристика
	|							И ПоследняяКорректировка.Серия = КорректировкаТовары.Серия
	|							И ПоследняяКорректировка.Упаковка = КорректировкаТовары.Упаковка
	|							И ПоследняяКорректировка.Склад = КорректировкаТовары.Склад
	|							И ПоследняяКорректировка.НомерГТД = КорректировкаТовары.НомерГТД
	|							И ПоследняяКорректировка.КодТНВЭД = КорректировкаТовары.КодТНВЭД
	|							И ПоследняяКорректировка.СтатьяРасходов = КорректировкаТовары.СтатьяРасходов
	|							И ПоследняяКорректировка.АналитикаРасходов = КорректировкаТовары.АналитикаРасходов
	|							И ПоследняяКорректировка.Подразделение = КорректировкаТовары.Подразделение
	|							И ПоследняяКорректировка.Дата = КорректировкаТовары.Ссылка.Дата
	|				ИНАЧЕ ПоследняяКорректировка.Содержание = КорректировкаТовары.Содержание
	|						И ПоследняяКорректировка.СтатьяРасходов = КорректировкаТовары.СтатьяРасходов
	|						И ПоследняяКорректировка.АналитикаРасходов = КорректировкаТовары.АналитикаРасходов
	|						И ПоследняяКорректировка.Подразделение = КорректировкаТовары.Подразделение
	|						И ПоследняяКорректировка.Дата = КорректировкаТовары.Ссылка.Дата
	...
	|	ВТРасхождения.Количество - ДанныеПоследнейКорректировки.Количество КАК Количество,
	|	ВТРасхождения.КоличествоУпаковок - ДанныеПоследнейКорректировки.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ВТРасхождения.Сумма - ДанныеПоследнейКорректировки.Сумма КАК Сумма,
	|	ВТРасхождения.СуммаНДС - ДанныеПоследнейКорректировки.СуммаНДС КАК СуммаНДС,
	|	ВТРасхождения.СуммаСНДС - ДанныеПоследнейКорректировки.СуммаСНДС КАК СуммаСНДС,
	|	ВЫБОР
	|		КОГДА ВТРасхождения.СтавкаНДС <> ДанныеПоследнейКорректировки.СтавкаНДС
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОшибкаПоНДС,
	|	ДанныеПоследнейКорректировки.Количество КАК ИсходноеКоличество,
	|	ДанныеПоследнейКорректировки.КоличествоУпаковок КАК ИсходноеКоличествоУпаковок,
	|	ДанныеПоследнейКорректировки.Сумма КАК ИсходнаяСумма,
	|	ДанныеПоследнейКорректировки.СуммаНДС КАК ИсходнаяСуммаНДС,
	|	ДанныеПоследнейКорректировки.СуммаСНДС КАК ИсходнаяСуммаСНДС,
	|	ДанныеПоследнейКорректировки.СтавкаНДС КАК ИсходнаяСтавкаНДС
	|ПОМЕСТИТЬ ВТПроверкиДанныхПоПоследнейКорректировке
	|ИЗ
	|	ВТДанныеПоследнейКорректировки КАК ДанныеПоследнейКорректировки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаТоваровДляЗапроса КАК ВТРасхождения
	|		ПО (ВЫБОР
	|				КОГДА &ЭтоКорректировкаДоВводаОстатковТоваровИУслуг
...
Процедура СообщитьОшибкиПоЦепочкеКорректировок(Выборка, Отказ)
	...
	ШаблонСообщенияРасходитсяЗначениеКоличество	= НСтр("ru = 'В поле ""%1"" в строке %2 указано количество отличное от количества %3 после последней корректировки'");
	ШаблонСообщенияРасходитсяЗначениеСумма		= НСтр("ru = 'В поле ""%1"" в строке %2 указана сумма отличная от суммы %3 после последней корректировки'");
	ШаблонСообщенияРасходитсяСтавкаНДС			= НСтр("ru = 'Ставка НДС в строке %1 отличается от ставки НДС %2 после последней корректировки'");
