...
Процедура Архивировать(Команда)
	...
	ИнтеграцияИСКлиент.АрхивироватьРаспоряжения(ЭтотОбъект, Элементы.СписокКОформлению, ИнтеграцияИСМПКлиент,
		ПредопределенноеЗначение("Документ.ОтчетИСМП.ПустаяСсылка"));
...
&НаКлиенте
Процедура ЗапроситеОтчет(Команда)
	
	ИнтеграцияИСМПКлиент.ПодготовитьСообщенияКПередаче(
		Элементы.Список, ПредопределенноеЗначение("Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеОтчет"));
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтвердитеОбработкуОтклонений(Команда)
	...
	Контекст = ИнтеграцияИСКлиент.СтруктураПодготовкиСообщенийКПередаче(
		...
		ПредопределенноеЗначение(
			"Перечисление.ДальнейшиеДействияПоВзаимодействиюИСМП.ПодтвердитеОбработкуОтклонений"),
		Новый ОписаниеОповещения("ПодтвердитеОбработкуОтклоненийЗавершение", ЭтотОбъект));
...
Процедура УстановитьУсловноеОформление()
	...
	СписокСтатусов.ЗагрузитьЗначения(Документы.ОтчетИСМП.СтатусыОшибок());
	...
	СписокДействий.ЗагрузитьЗначения(Документы.ОтчетИСМП.ВсеТребующиеОжидания()); 
...
Функция ВсеТребующиеДействия()
	...
	Возврат Документы.ОтчетИСМП.ВсеТребующиеДействия();
...
Функция ВсеТребующиеОжидания()
	...
	Возврат Документы.ОтчетИСМП.ВсеТребующиеОжидания();
...
Процедура НастроитьВидимостьДоступностьЭлементовСервер()
	...
	Если НЕ ПравоДоступа("Добавление", Метаданные.Документы.ОтчетИСМП) Тогда
...
//   * МассивДокументов - Массив из ДокументСсылка.ОтчетИСМП- список ссылок на обрабатываемые документы,
//   * НепроведенныеДокументы - Массив из ДокументСсылка.ОтчетИСМП - документы, исключенные из обработки,
...
&НаКлиенте
Процедура ПодтвердитеОбработкуОтклоненийЗавершение(Контекст, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОчиститьСообщения();
	ПодтвердитеОбработкуОтклоненийЗавершениеНаСервере(Контекст.МассивДокументов, ДополнительныеПараметры);
...
//
// Параметры:
//  ДокументыКОбработке - Массив из ДокументСсылка.ОтчетИСМП- список ссылок на обрабатываемые документы,
//  ДополнительныеПараметры - Структура, Неопределено - Дополнительные параметры
&НаСервере
Процедура ПодтвердитеОбработкуОтклоненийЗавершениеНаСервере(ДокументыКОбработке, ДополнительныеПараметры)
	
	Для Каждого ДокументСсылка Из ДокументыКОбработке Цикл
		...
		ПараметрыОбновленияСтатуса.ОперацияКвитанции = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСведенияОбОтклонениях;
