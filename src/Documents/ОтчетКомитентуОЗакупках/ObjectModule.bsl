...
Процедура ПроверкиСоответствияРеквизитовВДокументахПриобретения(Отказ)
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.ДокументПриобретения) Тогда
			...
			РеквизитыДокументаПриобретения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТовары.ДокументПриобретения, "ЦенаВключаетНДС, НаправлениеДеятельности, НалогообложениеНДС, Организация, Валюта");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если СуммаВознаграждения <> 0 Тогда
		ПроцентНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(ЭтотОбъект.СтавкаНДСВознаграждения);
		КомиссионнаяТорговляСервер.РассчитатьСуммуНДСВознагражденияВСтроках(ЭтотОбъект, ПроцентНДС);
	Иначе
