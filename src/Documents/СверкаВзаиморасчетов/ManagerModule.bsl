...
Функция РеквизитыДокумента(ДокументСсылка) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов.ДетальныеЗаписиРасчеты КАК СверкаВзаиморасчетовДетальныеЗаписиРасчеты
...
//
// Параметры:
//   МассивОбъектов  - Массив из ДокументСсылка.СверкаВзаиморасчетов - Массив ссылок на объекты которые нужно распечатать.
...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	|		ИНАЧЕ
	|			ЕСТЬNULL(ВЫБОР КОГДА РеестрДокументов.ДатаПервичногоДокумента = ДАТАВРЕМЯ(1,1,1) 
...
// Заполняет структуру данными о получателях печатных форм.
// Параметры:
// 	ДокументОбъект - ДокументОбъект.СверкаВзаиморасчетов - Объект сверки для заполненияч.
...
// 
// Параметры:
// 	ДокументСверки - ДокументСсылка.СверкаВзаиморасчетов - Описание
...
// * Действие - Строка -
// * ДокументСверки - ДокументСсылка.СверкаВзаиморасчетов -
Функция СтрокаПротокола() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("ДокументСверки", Документы.СверкаВзаиморасчетов.ПустаяСсылка());
...
// Параметры:
// 	ДанныеДокумента - ДокументОбъект.СверкаВзаиморасчетов - 
	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				КонецЦикла;
				ОписаниеСчетовФактур = СтрСоединить(ОписаниеСчетовФактур, ",");
				НоваяСтрока.ОписаниеДокумента = НоваяСтрока.ОписаниеДокумента + " (" + ОписаниеСчетовФактур + ")";
			КонецЕсли;
			//-- Локализация
...
Функция СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовПоСтарому(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент = Неопределено)
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СверкаВзаиморасчетов.ПФ_MXL_УстаревшийАктСверкиВзаиморасчетов");
...
// 
// Параметры:
// 	ДокументСсылка - ДокументСсылка.СверкаВзаиморасчетов - 
// 	Номер - Строка -
// 	Дата - Дата - 
