...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Если ПоступлениеПоРаздельнойЗакупке
		...
		ПараметрыОтбора.Вставить("Номенклатура.ТипНоменклатуры",
			НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТаре(Не ПоступлениеПоРаздельнойЗакупке));
	Иначе
...
Процедура ТоварыСписатьНаРасходыВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не СтрокаТаблицы.СписатьНаРасходы Тогда
		...
		Если СтрокаТаблицы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар") Или
			СтрокаТаблицы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара") Тогда
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Иначе
			...
			Если СтрокаТовары.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
				...
				СтрокаТовары.СписатьНаРасходы = Ложь;
				Если Не СтрокаТовары.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
					СтрокаТовары.Подразделение = ПредопределенноеЗначение("Справочник.СтруктураПредприятия.ПустаяСсылка");
				КонецЕсли;
