...
Процедура ПеревозкаПриИзменении(Элемент)
	ОбновитьДанные(
		Объект.ВидПродукции,
		Новый Структура("ИмяПоля, ИдентификаторСтроки", "Перевозка", Неопределено));
КонецПроцедуры
...
Процедура ПриемкаПриИзменении(Элемент)
	ОбновитьДанные(
		Объект.ВидПродукции,
		Новый Структура("ИмяПоля, ИдентификаторСтроки", "Приемка", Неопределено));
КонецПроцедуры
...
Процедура РеализацияПриИзменении(Элемент)
	ОбновитьДанные(
		Объект.ВидПродукции,
		Новый Структура("ИмяПоля, ИдентификаторСтроки", "Реализация", Неопределено));
	ОбновитьПодсказкиВвода(ЭтотОбъект);
КонецПроцедуры
...
Процедура ОтгрузкаПриИзменении(Элемент)
	ОбновитьДанные(
		Объект.ВидПродукции,
		Новый Структура("ИмяПоля, ИдентификаторСтроки", "Отгрузка", Неопределено));
КонецПроцедуры
...
Процедура ЗаписатьДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИсходящаяТранспортнаяОперацияЗЕРНО.Форма.ФормаДокумента.Записать");
...
Процедура ПровестиДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИсходящаяТранспортнаяОперацияЗЕРНО.Форма.ФормаДокумента.Провести");
...
Процедура ПровестиИЗакрыть(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("Документ.ИсходящаяТранспортнаяОперацияЗЕРНО.Форма.ФормаДокумента.ПровестиИЗакрыть");
...
Процедура АннулироватьСДИЗ(Команда)
	...
	Если Модифицированность Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения(
			"ВопросАннулированиеСДИЗЗавершение",
			ЭтотОбъект,
			МассивСтрок);
		ТекстВопроса = НСтр("ru = 'Документ ""Оформление СДИЗ ЗЕРНО"" был изменен. Провести?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	...
	Иначе
		...
		Если МассивСтрок.Количество() = 1 Тогда
			СтрокиТекстаВопроса.Добавить(
				СтрШаблон(НСтр("ru = 'Аннулировать сформированный СДИЗ %1 ?'"), МассивСтрок[0].Партия));
		Иначе
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения(
			"ПодтверждениеАннулированияСДИЗЗавершение",
			ЭтотОбъект,
			МассивСтрок);
...
&НаКлиенте
Процедура ВопросАннулированиеСДИЗЗавершение(РезультатВопроса, СтрокиДляАннулирования) Экспорт
	...
	Если Не Модифицированность И Объект.Проведен Тогда
		ПодтверждениеАннулированияСДИЗЗавершение(РезультатВопроса, СтрокиДляАннулирования);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтверждениеАннулированияСДИЗЗавершение(РезультатВопроса, СтрокиДляАннулирования) Экспорт
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("СтатусЗЕРНО") Тогда
		...
		Элементы.АннулироватьСДИЗ.Доступность = Форма.СтатусЗЕРНО <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОформлениеСДИЗЗЕРНО.Черновик")
...
Функция ЗаполнитьПартииНаСервере()
	...
	РезультатЗаполнения = Документы.ОформлениеСДИЗЗЕРНО.ЗаполнитьПартии(Объект, СписокСтрок, Истина);
...
Процедура ОбработатьВыборПартии(ВыбранноеЗначение, ТекущиеДанные) Экспорт
	...
	ОбновитьДанные(ВыбранноеЗначение,
		Новый Структура("ИмяПоля, ИдентификаторСтроки", "ТоварыПартия", ТекущиеДанные.ПолучитьИдентификатор()));
...
Функция ИнициализироватьДанныеЗаполненияСертификации()
	...
	Возврат Новый Структура("ТипДокумента, Номер, Дата, СрокДействия");
