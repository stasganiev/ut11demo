...
Функция ПараметрыРегистрацииСчетовФактурВыданных(Объект) Экспорт
	...
	Если ТипЗнч(Объект.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ДоговорСтруктура = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Договор, "ХозяйственнаяОперация, КомиссионерКонтрагент");
	Иначе
		ДоговорСтруктура = Новый Структура("ХозяйственнаяОперация, КомиссионерКонтрагент", Неопределено, Неопределено);
	КонецЕсли;
	...
	ПараметрыРегистрации.РеализацияЧерезКомиссионера = 
		КомиссионнаяТорговляСервер.РеализацияЧерезКомиссионера(ДоговорСтруктура.ХозяйственнаяОперация);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("aeb2c15d-5193-4318-91d2-cf1a555e5e55");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет первичные документы:
	|- Заполняет реквизиты ""Курс числитель"", ""Курс знаменатель""'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ПервичныйДокумент";
	...
	|	Документ.ПервичныйДокумент КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.КурсЧислитель = 0
	|	И ДанныеДокумента.КурсЗнаменатель = 0";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Замер = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(
		"Документ.ПервичныйДокумент.ОбработатьДанныеДляПереходаНаНовуюВерсию");

	ПолноеИмяОбъекта = "Документ.ПервичныйДокумент";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект.КурсЧислитель = 0 И ДокументОбъект.КурсЗнаменатель = 0 Тогда
					РаботаСКурсамиВалютУТ.ЗаполнитьКурсДокументаПоУмолчанию(
						ДокументОбъект.КурсЧислитель,
						ДокументОбъект.КурсЗнаменатель,
						ДокументОбъект.Валюта,
						ДокументОбъект.ВалютаВзаиморасчетов,
						ДокументОбъект.Организация,
						ДокументОбъект.Дата,
						ДокументОбъект.Договор,
						ДокументОбъект.ОбъектРасчетов)
			КонецЕсли;
