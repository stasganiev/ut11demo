...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ВосстановитьДанныеАдресаВМаршруте();
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИнтеграцияВЕТИСКлиент.ЭтоВыборЗаписиЖурнала(ВыбранноеЗначение) Тогда
		...
		НайденныеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("ЗаписьСкладскогоЖурнала", ВыбранноеЗначение.ЗаписьСкладскогоЖурнала));
...
Процедура ПереключательТТНПриИзменении(Элемент)
	...
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект, Элементы.Товары.ТекущаяСтрока, "ТТНУказан");
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТоварыСкоропортящаясяПродукция" Тогда
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Иначе
		...
		СобытияФормВЕТИСКлиентПереопределяемый.ПриИзмененииКоличества(ЭтотОбъект, ТекущиеДанные,
			КэшированныеЗначения, ПараметрыЗаполнения);
		ОбновитьКоличествоУпаковокПоФасовке(ТекущиеДанные.ПолучитьИдентификатор());
	КонецЕсли;
...
Процедура ТоварыЗаписьСкладскогоЖурналаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИнтеграцияВЕТИСКлиент.ЭтоВыборЗаписиЖурнала(ВыбранноеЗначение) Тогда
		...
		Если ВыбранноеЗначение.ЕдиницаИзмеренияВЕТИС <> ТекущиеДанные.ЕдиницаИзмеренияВЕТИС Тогда
			КоэффициентПересчета = ИнтеграцияВЕТИСВызовСервера.КоэффициентПересчетаМеждуЕдиницамиВЕТИС(
				ТекущиеДанные.ЕдиницаИзмеренияВЕТИС, ВыбранноеЗначение.ЕдиницаИзмеренияВЕТИС);
		КонецЕсли;
		ВыбранноеЗначение.КоэффициентПересчета = КоэффициентПересчета;
		Если ТекущиеДанные.КоличествоВЕТИС * КоэффициентПересчета > ВыбранноеЗначение.КоличествоВЕТИС Тогда
			ОповещениеВыбораЗаписи = Новый ОписаниеОповещения("ОбработатьВыборЗаписиЖурнала", ЭтотОбъект, ТекущиеДанные);
			ОповещениеИзмененияКоличества = Новый ОписаниеОповещения("ОбработатьИзменениеКоличества", ЭтотОбъект);
			СобытияФормВЕТИСКлиент.ОбработатьВыборЗаписиЖурналаСНедостаткомКоличества(
				Объект.Товары, ВыбранноеЗначение, ТекущиеДанные, ОповещениеВыбораЗаписи, ОповещениеИзмененияКоличества);
		Иначе
			ОбработатьВыборЗаписиЖурнала(ВыбранноеЗначение, ТекущиеДанные);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЗаполнитьПредставлениеПериодовПоСтроке(ЭтотОбъект, Элементы.Товары.ТекущиеДанные);
...
&НаКлиенте
Процедура ТоварыПанельСкоропортящаясяПродукцияПриИзменении(Элемент)
	...
	ЗаполнитьПредставлениеПериодовПоСтроке(ЭтотОбъект, ТекущиеДанные);
...
Процедура ОткрытьПодборТоваров(Команда)
	...
	ПараметрыПодбора = ИнтеграцияВЕТИСКлиентСервер.ПараметрыФормыПодбораТоваров(ЭтотОбъект, ПараметрыУказанияСерий);
...
Процедура Аннулировать(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ВСДКАннулированию = Новый Массив;
	Для Каждого СтрокаТовары Из Элементы.Товары.ВыделенныеСтроки Цикл
		ВСДСтроки = Объект.Товары.НайтиПоИдентификатору(СтрокаТовары).ВетеринарноСопроводительныйДокумент;
		Если ЗначениеЗаполнено(ВСДСтроки) Тогда
			ВСДКАннулированию.Добавить(ВСДСтроки);
		КонецЕсли;
	КонецЦикла;
	ПараметрыФормы.Вставить("ВСД", ВСДКАннулированию);
...
Процедура АрхивироватьДокумент(Команда)
	...
	ИнтеграцияИСКлиент.АрхивироватьДокументы(ЭтотОбъект, Объект.Ссылка, ИнтеграцияВЕТИСКлиент);
...
Функция ОписаниеОтбораЦелейВЕТИС(ГруппаСтрок = Ложь)
	...
	Возврат СтрШаблон(
		НСтр("ru = 'Наложен отбор по продукции: %1'"),
		Продукция);
...
Функция ДействияПриОбновленииРеквизитаТабличнойЧастиТовары()
	...
	Действия.Вставить("ОбновитьТипСыроеМолоко", Ложь);
...
Функция ДанныеСтрокиТабличнойЧастиТовары(СтрокаТабличнойЧасти)
	...
	ДанныеСтроки.Вставить("ТипСыроеМолоко");
...
Процедура ЗаполнитьПредставлениеПериодовПоСтроке(Форма, Строка)
	...
	Если Строка.ТипСыроеМолоко Тогда
...
Процедура ОбновитьРеквизитыТабличныхЧастей()
	...
	ПродукцияДокумента = Объект.Товары.Выгрузить(,"Продукция").ВыгрузитьКолонку("Продукция");
	ПродукцияЖивыеЖивотные = ИнтеграцияВЕТИСВызовСервера.ПродукцияПринадлежитТипуЖивыеЖивотные(ПродукцияДокумента);
	ПродукцияСыроеМолоко   = ИнтеграцияВЕТИСВызовСервера.ПродукцияПринадлежитТипуСыроеМолоко(ПродукцияДокумента);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТЧ.ТипСыроеМолоко = ПродукцияСыроеМолоко.Получить(СтрокаТЧ.Продукция);
		ОбновитьДатаПроизводстваСрокГодностиПоСтроке(СтрокаТЧ);
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		ЗаполнитьПредставлениеУпаковокПоСтроке(Объект, СтрокаТЧ);
	КонецЦикла;
...
Процедура РасчетСтрокиТоварыНаСервере(ДанныеСтроки, Действия, ДополнительныеПараметры = Неопределено)
	...
	КонецЕсли;
	Если Действия.ОбновитьТипСыроеМолоко Тогда
		ДанныеСтроки.ТипСыроеМолоко = ИнтеграцияВЕТИСВызовСервера.ПродукцияПринадлежитТипуСыроеМолоко(ДанныеСтроки.Продукция);
	КонецЕсли;
...
Процедура ПриСозданииЧтенииНаСервере(ОбновитьКомандыОснования = Истина)
	...
	ЗаполнитьПроизводителейИСрокиГодностиПоЗаписямЖурнала();
	ЗаполнитьФасовкуПоПродукции();
...
&НаСервере
Процедура ЗаполнитьПроизводителейИСрокиГодностиПоЗаписямЖурнала(ЗаписьЖурнала = Неопределено)
	...
	Иначе
		Если Не ЗначениеЗаполнено(ЗаписьЖурнала) Тогда
			...
			Возврат;
		КонецЕсли;
		Если Производители.НайтиСтроки(Новый Структура("ЗаписьСкладскогоЖурнала", ЗаписьЖурнала)).Количество() = 0 Тогда
	...
	Если ЗаписиЖурнала.Количество() = 0 И ЗначениеЗаполнено(ЗаписьЖурнала) Тогда
		ЗаписиЖурнала.Добавить(ЗаписьЖурнала);
	КонецЕсли;
	...
	|ГДЕ
	|	ЗаписиСкладскогоЖурналаВЕТИС.Ссылка В(&ЗаписиЖурнала)";
	
	ПериодыВЕТИС = Запрос.Выполнить().Выбрать();
	Пока ПериодыВЕТИС.Следующий() Цикл
		
		НадписьДатаПроизводства = ИнтеграцияВЕТИСКлиентСервер.ПредставлениеПериодаВЕТИС(
			ПериодыВЕТИС.ДатаПроизводстваТочностьЗаполнения,
			ПериодыВЕТИС.ДатаПроизводстваНачалоПериода,
			ПериодыВЕТИС.ДатаПроизводстваКонецПериода,
			ПериодыВЕТИС.ДатаПроизводстваСтрока);
		Если Не ПустаяСтрока(НадписьДатаПроизводства) Тогда
			НадписьДатаПроизводства = Новый ФорматированнаяСтрока(НадписьДатаПроизводства,,ЦветаСтиля.ЦветГиперссылкиГосИС,,"ДатаПроизводства");
		КонецЕсли;
		
		НадписьСрокГодности = ИнтеграцияВЕТИСКлиентСервер.ПредставлениеПериодаВЕТИС(
			ПериодыВЕТИС["СрокГодностиТочностьЗаполнения"],
			ПериодыВЕТИС["СрокГодностиНачалоПериода"],
			ПериодыВЕТИС["СрокГодностиКонецПериода"],
			ПериодыВЕТИС["СрокГодностиСтрока"]);
		Если Не ПустаяСтрока(НадписьСрокГодности) Тогда
			НадписьСрокГодности = Новый ФорматированнаяСтрока(НадписьСрокГодности,,ЦветаСтиля.ЦветГиперссылкиГосИС,,"СрокГодности");
		КонецЕсли;
		
		Строки = Объект.Товары.НайтиСтроки(Новый Структура("ЗаписьСкладскогоЖурнала", ПериодыВЕТИС.Ссылка));
		Для Каждого СтрокаТовары Из Строки Цикл
			ЗаполнитьЗначенияСвойств(СтрокаТовары, ПериодыВЕТИС);
			СтрокаТовары.НадписьДатаПроизводства = НадписьДатаПроизводства;
			СтрокаТовары.НадписьСрокГодности = НадписьСрокГодности;
		КонецЦикла;
...
&НаСервере
Процедура ЗаполнитьФасовкуПоПродукции(ВыбраннаяПродукция = Неопределено, ЗаписьСкладскогоЖурнала = Неопределено)
	
	Продукция = Новый Массив;
	ЗаписиСкладскогоЖурнала = Новый Массив;
	Если ВыбраннаяПродукция = Неопределено Тогда
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			Если ЗначениеЗаполнено(СтрокаТовары.Продукция) Тогда
				Продукция.Добавить(СтрокаТовары.Продукция);
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТовары.ЗаписьСкладскогоЖурнала) Тогда
				ЗаписиСкладскогоЖурнала.Добавить(СтрокаТовары.ЗаписьСкладскогоЖурнала);
			КонецЕсли;
		КонецЦикла;
		Фасовка.Очистить();
	Иначе
		Если Фасовка.НайтиСтроки(Новый Структура("Продукция", ЗаписьСкладскогоЖурнала)).Количество() = 0
				И ЗначениеЗаполнено(ЗаписьСкладскогоЖурнала)Тогда
			ЗаписиСкладскогоЖурнала.Добавить(ЗаписьСкладскогоЖурнала);
		КонецЕсли;
		Если Фасовка.НайтиСтроки(Новый Структура("Продукция", ВыбраннаяПродукция)).Количество() = 0 Тогда
			Продукция.Добавить(ВыбраннаяПродукция);
		КонецЕсли;
	КонецЕсли;
	
	Если Продукция.Количество() > 0 Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		|	Ссылка КАК Продукция,
		|	ФасовкаУпаковка КАК Упаковка,
		|	ФасовкаКоличествоЕдиницВУпаковке КАК КоличествоЕдиницВУпаковке,
		|	ФасовкаЕдиницаИзмерения КАК ЕдиницаИзмерения
		|ИЗ
		|	Справочник.ПродукцияВЕТИС КАК ПродукцияВЕТИС
		|ГДЕ
		|	Ссылка В (&Продукция)
		|И
		|	ФасовкаЕдиницаИзмерения <> ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмеренияВЕТИС.ПустаяСсылка)");
		Запрос.УстановитьПараметр("Продукция", Продукция);
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(Фасовка.Добавить(), Выборка);
		КонецЦикла;
	КонецЕсли;
	...
	Если ЗаписиСкладскогоЖурнала.Количество() > 0 Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		|	Ссылка КАК Продукция,
		|	УпаковкаВЕТИС КАК Упаковка,
		|	УровеньУпаковки КАК УровеньУпаковки,
		|	Ссылка.КоличествоВЕТИС / КоличествоУпаковокВЕТИС КАК КоличествоЕдиницВУпаковке,
		|	Ссылка.ЕдиницаИзмеренияВЕТИС КАК ЕдиницаИзмерения
		|ИЗ
		|	Справочник.ЗаписиСкладскогоЖурналаВЕТИС.УпаковкиВЕТИС КАК ЗаписиСкладскогоЖурналаВЕТИСУпаковкиВЕТИС
		|ГДЕ
		|	Ссылка В (&ЗаписиСкладскогоЖурнала)
		|	И КоличествоУпаковокВЕТИС > 0
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	КоличествоУпаковокВЕТИС Убыв
		|ИТОГИ ПО
		|	Ссылка");
		...
		Пока Выборка.Следующий() Цикл
			Упаковка = Выборка.Выбрать();
			Упаковка.Следующий();
			ЗаполнитьЗначенияСвойств(Фасовка.Добавить(), Упаковка);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьКоличествоУпаковокПоФасовке(ИдентификаторСтрокиТовары, ИзмененаПродукция = Ложь)
	
	ИзменяемаяСтрока = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтрокиТовары);
	УпаковкиСтроки = Объект.УпаковкиВЕТИС.НайтиСтроки(Новый Структура("ИдентификаторСтрокиТовары", ИзменяемаяСтрока.ИдентификаторСтроки));
	
	КоличествоУпаковокЗаполненоПоЗаписиСкладскогоЖурнала = Ложь;
	Если ЗначениеЗаполнено(ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала) Тогда
		СтрокаФасовка = Фасовка.НайтиСтроки(Новый Структура("Продукция", ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала));
		Если СтрокаФасовка.Количество() > 0 Тогда
			СтрокаФасовка = СтрокаФасовка[0];
			КоличествоУпаковок = Окр(ИзменяемаяСтрока.КоличествоВЕТИС
				* Справочники.ЕдиницыИзмеренияВЕТИС.КоэффициентПересчета(
					СтрокаФасовка.ЕдиницаИзмерения,
					ИзменяемаяСтрока.ЕдиницаИзмеренияВЕТИС)
				/ СтрокаФасовка.КоличествоЕдиницВУпаковке,3);
			Если КоличествоУпаковок = Цел(КоличествоУпаковок) Тогда
				Если УпаковкиСтроки.Количество() = 0 Тогда
					СтрокаУпаковка = Объект.УпаковкиВЕТИС.Добавить();
					СтрокаУпаковка.ИдентификаторСтрокиТовары = ИзменяемаяСтрока.ИдентификаторСтроки;
					СтрокаУпаковка.ИдентификаторСтроки = Новый УникальныйИдентификатор;
				Иначе
					СтрокаУпаковка = УпаковкиСтроки[0];
					Для Индекс = 1 По УпаковкиСтроки.ВГраница() Цикл
						Объект.УпаковкиВЕТИС.Удалить(УпаковкиСтроки[Индекс]);
					КонецЦикла;
				КонецЕсли;
				СтрокаУпаковка.УровеньУпаковки = СтрокаФасовка.УровеньУпаковки;
				СтрокаУпаковка.УпаковкаВЕТИС = СтрокаФасовка.Упаковка;
				СтрокаУпаковка.КоличествоУпаковокВЕТИС = КоличествоУпаковок;
				КоличествоУпаковокЗаполненоПоЗаписиСкладскогоЖурнала = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если КоличествоУпаковокЗаполненоПоЗаписиСкладскогоЖурнала Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаФасовка = Фасовка.НайтиСтроки(Новый Структура("Продукция", ИзменяемаяСтрока.Продукция));
	...
	Если ИзмененаПродукция Тогда
		Для Каждого СтрокаТабличнойЧасти Из УпаковкиСтроки Цикл
			Объект.УпаковкиВЕТИС.Удалить(СтрокаТабличнойЧасти);
		КонецЦикла;
		УпаковкиСтроки.Очистить();
	ИначеЕсли УпаковкиСтроки.Количество() > 1 
		Или (УпаковкиСтроки.Количество() = 1 И УпаковкиСтроки[0].УпаковкаВЕТИС <> СтрокаФасовка.Упаковка) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Автоматический пересчет упаковок не в базовой фасовке недоступен'"),,
			"Объект.Товары[" + ИдентификаторСтрокиТовары + "].КоличествоВЕТИС");
		Возврат;
	КонецЕсли;
	
	Если СтрокаФасовка.КоличествоЕдиницВУпаковке <> 0 Тогда
		КоличествоУпаковок = ИзменяемаяСтрока.КоличествоВЕТИС
			* Справочники.ЕдиницыИзмеренияВЕТИС.КоэффициентПересчета(
				СтрокаФасовка.ЕдиницаИзмерения,
				ИзменяемаяСтрока.ЕдиницаИзмеренияВЕТИС)
			/ СтрокаФасовка.КоличествоЕдиницВУпаковке;
	Иначе
	...
	Если КоличествоУпаковок <> Цел(КоличествоУпаковок) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Отгружаемое количество не соответствует фасовке'"),
			Объект.Ссылка,
			"Объект.Товары[" + ИдентификаторСтрокиТовары + "].КоличествоВЕТИС");
	КонецЕсли;
	...
	Если КоличествоУпаковок = 0 Тогда
		Для Каждого СтрокаТабличнойЧасти Из УпаковкиСтроки Цикл
			Объект.УпаковкиВЕТИС.Удалить(СтрокаТабличнойЧасти);
		КонецЦикла;
	ИначеЕсли УпаковкиСтроки.Количество() = 0 Тогда
		СтрокаУпаковка = Объект.УпаковкиВЕТИС.Добавить();
		СтрокаУпаковка.ИдентификаторСтрокиТовары = ИзменяемаяСтрока.ИдентификаторСтроки;
		СтрокаУпаковка.ИдентификаторСтроки = Новый УникальныйИдентификатор;
		СтрокаУпаковка.УровеньУпаковки = Перечисления.УровниУпаковокВЕТИС.ТорговыйУровень;
		СтрокаУпаковка.УпаковкаВЕТИС = СтрокаФасовка.Упаковка;
		СтрокаУпаковка.КоличествоУпаковокВЕТИС = КоличествоУпаковок;
	Иначе
		СтрокаУпаковка = УпаковкиСтроки[0];
	...
	ЗаполнитьПредставлениеУпаковокПоСтроке(Объект, ИзменяемаяСтрока);
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	Для Каждого Строка Из Объект.Товары Цикл
		...
		ЗаполнитьПредставлениеПериодовПоСтроке(ЭтотОбъект, Строка);
	КонецЦикла;
	...
	Иначе
		...
		Если ЕстьДополнительнаяСтрока Тогда
			...
			Иначе
				...
				ПредставлениеПредметаПродукции = Сред(ПредставлениеПредметаПродукции, СтрНайти(ПредставлениеПредметаПродукции, "|") + 1);
				ПредставлениеПредметаМаршрута  = Сред(ПредставлениеПредметаМаршрута, СтрНайти(ПредставлениеПредметаМаршрута, "|") + 1);
...
Процедура ОбновитьСтатусВЕТИС()
	...
	Исключения = "ТоварыНайти, ТоварыОтменитьПоиск, ТоварыСгенерироватьСерии, ТоварыКонтекстноеМенюКопированиеИВыделение, СвернутьПанель"
		+ ", РазвернутьПанель, ТоварыГруппаНоменклатура, ТоварыКоличество, Аннулировать";
	
	ИнтеграцияИСКлиентСервер.УстановитьДоступностьЭлементовГруппыФормыРекурсивно(Элементы.СтраницаТовары,
	...
	Если СтатусВЕТИС = Перечисления.СтатусыОбработкиИсходящихТранспортныхОперацийВЕТИС.Ошибка Тогда
		ТаблицаОшибок = ИнтеграцияВЕТИСВызовСервера.ОшибкиСервисаПоДокументу(ДокументОбъект.Ссылка);
...
Функция ЗаполнитьЗаписиСкладскогоЖурналаНаСервере()
	...
	Документы.ИсходящаяТранспортнаяОперацияВЕТИС.ЗаполнитьПотребительскиеУпаковки(Объект, РезультатЗаполнения.ИзмененныеСтроки);
	Для Каждого ЗаполненнаяСтрока Из РезультатЗаполнения.ИзмененныеСтроки Цикл
		ЗаполнитьПредставлениеУпаковокПоСтроке(Объект, ЗаполненнаяСтрока);
	КонецЦикла;
	ЗаполнитьПроизводителейИСрокиГодностиПоЗаписямЖурнала();
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ГрузополучательХозяйствующийСубъект")
		...
		Если (Не ЗначениеЗаполнено(Объект.ГрузополучательХозяйствующийСубъект)
			И ЗначениеЗаполнено(Форма.ГрузополучательИзОснованияДляОтбораХС))
			Или Форма.ГрузополучательСопоставлениеХСДляОтбораОснований.НайтиПоЗначению(Форма.ГрузополучательИзОснованияДляОтбораХС) <> Неопределено Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.ГрузополучательХозяйствующийСубъект)
			И ЗначениеЗаполнено(Форма.ГрузополучательИзОснованияДляОтбораХС)
			И Форма.ГрузополучательСопоставлениеХСДляОтбораОснований.НайтиПоЗначению(Форма.ГрузополучательИзОснованияДляОтбораХС) = Неопределено Тогда
	...
	Если СтруктураРеквизитов.Свойство("Товары") И ТекущиеДанные <> Неопределено Тогда
		...
		Элементы.ТоварыПанельГруппаСведенияОКарантинировании.Видимость = ТекущиеДанные.ТипЖивыеЖивотные;
		Элементы.ТоварыПанельНизкокачественнаяПродукция.Видимость      = Не ТекущиеДанные.ТипЖивыеЖивотные;
		Элементы.ТоварыПанельЭкспертизаРезультат.Видимость             = Не ТекущиеДанные.ТипЖивыеЖивотные;
		...
		Элементы.ТоварыПанельСкоропортящаясяПродукция.ТолькоПросмотр   = ТекущиеДанные.ТипЖивыеЖивотные;
		
		Элементы.ТоварыНадписьСрокГодности.Видимость      = Не ТекущиеДанные.ТипЖивыеЖивотные;
		Элементы.ТоварыНадписьДатаПроизводства.Заголовок = ИнтеграцияВЕТИСКлиентСервер.ПредставлениеПоляДатаПроизводства(ТекущиеДанные.ТипЖивыеЖивотные);
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения, ИдентификаторСтрокиТовары)
	...
	ДанныеВыбранногоСкладскогоЖурнала = Неопределено;
	...
	Если ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурналаДобавление")
		...
		ЗаполнитьСтрокуПоДаннымЗаписиСкладскогоЖурнала(ИзменяемаяСтрока, ДанныеВыбранногоСкладскогоЖурнала);
		...
		ИзменяемаяСтрока.НадписьДатаПроизводства = Неопределено;
		ИзменяемаяСтрока.НадписьСрокГодности = Неопределено;
		
		ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект, Объект.Товары);
		ЗаполнитьСтатусыУказанияСерийСервер();
		ЗаполнитьПроизводителейИСрокиГодностиПоЗаписямЖурнала(ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала);
		ЗаполнитьПредставлениеВСДПоСтроке(ИзменяемаяСтрока, ТекстЗапрещеннойЯчейкиЦвет);
	...
	Если ДополнительныеПараметры.Свойство("ТоварыПродукция")
		...
		Действия.ОбновитьТипСыроеМолоко               = Истина;
		
		ИнтеграцияВЕТИС.ПроверитьОчиститьЕдиницуИзмеренияВЕТИС(ИзменяемаяСтрока);
		
		ЗаполнитьФасовкуПоПродукции(ИзменяемаяСтрока.Продукция);
		ОбновитьКоличествоУпаковокПоФасовке(ИдентификаторСтрокиТовары, Истина);
		
		ДанныеСтроки = ДанныеСтрокиТабличнойЧастиТовары(ИзменяемаяСтрока);
		
		ДопПараметры = Новый Структура;
		ДопПараметры.Вставить("КэшируемыеПараметрыРегионализации", КэшируемыеПараметрыРегионализации);
		
		РасчетСтрокиТоварыНаСервере(ДанныеСтроки, Действия, ДопПараметры);
		...
		Если ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурнала")
			И ИзменяемаяСтрока.Количество > 0 Тогда
			
			ПараметрыЗаполнения.ПересчитатьКоличествоЕдиницВЕТИС = Истина;
			
			СобытияФормВЕТИСПереопределяемый.ПриИзмененииКоличества(ЭтотОбъект, ИзменяемаяСтрока,
				КэшированныеЗначения, ПараметрыЗаполнения);
			
			Если ЗначениеЗаполнено(ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала)
				И (ДанныеВыбранногоСкладскогоЖурнала = Неопределено
				   Или ДанныеВыбранногоСкладскогоЖурнала.КоличествоВЕТИС < ИзменяемаяСтрока.КоличествоВЕТИС) Тогда
				
				ИндексСтроки = Объект.Товары.Индекс(ИзменяемаяСтрока);
				
				ТекстСообщения = НСтр( "ru = 'Остатка продукции по записи складского журнала %1 не достаточно для оформления ВСД.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала);
				ОбщегоНазначения.СообщитьПользователю(
					ТекстСообщения,
					Объект.Ссылка,
					"Объект.Товары[" + ИндексСтроки + "].КоличествоВЕТИС");
		...
		ДопПараметры = Новый Структура("Объект, ТекстЗапрещеннойЯчейкиЦвет", Объект, ТекстЗапрещеннойЯчейкиЦвет);
		Если ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурналаДобавление") Тогда
			ЗаполнитьСлужебныеРеквизитыТоваров(ИзменяемаяСтрока, , ДопПараметры);
		ИначеЕсли ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурнала") Тогда
			ЗаполнитьСлужебныеРеквизитыТоваров(ИзменяемаяСтрока, "ЗаписьСкладскогоЖурнала", ДопПараметры);
		КонецЕсли;
		
		ОбновитьДатаПроизводстваСрокГодностиПоСтроке(ИзменяемаяСтрока);
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтеграцияВЕТИС.УстановитьУсловноеОформлениеПоляКоличество(ЭтотОбъект);
	
	// разрешить редактирование только для продукции Сырое молоко
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
	ЭлементУсловногоОформления.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСкоропортящаясяПродукция.Имя);
	ЭлементУсловногоОформления.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыНадписьДатаПроизводства.Имя);
	ЭлементУсловногоОформления.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыНадписьСрокГодности.Имя);
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ТипСыроеМолоко");
	...
	Для каждого Поле Из Элементы.Товары.ПодчиненныеЭлементы Цикл
		Если Поле.Имя = "ТоварыГруппаНоменклатура"
			Или Поле.Имя = "ТоварыНоменклатураЕдиницаИзмерения"
			Или Поле.Имя = "ТоварыКоличество" Тогда
			Продолжить;
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСтрокуПоДаннымЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти, ДанныеСкладскогоЖурнала = Неопределено)
	...
	Если Выборка.Следующий() Тогда
		...
		РеквизитыДляЗаполнения = "НизкокачественнаяПродукция, ПериодНахожденияЖивотныхНаТерриторииТС,
		                         ...
		                         |МестоДляКарантинирования, КоличествоДнейДляКарантинирования, Продукция,
		                         |СкоропортящаясяПродукция, СрокГодностиСтрока, СрокГодностиТочностьЗаполнения,
		                         |СрокГодностиНачалоПериода, СрокГодностиКонецПериода,
		                         |ДатаПроизводстваСтрока, ДатаПроизводстваТочностьЗаполнения,
		                         |ДатаПроизводстваНачалоПериода, ДатаПроизводстваКонецПериода";
		
		
		Если СтрокаТабличнойЧасти.КоличествоВЕТИС = 0 Тогда
			РеквизитыДляЗаполнения = РеквизитыДляЗаполнения + "," + "КоличествоВЕТИС";
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, Выборка, РеквизитыДляЗаполнения);
		Коэффициент = ?(Выборка.КоличествоВЕТИС = 0, 1, СтрокаТабличнойЧасти.Количество/Выборка.КоличествоВЕТИС);
		...
		Для Каждого СтрокаУпаковки Из Выборка.УпаковкиВЕТИС.Выгрузить() Цикл
			...
			НоваяСтрокаУпаковки.КоличествоУпаковокВЕТИС = НоваяСтрокаУпаковки.КоличествоУпаковокВЕТИС * Коэффициент;
			Если НоваяСтрокаУпаковки.КоличествоУпаковокВЕТИС = 0 Тогда
				НоваяСтрокаУпаковки.КоличествоУпаковокВЕТИС = 1;
			КонецЕсли;
		...
		ДанныеСкладскогоЖурнала = Новый Структура("КоличествоВЕТИС");
		ЗаполнитьЗначенияСвойств(ДанныеСкладскогоЖурнала, Выборка);
...
Процедура ОткрытьФормуРедактированияПунктовМаршрута()
	...
	ПараметрыФормы.ТолькоПросмотр = ТолькоПросмотр или РедактированиеФормыНедоступно;
	ПараметрыФормы.АдресВоВременномХранилище = РезультатВыгрузкиТабличныхЧастей.АдресВоВременномХранилище;
	ПараметрыФормы.АдресВоВременномХранилищеТовары = РезультатВыгрузкиТабличныхЧастей.АдресВоВременномХранилищеТовары;
	ПараметрыФормы.АдресВоВременномХранилищеРегионализация = РезультатВыгрузкиТабличныхЧастей.АдресВоВременномХранилищеРегионализация;
	ПараметрыФормы.УказываетсяРегионализация = Истина;
	ПараметрыФормы.УказываетсяТранспортноеСредствоПредприятия = Истина;
	ПараметрыФормы.БлокироватьПервуюСтроку = Истина;
	ПараметрыФормы.ВладелецСсылка =Объект.Ссылка;
	ПараметрыФормы.ХозяйствующийСубъект = Объект.ГрузоотправительХозяйствующийСубъект;
	ПараметрыФормы.ОбязательностьНомераТранспортногоСредства = Истина;
...
#Область ДатыСрокиГодности

&НаСервере
Процедура ОбновитьДатаПроизводстваСрокГодностиПоСтроке(СтрокаТабличнойЧасти)
	
	Если СтрокаТабличнойЧасти.ТипСыроеМолоко Тогда
		...
		ЗаполнитьПредставлениеПериодовПоСтроке(ЭтотОбъект, СтрокаТабличнойЧасти);
...
Процедура ОбработатьВводПериода(ВидПериода, РежимТолькоПросмотр = Ложь)
	...
	Если ВидПериода = "СрокГодности" Тогда
		ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияСрокаГодности(ЭтотОбъект,
			СтрокаТабличнойЧасти,
			ОбработчикОповещения,
			РежимТолькоПросмотр,
			СписокДоступнойТочности);
	Иначе
		ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияДатыПроизводства(ЭтотОбъект,
			СтрокаТабличнойЧасти,
			ОбработчикОповещения,
			РежимТолькоПросмотр, 
			СписокДоступнойТочности);
...
&НаСервере
Процедура ВосстановитьДанныеАдресаВМаршруте()
	
	Если Параметры.Свойство("Основание")
		И ТипЗнч(Параметры.Основание) = Тип("Структура")
		И Параметры.Основание.Свойство("ДанныеАдресаМаршрут")
		И ТипЗнч(Параметры.Основание.ДанныеАдресаМаршрут) = Тип("ФиксированноеСоответствие") Тогда
		Для Каждого КлючИЗначение Из Параметры.Основание.ДанныеАдресаМаршрут Цикл
			Объект.Маршрут[КлючИЗначение.Ключ-1].ДанныеАдресаСтруктура = КлючИЗначение.Значение;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбработатьВводПериодаЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Результат <> Неопределено Тогда
		...
		Если (ДополнительныеПараметры.ВидПериода = "ДатаПроизводства"
			...
			РеквизитыСтрокиТовары = Новый Структура(
				"Продукция, ДатаПроизводстваНачалоПериода,
				|СрокГодностиНачалоПериода, СрокГодностиТочностьЗаполнения,
				|ДатаПроизводстваТочностьЗаполнения");
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		СобытияФормВЕТИСКлиентПереопределяемый.ПриИзмененииКоличества(ЭтотОбъект, НоваяСтрока,
			КэшированныеЗначения, ПараметрыЗаполнения);
		ОбновитьКоличествоУпаковокПоФасовке(НоваяСтрока.ПолучитьИдентификатор());
...
&НаКлиенте
Процедура ОбработатьВыборЗаписиЖурнала(ВыбранноеЗначение, ТекущиеДанные) Экспорт
	
	СобытияФормВЕТИСКлиент.ОбработатьВыборЗаписиСкладскогоЖурналаИзФормыПодбора(ТекущиеДанные, ВыбранноеЗначение);
	ОбновитьДанные(ВыбранноеЗначение.ЗаписьСкладскогоЖурнала, "ТоварыЗаписьСкладскогоЖурнала");
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьИзменениеКоличества(ВыбранноеЗначение, ТекущиеДанные) Экспорт
	
	ОбновитьКоличествоУпаковокПоФасовке(ВыбранноеЗначение.ПолучитьИдентификатор());
