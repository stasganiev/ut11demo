...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если Маршрут.Количество() Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения)<>Тип("Структура") Тогда
			ДанныеЗаполнения = Новый Структура("ДанныеЗаполнения", ДанныеЗаполнения);
		КонецЕсли;
		ДанныеЗаполненияМаршрут = Новый Соответствие;
		Для Каждого СтрокаМаршрут Из Маршрут Цикл
			Если СтрокаМаршрут.ДанныеАдреса.Получить()<>Неопределено Тогда
				ДанныеЗаполненияМаршрут.Вставить(СтрокаМаршрут.НомерСтроки, СтрокаМаршрут.ДанныеАдреса.Получить());
			КонецЕсли;
		КонецЦикла;
		ДанныеЗаполнения.Вставить("ДанныеАдресаМаршрут", Новый ФиксированноеСоответствие(ДанныеЗаполненияМаршрут));
		
	ИначеЕсли ЗначениеЗаполнено(ГрузоотправительПредприятие) Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		КонечныеСтатусы = Документы.ИсходящаяТранспортнаяОперацияВЕТИС.КонечныеСтатусы(Ложь);
	...
	Если ВыполнятьПроверкиРеквизитовВетИС Тогда
		...
		Для Каждого СтрокаТовары Из Товары Цикл
			...
			Если СтрокаТовары.ПериодНахожденияЖивотныхНаТерриторииТС = Перечисления.ПериодыНахожденияЖивотныхНаТерриторииТСВЕТИС.ЗначениеВМесяцах
				...
				ШаблонСообщения = НСтр("ru = 'В строке №%1 не заполнено количество периодов нахождения животных на территории Т.С.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Товары.Индекс(СтрокаТовары) + 1);
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка,, "Объект", Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.ОсобыеОтметки) Тогда
			СтрокаТЧ.ХешСуммаОсобыеОтметки = ОбщегоНазначенияИС.ХэшСуммаСтроки(СтрокаТЧ.ОсобыеОтметки);
		Иначе
			СтрокаТЧ.ХешСуммаОсобыеОтметки = "";
		КонецЕсли;
	КонецЦикла;
