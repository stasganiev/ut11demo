...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДлительнаяОперация = ОбновитьДанныеДляНастройкиОтправкиПриглашений();
	Если ДлительнаяОперация.Статус = "Выполняется" Тогда
		Возврат;
	КонецЕсли;
	
	ПодготовитьФормуПослеОбновленияДанных(ДлительнаяОперация.АдресРезультата);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ДлительнаяОперация = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	Оповещение = Новый ОписаниеОповещения("ОбновитьДанныеДляНастройкиОтправкиПриглашенийЗавершение", ЭтотОбъект);
...
Процедура Далее(Команда)
	...
	Контекст = НовыйКонтекстПодготовкиВторогоЭтапа();
	
	СинхронизоватьАбонентовИПодготовитьСтраницуВторогоЭтапа(Контекст.ИзменятьФильтр);
...
&НаСервере
Функция ОбновитьДанныеДляНастройкиОтправкиПриглашений()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ИнтерфейсДокументовЭДО.ОбновитьДанныеДляНастройкиОтправкиПриглашений");
	
КонецФункции

&НаКлиенте
Процедура ОбновитьДанныеДляНастройкиОтправкиПриглашенийЗавершение(Результат, Контекст) Экспорт
	...
	ПодготовитьФормуПослеОбновленияДанных(Результат.АдресРезультата);
...
&НаСервере
Процедура ПодготовитьФормуПослеОбновленияДанных(АдресРезультата)
	
	КонтекстДиагностики = ПолучитьИзВременногоХранилища(АдресРезультата); // См. ИнтерфейсДокументовЭДО.ОбновитьДанныеДляНастройкиОтправкиПриглашений
...
&НаСервере
Процедура СинхронизоватьАбонентовИПодготовитьСтраницуВторогоЭтапа(ИзменятьФильтр)
	...
	ПодготовитьСтраницуВторогоЭтапа(ДлительнаяОперация.АдресРезультата, ИзменятьФильтр);
...
Функция ЗаполнитьПараметрыСвязиИдентификаторовЭДОПослеСинхронизацииАбонентов()
	...
	ПараметрыПолученияСвязи = ИнтерфейсДокументовЭДО.НовыеПараметрыПолученияСвязиИдентификаторов();
	ПараметрыПолученияСвязи.Организация = Организация;
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ИнтерфейсДокументовЭДО.ПараметрыСвязиИдентификаторов", ПараметрыПолученияСвязи,, Истина);
	
КонецФункции

// Возвращаемое значение:
//  Структура:
//  * ИзменятьФильтр - Булево
&НаКлиенте
Функция НовыйКонтекстПодготовкиВторогоЭтапа()
	Контекст = Новый Структура;
	Контекст.Вставить("ИзменятьФильтр", Истина);
	Возврат Контекст;
КонецФункции

// Параметры:
//  Результат - см. ДлительныеОперации.ВыполнитьФункцию
//  Контекст - см. НовыйКонтекстПодготовкиВторогоЭтапа
&НаКлиенте
Процедура ПодготовитьСтраницуВторогоЭтапаПослеСинхронизацииАбонентов(Результат, Контекст) Экспорт
	...
	ПодготовитьСтраницуВторогоЭтапа(Результат.АдресРезультата, Контекст.ИзменятьФильтр);
	
КонецПроцедуры

&НаСервере
Процедура ПодготовитьСтраницуВторогоЭтапа(Знач АдресРезультата, Знач ИзменятьФильтр)
...
Процедура ЗаполнитьРеквизитыПриглашения(ТекущиеДанные, ПараметрыСвязи)
	...
	ТекущиеДанные.ЭтоОблачныйЭДО = ПараметрыСвязи.ЭтоОблачныйЭДО;
...
Процедура ОтправитьПриглашения()
	...
	ПриглашенияЭДОСлужебныйКлиент.ОтправитьПриглашения(ПриглашенияКОтправке, ЭтотОбъект, Оповещение);
