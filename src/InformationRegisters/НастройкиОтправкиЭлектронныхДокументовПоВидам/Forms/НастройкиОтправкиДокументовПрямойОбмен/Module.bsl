...
Функция ЭтоНоваяНастройка()
	...
	Запрос.Текст = 
		...
		|	И НастройкиОтправкиЭлектронныхДокументов.Договор В (&Договор)
...
Функция ЗаписатьНастройкиНаСервере(КонтекстДиагностики)
	...
	Попытка
		...
		Если ОбщиеНастройкиПолученияОтсутствуют() Тогда
			...
			ТаблицаПредопределенногоПрофиля = ОтражениеВУчетеЭДО.ШаблонНастроекОтраженияВУчете(
				ОтражениеВУчетеЭДО.ПрофилиНастроекОтраженияВходящихДокументов().ПервоначальноеЗаполнение);
