...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Записано
		...
		Если МенеджерОборудованияИСКлиентПовтИсп.ИспользуетсяПодсистемаБиблиотекиБПО3() Тогда
			Запись.РабочееМесто = МодульМенеджерОборудованияВызовСервера.РабочееМестоКлиента();
		Иначе
			Запись.РабочееМесто = МодульМенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ОбменДаннымиЕГАИСКлиент.УстановитьПризнакНеобходимостиОбновленияНастроекОбменаПоРасписанию();
...
Процедура ТекстСопоставлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "КомандаЗапроситьКодФСРАР" Тогда
		...
		ПроверитьЗаполнениеПолучитьКодФСРАРИзУТМ();
...
Процедура ЗаполнитьДанные()
	...
	Если ЗначениеЗаполнено(СокрЛП(Запись.ИдентификаторФСРАР)) Тогда
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Код", СокрЛП(Запись.ИдентификаторФСРАР));
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	КлассификаторОрганизацийЕГАИС.СоответствуетОрганизации КАК СоответствуетОрганизации
		|ИЗ
		...
		Если Выборка.Следующий() Тогда
			...
			ДанныеСопоставления.Вставить("ОрганизацияЕГАИС", Выборка.Ссылка);
			ДанныеСопоставления.Вставить("Организация", Выборка.Контрагент);
			ДанныеСопоставления.Вставить("ТорговыйОбъект", Выборка.ТорговыйОбъект);
			ДанныеСопоставления.Вставить("СоответствуетОрганизации", Выборка.СоответствуетОрганизации);
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеСопоставления <> Неопределено Тогда
		...
		СоответствуетОрганизации = ДанныеСопоставления.СоответствуетОрганизации;
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(Выборка.Ссылка);
...
Процедура СоздатьЭлементКлассификатораОрганизацийЕГАИС()
	...
	ОрганизацияОбъект.ФорматОбмена = ОбменДаннымиЕГАИСКлиентСервер.ФорматОбмена();
...
Процедура ИзменитьРасписаниеОбменаНаСервере()
	...
	Если ЗаданияОбмена.Количество() <> 1 Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не найдено регламентное задание обработки ответов ЕГАИС'"));
		ОбменПоРасписанию = Ложь;
	Иначе
...
Функция КодФСРАРИзУТМНаСервере(НастройкаОбмена)
	...
	РезультатПолученияКодаФСРАР = ОбменДаннымиЕГАИСКлиентСервер.КодФСРАРv3(НастройкаОбмена);
	Если РезультатПолученияКодаФСРАР.ПроверитьУТМv4 Тогда
		РезультатПолученияКодаФСРАР = ОбменДаннымиЕГАИСКлиентСервер.КодФСРАРv4(НастройкаОбмена);
	КонецЕсли;
	
	Возврат РезультатПолученияКодаФСРАР;
...
Функция КодФСРАРИзУТМНаКлиенте(НастройкаОбмена)
	...
	#Если ВебКлиент Тогда
		...
		Результат.Вставить("КодФСРАР","");
		Результат.Вставить("ТекстОшибки", НСтр("ru = 'Обмен с ЕГАИС из вебклиента недоступен. Используйте тонкий клиент.'"));
		Возврат Результат;
		
	#Иначе
		
		Результат = ОбменДаннымиЕГАИСКлиентСервер.КодФСРАРv3(НастройкаОбмена);
		Если Результат.ПроверитьУТМv4 Тогда
			Результат = ОбменДаннымиЕГАИСКлиентСервер.КодФСРАРv4(НастройкаОбмена);
		КонецЕсли;
...
Процедура ПолучитьКодФСРАРИзУТМ(ОписаниеОповещения, Таймаут)
	...
	НастройкаОбмена = ОбменДаннымиЕГАИСКлиентСервер.НоваяНастройкаОбменаЕГАИС();
...
&НаКлиенте
Процедура ПроверитьЗаполнениеПолучитьКодФСРАРИзУТМ()
