...
// 
// Параметры:
//   ИдентификаторСессии - Строка - Идентификатор сесии
...
// Возвращает стурктуру для обновления записи по идентификаторам сесии и запроса
//
...
//     * ПокупкаПодтверждена - Булево, Неопределено - флаг подтверждения покупк 
//
	...
	Для Каждого РезультатПроверки Из РезультатыПроверки Цикл
		...
		Если ЗапросКМ.ЗапросРазрешенияПродажиКМ Тогда
			...
			Если ИнформацияПоРазрешительномуЗапросуПолученаРанее(ЗапросКМ) Тогда
				ЗаполнитьИнформациюПоРазрешительномуЗапросуПолученнуюРанее(Запись, Запись.ИдентификаторЗапросаРазрешения);
			КонецЕсли;
...
// 
// Параметры:
//   ИдентификаторСессии - Строка - иденификатор сессии проверки кода маркировки
...
Функция ИнформацияПоРазрешительномуЗапросуПолученаРанее(ЗапросКМ)
		...
		Если ПоложительныйОтветРазрешительногоРежима(ЗапросКМ.КодОтветаРазрешения)
...
Процедура ЗаполнитьИнформациюПоРазрешительномуЗапросуПолученнуюРанее(Запись, ИдентификаторЗапросаРазрешения)
	...
	Запрос.Текст = 
		...
		|	ОперацииПроверкиКМ.Дата КАК Дата
		|ИЗ
		|	РегистрСведений.ОперацииПроверкиКМ КАК ОперацииПроверкиКМ
		|ГДЕ
		|	ОперацииПроверкиКМ.ИдентификаторЗапросаРазрешения = &ИдентификаторЗапросаРазрешения
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
	
	Запрос.УстановитьПараметр("ИдентификаторЗапросаРазрешения", ИдентификаторЗапросаРазрешения);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		Запись.АдресСервераЗапросаРазрешения = Выборка.АдресСервераЗапросаРазрешения;
		Запись.ТекстЗапросаРазрешения = Новый ХранилищеЗначения(Выборка.ТекстЗапросаРазрешения.Получить());
		Запись.РезультатЗапросаРазрешения = Новый ХранилищеЗначения(Выборка.РезультатЗапросаРазрешения.Получить());
	КонецЕсли;
