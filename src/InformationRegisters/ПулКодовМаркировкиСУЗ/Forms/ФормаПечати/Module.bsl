	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаНоменклатура.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаНоменклатура.Номенклатура) Тогда
				СтрокаСоставаУпаковки.ПредставлениеНоменклатуры = ИнтеграцияИС.ПредставлениеНоменклатуры(
		...
		Если СодержимоеУпаковки.Количество() = 1 Тогда
			Содержимое = ИнтеграцияИС.ПредставлениеНоменклатуры(
...
Процедура ЗаполнитьЭлементДереваПоСтрокеДерева(Приемник, Источник, ПараметрыУпаковки, ПоляДерева)
	...
	Иначе
		...
		Иначе
			...
			Иначе
				Приемник.Содержимое = ИнтеграцияИС.ПредставлениеНоменклатуры(Источник.Номенклатура, Источник.Характеристика);
				Если ВидПродукции <> Неопределено Тогда
