...
Процедура ДоверенностьПриИзменении(Элемент)
	...
	МашиночитаемыеДоверенностиВызовСервера.ТипПоддержанОператором(
		ТекущиеДанные.ОператорЭДОИд, ТипЗнч(Доверенность));
...
Процедура ПодписатьИОтправитьПовторно(Команда)
	...
	ИначеЕсли Операция = УчетныеЗаписиЭДОКлиентСервер.ОперацииПомощникаРегистрацииСертификатов().Отправка Тогда
		ИнтерфейсДокументовЭДОКлиент.ОтправитьПолучитьДокументыЭДО(ЭтотОбъект);
		Закрыть();
...
&НаСервере
Функция РезультатПроверкиСертификатаИДоверенности(Организация, ИдентификаторОрганизации, Сертификат, СпособОбмена, ОператорЭДО)
	...
	Результат.Вставить("ТребуетсяДоверенность",
		УчетныеЗаписиЭДОСлужебный.ТребуетсяДоверенность(Организация, Сертификат, СпособОбмена, ОператорЭДО));
...
Процедура ВыполнитьПроверкиПередДобавлениемСертификата(Сертификат, ДанныеПрофиля)
	...
	РезультатыПроверки = РезультатПроверкиСертификатаИДоверенности(Организация, ДанныеПрофиля.ИдентификаторОрганизации,
		Сертификат, ДанныеПрофиля.СпособОбменаЭД, ДанныеПрофиля.ОператорЭДОИд);
	...
	МашиночитаемыеДоверенностиВызовСервера.ТипПоддержанОператором(
		ДанныеПрофиля.ОператорЭДОИд, ТипЗнч(ДанныеПрофиля.Доверенность));
...
Процедура ЗарегистрироватьСертификатВСервисе1СЭДО(ТекущиеДанные)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Доверенность) Тогда
		ПараметрыРегистрации.Доверенности.Вставить(ТекущиеДанные.Сертификат, ТекущиеДанные.Доверенность);
	КонецЕсли;
	
	СертификатыИДоверенностиУчетнойЗаписи = 
		СертификатыИДоверенностиУчетнойЗаписи(ТекущиеДанные.ИдентификаторОрганизации);
	
	Для Каждого Запись Из СертификатыИДоверенностиУчетнойЗаписи Цикл
		ПараметрыРегистрации.СертификатыПодписейОрганизации.Добавить(Запись.Ключ);
		Если ЗначениеЗаполнено(Запись.Значение) Тогда
			ПараметрыРегистрации.Доверенности.Вставить(Запись.Ключ, Запись.Значение);
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция СертификатыИДоверенностиУчетнойЗаписи(ИдентификаторУчетнойЗаписи)
	
	Возврат УчетныеЗаписиЭДОСлужебный.СертификатыИДоверенностиУчетнойЗаписи(ИдентификаторУчетнойЗаписи);
...
Процедура ДобавитьСертификатыВПрофиляЭДОНаСервере(Знач МассивДанных, СписокОшибок)
	...
	Таблица.Колонки.Добавить("Доверенность"            , Метаданные.ОпределяемыеТипы.МашиночитаемаяДоверенность.Тип);
