...
Функция ВыбранныеЧекиДляФискализации()
	...
	Для Каждого КлючЗаписи Из ВыделенныеСтроки Цикл
		Данные = РаспределеннаяФискализация.ДанныеЧекаВОчереди(КлючЗаписи.ИдентификаторЗаписи);
...
&НаСервере
Функция ДанныеЧекаВОчереди(ИдентификаторЗаписи)
	
	Возврат РаспределеннаяФискализация.ДанныеЧекаВОчереди(ИдентификаторЗаписи).ДанныеЧека;
	
КонецФункции

&НаСервере
Процедура ЗаписатьСтатусЧекаВОчереди(ОбщиеПараметры, СтатусЧека, ИдентификаторУстройстваККТ)
	
	РаспределеннаяФискализация.ЗаписатьСтатусЧекаВОчереди(ОбщиеПараметры, СтатусЧека, ИдентификаторУстройстваККТ);
...
Процедура НачатьФискализациюЧеков()
	...
	Если НомерСтроки < ЧекиДляФискализации.Количество() Тогда
		...
		Если Данные.СтатусЧека = ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Новый") 
			...
			ОбщиеПараметры = ДанныеЧекаВОчереди(Данные.ИдентификаторЗаписи);
...
Процедура НачатьФискализациюЧеков_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	РаспределеннаяФискализацияВызовСервера.ЗаписатьСтатусЧекаВОчереди(Параметры, СтатусЧека, Неопределено, ТекстСообщения);
