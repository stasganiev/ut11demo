...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьСписокВыбораДляТипаРегистрации();
	...
	ТипРегистрации = ТипРегистрацииНовыйИдентификатор();
...
Процедура ТипРегистрацииСуществующаяУчетнаяЗаписьПриИзменении(Элемент)
	...
	ПриИзмененииТипаРегистрацииСуществующейУчетнойЗаписи();
...
Процедура ТипРегистрацииНоваяУчетнаяЗаписьПриИзменении(Элемент)
	...
	ПриИзмененииТипаРегистрацииНовойУчетнойЗаписи();
...
Процедура ИдентификаторОрганизацииЧерез1СЭДОПриИзменении(Элемент)
	...
	ПриИзмененииИдентификатораОрганизацииЧерез1СЭДО();
...
Процедура ДоверенностьПриИзменении(Элемент)
	...
	МашиночитаемыеДоверенностиВызовСервера.ТипПоддержанОператором(
				ОператорЭДО, ТипЗнч(Доверенность));
...
Процедура НадписьПодключениеТакскомВМоделиСервисаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "ВыбратьСуществующий" Тогда
		СтандартнаяОбработка = Ложь;
		ТипРегистрации = ТипРегистрацииСуществующийИдентификатор();
		УправлениеФормой(ЭтотОбъект);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаУведомления.Видимость = Элементы.ГруппаУведомления.Видимость
		И Форма.ТипРегистрации = ТипРегистрацииНовыйИдентификатор();
	...
	Элементы.ИдентификаторОрганизацииЧерез1СЭДО.Доступность = Форма.ТипРегистрации = ТипРегистрацииСуществующийИдентификатор();
	Элементы.ОператорЭДОПодключенныйК1СЭДО.Доступность      = Форма.ТипРегистрации = ТипРегистрацииНовыйИдентификатор();
	
	ТребуетсяЗаявкаНаРегистрацию = Ложь;
	ПодключениеТакскомВМоделиСервиса = ПодключениеТакскомВМоделиСервиса(Форма);
	Если Форма.ТипРегистрации = ТипРегистрацииНовыйИдентификатор() И ПодключениеТакскомВМоделиСервиса Тогда
	...
	Элементы.Доверенность.АвтоОтметкаНезаполненного = Форма.ДоверенностьОбязательна;
	...
	Если ЗначениеЗаполнено(Форма.Организация)
		...
		И ((Форма.ТипРегистрации = ТипРегистрацииНовыйИдентификатор() И ЗначениеЗаполнено(Форма.ОператорЭДО))
			Или (Форма.ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() И ЗначениеЗаполнено(Форма.ИдентификаторОрганизацииСуществующий)))
	...
	Пометка = Форма.ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() И ЭлементСписка <> Неопределено И ЭлементСписка.Пометка;
...
Процедура УстановитьЗначенияПоУмолчанию()
	...
	Если Операция = Неопределено И ЗначениеЗаполнено(Организация) Тогда
		...
		ПараметрыДанных = УчетныеЗаписиЭДО.НовыеПараметрыДанныхПоИдентификаторамДляРегистрацииВЭДО();
		ПараметрыДанных.Организация = Организация;
		ПараметрыДанных.ЗаполнятьОператоровЭДО = Истина;
		ПараметрыДанных.ЗаполнятьИдентификаторыОрганизации = Истина;
		ПараметрыДанных.ЭтоОблачныйЭДО = ЭтоОблачныйЭДО;
		...
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
			"УчетныеЗаписиЭДО.ДанныеПоИдентификаторамДляРегистрацииВЭДО", ПараметрыДанных);
	Иначе
...
Процедура ЗаполнитьИдентификаторыДляРегистрацииПослеПолученияДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		КонтекстДиагностики = ЗаполнитьИдентификаторыДляРегистрацииПоАдресуРезультата(Результат.АдресРезультата);
		
		Если Не ЗначениеЗаполнено(Элементы.ИдентификаторОрганизацииЧерез1СЭДО.СписокВыбора) Тогда
			ОбработатьОшибкиВКонтекстномРежиме(Элементы.ПредупреждениеНадпись, Элементы.ГруппаПредупреждения);
		КонецЕсли;
...
&НаСервере
Функция ЗаполнитьИдентификаторыДляРегистрацииПоАдресуРезультата(Знач АдресРезультата)
	
	ДанныеПоИдентификаторам = ПолучитьИзВременногоХранилища(АдресРезультата); // См. УчетныеЗаписиЭДО.ДанныеПоИдентификаторамДляРегистрацииВЭДО
	УдалитьИзВременногоХранилища(АдресРезультата);
	
	КонтекстДиагностики = ДанныеПоИдентификаторам.КонтекстДиагностики;
	
	ЗаполнитьПодключенныхОператоровЭДО(ДанныеПоИдентификаторам.ОператорыЭДО);
	
	ПриИзмененииОператораЭДО();
	
	ЗаполнитьСписокИдентификаторов(ДанныеПоИдентификаторам.ИдентификаторыОрганизации);
...
Процедура ПолучитьСпособОбменаПоОператоруЭДО()
	...
	Если ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверкаЗаполнения()
	...
	Если Не ПодключениеТакскомВМоделиСервиса
		И Не ЗначениеЗаполнено(ЭлектроннаяПочта)
			И УведомлятьОСобытиях 
				И ТипРегистрации = ТипРегистрацииНовыйИдентификатор() Тогда
	...
	Если ТипРегистрации = ТипРегистрацииСуществующийИдентификатор()
...
Процедура ТестСвязиСервисаЭДО()
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО")
		И ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() Тогда
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") 
		...
		ИначеЕсли Не ЭтоОблачныйЭДО Тогда
			ВыполнитьОтправкуДанныхВСервисНастроек();
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатЧерезОблачныйЭДО()
	...
	ПараметрыРегистрации.Организация = Организация;
	ПараметрыРегистрации.ОператорЭДО = ИдентификаторОператораЭДО;
	ПараметрыРегистрации.АдресОрганизации = ЗначениеПолейАдреса;
	ПараметрыРегистрации.КодНалоговогоОргана = КодНалоговогоОргана;
	ПараметрыРегистрации.ЭлектроннаяПочта = ЭлектроннаяПочта;
	ПараметрыРегистрации.СертификатДляПодписания = СертификатКриптографии;
	ПараметрыРегистрации.СертификатыДляРегистрации.Добавить(СертификатКриптографии);
	Если ЗначениеЗаполнено(Доверенность) Тогда
		ПараметрыРегистрации.Доверенности.Вставить(СертификатКриптографии, Доверенность);
	КонецЕсли;
	Если ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() Тогда
	...
	ОбщийМодульИнтеграцияОблачногоЭДОКлиент.НачатьОтправкуСертификатовДляРегистрацииВЭДО(Оповещение, ПараметрыРегистрации,
		ЭтотОбъект);
...
Процедура ЗарегистрироватьСертификатЧерезОблачныйЭДОЗавершение(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.СертификатыУжеЗарегистрированы Тогда
		...
		УникальныйИдентификаторЗаявки1СЭДО = Результат.ИдентификаторЗаявки;
		ПаролиСертификатов                 = Результат.ПаролиСертификатов;
		РезультатРегистрации = ИнтеграцияОблачногоЭДОКлиент.НовыйРезультатРегистрацииСертификатовВЭДО();
		РезультатРегистрации.ИдентификаторыЭДО.Вставить(Результат.ИдентификаторЗаявки, ИдентификаторОрганизацииСуществующий);
...
Процедура Подключаемый_ПроверитьРезультатРегистрацииСертификатЧерезОблачныйЭДО()
	...
	ИдентификаторыЗаявокПоОрганизациям.Вставить(Организация, ИдентификаторыЗаявок);
...
&НаКлиенте
Процедура ОбновитьПараметрыУведомлений()
	
	ПараметрыУведомлений = ПараметрыУведомлений(ЭтотОбъект);
	...
	Если ЭтоОблачныйЭДО Тогда
		...
		ПараметрыОбновления.ВыбранныйСертификат = СертификатКриптографии;
		ПараметрыОбновления.ИдентификаторУчетнойЗаписиЭДО = ИдентификаторОрганизацииСуществующий;
...
Процедура ВыполнитьОтправкуДанныхВСервисНастроек()
	...
	Если ЭтоОблачныйЭДО Тогда
		Оповещение = Новый ОписаниеОповещения("ОтправкаДанныхВСервисНастроекЧерезОблачныйЭДОЗавершение", ЭтотОбъект, ИдентификаторОрганизации);
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияОблачногоЭДОКлиент");
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент.ОбновитьИнформациюОбУчетнойЗаписиЭДО(
			ИдентификаторОрганизации, Оповещение, ЭтотОбъект);
	Иначе
...
Процедура ПолучитьПараметрыУведомлений()
	...
	Если ЭтоОблачныйЭДО Тогда
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияОблачногоЭДОКлиент");
		ПараметрыПолучения = ОбщийМодульИнтеграцияОблачногоЭДОКлиент.НовыеПараметрыПолученияНастройкиУведомленийУчетнойЗаписиЭДО();
		ПараметрыПолучения.ОповещениеОЗавершении = Оповещение;
		ПараметрыПолучения.Организация = Организация;
		ПараметрыПолучения.ВыбранныйСертификат = СертификатКриптографии;
		ПараметрыПолучения.ИдентификаторУчетнойЗаписиЭДО = ИдентификаторОрганизацииСуществующий;
		ПараметрыПолучения.ПаролиСертификатов = ПаролиСертификатов;
		ПараметрыПолучения.ФормаВладелец = ЭтотОбъект;
		ОбщийМодульИнтеграцияОблачногоЭДОКлиент.ПолучитьНастройкиУведомленийУчетнойЗаписиЭДО(ПараметрыПолучения);
	Иначе
		КлючСинхронизации = СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации();
		КлючСинхронизации.ИдентификаторУчетнойЗаписи = ИдентификаторОрганизацииСуществующий;
		КлючСинхронизации.СпособОбмена = СпособОбменаЭД;
...
Функция СохранитьУчетнуюЗапись()
	...
	ОписаниеУчетнойЗаписи.ПараметрыУведомлений = ПараметрыУведомлений(ЭтотОбъект);
...
#Область ТипыРегистрацийУчетнойЗаписиЭДО

&НаКлиентеНаСервереБезКонтекста
Функция ТипРегистрацииНовыйИдентификатор()
	Возврат "ТипРегистрацииНовыйИдентификатор";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТипРегистрацииСуществующийИдентификатор()
	Возврат "ТипРегистрацииСуществующийИдентификатор";
КонецФункции
...
&НаСервере
Процедура ЗаполнитьСписокВыбораДляТипаРегистрации()
	Элементы.ТипРегистрацииСуществующаяУчетнаяЗапись.СписокВыбора.Добавить(
		ТипРегистрацииСуществующийИдентификатор(), НСтр("ru = 'Выбрать существующий'"));
	Элементы.ТипРегистрацииНоваяУчетнаяЗапись.СписокВыбора.Добавить(
		ТипРегистрацииНовыйИдентификатор(), НСтр("ru = 'Получить новый'"));
КонецПроцедуры
...
Процедура ПроверитьНеобходимостьДоверенности()
	...
	ТребуетсяДоверенность = УчетныеЗаписиЭДОСлужебный.ТребуетсяДоверенность(Организация, СертификатКриптографии,
		СпособОбменаЭД, ОператорЭДО);
	
	Если ТребуетсяДоверенность И Не ПустаяСтрока(ОператорЭДО) Тогда
		Отбор = Новый Структура("Идентификатор", ОператорЭДО);
...
&НаСервере
Процедура ПриИзмененииТипаРегистрацииНовойУчетнойЗаписи()
...
&НаСервере
Процедура ПриИзмененииТипаРегистрацииСуществующейУчетнойЗаписи()
...
Процедура ЗаполнитьСписокИдентификаторов(СписокИдентификаторов)
	...
	ТипРегистрации = ?(ЗначениеЗаполнено(ИдентификаторОрганизацииСуществующий),
		ТипРегистрацииСуществующийИдентификатор(), ТипРегистрацииНовыйИдентификатор());
	
КонецПроцедуры

&НаСервере
Процедура ПриИзмененииИдентификатораОрганизацииЧерез1СЭДО()
