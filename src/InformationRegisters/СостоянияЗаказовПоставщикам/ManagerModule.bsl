	...
	Для Каждого ТипДокумента Из СоотвествиеТипов Цикл
		...
		Если ТипДокумента.Ключ = "Документ.ЗаказПоставщику" Тогда
			...
			Если Не НоваяАрхитектураВзаиморасчетов Тогда
				...
				Иначе
					ТекстЗапросаСостояниеВзаиморасчетов = 
						СтрЗаменить(ТекстЗапросаСостояниеВзаиморасчетов, "ПОМЕСТИТЬ ВтСостояниеВзаиморасчетов", "");
				КонецЕсли;
			КонецЕсли;
...
Функция ТекстЗапросаСостояниеВзаиморасчетов()
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ШаблонПоляОплачено", СтрШаблон("%1 + %2", ВзаиморасчетыСервер.ШаблонПоляОплаченоПоставщику(), ВзаиморасчетыСервер.ШаблонПоляЗачтеноПоставщиком())); 
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ШаблонПоляПоставлено", ВзаиморасчетыСервер.ШаблонПоляПоставленоПоставщиком());
