...
Процедура ПроверитьОбновитьКлассификаторы(Отказ)
	...
	Если РаботаВМоделиСервисаПовтИсп.РазделениеВключено()
		Или КлассификаторыБылиЗагруженыРанее() Тогда
		Возврат;
	КонецЕсли;
...
Функция КлассификаторыБылиЗагруженыРанее()
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДопустимыеЦелиПоГруппамВЕТИС.ГруппаПриказа
		|ИЗ
		|	РегистрСведений.ДопустимыеЦелиПоГруппамВЕТИС КАК ДопустимыеЦелиПоГруппамВЕТИС";
