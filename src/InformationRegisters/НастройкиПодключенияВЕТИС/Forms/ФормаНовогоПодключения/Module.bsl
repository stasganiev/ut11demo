...
Функция НачатьПолучениеСведенийНаСервере(НастройкаПодключения)
	...
	ИнтеграцияИСПереопределяемый.НастроитьДлительнуюОперацию(СтруктураПараметров, ПараметрыВыполнения);
...
Процедура НачатьПолучениеВетеринарноСопроводительныхДокументов()
	...
	ПараметрыОтбора.Вставить("ТипВСД",    ПредопределенноеЗначение("Перечисление.ТипыВетеринарныхДокументовВЕТИС.Входящий"));
	
	Попытка
		Если ЗначениеЗаполнено(ДатаНачалаЗагрузкиВСД) Тогда
			ПараметрыОтбора.Вставить(
				"Интервал",
				ИнтеграцияВЕТИСКлиентСервер.СтруктураИнтервала(ДатаНачалаЗагрузкиВСД));
		КонецЕсли;
