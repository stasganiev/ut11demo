...
Процедура ЗаполнитьЗначенияРегистраЕМРЦ() Экспорт
	...
	Попытка
		...
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ОсобенностьУчета = Перечисления.ВидыМаркированнойПродукцииБПО.Табак; 
		НоваяСтрока.Период           = Дата('20240301');
		НоваяСтрока.Цена             = 0.02*3820*(1+0.2)*1.4;
		
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ОсобенностьУчета = Перечисления.ВидыМаркированнойПродукцииБПО.Табак; 
		НоваяСтрока.Период           = Дата('20250101');
		НоваяСтрока.Цена             = 0.02*3973*(1+0.2)*1.4;
		
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ОсобенностьУчета = Перечисления.ВидыМаркированнойПродукцииБПО.Табак; 
		НоваяСтрока.Период           = Дата('20260101');
		НоваяСтрока.Цена             = 0.02*4132*(1+0.2)*1.4;
