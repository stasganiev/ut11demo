...
Функция ПоместитьФайлДляСопоставления_POST(Запрос)
	...
	ИмяПланаОбмена = Запрос.ПараметрыЗапроса.Получить("ExchangePlanName");
	КодУзла = Запрос.ПараметрыЗапроса.Получить("NodeCode");
	ИдентифкаторФайла = Запрос.ПараметрыЗапроса.Получить("FileID");
	
	Попытка
		
		ОбменДаннымиОперацииВебСервисов.ПоместитьСообщениеДляСопоставленияДанных(
			ИмяПланаОбмена, КодУзла, ИдентифкаторФайла);
...
Функция СобратьФайлИзЧастей_POST(Запрос)
	...
	ИдентифкаторФайла = Неопределено;
	
	Попытка
		ОбменДаннымиОперацииВебСервисов.СобратьФайлИзЧастей(ИдентификаторСессии, КоличествоЧастей, ИдентифкаторФайла);
	Исключение
		Возврат ОтветОшибка(ИнформацияОбОшибке());
	КонецПопытки;
	
	Тело = Новый Структура("FileID", Строка(ИдентифкаторФайла));
...
Функция ВыполнитьЗагрузкуДанных_POST(Запрос)
	...
	Попытка 
		ОбменДаннымиОперацииВебСервисов.ВыполнитьЗагрузкуДанных(ИмяПланаОбмена, КодУзла, ИдентифкаторФайла, 
