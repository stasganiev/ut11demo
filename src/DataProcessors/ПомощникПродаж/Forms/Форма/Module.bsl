...
Процедура ЗаписатьТранспортнуюНакладную(МассивНайденныхДокументов, Основания, Отказ)
	...
	Если РезультатПроверки.ОбъектыПоКоторымМожноИНужноСоздатьТранспортныеНакладные.Количество() > 0 Тогда
		...
		Попытка
			...
			СозданныеТранспортныеНакладные = Документы.ТранспортнаяНакладная.СоздатьТранспортныеНакладные(МассивРаспоряжений);
...
Процедура СоздатьРеализациюПоЗаказу(Товары, МассивРеализаций, МассивНайденныхДокументов, ОснованиеОбъект, БезСменыСтатусаЗаказа, БезПроведения, Отказ)
	...
	Для Каждого ТекСклад Из МассивСкладов Цикл
		...
		Если ЗначениеЗаполнено(ТекСклад) Или МассивСкладов.Количество() = 1 Тогда
			...
			Для Каждого ВидГОСИС Из МассивГосИС Цикл
				...
				ПродажаОбъект = ЗаписатьРеализациюПоЗаказу(МассивНайденныхДокументов, ОснованиеОбъект, ТоварыПоСкладуВидуГОСИС, ТекСклад, Отказ, БезПроведения, БезСменыСтатусаЗаказа);
				Если Отказ Тогда
					Прервать;
				КонецЕсли; 
	...
	// Создание транспортной накладной по реализации
	Если Не БезПроведения И СоздаватьТранспортнуюНакладную И Не Отказ Тогда
		ЗаписатьТранспортнуюНакладную(МассивНайденныхДокументов, МассивРеализаций, Отказ);
	КонецЕсли;
	...
	Для Каждого ТекСклад Из МассивСкладов Цикл
		...
		Если ЗначениеЗаполнено(ТекСклад)
			...
			Если Не Отказ
				...
				ЗаписатьТранспортнуюНакладную(МассивНайденныхДокументов, МассивПередач, Отказ);
