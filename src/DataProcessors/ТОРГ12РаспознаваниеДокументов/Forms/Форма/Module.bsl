...
Процедура ТаблицаДокументаПриИзменении(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "Номенклатура" И Не Элемент.ТекущиеДанные = Неопределено Тогда  
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.ТСклад) Тогда
			ТекущаяСтрока.ТСклад = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(Объект, "Склад",
...
Функция СоздатьДокумент(ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Если УдалосьПровести Тогда
		Если ТекущиеНастройки.АвтоматическиУстанавливатьКонечноеСостояниеУПервичныхДокументов Тогда  
			...
			РегистрыСведений.СостоянияОригиналовПервичныхДокументов.ЗаписатьОбщееСостояниеОригиналаДокумента(СоздаваемыйДокумент, Состояние.Наименование);
		КонецЕсли;
	КонецЕсли;
	...
	Если СоздатьСчетФактуру Тогда
		...
		Если УдалосьПровести Тогда
			Если ТекущиеНастройки.АвтоматическиУстанавливатьКонечноеСостояниеУПервичныхДокументов Тогда
				Состояние = Справочники.СостоянияОригиналовПервичныхДокументов.ОригиналПолучен;
				РегистрыСведений.СостоянияОригиналовПервичныхДокументов.ЗаписатьОбщееСостояниеОригиналаДокумента(СчетФактураСсылка, Состояние.Наименование);
			КонецЕсли; 
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	НоменклатураСервер.УстановитьУсловноеОформлениеНомераГТД(ЭтаФорма,
												"НомерГТД",
												"ТаблицаДокумента.ВедетсяУчетПоГТД");      
	НоменклатураСервер.УстановитьУсловноеОформлениеНомераГТД(ЭтаФорма, "СтранаПроисхождения", 
												"ТаблицаДокумента.ВедетсяУчетПоГТД");
...
Процедура УстановитьПараметрыВыбораНоменклатуры()  
	...
	Элементы.Номенклатура.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
