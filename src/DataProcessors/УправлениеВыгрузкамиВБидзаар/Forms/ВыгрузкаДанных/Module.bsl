...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьИнтеграциюСЭТПБидзаар") Тогда
		
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Интеграция с ЭТП Бидзаар не активирована, 
	...
	Если Не ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		УчетнаяЗапись = Справочники.УчетныеЗаписиЭлектронныхТорговыхПлощадок.ОсновнаяУчетнаяЗапись(
			Перечисления.ВидыЭлектронныхТорговыхПлощадок.Бидзаар);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметры, Источник)
	...
	Если ИмяСобытия = "ОбновитьУчетнуюЗапись" Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры);
		УчетнаяЗаписьПриИзмененииНаСервере();
	ИначеЕсли ИмяСобытия = "УчетнаяЗаписьДеактивирована" Тогда
		Если УчетнаяЗапись = Параметры Тогда
...
Процедура УчетнаяЗаписьСоздание(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.УправлениеВыгрузкамиВБидзаар.Форма.НастройкаПодключения", , ЭтаФорма,
...
Процедура ДействияПоОтбору(Команда)
	...
	ОткрытьФорму(ФормаИмя, ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор, , , ОписаниеОповещения);
...
Процедура ОткрытьСписокПодключений(Команда)
	...
	ОткрытьФорму("Обработка.УправлениеВыгрузкамиВБидзаар.Форма.СписокПодключенийКСервису", ПараметрыФормы, ЭтаФорма,
			УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВосстановитьНастройки(СписокСвойств = Неопределено)
	...
	Если ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		Если Не Справочники.УчетныеЗаписиЭлектронныхТорговыхПлощадок.УчетнаяЗаписьАктуальна(УчетнаяЗапись,
			Перечисления.ВидыЭлектронныхТорговыхПлощадок.Бидзаар) Тогда
