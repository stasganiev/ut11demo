...
//   * ЧастотаОбновления              - Число - Частота обновления;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьФайлыНовостейССервера"; // Идентификатор.
	...
	ВсеШаблоны = ВсеШаблоныСообщенийЗагрузкиФайловНовостей();

	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПолучениеНастроекНовостей"); // Идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ФормированиеСпискаЛентНовостей"); // Идентификатор.
	ТаблицаHTTPСоединений = ЛентыНовостей.Скопировать(, "Сайт, Протокол, ВариантЛогинаПароля, Логин, Пароль");
	ТаблицаHTTPСоединений.Свернуть("Сайт, Протокол, ВариантЛогинаПароля, Логин, Пароль");

	// 1. Правильно ли заполнено переопределение?
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПравильностьВнедренияПодсистемы/ТолькоПереопределения"); // Идентификатор.
	ЕстьОшибкиВнедрения = Ложь;
	Результат = ОбработкаНовостейСлужебный.ПроверитьПравильностьВнедренияПодсистемы("ТолькоПереопределения");
	Если НЕ ПустаяСтрока(Результат) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены ошибки внедрения подсистемы ИнтернетПоддержкаПользователей.Новости.
				...
				|%1'"),
			Результат);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SUBSYSTEM_PLUGGED_INCORRECT/GET_NEWS",
			ТекстСообщения);
	КонецЕсли;
	...
	Если ЕстьОшибкиВнедрения = Истина Тогда
		НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ЛентыНовостейПриНеправильномВнедренииПодсистемы"); // Идентификатор.
		...
		Для Каждого ТекущаяЛентаНовостей Из ЛентыНовостей Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если ЛогШаговВыполнения.Количество() = 0 Тогда
			ЗавершитьШагВыполнения(КонтекстВыполнения, "",
				НСтр("ru = 'Подсистема Новости внедрена неправильно, но нет лент новостей с параметром [config]'"));
		Иначе
			ЛогШаговВыполнения.Вставить(0,
				НСтр("ru = 'Подсистема Новости внедрена неправильно, и найдены ленты новостей с параметром [config], которые будут исключены:'"));
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SUBSYSTEM_PLUGGED_INCORRECT/GET_NEWS",
				ЛогШаговВыполнения);
		КонецЕсли;
	КонецЕсли;

	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПредопределенныеКатегории"); // Идентификатор.
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		ВерсияБиблиотеки = ОбработкаНовостейКлиентСервер.ВнутреннееПредставлениеНомераВерсии(
		...
		Отборы = Новый Структура;
			Отборы.Вставить("ЭтотПродукт"                             , ЭтотПродукт);
			Отборы.Вставить("ВерсияПлатформы"                         , ВерсияПлатформы);
			Отборы.Вставить("ВерсияБиблиотеки"                        , ВерсияБиблиотеки);
			Отборы.Вставить("МетаданныеВерсия"                        , МетаданныеВерсия);
			Отборы.Вставить("МетаданныеИмя"                           , МетаданныеИмя);
			Отборы.Вставить("ЭтоРазделеннаяКонфигурация"              , ЭтоРазделеннаяКонфигурация);
			Отборы.Вставить("ЭтоРазделеннаяКонфигурация_Представление", ЭтоРазделеннаяКонфигурация_Представление);
			Отборы.Вставить("РазделениеВключено"                      , РазделениеВключено);
			Отборы.Вставить("РазделениеВключено_Представление"        , РазделениеВключено_Представление);
			Отборы.Вставить("РежимРаботыПрограммы_Представление"      , РежимРаботыПрограммы_Представление);
...
#Область ПолучениеФайлов
	...
	Шаг = НаименованиеМетода + "/Основные:ЦиклПоВидамСоединений";
	КонтекстВыполненияСоединения = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
	ЛогШаговВыполнения = Новый Массив;

	Для каждого ТекущееHTTPСоединение Из ТаблицаHTTPСоединений Цикл

		НачатьШагВыполнения(КонтекстВыполненияСоединения, "Цикл:"
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Обработка: %1, %2'"),
					ТекущееHTTPСоединение.Протокол,
					ТекущееHTTPСоединение.Сайт));

		Шаг = НаименованиеМетода + "/Основные:ЦиклПоЛентамНовостей";
		КонтекстВыполненияЛентыНовостей = НачатьШагВыполненияВложенный(КонтекстВыполненияСоединения, Шаг);

		Если ВРег(ТекущееHTTPСоединение.Протокол) = ВРег("file") Тогда
#Область Получение_Файл

			ОписательУсловияПоиска = Новый Структура;
				ОписательУсловияПоиска.Вставить("Сайт",                ТекущееHTTPСоединение.Сайт); // Должно быть пустое
				ОписательУсловияПоиска.Вставить("Протокол",            ТекущееHTTPСоединение.Протокол);
				ОписательУсловияПоиска.Вставить("ВариантЛогинаПароля", ТекущееHTTPСоединение.ВариантЛогинаПароля); // Должно быть = БезЛогинаПароля
				ОписательУсловияПоиска.Вставить("Логин",               ТекущееHTTPСоединение.Логин); // Должно быть пустое
				ОписательУсловияПоиска.Вставить("Пароль",              ТекущееHTTPСоединение.Пароль); // Должно быть пустое
...
#Область ЗагрузкаФайлов_Файл_ЛентаНовостей
				НачатьШагВыполнения(КонтекстВыполненияЛентыНовостей, "Цикл:Файл:" + ТекущаяЛентаНовостей.ЛентаНовостей); // Идентификатор.
				ЛогШаговВыполненияЛентыНовостей = Новый Массив;

				ЕстьОшибки = Ложь;

				// Новости могут загружаться как по расписанию (ЧастотаОбновления > 0),
				//  так и при обновлении версии конфигурации (загрузка из макетов).
				// И тогда в обработку могут попасть локальные ленты новостей или ленты новостей с отключенным расписанием.
				// В этом случае ПараметрыОперации для них будет пустой.
				Операция          = "БИП:Новости/ПолучениеНовостей/" + ТекущаяЛентаНовостей.КодЛентыНовостей;
				ПараметрыОперации = ОбработкаНовостейСлужебный.ПараметрыВыполненияОперацииРегламентныхЗаданий(Операция);
					...
					Если (СтрНайти(ВРег(СокрЛП(ТекущаяЛентаНовостей.ИмяФайла)), ВРег("Метаданные.")) = 1)
						...
						Если МассивСлов.Количество() = 5 Тогда
							...
							Исключение
								...
								ЛогШаговВыполненияЛентыНовостей.Добавить(ТекстСообщения);
				...
				// И тогда в обработку могут попасть локальные ленты новостей или ленты новостей с отключенным расписанием.
				// В этом случае ПараметрыОперации для них будет пустой.
				Если ТипЗнч(ПараметрыОперации) = ТипСтруктура Тогда

					// Если файл загрузить так и не удалось, то следующие попытки делать через случайное время.
					Если ЕстьОшибки = Ложь Тогда
						РасписанияРегламентныхЗаданий.ОперацияВыполнена(Операция, ПараметрыОперации);
					Иначе
						БазовыйИнтервалСекунд = ПреобразоватьЧастотуОбновленияВСекунды(ТекущаяЛентаНовостей.ЧастотаОбновления);
						// Если частота обновления меньше 1 часа, то сделать 30 минут
						Если БазовыйИнтервалСекунд < 60*60 Тогда
							БазовыйИнтервалСекунд = 30*60;
						КонецЕсли;
						РасписанияРегламентныхЗаданий.ОтложитьОперациюСлучайно(Операция, БазовыйИнтервалСекунд, -10*60, 20*60);
					КонецЕсли;
				...
				ЛогШаговВыполненияЛентыНовостей.Добавить(ТекстСообщения);
				ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполненияЛентыНовостей); // Идентификатор.
#КонецОбласти
			КонецЦикла;

#КонецОбласти
		ИначеЕсли (ВРег(ТекущееHTTPСоединение.Протокол) = ВРег("http"))
				ИЛИ (ВРег(ТекущееHTTPСоединение.Протокол) = ВРег("https")) Тогда // http, https
#Область Получение_HTTP
			...
			HTTPСоединение = Неопределено;
			Если НЕ ОбработкаНовостейСлужебный.СоединитьсяССерверомНовостей(
						ТекущееHTTPСоединение.Сайт,
						?(ВРег(ТекущееHTTPСоединение.Протокол) = ВРег("https") , "https", "http"),
						ТекущееHTTPСоединение.Логин,
						ТекущееHTTPСоединение.Пароль,
						HTTPСоединение,
						КонтекстВыполненияЛентыНовостей) Тогда
				// Если была ошибка, то подробности запишутся в журнал регистрации в ОбработкаНовостейСлужебныйСоединитьсяССерверомНовостей.
				ЕстьОшибки = Истина;
			КонецЕсли;
			...
			Если ЕстьОшибки = Истина Тогда
				Продолжить; // Следующее ТекущееHTTPСоединение, т.к. все ленты новостей для этого соединения недоступны.
			КонецЕсли;

			ОписательУсловияПоиска = Новый Структура;
				ОписательУсловияПоиска.Вставить("Сайт",                ТекущееHTTPСоединение.Сайт);
				ОписательУсловияПоиска.Вставить("Протокол",            ТекущееHTTPСоединение.Протокол);
				ОписательУсловияПоиска.Вставить("ВариантЛогинаПароля", ТекущееHTTPСоединение.ВариантЛогинаПароля);
				ОписательУсловияПоиска.Вставить("Логин",               ТекущееHTTPСоединение.Логин);
				ОписательУсловияПоиска.Вставить("Пароль",              ТекущееHTTPСоединение.Пароль);
				ОписательУсловияПоиска.Вставить("Пометка",             Истина);
			МассивОбрабатываемыхЛентНовостей = ЛентыНовостей.НайтиСтроки(ОписательУсловияПоиска);

			Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
#Область Получение_HTTP_ЛентаНовостей
				НачатьШагВыполнения(КонтекстВыполненияЛентыНовостей, "Цикл:HTTP:" + ТекущаяЛентаНовостей.ЛентаНовостей); // Идентификатор.
				ЛогШаговВыполненияЛентыНовостей = Новый Массив;
				лкИмяФайлаНаСервере = "";
				ЕстьОшибки = Ложь;
				...
				// Чтобы уменьшить количество переменных, все поместим в КонтекстВыполненияЛентыНовостей.ВходящиеПараметры.
				В = КонтекстВыполненияЛентыНовостей.ВходящиеПараметры;
					В.Вставить("ЛентаНовостей"   , ТекущаяЛентаНовостей.ЛентаНовостей);
					В.Вставить("КодЛентыНовостей", ТекущаяЛентаНовостей.КодЛентыНовостей);
					В.Вставить("МаксимумПопытокЗагрузкиФайлаНовостей",
						НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла);
					В.Вставить("НомерПопыткиЗагрузкиФайлаНовостей"   , 0);
					В.Вставить("ЭтоПереадресация", Ложь);
					В.Вставить("URLПереадресации", ""); // Только для логирования
					В.Вставить("ВременныйФайл"   , Новый Структура); // Будет подставляться для каждой попытки
						В.ВременныйФайл.Вставить("Имя"         , ""); // Будет подставляться для каждой попытки
						В.ВременныйФайл.Вставить("Размер"      , 0); // Будет подставляться для каждой попытки
						В.ВременныйФайл.Вставить("ПервыеСтроки", ""); // Будет подставляться для каждой попытки
					В.Вставить("Запрос", Новый Структура);
						В.Запрос.Вставить("Сайт"       , ТекущаяЛентаНовостей.Сайт);
						В.Запрос.Вставить("Протокол"   , ?(ВРег(ТекущаяЛентаНовостей.Протокол) = ВРег("https") , "https", "http"));
						В.Запрос.Вставить("ИмяФайла"   , ТекущаяЛентаНовостей.ИмяФайла);
						В.Запрос.Вставить("URL"        , ""); // Будет подставляться для каждой попытки
						В.Запрос.Вставить("Заголовки"  , ""); // Будет подставляться для каждой попытки
						В.Запрос.Вставить("ETag"       , ""); // Будет подставляться для каждой попытки
					В.Вставить("Ответ", Новый Структура);
						В.Ответ.Вставить("КодСостояния", ""); // Будет подставляться для каждой попытки
						В.Ответ.Вставить("Заголовки"   , ""); // Будет подставляться для каждой попытки
						В.Ответ.Вставить("Длительность", 0); // Будет подставляться для каждой попытки
					В.Вставить("ЗагрузкаУспешна"             , Ложь); // Будет подставляться для каждой попытки
					В.Вставить("ПодробноеПредставлениеОшибки", ""); // Будет подставляться для каждой попытки

				// Шаг 2 / 2, получение файла.
				ФайлПолучен = Ложь;
				В.НомерПопыткиЗагрузкиФайлаНовостей = 0;
				...
				Пока (ФайлПолучен <> Истина) И (В.НомерПопыткиЗагрузкиФайлаНовостей < В.МаксимумПопытокЗагрузкиФайлаНовостей) Цикл
					Попытка
#Область Получение_HTTP_ЛентаНовостей_СохранитьПараметрыТекущейПопытки
						Запись = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьМенеджерЗаписи();
							Запись.ЛентаНовостей = В.ЛентаНовостей;
...
#Область Получение_HTTP_ЛентаНовостей_ИнициализацияПопытки
						В.НомерПопыткиЗагрузкиФайлаНовостей = В.НомерПопыткиЗагрузкиФайлаНовостей + 1;
						В.ЗагрузкаУспешна                   = Ложь; // Будет подставляться для каждой попытки
						В.ПодробноеПредставлениеОшибки      = ""; // Будет подставляться для каждой попытки
						...
						лкИмяФайлаНаСервере = СокрЛП(В.Запрос.ИмяФайла);
						...
						лкИмяФайлаНаСервере = СтрЗаменить(лкИмяФайлаНаСервере, "[filter]", Справочники.ЛентыНовостей.СформироватьТекстУсловияДляСервераНовостей(В.ЛентаНовостей));

						В.ВременныйФайл.Имя          = ПолучитьИмяВременногоФайла("xml"); // Будет подставляться для каждой попытки
						В.ВременныйФайл.Размер       = 0; // Будет подставляться для каждой попытки
						В.ВременныйФайл.ПервыеСтроки = ""; // Будет подставляться для каждой попытки
						В.Запрос.URL                 = В.Запрос.Протокол + "://" + В.Запрос.Сайт + "/" + лкИмяФайлаНаСервере; // Будет подставляться для каждой попытки
#КонецОбласти

#Область Получение_HTTP_ЛентаНовостей_Загрузка

						лкЗагрузкаФайлов_ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();

						ЛогШаговВыполненияЛентыНовостей.Добавить(
							ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.НачалоПопытки, В));

						лкЗаголовки = Новый Соответствие;
						лкЗаголовки.Вставить("X-Correlation-ID", СокрЛП(Новый УникальныйИдентификатор));
						...
						В.Запрос.ETag = "";
						Если НЕ ПустаяСтрока(ТекущаяЛентаНовостей.ПоследняяНовостьУИН)
								И (ТекущаяЛентаНовостей.ПоследняяНовостьДатаПубликации > '00010101') Тогда
							В.Запрос.ETag = СокрЛП(ВРег(ТекущаяЛентаНовостей.ПоследняяНовостьУИН))
								+ "/" + Формат(ТекущаяЛентаНовостей.ПоследняяНовостьДатаПубликации, "ДФ=yyyy-MM-ddTHH:mm:ssZ");
						КонецЕсли;
						лкЗаголовки.Вставить(
							"If-None-Match",
							В.Запрос.ETag);
						Заголовки = Новый Массив;
						Для каждого ТекущийЗаголовок Из лкЗаголовки Цикл
							Заголовки.Добавить(ТекущийЗаголовок.Ключ + ":" + ТекущийЗаголовок.Значение);
						КонецЦикла;
						В.Запрос.Заголовки = СтрСоединить(Заголовки, Символы.ПС); // Будет подставляться для каждой попытки
						HTTPЗапрос = Новый HTTPЗапрос(лкИмяФайлаНаСервере, лкЗаголовки);
						HTTPОтвет = HTTPСоединение.Получить(HTTPЗапрос, В.ВременныйФайл.Имя);
						...
						Для каждого ТекущийЗаголовок Из HTTPОтвет.Заголовки Цикл
							Заголовки.Добавить(ТекущийЗаголовок.Ключ + ":" + ТекущийЗаголовок.Значение);
						КонецЦикла;
						В.Ответ.КодСостояния = HTTPОтвет.КодСостояния; // Будет подставляться для каждой попытки
						В.Ответ.Заголовки    = СтрСоединить(Заголовки, Символы.ПС); // Будет подставляться для каждой попытки
						В.Ответ.Длительность = Формат((лкЗагрузкаФайлов_ВремяКонца - лкЗагрузкаФайлов_ВремяНачала) / 1000,"ЧДЦ=3; ЧРД=,; ЧРГ=' '; ЧГ=3,0"); // Будет подставляться для каждой попытки

						ЛогШаговВыполненияЛентыНовостей.Добавить(
							ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ПолученОтвет, В));

						Если (HTTPОтвет.КодСостояния = 304) Тогда
							// Код состояния = 304, если новости не изменились и их нет смысла загружать - пропустить ленту новостей.
							ЛогШаговВыполненияЛентыНовостей.Добавить(
								ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ПолученОтвет304, В));
							ФайлПолучен = Истина; // Чтобы выйти из цикла попыток
							Продолжить;
						КонецЕсли;

						ВременныйФайл = Новый Файл(В.ВременныйФайл.Имя);
						Если ВременныйФайл.Существует() Тогда
							В.ВременныйФайл.Размер       = Формат(ВременныйФайл.Размер(), "ЧЦ=15; ЧДЦ=; ЧРГ=' '; ЧГ=3,0");
							В.ВременныйФайл.ПервыеСтроки = ОбработкаНовостейСлужебный.СодержимоеТекстовогоФайла(ВременныйФайл.ПолноеИмя, 10);
							ЛогШаговВыполненияЛентыНовостей.Добавить(
								ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ПолученОтветФайл, В));

							ТекущаяЛентаНовостей.АдресВХранилище = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(В.ВременныйФайл.Имя));
							ФайлПолучен = Истина;

							Если (HTTPОтвет.КодСостояния >= 500) И (HTTPОтвет.КодСостояния <= 599) Тогда
								Попытка
									ЧтениеТекста = Новый ЧтениеТекста(В.ВременныйФайл.Имя, "UTF-8", Символы.ПС, Символы.ПС, Ложь);
							...
							Попытка
								УдалитьФайлы(В.ВременныйФайл.Имя);
							Исключение
								В.ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
								ЛогШаговВыполненияЛентыНовостей.Добавить(
									ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.НеУдалосьУдалитьФайл, В));
							КонецПопытки;

						Иначе // Временный файл не существует
							ЛогШаговВыполненияЛентыНовостей.Добавить(
								ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ПолученОтветНоФайлаНет, В));
						КонецЕсли;
...
#КонецОбласти
						...
						Если (HTTPОтвет.КодСостояния >= 300) И (HTTPОтвет.КодСостояния <= 399) Тогда
							...
							Если (HTTPОтвет.КодСостояния = 301) // 301 Moved Permanently
									ИЛИ (HTTPОтвет.КодСостояния = 302) // 302 Found
									ИЛИ (HTTPОтвет.КодСостояния = 307) // 307 Temporary Redirect
									ИЛИ (HTTPОтвет.КодСостояния = 308) Тогда // 308 Permanent Redirect
								// Новый адрес должен находиться в заголовке "Location".
								// Если этот заголовок есть и корректно заполнен, то включить переадресацию.
								В.URLПереадресации = HTTPОтвет.Заголовки.Получить("Location");
								Если (ТипЗнч(В.URLПереадресации) = ТипСтрока)
										И (НЕ ПустаяСтрока(В.URLПереадресации)) Тогда
									ЛогШаговВыполненияЛентыНовостей.Добавить(
										ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.Переадресация, В));
									СтруктураСсылки   = ОбщегоНазначенияКлиентСервер.СтруктураURI(В.URLПереадресации);
									В.Запрос.Сайт     = СтруктураСсылки.ИмяСервера;
									В.Запрос.Протокол = ?(ВРег(СтруктураСсылки.Схема) = ВРег("https") , "https", "http");
									В.Запрос.ИмяФайла = СтруктураСсылки.ПутьНаСервере;
								КонецЕсли;
								// Это первая переадресация, то принудительно увеличить количество попыток до 5 (максимум переадресаций)
								//  и продолжить попытки загрузки.
								Если (В.ЭтоПереадресация = Ложь) Тогда
									В.ЭтоПереадресация = Истина;
									В.МаксимумПопытокЗагрузкиФайлаНовостей = 5; // Не более 5 переадресаций.
								КонецЕсли;
							КонецЕсли;
						ИначеЕсли (HTTPОтвет.КодСостояния >= 500) И (HTTPОтвет.КодСостояния <= 599) Тогда
							...
							ЕстьОшибки = Истина;
							// Выйти из цикла многократной попытки получения файла с сервера, т.к. следующие попытки ни к чему не приведут.
							Прервать;
						КонецЕсли;
					...
					Исключение
						В.ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
						ЛогШаговВыполненияЛентыНовостей.Добавить(
							ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ИсключениеПриПопыткеЗагрузкиФайлаНовостей, В));
						// Пропустить ленту новостей.
						ЕстьОшибки = Истина;
					КонецПопытки;

				КонецЦикла; // Очередная попытка загрузки файла
				...
				Если ТипЗнч(ПараметрыОперации) = ТипСтруктура Тогда
					...
					Если ЕстьОшибки = Ложь Тогда
						РасписанияРегламентныхЗаданий.ОперацияВыполнена(Операция, ПараметрыОперации);
					Иначе
						// В случае контролируемых ошибок на сервере (код состояния 5хх),
						//  сдвинуть дату следующего получения новостей до ДатаРегламентныхРаботДО + случайное время.
						// Если ошибка неконтролируемая (просто "упало"), то добавить случайное время.

						Если (ДатаРегламентныхРаботДО >= ТекущаяДатаСеанса) Тогда // Контролируемая ошибка
							РасписанияРегламентныхЗаданий.ОтложитьОперациюСлучайно(Операция,
								ДатаРегламентныхРаботДО - ТекущаяДатаСеанса, 0, 60*60);
						Иначе
							БазовыйИнтервалСекунд = ПреобразоватьЧастотуОбновленияВСекунды(ТекущаяЛентаНовостей.ЧастотаОбновления);
							// Если частота обновления меньше 1 часа, то сделать 30 минут
							Если БазовыйИнтервалСекунд < 60*60 Тогда
						...
						// Если файл не был загружен, то не обрабатывать его в РазборXMLФайловНовостей().
						ТекущаяЛентаНовостей.Пометка = Ложь;
					КонецЕсли;

				КонецЕсли;

				В.ЗагрузкаУспешна = (ЕстьОшибки = Ложь);
				ЛогШаговВыполненияЛентыНовостей.Добавить(
					ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.РезультатЗагрузкиФайлаНовостей, В));

				ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполненияЛентыНовостей); // Идентификатор.
#КонецОбласти
			КонецЦикла; // Цикл по лентам новостей

#КонецОбласти
		КонецЕсли;

		ЗавершитьШагВыполнения(КонтекстВыполненияСоединения, "", "", КонтекстВыполненияЛентыНовостей);

	КонецЦикла;

	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияСоединения);
...
#Область ВспомогательныеОбработки
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		НаименованиеМетода + "/Вспомогательные:НайтиИОбработатьНеактивныеЛентыНовостей");

		// После загрузки файлов дополнительно проанализировать регистр сведений ДатыЗагрузкиПоследнихНовостей.
		ОбработкаНовостейСлужебный.НайтиИОбработатьНеактивныеЛентыНовостей(КонтекстВыполненияВложенный);
...
//  запуск разбора для каждого файла в отдельности.
//
// Параметры:
//  ЛентыНовостей - ТаблицаЗначений - таблица лент новостей со следующими колонками:
//   * Пометка                - Булево - Если ИСТИНА, то данную ленту новостей надо загрузить;
...
//  КонтекстВыполнения - Структура, Неопределено - см. ОбработкаНовостейКлиентСервер.НовыйКонтекстВыполнения.
//
Процедура РазборXMLФайловНовостей(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт

#Область ИнициализацияМетода

	НаименованиеМетода = "Обработка.УправлениеНовостями.РазборXMLФайловНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	ВсеШаблоны = ВсеШаблоныСообщенийРазбораXMLФайловНовостей();

	// {Раздельное обновление ИБ}
	КонтекстВыполненияЭталон = Новый Структура;
		КонтекстВыполненияЭталон.Вставить("ВходящиеПараметры", Неопределено);
	ЗаполнитьЗначенияСвойств(КонтекстВыполненияЭталон, КонтекстВыполнения);

	ВходящиеПараметры = Новый Структура;
	Если ТипЗнч(КонтекстВыполненияЭталон.ВходящиеПараметры) = ТипСтруктура Тогда
		ВходящиеПараметры = КонтекстВыполненияЭталон.ВходящиеПараметры;
	КонецЕсли;

	ВходящиеПараметрыЭталон = Новый Структура;
		ВходящиеПараметрыЭталон.Вставить("ОбрабатыватьОбластиДанных", Ложь);
	ЗаполнитьЗначенияСвойств(ВходящиеПараметрыЭталон, ВходящиеПараметры);
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если (ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей())
			...
			Если (ВходящиеПараметрыЭталон.ОбрабатыватьОбластиДанных = Истина) Тогда
...
#Область Обработка
	...
	// Основные обработки разбора файлов.
#Область ОсновныеОбработки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "СохранениеДатыПоследнихЗагрузок"); // Идентификатор.
	// Определение текущего состояния загрузки новостей.
	ТаблицаДатЗагрузокНовостейПередПолучением = ОбработкаНовостейСлужебный.ПолучитьДатыЗагрузкиНовостей();
	...
	//  и потом повторно использовать при каждом разборе файла с новостями.
	Прокси = Неопределено;

	// Прочитать файлы
	НачатьШагВыполнения(КонтекстВыполнения, "ОбработкаЛентНовостей");

	КонтекстВыполненияЛентыНовостей = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Основные/ЦиклПоЛентамНовостей"); // Идентификатор.

	Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл

		// Чтобы уменьшить количество переменных, все поместим в КонтекстВыполненияЛентыНовостей.ВходящиеПараметры.
		В = КонтекстВыполненияЛентыНовостей.ВходящиеПараметры;
			В.Вставить("ЛентаНовостей"               , ТекущаяСтрока.ЛентаНовостей);
			В.Вставить("ОбязательныйКанал"           , ТекущаяСтрока.ОбязательныйКанал);
			В.Вставить("Пометка"                     , ТекущаяСтрока.Пометка);
			В.Вставить("ИмяФайла"                    , ТекущаяСтрока.ИмяФайла);
			В.Вставить("Сайт"                        , ТекущаяСтрока.Сайт);
			В.Вставить("Протокол"                    , ТекущаяСтрока.Протокол);
			В.Вставить("ИмяВременногоФайла"          , ""); // Будет подставляться для каждой попытки
			В.Вставить("ПодробноеПредставлениеОшибки", ""); // Будет подставляться для каждой попытки
			В.Вставить("ПервыеСтрокиФайла"           , ""); // Будет подставляться для каждой попытки
			В.Вставить("ВерсияRSS"                   , ""); // Будет подставляться для каждой попытки
			В.Вставить("ТипНовостей"                 , ""); // Будет подставляться для каждой попытки
			В.Вставить("ЕстьОшибки"                  , Ложь);

		НачатьШагВыполнения(КонтекстВыполненияЛентыНовостей, ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.НачалоОбработкиЛенты, В));
		ЛогШаговВыполнения = Новый Массив;

		// Лента новостей должна быть помечена для обработки или являться обязательным каналом
		Если (НЕ ТекущаяСтрока.Пометка) И (НЕ ТекущаяСтрока.ОбязательныйКанал) Тогда
			ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.БудетПропущена, В));
			ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполнения);
			Продолжить;
		КонецЕсли;

		Если  ПустаяСтрока(ТекущаяСтрока.АдресВХранилище) Тогда
			ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ПустойФайл, В));
			ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполнения);
			Продолжить;
		КонецЕсли;

		// Непосредственно загрузка из файла.
		В.ИмяВременногоФайла = СокрЛП(ПолучитьИмяВременногоФайла("xml"));
		...
		Если ТипЗнч(ДвоичныеДанные) = ТипДвоичныеДанные Тогда
			ДвоичныеДанные.Записать(В.ИмяВременногоФайла);
		КонецЕсли;
		Файл = Новый Файл(В.ИмяВременногоФайла);
		Если НЕ Файл.Существует() Тогда
			ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаСозданияФайла, В));
			ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполнения);
			Продолжить;
		КонецЕсли;
		...
		лкЧтениеТекста = Новый ЧтениеТекста(В.ИмяВременногоФайла);
		ЧтениеХМЛ = Новый ЧтениеXML;
		ЧтениеХМЛ.ОткрытьФайл(
			В.ИмяВременногоФайла,
		...
		Исключение
			...
			В.ПервыеСтрокиФайла = лкЧтениеТекста.Прочитать(2000);

			ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаПолученияТипаДанных, В));

			// Пропустить ленту новостей
			В.ЕстьОшибки = Истина;
		КонецПопытки;

		Если В.ЕстьОшибки = Ложь Тогда
			...
			ИначеЕсли (НРег(ХМЛТип.ИмяТипа) = "rss") Тогда // И (ВРег(ХМЛТип.URIПространстваИмен)=ВРег("http://www.w3.org/2005/RSS_2_0")).
				...
				Если (ОбъектХДТО.Свойства().Получить("version") = Неопределено) Тогда
					ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаОпределенияВерсииRSS, В));
					// Пропустить ленту новостей
					В.ЕстьОшибки = Истина;
				Иначе
					лкВерсия = ОбъектХДТО.Получить("version");
					Если (Число(лкВерсия) <> 2.0) Тогда
						В.ВерсияRSS = ОбъектХДТО.version;
						ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаНеподдерживаемаяВерсияRSS, В));
						// Пропустить ленту новостей
						В.ЕстьОшибки = Истина;
					КонецЕсли;
				КонецЕсли;
			Иначе
				// Попытаться прочитать первые 2000 символов файла.
				В.ПервыеСтрокиФайла = лкЧтениеТекста.Прочитать(2000);
				В.ТипНовостей = (ХМЛТип.URIПространстваИмен + ", " + ХМЛТип.ИмяТипа);
				ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаНеподдерживаемыйТипНовостей, В));
				// Пропустить ленту новостей.
				В.ЕстьОшибки = Истина;
			КонецЕсли;
			...
			Если В.ЕстьОшибки = Ложь Тогда
				...
				КонтекстВыполненияРазбораXMLФайла = НачатьШагВыполненияВложенный(
					КонтекстВыполненияЛентыНовостей,
					"Основные/ЦиклПоЛентамНовостей/РазборXMLФайлаНовостей"); // Идентификатор.

					РазборXMLФайлаНовостей(
						ОбъектХДТО,
						НРег(ХМЛТип.ИмяТипа),
						ТекущаяСтрока.ЛентаНовостей,
						Прокси,
						КонтекстВыполненияРазбораXMLФайла);

				ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", "", КонтекстВыполненияРазбораXMLФайла);
		...
		Попытка
			...
			ЧтениеХМЛ.Закрыть();
			УдалитьФайлы(В.ИмяВременногоФайла);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			В.ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОшибкаУдаленияВременногоФайла, В));
		КонецПопытки;

		ЛогШаговВыполнения.Добавить(ОбработкаНовостейКлиентСервер.ЗаполнитьШаблон(ВсеШаблоны.ОкончаниеОбработкиЛенты, В));
		ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполнения);

	КонецЦикла;

	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияЛентыНовостей);

#КонецОбласти

	// Вспомогательные обработки после разбора файлов новостей.
...
#Область ВспомогательныеОбработкиОсновные_КатегорииНовостейПростыеНеобработанные
	...
	Шаг = НаименованиеМетода + "/Основные:ПересчитатьНеобработанныеКатегорииНовостей";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		ОбработкаНовостейСлужебный.ПересчитатьНеобработанныеКатегорииНовостей(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#Область ВспомогательныеОбработкиОсновные_ОптимизацияОтборовПоНовостям
	...
	// Доступные категории лент новостей уже обновлены в "РазборXMLФайлаНовостей".
	...
	Шаг = НаименованиеМетода + "/Основные:ОптимизироватьОтборыПоНовостям";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_РедкоМеняющиеся
	...
	Шаг = НаименованиеМетода + "/Основные:ПересчитатьОтборыПоНовостям_РедкоМеняющиеся";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_Общие
	...
	// Может работать в модели сервиса.
	Шаг = НаименованиеМетода + "/Основные:ПересчитатьОтборыПоНовостям_Общие";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_ДляОбластиДанных
	...
	// Может работать в модели сервиса.
	Шаг = НаименованиеМетода + "/Основные:ПересчитатьОтборыПоНовостям_ДляОбластиДанных";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		// {Раздельное обновление ИБ}
		ЛогШаговВыполнения = Новый Массив;
		Если ОбрабатыватьОбластиДанных = Истина Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Выполнение пересчета отборов по новостям для области данных.'"));
			ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполненияВложенный);
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru='Пропуск пересчета отборов по новостям для области данных.'"));
		КонецЕсли;
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", ЛогШаговВыполнения, КонтекстВыполненияВложенный);
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_Пользовательские
	...
	//  т.к. он выполняется в ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Общие.
...
#Область ВспомогательныеОбработкиОсновные_ОбновлениеПериодическихСвойствНовостей
	...
	Шаг = НаименованиеМетода + "/Основные:ОбновлениеПериодическихСвойствНовостей";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		ОбработкаНовостейСлужебный.ОбновлениеПериодическихСвойствНовостей(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#Область ВспомогательныеОбработкиОсновные_ОбновлениеПривязокКМетаданным
	...
	//  и другие периодические свойства новостей. Также надо добавить записи для новых новостей.
	Шаг = НаименованиеМетода + "/Основные:ОбновлениеПривязокКМетаданным";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		ОбработкаНовостейСлужебный.ОбновлениеПривязокКМетаданным(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#Область ВспомогательныеОбработкиОсновные_СбросУНовостейПризнакаТребуетсяПересчетОтборов
	...
	Шаг = НаименованиеМетода + "/Основные:СбросУНовостейПризнакаТребуетсяПересчетОтборов";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		ОбработкаНовостейСлужебный.СбросУНовостейПризнакаТребуетсяПересчетОтборов(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#КонецОбласти
	...
	// Определенные разработчиком (переопределяемые) обработки после разбора файлов новостей.
#Область ВспомогательныеОбработкиПереопределяемые

	НачатьШагВыполнения(КонтекстВыполнения, "ПереопределяемыеДействия:ДополнительноОбработатьНовостиПослеПолучения"); // Идентификатор.
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьНовостиПослеПолучения(ТаблицаДатЗагрузокНовостейПередПолучением);
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьСтандартныйСписокССервера"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
			КонтекстВыполнения,
			НаименованиеМетода + "/ПодключитьсяКВебСервисуОбновленияСтандартныхСписков");
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:УдалосьПодключитьсяКВебСервисам"); // Идентификатор.
	Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.NOT_CONNECTED", НСтр("ru = 'Не удалось подключиться к веб-сервисам.'"));
...
#Область Обработка
	...
	ЛогШаговВыполнения = Новый Массив;
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ЗагрузкаДанных"); // Идентификатор.
		...
		Если НЕ ПустаяСтрока(СокрЛП(Список.Код)) Тогда
			...
			Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					errCode,
					errDescription,
					ОбработкаНовостейСлужебный.ПредставлениеОбъектаХДТО(СписокОбновленныхДанных, Прокси.ФабрикаXDTO));
				ЛогШаговВыполнения.Добавить(ТекстСообщения);
						...
						Для каждого ТекущаяСтрокаХДТО Из СписокОбновленныхДанных.categoryValue Цикл
							...
							Иначе
								Попытка
									...
									ОбработкаНовостейСлужебный.ДополнительноОбработатьКлассификаторПослеПолученияПослеЗаписи(лкОбъект.Ссылка);
								Исключение
									ИнформацияОбОшибке = ИнформацияОбОшибке();
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			КонецЕсли;
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Список.Код - пустая'"));
		КонецЕсли;
	...
	ИначеЕсли Список = "Список лент новостей" Тогда // Идентификатор.
		...
		Если ТипЗнч(СписокВсехДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				errCode,
				errDescription,
				ОбработкаНовостейСлужебный.ПредставлениеОбъектаХДТО(СписокВсехДанных, Прокси.ФабрикаXDTO));
			...
			Если (СписокВсехДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
				...
				Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Для каждого ТекущаяСтрокаХДТО Из СписокВсехДанных.channel Цикл
						ЛентаНовостейОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательЛентыНовостей(Новый УникальныйИдентификатор(ТекущаяСтрокаХДТО.guid));
						Если НЕ ПустаяСтрока(ЛентаНовостейОписатель.ВерсияДанных) Тогда // Объект существует в ИБ.
							// Найти эту ленту новостей в списке и отметить, что она присутствует на сервере 1С.
							НайденнаяЛентаНовостей = СписокВсехЗагруженныхСерверныхЛентНовостей.НайтиПоЗначению(ЛентаНовостейОписатель.Ссылка);
	...
	ИначеЕсли СтрНайти(ВРег(Список), ВРег("Значения категории новостей:")) = 1 Тогда // Идентификатор.
		...
		Если НЕ ПустаяСтрока(СписокУИН) Тогда
			...
			ВладелецОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательКатегорииНовостей(Новый УникальныйИдентификатор(СписокУИН));
			Если НЕ ПустаяСтрока(ВладелецОписатель.ВерсияДанных) Тогда
				СписокОбновленныхДанных = Неопределено;
				Попытка
					...
					Если (СписокОбновленныхДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
						...
						Если ТипЗнч(СписокОбновленныхДанных.categoryValue) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
							...
							Для каждого ТекущаяСтрокаХДТО Из СписокОбновленныхДанных.categoryValue Цикл
								...
								// Загрузить значение.
								Режим = "";
								лкОбъект = ЗаполнитьОбъектЗначениеКатегорииНовостей(ВладелецОписатель.Ссылка, ТекущаяСтрокаХДТО, Режим);
							...
							Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
							Запись.Список = ВладелецОписатель.Ссылка;
							Запись.Прочитать(); // Запись будет ниже.
							Если Запись.Выбран() Тогда
								Если Запись.ТекущаяВерсияНаКлиенте < ДатаСамогоНовогоЭлемента Тогда
									Запись.Список = ВладелецОписатель.Ссылка;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:РазрешенаРаботаСНовостямиЧерезИнтернет"); // Идентификатор.
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Подключение:ВебСервисыОбновленияСтандартныхСписков"); // Идентификатор.
	Прокси = Неопределено;
	Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
		...
		Прокси = ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполненияВложенный);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
		// Это не критичная ошибка, продолжаем выполнение.
	КонецЕсли;
	...
	// Обновление категорий новостей - обновлять всегда, чтобы оперативно исправить задвоение кодов категорий.
	БылоОбновлениеКатегорийНовостей = Ложь;
	...
	Иначе
		...
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
			КонтекстВыполнения,
			НаименованиеМетода + "/ОбновитьСписокСправочниковИВерсииССервера");
		...
		НачатьШагВыполнения(КонтекстВыполнения, "Основное:ПолучениеСписковТребующихОбновление"); // Идентификатор.
		...
		Для каждого ТекущийОбновляемыйСписок Из ТаблицаОбновляемыхДанных Цикл
			...
			КонецЕсли;
			Если ТекущийОбновляемыйСписок.Список = "Список категорий новостей" Тогда // Идентификатор.
				БылоОбновлениеКатегорийНовостей = Истина;
			КонецЕсли;
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ТекущийОбновляемыйСписок.Список); // Идентификатор.
		...
		Если БылоОбновлениеЛентНовостей <> Истина Тогда
			ИдентификаторСписка = "Список лент новостей"; // Идентификатор.
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ИдентификаторСписка); // Идентификатор.
			БылаОшибка = НЕ ОбновитьСтандартныйСписокССервера(
				ИдентификаторСписка, // Идентификатор.
				'00010101',
				Прокси,
				КонтекстВыполненияВложенный);
			Если БылаОшибка = Истина Тогда
		...
		// Категории новостей обновлять всегда, вне зависимости от того, были ли они обновлены на сервере,
		//  чтобы оперативно исправить задвоение кодов категорий.
		Если БылоОбновлениеКатегорийНовостей <> Истина Тогда
			ИдентификаторСписка = "Список категорий новостей"; // Идентификатор.
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ИдентификаторСписка); // Идентификатор.
			БылаОшибка = НЕ ОбновитьСтандартныйСписокССервера(
				ИдентификаторСписка, // Идентификатор.
				'00010101',
		...
		// Удалить неправильные отборы, которые могут помешать проверке общих и пользовательских отборов.
		// В разделенном сеансе будут пересчитаны только пользовательские отборы и общие для области данных.
		Шаг = НаименованиеМетода + "/ОптимизироватьОтборыПоНовостям";
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
...
// Процедура обновляет список обновляемых справочников и их версии с сервера 1С (Регистр сведений ДатыОбновленияСтандартныхСписковНовостей).
//
// Параметры:
//  КонтекстВыполнения - Структура, Неопределено - см. ОбработкаНовостейКлиентСервер.НовыйКонтекстВыполнения.
//
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьСписокСправочниковИВерсииССервера"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	// Непосредственно загрузка данных с помощью веб-сервисов.
	Шаг = НаименованиеМетода + "/ПодключитьсяКВебСервисуОбновленияСтандартныхСписков";
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		Прокси = ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполненияВложенный);
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
...
#Область Обработка_ПолучениеПараметровВыполнения
	// Имя и версия текущей конфигурации из метаданных (по-умолчанию).
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПолучениеПараметровВыполнения"); // Идентификатор.
	configName    = СокрЛП(Метаданные.Имя);
	configVersion = ОбработкаНовостейКлиентСервер.ВнутреннееПредставлениеНомераВерсии(СокрЛП(Метаданные.Версия));
	// Возможно, имя конфигурации из метаданных было заменено на какой-то синоним - взять синоним.
	Продукты = ОбработкаНовостейВызовСервера.ПолучитьЗначениеПредопределеннойКатегории("Product");
	Если Продукты.Количество() > 0 Тогда
		Продукт = Продукты[Продукты.Количество()-1];
...
#Область Обработка_ПолучениеДатОбновленияСписков
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ДатыОбновленияСписков"); // Идентификатор.
	Попытка
		ДатыОбновленияСписков = Прокси.getLastUpdatesList(configName, configVersion, errCode, errDescription);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Успешно обновлено.
				|configName = %1
				|configVersion = %2
				|errCode = %3
				|errDescription = %4
			...
			?(ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO,
				ОбработкаНовостейСлужебный.ПредставлениеОбъектаХДТО(ДатыОбновленияСписков, Прокси.ФабрикаXDTO),
				НСтр("ru='Получено значение с типом значения, отличным от ОбъектXDTO'"))));
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.NOT_CONNECTED", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка обновления.
				|configName = %1
				|configVersion = %2
				|Причина:
				|%3'"),
...
#Область Обработка_ЗагрузкаДанных

	НачатьШагВыполнения(КонтекстВыполнения, "Основное:ПолучениеДанных"); // Идентификатор.
				...
				Для каждого ТекущаяЗапись Из ДатыОбновленияСписков.updatesItem Цикл
					Если (ТекущаяЗапись.listName = "Список категорий новостей") // Идентификатор.
						...
						Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
							...
							Запись.Прочитать(); // Запись будет ниже. Чтение необходимо, чтобы прочитать ТекущаяВерсияНаКлиенте.
					...
					ИначеЕсли ТекущаяЗапись.listName = "Значения категории новостей" Тогда // Идентификатор.
						...
						Если ТекущаяЗапись.Свойства().Получить("categoryGuid") <> Неопределено Тогда
							Если НЕ ПустаяСтрока(ТекущаяЗапись.categoryGuid) Тогда
								...
								КатегорияОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательКатегорииНовостей(Новый УникальныйИдентификатор(ТекущаяЗапись.categoryGuid));
								Если ПустаяСтрока(КатегорияОписатель.ВерсияДанных) Тогда // Категория не найдена.
									...
									Если ТекущаяЗапись.Свойства().Получить("categoryCode") <> Неопределено Тогда
										Если НЕ ПустаяСтрока(ТекущаяЗапись.categoryCode) Тогда
											КатегорияОписатель.Ссылка = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(ТекущаяЗапись.categoryCode);
											Если НЕ КатегорияОписатель.Ссылка.Пустая() Тогда
								...
								Если КатегорияНайдена = Истина Тогда
									Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
										Запись.Список = КатегорияОписатель.Ссылка;
										Запись.Прочитать(); // Запись будет ниже. Чтение необходимо, чтобы прочитать ТекущаяВерсияНаКлиенте.
										Если Запись.Выбран() Тогда // Обновить
											Запись.Список = КатегорияОписатель.Ссылка;
											// Запись.ТекущаяВерсияНаКлиенте = // не трогать
	...
	КонецЕсли;
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Получение списка последних обновлений, результат
			|errCode = %1
			|errDescription = %2'"),
		errCode,
		errDescription));
...
// И так как автоматизировать получение даты невозможно, то разбор будет производиться вручную.
//
...
// Возвращает количество секунд между обновлениями в зависимости от параметра ЧастотаОбновления.
//
// Параметры:
//  ЧастотаОбновления - число - реквизит ЧастотаОбновления ленты новостей.
//
// Возвращаемое значение:
//   Число - число секунд в зависимости от параметра ЧастотаОбновления.
//
Функция ПреобразоватьЧастотуОбновленияВСекунды(ЧастотаОбновления)

	Результат = 30*60;

	Если ЧастотаОбновления = 1 Тогда // Ежедневно
		Результат = 24*60*60;
	ИначеЕсли ЧастотаОбновления = 2 Тогда // Ежечасно
		Результат = 60*60;
	ИначеЕсли ЧастотаОбновления = 3 Тогда // Каждые 15 минут
		Результат = 15*60;
	ИначеЕсли ЧастотаОбновления = 4 Тогда // Ежеминутно
		Результат = 60;
	КонецЕсли;
...
#Область ЗагрузкаНовостей

// Непосредственно разбор файла новостей, заполнение категорий,
// заполнение дополнительных действий 1С (обработчиков гиперссылок).
//
Процедура РазборXMLФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, Прокси, КонтекстВыполнения = Неопределено)

#Область ИнициализацияМетода

	НаименованиеМетода = "Обработка.УправлениеНовостями.РазборXMLФайлаНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основное:ДатыЗагрузкиПоследнихНовостей"); // Идентификатор.
	Запись = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьМенеджерЗаписи();
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		НаименованиеМетода + "/ОбработкаФайла");

	Если НРег(ФорматНовостей) = "rss" Тогда

		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Разбор файла новостей для ленты новостей %1
		...
		Если ТипЗнч(ОбъектХДТО.Свойства().Получить("channel")) = ТипСвойствоXDTO
			...
			Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
				...
				Для каждого ТекущиеОбновляемыеДанные Из ТаблицаОбновляемыхДанных Цикл
					Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
						// Один раз проинициализировав переменную, будем ее использовать для разбора всех остальных файлов.
						Прокси = ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполненияВложенный);
					КонецЕсли;
			...
			Иначе
				ЛогШаговВыполнения.Добавить(
					НСтр("ru='Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(rss): Отключена работа с новостями через интернет.
			...
			Если НеобходимоОбработатьФайл = Истина Тогда
				...
				// Цикл по разбору каждой новости.
	...
	ИначеЕсли НРег(ФорматНовостей) = "feed" Тогда
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				НСтр("ru='Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(atom): Отключена работа с новостями через интернет.
					|Обновление классификаторов не выполнялось.'"));
		КонецЕсли;
...
Процедура ДобавитьНеобработанноеЗначениеКатегорииДляНовости_category(ОбъектНовость, КатегорияИЗначение)
	...
	Иначе
		...
		ИмяПоля_Записи     = "РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записи";
		ИмяПоля_Записывать = "РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записывать";
		ДополнительныеСвойстваЭталон = Новый Структура;
			ДополнительныеСвойстваЭталон.Вставить(ИмяПоля_Записи    , Новый Массив);
			ДополнительныеСвойстваЭталон.Вставить(ИмяПоля_Записывать, Истина);
		ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ОбъектНовость.ДополнительныеСвойства);
		...
		Если ГдеРазделитель > 0 Тогда
			...
			// Добавить данные.
			// Запись в регистр сведений произойдет при записи объекта новости.
			Запись = РегистрыСведений.КатегорииНовостейПростыеНеобработанные.НовыйОписательЗаписи();
			...
			ДополнительныеСвойстваЭталон[ИмяПоля_Записи].Добавить(Запись);

			ОбъектНовость.ДополнительныеСвойства.Вставить(ИмяПоля_Записи, ДополнительныеСвойстваЭталон[ИмяПоля_Записи]);
			ОбъектНовость.ДополнительныеСвойства.Вставить(ИмяПоля_Записывать, Истина); // Записывать всегда
		Иначе
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьНовость"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		НаименованиеМетода + "/Основное:ЗагрузкаНовости"); // Идентификатор.
	...
	Если НРег(ФорматНовостей) = "rss" Тогда
		...
		СтруктураНовости = ОбработкаНовостейСлужебный.ПолучитьСсылкуНаНовость(ЛентаНовостей, УИННовости);
		Если СтруктураНовости.Ссылка.Пустая() Тогда
			Если ЗначениеПометкаУдаления = Истина Тогда
				...
				ЗаписыватьНовость = Ложь;
			Иначе
				ОбъектНовость = ОбработкаНовостейСлужебный.ПолучитьОбъектНовости(
		...
		Если ЗаписыватьНовость = Истина Тогда
			...
			// Простые категории в RSS и в Atom хранятся по-разному, поэтому загрузка реализована напрямую.
			...
			// Записывать новость с помощью БлокировкаДанных.
			// Не смотря на то, что новости получаются регламентным заданием с установленным ключом [ВсеОбновленияНовостей],
			//  возможна ситуация, когда новость с одинаковым набором ЛентаНовостей+УИННовости запишется одновременно
			//  и произойдет задвоение, что потом приведет к невозможности выгрузить новость
			//  коробка -> модель сервиса, модель сервиса -> коробка.
			// Непосредственно перед записью снова прочитаем новость по ключам ЛентаНовостей+УИННовости, чтобы убедиться,
			//  что в каком-либо параллельном потоке не создали аналогичную новость.
			Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("Справочник.Новости");
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				ЭлементБлокировки.УстановитьЗначение("ЛентаНовостей", ОбъектНовость.ЛентаНовостей);
				ЭлементБлокировки.УстановитьЗначение("УИННовости"   , ОбъектНовость.УИННовости);

			НачатьТранзакцию();

			Попытка

				ЛогШаговВыполнения.Добавить(НСтр("ru='Начало попытки записи новости'"));

				Блокировка.Заблокировать();

				СтруктураНовостиПередЗаписью = ОбработкаНовостейСлужебный.ПолучитьСсылкуНаНовость(ОбъектНовость.ЛентаНовостей, ОбъектНовость.УИННовости);
				Если (СтруктураНовости.Ссылка.Пустая()) И (НЕ СтруктураНовостиПередЗаписью.Ссылка.Пустая()) Тогда
					// В процессе заполнения (от ОбработкаНовостейСлужебный.ПолучитьСсылкуНаНовость до текущего шага) какой-то поток
					//  успел записать новость с такими же ключами (ЛентаНовостей и УИННовости).
					// Не сохранять такую новость.

					ОтменитьТранзакцию(); // Запись не происходила. Транзакцию можно отменить.

					ЛогШаговВыполнения.Добавить(НСтр("ru='Найдена новость с такими же значениями ключей (ЛентаНовостей и УИННовости). Запись отменена.'"));
				...
				Иначе
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru='Успешная попытка записи новости'"));
		...
		РезультатЗагрузкиНовости = ""
			...
			+ УИННовости + "; "
			+ ЗаголовокНовости
			+ НСтр("ru = 'ДатаПубликации'") + "="
				+ Формат(ДатаПубликации, "ДЛФ=DT") + "/"
				+ ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru='(в базе не было)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")") + "; "
			+ НСтр("ru = 'ПометкаУдаления'") + "="
				+ ЗначениеПометкаУдаления + "/"
				+ ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru='(в базе не было)'"), " (" + ЗначениеПометкаУдаленияИБ + ")") + "; "
			+ ?(ЗаписыватьНовость = Истина, НСтр("ru='Будет записана'"), НСтр("ru='Будет пропущена'")) + "; "
			+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru='В базе еще не создана'"), НСтр("ru='Уже существует в базе'")) + "; "
			+ НСтр("ru = 'УИН_ИБ'") + "="
				+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru='(еще не создана)'"), "[" + СтруктураНовости.УИН_ИБ + "]")
			+ НСтр("ru = 'НавигационнаяСсылка'") + "="
				+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru='(еще не создана)'"), "[" + ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка) + "]");
...
Процедура ЗагрузитьБлокДанныхКатегорий1С_КатегорияИнтервалВерсий(ДанныеХДТО, ОбъектНовость)
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		ИмяПоля_Записи     = "РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записи";
		ИмяПоля_Записывать = "РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записывать";
		ДополнительныеСвойстваЭталон = Новый Структура;
			ДополнительныеСвойстваЭталон.Вставить(ИмяПоля_Записи    , Новый Массив);
			ДополнительныеСвойстваЭталон.Вставить(ИмяПоля_Записывать, Истина);
		ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ОбъектНовость.ДополнительныеСвойства);
		...
		// Добавить данные.
		// Запись в регистр сведений произойдет при записи объекта новости.
		Запись = РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные.НовыйОписательЗаписи();
			Запись.КатегорияНовостейСтрока = КатегорияНовостейСтрока;
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				...
				ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru='Новости. Загрузка новостей. Разбор файлов новостей. Разбор файла новостей. Разбор новости. Чтение новости. Блок бинарных данных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
//
// Параметры:
//  Обновление         - ОбъектХДТО - объект с полями:
//   * listName;
//   * categoryCode;
//   * categoryGuid;
//   * updated;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ТипОбъектаОбновления"); // Идентификатор.
	Если ТипЗнч(Обновление) <> ТипОбъектXDTO Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Тип значения параметра Обновление = [%1]. Ожидается [ОбъектXDTO].'"),
			ТипЗнч(Обновление));
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.PARAMETER_OF_WRONG_TYPE", ТекстСообщения);
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Данные:ПолучениеСвойствОбъекта"); // Идентификатор.
	Попытка
		...
		КонецЕсли;
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"listName=%1; categoryCode=%2; categoryGuid=%3; updated=%4",
			listName, categoryCode, categoryGuid, updated));
	Исключение
		// Пропустить строку обновлений.
		ЕстьОшибки = Истина;
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CANNOT_PARSE", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка загрузки строки обновления списков по причине:
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки;

	НачатьШагВыполнения(КонтекстВыполнения, "Данные:ЗаписьДанных"); // Идентификатор.
	Если ЕстьОшибки = Ложь Тогда
...
Функция ЗаполнитьОбъектКатегорияНовостей(ОбъектХДТО, Режим = "")
	...
	КатегорияОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательКатегорииНовостей(Новый УникальныйИдентификатор(ОбъектХДТО.guid));
	Если ПустаяСтрока(КатегорияОписатель.ВерсияДанных) Тогда // Новый объект
		лкОбъект = ПланыВидовХарактеристик.КатегорииНовостей.СоздатьЭлемент();
		лкОбъект.УстановитьСсылкуНового(КатегорияОписатель.Ссылка);
		Режим = "Создание";
	Иначе // Объект есть в базе данных
		лкОбъект = КатегорияОписатель.Ссылка.ПолучитьОбъект();
		Режим = "Обновление";
	КонецЕсли;
	...
	// Проверить, сколько категорий новостей с таким же кодом. В случае, если есть несколько категорий с таким же кодом,
	//  то записать в журнал регистрации.
	НайденныеДубликаты = Новый Массив;
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		...
		|";
	Запрос.УстановитьПараметр("КатегорияКод", лкОбъект.Код);
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			Если лкОбъект.ЭтоНовый() Тогда
				НайденныеДубликаты.Добавить(Выборка.Категория);
			Иначе
				Если СокрЛП(ОбъектХДТО.guid) <> СокрЛП(Выборка.Категория.УникальныйИдентификатор()) Тогда
					НайденныеДубликаты.Добавить(Выборка.Категория);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если НайденныеДубликаты.Количество() > 0 Тогда
			ЛогШаговВыполнения = Новый Массив;
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При загрузке категории новостей [%1] [%2] (%3), режим [%4] обнаружены дубликаты:'", КодОсновногоЯзыка),
					лкОбъект.Код,
					лкОбъект.Наименование,
					ОбъектХДТО.guid,
					Режим));
			Для Каждого НайденныйДубликат Из НайденныеДубликаты Цикл
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='[%1] [%2] (%3);'", КодОсновногоЯзыка),
						НайденныйДубликат.Код,
						НайденныйДубликат.Наименование,
						СокрЛП(НайденныйДубликат.УникальныйИдентификатор())));
			КонецЦикла;
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru='Новости. Загрузка классификаторов. Список категорий. Ошибка'", КодОсновногоЯзыка), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				ПланыВидовХарактеристик.КатегорииНовостей, // ОбъектМетаданных
				, // Данные
...
Функция ЗаполнитьОбъектЗначениеКатегорииНовостей(лкВладелец, ОбъектХДТО, Режим = "")
	...
	ЗначениеКатегорииНовостейОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательЗначенияКатегорийНовостей(Новый УникальныйИдентификатор(ОбъектХДТО.guid));
	Если ПустаяСтрока(ЗначениеКатегорииНовостейОписатель.ВерсияДанных) Тогда // Новый объект
		лкОбъект = Справочники.ЗначенияКатегорийНовостей.СоздатьЭлемент();
		лкОбъект.УстановитьСсылкуНового(ЗначениеКатегорииНовостейОписатель.Ссылка);
		Режим = "Создание";
	Иначе // Объект есть в базе данных
		лкОбъект = ЗначениеКатегорииНовостейОписатель.Ссылка.ПолучитьОбъект();
		Режим = "Обновление";
	КонецЕсли;
	...
	Если ОбъектХДТО.parentGuid <> Неопределено Тогда
		РодительОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательЗначенияКатегорийНовостей(Новый УникальныйИдентификатор(ОбъектХДТО.parentGuid));
		Если НЕ ПустаяСтрока(РодительОписатель.ВерсияДанных) Тогда // Есть такой элемент в базе данных
			лкОбъект.Родитель = РодительОписатель.Ссылка;
		КонецЕсли;
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗаполнитьОбъектЛентаНовостей"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ЗаполнениеОбъектаЛентаНовостей"); // Идентификатор.

	ЛентаНовостейОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательЛентыНовостей(Новый УникальныйИдентификатор(ОбъектХДТО.guid));
	Если ПустаяСтрока(ЛентаНовостейОписатель.ВерсияДанных) Тогда // Новый объект
		лкОбъект = Справочники.ЛентыНовостей.СоздатьЭлемент();
		лкОбъект.УстановитьСсылкуНового(ЛентаНовостейОписатель.Ссылка);
		Режим = "Создание"; // Идентификатор.
	Иначе // Объект есть в базе данных
		лкОбъект = ЛентаНовостейОписатель.Ссылка.ПолучитьОбъект();
		...
		Если ТипЗнч(СвойствоКатегория) = ТипСвойствоXDTO Тогда
			Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
				...
				Для каждого ТекущаяКатегорияХДТО Из ОбъектХДТО.categoriesRefList.categoryRef Цикл
					...
					// Не рекомендуется заполнять категории по коду, т.к. с одним кодом может быть несколько категорий. Лучше искать по УИН.
					КатегорияОписатель = ОбработкаНовостейСлужебный.ПолучитьОписательКатегорииНовостей(Новый УникальныйИдентификатор(ТекущаяКатегорияХДТО.guid));
					// Для неправильного УИН будет "Объект не найден" с НЕпустой ссылкой. Сравнивать по "ВерсияДанных".
					Если ПустаяСтрока(КатегорияОписатель.ВерсияДанных) Тогда
						КатегорияОписатель.Ссылка = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(ТекущаяКатегорияХДТО.Code);
						Если НЕ КатегорияОписатель.Ссылка.Пустая() Тогда
							МассивДоступныхКатегорий.Добавить(КатегорияОписатель.Ссылка);
						КонецЕсли;
					Иначе
						МассивДоступныхКатегорий.Добавить(КатегорияОписатель.Ссылка);
					КонецЕсли;
				...
				// Удалять будем задом-наперед, т.к. при удалении будет сдвигаться индекс текущего элемента.
				ТекущийИндекс = лкОбъект.ДоступныеКатегорииНовостей.Количество() - 1;
				Пока (ТекущийИндекс >= 0) Цикл
					ТекущаяКатегория = лкОбъект.ДоступныеКатегорииНовостей[ТекущийИндекс];
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ПодключитьсяКВебСервисуОбновленияСтандартныхСписков"; // Идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Подключение"); // Идентификатор.

	МестоположениеWSDL = ОбработкаНовостейСлужебный.ПолучитьАдресWSDLОпределенияВебСервисаInformation();
	...
	Попытка
		...
		Прокси = ИнтернетПоддержкаПользователей.СоздатьWSПрокси(ПараметрыПрокси);

		Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков'"));
			ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.NOT_CONNECTED", ЛогШаговВыполнения);
		Иначе
...
// Список всех текстовых шаблонов для сообщений для метода ЗагрузитьФайлыНовостейССервера.
//
// Возвращаемое значение:
//   Структура - список шаблонов.
//
Функция ВсеШаблоныСообщенийЗагрузкиФайловНовостей()

	Результат = Новый Структура;

	Результат.Вставить("НачалоПопытки",
		НСтр("ru='Начало попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл [{{ВременныйФайл.Имя}}].
			|(URL: {{Запрос.URL}}).'"));

	Результат.Вставить("ПолученОтвет",
		НСтр("ru='При попытке {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл получен ответ сервера с кодом [{{Ответ.КодСостояния}}], заголовки:
			|{{Ответ.Заголовки}}
			|
			|Заголовки запроса:
			|{{Запрос.Заголовки}}
			|
			|(URL: {{Запрос.URL}}).
			|Время загрузки (сек): {{Ответ.Длительность}}'"));

	Результат.Вставить("ПолученОтвет304",
		НСтр("ru='Успешное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}].
			|(URL: {{Запрос.URL}}).
			|ETag: [{{Запрос.ETag}}]
			|Список новостей не изменился, поэтому файл не был загружен. Лента новостей - актуальна.'"));

	Результат.Вставить("ПолученОтветФайл",
		НСтр("ru='Успешное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}].
			|во временный файл [{{ВременныйФайл.Имя}}].
			|Получен файл размером {{ВременныйФайл.Размер}} байт.
			|(URL: {{Запрос.URL}}).
			|Первые строки файла:
			|{{ВременныйФайл.ПервыеСтроки}}'"));

	Результат.Вставить("НеУдалосьУдалитьФайл",
		НСтр("ru='ЗагрузитьФайлыНовостейССервера (получено из интернет):
			|Не удалось удалить временный файл [{{ВременныйФайл.Имя}}]
			|для ленты новостей [{{ЛентаНовостей}}] по причине:
			|{{ПодробноеПредставлениеОшибки}}
			|
			|Рекомендуется вручную удалить этот временный файл.'"));

	Результат.Вставить("ПолученОтветНоФайлаНет",
		НСтр("ru='Неудачное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл [{{ВременныйФайл.Имя}}].
			|(URL: {{Запрос.URL}}).
			|После попытки загрузки, файла все еще не существует.'"));

	Результат.Вставить("ИсключениеПриПопыткеЗагрузкиФайлаНовостей",
		НСтр("ru='Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			|Ошибка получения файла [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл [{{ВременныйФайл.Имя}}] по причине:
			|{{ПодробноеПредставлениеОшибки}}'"));

	Результат.Вставить("РезультатЗагрузкиФайлаНовостей",
		НСтр("ru='Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			|Загрузка файла новостей [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл [{{ВременныйФайл.Имя}}].
			|(URL: {{Запрос.URL}}).
			|Успешно? {{ЗагрузкаУспешна}}'"));

	Результат.Вставить("Переадресация",
		НСтр("ru='Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			|Загрузка файла новостей [{{Запрос.ИмяФайла}}] с сервера [{{Запрос.Сайт}}] для ленты новостей [{{ЛентаНовостей}}]
			|во временный файл [{{ВременныйФайл.Имя}}].
			|(URL: {{Запрос.URL}}).
			|Код состояния: {{Ответ.КодСостояния}}
			|Новый адрес: {{URLПереадресации}}'"));

	Возврат Результат;

КонецФункции

// Список всех текстовых шаблонов для сообщений для метода РазборXMLФайловНовостей.
//
// Возвращаемое значение:
//   Структура - список шаблонов.
//
Функция ВсеШаблоныСообщенийРазбораXMLФайловНовостей()

	Результат = Новый Структура;

	Результат.Вставить("НачалоОбработкиЛенты",
		НСтр("ru='Лента: [{ЛентаНовостей}], обязательная: {ОбязательныйКанал}, пометка: {Пометка}'"));

	Результат.Вставить("БудетПропущена",
		НСтр("ru='Лента: [{ЛентаНовостей}] будет пропущена: обязательная: {ОбязательныйКанал}, пометка: {Пометка}'"));

	Результат.Вставить("ПустойФайл",
		НСтр("ru='Не загружен файл новостей для ленты новостей [{ЛентаНовостей}] (пустой адрес в хранилище после загрузки файла [{ИмяФайла}]
			|с сайта [{Сайт}] по протоколу [{Протокол}]). Лента новостей не обработана.'"));

	Результат.Вставить("ОшибкаСозданияФайла",
		НСтр("ru='Отсутствует загруженный файл [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}].
			|Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.'"));

	Результат.Вставить("ОшибкаПолученияТипаДанных",
		НСтр("ru='В загруженном файле для ленты новостей [{ЛентаНовостей}] находится текст не в формате xml. Лента новостей будет пропущена.
			|
			|Подробное представление ошибки:
			|{ПодробноеПредставлениеОшибки}
			|
	...
	Результат.Вставить("ОшибкаОпределенияВерсииRSS",
		НСтр("ru='Невозможно определить версию rss в ленте новостей [{ЛентаНовостей}] - отсутствует свойство version.
			|Необходимы данные в формате версии 2.0.'"));

	Результат.Вставить("ОшибкаНеподдерживаемаяВерсияRSS",
		НСтр("ru='Неподдерживаемая версия новостей RSS [{ВерсияRSS}] в ленте новостей [{ЛентаНовостей}].'"));

	Результат.Вставить("ОшибкаНеподдерживаемыйТипНовостей",
		НСтр("ru='Неподдерживаемый тип новостей [{ТипНовостей}] в файле [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}]
			|
			|Первые строки файла:
			|{ПервыеСтрокиФайла}'"));

	Результат.Вставить("ОшибкаУдаленияВременногоФайла",
		НСтр("ru='Не удалось удалить временный файл [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}] по причине:
			|{ПодробноеПредставлениеОшибки}
			|
			|Рекомендуется вручную удалить этот временный файл.'"));

	Результат.Вставить("ОкончаниеОбработкиЛенты",
		НСтр("ru='Завершение обработки ленты: [{ЛентаНовостей}]. Успешно: {ЕстьОшибки|БЛ=БезОшибок;БИ=ЕстьОшибки}'"));
