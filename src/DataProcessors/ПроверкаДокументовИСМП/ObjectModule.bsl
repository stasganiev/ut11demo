...
Функция ФорматЭдоПоТипуДокумента(ТипДокумента)
	...
	Результат = Новый Структура("ФорматЭДО, НаименованиеФормата", ФорматЭДО, НаименованиеФормата);
...
Процедура ЗапроситьСтатусОтправкиИСтатусГИСМТ(ПараметрыОбработки)
	...
	Если ЗначениеЗаполнено(СтатусОбработкиДокумента.СтатусДокументаВСервисе) Тогда
		ПараметрыОбработки.СтатусОтправки = Перечисления.СтатусОтправкиГИСМТ.ДоставленВГИСМТ;
		ПараметрыОбработки.Статус            = СтатусОбработкиДокумента.СтатусДокументаВСервисе;
...
Функция НаличиеИдентификатораЭДОЛайтУОрганизации(ПараметрыОбработки)
	...
	РезультатПроверки.СостояниеОшибки = СтрШаблон(
		НСтр("ru = 'В личном кабинете ГИСМТ у организации ИНН %1 отсутствует идентификатор ЭДО Лайт'"),
		ПараметрыОбработки.РеквизитыДокумента.ОрганизацияИНН);
...
Функция НаличиеИдентификатораЭДОЛайтУКонтрагента(ПараметрыОбработки)
	...
	РезультатПроверки.СостояниеОшибки = СтрШаблон(
		НСтр("ru = 'В личном кабинете ГИСМТ у контрагента ИНН %1 отсутствует идентификатор ЭДО Лайт'"),
		ПараметрыОбработки.РеквизитыДокумента.КонтрагентИНН);
...
Функция ПроверкаФорматаЭлектронногоДокументаВНастройкахЭДО(ПараметрыОбработки)
	...
	ФорматЭдоПоТипуДокумента = ФорматЭдоПоТипуДокумента(ПараметрыОбработки.ТипДокумента);
	
	Если ПараметрыОбработки.НастройкиОбменаЭД.Формат <> ФорматЭдоПоТипуДокумента.ФорматЭДО Тогда
		РезультатПроверки.ЕстьОшибки  = Истина;
		РезультатПроверки.ТекстОшибки = НСтр("ru = 'Неверный формат ЭД'");
		РезультатПроверки.Данные = ФорматЭдоПоТипуДокумента.НаименованиеФормата;
	КонецЕсли;
...
Функция ЗапроситьСтатусРегистрацииУчастникаИТоварныеГруппы(Организация, ИНН)
	...
	URLЗапроса = СтрШаблон(
		...
		ИНН);
	РезультатЗапроса = ОбщегоНазначенияИС.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ОбщегоНазначенияИСМПКлиентСервер.ПараметрыОтправкиHTTPЗапросов(""));
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				ВозвращаемоеЗначение.СтатусДокументаВСервисе = Статус;
