...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ПечатнаяФорма.ИмяМакета = "ПФ_MXL_ЛистСогласования" Тогда
			ПолныйПутьКМакету = "Обработка.ИнтеграцияС1СДокументооборот.ПФ_MXL_ЛистСогласования";
			МодульУправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				КоллекцияПечатныхФорм,
				ПечатнаяФорма.ИмяМакета,
				ПечатнаяФорма.СинонимМакета,
				ПодготовитьЛистСогласования(МассивОбъектов, ПолныйПутьКМакету, МодульУправлениеПечатью),,
				ПолныйПутьКМакету);
		КонецЕсли;
	КонецЦикла;
...
//
Функция ПодготовитьЛистСогласования(ПредметыСогласования, ПолныйПутьКМакету = "",
		МодульУправлениеПечатью = Неопределено) Экспорт
	
	Если МодульУправлениеПечатью = Неопределено Тогда
		Макет = Обработки.ИнтеграцияС1СДокументооборот.ПолучитьМакет("ПФ_MXL_ЛистСогласования");
	Иначе
		Макет = МодульУправлениеПечатью.МакетПечатнойФормы(ПолныйПутьКМакету);
	КонецЕсли;
	
	// Без поддержки листа согласования со стороны веб-сервиса этой команды не должно быть в интерфейсе.
	// В случае неожиданной смены версии на стороне сервиса выдадим исключение.
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПараметрыПечати_ИнтеграцияС1СДокументооборот_ЛистСогласования";
	...
	Иначе
		// Единственная ссылка.
		МассивПредметов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПредметыСогласования);
	КонецЕсли;
	...
	Для Каждого ПредметСогласования Из МассивПредметов Цикл
		...
		Если СвязанныйОбъектДО = Неопределено Тогда
			...
			ТабличныйДокумент.Вывести(ОбластьСогласованиеНеЗапускалось);
