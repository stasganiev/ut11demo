	...
	Для Каждого СтрокаТаблицы Из ПодобранныеСтроки Цикл
		...
		Если ВыборкаДоговораПоКонтрагенту.НайтиСледующий(Отбор) Тогда
			...
			Если ВыборкаДоговора.Следующий() Тогда
				ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ВыборкаДоговора);
			КонецЕсли;
...
Процедура КонтрагентПриИзменении(ТекущаяСтрокаТаблицы)
	...
	ТекущаяСтрокаТаблицы.Договор = ДоговорПоУмолчанию(
		Объект.Организация, ТекущаяСтрокаТаблицы.Партнер, ТекущаяСтрокаТаблицы.Контрагент, Валюта);
...
&НаСервере
Функция ДоговорПоУмолчанию(Организация, Партнер, Контрагент, Валюта)
