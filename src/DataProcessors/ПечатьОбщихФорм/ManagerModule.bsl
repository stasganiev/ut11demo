...
Процедура ПоместитьВременнуюТаблицуДанныхПоставщика(МенеджерВременныхТаблиц) Экспорт
	...
	|		КОГДА НЕ ДанныеОрганизаций.РегистрацияВНалоговомОргане ЕСТЬ NULL ТОГДА
	...
	Для Каждого Элемент Из СтруктураСуммПоСтроке Цикл
		Если ЗначениеЗаполнено(Элемент.Значение) Тогда
			...
			ИначеЕсли ОблагаетсяНДССМежценовойРазницы
				...
				ЗначениеСтавкиНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(СтрокаТовары.СтавкаНДС);
				ИтоговыеСуммы["ИтогоСтоимость"] = ИтоговыеСуммы["ИтогоСтоимость"] + ?(ЗначениеСтавкиНДС = 0, 0, 100 * СтрокаТовары.СуммаНДС / ЗначениеСтавкиНДС);
			Иначе
...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			ПараметрыПечати = Новый Структура("Корректировочный,СчетФактураНаАванс,ЕстьПрослеживаемыеТовары", Ложь, Ложь, ДанныеПечати.ЕстьПрослеживаемыеТовары);
		...
		Иначе
			...
			Если Не СтрокаБезРасхождений Тогда
				...
				Если ОперацияОблагаетсяНДССМежценовойРазницы Тогда
					...
					Если ИспользоватьПечатныеФормыСФ2_5 Тогда
						ОбластьСтрока.Параметры.СтоимостьБезНДСДоИзменения   = ?(ЗначениеСтавкиНДС = 0, 0, 100 * СтрокаТовары.СуммаНДСДо / ЗначениеСтавкиНДС);
						ОбластьСтрока.Параметры.СтоимостьБезНДСПослеИзменения   = ?(ЗначениеСтавкиНДС = 0, 0, 100 * СтрокаТовары.СуммаНДС / ЗначениеСтавкиНДС);
					Иначе 
						ОбластьСтрока.Параметры.СуммаБезНДСДо   = ?(ЗначениеСтавкиНДС = 0, 0, 100 * СтрокаТовары.СуммаНДСДо / ЗначениеСтавкиНДС);
						ОбластьСтрока.Параметры.СуммаБезНДС   = ?(ЗначениеСтавкиНДС = 0, 0, 100 * СтрокаТовары.СуммаНДС / ЗначениеСтавкиНДС);
					КонецЕсли;
