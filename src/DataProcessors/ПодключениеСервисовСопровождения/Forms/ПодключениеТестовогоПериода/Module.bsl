...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ОписательПодключения = Параметры.ОписательПодключения;
...
Процедура ПодключитьТестовыйПериод()
	...
	Иначе
		...
		Если ДанныеПодключения.КоличествоИтерацийПроверки >= 3 Тогда
			УстановитьВидимостьДоступность(СостояниеПодключения);
			Возврат;
		КонецЕсли;
		...
		ДанныеПодключения.КоличествоИтерацийПроверки = ДанныеПодключения.КоличествоИтерацийПроверки + 1;
...
&НаСервере
Процедура ЗаполнитьДанныеФормы(Знач ОписательПодключения = Неопределено)
...
Процедура НастроитьОтображениеФормы()
	...
	Если СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ОшибкаПодключения") Тогда
		Элементы.ДекорацияИнформацияОбОшибке.Заголовок = ИнформацияОбОшибке;
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		Элементы.ДекорацияНадписьПодключен.Заголовок = НСтр("ru = 'Сервис успешно подключен.'");
...
&НаСервере
Процедура УстановитьВидимостьДоступность(Знач СостояниеПодключения, Знач Подключение = Ложь)
