...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияСМаркетплейсамиСерверЛокализация.ДобавитьЗаполнитьУчетнуюЗапись(ЭтаФорма);
...
Процедура ДополнитьСтруктуруДействиямиРасчетаЦены(СтруктураДействий, ТекущаяСтрока)
	...
	ОписаниеОбъекта.Вставить("Дата", ТекущаяДатаСеанса);
...
Процедура ПеретащитьВКорзинуНаСервере(Данные)
	...
	РеквизитыОтбора = "Артикул, НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика,
		...
		|СрокПоставки, ВариантОбеспечения";
	Если ИспользоватьДатыОтгрузки Тогда
	...
	Для Каждого НоваяСтрока Из НовыеСтроки Цикл
		...
		Если СтруктураДействий.Свойство("ЗаполнитьПризнакНаличияНоменклатурыПродаваемойСовместно") Тогда
			СтруктураДействий.Удалить("ЗаполнитьПризнакНаличияНоменклатурыПродаваемойСовместно");
			СтруктураДействий.Удалить("ПересчитатьСуммуСУчетомПогрешностиОкругления");
		КонецЕсли;
		...
		ЗаполнитьВариантОбеспеченияИФлагНаСервере(ПараметрыСтроки);
	...
	Если СтрокиДляУстановкиСтатусаСерий.Количество()
		...
		ЗаполнитьСтатусУказанияСерииВСтроке(СтрокиДляУстановкиСтатусаСерий);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСтатусУказанияСерииВСтроке(Знач СтрокиДляУстановкиСтатусаСерий)

	Если ПараметрыУказанияСерий = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если ТипЗнч(СтрокиДляУстановкиСтатусаСерий) = Тип("Число") Тогда
		ТекущаяСтрока = Объект.Корзина.НайтиПоИдентификатору(СтрокиДляУстановкиСтатусаСерий);
		СтрокиДляУстановкиСтатусаСерий = Новый Массив;
		СтрокиДляУстановкиСтатусаСерий.Добавить(ТекущаяСтрока);
	КонецЕсли;
