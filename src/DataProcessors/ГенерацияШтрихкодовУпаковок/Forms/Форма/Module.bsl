...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РежимУказанияПараметровГенерации   = Параметры.РежимУказанияПараметровГенерации;
	РежимГрупповойГенерации            = Параметры.РежимГрупповойГенерации;
	
	Если РежимУказанияПараметровГенерации
		И Параметры.Свойство("СкрытьФлагЗапомнитьПараметрыГенерации") Тогда
		СкрытьФлагЗапомнитьПараметрыГенерации = Параметры.СкрытьФлагЗапомнитьПараметрыГенерации;
	КонецЕсли;
	
	Если РежимГрупповойГенерации Тогда
		ВидимостьШаблонаЭтикетки = Параметры.ВидимостьШаблонаЭтикетки;
		ШаблонЭтикетки           = Параметры.ШаблонЭтикетки;
		КоличествоШтрихкодов     = Параметры.КоличествоШтрихкодов;
	...
	МассивДоступныхТиповШК             = Новый Массив;

	Для Каждого ДоступныйТип Из Параметры.ДоступныеТипыШтрихкодовСтрокой Цикл
		МассивДоступныхТиповШК.Добавить(ДоступныйТип.Значение);
	КонецЦикла;
	
	Если МассивДоступныхТиповШК.Количество() = 0 Тогда
		МассивДоступныхТиповШК.Добавить("Code128Формат2");
		МассивДоступныхТиповШК.Добавить("Code128Формат3");
	КонецЕсли;
	...
	Если ВозвращатьАдресВременногоХранилища Тогда
		Если ЭтоАдресВременногоХранилища(Параметры.АдресПредыдущихШтрихкодов) Тогда
			...
			КонецЕсли;
			МассивДоступныхТиповШК = ПоследниеШтрихкоды.Получить("ПоследнийМассивДоступныхТиповШК");
		КонецЕсли;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТипУпаковки) И ЗначениеЗаполнено(Параметры.ТипУпаковки) Тогда
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТипШтрихкода) И ЗначениеЗаполнено(Параметры.ТипШтрихкода) Тогда
		ТипШтрихкода = Параметры.ТипШтрихкода;
		Если НЕ ЗначениеЗаполнено(ТипШтрихкодаСтрокой) Тогда
			ТипШтрихкодаСтрокой = ТипШтрихкодаСтрокойПоЗначениюПеречисления(ТипШтрихкода);
		КонецЕсли;
		Если ЗначениеЗаполнено(ТипШтрихкодаСтрокой) 
			И МассивДоступныхТиповШК.Найти(ТипШтрихкодаСтрокой) = Неопределено Тогда
			МассивДоступныхТиповШК.Добавить(ТипШтрихкодаСтрокой);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.Номенклатура) Тогда
		...
		Если Не РежимГрупповойГенерации Тогда
	...
	// Удаляем недоступные типы штрихкодов
	КоличествоТиповШтрихкода = Элементы.ТипШтрихкодаСтрокой.СписокВыбора.Количество();
	Для ОбратныйИндекс = 1 По КоличествоТиповШтрихкода Цикл
		ТипШКДляПоиска = Элементы.ТипШтрихкодаСтрокой.СписокВыбора[КоличествоТиповШтрихкода - ОбратныйИндекс].Значение;
		Если МассивДоступныхТиповШК.Найти(ТипШКДляПоиска) = Неопределено Тогда
			Элементы.ТипШтрихкодаСтрокой.СписокВыбора.Удалить(КоличествоТиповШтрихкода - ОбратныйИндекс);
		КонецЕсли;
	КонецЦикла;
...
Процедура Готово(Команда)
	...
	Если Не ПустаяСтрока(АдресПредыдущихШтрихкодов) Тогда
		...
		Если ТипШтрихкодаСтрокой = "GS1128" Тогда
			GTIN = Сред(Штрихкод,5,14);
			ШтрихкодыПоGTIN = ПредыдущиеШтрихкоды.Получить("GTIN");
			Если ШтрихкодыПоGTIN = Неопределено Тогда
				ШтрихкодыПоGTIN = Новый Соответствие;
			КонецЕсли;
			ОписаниеШтрихкодаGTIN = Новый Структура;
			ОписаниеШтрихкодаGTIN.Вставить("ТипУпаковки", ТипУпаковки);
			ОписаниеШтрихкодаGTIN.Вставить("ТипШтрихкода", ТипШтрихкода);
			ОписаниеШтрихкодаGTIN.Вставить("ЗначениеШтрихкода", Штрихкод);
			ОписаниеШтрихкодаGTIN.Вставить("ТипШтрихкодаСтрокой", ТипШтрихкодаСтрокой);
			Для каждого Строка Из ИдентификаторыПрименения Цикл
				Если Строка.ИмяИдентификатора = ВРЕГ("СерийныйНомер") Тогда
					ОписаниеШтрихкодаGTIN.Вставить("СерийныйНомер", Число(Строка.Значение));
				КонецЕсли;
			КонецЦикла;
			ШтрихкодыПоGTIN.Вставить(GTIN, Штрихкод);
			ПредыдущиеШтрихкоды.Вставить("GTIN", ШтрихкодыПоGTIN);
		КонецЕсли;
	...
	ВозвращаемыеПараметры.Вставить("ЗапомнитьПараметрыГенерации", ЗапомнитьПараметрыГенерации);
	
	Если РежимГрупповойГенерации Тогда
		ВозвращаемыеПараметры.Вставить("Номенклатура",          Номенклатура);
		ВозвращаемыеПараметры.Вставить("Характеристика",        Характеристика);
		ВозвращаемыеПараметры.Вставить("ШаблонЭтикетки",        ШаблонЭтикетки);
		ВозвращаемыеПараметры.Вставить("КоличествоШтрихкодов",  КоличествоШтрихкодов);
		ВозвращаемыеПараметры.Вставить("КоличествоЭкземпляров", КоличествоЭкземпляров);
	КонецЕсли;
	...
	Если ВозвращатьАдресВременногоХранилища
	   И ВладелецФормы <> Неопределено Тогда
		...
		ПоследнийМассивДоступныхТиповШК = Новый Массив;
		Для Каждого ЭлементСпискаВыбора Из Элементы.ТипШтрихкодаСтрокой.СписокВыбора Цикл
			ПоследнийМассивДоступныхТиповШК.Добавить(ЭлементСпискаВыбора.Значение);
		КонецЦикла;
		ПоследниеШтрихкоды.Вставить("ПоследнийМассивДоступныхТиповШК", ПоследнийМассивДоступныхТиповШК);
		...
		ИначеЕсли ТипШтрихкодаСтрокой = "GS1128"
			...
			КонецЦикла;
		КонецЕсли;
		Если ТипЗнч(ТекущееЗначениеНумератора) = Тип("Строка") Тогда
			ТекущееЗначениеНумератора = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ТекущееЗначениеНумератора);
		КонецЕсли;
		
		ПоследнееЗначениеНумератора = ПоследниеНомераПоТипамШтрихкодов.Получить(ТекущийКлючНумератора);
		Если ТипЗнч(ПоследнееЗначениеНумератора) = Тип("Строка") Тогда
			ПоследнееЗначениеНумератора = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ПоследнееЗначениеНумератора);
		КонецЕсли;
		...
		ПоследниеШтрихкоды.Вставить("ПоследнийКлючНумератора",          ТекущийКлючНумератора);
...
#Область ДополнительныеПараметрыИдентификаторов

	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИдентификаторыПримененияДополнительныйПараметрЗначение.Имя);
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИдентификаторыПримененияДополнительныйПараметрПредставление.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИдентификаторыПрименения.ДополнительныйПараметрИмя");
...
#Область ЗаполняемыеВнеГенератораИдентификаторы
	
	//310Х вес нетто: заполняется по факту содержимым (взвешивается на этапе наполнения)
	ВесУказываетсяПриСканировании = Ложь;
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияИСМПКлиентСерверПовтИсп");
		ВесУказываетсяПриСканировании = Модуль.НастройкиСканированияКодовМаркировки().ЗапрашиватьКоличествоМерногоТовара;
	КонецЕсли;
	
	Если ВесУказываетсяПриСканировании Тогда
	
		Элемент = УсловноеОформление.Элементы.Добавить();
		
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИдентификаторыПримененияЗначение.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИдентификаторыПрименения.ИмяИдентификатора");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НачинаетсяС;
		ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'МАССАНЕТТОВКГ'");
		
		Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);

	КонецЕсли;
	
	//37 количество вложенных единиц: заполняется по факту содержимым (считается на этапе наполнения)
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИдентификаторыПрименения.ИмяИдентификатора");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НачинаетсяС;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'КОЛИЧЕСТВОВЛОЖЕННЫХЕДИНИЦ'");
...
&НаКлиентеНаСервереБезКонтекста
Функция ТипШтрихкодаСтрокойПоЗначениюПеречисления(ТипШтрихкода)
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаГрупповаяГенерацияШтрихкодов.Видимость = Форма.РежимГрупповойГенерации;
	Элементы.ШаблонЭтикетки.Видимость                     = Форма.ВидимостьШаблонаЭтикетки;
	...
	ИначеЕсли Форма.ТипШтрихкодаСтрокой = "SSCC" Тогда
		...
		Если Форма.РежимУказанияПараметровГенерации Тогда
			Элементы.ФормаПечатьЭтикетокШтрихкода.Видимость = Ложь;
			Элементы.СерийныйНомерSSCC.Видимость            = Ложь;
			Элементы.ГруппаРеквизитыSSCCШтрихкод.Видимость  = Ложь;
			Элементы.ГруппаРеквизитыSSCCЗапомнитьПараметрыГенерации.Видимость = Не Форма.СкрытьФлагЗапомнитьПараметрыГенерации;
		КонецЕсли;
	...
	ИначеЕсли Форма.ТипШтрихкодаСтрокой = "GS1128"
		...
		Если Форма.РежимУказанияПараметровГенерации Тогда
			Элементы.ФормаПечатьЭтикетокШтрихкода.Видимость = Ложь;
			Элементы.ГруппаРеквизитыGS1Штрихкод.Видимость   = Ложь;
			Элементы.ГруппаРеквизитыGS1ЗапомнитьПараметрыГенерации.Видимость = Не Форма.СкрытьФлагЗапомнитьПараметрыГенерации;
		КонецЕсли;
...
Функция ПолучитьПоследниеЗаписанныеУникальныеЗначения(НастройкиЗаполнения, ПромежуточныеРезультаты)
		...
		Если НЕ РезультатЧтенияШтрихкода = Неопределено Тогда
			...
			ИначеЕсли ТипШтрихкодаСтрокой = "GS1128"
				...
				Для каждого Строка Из РезультатЧтенияШтрихкода Цикл
					Если Строка.ИмяИдентификатора = ВРЕГ("СерийныйНомер") Тогда
						ПоследнийНомерКэша = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Строка.Значение);
						Прервать;
					КонецЕсли;
				КонецЦикла; 
			КонецЕсли;
	...
	Если ПоследнийНомерКэша = Неопределено Тогда
		ПоследнийНомерКэша = 0;
	КонецЕсли;
	...
	Если ЭтоАдресВременногоХранилища(АдресПредыдущихШтрихкодов) Тогда
		...
		Если ПоследнийНомер = Неопределено Тогда
			ПоследнийНомер = 0;
		ИначеЕсли ТипЗнч(ПоследнийНомер) = Тип("Строка") Тогда
	...
	ИначеЕсли ТипШтрихкодаСтрокой = "GS1128"
		...
		|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
		|ГДЕ
		|	ШтрихкодыУпаковокТоваров.ТипШтрихкода В (&ТипыШтрихкода)
		...
		Если НастройкиЗаполнения.СохранятьТекущиеНастройкиИдентификаторовПрименения Тогда
			...
			Для каждого СтрокаТекущихЗначений Из ИдентификаторыПрименения Цикл
				...
				Иначе
					...
					Строка.Значение = Строка.ТипЗначения.ПривестиЗначение(СтрокаТекущихЗначений.Значение);
				КонецЕсли;
...
Функция НоваяСтрокаТаблицыИдентификаторов(ТаблицыИдентификаторов,
	...
	НоваяСтрока.Значение = НоваяСтрока.ТипЗначения.ПривестиЗначение(Значение);
	...
	Если ПромежуточныеРезультаты.Свойство("ШтрихкодДляКопированияНастроек")
	   И НЕ НастройкиЗаполнения.СохранятьТекущиеНастройкиИдентификаторовПрименения Тогда
		...
		Если НЕ НастройкиЗаполнения.СохранятьТекущиеНастройкиИдентификаторовПрименения Тогда
			...
			НомерПоПорядку = 1;
			УстанавливатьКоличествоВложенныхЕдиниц = НастройкиЗаполнения.УстановливатьКоличествоВложенныхЕдиниц;
			...
			Если НЕ Результат = Неопределено Тогда
				Для каждого СвойстваПараметраПрименения Из Результат Цикл
					...
					КонецЕсли;
					Если УстанавливатьКоличествоВложенныхЕдиниц
						И СтрокаИдентификаторов.ИмяИдентификатора = "GTIN01" Тогда
						УстанавливатьКоличествоВложенныхЕдиниц = Ложь;
					КонецЕсли;
				...
				Для каждого Строка Из ПорядокИдентификаторов Цикл
					Если Строка.ИмяИдентификатора <> ВРЕГ("НомерПартии")
						И Строка.ИмяИдентификатора <> ВРЕГ("СерийныйНомер")
						И ИзмененныеИдентификаторы.Найти(Строка.КлючИдентификатора) = Неопределено Тогда
...
Функция ШтрихкодыНоменклатуры(Отбор)
	...
	ЗаполнитьЗначенияСвойств(ТаблицаНоменклатуры.Добавить(), Отбор);
	
	Штрихкоды = Новый Массив;
	СоответствиеGTIN = Новый Соответствие;
	ШтрихкодированиеОбщегоНазначенияИС.ЗаполнитьПроверяемыеGTIN(ТаблицаНоменклатуры, Штрихкоды, СоответствиеGTIN, Ложь);
...
Функция ПолучитьИдентификаторыОрганизации(ТипШтрихкодаСтрокой, Знач ВключаяДату = Неопределено)
	...
	СписокИдентификаторовОрганизации = Модуль.ИдентификаторыОрганизации(ВключаяДату, ТолькоКодФСРАР);
...
Процедура ОбработатьПодборИдентификаторовПримененияGS1(ИзмененияИдентификаторов)
	...
	Для каждого КлючИдентификатора Из ПодобранныеИдентификаторы Цикл
		...
		Индекс = Неопределено;
		Для Каждого ЭлементДанных Из ИдентификаторыПрименения Цикл
			СвойстваИдентификатораДляПоиска = СвойстваИдентификаторов.Получить(ЭлементДанных.КлючИдентификатора);
			Если СвойстваИдентификатора.Индекс > СвойстваИдентификатораДляПоиска.Индекс Тогда
		...
		Если Индекс <> Неопределено Тогда
			НоваяСтрока = ИдентификаторыПрименения.Вставить(Индекс);
		Иначе
...
Функция ПроверкаКлиент()
	...
	Если ВидимостьШаблонаЭтикетки
		И Не ЗначениеЗаполнено(ШаблонЭтикетки) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Шаблон этикетки""'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ШаблонЭтикетки",,Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ТипУпаковки) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Тип упаковки""'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ТипУпаковки",,Отказ);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТипШтрихкодаСтрокой) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Тип штрихкода""'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ТипШтрихкодаСтрокой",,Отказ);
	КонецЕсли;
	Если ТипШтрихкодаСтрокой = "SSCC" Тогда
		Если НЕ ЗначениеЗаполнено(ПрефиксКомпанииGS1) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Префикс компании GS1""'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ПрефиксКомпанииGS1",,Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СерийныйНомерSSCC) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Серийный номер""'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"СерийныйНомерSSCC",,Отказ);
		КонецЕсли;
	ИначеЕсли ТипШтрихкодаСтрокой = "Code128Формат1" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДатаМаркировки) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ДатаМаркировки",,Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(НомерПоПорядку) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Номер по порядку""'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"НомерПоПорядку",,Отказ);
		КонецЕсли;
	ИначеЕсли ТипШтрихкодаСтрокой = "Code128Формат1" Тогда
		Если НЕ ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Идентификатор организации""'");
		...
		Если НЕ ЗначениеЗаполнено(ТипЛогистическойЕдиницы) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ТипЛогистическойЕдиницы",,Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ГодГенерацииШтрихкода) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Год генерации штрихкода""'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ГодГенерацииШтрихкода",,Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(НомерПоПорядку) Тогда
	...
	ИначеЕсли ТипШтрихкодаСтрокой = "GS1DataBarExpandedStacked"
		...
		Если ИдентификаторыПрименения.Количество() = 0 Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ИдентификаторыПрименения",,Отказ);
		Иначе
			ШаблонИмениПоля = "ИдентификаторыПрименения[%1].Значение";
			Индекс = 0;
			Для каждого Строка Из ИдентификаторыПрименения Цикл
				Если НЕ ЗначениеЗаполнено(Строка.Значение)
						И Строка.ИмяИдентификатора <> ВРЕГ("МассаНеттоВКг")
						И Строка.ИмяИдентификатора <> ВРег("КоличествоВложенныхЕдиниц") Тогда
...
Процедура ТипУпаковкиПриИзмененииСервер()
	...
	Если КоличествоВложенныхЕдиниц > 0 Тогда
		НастройкиЗаполнения.УстановливатьКоличествоВложенныхЕдиниц = Истина;
		НастройкиЗаполнения.КоличествоВложенныхЕдиницДляУстановки = КоличествоВложенныхЕдиниц;
	КонецЕсли;
...
Процедура ТипШтрихкодаСтрокойПриИзмененииСервер()
	...
	ТипШтрихкода = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ТипШтрихкодаПоСтроке(ТипШтрихкодаСтрокой);
...
Процедура УстановитьВесТоваровСервер(Вес, ДополнительныеПараметры)
	...
	Если СтрокаИдентификатора = Неопределено Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "ИдентификаторыПрименения");
		Возврат;
	КонецЕсли;
