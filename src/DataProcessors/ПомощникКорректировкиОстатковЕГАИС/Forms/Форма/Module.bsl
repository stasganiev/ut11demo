...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ИнтервалОжиданияМарокПоСправке2 = 16;
...
//#Область Шаг3_Марки
	...
	ИначеЕсли СтрНачинаетсяС(Действие, ВРег("МаркиВыбратьСправки")) Тогда
		...
		ОткрытьФорму("Обработка.ПомощникКорректировкиОстатковЕГАИС.Форма.ВыборСправки2", МаркиКВыборуСправок, ЭтотОбъект);
...
#Область Шаг1_ОбработчикиСобытий
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПомарочныеОстатки
...
Процедура ВыгрузитьВЕГАИС(Команда)
	...
	ОбменДаннымиЕГАИСКлиент.ВыполнитьОбмен(Объект.ОрганизацияЕГАИС,
...
Процедура СтраницаНачальныеДанные_ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОрганизацияЕГАИС) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ОрганизацияЕГАИС",, Отказ);
	Иначе
	...
	Если ВариантКорректировки = 0 Тогда
		Если НЕ ЗначениеЗаполнено(Объект.ОстаткиВРегистре1) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ОстаткиВРегистре1",, Отказ);
		КонецЕсли;
	КонецЕсли;
...
Функция СтраницаПроверкаДанных_ПриОткрытии(ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ЭтоПереходДалее Тогда
		...
		Если Элементы.ПанельНеПодтвержденныеДокументы.ТекущаяСтраница = Элементы.СтраницаНеПодтвержденныеДокументы_НетДокументов
			...
			И Объект.ОстаткиВРегистре1.Пустая() Тогда
			ЗапроситьОстаткиЕГАИС();
		КонецЕсли;
	КонецЕсли;
...
Процедура СтраницаПроверкаМарок_ПриОткрытии(ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ПомарочныеОстатки.Количество() = 0 И ЭтоПереходДалее Тогда
		ПропуститьСтраницу = Истина;
	КонецЕсли;
...
Функция ДатаОстатковЕГАИС()
	...
	Если ЗначениеЗаполнено(Объект.ОстаткиВРегистре2) Тогда
...
Функция ТекстПоследнейОшибкиОбмена(ДокументСсылка)
	...
	Возврат ОбменДаннымиЕГАИСВызовСервера.ТекстОшибкиИзПротокола(ДокументСсылка);
...
Процедура НайтиДокументыТребующиеОформления()
	...
	Запрос.УстановитьПараметр("НеОформлен", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("ОснованиеСтатусыОформленияДокументовЕГАИС"));
...
Процедура МаркиУстановитьОписаниеДействий()
	...
	ИначеЕсли ТекущиеДанные.Проверено = 2 Тогда
		...
		ОписаниеСтрокиИДействий.Добавить(
			НСтр("ru = 'По данным ЕГАИС есть остатки поштучной алкогольной продукции, соответствующие номенклатуре выбранной строки.
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ОткрытьФорму("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ПроверкаИПодбор", ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,
...
Функция СканированиеПродукцииПослеЗакрытияНаСервере(АдресВременногоХранилища)
	...
	Попытка
		НачатьТранзакцию();
		ШтрихкодированиеЕГАИС.СоздатьШтрихкодыУпаковок(
			ДеревоУпаковок,
			ШтрихкодыУпаковок);
...
&НаСервере
Функция ЗаполнитьТоварыИАкцизныеМарки(ДеревоУпаковок)
...
Процедура ПослеПроверкиАкцизныхМарокУТМ(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОтветОбработан Тогда
		...
		Если Объект.МаксимумМарокКПроверкеВУТМ = 0 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'УТМ не отвечает на запрос статуса марок.'"));
			МаркиЗаполнитьОписаниеИФильтры(ПомарочныеОстаткиТекущийФильтр);
			Возврат;
		КонецЕсли;
...
Процедура РазобратьОтветПроверкиАкцизныхМарокУТМ(ЭлементОтвета, ДополнительныеПараметры)
	...
	ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(ЭлементОтвета.ВходящееСообщение);
...
Процедура ОстаткиЗапросМарок(ПоТекущейСтроке)
	...
	ЗначенияНастройкиЗапросаМарок = "АлкогольнаяПродукция, Номенклатура, Характеристика,Серия";
	Если ПоТекущейСтроке Тогда
		...
		Если ЗначениеЗаполнено(ДанныеСтроки.Справка2)
			И ДанныеСтроки.Расхождения = 2 Тогда
				ПоляСоответствия = Новый Структура(ЗначенияНастройкиЗапросаМарок);
	...
	Если ЗапроситьОстаткиПоСправкам.Количество() Тогда
		Если ТаймерЗапросов <= 0 Тогда
			ТаймерЗапросов = КоличествоМинутДоОчередногоЗапросаОстатковСправок(Объект.ИнтервалОбменаУТМ * Объект.ИнтервалОжиданияМарокПоСправке2);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция КоличествоМинутДоОчередногоЗапросаОстатковСправок(ИнтервалМеждуЗапросамиВСекундах)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Прошло = ТекущаяУниверсальнаяДата() - Выборка.ДатаМодификацииУниверсальная;
		Возврат Макс(0, Цел((ИнтервалМеждуЗапросамиВСекундах-Прошло)/60) + 1);
	КонецЕсли;
...
Процедура ПодготовитьКПередачеЗапросОстатковПоСправке2()
	...
	ТаймерЗапросов = Объект.ИнтервалОжиданияМарокПоСправке2;
...
Процедура ЗапросОстатковПоСправкамОбработчикОжидания()
	...
	Если Не АвтоматическийОбменНастроен Тогда
		ОбменДаннымиЕГАИСКлиент.ВыполнитьОбмен(
			Объект.ОрганизацияЕГАИС,
			Новый ОписаниеОповещения("ПроверкаПолученияОтчетаЕГАИС", ЭтотОбъект),
			УникальныйИдентификатор);
	ИначеЕсли ТаймерЗапросов <= 0 Или ТаймерЗапросов = Объект.ИнтервалОжиданияМарокПоСправке2 Тогда
		Если ПараметрыПриложения["ЕГАИС.ДанныеСеансаОбмена"] <> Неопределено Тогда
			ОбменДаннымиЕГАИСКлиент.ВыполнитьОбмен(
				...
				УникальныйИдентификатор);
		Иначе
			ПодключитьОбработчикОжидания("ЗапросОстатковПоСправкамОбработчикОжидания", 10, Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнтервальныйЗапросМарокПоСправке2()
	...
	Если ОтчетОстаткиПоСправкам.Количество() Тогда
		...
		//Таймер на получение ответа от ЕГАИС, двухминутными интервалами
		...
		//Ответ получен. Выдерживаем интервал.
		ИначеЕсли ТаймерЗапросов > 0 Тогда
...
Процедура ОстаткиЗаполнитьНесопоставленнуюПродукцию()
	...
	|"
	+
	ИнтеграцияЕГАИС.ТекстЗапросаВТОбъемДАЛЕдиницыНоменклатуры(,,Истина)
	+
	"
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЗ.Номенклатура КАК Номенклатура,
	|	ТЗ.Характеристика КАК Характеристика,
	|	ТЗ.ОбъемДАЛ КАК ОбъемДАЛ
	|ИЗ ВТОбъемДАЛ КАК ТЗ";
	
	КоэффициентыПересчета = ЗапросКоэффициентовПересчета.Выполнить().Выгрузить();
	КоэффициентыПересчета.Индексы.Добавить("Номенклатура,Характеристика");
	...
	Для Каждого СтрокаСопоставления Из ТаблицаПродукции Цикл
		...
		Если СтрокаСопоставления.Сопоставлено Тогда
			...
			Если СтрокаСопоставления.ПродаетсяВРозлив Тогда
				СтрокаКоэффициентПересчета = КоэффициентыПересчета.НайтиСтроки(Новый Структура("Номенклатура,Характеристика", СтрокаСопоставления.Номенклатура, СтрокаСопоставления.Характеристика));
				Если СтрокаКоэффициентПересчета.Количество() Тогда
					ОбъемДАЛ = СтрокаКоэффициентПересчета[0].ОбъемДАЛ;
				КонецЕсли;
			КонецЕсли;
...
Функция ПоместитьДляСопоставления(Номенклатура, Характеристика, АлкогольнаяПродукция, УникальныйИдентификатор)
	...
	Таблица.Колонки.Добавить("ИдентификаторУпаковки",Метаданные.ОпределяемыеТипы.СтрокаЕГАИС.Тип);
...
Функция ПоместитьНесопоставленныеТоварыВоВремененноеХранилище()
	...
	Товары.Колонки.Добавить("ИдентификаторУпаковки",Метаданные.ОпределяемыеТипы.СтрокаЕГАИС.Тип);
...
Процедура ОстаткиУстановитьОписаниеСтроки()
	...
	ИначеЕсли ТекущиеДанные.Расхождения = 2 Тогда
		...
		ОписаниеДействий.Добавить(
			НСтр("ru = 'Остаток ЕГАИС для поштучной справки №2 отличается от количества акцизных марок в Регистре №3.
		...
		Если ОтчетОстаткиПоСправкам.Количество() > 0 Или ТаймерЗапросов > 0 Тогда
			...
			ОписаниеДействий.Добавить(
				СтрШаблон(
					НСтр("ru = 'Запрос будет добавлен в очередь (Разрешено отправлять в ЕГАИС не чаще чем 1 раз в %1 минут).'"),
					Объект.ИнтервалОбменаУТМ * Объект.ИнтервалОжиданияМарокПоСправке2 / 60));
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			ОписаниеДействий.Добавить(НСтр("ru = 'Нет доступных сопоставленных остатков для распределения.'"));
...
Функция ЗагрузитьПолученныеРанееОтчетыОстаткиМарок()
	...
	Пока Выборка.Следующий() Цикл
		...
		НайденныеСтроки = Объект.Остатки.НайтиСтроки(Новый Структура("Справка2, Расхождения", Выборка.Справка2, 2));
...
Процедура РассчитатьКоличествоКорректировки()
	...
	|"
	+
	ИнтеграцияЕГАИС.ТекстЗапросаВТКоэффициентыПересчетаВЕдиницыЕГАИС("ТаблицаОстатков",,Истина)
	+
	"
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Номенклатура,
	...
	|	ВТКоэффициентыПересчетаВЕдиницыЕГАИС.Коэффициент КАК ОбъемДАЛ
	|ИЗ
	|	ТаблицаОстатков КАК ТаблицаОстатков
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКоэффициентыПересчетаВЕдиницыЕГАИС КАК ВТКоэффициентыПересчетаВЕдиницыЕГАИС
	|		ПО ВТКоэффициентыПересчетаВЕдиницыЕГАИС.Номенклатура = ТаблицаОстатков.Номенклатура
	|		И ВТКоэффициентыПересчетаВЕдиницыЕГАИС.Характеристика = ТаблицаОстатков.Характеристика
	|		И ВТКоэффициентыПересчетаВЕдиницыЕГАИС.Серия = ТаблицаОстатков.Серия
	|		И ВТКоэффициентыПересчетаВЕдиницыЕГАИС.АлкогольнаяПродукция = ТаблицаОстатков.АлкогольнаяПродукция
	|ГДЕ
	...
	Для Каждого ДокументСсылка Из МассивДокументов Цикл
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Проведен") Тогда
			...
			Если СтатусОбработки = Перечисления.СтатусыОбработкиАктаПостановкиНаБалансЕГАИС.Черновик
				...
				ОбменДаннымиЕГАИСВызовСервера.ПодготовитьКПередаче(ДокументСсылка, Перечисления.ДальнейшиеДействияПоВзаимодействиюЕГАИС.ПередайтеДанные);
...
Процедура ОпределитьТранспортныйМодуль()
	...
	ДоступныеМодули = ОбменДаннымиЕГАИСВызовСервера.НастройкиОбменаЕГАИС(ОрганизацияЕГАИС);
