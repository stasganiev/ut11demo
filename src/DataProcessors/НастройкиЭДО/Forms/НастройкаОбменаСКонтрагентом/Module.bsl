...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Организация = Контрагент Тогда
		Заголовок = НСтр("ru='Настройки отправки'");
		Элементы.УчетнаяЗаписьАбонента.Видимость = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ПринятьПриглашение()
	
	ПриглашениеНаИдентификатор = СинхронизацияЭДОКлиент.НовоеПриглашениеНаИдентификатор();
	ПриглашениеНаИдентификатор.ИдентификаторОрганизации = ИдентификаторОтправителя;
	ПриглашениеНаИдентификатор.ИдентификаторКонтрагента = ИдентификаторПолучателя;
	ПриглашениеНаИдентификатор.Организация = Организация;
	ПриглашениеНаИдентификатор.Контрагент = Контрагент;

	ПриглашенияЭДОКлиент.АктуализироватьКлючПриглашенияПоНатуральнымКлючам(ПриглашениеНаИдентификатор);
	
	Если ЗначениеЗаполнено(ПриглашениеНаИдентификатор.КлючПриглашения) Тогда
		ВходящееПриглашение = СинхронизацияЭДОКлиент.НовоеВходящееПриглашение();
		ЗаполнитьЗначенияСвойств(ВходящееПриглашение, ПриглашениеНаИдентификатор, , "Вид");

		ВходящееПриглашение = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВходящееПриглашение);

		Оповещение = Новый ОписаниеОповещения("ПослеПринятияПриглашения", ЭтотОбъект);
		СинхронизацияЭДОКлиент.ПринятьПриглашения(ВходящееПриглашение, ЭтотОбъект, Оповещение);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ПослеПринятияПриглашения(РезультатПринятия, ДополнительныеПараметры) Экспорт
	ЗавершитьРаботуМастера(РезультатПринятия);
КонецПроцедуры
...
Процедура Готово(Команда)
	...
	Если ТребуетсяОтправкаПриглашения(ЭтотОбъект) Тогда
...
Процедура СоздатьНастройкиОтправкиЭДО(Организация, Контрагент, Договор, ИдентификаторОтправителя, ИдентификаторПолучателя, Отказ);
	...
	НастройкиЭДО.СоздатьНастройкиОтраженияВУчете(Организация, Контрагент, "", "");
...
Процедура ОбновитьЭтапыМастераНастройкиПодключения(Форма)
	...
	Если Не Форма.ПредварительнаяНастройка
		...
		Элементы.УчетнаяЗаписьАбонента.Видимость = Не Форма.СкрыватьНастройкиТранспорта И Форма.Организация <> Форма.Контрагент;
	...
	Иначе
		...
		Элементы.УчетнаяЗаписьАбонента.Видимость = Форма.ПроверкаИдентификаторАбонента
			И Форма.ПредварительнаяНастройка И Форма.ПроверкаРегистрацияУОператора = "Выполнено"
			И Форма.Организация <> Форма.Контрагент;
	...
	Элементы.ФормаПодключить.Доступность = (ВсеГотовоКУстановкеСвязи Или Не ТребуетсяОтправкаПриглашения(Форма))
		И (Форма.ПроверкаВозможностьРоуминга = "Выполнено"
			Или Форма.ПроверкаВозможностьРоуминга = "Внимание"
			Или Форма.Организация = Форма.Контрагент)
...
Процедура ОбновитьДанныеПриглашенияКонтрагента(Форма, ПараметрыПриглашения = Неопределено)
	...
	Форма.СтатусПриглашения                = ПараметрыПриглашения.СтатусПриглашения;
	...
	Если ЗначениеЗаполнено(ПараметрыПриглашения.СписокИдентификаторовКонтрагента) Тогда
		СписокВыбора.Очистить();
...
Функция ТребуетсяОтправкаПриглашения(Форма)
	...
	Если Форма.Организация = Форма.Контрагент Тогда
		ТребуетсяОтправка = Ложь;
	КонецЕсли;
