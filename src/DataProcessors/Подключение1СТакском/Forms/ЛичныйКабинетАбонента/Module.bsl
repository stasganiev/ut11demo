...
Процедура ЛичныйКабинетПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Попытка
		ПродвижениеЭлемента = ДанныеАктивногоЭлемента.target;
	Исключение
		ПродвижениеЭлемента = Неопределено;
	КонецПопытки;
	...
	Если ПродвижениеЭлемента <> Неопределено Тогда 
		Если НРег(СокрЛП(ПродвижениеЭлемента)) = "_blank" Тогда 
