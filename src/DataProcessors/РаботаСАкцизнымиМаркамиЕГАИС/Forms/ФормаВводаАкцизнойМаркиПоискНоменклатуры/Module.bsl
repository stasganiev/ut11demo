...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоРегистрацииШтрихкодовНоменклатурыДоступно = ИнтеграцияИС.ПравоРегистрацииШтрихкодовНоменклатуры();
	
	СтандартныеПодсистемыСервер.СброситьРазмерыИПоложениеОкна(ЭтотОбъект);
	ПоказатьИнформациюОТребуемыхДействиях();
	
	ОбщегоНазначенияСобытияФормИСПереопределяемый.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект, Перечисления.ВидыПродукцииИС.Алкогольная, "Номенклатура");
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Не Элементы.ОтсканироватьШК.Видимость Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗавершитьРедактирование(Команда)
	...
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаНеТребуетсяУказаниеДополнительныхДанных Тогда
		...
		Если Не ЗначениеЗаполнено(Штрихкодноменклатуры) Тогда
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Номенклатура)
			И КоличествоВПотребительскойУпаковке <> 0 Тогда
			Результат.Вставить("Коэффициент", КоличествоВПотребительскойУпаковке);
		КонецЕсли;
		...
		Если ЗапомнитьВыбор Тогда
			...
			Если ЗначениеЗаполнено(Номенклатура)
				И КоличествоВПотребительскойУпаковке <> 0 Тогда
				СтруктурыДанных.ДанныеВыбора.Вставить("Коэффициент", КоличествоВПотребительскойУпаковке);
			КонецЕсли;
			СтруктурыДанных.ЗапомнитьВыбор              = Истина;
			
			Результат.ДополнительныеПараметры = СтруктурыДанных;
...
Процедура ПоказатьИнформациюОТребуемыхДействиях()
	...
	Если ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417 Тогда
		...
		ПоказатьИнформациюОТребуемыхДействияхPDF417();
		
	ИначеЕсли ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix Тогда
		
		ПоказатьИнформациюОТребуемыхДействияхDataMatrix();
		
	Иначе
		
		ПоказатьИнформациюОТребуемыхДействияхДляПартионнойПродукции();
...
&НаСервере
Процедура ПоказатьИнформациюОТребуемыхДействияхPDF417()
...
Процедура ПоказатьНеоднозначноеСоответствиеНетВДанныхОснования()
	...
	Элементы.ДекорацияОшибкаДобавленияАлкогольнойПродукции.Заголовок = Новый ФорматированнаяСтрока(
		Префикс, " ",
		Новый ФорматированнаяСтрока(
			ШтрихкодированиеЕГАИС.ПредставлениеШтрихкода(Штрихкод, ТипШтрихкода, Перечисления.ВидыУпаковокИС.Потребительская),
			Новый Шрифт(,,,,Истина),
			ЦветаСтиля.ЦветГиперссылкиГосИС,,
...
&НаСервере
Процедура ПоказатьИнформациюОТребуемыхДействияхDataMatrix()
...
#Область ФормированиеНачальнойСтраницыПартионнаяПродукция

&НаСервере
Процедура ПоказатьИнформациюОТребуемыхДействияхДляПартионнойПродукции()
	
	Если КонтролироватьПоДокументуОснованию Тогда
		
		Если ДанныеДокументаОснования.Количество() = 0 Тогда
			
			ПоказатьНетПартионнойАлкогольнойПродукцииВОсновании();
			Возврат;
			
		Иначе
			
			ПоказатьТребуетсяИдентификацияПартионнойАлкогольнойПродукцииСКонтролем();
			
		КонецЕсли;
		
	Иначе
		
		ПоказатьТребуетсяИдентификацияПартионнойАлкогольнойПродукцииБезКонтроля();
		
	КонецЕсли;
	
	// Так как номенклатура и характеристика ранее были определены по штрихкоду,
	// то их выбор заблокирован.
	Элементы.Номенклатура.ТолькоПросмотр   = Истина;
	Элементы.Характеристика.ТолькоПросмотр = Истина;
	НоменклатураЗаблокирована = Истина;
...
&НаСервере
Процедура ПоказатьНетПартионнойАлкогольнойПродукцииВОсновании()
	...
	Префикс = НСтр("ru = 'Невозможно добавить алкогольную продукцию со штрихкодом'");
...
&НаСервере
Процедура ПоказатьТребуетсяИдентификацияПартионнойАлкогольнойПродукцииСКонтролем()
	...
	ВывестиТекстИдентификацияПартионнойАлкогольнойПродукции();
	
	Элементы.ОтсканироватьШК.Видимость = Ложь;
	ВывестиИнформациюОВыбореАлкогольнойПродукции();

КонецПроцедуры

&НаСервере
Процедура ПоказатьТребуетсяИдентификацияПартионнойАлкогольнойПродукцииБезКонтроля()
...
&НаСервере
Процедура ВывестиТекстИдентификацияПартионнойАлкогольнойПродукции()
	...
	МассивСтрок.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Партионная алкогольная продукция %1'"),
				Строка(ТипШтрихкода)),,
			ЦветаСтиля.ПоясняющийТекст));
	МассивСтрок.Добавить(" ");
	МассивСтрок.Добавить(
...
&НаСервере
Процедура ВывестиИнформациюОВыбореАлкогольнойПродукции()
	
	ТекстУкажитеДополнительныеДанные = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Укажите данные алкогольной продукции вручную.'"),,
...
Процедура ЗаполнитьСписокВыбораАлкогольнойПродукцииПоНоменклатуре()
	...
	Если КонтролироватьПоДокументуОснованию Тогда
		...
		Элементы.АлкогольнаяПродукция.РежимВыбораИзСписка = СписокВыбора.Количество() > 0;
...
Процедура ПриИзмененииНоменклатуры()
	...
	КоличествоВПотребительскойУпаковке = КоличествоВПотребительскойУпаковке(Номенклатура);
...
Процедура ПриИзмененииАлкогольнойПродукции()
	...
	Иначе
		...
		Если НоменклатураЗаблокирована Тогда
			Элементы.Номенклатура.ТолькоПросмотр = Истина;
		Иначе
...
&НаСервереБезКонтекста
Функция КоличествоВПотребительскойУпаковке(Номенклатура)
	
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		ОписаниеНоменклатуры = ОбщегоНазначенияИС.ОписаниеНоменклатуры(Номенклатура);
		Результат = ОписаниеНоменклатуры[Номенклатура].КоличествоВПотребительскойУпаковке;
	Иначе
		Результат = 0;
	КонецЕсли;
...
Процедура НоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не Элементы.Номенклатура.РежимВыбораИзСписка Тогда
		...
		СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
			...
			СтандартнаяОбработка,, ПараметрыОтбора);
