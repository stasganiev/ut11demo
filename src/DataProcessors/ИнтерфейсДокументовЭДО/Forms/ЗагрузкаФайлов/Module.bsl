...
Асинх Процедура ФайлыПредставлениеПолучателяПриИзменении(Элемент)
	...
	НайденныеСтроки = СведенияПолучателей.НайтиСтроки(
		Новый Структура("ПредставлениеПолучателя", ТекущиеДанные.ПредставлениеПолучателя));
	
	Если НайденныеСтроки.Количество() = 0 Тогда
		Для Каждого Строка Из СведенияПолучателей Цикл
			Если СтрНайти(НРег(Строка.ПредставлениеПолучателя), НРег(ТекущиеДанные.ПредставлениеПолучателя)) > 0 Тогда
	...
	ТекущиеДанные.ПредставлениеПолучателя = НайденныеСтроки[0].ПредставлениеПолучателя;
...
Процедура ОтправительПриИзмененииСервер(ВыбранноеЗначение)
	...
	НайденныеСтроки = СведенияОтправителей.НайтиСтроки(Новый Структура("ПредставлениеОтправителя", ВыбранноеЗначение));
	Если НайденныеСтроки.Количество() = 0 Тогда
		Для Каждого Строка Из СведенияОтправителей Цикл
			Если СтрНайти(НРег(Строка.ПредставлениеОтправителя), НРег(ВыбранноеЗначение)) > 0 Тогда
				НайденныеСтроки.Добавить(Строка);
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
&НаКлиенте
Процедура ОбновитьПредставлениеФайла() Экспорт
...
Процедура ПодготовитьПредставлениеФайла(ИдентификаторСтроки)
	...
	КартинкаФайла = ИнтеграцияБСПБЭДКлиент.ИндексПиктограммыФайла(ОписаниеФайла.Расширение);
...
&НаКлиенте
Процедура СформироватьПредставлениеPDFФайла() Экспорт
...
Процедура ЗаполнитьСписокВыборкаОтправителя()
	...
	Для Каждого СтрокаСведений Из СведенияОтправителей Цикл
		ШаблонТекста = "%1 (%2)";
		ПредставлениеОтправителя = СтрШаблон(ШаблонТекста, СтрокаСведений.Отправитель, СтрокаСведений.ИдентификаторОтправителя);
		
		СтрокаСведений.ПредставлениеОтправителя = ПредставлениеОтправителя;
		Элементы.Отправитель.СписокВыбора.Добавить(ПредставлениеОтправителя);
	КонецЦикла;
...
Процедура ЗаполнитьСписокВыборкаПолучетелей()
	...
	Для Каждого СтрокаСведений Из СведенияПолучателей Цикл
		ПредставлениеПолучателя = ПредставлениеПолучателя(
				СтрокаСведений.ПредставлениеПолучателя,
				СтрокаСведений.ИдентификаторПолучателя,
				СтрокаСведений.ПредставлениеДоговор);
		
		СтрокаСведений.ПредставлениеПолучателя = ПредставлениеПолучателя;
		Элементы.ФайлыПредставлениеПолучателя.СписокВыбора.Добавить(ПредставлениеПолучателя);
	КонецЦикла;
...
&НаКлиенте
Процедура ПолучательНачалоВыбораЗавершить(Получатель, ДополнительныеПараметры) Экспорт
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекущиеДанные.Получатель = Получатель;
...
Процедура ДобавитьПолучателейВСписокФайлов(Получатели)
	...
	Для Каждого ДанныеПолучателя Из Получатели Цикл
		...
		Если ПолучательПрисутствуетВФайлах(ПредставлениеПолучателя)
