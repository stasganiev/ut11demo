...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "УдалитьИзСпискаДокументовДляФормированияОтветногоТитула" Тогда 
		Отбор = Новый Структура("ЭлектронныйДокумент", Параметр);
		НайденныеСтроки = СписокДокументовДляФормированияОтветногоТитула.НайтиСтроки(Отбор);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			СписокДокументовДляФормированияОтветногоТитула.Удалить(НайденнаяСтрока);
		КонецЦикла;
		Если Не СписокДокументовДляФормированияОтветногоТитула.Количество() Тогда
			Закрыть();
		КонецЕсли;
	КонецЕсли;
...
Процедура МаршрутПодписанияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтрокаТаблицы = СтрокаТаблицыСпискаДокументовКОтправке();
	...
	МаршрутыПодписанияБЭДКлиент.ВыбратьМаршрутПодписания(Отбор, Маршрут, УникальныйИдентификатор, Оповещение); 
...
Процедура СписокПечатныхФормПометкаПриИзменении(Элемент)
	...
	ИдентификаторПечатнойФормы = ТекущиеДанные.Значение;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокЗапрещенныхКФормированию
...
Процедура СписокЗапрещенныхКФормированиюВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущиеДанные.ЭтоПрямойОбмен Тогда
		
		ПараметрыФормы = НастройкиОтправкиЭДОКлиент.НовыеПараметрыФормыНастройкиПрямогоОбмена();
		ПараметрыФормы.Организация = ТекущиеДанные.Отправитель;
		ПараметрыФормы.Контрагент  = ТекущиеДанные.Получатель;
		...
		Для Каждого СтрокаТЧ Из ДокументыКОтправке Цикл
			...
			Если СтрокаТЧ.ФормированиеЗапрещено Тогда
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					ТекущаяСтрока.ЭтоПрямойОбмен = СтрокаТЧ.ЭтоПрямойОбмен;
...
Процедура СформироватьЗаголовокКомандыПодписать(Форма)
	...
	Если Форма.ПодписатьОтправить Тогда
		...
		Иначе
			Если КоличествоВыбрано > 0 Тогда
...
Процедура ЗаполнитьДанныеОтправляемогоДокумента(СтрокаТаблицы, ОбъектУчета, ОписаниеОбъектаУчета, ВидДокумента)
	...
	Иначе 
		...
		СтрокаТаблицы.НастройкиСозданы         = Истина;
		СтрокаТаблицы.ЭтоПрямойОбмен           = СинхронизацияЭДО.ЭтоПрямойОбмен(НастройкиОтправки.СпособОбмена);
...
&НаКлиенте
Функция СтрокаТаблицыСпискаДокументовКОтправке()
	...
	Если СписокДокументов.Количество() Тогда
		...
		КонецЕсли;
	ИначеЕсли ДокументыКОтправке.Количество() Тогда
		СтрокаТаблицы = ДокументыКОтправке[0];
	Иначе
		СтрокаТаблицы = Неопределено;
	КонецЕсли;
