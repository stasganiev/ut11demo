...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоОблачныйЭДО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоОблачныйЭДО", Ложь);
	
	Если ЭтоОблачныйЭДО Тогда
		ПолучитьСтатусыЭлектронныхДокументовОблачногоЭДО(АдресДанных);
	Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	ОжидатьПолученияСтатусовЭлектронныхДокументов();
...
#Область ЗаполнениеСтатусовПоДаннымОблачногоЭДО

&НаСервере
Процедура ПолучитьСтатусыЭлектронныхДокументовОблачногоЭДО(АдресДанных = "")
	
	СтатусыДокументов.ПолучитьЭлементы().Очистить();
	
	Если ЗначениеЗаполнено(АдресДанных) Тогда
		ОписаниеОбъектаУчета = ПолучитьИзВременногоХранилища(АдресДанных); // см. ИнтеграцияЭДО.ОписаниеОбъектаУчета
		УдалитьИзВременногоХранилища(АдресДанных);
	Иначе
		ОписаниеОбъектаУчета = ИнтеграцияЭДО.ОписаниеОбъектаУчета(ОбъектУчета);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОписаниеОбъектаУчета) Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектУчета = ОписаниеОбъектаУчета[0].ОбъектУчета;
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ОперацияПолученияСтатусов = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ИнтеграцияОблачногоЭДО.СтатусыЭлектронныхДокументов", ОписаниеОбъектаУчета, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ОжидатьПолученияСтатусовЭлектронныхДокументов()
	
	УстановитьСтраницуФормы();
	
	ДлительнаяОперация = ОперацияПолученияСтатусов; // см. ДлительныеОперации.ВыполнитьФункцию
	
	Если ДлительнаяОперация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьСтатусыЭлектронныхДокументовПослеПолучения", ЭтотОбъект);
...
&НаКлиенте
Процедура ЗаполнитьСтатусыЭлектронныхДокументовПослеПолучения(ДлительнаяОперация, Контекст) Экспорт
	
	Если ДлительнаяОперация <> Неопределено
		И ДлительнаяОперация.Статус = "Выполнено" Тогда
		ЗаполнитьСтатусыЭлектронныхДокументов(ДлительнаяОперация.АдресРезультата);
	КонецЕсли;
	
	ОперацияПолученияСтатусов = Неопределено;
...
&НаКлиенте
Процедура УстановитьСтраницуФормы()
	
	Если ОперацияПолученияСтатусов <> Неопределено Тогда
		ДлительнаяОперация = ОперацияПолученияСтатусов; // см. ДлительныеОперации.ВыполнитьФункцию
	Иначе
	...
	Иначе
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаСтатусыДокументов;
	КонецЕсли;
