...
Процедура ВходящиеЭлектронныеДокументыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные = Неопределено
		Или ТекущиеДанные.Свойство("ГруппировкаСтроки") 
...
Процедура ОбновитьОбластьПросмотра()
	...
	Если Не ДанныеОбластиПросмотра.Распознан И Не ДанныеОбластиПросмотра.Внутренний
		И ДанныеОбластиПросмотра.ПредставлениеДокумента = Неопределено Тогда
		...
		СообщениеЭДО = ДанныеОбластиПросмотра.СообщениеЭДО;
		
		Предпросмотр = Неопределено;
		
		Если СообщениеЭДО <> Неопределено И СообщениеЭДО <> ТекущееСообщениеЭДО Тогда
			Элементы.ГруппаПредставлениеПрисоединенногоФайлаСВизуализацией.Видимость  = Истина;
			Элементы.ГруппаПредставлениеПрисоединенногоФайлаБезВизуализации.Видимость = Ложь;
			Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаОжидание;
			ПрочитатьПроштампованныйФайл(ТекущиеДанные.ЭлектронныйДокумент);
		КонецЕсли;
		ТекущееСообщениеЭДО = СообщениеЭДО;
	...
	Иначе
		...
		Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаОжидание;
		ТекущееСообщениеЭДО = Неопределено;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДанныеОбластиПросмотра.ФайлСсылка) Тогда
	...
	УстановитьВидимостьОписанияФайла();
	
КонецПроцедуры

&НаКлиенте
Процедура ПрочитатьПроштампованныйФайл(ЭлектронныйДокумент)
	
	Элементы.ТекущийДокументPDF.Видимость = Ложь;
	
	ДанныеПроштамповки = ИнтерфейсДокументовЭДОВызовСервера.ПроштампованныйФайлPDF(СообщениеЭДО, УникальныйИдентификатор);
	Если ДанныеПроштамповки.ИспользоватьПросмотрПроштампованногоФайла Тогда
		ОповещениеОЗавершенииПолученияФайлаPDF = Новый ОписаниеОповещения("ПослеПолученияТекущегоФайлаPDF", ЭтотОбъект,
			Новый Структура("ЭлектронныйДокумент", ЭлектронныйДокумент));
		ИнтерфейсДокументовЭДОКлиент.НачатьПолучениеПроштампованногоФайлаPDF(ОповещениеОЗавершенииПолученияФайлаPDF,
			ДанныеПроштамповки.ДлительнаяОперация);
	Иначе
		Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаФайл;
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаБезВизуализации.Видимость = Истина;
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаСВизуализацией.Видимость  = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеПолученияТекущегоФайлаPDF(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ТекущиеДанные.ЭлектронныйДокумент <> ДополнительныеПараметры.ЭлектронныйДокумент Тогда
		Возврат;
	КонецЕсли;
	...
	Если РезультатВыполнения = Неопределено Тогда
		...
		УстановитьВидимостьОписанияФайла();
		Возврат;
	ИначеЕсли РезультатВыполнения.ПредставлениеДокумента = Неопределено Тогда
		Если ЗначениеЗаполнено(РезультатВыполнения.ТекстОшибки) Тогда
			Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаОшибка;
		Иначе
			Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаФайл;
			УстановитьВидимостьОписанияФайла();
	...
	Отказ = Ложь;
	ВизуализироватьДокументPDF(РезультатВыполнения.ПредставлениеДокумента, Отказ);
...
// Параметры:
//  ПредставлениеДокумента - ДвоичныеДанные
//  Отказ - Булево - флаг, показывающий успешно ли прошла операция.
//
&НаСервере
Процедура ВизуализироватьДокументPDF(ПредставлениеДокумента, Отказ)
...
Процедура ОбновитьВидимостьКомандТекущейСтроки(ТекущиеДанные)
	...
	ИначеЕсли ТекущиеДанные.СостояниеЭДО = 
		...
		Или ТекущиеДанные.СостояниеЭДО =
		ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяПодписаниеИзвещенияПоОтклонению") Тогда
...
Процедура УстановитьВидимостьКомандФормыПоПравам()
	...
	Элементы.ГруппаКнопокПодписатьСлужебныеСообщения.Видимость = ЕстьПравоОбработкиДокументов;
	...
	Элементы.ЗагрузитьФайлы.Видимость = ЕстьПравоОбработкиДокументов;
	Элементы.ЗагрузитьФайлы1.Видимость = ЕстьПравоОбработкиДокументов;
	Элементы.ЗагрузитьФайлы2.Видимость = ЕстьПравоОбработкиДокументов;
	...
	Элементы.ПроверитьПодписиСформировать1.Видимость = ЕстьПравоАдминистрирования;
...
&НаКлиенте
Процедура УстановитьВидимостьОписанияФайла()
	Если Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаФайл Тогда
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаБезВизуализации.Видимость = Не Элементы.ТекущийДокументPDF.Видимость;
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаСВизуализацией.Видимость  = Элементы.ТекущийДокументPDF.Видимость;
	Иначе
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаБезВизуализации.Видимость = Ложь;
		Элементы.ГруппаПредставлениеПрисоединенногоФайлаСВизуализацией.Видимость  = Ложь;
	КонецЕсли;
