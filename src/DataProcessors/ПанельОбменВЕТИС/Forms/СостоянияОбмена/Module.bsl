...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ХранитьФайлыВТомахНаДиске" Тогда
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "НастройкиРегламентныхЗаданийВЕТИС" Тогда
		
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("ОрганизацииВЕТИС", ОрганизацииВЕТИС);
		ОткрытьФорму("Справочник.НастройкиРегламентныхЗаданийВЕТИС.Форма.ФормаНастроек", ПараметрыОткрытияФормы, ЭтотОбъект);
...
Процедура ОтобразитьСостояниеОбмена(РезультатыЗапроса)
	...
	ПроблемыКлассификаторовВПанелиОбмена = ИнтеграцияВЕТИС.ПроблемыКлассификаторовВПанелиОбмена();
...
Процедура ПроверитьНастройкиРегламентногоЗадания()
	...
	Если ОбменДаннымиВЕТИС.ЕстьПроблемыОтправкиПолученияДанныхВЕТИС(ОрганизацииВЕТИС) Тогда
		...
		ИмяЭлемента = "РекомендуетсяНастроитьРегламентныеЗадания";
		
		ТекстЗаголовка = Новый ФорматированнаяСтрока(
			НСтр("ru='Рекомендуется настроить регламентные задания для обмена с ВетИС'"),,,,
			"НастройкиРегламентныхЗаданийВЕТИС");
		Элементы[ИмяЭлемента].Заголовок = ТекстЗаголовка;
		
		ТекстПодсказки = НСтр("ru = 'Некорректные настройки могут привести к значительному увеличению времени выполнения обмена.'");
		
		Если Не ПравоДоступа("Изменение", Метаданные.Справочники.НастройкиРегламентныхЗаданийВЕТИС) Тогда
	...
	Если ИнтеграцияИС.СерииИспользуются() Тогда
		...
		Если Элементы.ГруппаРегламентноеЗаданиеСверткаРегистраСоответствиеНоменклатурыВЕТИС.Видимость Тогда
			...
			ИмяЭлемента = "РекомендуетсяНастроитьСверткаРегистраСоответствиеНоменклатурыВЕТИС";
			...
			Если Не ПравоДоступа("Изменение", Метаданные.Константы.ИспользоватьАвтоматическуюСверткуРегистраСоответствиеНоменклатурыВЕТИС) Тогда
