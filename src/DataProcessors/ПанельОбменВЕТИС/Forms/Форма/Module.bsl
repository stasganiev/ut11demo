...
Процедура НадписьСообщенияОжидающиеОтправкиНажатие(Элемент)
	...
	Если ЕстьТолькоСообщенияОжидающиеОтправки Тогда
		...
		ОткрытьФорму("РегистрСведений.ОчередьСообщенийВЕТИС.Форма.ФормаСписка", ПараметрыОткрытияФормы, ЭтотОбъект);
...
Процедура НастройкаВЕТИС(Команда)
	...
	ОткрытьФорму("Обработка.ПанельАдминистрированияВЕТИС.Форма.НастройкиВЕТИС", , ЭтотОбъект);
...
Процедура ОтобразитьСостояниеОбмена()
	...
	ПроблемыКлассификаторов = ИнтеграцияВЕТИС.ПроблемыКлассификаторовВПанелиОбмена();
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			...
			ПерваяПроблема = СтрШаблон(
				НСтр("ru = 'Есть сообщения (%1), ожидающие отправки. Выполните обмен.'"),
				Проблемы.Количество);
	...
	Если ОбменДаннымиВЕТИС.ЕстьПроблемыОтправкиПолученияДанныхВЕТИС(ОрганизацииВЕТИС) Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
			ПерваяПроблема = НСтр("ru='Имеются проблемы обменов данными с ВетИС'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
		КонецЕсли;
	КонецЕсли;
		...
		Если ИнтеграцияИС.СерииИспользуются() Тогда
			...
			Если Не ЗаданиеВключено Тогда
				Если ПерваяПроблема <> Неопределено Тогда
					ЕщеПроблемы = ЕщеПроблемы + 1;
				Иначе
					ПерваяПроблема = НСтр("ru='Рекомендуется настроить свертку регистра соответствия номенклатуры ВетИС'");
					Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаВетеринарноСопроводительныеДокументы()
	...
	|	И ЕСТЬNULL(Статусы.СтатусОформления, ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.НеОформлено)) В (ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.НеОформлено), ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.ОформленоЧастично), ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.ЕстьОшибкиОформления), ЗНАЧЕНИЕ(Перечисление.СтатусыОформленияДокументовГосИС.ТребуетсяСопоставлениеНоменклатуры))
...
Функция ТекстаЗапросаКоличестваДокументовПоДальнейшемуДействию(МетаданныеДокументаВЕТИС, ОтборДействия)
	...
	Иначе
		...
		СтрокиОтбораДокументов.Добавить("ДокументВЕТИС.ДокументОснование В (&ПустойДокументОснование)");
...
Процедура ОбработкаНавигационнойСсылкиЛогирования(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьЛогЗапросов" Тогда
		...
		ЛогированиеЗапросовИСКлиент.ОткрытьПросмотрЛогаЗапросов(УникальныйИдентификатор, "ВЕТИС");
...
Процедура ОбновитьПредставлениеЛогирования()
	...
	ДанныеПодсказки.Добавить(НСтр("ru='техническую информацию по взаимодействию с ВетИС'"));
	ДанныеПодсказки.Добавить(" ");
