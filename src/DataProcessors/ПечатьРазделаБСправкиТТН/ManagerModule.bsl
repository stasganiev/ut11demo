...
Функция ПечатьТТНСправкаРазделБ(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	НезаполненныеЗначенияКонтрагента = ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа("ОрганизацияКонтрагентГосИС");
	
	ЗапросПоЛицензиатам = Новый Запрос;
	ЗапросПоЛицензиатам.УстановитьПараметр("МассивДокументов",                 МассивОбъектов);
	ЗапросПоЛицензиатам.УстановитьПараметр("НезаполненныеЗначенияКонтрагента", НезаполненныеЗначенияКонтрагента);
	...
	|		И НЕ ТТНИсходящаяЕГАИС.Грузоотправитель.Контрагент В (&НезаполненныеЗначенияКонтрагента)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ТТНИсходящаяЕГАИС.Ссылка В(&МассивДокументов)
	|		И НЕ ТТНИсходящаяЕГАИС.Грузополучатель.Контрагент В (&НезаполненныеЗначенияКонтрагента)
	|	) КАК ВложенныйЗапрос";
	...
	Запрос.УстановитьПараметр("ПустаяОрганизация"       , ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("ОрганизацияКонтрагентГосИС"));
