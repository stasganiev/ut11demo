...
//
// Возвращаемое значение:
//  Структура - список методов, см. ИнтеграцияСМаркетплейсамиСервер.ИспользуемыеМетодыМенеджераОбъектаМетаданных.
//
...
Процедура НастроитьДинамическийСписокОтображенияРезультатаЗагрузки(ТаблицаФормы, СписокЗагруженныхОбъектов,
	...
	Если (ИмяТабличнойЧасти = "ДанныеПоставок" Или ИмяТабличнойЧасти = "ДанныеЗаявокНаВозврат")
	  	И Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоСкладов")
	  Или ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
...
//
// Возвращаемое значение:
//   Структура - данные и информация об ошибках,
...
Функция ПолучитьДанныеИнтеграционнымиМетодами(ТаблицаДляЗагрузкиДанных, Параметры) Экспорт
	...
	Если Параметры.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		Результат = ПолучитьДанныеДанныеОРеализованныхТоварах(ТаблицаДляЗагрузкиДанных, Параметры);
	Иначе
...
//
// Возвращаемое значение:
//   Структура - см. ИнтеграцияСМаркетплейсамиСервер.РезультатЗаписиЗагружаемыхДанных
//
...
Функция ПолучитьДанныеДанныеОРеализованныхТоварах(ТаблицаДляЗагрузкиДанных, Параметры)
	...
	Если ТаблицаОборотовТоваров.Количество() = 0 Тогда
		Результат.ИнформацияОбОшибке.ОписаниеОшибки = НСтр("ru = 'Нет данных для загрузки.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого СтрокаОборотов Из ТаблицаОборотовТоваров Цикл
		...
		Если Не ЭтоВозврат Тогда
			...
			Если Параметры.НачалоПериода >= ДатаНовойВерсииОтчета Тогда
				...
				НоваяСтрока.БаллыЗаСкидки            = СтрокаОборотов.ЦенаДоСкидки * СтрокаОборотов.Заказано
		...
		Иначе
			...
			Если Параметры.НачалоПериода >= ДатаНовойВерсииОтчета Тогда
				...
				НоваяСтрока.БаллыЗаСкидкиВозврат     = СтрокаОборотов.ЦенаДоСкидки * - СтрокаОборотов.Заказано
														- СтрокаОборотов.ЦенаДляКлиента * - СтрокаОборотов.Заказано;
			КонецЕсли;
			...
			НоваяСтрока.ДоплатаВозврат               =
				?(СтрокаОборотов.Заказано = 0 Или Параметры.НачалоПериода >= Дата(2023,10,01),
					0, - СтрокаОборотов.СуммаСкидки);
		КонецЕсли;
		НоваяСтрока.СхемаДоставки                    = ?(ЗначениеЗаполнено(СтрокаОборотов.СхемаДоставки),
			СтрокаОборотов.СхемаДоставки, Перечисления.СхемыРаботыТорговыхПлощадок.FBO);
...
Процедура СопоставитьДанныеОРеализованныхТоварах(ТаблицаСопоставленияДанных, ДанныеИзФайла, СписокНеоднозначностей,
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗагружаемыеДанные.НомерСтроки КАК НомерСтроки,
		|	ВЫРАЗИТЬ(ЗагружаемыеДанные.ИдентификаторОбъектаМаркетплейса КАК СТРОКА(50)) КАК ИдентификаторОбъектаМаркетплейса,
		|	ЗагружаемыеДанные.ПредставлениеОбъектаМаркетплейса КАК ПредставлениеОбъектаМаркетплейса,
		|	ВЫРАЗИТЬ(ЗагружаемыеДанные.ИдентификаторПубликации КАК СТРОКА(50)) КАК ИдентификаторПубликации,
		|	ЗагружаемыеДанные.Штрихкод КАК Штрихкод,
		...
		|	ВЫРАЗИТЬ(ЗагружаемыеДанные.НомерДокумента КАК СТРОКА(50)) КАК НомерДокумента,
	...
	ПериодОборотаЗаказовЗаполнен = ПериодОборотаЗаказовЗаполнен
		И НастройкиУчетнойЗаписи.ПродажиРазделяютсяПоДоговорам;
	...
	ИнтеграцияСМаркетплейсамиСервер.УстановитьПроцентПрогресса(1, 1,, ПараметрыПрогресса);
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫРАЗИТЬ(ТаблицаОтправлений.НомерОтправления КАК СТРОКА(50)) КАК НомерОтправления,
		|	ТаблицаОтправлений.ДатаОтправления КАК ДатаОтправления,
		|	ВЫРАЗИТЬ(ТаблицаОтправлений.ИдентификаторПубликации КАК СТРОКА(50)) КАК ИдентификаторПубликации,
	...
	Запрос.Текст =
		...
		|				КОНЕЦ / ЗагружаемыеДанные.КоличествоУпаковок КАК ЧИСЛО(31, 2))
		...
		|						ТОГДА ЗагружаемыеДанные.РеализованоНаСуммуВозврат + ЗагружаемыеДанные.ДоплатаВозврат
		|					ИНАЧЕ ЗагружаемыеДанные.РеализованоНаСуммуВозврат
		|				КОНЕЦ / ЗагружаемыеДанные.КоличествоУпаковокВозврат КАК ЧИСЛО(31, 2))
	...
	Пока ВыборкаИдентификаторов.Следующий() Цикл
		...
		Если ВыборкаДанных.Следующий() Тогда
			...
			Если Не ЗначениеЗаполнено(ВыборкаДанных.СхемаДоставки) Тогда
				Если ВыполнятьИндивидуальныйПоискСхемыДоставки
				  И ПоискНомеровОтправлений Тогда
					...
					КонецЕсли;
				ИначеЕсли Не НастройкиУчетнойЗаписи.ПродажиРазделяютсяПоДоговорам Тогда
			...
			Если НоваяСтрока.СхемаДоставки = Перечисления.СхемыРаботыТорговыхПлощадок.FBO Тогда
				НоваяСтрока.ДоговорДляОтраженияПродажи =
...
Функция ЗаписатьДанныеПоставок(ТаблицаСопоставленияДанных, Параметры, СсылкаДляОбновленияДанных,
	...
	АвтоматическоеФормированиеРасходныхОрдеровНаТовары =
		НастройкиУчетнойЗаписи.ДополнительныеНастройки.Получить("АвтоматическоеФормированиеРасходныхОрдеровНаТовары");
	АвтоматическоеФормированиеРасходныхОрдеровНаТовары =
...
Функция ЗаписатьДанныеОРеализованныхТоварах(ТаблицаСопоставленияДанных, Параметры, СсылкаДляОбновленияДанных,
			ПараметрыПрогресса)
	...
	НастройкиУчетнойЗаписи      = Справочники.УчетныеЗаписиМаркетплейсов.НастройкиУчетнойЗаписи(Параметры.УчетнаяЗапись);
	ДополнительныеНастройки     = НастройкиУчетнойЗаписи.ДополнительныеНастройки;
	УслугаПоКомиссионнойПродаже = Неопределено;
	Если ДополнительныеНастройки <> Неопределено Тогда
		УслугаПоКомиссионнойПродаже = ДополнительныеНастройки.Получить("УслугаПродажиЧерезТорговуюПлощадку");
	КонецЕсли;
	...
	Если Не ТипЗнч(СсылкаДляОбновленияДанных) = Тип("ДокументСсылка.ОтчетКомиссионера")
	  Или Не ЗначениеЗаполнено(СсылкаДляОбновленияДанных) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапросаПоискаОтчетаКомиссионера();
		Запрос.УстановитьПараметр("Организация",    НастройкиУчетнойЗаписи.Организация);
		Запрос.УстановитьПараметр("Партнер",        НастройкиУчетнойЗаписи.Партнер);
		Запрос.УстановитьПараметр("Контрагент",     НастройкиУчетнойЗаписи.Контрагент);
		Запрос.УстановитьПараметр("Соглашение",     НастройкиУчетнойЗаписи.Соглашение);
		Запрос.УстановитьПараметр("ДоговорFBO",     НастройкиУчетнойЗаписи.ДоговорПродажиЧерезСкладыТорговойПлощадки);
		Запрос.УстановитьПараметр("ДоговорFBS",     НастройкиУчетнойЗаписи.ДоговорПродажиЧерезСкладыСобственные);
		...
		ЗаполняемыеДокументы = Новый ТаблицаЗначений;
		ЗаполняемыеДокументы.Колонки.Добавить("ОтчетКомиссионераСсылка",    Новый ОписаниеТипов("ДокументСсылка.ОтчетКомиссионера"));
		ЗаполняемыеДокументы.Колонки.Добавить("ОтчетКомиссионераОбъект",    Новый ОписаниеТипов("ДокументОбъект.ОтчетКомиссионера"));
		ЗаполняемыеДокументы.Колонки.Добавить("Договор",                    Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
		ЗаполняемыеДокументы.Колонки.Добавить("СостояниеСсылки",            Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(20)));
		ЗаполняемыеДокументы.Колонки.Добавить("БазоваяСуммаВознаграждения", Метаданные.ОпределяемыеТипы.ДенежнаяСуммаЛюбогоЗнака.Тип);

		// Документ для пустого договора.
		НоваяСтрока = ЗаполняемыеДокументы.Добавить();

		ДокументНайден = Истина;

		// Документ по продажам через склады торговой площадки.
		НоваяСтрока = ЗаполняемыеДокументы.Добавить();
		НоваяСтрока.Договор = ?(ЗначенияОсновныхНастроек.ИспользуютсяДоговорыКонтрагентов,
								НастройкиУчетнойЗаписи.ДоговорПродажиЧерезСкладыТорговойПлощадки,
								Справочники.ДоговорыКонтрагентов.ПустаяСсылка());

		ВыборкаДокументовFBO = ПакетЗапроса[ПакетЗапроса.ВГраница() - 2].Выбрать();
		Если ВыборкаДокументовFBO.Следующий() Тогда
			НоваяСтрока.ОтчетКомиссионераСсылка = ВыборкаДокументовFBO.Ссылка;
			НоваяСтрока.ОтчетКомиссионераОбъект = ВыборкаДокументовFBO.Ссылка.ПолучитьОбъект();
		Иначе
			ДокументНайден = Ложь;
		КонецЕсли;

		Если ЗначенияОсновныхНастроек.ИспользуютсяДоговорыКонтрагентов
		  И НастройкиУчетнойЗаписи.ПродажиРазделяютсяПоДоговорам Тогда
			// Документ по продажам через собственные склады.
			НоваяСтрока = ЗаполняемыеДокументы.Добавить();
			НоваяСтрока.Договор = НастройкиУчетнойЗаписи.ДоговорПродажиЧерезСкладыСобственные;
		КонецЕсли;

		Если ЗначенияОсновныхНастроек.ИспользуютсяДоговорыКонтрагентов Или Не ДокументНайден Тогда
			ВыборкаДокументовFBS = ПакетЗапроса[ПакетЗапроса.ВГраница() - 1].Выбрать();
			Если ВыборкаДокументовFBS.Следующий() Тогда
				НоваяСтрока.ОтчетКомиссионераСсылка = ВыборкаДокументовFBS.Ссылка;
				НоваяСтрока.ОтчетКомиссионераОбъект = ВыборкаДокументовFBS.Ссылка.ПолучитьОбъект();
			КонецЕсли;
		КонецЕсли;
	Иначе
		НоваяСтрока = ЗаполняемыеДокументы.Добавить();
		НоваяСтрока.ОтчетКомиссионераОбъект = ВыборкаДокументовFBS.Ссылка.ПолучитьОбъект();
		НоваяСтрока.Договор = ?(ЗначенияОсновныхНастроек.ИспользуютсяДоговорыКонтрагентов,
			НоваяСтрока.ОтчетКомиссионераОбъект.Договор, Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
	КонецЕсли;

	//Заполнение документов.
	РеквизитыШапки = Новый Структура;
	РеквизитыШапки.Вставить("Организация", НастройкиУчетнойЗаписи.Организация);
	РеквизитыШапки.Вставить("Партнер",     НастройкиУчетнойЗаписи.Партнер);
	РеквизитыШапки.Вставить("Контрагент",  НастройкиУчетнойЗаписи.Контрагент);
	РеквизитыШапки.Вставить("Соглашение",  ?(ЗначенияОсновныхНастроек.ИспользоватьСоглашенияСКлиентами,
		НастройкиУчетнойЗаписи.Соглашение, Справочники.СоглашенияСКлиентами.ПустаяСсылка()));

	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Дата",                         КонецДня(КонецПериода));
	...
	Для Каждого СтрокаДокумента Из ЗаполняемыеДокументы Цикл
		Если ЗначениеЗаполнено(СтрокаДокумента.ОтчетКомиссионераСсылка) Тогда
			ОписаниеОшибкиБлокировки = ЗаблокироватьОбъект(СтрокаДокумента.ОтчетКомиссионераОбъект);
			Если Не ПустаяСтрока(ОписаниеОшибкиБлокировки) Тогда
				Результат.ОписаниеОшибки = ОписаниеОшибкиБлокировки;
				Возврат Результат;
			КонецЕсли;

			СтрокаДокумента.СостояниеСсылки = ДоступныеСостоянияДокументов.СостояниеОбновлен;
		Иначе
			СтрокаДокумента.ОтчетКомиссионераОбъект = Документы.ОтчетКомиссионера.СоздатьДокумент();
			СтрокаДокумента.СостояниеСсылки = ДоступныеСостоянияДокументов.СостояниеНовый;
		КонецЕсли;

		ДанныеЗаполнения.РеквизитыШапки.Вставить("Договор", СтрокаДокумента.Договор);

		СтрокаДокумента.ОтчетКомиссионераОбъект.ПометкаУдаления             = Ложь;
		СтрокаДокумента.ОтчетКомиссионераОбъект.Дата                        = КонецДня(КонецПериода);
		СтрокаДокумента.ОтчетКомиссионераОбъект.НомерВходящегоДокумента     = НомерВходящегоДокумента;
		СтрокаДокумента.ОтчетКомиссионераОбъект.ДатаВходящегоДокумента      = ДатаВходящегоДокумента;
		СтрокаДокумента.ОтчетКомиссионераОбъект.ХозяйственнаяОперация       = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию;
		СтрокаДокумента.ОтчетКомиссионераОбъект.НачалоПериода               = НачалоПериода;
		СтрокаДокумента.ОтчетКомиссионераОбъект.КонецПериода                = КонецПериода;
		СтрокаДокумента.ОтчетКомиссионераОбъект.Услуга                      = УслугаПоКомиссионнойПродаже;
		СтрокаДокумента.ОтчетКомиссионераОбъект.Комментарий                 = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '(Ozon) Отчет о реализации №%1 за период с %2 по %3'"),
				НомерВходящегоДокумента,
				Формат(НачалоПериода, "ДФ = дд.ММ.гггг"),
				Формат(КонецПериода, "ДФ = дд.ММ.гггг"));

		СтрокаДокумента.ОтчетКомиссионераОбъект.Заполнить(ДанныеЗаполнения);

		СтрокаДокумента.ОтчетКомиссионераОбъект.Контрагент = НастройкиУчетнойЗаписи.Контрагент;
		СтрокаДокумента.ОтчетКомиссионераОбъект.Договор = СтрокаДокумента.Договор;

		СтрокаДокумента.ОтчетКомиссионераОбъект.СпособРасчетаВознаграждения =
			Перечисления.СпособыРасчетаКомиссионногоВознаграждения.ПроизвольнаяСумма;

		СтрокаДокумента.ОтчетКомиссионераОбъект.Товары.Очистить();
	КонецЦикла;
	...
	Для Каждого СтрокаТовара Из ТаблицаСопоставленияДанных Цикл
		...
		СтрокаДокумента = ЗаполняемыеДокументы.Найти(СтрокаТовара.ДоговорДляОтраженияПродажи, "Договор");
		Если СтрокаДокумента = Неопределено Тогда
			ОписаниеОшибок.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для договора ""%1"" не найден документ для заполнения.'"),
					СтрокаТовара.ДоговорДляОтраженияПродажи));
			Продолжить;
		КонецЕсли;

		ОтчетКомиссионераОбъект = СтрокаДокумента.ОтчетКомиссионераОбъект;

		ЭтоПродажа = ЗначениеЗаполнено(СтрокаТовара.КоличествоУпаковок);
		ДобавитьВозврат = Ложь;
		...
		Если ЭтоПродажа Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТовара.Упаковка)
			  И СтрокаТовара.Упаковка <> СтрокаТовара.НоменклатураЕдиницаИзмерения Тогда
				Если ЗначенияОсновныхНастроек.ИспользоватьУпаковкиНоменклатуры Тогда
					СтрокаТабличнойЧасти.Упаковка = СтрокаТовара.НоменклатураЕдиницаИзмерения;
					КоличествоУпаковокВСтроке     = СтрокаТовара.КоличествоУпаковок;
					КоличествоВСтроке             = Окр(КоличествоУпаковокВСтроке
						* СтрокаТовара.УпаковкаКоэффициент, 3, РежимОкругления.Окр15как20);
				Иначе
			...
			СтрокаТабличнойЧасти.НомерДокументаОтгрузки        = СтрокаТовара.НомерДокумента;
			СтрокаТабличнойЧасти.ДатаДокументаОтгрузки         = СтрокаТовара.ДатаДокумента;
			СтрокаТабличнойЧасти.НомерСчетаФактурыКомиссионера = СтрокаТовара.НомерСчетаФактуры;
			СтрокаТабличнойЧасти.ДатаСчетаФактурыКомиссионера  = СтрокаТовара.ДатаСчетаФактуры;

			СтрокаТабличнойЧасти.Цена                          = СтрокаТовара.Цена;
			СтрокаТабличнойЧасти.ЦенаПродажи                   = СтрокаТовара.ЦенаПродажи;
			СтрокаТабличнойЧасти.Количество                    = Окр(КоличествоВСтроке, 3, РежимОкругления.Окр15как20);
			СтрокаТабличнойЧасти.КоличествоУпаковок            = КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.КоличествоУпаковокФакт        = КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.КоличествоУпаковокУчет        = КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.Сумма                         = СтрокаТовара.Сумма;
			СтрокаТабличнойЧасти.СуммаПродажи                  = СтрокаТовара.СуммаПродажи;
			СтрокаТабличнойЧасти.СуммаВознаграждения           = СтрокаТовара.СуммаВознаграждения;

			ЗаполнитьДанныеСтрокиОтчетаКомиссионера(
				СтрокаТабличнойЧасти,
				ОтчетКомиссионераОбъект,
				КэшированныеЗначения,
				ЗначенияОсновныхНастроек);

			ДобавитьВозврат = ЗначениеЗаполнено(СтрокаТовара.КоличествоУпаковокВозврат);
		Иначе
			ДобавитьВозврат = Истина;
		КонецЕсли;
		...
		Если ДобавитьВозврат Тогда
			...
			Если ЗначенияОсновныхНастроек.ИспользоватьУпаковкиНоменклатуры
			  И СтрокаТовара.Упаковка <> СтрокаТовара.НоменклатураЕдиницаИзмерения Тогда
				СтрокаТабличнойЧасти.Упаковка = СтрокаТовара.НоменклатураЕдиницаИзмерения;
				КоличествоУпаковокВСтроке     = Окр(СтрокаТовара.КоличествоУпаковокВозврат
					* СтрокаТовара.УпаковкаКоэффициент,3, РежимОкругления.Окр15как20);
				КоличествоВСтроке             = КоличествоУпаковокВСтроке;
			Иначе
				СтрокаТабличнойЧасти.Упаковка = СтрокаТовара.Упаковка;
				КоличествоУпаковокВСтроке     = СтрокаТовара.КоличествоУпаковокВозврат;
				КоличествоВСтроке             = Окр(КоличествоУпаковокВСтроке
					* СтрокаТовара.УпаковкаКоэффициент, 3, РежимОкругления.Окр15как20);
			КонецЕсли;
			...
			СтрокаТабличнойЧасти.Цена                          = СтрокаТовара.ЦенаВозврат;
			СтрокаТабличнойЧасти.ЦенаПродажи                   = СтрокаТовара.ЦенаПродажиВозврат;
			СтрокаТабличнойЧасти.Количество                    = - КоличествоВСтроке;
			СтрокаТабличнойЧасти.КоличествоУпаковок            = - КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.КоличествоУпаковокФакт        = - КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.КоличествоУпаковокУчет        = - КоличествоУпаковокВСтроке;
			СтрокаТабличнойЧасти.Сумма                         = - СтрокаТовара.СуммаВозврат;
			СтрокаТабличнойЧасти.СуммаПродажи                  = - СтрокаТовара.СуммаПродажиВозврат;
			СтрокаТабличнойЧасти.СуммаВознаграждения           = - СтрокаТовара.СуммаВознагражденияВозврат;
	...
	ЗаполнитьЗначенияСвойств(ПараметрыЗапроса, Параметры.ПараметрыШапки.Параметры);

	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаРасчетаИтоговыхПоказателей();
	Запрос.УстановитьПараметр("ТаблицаСопоставленияДанных", ТаблицаСопоставленияДанных);
	Запрос.УстановитьПараметр("НачалоПериода", ПараметрыЗапроса.НачалоПериода);

	ОбщиеИтоги = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Если ОбщиеИтоги.Следующий() Тогда
		Если ОбщиеИтоги.ЕстьБазоваяКомиссия Или ОбщиеИтоги.ЕстьБаллыЗаСкидки Тогда
			БазоваяСуммаВознаграждения = Окр(ОбщиеИтоги.БазоваяКомиссия * 0.01, 2);

			Если ОбщиеИтоги.БазоваяКомиссия - ОбщиеИтоги.БаллыЗаСкидки < БазоваяСуммаВознаграждения Тогда
				КоэффициентПересчета = ?(ОбщиеИтоги.СуммаПродажи = 0, 1,
					БазоваяСуммаВознаграждения / ОбщиеИтоги.СуммаПродажи);
				МаксимальнаяСумма = 0;

				ИтогиПоДоговору = ОбщиеИтоги.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ИтогиПоДоговору.Следующий() Цикл
					СтрокаДокумента = ЗаполняемыеДокументы.Найти(ИтогиПоДоговору.Договор, "Договор");
					Если СтрокаДокумента <> Неопределено Тогда
						СтрокаДокумента.БазоваяСуммаВознаграждения =
							Окр(ИтогиПоДоговору.СуммаПродажи * КоэффициентПересчета, 2);
						МаксимальнаяСумма = Макс(МаксимальнаяСумма, СтрокаДокумента.БазоваяСуммаВознаграждения);
					КонецЕсли;
				КонецЦикла;

				Погрешность = БазоваяСуммаВознаграждения - ЗаполняемыеДокументы.Итог("БазоваяСуммаВознаграждения");
				Если Погрешность <> 0 Тогда
					СтрокаДокумента = ЗаполняемыеДокументы.Найти(МаксимальнаяСумма, "БазоваяСуммаВознаграждения");
					Если СтрокаДокумента <> Неопределено Тогда
						СтрокаДокумента.БазоваяСуммаВознаграждения = СтрокаДокумента.БазоваяСуммаВознаграждения
							+ Погрешность;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого СтрокаДокумента Из ЗаполняемыеДокументы Цикл
		...
		Если ОтчетКомиссионераОбъект.Товары.Количество() > 0 Тогда
			...
			Если ЗначенияОсновныхНастроек.КомиссионерВедетУчетПоРНПТ
				И ЗначениеЗаполнено(ОтчетКомиссионераОбъект.Организация) Тогда
				
				ПараметрыПолученияКоэффициентаРНПТ =
					УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ПараметрыПолученияКоэффициентаРНПТ(
						ОтчетКомиссионераОбъект,
						"Партнер");
			...
			Если ЗначенияОсновныхНастроек.ИспользуютсяДоговорыКонтрагентов Тогда
				УчетныйВидЦеныДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтчетКомиссионераОбъект.Договор, "ВидЦенУчетный");
				ПараметрыЗаполнения.Вставить("ВидЦены", УчетныйВидЦеныДоговора);
			КонецЕсли;
			...
			Если Не ЦеныЗаполнены Тогда
				КэшированныеЗначения = Неопределено;
				ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ОтчетКомиссионераОбъект.Товары, СтруктураДействий, КэшированныеЗначения);
			КонецЕсли;

			Если Не ЦеныЗаполнены И Не ОтчетКомиссионераОбъект.ЦенаВключаетНДС Тогда
			...
			Если Не ЗначениеЗаполнено(ОтчетКомиссионераОбъект.СтавкаНДСВознаграждения) Тогда
				...
				СтруктураДействий = Новый Структура;
				СтруктураДействий.Вставить("СкорректироватьСтавкуНДС", ПараметрыСтавкиНДС);
				
				ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, Неопределено);
				ОтчетКомиссионераОбъект.СтавкаНДСВознаграждения = ТекущаяСтрока.СтавкаНДС;
			КонецЕсли;

			Если СтрокаДокумента.БазоваяСуммаВознаграждения <> 0 Тогда
				ОтчетКомиссионераОбъект.ПроцентВознаграждения = 0;
				ОтчетКомиссионераОбъект.СуммаВознаграждения   = СтрокаДокумента.БазоваяСуммаВознаграждения;

				КомиссионнаяТорговляСервер.РассчитатьСуммуВознаграждения(ОтчетКомиссионераОбъект);
			КонецЕсли;
			...
			Если ОтчетКомиссионераОбъект.ЭтоНовый() И НЕ ЗначениеЗаполнено(ОтчетКомиссионераОбъект.Номер) Тогда
			...
			Если РезультатЗаписиДокумента.ЗаписьВыполнена Тогда
				...
				Результат.СостоянияСсылок.Вставить(ОтчетКомиссионераОбъект.Ссылка, СтрокаДокумента.СостояниеСсылки);
			КонецЕсли;
...
Функция ТекстЗапросаПоискаОтчетаКомиссионера()
	...
	ТекстЗапроса =
		...
		|	ОтчетКомиссионера.ПометкаУдаления КАК ПометкаУдаления,
		|	ОтчетКомиссионера.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
		|	ОтчетКомиссионера.НачалоПериода КАК НачалоПериода,
		|	ОтчетКомиссионера.КонецПериода КАК КонецПериода
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДокументыПериода.Ссылка КАК Ссылка,
		|	ВЫБОР
		|		КОГДА НЕ ДокументыПериода.ПометкаУдаления
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Порядок
		|ИЗ
		...
		|ГДЕ
		|	ДокументыПериода.Контрагент = &Контрагент
		|	И ДокументыПериода.Соглашение = &Соглашение
		|	И ДокументыПериода.Договор = &ДоговорFBO
		|	И ДокументыПериода.НомерВходящегоДокумента = &НомерДокумента
		|	И ДокументыПериода.НачалоПериода = &НачалоПериода
		|	И ДокументыПериода.КонецПериода = &КонецПериода
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок
		|;
		...
		|	ДокументыПериода.Контрагент = &Контрагент
		|	И ДокументыПериода.Соглашение = &Соглашение
		|	И ДокументыПериода.Договор = &ДоговорFBS
		|	И ДокументыПериода.НомерВходящегоДокумента = &НомерДокумента
		|	И ДокументыПериода.НачалоПериода = &НачалоПериода
		|	И ДокументыПериода.КонецПериода = &КонецПериода
		|
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ДокументыПериода";

	Возврат ТекстЗапроса;

КонецФункции

Функция ТекстЗапросаРасчетаИтоговыхПоказателей()

	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаСопоставленияДанных.ДоговорДляОтраженияПродажи КАК ДоговорДляОтраженияПродажи,
		|	ТаблицаСопоставленияДанных.СуммаПродажи КАК СуммаПродажи,
		|	ТаблицаСопоставленияДанных.СуммаВознаграждения КАК СуммаВознаграждения,
		|	ТаблицаСопоставленияДанных.БазоваяКомиссия КАК БазоваяКомиссия,
		|	ТаблицаСопоставленияДанных.БаллыЗаСкидки КАК БаллыЗаСкидки,
		|	ТаблицаСопоставленияДанных.СуммаПродажиВозврат КАК СуммаПродажиВозврат,
		|	ТаблицаСопоставленияДанных.СуммаВознагражденияВозврат КАК СуммаВознагражденияВозврат,
		|	ТаблицаСопоставленияДанных.БазоваяКомиссияВозврат КАК БазоваяКомиссияВозврат,
		|	ТаблицаСопоставленияДанных.БаллыЗаСкидкиВозврат КАК БаллыЗаСкидкиВозврат
		|ПОМЕСТИТЬ ТаблицаСопоставленияДанных
		|ИЗ
		|	&ТаблицаСопоставленияДанных КАК ТаблицаСопоставленияДанных
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаСопоставленияДанных.ДоговорДляОтраженияПродажи КАК Договор,
		|	ЕСТЬNULL(СУММА(ТаблицаСопоставленияДанных.СуммаПродажи - ТаблицаСопоставленияДанных.СуммаПродажиВозврат), 0) КАК СуммаПродажи,
		|	ЕСТЬNULL(СУММА(ТаблицаСопоставленияДанных.БаллыЗаСкидки - ТаблицаСопоставленияДанных.БаллыЗаСкидкиВозврат), 0) КАК БаллыЗаСкидки,
		|	ЕСТЬNULL(СУММА(ТаблицаСопоставленияДанных.СуммаВознаграждения - ТаблицаСопоставленияДанных.СуммаВознагражденияВозврат), 0) КАК СуммаВознаграждения,
		|	ЕСТЬNULL(СУММА(ТаблицаСопоставленияДанных.БазоваяКомиссия - ТаблицаСопоставленияДанных.БазоваяКомиссияВозврат), 0) КАК БазоваяКомиссия,
		|	ЕСТЬNULL(МАКСИМУМ(ТаблицаСопоставленияДанных.БазоваяКомиссия <> 0
		|				ИЛИ ТаблицаСопоставленияДанных.БазоваяКомиссияВозврат <> 0), ЛОЖЬ) КАК ЕстьБазоваяКомиссия,
		|	ЕСТЬNULL(МАКСИМУМ(ТаблицаСопоставленияДанных.БаллыЗаСкидки <> 0
		|				ИЛИ ТаблицаСопоставленияДанных.БаллыЗаСкидкиВозврат <> 0
		|				ИЛИ &НачалоПериода >= ДАТАВРЕМЯ(2023, 10, 1)), ЛОЖЬ) КАК ЕстьБаллыЗаСкидки
		|ИЗ
		|	ТаблицаСопоставленияДанных КАК ТаблицаСопоставленияДанных
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаСопоставленияДанных.ДоговорДляОтраженияПродажи
		|ИТОГИ
		|	СУММА(СуммаПродажи),
		|	СУММА(БаллыЗаСкидки),
		|	СУММА(СуммаВознаграждения),
		|	СУММА(БазоваяКомиссия),
		|	МАКСИМУМ(ЕстьБазоваяКомиссия),
		|	МАКСИМУМ(ЕстьБаллыЗаСкидки)
		|ПО
		|	ОБЩИЕ,
		|	Договор
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ТаблицаСопоставленияДанных";

	Возврат ТекстЗапроса;

КонецФункции

Процедура ЗаполнитьДанныеСтрокиОтчетаКомиссионера(ТекущаяСтрока, ОтчетКомиссионераОбъект, КэшированныеЗначения,
		ЗначенияОсновныхНастроек)
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС",
			ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ОтчетКомиссионераОбъект));
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС",
			Новый Структура("ЦенаВключаетНДС", ОтчетКомиссионераОбъект.ЦенаВключаетНДС));
	СтруктураДействий.Вставить("ПересчитатьСуммуПродажиНДС",
			ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ОтчетКомиссионераОбъект));
	СтруктураДействий.Вставить("ЗаполнитьКодТНВЭД",
			ОтчетКомиссионераОбъект.НалогообложениеНДС);

	ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);

	Если Не ЗначенияОсновныхНастроек.КомиссионерВедетУчетПоРНПТ Тогда
...
Процедура Подключаемый_ИзменитьПоведениеЭлементовФормы(Форма, ЗначенияПараметров) Экспорт
	...
	ИначеЕсли Форма.ИмяТабличнойЧасти = "ДанныеОРеализованныхТоварах" Тогда
		...
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВалютаУчета") Тогда
			...
			МассивРеквизитов.Добавить(Новый РеквизитФормы("ВалютаУчета",
				Новый ОписаниеТипов("СправочникСсылка.Валюты")));
			МассивРеквизитов.Добавить(Новый РеквизитФормы("Организация",
				Новый ОписаниеТипов("СправочникСсылка.Организации")));
			МассивРеквизитов.Добавить(Новый РеквизитФормы("Контрагент",
		...
		ЭлементДоговор = Элементы["ТаблицаСопоставленияДанных_ДоговорДляОтраженияПродажи"];
		ЭлементДоговор.СвязиПараметровВыбора = НовыеСвязи;
...
//
// Возвращаемое значение:
//  Структура - список параметров, см. НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
