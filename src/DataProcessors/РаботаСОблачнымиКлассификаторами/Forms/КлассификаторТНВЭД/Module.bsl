...
Процедура ЗагрузитьДанныеИзСервиса()
	...
	Если Не ДеревоВОбработке Тогда
		...
		ПараметрАдресКэша = ?(Элементы.Дерево.Отображение = ОтображениеТаблицы.Список,
			АдресКэшаРезультатПоиска, АдресКэша);
		
		ОблачныеКлассификаторыКлиент.ЗагрузитьВБазуДанныеТНВЭД(ПараметрАдресКэша, ВыбранныеЭлементы.ВыгрузитьЗначения(),
