...
#КонецЕсли
	...
	// Сервис прогнозирования
	Если Не ВыполненоПодключение
		И ДоступныПраваАдминистратораСервисаПрогнозированияПродаж Тогда
		Отказ = Истина;
		ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.МастерПодключенияКСервису",
			Новый Структура);
	ИначеЕсли Не ВыполненоПодключение
		И Не ДоступныПраваАдминистратораСервисаПрогнозированияПродаж Тогда
		Отказ = Истина;
		ТекстОшибки = НСтр("ru = 'Сервис прогнозирования не настроен. Текущий пользователь не имеет прав для настройки сервиса.'");
	...
	Подключаемый_ИндикаторВыгрузки(); // Сервис прогнозирования.
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ВыборМоделиПрогнозирования" Тогда
		// Сервис прогнозирования.
		Если Параметр.Свойство("ИмяВСервисе")
			И ЗначениеЗаполнено(Параметр.ИмяВСервисе) Тогда 
			
			ЗагрузитьДанныеИзСервисаНаСервере(Параметр.ИмяВСервисе);
			Если Не ПустаяСтрока(ИдентификаторЗаданияЗагрузитьПрогноз) Тогда
				ПодключитьОбработчикОжидания("Подключаемый_ЗагрузитьПрогнозНаКлиенте", 1, Истина);
			КонецЕсли;
			
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "ЗапускВыгрузкиДанныхВСервисПрогнозирования" Тогда
		// Оповещение о событии может прийти раньше фактического старта выгрузки, ввиду использования фоновых заданий.
		Вызвать_Подключаемый_ИндикаторВыгрузки();
	ИначеЕсли ИмяСобытия = "СозданиеСценарияПрогнозирования" Тогда
...
&НаКлиенте
Процедура СценарийПрогнозированияПриИзменении(Элемент)
	ОчиститьСообщения();
	ОбновитьИнформациюПоСценарию();
КонецПроцедуры

&НаКлиенте
Процедура СценарийПрогнозированияСоздание(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СценарийПрогнозированияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	ОчиститьСообщения();
	СценарийПрогнозированияОбработкаВыбораНаСервере(ВыбранноеЗначение, СтандартнаяОбработка);
...
&НаКлиенте
Процедура ВидПланаСоздание(Элемент, СтандартнаяОбработка)
	...
	Если СценарийПрогнозирования.Пустая() Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияОбОшибке,
			Неопределено, "СценарийПрогнозирования");
		Возврат;
	КонецЕсли;
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	// Сервис прогнозирования.
	Если СценарийПрогнозирования.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнен сценарий планирования'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, Неопределено, "СценарийПрогнозирования");
		Возврат;
	ИначеЕсли ВидПлана.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнен вид плана'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, Неопределено, "ВидПлана");
		Возврат;
	ИначеЕсли Период.ДатаОкончания < Период.ДатаНачала Тогда
		ТекстОшибки = НСтр("ru = 'Конец периода должен быть больше начала'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, Неопределено, "КонецПериода");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ОбновитьПрогнозНаКлиенте()
	Подключаемый_ОбновитьПрогнозНаСервере();
	Если Не ПустаяСтрока(ИдентификаторЗаданияОбновитьПрогноз) Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ОбновитьПрогнозНаКлиенте", 1, Истина);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ЗагрузитьПрогнозНаКлиенте()
	Подключаемый_ЗагрузитьПрогнозНаСервере();
	Если Не ПустаяСтрока(ИдентификаторЗаданияЗагрузитьПрогноз) Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ЗагрузитьПрогнозНаКлиенте", 1, Истина);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура Вызвать_Подключаемый_ИндикаторВыгрузки()
	ПодключитьОбработчикОжидания("Подключаемый_ИндикаторВыгрузки", 2, Истина);
КонецПроцедуры
...
Процедура Подключаемый_ИндикаторВыгрузки()
	...
	Если ИдетВыгрузкаДанных Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ИндикаторВыгрузки", 4, Истина);
	КонецЕсли;
	УстановитьВидимостьДоступность();
...
&НаСервере
Процедура Подключаемый_ОбновитьПрогнозНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или Не ПустаяСтрока(ИдентификаторЗаданияОбновитьПрогноз)
			И ДлительныеОперации.ЗаданиеВыполнено(Новый УникальныйИдентификатор(ИдентификаторЗаданияОбновитьПрогноз)) Тогда
		ИдентификаторЗаданияОбновитьПрогноз = "";
		ОбновитьИнформациюПоСценарию();
		
		ТекстОшибки = ПолучитьИзВременногоХранилища(АдресХранилищаОбновитьПрогноз);
...
&НаСервере
Процедура Подключаемый_ЗагрузитьПрогнозНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или Не ПустаяСтрока(ИдентификаторЗаданияЗагрузитьПрогноз)
			И ДлительныеОперации.ЗаданиеВыполнено(Новый УникальныйИдентификатор(ИдентификаторЗаданияЗагрузитьПрогноз)) Тогда
		ИдентификаторЗаданияЗагрузитьПрогноз = "";
		ОбновитьИнформациюПоСценарию();
		
		ТекстОшибки = ПолучитьИзВременногоХранилища(АдресХранилищаЗагрузитьПрогноз);
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура МастерПодключенияКСервису(Команда)
	ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.МастерПодключенияКСервису");
КонецПроцедуры

&НаКлиенте
Процедура КомандыАдминистратора(Команда)
	ТекстВопроса = НСтр("ru = 'Команды предназначены для администратора. Продолжить?'");
	Оповещение = Новый ОписаниеОповещения("КомандыАдминистратораЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
КонецПроцедуры

&НаКлиенте
Процедура КомандыАдминистратораЗавершение(Результат, ПараметрКоманды) Экспорт
	
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.ФормаКомандАдминистратора");
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкиСервиса(Команда)
	ТекстВопроса = НСтр("ru = 'Просмотр настроек предназначен для администратора. Продолжить?'");
	Оповещение = Новый ОписаниеОповещения("НастройкиСервисаЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
КонецПроцедуры

&НаКлиенте
Процедура НастройкиСервисаЗавершение(Результат, ПараметрКоманды) Экспорт
	
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.РедактированиеСохраненныхНастроек");
...
&НаКлиенте
Процедура ИсторияОбменовДанными(Команда)
	ОткрытьФорму("РегистрСведений.ЖурналСервисаПрогнозирования.ФормаСписка");
КонецПроцедуры

&НаКлиенте
Процедура КоллекцииСервисаПрогнозирования(Команда)
	ОткрытьФорму("РегистрСведений.КоллекцииСервисаПрогнозированияПродаж.ФормаСписка");
КонецПроцедуры

&НаКлиенте
Процедура СтатусыОбменаДанными(Команда)
	ОткрытьФорму("РегистрСведений.СтатусыОбменаДаннымиССервисомПрогнозированияПродаж.ФормаСписка");
КонецПроцедуры

&НаКлиенте
Процедура ИдентификаторыОбученияМоделей(Команда)
	ОткрытьФорму("РегистрСведений.СервисПрогнозированияИдентификаторыОбучения.ФормаСписка");
КонецПроцедуры

&НаСервере
Функция ПроверитьВыгрузкуДанных()
	Если СервисПрогнозирования.ИдетОбменДанными() Тогда
		ТекстОшибки = НСтр("ru = 'Активна выгрузка данных. До ее окончания запрещено обновление данных.'");
...
Процедура СтатусОбученияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ЗапуститьОбучение" Тогда
		
		Если Не ИспользоватьПланированиеПродаж
			И ТипПлана = ПредопределенноеЗначение("Перечисление.ТипыПланов.ПланПродаж") Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибкиНеИспользуетсяПланированиеПродажПоНоменклатуре());
			Возврат;
		КонецЕсли;
		Если Не ИспользоватьПланированиеПродажПоКатегориям
			И ТипПлана = ПредопределенноеЗначение("Перечисление.ТипыПланов.ПланПродажПоКатегориям") Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибкиНеИспользуетсяПланированиеПродажПоКатегориям());
			Возврат;
		КонецЕсли;
		
		ОбновитьПрогнозНаСервере();
		Если Не ПустаяСтрока(ИдентификаторЗаданияОбновитьПрогноз) Тогда
			ПодключитьОбработчикОжидания("Подключаемый_ОбновитьПрогнозНаКлиенте", 1, Истина);
		Иначе
			Подключаемый_ИндикаторВыгрузки();
		КонецЕсли;
		
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ЗагрузитьПрогноз" Тогда
		...
		Если Не ГотовКПолучению И Загружен Тогда
			Если Не ПроверитьВыгрузкуДанных() Тогда
				Возврат;
			КонецЕсли;
			...
			ОткрытьФорму("Обработка.ПанельУправленияСервисомПрогнозирования.Форма.ФормаВыбораМоделиПрогнозирования",
				ПараметрыФормы,
				ЭтотОбъект,
				,
				,
...
&НаКлиенте
Процедура ОтладочнаяИнформацияНажатие(Элемент)
	
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Информация", ОтладочнаяИнформацияПоВидуПлана());
...
Асинх Процедура Сформировать()
	...
	НастройкиКорректны = ПроверитьКорректностьИУстановитьПараметрыОтчета(); // Сервис прогнозирования.
...
&НаСервере
Процедура УстановитьПараметрДанныхКомпоновки(ИмяПараметра, ЗначениеПараметра, Использование = Истина)
	ДоступныйПараметрСценарий = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы.Найти(ИмяПараметра);
	Если ДоступныйПараметрСценарий <> Неопределено Тогда
		ЗаданныйПараметрСценарий = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти(ИмяПараметра);
		Если ЗаданныйПараметрСценарий <> Неопределено Тогда
			ЗаданныйПараметрСценарий.Значение = ЗначениеПараметра;
			ЗаданныйПараметрСценарий.Использование = Использование;
		Иначе
			НовыйПараметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйПараметр, ДоступныйПараметрСценарий);
...
&НаСервере
Функция ПроверитьКорректностьНастроекСервиса()
	...
	Если Не ЗначениеЗаполнено(НастройкиСервиса.ТокенПриложения)
		...
		ТекстОшибки = НСтр("ru = 'Не произведена регистрация. Используйте мастер подключения к сервису.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
&НаСервере
Функция ПроверитьКорректностьИУстановитьПараметрыОтчета()
	
	НастройкиКорректны = ПроверитьКорректностьНастроекСервиса();
	Если Не НастройкиКорректны Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура УстановитьПараметрыОтчета()
	...
	УстановитьПараметрДанныхКомпоновки("Сценарий", СценарийПрогнозирования);
	УстановитьПараметрДанныхКомпоновки("ВидПлана", ВидПлана);
	УстановитьПараметрДанныхКомпоновки("НачалоПериода", Период.ДатаНачала, ЗначениеЗаполнено(Период.ДатаНачала));
	УстановитьПараметрДанныхКомпоновки("КонецПериода", Период.ДатаОкончания, ЗначениеЗаполнено(Период.ДатаОкончания));
	УстановитьПараметрДанныхКомпоновки("СценарийПодменыПродаж", НастройкиСервиса.СценарийПрогнозированияФактИПлан,
		ЗначениеЗаполнено(НастройкиСервиса.СценарийПрогнозированияФактИПлан));
	
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		ДатаНачалаПрогнозирования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПлана, "НачалоПрогнозирования");
		ПериодичностьВидаПлана = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПлана, "Периодичность");
		ДатаНачалаВыводаФактаПродаж = ДобавитьМесяц(ДатаНачалаПрогнозирования, -1);
		УстановитьПараметрДанныхКомпоновки("ДатаНачалаПрогнозирования", ДатаНачалаПрогнозирования, Истина);
		УстановитьПараметрДанныхКомпоновки("ДатаОкончанияВыводаФактаПродаж", ДатаНачалаПрогнозирования, Истина);
		УстановитьПараметрДанныхКомпоновки("ДатаНачалаВыводаФактаПродаж", ДатаНачалаВыводаФактаПродаж, Истина);
		УстановитьПараметрДанныхКомпоновки("ПериодичностьОтчета", ПериодичностьВидаПлана, Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПараметрДетализацииПоУмолчанию()
	
	Если Не ЗначениеЗаполнено(ВидПлана) Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияДетализаций = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПлана, "ЗаполнятьСкладВТЧ,
		|ЗаполнятьПартнераВТЧ, ЗаполнятьПоХарактеристикамНоменклатуры");
	ИспользоватьДетализацию = Ложь;
	ДетализацияПоУмолчанию  = "ПоСкладам";
	
	Если ТипПлана = Перечисления.ТипыПланов.ПланПродаж Тогда
		
		Если ЗначенияДетализаций.ЗаполнятьПоХарактеристикамНоменклатуры
			И ЗначенияДетализаций.ЗаполнятьСкладВТЧ
			И ЗначенияДетализаций.ЗаполнятьПартнераВТЧ Тогда
			
			ДетализацияПоУмолчанию  = "ПоСкладамКлиентамХарактеристикам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьПоХарактеристикамНоменклатуры
			И ЗначенияДетализаций.ЗаполнятьПартнераВТЧ Тогда
			
			ДетализацияПоУмолчанию  = "ПоКлиентамХарактеристикам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьПоХарактеристикамНоменклатуры
			И ЗначенияДетализаций.ЗаполнятьСкладВТЧ Тогда
			
			ДетализацияПоУмолчанию  = "ПоСкладамХарактеристикам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьСкладВТЧ
			И ЗначенияДетализаций.ЗаполнятьПартнераВТЧ Тогда
			
			ДетализацияПоУмолчанию  = "ПоСкладамКлиентам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьПоХарактеристикамНоменклатуры Тогда
			
			ДетализацияПоУмолчанию  = "ПоХарактеристикам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьПартнераВТЧ Тогда
			
			ДетализацияПоУмолчанию  = "ПоКлиентам";
			ИспользоватьДетализацию = Истина;
			
		ИначеЕсли ЗначенияДетализаций.ЗаполнятьСкладВТЧ Тогда
			ИспользоватьДетализацию = Истина;
		КонецЕсли;
		
	Иначе
		
		Если ЗначенияДетализаций.ЗаполнятьСкладВТЧ Тогда
			ИспользоватьДетализацию = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	УстановитьПараметрДанныхКомпоновки("Детализация", ДетализацияПоУмолчанию, ИспользоватьДетализацию);
...
Функция ПоляУниверсальногоПоискаПоТипам(АдресСхемы, Знач Настройки)
	...
	// Сервис прогнозирования.
	ПараметрДетализации = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(
		Новый ПараметрКомпоновкиДанных("Детализация"));
	Если ПараметрДетализации <> Неопределено И ПараметрДетализации.Использование Тогда
		ДополнитьПоляУниверсальногоПоискаПоПараметруДетализации(ПараметрДетализации.Значение, ПоляПоискаПоТипам);
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ДополнитьПоляУниверсальногоПоискаПоПараметруДетализации(ВариантДетализации, ПоляПоискаПоТипам)
	
	Если СтрНайти(ВариантДетализации, "Складам") Тогда
		ЗаголовокПоля = НСтр("ru = 'Склад'");
		ПоляПоТипу = Новый Соответствие;
		ПоляПоТипу.Вставить(Новый ПолеКомпоновкиДанных("Склад"), ЗаголовокПоля);
		ПоляПоискаПоТипам.Вставить(Тип("СправочникСсылка.Склады"), ПоляПоТипу);
	КонецЕсли;
	Если СтрНайти(ВариантДетализации, "Клиентам") Тогда
		ЗаголовокПоля = НСтр("ru = 'Клиент'");
		ПоляПоТипу = Новый Соответствие;
		ПоляПоТипу.Вставить(Новый ПолеКомпоновкиДанных("Партнер"), ЗаголовокПоля);
		ПоляПоискаПоТипам.Вставить(Тип("СправочникСсылка.Партнеры"), ПоляПоТипу);
	КонецЕсли;
	Если СтрНайти(ВариантДетализации, "Характеристикам") Тогда
		ЗаголовокПоля = НСтр("ru = 'Характеристика'");
		ПоляПоТипу = Новый Соответствие;
		ПоляПоТипу.Вставить(Новый ПолеКомпоновкиДанных("Характеристика"), ЗаголовокПоля);
		ПоляПоискаПоТипам.Вставить(Тип("СправочникСсылка.ХарактеристикиНоменклатуры"), ПоляПоТипу);
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность()
	...
	// Заголовок.
	ШаблонЗаголовка = НСтр("ru = 'Сервис прогнозирования продаж (%1)'"); // Сервис прогнозирования.
	...
	Если ЗначениеЗаполнено(ОтчетНаименованиеТекущегоВарианта) Тогда
		Заголовок = СтрШаблон(ШаблонЗаголовка, ОтчетНаименованиеТекущегоВарианта); // Сервис прогнозирования.
	Иначе
		ОписаниеНастроекОтчета = ОписаниеНастроекОтчета(НастройкиОтчета);
		Заголовок = ОписаниеНастроекОтчета.Наименование;
	КонецЕсли;
	
	Если ИдетВыгрузкаДанных Тогда // Сервис прогнозирования.
		Элементы.СтатусОбучения.Видимость = Ложь;
		Элементы.ИндикаторВыгрузки.Видимость = Истина;
	Иначе
		Элементы.СтатусОбучения.Видимость = Не ИзмененАПИСервиса;
		Элементы.ИндикаторВыгрузки.Видимость = Ложь;
	КонецЕсли;

	// Сервис прогнозирования.
	Если ПереключательИнформацииИзмененияИнтерфейса = 0 Тогда
		Элементы.ГруппаЛинияИнформацияИзмененияИнтерфейса.Видимость = Ложь;
	...
	ИначеЕсли ПереключательИнформацииИзмененияИнтерфейса = 1 Тогда
		Элементы.ГруппаЛинияИнформацияИзмененияИнтерфейса.Видимость = Истина;
	...
	Элементы.МастерПодключенияКСервисуШапка.Видимость = ДоступныПраваАдминистратораСервисаПрогнозированияПродаж;
	Элементы.МастерПодключенияКСервису.Видимость = ДоступныПраваАдминистратораСервисаПрогнозированияПродаж;
	Элементы.КоманднаяПанельСервисаПрогнозированияКомандыАдминистратора.Видимость = ДоступныПраваАдминистратораСервисаПрогнозированияПродаж;
...
Процедура ОбновитьКомандыВыбораВариантов()
	...
	ТекущийВариантОтчетаНедоступен = ЭтоНедоступныйВариантОтчета(КлючТекущегоВарианта); // Сервис прогнозирования.
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		// Сервис прогнозирования.
		Если ТекущийВариантОтчетаНедоступен Тогда
			КлючТекущегоВарианта = СтрокаТаблицы.КлючВарианта;
			ТекущийВариантОтчетаНедоступен = Ложь;
		КонецЕсли;
...
// Сервис прогнозирования.
&НаСервере
Процедура ОбновитьНастройкиКДПоТекущемуВариантуОтчета()
	
	СхемаКомпоновкиДанных = ПолучитьИзВременногоХранилища(НастройкиОтчета.АдресСхемы);
	
	Если КлючТекущегоВарианта = "Основной" Тогда
		Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.ВариантыНастроек.Основной.Настройки);
	ИначеЕсли КлючТекущегоВарианта = "ПрогнозПоКатегориям" Тогда
		Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.ВариантыНастроек.ПрогнозПоКатегориям.Настройки);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьИнформациюПоСценарию()
	...
	Статусы = СервисПрогнозирования.СтатусОбученияМодели(СценарийПрогнозирования, ВидПлана,
		Не ПустаяСтрока(ИдентификаторЗаданияОбновитьПрогноз),
		Не ПустаяСтрока(ИдентификаторЗаданияЗагрузитьПрогноз));
	СтатусОбучения = Статусы.СтатусПрогнозаИОбучения;
	ГотовКПолучению = Статусы.ГотовКПолучению;
	Загружен = Статусы.Загружен;
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьДанныеИзСервисаНаСервере(ИмяМодели = Неопределено)
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		РезультатРасчета = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
			"Обработки.ПанельУправленияСервисомПрогнозирования.ЗагрузитьДанныеИзСервиса",
			ВидПлана, ГотовКПолучению Или Загружен, ИмяМодели);
		
		Если РезультатРасчета.Статус = "Выполняется" Тогда
			ИдентификаторЗаданияЗагрузитьПрогноз = РезультатРасчета.ИдентификаторЗадания;
			АдресХранилищаЗагрузитьПрогноз = РезультатРасчета.АдресРезультата;
		ИначеЕсли РезультатРасчета.Статус = "Выполнено" Тогда
			АдресХранилищаЗагрузитьПрогноз = РезультатРасчета.АдресРезультата;
			Подключаемый_ЗагрузитьПрогнозНаСервере(Истина);
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьПрогнозНаСервере()
	
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		
		Если СервисПрогнозирования.ИдетОбменДанными() Тогда
			ТекстОшибки = НСтр("ru = 'Активна выгрузка данных. До ее окончания запрещен запуск прогнозирования.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
			Возврат;
		КонецЕсли;
		...
		НастройкиСервиса = СервисПрогнозирования.ПолучитьНастройкиСервиса();
		ТребуемыеРеквизиты = "НачалоПрогнозирования, ЗаполнятьПартнераВТЧ, ЗаполнятьПоХарактеристикамНоменклатуры";
		...
		Иначе
			...
			Если РеквизитыВидаПлана.ЗаполнятьПартнераВТЧ
				И Не КоллекцияПродажи.ВложенноеОписание["КодКлиента"].Выгружать Тогда
					ТекстОшибки = НСтр("ru = 'Прогнозирование с детализацией до клиента невозможна, поскольку в сервис прогнозирования не выгружена информация о кодах покупателей.
			...
			Если РеквизитыВидаПлана.ЗаполнятьПоХарактеристикамНоменклатуры
				И Не КоллекцияПродажи.ВложенноеОписание["КодХарактеристики"].Выгружать Тогда
					ТекстОшибки = НСтр("ru = 'Прогнозирование с детализацией до характеристик номенклатуры невозможна, поскольку в сервис прогнозирования не выгружена информация о кодах характеристик.
		...
		ТекстОшибки = СервисПрогнозирования.ПроверитьКонфигурацииКоллекций(НастройкиСервиса);
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
			Возврат;
		КонецЕсли;
		...
		НоваяСтрока.Дата   = НачалоДня(РеквизитыВидаПлана.НачалоПрогнозирования);
		...
		РезультатРасчета = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
			"Обработки.ПанельУправленияСервисомПрогнозирования.ОбновитьПрогнозНаСервере",
			ВидПлана);
		
		Если РезультатРасчета.Статус = "Выполняется" Тогда
			ИдентификаторЗаданияОбновитьПрогноз = РезультатРасчета.ИдентификаторЗадания;
			АдресХранилищаОбновитьПрогноз = РезультатРасчета.АдресРезультата;
		ИначеЕсли РезультатРасчета.Статус = "Выполнено" Тогда
			АдресХранилищаОбновитьПрогноз = РезультатРасчета.АдресРезультата;
			Подключаемый_ОбновитьПрогнозНаСервере(Истина);
		КонецЕсли;
		ОбновитьИнформациюПоСценарию();
...
&НаСервере
Процедура СценарийПрогнозированияОбработкаВыбораНаСервере(ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура ВидПланаПриИзмененииНаСервере()
	...
	Если ВидПланаИсходный <> ВидПлана Тогда
		ТипПлана = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПлана, "ТипПлана");
		ИдентификаторЗаданияЗагрузитьПрогноз = "";
		ИдентификаторЗаданияОбновитьПрогноз = "";
		ИзменитьВариантОтчетаПоТипуВидаПлана();
		УстановитьПараметрДетализацииПоУмолчанию();
	КонецЕсли;
	...
	ОбновитьКомандыВыбораВариантов();
	УстановитьВидимостьДоступность();
	ВидПланаИсходный = ВидПлана;
	
КонецПроцедуры

&НаСервере
Процедура ИзменитьВариантОтчетаПоТипуВидаПлана()
	
	Если ТипПлана = Перечисления.ТипыПланов.ПланПродаж Тогда
		
		КлючиВариантовПоТоварам = Новый Массив();
		КлючиВариантовПоТоварам.Добавить("Основной");
		КлючиВариантовПоТоварам.Добавить("ФактПрогноз");
		КлючиВариантовПоТоварам.Добавить("ФактПрогнозРаздельно");
		КлючиВариантовПоТоварам.Добавить("СравнениеСПланом");
		
		Если КлючиВариантовПоТоварам.Найти(КлючТекущегоВарианта) = Неопределено Тогда
			
			КлючТекущегоВарианта = "Основной";
			ОтчетНаименованиеТекущегоВарианта = НСтр("ru = 'Прогноз'");
			ОбновитьНастройкиКДПоТекущемуВариантуОтчета();
			ОбновитьИнформациюОВариантеОтчета();
	...
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям Тогда
		...
		КлючиВариантовПоКатегориям = Новый Массив();
		КлючиВариантовПоКатегориям.Добавить("ПрогнозПоКатегориям");
		КлючиВариантовПоКатегориям.Добавить("ФактПрогнозПоКатегориям");
		КлючиВариантовПоКатегориям.Добавить("ФактПрогнозПоКатегориямРаздельно");
		КлючиВариантовПоКатегориям.Добавить("СравнениеСПланомПоКатегориям");
		
		Если КлючиВариантовПоКатегориям.Найти(КлючТекущегоВарианта) = Неопределено Тогда
			
			КлючТекущегоВарианта = "ПрогнозПоКатегориям";
			ОтчетНаименованиеТекущегоВарианта = НСтр("ru = 'Прогноз по категориям'");
			ОбновитьНастройкиКДПоТекущемуВариантуОтчета();
			ОбновитьИнформациюОВариантеОтчета();
...
&НаСервере
Функция ОтладочнаяИнформацияПоВидуПлана()
	
	ПараметрыШаблона = Новый Массив();
	
	ШаблонОтвета = НСтр("ru = 'Аутентификационные данные:
	|%1
	|========================================
	|Настройки моделей (могут отличаться от данных на сервере):
	|%2
	|========================================
	|Общая информация (может отличаться от данных на сервере):
	|Последняя известная дата продаж (без учета отборов): %3
	...
	ШаблонАутентификационныеДанные = НСтр("ru = 'Версия: %1
	|Токены заданы: %2
	|Дата обновления токенов: %3
	|Идентификатор (application_uuid): %4'");
	...
	// Модели (виды планов).
	ИнформацияОПоследнихМоделях = РегистрыСведений.СервисПрогнозированияИдентификаторыОбучения.ПолнаяИнформацияОПоследнихМоделях(10);
	ТаблицаМоделейСтрокой = СервисПрогнозирования.ПредставитьТаблицуСтрокой(ИнформацияОПоследнихМоделях);
	ПараметрыШаблона.Добавить(ТаблицаМоделейСтрокой);
	
	// Общая информация.
	ДатаОкончанияПродаж = СервисПрогнозированияПереопределяемый.ПолучитьДатыНачалаОкончанияПродаж().ДатаОкончанияПродаж;
	ПараметрыШаблона.Добавить(ДатаОкончанияПродаж);
	...
	Для Каждого КлючЗначение Из ВсеКоллекции Цикл
		ИнфоОКоллекцияхСтрокой = ИнфоОКоллекцияхСтрокой
			+ СтрШаблон(ШаблонСтрокиПараметровМодели, КлючЗначение.Ключ, Строка(КлючЗначение.Значение.ИдКоллекции))
			+ Символы.ПС;
	КонецЦикла;
	...
	ИнформацияОПоследнихЗаписяхСтрокой = СервисПрогнозирования.ПредставитьТаблицуСтрокой(ИнформацияОПоследнихЗаписях);
...
&НаКлиентеНаСервереБезКонтекста
Функция ТекстОшибкиНеИспользуетсяПланированиеПродажПоНоменклатуре()
	
	Возврат НСтр("ru = 'Прогнозирование по номенклатуре невозможно, поскольку в информационной базе не используется планирование продаж по номенклатуре.
		|Требуется включить использование планов продаж в разделе НСИ и администрирование -
...
&НаКлиентеНаСервереБезКонтекста
Функция ТекстОшибкиНеИспользуетсяПланированиеПродажПоКатегориям()
	
	Возврат НСтр("ru = 'Прогнозирование по категориям невозможно, поскольку в информационной базе не используется планирование продаж по категориям.
		|Требуется включить использование планов продаж по категориям в разделе НСИ и администрирование -
		|Настройка НСИ и разделов - Бюджетирование и планирование - Планирование.'");
...
Процедура ПолучитьНастройкиСервиса()
	...
	ВыполненоПодключение = ЗначениеЗаполнено(Настройки.ТокенПриложения);
	ИзмененАПИСервиса    = Настройки.ИзмененАПИСервиса;
	
	// Снятие флага в связи с отсутствием необходимости в перерегистрации.
	Если ИзмененАПИСервиса
		И Не ВыполненоПодключение Тогда
		ИзмененАПИСервиса = Ложь;
	КонецЕсли;
...
&НаСервере
Функция ЭтоНедоступныйВариантОтчета(КлючВарианта)
	...
	Если ТипПлана = Перечисления.ТипыПланов.ПланПродаж Тогда
		Возврат КлючиВариантовПоТоварам.Найти(КлючВарианта) = Неопределено;
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям Тогда
		Возврат КлючиВариантовПоКатегориям.Найти(КлючВарианта) = Неопределено;
	Иначе
		Возврат Истина; // Сервис прогнозирования.
	КонецЕсли;
