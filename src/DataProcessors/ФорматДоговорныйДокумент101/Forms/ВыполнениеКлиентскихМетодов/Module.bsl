...
// Параметры:
//  ПомещенныйФайл - см. НовыеПараметрыФайла
//  ПараметрыФормы - см. ПараметрыПодготовкиФайла
//
&НаКлиенте
Процедура ЗагрузитьИзФайлаЗавершение(ПомещенныйФайл, ПараметрыФормы) Экспорт
	
	Если ПомещенныйФайл = Неопределено Или ПроверкаРасширенияФайлаПровалена(ПомещенныйФайл.Имя) Тогда
		Возврат;
	КонецЕсли;
	...
	СтруктураСАдресомХранения = Новый Структура("АдресПДФКонтейнера", ПомещенныйФайл.Хранение);
	ДействияПередОткрытиемФормы.Вставить("ОбернутьДанныеСообщенияВПДФ", СтруктураСАдресомХранения);
...
// Параметры:
//  ПомещенныйФайл  - см. НовыеПараметрыФайла
//  ПараметрКоманды - см. ПараметрыПодготовкиФайла
//
&НаКлиенте
Процедура ЗапроситьФайлЗавершение(ПомещенныйФайл, ПараметрКоманды) Экспорт
