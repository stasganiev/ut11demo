...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ВсеОповещения.СкопированыВсеКатегории Тогда
		Если ТипЗнч(Параметр) = ТипСтруктура Тогда
			ЗаполнитьБуферОбменаКатегорийСервер(Параметр);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = ВсеОповещения.ПослеЗаписиНовости Тогда
...
//
Процедура ЗаполнитьБуферОбменаКатегорийСервер(Знач БуферыОбмена)

	БуферыОбменаЭталон = Новый Структура;
		БуферыОбменаЭталон.Вставить("БуферОбмена_КатегорииПростые"        , Новый СписокЗначений);
		БуферыОбменаЭталон.Вставить("БуферОбмена_КатегорииИнтервалыВерсий", Новый СписокЗначений);

	ЗаполнитьЗначенияСвойств(БуферыОбменаЭталон, БуферыОбмена);

	ЭтотОбъект.БуферОбмена_КатегорииПростые.Загрузить(БуферыОбменаЭталон.БуферОбмена_КатегорииПростые.Выгрузить());
	ЭтотОбъект.БуферОбмена_КатегорииИнтервалыВерсий.Загрузить(БуферыОбменаЭталон.БуферОбмена_КатегорииИнтервалыВерсий.Выгрузить());
...
Процедура ПересчетСлужебныхДанныхНаСервере()
	...
	НаименованиеМетода = "Новости.ФормаСпискаДляРедактирования.ПересчетСлужебныхДанныхНаСервере"; // Идентификатор.

	КонтекстВыполнения = ОбработкаНовостейКлиентСервер.НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполненияВложенный = ОбработкаНовостейКлиентСервер.НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Запуск:" + НаименованиеМетода,
		НаименованиеМетода);
	...
	ШагВыполнения = ОбработкаНовостейКлиентСервер.ЗавершитьШагВыполнения(КонтекстВыполнения, 0, "", КонтекстВыполненияВложенный);
