...
#Область ЗаполнитьТаблицуНовостей
	...
	Иначе
		...
		ВременнаяТаблицаНовостей = ОбработкаНовостейСлужебный.ПолучитьКонтекстныеНовости(
	...
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьТаблицуКонтекстныхНовостей(
		?(ПустаяСтрока(Параметры.ИмяМетаданных), Неопределено, Параметры.ИмяМетаданных),
		?(ПустаяСтрока(Параметры.ИмяФормы), Неопределено, Параметры.ИмяФормы),
		?(ПустаяСтрока(Параметры.ИмяСобытия), Неопределено, Параметры.ИмяСобытия),
...
#КонецОбласти
	...
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьФормуКонтекстныхНовостейПриСозданииНаСервере(
		ЭтотОбъект,
		Отказ,
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтотОбъект.ТаблицаНовостей.Количество() <= 0 Тогда
		...
		ИнтеграцияПодсистемБИПКлиент.ПереопределитьНадписиСообщенияПриОтсутствииКонтекстныхНовостей(
			ЭтотОбъект,
			ТекстСообщения,
			ПояснениеСообщения,
			Отказ);
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтеграцияПодсистемБИП.ДополнительноУстановитьУсловноеОформлениеФормыКонтекстныхНовостей(ЭтотОбъект, УсловноеОформление);
...
#Область ИнициализацияМетода
	...
	Если ПустаяСтрока(СортировкаСпискаНовостей) Тогда
		КолонкиСортировки = "ЭтоПостояннаяНовость УБЫВ, ДатаПубликации УБЫВ";
	Иначе
