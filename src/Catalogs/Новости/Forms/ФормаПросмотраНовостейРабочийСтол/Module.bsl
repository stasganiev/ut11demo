...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьФормуПросмотраНовостейДляРабочегоСтолаПриСозданииНаСервере(
		ЭтотОбъект,
	...
	ВозвращаемыеЗначенияЭталон = Новый Структура;
		ВозвращаемыеЗначенияЭталон.Вставить("ПропуститьЗаполнениеНовостями", Ложь);
	Если ТипЗнч(ВозвращаемыеЗначения) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(ВозвращаемыеЗначенияЭталон, ВозвращаемыеЗначения);
		Если ВозвращаемыеЗначенияЭталон.ПропуститьЗаполнениеНовостями = Истина Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.ОбновленыНастройкиЧтенияНовостей Тогда
...
&НаСервере
Процедура ЗавершитьПолучениеНовостейВФонеНаСервере(Знач ФоновоеЗадание)
...
// Возвращает правильно заполненную структуру фонового задания или Неопределено (если задание больше не выполняется).
//
// Параметры:
//  СостояниеФоновогоЗаданияПолученияНовостей - Неопределено, Структура - см. ДлительныеОперации.ВыполнитьВФоне.
//
// Возвращаемое значение:
//   Неопределено, Структура - см. ДлительныеОперации.ВыполнитьВФоне (возвращаемое значение).
//
...
Функция ПолучитьФоновоеЗаданиеПолученияНовостей(СостояниеФоновогоЗаданияПолученияНовостей)
	...
	Если ТипЗнч(СостояниеФоновогоЗаданияПолученияНовостей) <> ТипСтруктура Тогда
		Возврат Неопределено;
	КонецЕсли;

	СостояниеФоновогоЗаданияПолученияНовостейЭталон = Новый Структура;
		СостояниеФоновогоЗаданияПолученияНовостейЭталон.Вставить("Статус"              , Неопределено);
		СостояниеФоновогоЗаданияПолученияНовостейЭталон.Вставить("ИдентификаторЗадания", Неопределено);
		СостояниеФоновогоЗаданияПолученияНовостейЭталон.Вставить("АдресРезультата"     , Неопределено);
	ЗаполнитьЗначенияСвойств(СостояниеФоновогоЗаданияПолученияНовостейЭталон, СостояниеФоновогоЗаданияПолученияНовостей);

	Если СостояниеФоновогоЗаданияПолученияНовостейЭталон.Статус <> Неопределено
			И СостояниеФоновогоЗаданияПолученияНовостейЭталон.ИдентификаторЗадания <> Неопределено
			И СостояниеФоновогоЗаданияПолученияНовостейЭталон.АдресРезультата <> Неопределено Тогда
		// Другие проверки не выполняем. Считаем, что в параметре уже есть все необходимые ключи.
		Результат = СостояниеФоновогоЗаданияПолученияНовостейЭталон;
	КонецЕсли;
