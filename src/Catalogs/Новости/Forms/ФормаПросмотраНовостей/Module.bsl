...
&НаКлиенте
Перем СпособОповещенияСистемаВзаимодействия;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураНастроекПоказаНовостейНастроенныеАдминистратором = ОбработкаНовостейСлужебный.ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором();
		...
		Если ПапкаНайдена = Истина Тогда
			ИнтерактивныеДействия = ОбновитьСписокНовостейСервер( // Здесь получается список новостей.
				Новый Структура("ВариантОтбора",
					ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора)); // Здесь же вызовется ОбновитьИнформационныеСтрокиСервер();
		КонецЕсли;
	ИначеЕсли (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 6) Тогда // Прочитать позже.
		ЭтотОбъект.СписокРаскрытыхПапок.ЗагрузитьЗначения(СтруктураНастроекПоказаНовостей.СписокРаскрытыхПапок.ВыгрузитьЗначения());
		Элементы.Папки.ТекущаяСтрока = 0;
		ПапкаНайдена = Ложь;
		Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
	...
	ИнтеграцияПодсистемБИП.ДополнительноОбработатьФормуПросмотраНовостейПриСозданииНаСервере(
		ЭтотОбъект,
		Отказ,
...
Процедура ПриОткрытии(Отказ)
	...
	СпособОповещенияСистемаВзаимодействия = ПредопределенноеЗначение("Перечисление.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ВсеОповещения.НовостьПрочтена Тогда
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.ПоказатьОповещение Тогда // ////? Кто создает это событие?
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.ИзмененаПометкаСпискаНовостей Тогда
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.ИзмененаПрочтенностьСпискаНовостей Тогда
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.АктивизироватьПапкуОтбора Тогда
		ПараметрЭталон = Новый Структура;
			ПараметрЭталон.Вставить("ВариантОтбора" , Неопределено);
			ПараметрЭталон.Вставить("ЗначениеОтбора", Неопределено);
		Если ТипЗнч(Параметр) = ТипСтруктура Тогда
			ЗаполнитьЗначенияСвойств(ПараметрЭталон, Параметр);
		КонецЕсли;
		Если ПараметрЭталон.ВариантОтбора <> Неопределено
				И ПараметрЭталон.ЗначениеОтбора <> Неопределено Тогда
			ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора  = ПараметрЭталон.ВариантОтбора;
			ЭтотОбъект.ТекущаяСтрокаПапки_ЗначениеОтбора = ПараметрЭталон.ЗначениеОтбора;
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.АктивизироватьОткрытуюФормуСпискаНовостей Тогда
		ЭтотОбъект.Активизировать();

	ИначеЕсли ИмяСобытия = ВсеОповещения.ЗагруженыНовости Тогда
		Если Источник <> ЭтотОбъект.УникальныйИдентификатор Тогда
			КомандаОбновитьСписокНовостей(Неопределено);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = ВсеОповещения.ИзмененоСостояниеПодпискиНаЛентуНовостей Тогда
		Если Источник = ЭтотОбъект.УникальныйИдентификатор Тогда // Инициировано в этой же форме, т.е. уже обработано.
			Возврат;
		КонецЕсли;
		ПараметрЭталон = Новый Структура;
			ПараметрЭталон.Вставить("ЛентаНовостей" , Неопределено);
			ПараметрЭталон.Вставить("Значение"      , Неопределено);
			ПараметрЭталон.Вставить("СохранятьВБазе", Неопределено);
		Если ТипЗнч(Параметр) = ТипСтруктура Тогда
			ЗаполнитьЗначенияСвойств(ПараметрЭталон, Параметр);
		КонецЕсли;
		Если ПараметрЭталон.ЛентаНовостей <> Неопределено
				И ПараметрЭталон.Значение <> Неопределено
				И ПараметрЭталон.СохранятьВБазе <> Неопределено Тогда
			ИзменитьПодпискуНаКлиенте(ПараметрЭталон.ЛентаНовостей, ПараметрЭталон.Значение, ПараметрЭталон.СохранятьВБазе);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = ВсеОповещения.ИзмененоСостояниеОповещенияЛентыНовостей Тогда
		Если Источник = ЭтотОбъект.УникальныйИдентификатор Тогда // Инициировано в этой же форме, т.е. уже обработано.
			Возврат;
		КонецЕсли;
		ПараметрЭталон = Новый Структура;
		...
		Если ПараметрЭталон.ЛентаНовостей <> Неопределено
				И ПараметрЭталон.Значение <> Неопределено
				И ПараметрЭталон.СохранятьВБазе <> Неопределено Тогда
			ИзменитьСпособОповещенияНаКлиенте(ПараметрЭталон.ЛентаНовостей, ПараметрЭталон.Значение, ПараметрЭталон.СохранятьВБазе);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = ВсеОповещения.ИзмененПризнакПрочитатьПозже Тогда
		Если Источник = ЭтотОбъект.УникальныйИдентификатор Тогда // Инициировано в этой же форме, т.е. уже обработано.
			Возврат;
		КонецЕсли;
		// Если нажали "Изменить признак Прочитать позже" в любой папке, то в папке "Прочитать позже" появится новость,
		//  но визуально ничего не изменится.
		// Если нажали "Изменить признак Прочитать позже" в папке "Прочитать позже", то новость исчезнет
		//  из списка и надо обновить список новостей.
		Если (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 6) Тогда
			ЭтотОбъект.ТекущаяСтрокаПапки_ПринудительноеОбновление = Истина;
			ЭтотОбъект.ПодключитьОбработчикОжидания("Подключаемый_ПапкиПриАктивизацииСтрокиОтложенно", 0.2, Истина);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = ВсеОповещения.ОткрытьФормуНастроек Тогда
		ОткрытьФормуНастроек("СтраницаВнешнийВид", Неопределено);

	ИначеЕсли ИмяСобытия = ВсеОповещения.ОткрытьФормуНастроекНастройкиЛентНовостей Тогда
		ОткрытьФормуНастроек("СтраницаЛентыНовостей", Параметр);
...
Процедура ОбработкаПолученияНавигационнойСсылки(Ключ, Представление, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если Ключ = "All" Тогда
		СтандартнаяОбработка = Ложь;
		НавигационнаяСсылка = "e1cib/list/Справочник.Новости";
	ИначеЕсли Ключ = "Marked" Тогда
		СтандартнаяОбработка = Ложь;
		НавигационнаяСсылка = "e1cib/command/Справочник.Новости.Команда.КомандаСписокПомеченныхНовостей";
	ИначеЕсли Ключ = "Urgent" Тогда
		СтандартнаяОбработка = Ложь;
		НавигационнаяСсылка = "e1cib/command/Справочник.Новости.Команда.КомандаСписокОченьВажныхНовостейТребующихПрочтения";
	ИначеЕсли Ключ = "ReadLater" Тогда
		СтандартнаяОбработка = Ложь;
		НавигационнаяСсылка = "e1cib/command/Справочник.Новости.Команда.КомандаСписокНовостейПрочитатьПозже";
	ИначеЕсли Ключ = "Settings" Тогда
		СтандартнаяОбработка = Ложь;
		НавигационнаяСсылка = "e1cib/command/Справочник.Новости.Команда.КомандаСписокНовостейНастройки";
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОбработкаПолученияСпискаНавигационныхСсылок(СписокНавигационныхСсылок, КлючПоУмолчанию)

	СписокНавигационныхСсылок.Добавить("All"      , НСтр("ru = 'Новости: Все новости'")                  , Ложь);
	СписокНавигационныхСсылок.Добавить("Marked"   , НСтр("ru = 'Новости: Помеченные новости'")           , Ложь);
	СписокНавигационныхСсылок.Добавить("Urgent"   , НСтр("ru = 'Новости: Важные и очень важные новости'"), Ложь);
	СписокНавигационныхСсылок.Добавить("ReadLater", НСтр("ru = 'Новости: Прочитать позже'")              , Ложь);
	СписокНавигационныхСсылок.Добавить("Settings" , НСтр("ru = 'Новости: Окно настроек'")                , Ложь);
...
Процедура ПапкиПриАктивизацииСтроки(Элемент)
	...
	Если ПараметрыСеанса_ТекущийПользователь.Пустая() Тогда // Неразделенный режим - нельзя установить никакие настройки
		Элементы.НовостиКонтекстноеМенюГруппаПрочитатьПозже.Видимость = Ложь;
	...
	Иначе
		Элементы.НовостиКонтекстноеМенюГруппаПрочитатьПозже.Видимость = Истина;
	...
	Если (ТекущиеДанные.ВариантОтбора = 6) Тогда // "Прочитать позже"
		Элементы.НовостиКонтекстноеМенюКомандаПрочитатьПозжеДобавитьВСписок.Видимость = Ложь; // Уже в списке
		Элементы.НовостиКонтекстноеМенюКомандаПрочитатьПозжеУдалитьИзСписка.Видимость = Истина;
	Иначе
		Элементы.НовостиКонтекстноеМенюКомандаПрочитатьПозжеДобавитьВСписок.Видимость = Истина;
		Элементы.НовостиКонтекстноеМенюКомандаПрочитатьПозжеУдалитьИзСписка.Видимость = Ложь;
	КонецЕсли;
...
Процедура НовостиПередУдалением(Элемент, Отказ)
	...
	Если (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 0) // Все новости
			...
			ИЛИ (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 6) // Прочитать позже;
...
Процедура КомандаНастройкиПоказаНовостей(Команда)
	...
	ОткрытьФормуНастроек("СтраницаВнешнийВид");
...
&НаКлиенте
Процедура КомандаПрочитатьПозже(Команда)
	...
	Если Элементы.Новости.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		КонецЦикла;
		Если МассивНовостей.Количество() > 0 Тогда
			Если Команда.Имя = "КомандаПрочитатьПозжеУдалитьИзСписка" Тогда // Мы в папке "Прочитать позже" - сбросить признак.
				ОбработкаНовостейКлиент.ИзменитьПризнакПрочитатьПозже(МассивНовостей, Ложь);
				Если МассивНовостей.Количество() = 1 Тогда
					ТекстОповещения = НСтр("ru = 'Новость удалена из списка [Прочитать позже]'");
				Иначе // > 1
					ТекстОповещения = НСтр("ru = 'Новости удалены из списка [Прочитать позже]'");
				КонецЕсли;
				ПоказатьОповещениеПользователя(
					ТекстОповещения,
					,
					,
					БиблиотекаКартинок.ПометкаНовостиЗакладкаКонтур,
					СтатусОповещенияПользователя.Информация,
					"");
			Иначе // Мы в любой папке, кроме "Прочитать позже" - включить в список "Прочитать позже".
				ОбработкаНовостейКлиент.ИзменитьПризнакПрочитатьПозже(МассивНовостей, Истина);
				Если МассивНовостей.Количество() = 1 Тогда
					ТекстОповещения = НСтр("ru = 'Новость добавлена в список [Прочитать позже]'");
				Иначе // > 1
					ТекстОповещения = НСтр("ru = 'Новости добавлены в список [Прочитать позже]'");
				КонецЕсли;
				ПоказатьОповещениеПользователя(
					ТекстОповещения,
					,
					,
					БиблиотекаКартинок.ПометкаНовостиЗакладка,
					СтатусОповещенияПользователя.Информация,
					"");
			КонецЕсли;
		КонецЕсли;
...
#Область Обработка
	...
	ИначеЕсли (Команда.Имя = "КомандаОповещенияСистемаВзаимодействия")
			И (ТекущиеДанные.СпособОповещения <> "СистемаВзаимодействия") Тогда // Идентификатор.
		ИзменитьСпособОповещенияНаКлиенте(ЛентаНовостей, "СистемаВзаимодействия", Истина); // Идентификатор.
...
Функция ОбновитьСписокНовостейСервер(Знач ПараметрыПолученияНовостей)
	...
	// Чтобы не делать много колонок с иконками, будет выводиться только одна колонка.
	// Если новость - постоянная ("в топе"), то выводится иконка "постоянная новость".
	// Иначе, если новость - важная или очень важная, то выводится иконка "важности".
	// Иначе ничего не выводится.
	БиблиотекаКартинокНовостиВажная     = БиблиотекаКартинок.ВажностьНовостиВажная;
	БиблиотекаКартинокНовостиПостоянная = БиблиотекаКартинок.НовостиПостоянная;
	Для Каждого ТекущаяСтрока Из ЭтотОбъект.Новости Цикл
		Если ТекущаяСтрока.ЭтоПостояннаяНовость Тогда
			ТекущаяСтрока.НовостиИконка = БиблиотекаКартинокНовостиПостоянная;
		ИначеЕсли ТекущаяСтрока.Важность > 0 Тогда
			ТекущаяСтрока.НовостиИконка = БиблиотекаКартинокНовостиВажная;
		Иначе
			ТекущаяСтрока.НовостиИконка = Неопределено;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьСписокПапокОтбора()
	...
	// ////? {Количество непрочтенных новостей} Добавить в запрос проверку на количество показываемых
	//   последних новостей, чтобы количество непрочтенных новостей считалось точнее.
	Запрос = Новый Запрос;
			...
			НоваяСтрока = СтрокаВсе.ПолучитьЭлементы().Добавить();
				...
				Иначе
					...
					ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
						НоваяСтрока.СпособОповещения = "СистемаВзаимодействия"; // Идентификатор.
						НоваяСтрока.Иконка           = БиблиотекаКартинок.НовостиОповещенияСистемаВзаимодействия;
					Иначе
						НоваяСтрока.СпособОповещения = "ПоУмолчанию"; // Идентификатор.
	...
	// 6. Прочитать позже.
	НоваяСтрока = КорневойУровень.Добавить();
		НоваяСтрока.ВариантОтбора          = 6;
		НоваяСтрока.ЗначениеОтбора         = Неопределено;
		НоваяСтрока.Наименование           = НСтр("ru='Прочитать позже'");
		НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
		НоваяСтрока.ВыделеннаяСтрока       = Ложь;
		НоваяСтрока.Иконка                 = БиблиотекаКартинок.ПометкаНовостиЗакладка;
				...
				Пока Выборка.Следующий() Цикл
					НоваяСтрока = СтрокаВсе.ПолучитьЭлементы().Добавить();
						...
						Если Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда // Идентификатор.
							НоваяСтрока.СпособОповещения = "Отключено"; // Идентификатор.
						ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда // Идентификатор.
							НоваяСтрока.СпособОповещения = "СистемаВзаимодействия"; // Идентификатор.
...
Процедура ПапкиПриАктивизацииСтрокиСервер(Знач ТекущиеДанные)
	...
	ПараметрыПолученияНовостей = Новый Структура;
		ПараметрыПолученияНовостей.Вставить("Метаданные"   , "Справочник.Новости");
		ПараметрыПолученияНовостей.Вставить("Форма"        , "ФормаПросмотраНовостей");
		ПараметрыПолученияНовостей.Вставить("ВариантОтбора", ТекущиеДанные.ВариантОтбора);

	Если ТекущиеДанные.ВариантОтбора = 0 Тогда // Все
		ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
	ИначеЕсли ТекущиеДанные.ВариантОтбора = 1 Тогда // Лента новостей
		ПараметрыПолученияНовостей.Вставить("ЛентаНовостей", ТекущиеДанные.ЗначениеОтбора);
		ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
	ИначеЕсли ТекущиеДанные.ВариантОтбора = 2 Тогда // Результаты поиска
		...
		Иначе
			ПараметрыПолученияНовостей.Вставить("СписокНайденныхНовостей", ЭтотОбъект.СписокНайденныхНовостей);
			ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
		КонецЕсли;
	...
	ИначеЕсли ТекущиеДанные.ВариантОтбора = 4 Тогда // Помеченные новости
		ПараметрыПолученияНовостей.Вставить("Тег", "Пометка1");
		ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
	ИначеЕсли ТекущиеДанные.ВариантОтбора = 5 Тогда // Важные новости
		ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
	ИначеЕсли ТекущиеДанные.ВариантОтбора = 6 Тогда // Прочитать позже
		ОбновитьСписокНовостейСервер(ПараметрыПолученияНовостей);
...
Процедура ВосстановитьТекущееСостояниеПапокОтбораНовостей()
	...
	Для каждого ТекущийЭлементСписка Из ЭтотОбъект.СписокРаскрытыхПапок Цикл
		Если ТипЗнч(ТекущийЭлементСписка.Значение) = ТипСтруктура Тогда
			Продолжить;
		КонецЕсли;
		ТекущийЭлементСпискаЗначениеЭталон = Новый Структура;
			ТекущийЭлементСпискаЗначениеЭталон.Вставить("ВариантОтбора" , Неопределено);
			ТекущийЭлементСпискаЗначениеЭталон.Вставить("ЗначениеОтбора", Неопределено);
		ЗаполнитьЗначенияСвойств(ТекущийЭлементСпискаЗначениеЭталон, ТекущийЭлементСписка.Значение);
		...
		Для каждого ТекущаяПапка Из ВсеПапкиВерхнегоУровня Цикл
			Если (ТекущаяПапка.ВариантОтбора = ТекущийЭлементСпискаЗначениеЭталон.ВариантОтбора)
					И (ТекущаяПапка.ЗначениеОтбора = ТекущийЭлементСпискаЗначениеЭталон.ЗначениеОтбора) Тогда
...
Процедура ПослеНастройкиПоказаНовостей(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = ТипСтруктура Тогда // Нажали ОК, в противном случае вернется или Неопределено (нажали Esc), или КодВозвратаДиалога.Отмена.
		...
		Оповестить(
			ВсеОповещения.ОбновленыНастройкиЧтенияНовостей,
			Результат,
			ЭтотОбъект);
	КонецЕсли;
...
Процедура ПослеПоиска(Результат, ДополнительныеПараметры) Экспорт
	...
	РезультатЭталон = Новый Структура;
		РезультатЭталон.Вставить("НастройкиПоиска"        , Неопределено);
		РезультатЭталон.Вставить("МассивНайденныхНовостей", Неопределено);
	ЗаполнитьЗначенияСвойств(РезультатЭталон, Результат);

	Если ТипЗнч(РезультатЭталон.НастройкиПоиска) = ТипСписокЗначений Тогда
		ЭтотОбъект.НастройкиПоиска = РезультатЭталон.НастройкиПоиска;
	...
	Если ТипЗнч(РезультатЭталон.МассивНайденныхНовостей) = ТипМассив Тогда
		ЭтотОбъект.СписокНайденныхНовостей.ЗагрузитьЗначения(РезультатЭталон.МассивНайденныхНовостей);
	КонецЕсли;
...
Функция НайтиИАктивизироватьПапку(ЗначениеОтбора) Экспорт // АПК:78 Вызывается извне.
	...
	Если (ТипЗнч(ЗначениеОтбора) <> ТипСтруктура) Тогда
		Возврат Ложь;
	КонецЕсли;

	ЗначениеОтбораЭталон = Новый Структура;
		ЗначениеОтбораЭталон.Вставить("ВариантОтбора" , Неопределено);
		ЗначениеОтбораЭталон.Вставить("ЗначениеОтбора", Неопределено);
	ЗаполнитьЗначенияСвойств(ЗначениеОтбораЭталон, ЗначениеОтбора);

	Если ЗначениеОтбораЭталон.ВариантОтбора = Неопределено
			ИЛИ ЗначениеОтбораЭталон.ЗначениеОтбора = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	// Найти идентификатор строки по значениям отбора.
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		Если (ТекущаяПапка.ВариантОтбора = ЗначениеОтбораЭталон.ВариантОтбора)
				И (ТекущаяПапка.ВариантОтбора = ЗначениеОтбораЭталон.ЗначениеОтбора) Тогда
			Элементы.Папки.ТекущаяСтрока = ТекущаяПапка.ПолучитьИдентификатор();
			Результат = Истина;
			Прервать;
		Иначе
			...
			Для каждого ТекущаяПодПапка Из ТекущаяПапка.ПолучитьЭлементы() Цикл
				Если (ТекущаяПодПапка.ВариантОтбора = ЗначениеОтбораЭталон.ВариантОтбора)
						И (ТекущаяПодПапка.ЗначениеОтбора = ЗначениеОтбораЭталон.ЗначениеОтбора) Тогда
...
Процедура ИзменитьПодпискуНаКлиенте(Знач ЛентаНовостей, СостояниеПодписки, СохранятьВБазе)
	...
	Если СостояниеПодписки = Истина Тогда
		...
		Если (ТекущиеДанные.ПолучитьРодителя() <> Неопределено)
			...
			Иначе
				...
				ИначеЕсли СохраненнаяСтрока.СпособОповещения = "СистемаВзаимодействия" Тогда // Идентификатор.
					СохраненнаяСтрока.Иконка = БиблиотекаКартинок.НовостиОповещенияСистемаВзаимодействия;
				Иначе // "ПоУмолчанию"; // Идентификатор.
	...
	// Если в папке "Отключенные ленты новостей" ничего нет, то не показывать ее.
	Если ПапкаВсеОтписанные.Количество() = 0 Тогда
		Для Каждого ТекущаяПапкаДляУдаления Из КорневойУровень Цикл
			Если ТекущаяПапкаДляУдаления.ВариантОтбора = 9 Тогда
				КорневойУровень.Удалить(ТекущаяПапкаДляУдаления);
				Прервать; // Т.к. эта папка единственная, то дальше проверять не нужно.
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ИзменитьСпособОповещенияНаКлиенте(Знач ЛентаНовостей, СпособОповещения, СохранятьВБазе)
	...
	Если ТипЗнч(СпособОповещения) = ТипСпособыОповещенияПользователяОНовостях Тогда
		...
		ИначеЕсли СпособОповещения = СпособОповещенияСистемаВзаимодействия Тогда
			СпособОповещенияСтрока = "СистемаВзаимодействия";
		Иначе
	...
	ИначеЕсли (СпособОповещенияСтрока = "СистемаВзаимодействия")
			И (ТекущиеДанные.СпособОповещения <> "СистемаВзаимодействия") Тогда // Идентификатор.
		// По-умолчанию.
		ТекущиеДанные.СпособОповещения = "СистемаВзаимодействия"; // Идентификатор.
		ТекущиеДанные.Иконка           = БиблиотекаКартинок.НовостиОповещенияСистемаВзаимодействия;
		БылиИзменения = Истина;
...
&НаКлиенте
Процедура ОткрытьФормуНастроек(Страница="", ЛентаНовостей = Неопределено)
	...
	ПараметрыФормы = Новый Структура("Страница, ЛентаНовостей", Страница, ЛентаНовостей);
