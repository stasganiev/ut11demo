...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ДанныеДляПередачиНаКлиента) = ТипСтруктура Тогда
		Если ТипЗнч(ДанныеДляПередачиНаКлиента.СписокРазделовДляСвертки) = ТипСписокЗначений Тогда
			СписокРазделовДляСвертки = ДанныеДляПередачиНаКлиента.СписокРазделовДляСвертки;
		КонецЕсли;
		Если ТипЗнч(ДанныеДляПередачиНаКлиента.СписокРазделовДляМаркеровСостояния) = ТипСписокЗначений Тогда
...
//
Функция ПолучитьBase64ИзФайлаСервер(Знач АдресВХранилище)
...
//
Процедура ПослеВыбораФайла(ПомещенныйФайл, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СтруктураСохраненногоФайла = ПолучитьBase64ИзФайлаСервер(ПомещенныйФайл.Хранение);
