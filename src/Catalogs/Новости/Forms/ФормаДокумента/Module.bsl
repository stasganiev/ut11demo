...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.КатегорияПродукт         = М.КатегорияПродукт();
	ЭтотОбъект.КатегорияВерсияПродукта  = М.КатегорияВерсияПродукта();
	ЭтотОбъект.КатегорияВерсияПлатформы = М.КатегорияВерсияПлатформы();
...
Процедура ДействияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ЭтотОбъект.ПодключитьОбработчикОжидания("ЗаполнитьДобавленныеКолонкиВсехТаблицНаКлиенте", 0.1, Истина);
	Если НЕ ОтменаРедактирования Тогда
		ЭтотОбъект.НеобходимоПересчитатьСписокДействий = Истина;
	КонецЕсли;
...
Процедура КомандаЗагрузитьФайлКакБинарныеДанные(Команда)
	...
	Если Элементы.БинарныеДанные.ТекущиеДанные <> Неопределено Тогда
		...
		ПараметрыЗагрузки = ФайловаяСистемаКлиент.ПараметрыЗагрузкиФайла();
			...
			ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Изображения'") + "|*.JPG;*.JPEG;*.PNG;*.GIF";
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для Каждого ТекущееИмяКоманды Из ИменаКомандНовостейДляУдаления Цикл
			НайденнаяКоманда = ЭтотОбъект.Команды.Найти(ТекущееИмяКоманды);
			Если ТипЗнч(НайденнаяКоманда) = ТипКомандаФормы Тогда
				ЭтотОбъект.Команды.Удалить(НайденнаяКоманда);
			КонецЕсли;
		КонецЦикла;
