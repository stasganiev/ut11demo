...
#КонецОбласти
	...
	ЭтотОбъект.ТекстНовостей = ОбработкаНовостейСлужебный.ПолучитьХТМЛТекстНовостей(
...
//
Функция ЗаписатьИзменениеСостоянияНовостиСервер(Знач ЛокальныйСписокНовостей, Знач ЛокальноеОповещениеВключено, Знач ЛокальныйТекущийПользователь)

	СписокДанныхДляОчисткиКэшаКонтекстныхНовостей = Новый СписокЗначений;

	Для Каждого ТекущаяНовость Из ЛокальныйСписокНовостей Цикл
		Запись = РегистрыСведений.СостоянияНовостей.СоздатьМенеджерЗаписи();
		Запись.Пользователь = ЛокальныйТекущийПользователь;
		...
		Запись.ОповещениеВключено   = ЛокальноеОповещениеВключено;
		// Запись.ДатаНачалаОповещения Не трогать.
		Запись.Записать(Истина);
	КонецЦикла;

	Если ЛокальноеОповещениеВключено = Ложь Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|";
		Запрос.УстановитьПараметр("МассивНовостейДляОчисткиКэшаКонтекстныхНовостей", ЛокальныйСписокНовостей.ВыгрузитьЗначения());
