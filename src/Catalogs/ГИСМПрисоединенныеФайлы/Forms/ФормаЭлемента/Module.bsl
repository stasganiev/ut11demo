...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщенияXML = РаботаСXMLИС.ФорматироватьXMLСПараметрами(ТекстСообщенияXML, РаботаСXMLИС.ПараметрыФорматированияXML(Истина, "  "));
	...
	Если СертификатКриптографии = Неопределено Тогда
		АдресСертификата = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	Иначе
	...
	СтруктураЦветаСтиля.Вставить("ЦветТекстаТребуетВниманияГосИС", ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
	СтруктураЦветаСтиля.Вставить("ЦветГиперссылкиГосИС", ЦветаСтиля.ЦветГиперссылкиГосИС);
...
Процедура ПослеПроверкиПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат = Неопределено Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось получить менеджер криптографии'"));
