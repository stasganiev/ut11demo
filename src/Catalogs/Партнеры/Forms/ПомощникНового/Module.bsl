...
Процедура ПриИзмененииНомераСчетаЛокализация(ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ДенежныеСредстваКлиентСерверЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(ЭтотОбъект["НомерСчета" + Постфикс]) Тогда
...
Процедура ОпределитьБанкРФ(ЭтоГоловнойКонтрагент)
	...
	Если НЕ ЭтотОбъект["ЭтоIBAN" + Постфикс]
		И ДенежныеСредстваКлиентСерверЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(ЭтотОбъект["НомерСчета" + Постфикс]) Тогда
		ЭтотОбъект["ИностранныйБанк" + Постфикс] = Ложь;
	КонецЕсли;
