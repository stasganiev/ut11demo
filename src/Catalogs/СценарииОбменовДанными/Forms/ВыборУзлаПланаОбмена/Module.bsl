...
Процедура ПрочитатьДеревоУзловОбмена()
	...
	ОбменСообщениямиПустаяСсылка = ПланыОбмена["ОбменСообщениями"].ПустаяСсылка();
	...
	Пока ВыборкаПоТипамУзлов.Следующий() Цикл
		...
		Пока ВыборкаПоУзлам.Следующий() Цикл
			...
			НастройкиТранспора = ТранспортСообщенийОбмена.НастройкиТранспорта(ВыборкаПоУзлам.Ссылка, "SM");
			
			Если НастройкиТранспора.ВнутренняяПубликация Тогда 
