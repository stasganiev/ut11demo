...
Процедура НастроитьФормуЛокализация()
	...
	ЭтоПодконтрольнаяПродукцияВЕТИС = 
		Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
	...
	Элементы.НадписьЕстьОтборПолитикиПоУчетуОстатковПоСериямЕстьОрдерныеСкладыВЕТИС.Видимость =
		Объект.ИспользоватьСерии
		И Не Объект.НастройкиСерийБерутсяИзДругогоВидаНоменклатуры
