...
Функция СообщениеЗагрузкиПартииПоНомеру(НомерПартии, ДополнительныеПараметры) Экспорт
...
Функция СообщенияЗагрузкиПартий(ПараметрыФормирования, ПараметрыОбмена = Неопределено)
	...
	Если ПараметрыФормирования.Интервал <> Неопределено Тогда
		Интервал = ПараметрыФормирования.Интервал;
		Если ЗначениеЗаполнено(Интервал.НачалоПериода) Тогда
			ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЗапросПартийXDTO, "recordsModifiedFrom", Интервал.НачалоПериода, СообщениеXML);
...
Функция ПараметрыЗагрузкиПартии() Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа", Новый Соответствие());
...
Функция ЗагрузитьПартиюЗерна(ЭлементДанных, ПараметрыЗагрузкиПартии, СуществующийОбъект = Неопределено, ДанныеСопоставления = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СуществующийОбъект.ВладелецПартии) Тогда
		ПараметрыЗагрузкиПартии.ПроверитьЗаполнениеРеквизитовАдресИндекКодАльфа.Вставить(
