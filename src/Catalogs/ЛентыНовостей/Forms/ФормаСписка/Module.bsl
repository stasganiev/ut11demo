...
Процедура КомандаЗагрузитьНовости(Команда)
	...
	Если Команда.Имя = "КомандаЗагрузитьНовостиПоВсемЛентамНовостей" Тогда
		...
		Элементы.Список.Обновить();
		Оповестить(
			ОбработкаНовостейКлиент.ВсеОповещения().ЗагруженыНовости,
			,
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Справочник.ЛентыНовостей.ФормаСписка.ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылкиСервер"; // Идентификатор.
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru='БИП:Новости.Загрузка классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()); // ИмяСобытия.
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НаименованиеМетода + "/Обработки.УправлениеНовостями.ОбновитьСтандартныйСписокССервера");
	...
	НачатьШагВыполнения(КонтекстВыполнения, НаименованиеМетода + "/ОбработкаНовостейСлужебный.ОптимизироватьОтборыПоНовостям");
