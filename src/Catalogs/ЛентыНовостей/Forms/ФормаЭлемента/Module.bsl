...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
					...
					Если ЭтотОбъект.РазделениеВключено Тогда
						ТекстЗаголовка = ТекстЗаголовка
							...
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='по причине: %1 (разделение включено).'"),
								ПричинаОтключения);
					Иначе
						ТекстЗаголовка = ТекстЗаголовка
							+ " "
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='по причине: %1 (неразделенные данные).'"),
								ПричинаОтключения);
					КонецЕсли;
				КонецЕсли;
	...
	Иначе
		...
		Если Объект.ЗагруженоССервера = Ложь Тогда
			...
			МассивРедакторовЛентыНовостей = ОбработкаНовостейСлужебный.ПолучитьРедакторовЛентыНовостей(
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(
					НоваяСтрока.ПользовательПредставление,
					НСтр("ru='(полные права)'"),
					НСтр("ru='(полные права, может редактировать новости во всех лентах новостей)'"));
			КонецЦикла;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок
				= НСтр("ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)'");
		КонецЕсли;
	...
	// - изменять возможность пользователей отключать / включать ленту новостей.
	...
	Иначе
		Если НЕ ЭтотОбъект.ТолькоПросмотр Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Элементы.ОбязательныйКанал.Доступность = Ложь;
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить(
		ОбработкаНовостейКлиент.ВсеОповещения().ПослеЗаписиЛентыНовостей,
		,
...
Процедура ДекорацияСтатистикаПодсказкаОбработкаНавигационнойСсылки(
	...
	Если ВРег(НавигационнаяСсылкаФорматированнойСтроки) = ВРег("1C:ResetLastDownloadDate") Тогда
		...
		ОбновитьИнформационныеСтроки();
		Оповестить(
			ОбработкаНовостейКлиент.ВсеОповещения().ОчищенаДатаПоследнейЗагрузкиНовостей,
			,
			Объект.Ссылка);
...
&НаКлиенте
Процедура ДоступныеКатегорииНовостейКатегорияНовостейАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)

	ТипСтруктура = Тип("Структура");
	Если ТипЗнч(ПараметрыПолученияДанных) = ТипСтруктура Тогда
		ПараметрыПолученияДанных.Вставить("ЛентаНовостей", Объект.Ссылка); // Может быть пустая ссылка, это нормально.
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыИзФормыНастройкиИсключенийВидимости(Знач СтруктураНастроек)
	...
	СтруктураНастроекЭталон = Новый Структура;
		СтруктураНастроекЭталон.Вставить("ВидимостьПоУмолчанию", Истина);
		СтруктураНастроекЭталон.Вставить("СписокИсключений"    , Неопределено);
	Если ТипЗнч(СтруктураНастроек) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(СтруктураНастроекЭталон, СтруктураНастроек);
	КонецЕсли;

	Объект.ВидимостьПоУмолчанию = ?(СтруктураНастроекЭталон.ВидимостьПоУмолчанию = Истина, Истина, Ложь);

	Объект.ИсключенияВидимости.Очистить();
	Если ТипЗнч(СтруктураНастроекЭталон.СписокИсключений) = ТипСписокЗначений Тогда
		Для каждого ТекущееИсключение Из СтруктураНастроекЭталон.СписокИсключений Цикл
...
Процедура ПослеПодбораКатегорий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = ТипМассив Тогда
		...
		ТекущийИндекс = Объект.ДоступныеКатегорииНовостей.Количество() - 1;
		Пока (ТекущийИндекс >= 0) Цикл
			ТекущаяСтрока = Объект.ДоступныеКатегорииНовостей[ТекущийИндекс];
