...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Метаданные.ОпределяемыеТипы.ДокументыСАТУРН.Тип.СодержитТип(ТипЗнч(Документ)) Тогда
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.НакладнаяСАТУРН") Тогда
...
Процедура ЗаполнитьДеревоФайлов()
	...
	ЭтоДокументСАТУРН = Метаданные.ОпределяемыеТипы.ДокументыСАТУРН.Тип.Типы().Найти(ТипДокумента) <> Неопределено;
	
	Если ЭтоДокументСАТУРН Тогда
