...
&НаКлиенте
Процедура ИзменитьПредприятие(Команда)
	
	ТекущиеДанные = Элементы.Предприятия.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на предприятии, которое необходимо изменить'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
	Предприятие = Элементы.Предприятия.ТекущиеДанные.Предприятие;
	
	ОчиститьСообщения();
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		ТекстВопроса = НСтр("ru = 'Редактирование предприятия возможно только для записанного хозяйствующего субъекта. Записать?'");
		
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ВопросПриСозданииНовогоПредприятияЗавершение", ЭтотОбъект, Предприятие);
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		
	Иначе
		
		ОткрытьФормуПомощникаСозданияНовогоПредприятия(Предприятие);
...
Процедура ЗаполнитьДанныеСопоставления()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент)
		...
		ДанныеСопоставления = РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(Объект.Контрагент);
...
Функция ТаблицаИдентификаторовПредприятий()
	...
	ТаблицаПредприятий.Колонки.Добавить("Идентификатор", Метаданные.ОпределяемыеТипы.УникальныйИдентификаторИС.Тип);
	ТаблицаПредприятий.Колонки.Добавить("GLN",           Метаданные.ОпределяемыеТипы.GLN.Тип);
	...
	Пока Не ПолучениеДанныхЗавершено Цикл
		...
		Если ЗначениеЗаполнено(ДанныеВЕТИС.ТекстОшибки) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ДанныеВЕТИС.ТекстОшибки);
...
&НаКлиенте
Процедура ОткрытьФормуПомощникаСозданияНовогоПредприятия(Предприятие = Неопределено)
	
	ПараметрыОткрытияФормы = Новый Структура;
	ПараметрыОткрытияФормы.Вставить("ХозяйствующийСубъект", Объект.Ссылка);
	ПараметрыОткрытияФормы.Вставить("Предприятие",          Предприятие);
...
&НаКлиенте
Процедура ВопросПриСозданииНовогоПредприятияЗавершение(РезультатВопроса, Предприятие) Экспорт
