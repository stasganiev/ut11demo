...
Процедура ОбработатьНайденныеХозяйствующиеСубъекты(НомерСтраницы)
	...
	ИначеЕсли ПараметрыПоиска.Количество() > 0 Тогда
		...
		Результат = ЦерберВЕТИСВызовСервера.СписокХозяйствующихСубъектов(ПараметрыПоиска, НомерСтраницы, КоличествоЭлементовНаСтранице);
...
Процедура ЗаполнитьСписокХозяйствующихСубъектов(Результат, НомерСтраницы)
	...
	УстановитьОтборыВФильтре(ПараметрыПоиска, Фильтры);
	
	Для Каждого СтрокаТЧ Из Результат.Список Цикл
		
		Если Не ДанныеСоответствуютОтборамФильтра(СтрокаТЧ, Фильтры) Тогда
			Продолжить;
		КонецЕсли;
...
&НаСервере
Процедура УстановитьОтборыВФильтре(ПараметрыПоиска, Фильтры)
	
	Если Фильтры = Неопределено Тогда
		Фильтры = Новый Структура;
	КонецЕсли;
	
	Фильтры.Вставить("Тип",                Ложь);
	Фильтры.Вставить("Наименование",       Ложь);
	Фильтры.Вставить("НаименованиеПолное", Ложь);
	Фильтры.Вставить("ФИО",                Ложь);
	
	Фильтры.Вставить("ТипЗначение",                Неопределено);
	Фильтры.Вставить("НаименованиеЗначение",       "");
	Фильтры.Вставить("НаименованиеПолноеЗначение", "");
	Фильтры.Вставить("ФИОЗначение",                "");
	
	Если ПараметрыПоиска.Свойство("Тип") И ЗначениеЗаполнено(ПараметрыПоиска.Тип) Тогда
		Фильтры.Тип = Истина;
		ТипыХозяйствующихСубъектовВЕТИС = Новый Соответствие;
		ТипыХозяйствующихСубъектовВЕТИС.Вставить(Перечисления.ТипыХозяйствующихСубъектовВЕТИС.ЮридическоеЛицо,               1);
		ТипыХозяйствующихСубъектовВЕТИС.Вставить(Перечисления.ТипыХозяйствующихСубъектовВЕТИС.ФизическоеЛицо,                2);
		ТипыХозяйствующихСубъектовВЕТИС.Вставить(Перечисления.ТипыХозяйствующихСубъектовВЕТИС.ИндивидуальныйПредприниматель, 3);
		Фильтры.ТипЗначение = ТипыХозяйствующихСубъектовВЕТИС[ПараметрыПоиска.Тип];
	КонецЕсли;
	
	Если ПараметрыПоиска.Свойство("Наименование") И ЗначениеЗаполнено(ПараметрыПоиска.Наименование) Тогда
		Фильтры.Наименование = Истина;
		Фильтры.НаименованиеЗначение = НРег(ПараметрыПоиска.Наименование);
	КонецЕсли;
	
	Если ПараметрыПоиска.Свойство("НаименованиеПолное") И ЗначениеЗаполнено(ПараметрыПоиска.НаименованиеПолное) Тогда
		Фильтры.НаименованиеПолное = Истина;
		Фильтры.НаименованиеПолноеЗначение = НРег(ПараметрыПоиска.НаименованиеПолное);
	КонецЕсли;
	
	Если ПараметрыПоиска.Свойство("ФИО") И ЗначениеЗаполнено(ПараметрыПоиска.ФИО) Тогда
		Фильтры.ФИО = Истина;
		Фильтры.ФИОЗначение = НРег(ПараметрыПоиска.ФИО);
	КонецЕсли;
...
&НаСервере
Функция ДанныеСоответствуютОтборамФильтра(Данные, Фильтры)
	
	Результат = Истина;
	
	Если Фильтры.Тип И Данные.type <> Фильтры.ТипЗначение Тогда
		Результат = Ложь;
	КонецЕсли;
	
	Если Фильтры.Наименование И СтрНайти(НРег(Данные.name), Фильтры.НаименованиеЗначение) = 0
		Или Фильтры.НаименованиеПолное И СтрНайти(НРег(Данные.fullName), Фильтры.НаименованиеПолноеЗначение) = 0 Тогда
		Результат = Ложь;
	КонецЕсли;
	
	Если Фильтры.ФИО И СтрНайти(НРег(Данные.fio), Фильтры.ФИОЗначение) = 0 Тогда
		Результат = Ложь;
	КонецЕсли;
	
	Если Не ЦерберВЕТИСВызовСервера.АдресСоответствуетФильтру(Данные.juridicalAddress, Фильтры) Тогда
		Результат = Ложь;
	КонецЕсли;
...
Процедура СформироватьПредставлениеПараметровПоиска(ПараметрыОтбора, ПриСоздании = Ложь)
	...
	НулевойРезультат           = ХозяйствующиеСубъекты.Количество() = 0;
