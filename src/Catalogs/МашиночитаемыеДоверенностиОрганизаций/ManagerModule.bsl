...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Реквизиты = "ДатаВыдачи, ДатаОкончания, ДатаОбновленияСтатуса, СтатусВРеестреФНС,
...
Функция МашиночитаемыеПолномочияДоверенности(Доверенность) Экспорт
	...
	XMLфайлМЧД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Доверенность, "XMLфайлМЧД");
	...
	Если Не РезультатЧтения.Успех Или ТипЗнч(РезультатЧтения.ДанныеДоверенности) <> Тип("Структура") Тогда
	...
	ВерсияФорматаМЧД = РезультатЧтения.ВерсияФорматаМЧД;
	Полномочия = РезультатЧтения.ДанныеДоверенности.Полномочия; // Массив из см. НовыеПолномочияПредставителя
	
	НормализоватьТаблицуПолномочий(Полномочия, ВерсияФорматаМЧД);
...
// Параметры:
//  Полномочия       - Массив из см. МашиночитаемыеДоверенности.НовыеПолномочияПредставителя
//  ВерсияФорматаМЧД - Строка
//
Процедура НормализоватьТаблицуПолномочий(Полномочия, ВерсияФорматаМЧД)
	
	Если ВерсияФорматаМЧД = МашиночитаемыеДоверенности.ФорматМЧД_2022_Версия_002() Тогда
		
		ПолномочияОбработанная = Новый Массив;
		
		ЗаменятьДанныеМассиваПолномочий = Ложь;
		
		Для Каждого СтрокаТаблицы Из Полномочия Цикл
			
			ЗаменятьДанныеМассиваПолномочий = Истина;
			
			МассивКодов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаТаблицы.Код, ";");
			МассивНаименований = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаТаблицы.Содержание, ";");
			
			СчетчикКодов = 0;
			
			Для Каждого ЭлементКод Из МассивКодов Цикл
				
				НовоеПолномочие = МашиночитаемыеДоверенности.НовыеПолномочияПредставителя();
				НовоеПолномочие.Код = ЭлементКод;
				Попытка
					НовоеПолномочие.Содержание = МассивНаименований[СчетчикКодов];
				Исключение
					НовоеПолномочие.Содержание = "<Не указано>";
				КонецПопытки;
				
				СчетчикКодов = СчетчикКодов + 1;
				
				ПолномочияОбработанная.Добавить(НовоеПолномочие);
				
			КонецЦикла;
			
		КонецЦикла;
		
		Если ЗаменятьДанныеМассиваПолномочий Тогда
			
			Полномочия = ПолномочияОбработанная;
