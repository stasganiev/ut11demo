...
//
// Параметры:
//   Правило - СправочникОбъект.ПравилаИнтеграцииС1СДокументооборотом - ссылка на правило интеграции.
...
//
// Параметры:
//   ДанныеПравила - см. Справочники.ПравилаИнтеграцииС1СДокументооборотом.ДанныеПравилаДляВыгрузки
//
// Возвращаемое значение:
//   Неопределено, Структура - вид документа ДО:
...
Функция ВидДокументаИзПравила(ДанныеПравила) Экспорт
	...
	СтрокиВидДокумента = ДанныеПравила.ПравилаЗаполненияРеквизитовДО.НайтиСтроки(ПараметрыОтбора); // Массив из см. Справочники.ПравилаИнтеграцииС1СДокументооборотом.ПравилаЗаполненияРеквизитовДО
	ВидДокумента = Неопределено;
	Если СтрокиВидДокумента.Количество() > 0 Тогда
		Строка = СтрокиВидДокумента[0];
		ВидДокумента = Новый Структура;
		ВидДокумента.Вставить("ID", Строка.ЗначениеРеквизитаДОID);
		ВидДокумента.Вставить("type", Строка.ЗначениеРеквизитаДОТип);
		ВидДокумента.Вставить("name", Строка(Строка.ЗначениеРеквизитаДО));
	...
	Иначе // Вид документа может быть задан только в шаблоне.
		...
		Если СтрокиВидДокумента.Количество() > 0 Тогда
			Строка = СтрокиВидДокумента[0];
			ВидДокумента = Новый Структура;
			ВидДокумента.Вставить("ID", Строка.ШаблонID);
			ВидДокумента.Вставить("type", Строка.ШаблонТип);
			ВидДокумента.Вставить("name", Строка(Строка.ШаблонЗначение));
		КонецЕсли;
...
//     * ПравилаЗаполненияРеквизитовДО - см. Справочник.ПравилаИнтеграцииС1СДокументооборотом.ПравилаЗаполненияРеквизитовДО
//     * ПрисоединяемыеПечатныеФормы - см. Справочник.ПравилаИнтеграцииС1СДокументооборотом.ПрисоединяемыеПечатныеФормы
//
Функция ДанныеПравилаДляВыгрузки(Правило) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		ПрисоединяемыеПечатныеФормы.ДополнительныеПараметры КАК ДополнительныеПараметры,
...
Функция ЗаполнитьОбъектПоОбъектуXDTO(Прокси, Объект, ОбъектXDTO, Правило,
	...
	Реквизиты = ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитыОбъектаИС(МетаданныеОбъекта.ПолноеИмя());
...
//       ** СписокДоступныхТипов - СписокЗначений из Структура:
//         *** xdtoClassName - Строка
//         *** presentation - Строка
...
Процедура ЗаполнитьСтруктуруРеквизитовОбъектаДОПоПравилу(Источник, СтруктураРеквизитов, Правило,
				...
				Иначе // обычный или дополнительный реквизит
					...
					Иначе // обычный реквизит объекта
						...
						Исключение // в случае отсутствия реквизита выдадим понятное сообщение об ошибке
							...
							// Иная ошибка.
							ВызватьИсключение ОбработкаОшибок.ПодробноеПредставлениеОшибки(Инфо);
	...
	Если СтруктураРеквизитов.Свойство("ПометкаУдаления") Тогда
		СтруктураРеквизитов.ПометкаУдаления = ИсточникОбъект.ПометкаУдаления;
	КонецЕсли;
...
Процедура ЗаполнитьФормуОбъектаДОПоПравилу(Источник, Форма, Правило) Экспорт
	...
	РеквизитыОбъектаДО = РеквизитыОбъектаДО(ДанныеПравила.ТипОбъектаДО, ПараметрыЗаполнения, НастройкиВида);
	...
	Если ДанныеПравила.ТипОбъектаДО <> "DMCorrespondent" Тогда
		...
		Для Каждого ПараметрыСоздания Из ПечатныеФормыКСозданию Цикл
			Файлы = Форма.Файлы; // ДанныеФормыКоллекция
...
//     * Таблица - Строка
//
Функция РеквизитыОбъектаДО(ТипОбъектаДО, Параметры = Неопределено, НастройкиВида = Неопределено,
