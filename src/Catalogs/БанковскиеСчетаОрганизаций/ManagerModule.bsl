...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.БанковскиеСчетаОрганизаций.ЗаполнитьРеквизитыБанков";
	Обработчик.Версия = "11.5.15.31";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c692545a-06cc-4ca4-bdf1-71fbf1197a07");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.БанковскиеСчетаОрганизаций.ЗарегистрироватьДанныеДляЗаполненияРеквизитовБанков";
	Обработчик.ПроцедураПроверки = "Справочники.БанковскиеСчетаОрганизаций.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение реквизитов банков в справочнике ""Банковские счета организаций"":
	...
	Изменяемые.Добавить(Метаданные.Справочники.БанковскиеСчетаОрганизаций.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.БанковскиеСчетаОрганизаций.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеДляЗаполненияРеквизитовБанков(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Справочник.БанковскиеСчетаОрганизаций";
	...
	|	БанковскиеСчетаОрганизаций.СВИФТБанка КАК СВИФТ,
	|	БанковскиеСчетаОрганизаций.КоррСчетБанка КАК КоррСчет,
	|	БанковскиеСчетаОрганизаций.СтранаБанка КАК Страна
	|ПОМЕСТИТЬ БанковскиеСчетаБезСсылкиБанка
	|ИЗ
	|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
	|ГДЕ
	|	БанковскиеСчетаОрганизаций.Банк = ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)
	|	И (БанковскиеСчетаОрганизаций.БИКБанка <> """"
	|			ИЛИ БанковскиеСчетаОрганизаций.СВИФТБанка <> """")
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	БанковскиеСчетаОрганизаций.Ссылка,
	|	БанковскиеСчетаОрганизаций.БИКБанкаДляРасчетов,
	|	БанковскиеСчетаОрганизаций.СВИФТБанкаДляРасчетов,
	|	БанковскиеСчетаОрганизаций.КоррСчетБанкаДляРасчетов,
	|	БанковскиеСчетаОрганизаций.СтранаБанкаДляРасчетов
	|ИЗ
	|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
	|ГДЕ
	|	БанковскиеСчетаОрганизаций.БанкДляРасчетов = ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)
	|	И (БанковскиеСчетаОрганизаций.БИКБанкаДляРасчетов <> """"
	|			ИЛИ БанковскиеСчетаОрганизаций.СВИФТБанкаДляРасчетов <> """")
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Страна
	|;
	...
	|			ПО БанковскиеСчетаОрганизаций.Банк = ИностранныеБанки.Банк
	|	ГДЕ
	|		БанковскиеСчетаОрганизаций.Банк <> ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИностранныеБанки КАК ИностранныеБанки
	|			ПО БанковскиеСчетаОрганизаций.БанкДляРасчетов = ИностранныеБанки.Банк
	|	ГДЕ
	|		БанковскиеСчетаОрганизаций.БанкДляРасчетов <> ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
...
Процедура ЗаполнитьРеквизитыБанков(Параметры) Экспорт
	...
	Если ВыбранныеДанные.Количество() <> 0 Тогда
		...
		Для Каждого ТекущийСчет Из ВыбранныеДанные Цикл
			...
			Попытка
				...
				ЭлементБлокировки = Блокировка.Добавить("Справочник.БанковскиеСчетаОрганизаций");
	...
	Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(
		Параметры.Очередь, "Справочник.БанковскиеСчетаОрганизаций");
	
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		
		ШаблонСообщения = НСтр("ru = 'Не удалось заполнить реквизиты банков в справочнике ""Банковские счета организаций"" (пропущены): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
		
	Иначе
		
		ШаблонСообщения = НСтр("ru = 'Обработана порция банковских счетов организаций: %1'");
