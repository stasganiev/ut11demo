...
Процедура ДополнитьСписокДоверенностейПоОтбору(Отбор, Доверенности) Экспорт
	...
	РеквизитыДоверителя.КПП = Отбор.Доверитель.КПП;
	
	РеквизитыПредставителя = НовыеРеквизитыОтбора();
	РеквизитыПредставителя.ИНН = ПредставительИНН;
	
	ТаблицаДоверенностей = ПолучитьДоверенностиПоОтбору(РеквизитыДоверителя, 
		РеквизитыПредставителя, 
		ТекущаяДатаСеанса());
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Доверенности, ТаблицаДоверенностей.ВыгрузитьКолонку("Ссылка"));
			
	ДоверенностиГоловнойОрганизации = ДоверенностиГоловнойОрганизации(РеквизитыДоверителя, РеквизитыПредставителя);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Доверенности,
			ДоверенностиГоловнойОрганизации.ВыгрузитьКолонку("Ссылка"),
			Истина);
		...
		Для Каждого СведенияОДоверителе Из СведенияОДоверителях Цикл
			Если СведенияОДоверителе.ТипПерПолн = ТипыДоверителей.ЮридическоеЛицо Тогда
				...
				Для Каждого ЛицоБезДоверенности Из ЛицаБезДоверенности Цикл
					...
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли СведенияОДоверителе.ФилПерПолн = ТипыДоверителей.ФилиалЮридическогоЛица Тогда
				Возврат СведенияОДоверителе.ПередПолн.ФилПерПолн.СвРукФил.СНИЛС;
			ИначеЕсли СведенияОДоверителе.ФилПерПолн = ТипыДоверителей.ФилиалИностраннойОрганизации Тогда
...
Функция ДанныеДляПодачиЗаявленияНаОтмену(ДанныеДоверенности) Экспорт
	...
	ИначеЕсли ДанныеДоверенности.Документ.Довер <> Неопределено
		...
		ВозвращаемыеДанные.Вставить("ДоверительЮЛ_ИНН", СведенияОДоверителе.ИННФЛ);
		ВозвращаемыеДанные.Вставить("ДоверительЮЛ_КПП", "");
		ВозвращаемыеДанные.Вставить("ДоверительЮЛ_ОГРН", СведенияОДоверителе.ОГРНИП);
	Иначе
...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Реквизиты = "НомерДоверенности, ДатаВыдачи, ДатаОкончания, ПолномочияОграничены, ДатаОтзыва, Верна, 
				|СтатусВРеестреФНС, Отозвана, НомерРодительскойДоверенности, ИННДоверителяРодительскойДоверенности";
	...
	Результат.ТипПередоверия = ?(Выборка.ПередовериеРазрешено, ПризнакВозможностиПередоверия().ПоследующееПередоверие, "");
...
Функция ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, ЭтоРучноеСоздание, ОбновлятьСуществующий = Ложь,
	ДополнительныеСведения = Неопределено, МЧД = Неопределено)
...
// Дополняет результат номерами доверенностей с ИНН доверителей по участникам.
// 
// Параметры:
//  Результат - Структура:
...
Процедура ДополнитьНомерамиДоверенностейСИННДоверителейПоУчастникам(Результат, ДоверительИНН, ПредставительИНН, ДействительныеНаДату,
	...
	Для Каждого СтрокаТЗ Из Доверенности Цикл
		...
		Если СтрокаТЗ.ПолномочияОграничены И ЭлектронныйДокумент <> Неопределено Тогда
			...
			РезультатПроверки = 
				МашиночитаемыеДоверенности.ПроверитьПолномочияДоверенности(СтрокаТЗ.Ссылка, ЭлектронныйДокумент);
		...
		ИННДоверителей = Новый Структура("ИННДоверителя, ИННДоверителяРодительскойДоверенности", 
			СтрокаТЗ.ИННДоверителя, СтрокаТЗ.ИННДоверителяРодительскойДоверенности);
			
		Результат.НомераМЧДСИННДоверителей.Вставить(СтрокаТЗ.НомерДоверенности, ИННДоверителей);
...
Процедура ПолучитьПоследнююВыданнуюДоверенность(Отбор, ЭлектронныйДокумент, Результат) Экспорт
	...
	ДоверенностиГоловнойОрганизации = ДоверенностиГоловнойОрганизации(Доверитель, Представитель);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДоверенностиГоловнойОрганизации, Доверенности);
...
Функция ПолучитьДоверенностиПоОтбору(Доверитель, Представитель, ДействительныеНаДату)
	...
	Запрос.Текст =
		...
		|	МЧД003.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
		|	МЧД003.ВариантЗаполненияПолномочий = ЗНАЧЕНИЕ(Перечисление.ВариантыЗаполненияПолномочийМЧД.КлассификаторФНС) КАК
	...
	Если Не ПустаяСтрока(ДоверительКПП) Тогда
		...
		ТекстыУсловий.Добавить("ТаблицаДоверители.КПП = &ДоверительКПП");
	КонецЕсли;
	...
	ТекстУсловий    = ТекстОтбора(ТекстыУсловий);
	...
	ТаблицаМЧД = НоваяТаблицаДоверенностей();
...
// * ИННДоверителя - Строка
// * ИННДоверителяРодительскойДоверенности - Строка
Функция НоваяТаблицаДоверенностей()
	...
	Результат.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.МЧД003"));
...
Функция ТекстОтбора(Знач НаборУсловий)
	...
	Возврат СтрСоединить(ПодстрокиРезультата, " И ");
...
// Возвращаемое значение:
//  ТаблицаЗначений - см. НоваяТаблицаДоверенностей 
Функция ДоверенностиГоловнойОрганизации(ОтборДоверителя, ОтборПредставителя)
	Результат = НоваяТаблицаДоверенностей();

	Если ЗначениеЗаполнено(ОтборДоверителя.КПП) 
		И ОбщегоНазначенияБЭД.ЭтоПодразделениеФилиалИлиПредставительство(ОтборДоверителя.КПП) Тогда
		
		ОтборГоловнойОрганизации = ОбщегоНазначения.СкопироватьРекурсивно(ОтборДоверителя);
		ОтборГоловнойОрганизации.КПП = ОбщегоНазначенияБЭД.КППГоловнойОрганизации(ОтборДоверителя.КПП);
		Результат = ПолучитьДоверенностиПоОтбору(ОтборГоловнойОрганизации, ОтборПредставителя, ТекущаяДатаСеанса());
