...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЗначениеЗаполнено(ХешФайла) Тогда
		ДанныеФайла = XMLфайлМЧД.Получить();
...
Процедура ЗаписатьВЖурнал(Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ХешФайла) Или Не Подписана Или ДанныеДоверенности = Неопределено
		Или СуществуетДоверенностьОрганизации() Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ПредставительОбъекта Из СведенияФайла.Представители Цикл
		ФИО = ОбщегоНазначенияБЭДКлиентСервер.ФИОСтрокой(ПредставительОбъекта);
		Если ПредставительОбъекта.ТипУполномоченногоПредставителя = "ФЛ" Тогда
			...
			Представитель.ДанныеФизическогоЛица.ФИО = ФИО;
...
Функция СуществуетДоверенностьОрганизации()
	Запрос = Новый Запрос;
