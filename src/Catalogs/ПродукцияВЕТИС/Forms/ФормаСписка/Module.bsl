...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СФормироватьПараметрыОтбора(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ПродукцияВЕТИС" Тогда
		...
		НастроитьВидимостьТаблицыПоиска(ЭтотОбъект);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Производитель > 1 Тогда
			...
			Если СтрДлина(ПредставлениеПроизводителей) > 75 Тогда
				ПредставлениеПроизводителей = СтрШаблон(НСтр("ru = '%1 (+ еще %2)'"), СтрокаСписка.Данные["Производитель"], Выборка.Производитель - 1);
			КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Маркировка > 1 Тогда
			...
			Если СтрДлина(Маркировка) > 75 Тогда
				Маркировка = СтрШаблон(НСтр("ru = '%1 (+ еще %2)'"), СтрокаСписка.Данные["Маркировка"], Выборка.Маркировка - 1);
			КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНаименованияПродукции
...
Процедура НаименованияПродукцииВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		...
		ОткрытьФорму("Справочник.ПродукцияВЕТИС.Форма.ФормаЭлемента", ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура ОткрытьПараметрыПоиска(Команда)
	...
	ОткрытьФорму(
		...
		Неопределено, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.ПолноеНаименование");
	ОтборЭлемента.ВидСравнения  = ВидСравненияКомпоновкиДанных.Заполнено;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Список.ПолноеНаименование"));
...
Процедура ВывестиИнформациюОВидеТипеПродукции(Форма)
	...
	ШрифтГиперссылки = Новый Шрифт(,,,,Истина);
...
Процедура ОпределитьНаличиеПродукции()
	...
	ТаблицаФормы = НаименованияПродукции;
	...
	Если ЗначениеЗаполнено(МаркировкаОтбор) Тогда 
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Производитель продукции не найден. Фильтр не изменен.'"),,
				"МаркировкаОтбор");
		КонецЕсли;
