...
Процедура ПрочитатьОбъект(ОбновитьЭлементы, СсылкаНаОбъект)
	...
	Если ЗначениеЗаполнено(СправочникОбъект.ПолноеНаименование) Тогда
		НаименованиеПродукции = СправочникОбъект.ПолноеНаименование;
	Иначе
		НаименованиеПродукции = СправочникОбъект.Наименование;
	КонецЕсли;
...
Функция ЗагрузитьПродукциюНаСервере()
	...
	Если Не Объект.ЭтоГруппа Тогда
		Возврат ИнтеграцияВЕТИС.ЗагрузитьНаименованиеПродукцииПоИдентификатору(Объект.Идентификатор, Ссылка);
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Продукция) Тогда
		Возврат ИнтеграцияВЕТИС.ЗагрузитьПродукциюПоИдентификатору(Объект.Идентификатор, Ссылка);
	Иначе
		Возврат ИнтеграцияВЕТИС.ЗагрузитьВидПродукцииПоИдентификатору(Объект.Идентификатор, Ссылка);
	КонецЕсли;
...
Процедура СформироватьНадписьОписаниеПродукции()
	...
	Если ЗначениеЗаполнено(Объект.Продукция) Тогда
		Описание =  Строка(Объект.Продукция);
		ДлиннаяСтрока = СтрДлина(Описание)>30;
		Описание = Новый ФорматированнаяСтрока(
			Лев(Описание,30) + ?(ДлиннаяСтрока,"...",""),
			Новый Шрифт(,,,,Истина),
			ЦветаСтиля.ЦветГиперссылкиГосИС,,
			"ОткрытьПродукция");
		ОписаниеПродукции = Новый ФорматированнаяСтрока(ОписаниеПродукции, " > ", Описание);
	ИначеЕсли ЗначениеЗаполнено(ПродукцияСтрока) Тогда
		Описание = ПродукцияСтрока;
		ДлиннаяСтрока = СтрДлина(Описание)>30;
		Описание = Новый ФорматированнаяСтрока(
				Лев(Описание,30) + ?(ДлиннаяСтрока,"...",""),
				Новый Шрифт(,,,,Истина));
	...
	Если ЗначениеЗаполнено(Объект.ВидПродукции) Тогда
		Описание = Строка(Объект.ВидПродукции);
		ДлиннаяСтрока = СтрДлина(Описание)>30;
		Описание = Новый ФорматированнаяСтрока(
			Лев(Описание,30) + ?(ДлиннаяСтрока,"...",""),
			Новый Шрифт(,,,,Истина),
			...
			"ОткрытьВидПродукции");
		ОписаниеПродукции = Новый ФорматированнаяСтрока(ОписаниеПродукции, " > ", Описание);
	ИначеЕсли ЗначениеЗаполнено(ВидПродукцииСтрока) Тогда
		Описание = ВидПродукцииСтрока;
		ДлиннаяСтрока = СтрДлина(Описание)>30;
		Описание = Новый ФорматированнаяСтрока(
				Лев(Описание,30) + ?(ДлиннаяСтрока,"...",""),
				Новый Шрифт(,,,,Истина));
	...
	Элементы.ОписаниеПродукции.Подсказка = Строка(ОписаниеПродукции);
...
Процедура СформироватьНадписьПроизводители()
	...
	Иначе
		...
		КонецЕсли;
		ВыводитьТекстГиперссылку = Истина;
	КонецЕсли;
	...
	Если ВыводитьТекстГиперссылку Тогда
		ГиперссылкаПроизводители = ?(КоличествоПроизводителей>1,
			"ОткрытьСписокПроизводителей", "ОткрытьФормуПроизводителя");
...
Процедура ЗагрузитьДанныеПоИдентификатору()
	...
	Иначе
		...
		НаименованиеПродукции = Данные.Наименование;
