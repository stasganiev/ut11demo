...
Процедура НачатьВыборЗначения(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗаполнения = "Parameter" Тогда
		...
		ОткрытьФорму("Справочник.ШаблоныЭтикетокИЦенниковБПО.Форма.РедакторЭтикетокФормаВыбораПоляСКД", Новый Структура("АдресХранилищаСКД", АдресХранилищаСКД),,,,, ОповещениеПриЗавершении);
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если Не ПроверитьЗаполнениеРеквизитов(МассивОшибок) Тогда
		...
		Для Каждого ТекОшибка Из МассивОшибок Цикл
			...
			ОбщегоНазначенияБПОКлиент.СообщитьПользователю(ТекОшибка);
...
Процедура СформироватьОписаниеОповещенияОЗакрытии()
	...
	Если Тип = "Text" Тогда
		...
		ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.ОписаниеПоля.Вставить("Фон", Фон);
	ИначеЕсли Тип = "Barcode" Тогда
		ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.ОписаниеПоля.Вставить("Верх", Верх+1);
		ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.ОписаниеПоля.Вставить("Лево", Лево+1);
		ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.ОписаниеПоля.Вставить("Низ", Низ);
...
&НаКлиенте
Процедура ВыборФайлаЗавершить(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДвоичныеДанныеКартинки = ПолучитьИзВременногоХранилища(Результат.Адрес);
...
&НаКлиенте
Процедура ВыбратьФайлДляЗначенияПоУмолчаниюЗавершить(Результат, ДополнительныеПараметры) Экспорт
