...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресМакета") И Не Параметры.АдресМакета = "" Тогда
...
Процедура ШагСеткиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение<>ШагСетки Тогда
		...
		УпраздняемыеПоля = Новый Массив; //Массив из см.ПолучитьПроекцииПолей
		...
		Если УпраздняемыеПоля.Количество()>0 Тогда
			...
			Для Каждого ТекущееПоле Из УпраздняемыеПоля Цикл
				ТекПоле = ТекущееПоле; // Структура
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.Вставить("Макет", ПоместитьВоВременноеХранилище(Макет, Новый УникальныйИдентификатор));
...
Процедура ГраницаВокруг(Команда)
	...
	СтруктураПоискаСтрок = СтруктураПоиска();
	СтруктураПоискаСтрок.Наименование = Элементы.Макет.ТекущаяОбласть.Имя;
	НайденныеСтроки = Поля.НайтиСтроки(СтруктураПоискаСтрок);
	
	Если НайденныеСтроки.Количество()>0 Тогда
		ТД = НайденныеСтроки[0];//см. СтруктураПоиска
...
Процедура ИзменитьДополнительныеДанные(Команда)
	...
	ОткрытьФорму("Справочник.ШаблоныЭтикетокИЦенниковБПО.Форма.РедакторЭтикетокДополнительныеДанные", Новый Структура("АдресХранилищаСКД", АдресХранилищаСКД),,,,, ОповещениеПриЗавершении);
...
Процедура РедактироватьСКД(Команда)
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ОбработкаОткрытияНастройкиСКД", ЭтотОбъект);
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("АдресХранилищаСКД", АдресХранилищаСКД);
	ПараметрыОткрытия.Вставить("УникальныйИдентификатор", УникальныйИдентификатор);
	ПараметрыОткрытия.Вставить("ОбработчикОповещения", ОбработчикОповещения);
...
Процедура ЗаполнитьПоДаннымМакета(ЧтениеИзФайла = Ложь)
	...
	ТекстСообщения = "";
	СодержимоеXMLФайла = Неопределено; // см. МенеджерОборудованияВызовСервера.СоздатьОписаниеМакета
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьПоДаннымМакетаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	
	Если ЧтениеИзФайла Тогда
		ТекстовыйДокумент.НачатьЧтение(ОписаниеОповещения, ИмяФайла);
	Иначе
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("XMLОписаниеМакета", XMLОписаниеМакета);
		ЗаполнитьПоДаннымМакетаЗавершение(ДополнительныеПараметры);
	КонецЕсли;
	#КонецЕсли
...
&НаКлиенте
Процедура ЗаполнитьПоДаннымМакетаЗавершение(ДополнительныеПараметры) Экспорт
	
	ТекстСообщения = "";
	СодержимоеXMLФайла = Неопределено; // см. МенеджерОборудованияВызовСервера.СоздатьОписаниеМакетаИсточник
	
	Если ДополнительныеПараметры.Свойство("ТекстовыйДокумент") Тогда
		
		Источник = ДополнительныеПараметры.ТекстовыйДокумент.ПолучитьТекст();
		
	ИначеЕсли ДополнительныеПараметры.Свойство("XMLОписаниеМакета") Тогда
		
		Источник = ДополнительныеПараметры.XMLОписаниеМакета;
	...
	Если ПолучитьОписаниеМакетаНаСервере(Источник, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
		...
		Для Каждого ТекПоле Из СодержимоеXMLФайла.Поля Цикл
			...
			ИначеЕсли ТекПоле.Тип="Barcode" Тогда
				ПредставлениеПоля = СтрШаблон(НСтр("ru = 'Штрихкод %1'"), Строка(ТекПоле.ТипШтрихкода));
				НовоеПоле.Представление = ПредставлениеПоля;
...
Процедура НачатьРедактированиеПоля(Поле, ВыбранноеПолеСКД=Неопределено)
	...
	ОписаниеПоля.Вставить("Фон",					Поле.Фон);
					...
					Если ГоризонтальноеПересечение Тогда
						...
						Если ВертикальноеПересечение Тогда
							...
							Если ТекПоле.Тип = "Image" Тогда
								ПредставлениеПоля = НСтр("ru = 'Картинка'");
							Иначе
...
// Заполняет список параметров.
// 
// Параметры:
// 	КоллекцияИсточник - Массив - массив источник.
// 	КоллекцияПриемник - Массив - массив приемник.
&НаКлиенте
...
Процедура ВыборФайла()
	...
	ДОФ.ПолноеИмяФайла = ИмяФайла;
	ДОФ.Фильтр = НСтр("ru = 'Файл шаблона'") + "(*.xml)|*.xml|" + НСтр("ru = 'Файл шаблона'") + "(*.mxl)|*.mxl"; 
	ДОФ.МножественныйВыбор = Ложь;
...
Процедура ИмпортШаблонаИзФайлаЗавершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы.Количество() > 0 Тогда
		...
		Если XMLОписаниеМакета = "" Тогда
			ТекстСообщения = НСтр("ru='Не удалось конвертировать макет.'");
			ОбщегоНазначенияБПОКлиент.СообщитьПользователю(ТекстСообщения);
		Иначе
			ЗаполнитьПоДаннымМакета();
		КонецЕсли;
...
Процедура ЗагрузкаШаблонаИзФайлаЗавершениеСервер(АдресФайла)
	...
	ИмяПромежуточногоФайла = Каталог + ИмяВременногоФайла;
	ДвоичныеДанные.Записать(ИмяПромежуточногоФайла);
	МакетВременный = Новый ТабличныйДокумент;
	МакетВременный.Очистить();
	МакетВременный.Прочитать(ИмяПромежуточногоФайла);
	УдалитьФайлы(ИмяПромежуточногоФайла);
	XMLОписаниеМакета = ПечатьЭтикетокИЦенниковБПО.КонвертироватьШаблонВXML(МакетВременный, Истина);
...
Процедура СохранитьОписаниеПоля(ОписаниеПоля, РедактируемоеПоле=Неопределено)
	...
	Если ОписаниеПоля.Свойство("Шрифт") И Не ОписаниеПоля.Шрифт = Неопределено Тогда
		ОписаниеПоля.Шрифт = ПодобратьШрифт(ОписаниеПоля.Шрифт);
	КонецЕсли;
	...
	Если ОписаниеПоля.Тип<>"UserData" Тогда
		...
		Если ОписаниеПоля.Тип = "Text" Тогда
			...
			ПолеМакета.Фон                          = ОписаниеПоля.Фон;
		ИначеЕсли ОписаниеПоля.Тип = "Barcode" Тогда
			ПредставлениеПоля						= СтрШаблон(НСтр("ru = 'Штрихкод %1'"), Строка(ОписаниеПоля.ТипШтрихкода));
			ПолеМакета.Представление				= ПредставлениеПоля;
		...
		ИначеЕсли ОписаниеПоля.Тип = "Image" Тогда
			ПолеМакета.Представление				= НСтр("ru = 'Картинка'");
			ПолеМакета.РамкаСлева					= ОписаниеПоля.РамкаСлева;
			ПолеМакета.РамкаСверху					= ОписаниеПоля.РамкаСверху;
			ПолеМакета.РамкаСправа					= ОписаниеПоля.РамкаСправа;
			ПолеМакета.РамкаСнизу					= ОписаниеПоля.РамкаСнизу;
...
Функция ПолучитьXMLОписаниеЭтикетки()
	...
	Возврат ОборудованиеПринтерыЭтикеток.СохранитьXMLФайл(ШиринаЭтикетки, ВысотаЭтикетки, Ориентация, Поля);
...
Процедура СохранитьОформлениеПоля(ОписаниеПоля, ОткрытиеМакета=Ложь, ТабличныйДокумент=Неопределено)
	...
	#Иначе
		...
		Если ОписаниеПоля.Тип = "Text" Тогда
			...
			Если ОписаниеПоля.Свойство("Фон") Тогда
...
#Если Не МобильныйКлиент И Не МобильноеПриложениеКлиент Тогда 
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьXMLФайлЗавершение", ЭтотОбъект);
		ТекстовыйДокумент.НачатьЗапись(ОписаниеОповещения, ИмяФайла);
#КонецЕсли
...
&НаКлиенте
Процедура СохранитьXMLФайлЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат Тогда
		ТекстСообщения = НСтр("ru='Файл %ИмяФайла% успешно записан.'");
...
Процедура СохранитьОформлениеПоляНаСервере(ОписаниеПоля, ОткрытиеМакета=Ложь, ТабличныйДокумент=Неопределено)
	...
	ИначеЕсли ОписаниеПоля.Тип = "Barcode" Тогда		
		...
		ПредставлениеПоля = СтрШаблон(НСтр("ru = 'Штрихкод %1'"), Строка(ОписаниеПоля.ТипШтрихкода));
		Область.Текст = ПредставлениеПоля;
		Область.РазмещениеТекста=ТипРазмещенияТекстаТабличногоДокумента.Переносить;
...
//
// Параметры:
//   ИмяПоля - Строка - Идентификатор поля макета.
//   ИндексГраницы - Число - Индекс Границы (0-нет границ,1-левая граница,2-верхняя граница,3-правая граница,4-нижняя граница,5-все границы)
//
&НаКлиенте
...
//
// Параметры:
//   ПолеТабличнойЧасти - Строка - Поле табличной части.
//   ИндексГраницы - Число - Индекс Границы (0-нет границ,1-левая граница,2-верхняя граница,3-правая граница,4-нижняя граница,5-все границы)
//   ВеличинаИзменения - Число - 
//  
&НаКлиенте
...
&НаКлиенте
Функция СтильШрифтаНаКлиенте(Шрифт)
	#Если Не ВебКлиент Тогда
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.УстановитьСтроку();
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Шрифт, НазначениеТипаXML.Явное);

		СтрокаШрифт = ЗаписьXML.Закрыть();
		// Определить стиль
		ИмяСтиля = "";
		ПозицияПоиска = СтрНайти(СтрокаШрифт, "ref=""");
		Если ПозицияПоиска > 0 Тогда
			ИмяСтиля = Сред(СтрокаШрифт, ПозицияПоиска + 5);
			ПозицияПоиска = СтрНайти(ИмяСтиля, """");
			ИмяСтиля = Лев(ИмяСтиля, ПозицияПоиска - 1);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СтильШрифтаНаСервере(Шрифт)
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Шрифт, НазначениеТипаXML.Явное);
...
&НаСервереБезКонтекста
Функция ШрифтЭлементаСтиляНаСервере(СтильШрифта)
	Возврат Метаданные.ЭлементыСтиля[Сред(СтильШрифта,7)].Значение;
КонецФункции

// Подобрать похожий шрифт для шрифтов windows и шрифтов стиля
// Параметры:
//  Шрифт - Шрифт
&НаКлиенте
Функция ПодобратьШрифт(Шрифт)
	...
	#Если ВебКлиент Тогда
		...
		СтильШрифта = СтильШрифтаНаСервере(Шрифт);
		
	#Иначе
		
		СтильШрифта = СтильШрифтаНаКлиенте(Шрифт);
		
	#КонецЕсли
	
	Если Шрифт.Вид = ВидШрифта.WindowsШрифт Тогда
		
		Если СтильШрифта = "sys:DefaultGUIFont" Тогда
			ИмяШрифта = "MS Sans Serif";
		ИначеЕсли СтильШрифта = "sys:OEMFixedFont" Тогда
			ИмяШрифта = "Courier New";
		ИначеЕсли СтильШрифта = "sys:ANSIFixedFont" Тогда
			ИмяШрифта = "Courier New";
		ИначеЕсли СтильШрифта = "sys:ANSIVariableFont" Тогда
			ИмяШрифта = "MS Sans Serif";
		ИначеЕсли СтильШрифта = "sys:SystemFont" Тогда
			ИмяШрифта = "System";
		КонецЕсли;
		
	ИначеЕсли Шрифт.Вид = ВидШрифта.ЭлементСтиля Тогда
		
		Если СтильШрифта = "style:TextFont" Тогда
			ИмяШрифта = "Arial";
		ИначеЕсли СтильШрифта = "style:SmallTextFont" Тогда
			ИмяШрифта = "Arial";
			РазмерШрифта = 8;
		ИначеЕсли СтильШрифта = "style:NormalTextFont" Тогда
			ИмяШрифта = "Arial";
			РазмерШрифта =10;
		ИначеЕсли СтильШрифта = "style:LargeTextFont" Тогда
			ИмяШрифта = "Arial";
			РазмерШрифта =14;
		ИначеЕсли СтильШрифта = "style:ExtraLargeTextFont" Тогда
			ИмяШрифта = "Arial";
			РазмерШрифта =18;
		Иначе
			Возврат ПодобратьШрифт(ШрифтЭлементаСтиляНаСервере(СтильШрифта));
		КонецЕсли;
	...
	КонецЕсли;
	Возврат Новый Шрифт(ИмяШрифта, РазмерШрифта, 
		Шрифт.Полужирный, Шрифт.Наклонный, Шрифт.Подчеркивание, Шрифт.Зачеркивание, Шрифт.Масштаб);
