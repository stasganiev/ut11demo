...
//
// Параметры:
//  Идентификатор      - Строка, Справочник.ИдентификаторыСервисовСопровождения - идентификатор сервиса
//                       в системе Портал 1С:ИТС;
...
//
// Параметры:
//  СервисыСопровождения - Массив из Структура - содержит структуры с описанием сервиса:
...
//                              ПодключениеСервисовСопровождения.НовыйОписательСервиса().
//
Процедура ОбновитьДанныеСсылокСервисовСопровождения(Знач ДанныеСервисовСопровождения) Экспорт
	...
	Попытка
		...
		Для Каждого ОписательСервиса Из ДанныеСервисовСопровождения Цикл
			...
			ПараметрыПоиска.Вставить("Идентификатор", ОписательСервиса.Идентификатор);
			Если ВыборкаДетальныеЗаписи.НайтиСледующий(ПараметрыПоиска) Тогда
				Если ВыборкаДетальныеЗаписи.URLОписание <> ОписательСервиса.URLОписание Тогда
					ИдентификаторОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
					ИдентификаторОбъект.Заблокировать();
	...
	Исключение
		ОтменитьТранзакцию();
		ИнформацияОбОшибке = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ПодключениеСервисовСопровождения.ЗаписатьИнформациюВЖурналРегистрации(
			ИнформацияОбОшибке,
			Истина);
		ВызватьИсключение ИнформацияОбОшибке;
	КонецПопытки;
...
//
// Возвращаемое значение:
//  Строка - начальная строка, дополненная вначале символом "?".
//
