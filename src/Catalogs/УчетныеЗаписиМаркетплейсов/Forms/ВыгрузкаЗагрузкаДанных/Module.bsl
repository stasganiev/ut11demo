...
Процедура ТаблицаСопоставленияДанныхПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ОтменаРедактирования Тогда
		...
		Если (ФильтрТаблицаСопоставления = "Сопоставленные" И Не ТекущиеДанные.СтрокаОбработана)
			Или (ФильтрТаблицаСопоставления = "Несопоставленные" И ТекущиеДанные.СтрокаОбработана) Тогда
...
Процедура УстановитьУсловноеОформление(СписокКолонок = Неопределено)
	...
	Для Каждого ДанныеПоКолонке Из ИнформацияПоКолонкам Цикл
...
Процедура НастроитьОтображениеЭлементовСтраницы()
	...
	Если ЭтоЗагрузкаДанных Тогда
		...
		ИначеЕсли ВариантВыгрузкиЗагрузки = "ИнтеграционнымиМетодами" Тогда
			Если ЭтоЗагрузкаДанных Тогда
				ТекстПодсказки = НСтр("ru = 'С помощью методов API выполняется автоматическое чтение данных с торговой площадки и их подготовка для записи в учетную систему 1С.'") + Символы.ПС;
			Иначе
				ТекстПодсказки = НСтр("ru = 'С помощью методов API выполняется автоматическая отправка данных из учетной системы 1С на торговую площадку.'") + Символы.ПС;
			КонецЕсли;
		Иначе
...
Процедура УстановитьФильтрОсновныхКолонок(ОтображатьТолькоОсновные)
	...
	ОтображаемыеКолонки = ИнформацияПоКолонкам.НайтиСтроки(Отбор);
...
Функция ТаблицаСДаннымиПустая()
	...
	Если ВысотаЗаголовкаТаблицыМакетаСШаблоном = 0
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахДляТЧ()
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		...
		ИначеЕсли Колонка.Имя = "НомерСтроки" Или Колонка.Имя = "РезультатСопоставленияСтроки"
			Или Колонка.Имя = "ОписаниеОшибки" Или Колонка.Имя = "СтрокаОбработана" Тогда
		...
		ИначеЕсли Колонка.Имя = "РезультатСопоставленияСтроки" Или Колонка.Имя = "ОписаниеОшибки"
			Или Колонка.Имя = "СтрокаОбработана" Тогда
			НовыйЭлемент.Видимость = Ложь;
		КонецЕсли;
...
Процедура ПослеПолученияДанныхИнтеграционнымиМетодами(ФоновоеЗадание, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если Не ПустаяСтрока(ИнформацияОбОшибке.ОписаниеОшибки) Тогда
