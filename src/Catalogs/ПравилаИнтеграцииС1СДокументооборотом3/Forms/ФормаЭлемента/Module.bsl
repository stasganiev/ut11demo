...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВопросЗадан = Ложь;
	ДоступенОтборЗначенийДопРеквизитов =
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.15.1");
...
Процедура КлючевыеРеквизитыДОЗначениеРеквизитаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ЭтоДополнительныйРеквизитДО
			И ДоступенОтборЗначенийДопРеквизитов
			И ПервыйТип = "DMObjectPropertyValue" Тогда
		Отбор = Новый Структура("additionalPropertyID", ТекущиеДанные.Имя);
	КонецЕсли;
...
&НаСервере
Процедура ДополнитьПравилаЗаполнения(ПравилаЗаполнения, ИмяРеквизитаИС, ИмяРеквизитаДО, Обновлять = Истина)
...
Процедура ЗаполнитьРеквизитыДокументооборота(ДопЗапрос = Неопределено, ОтветНаДопЗапрос = Неопределено)
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаДО) Тогда
		РеквизитыXDTO = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.РеквизитыОбъектаДО(
			Объект.ТипОбъектаДО,
			Объект.ВидДокументаДОID,
			Ложь,,
			ДопЗапрос,
		...
		Для Каждого Строка Из РеквизитыXDTO Цикл
			...
			ДанныеРеквизита.Имя = Строка.Имя;
			ДанныеРеквизита.Тип = Строка.Тип;
			ДанныеРеквизита.Представление = Строка.Представление;
			ДанныеРеквизита.ЭтоДополнительныйРеквизитДО = Строка.ДопРеквизит;
			ДанныеРеквизита.ДополнительныйРеквизитДОID = Строка.ДопРеквизитID;
			ДанныеРеквизита.ДополнительныйРеквизитДОТип = Строка.ДопРеквизитТип;
...
Процедура ЗаполнитьРеквизитыПотребителя()
	...
	Для Каждого Элемент Из РеквизитыОбъектаДО Цикл // см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеРеквизита
...
Процедура ЗаполнитьПоУмолчаниюНаСервере(НачальноеЗаполнение = Ложь, ЗаполнятьИзОбъектаДО = Истина)
	...
	Если СтандартнаяОбработка Тогда
		...
		Если ИнтеграцияС1СДокументооборот3.ЭтоДокументДО3(Объект.ТипОбъектаДО) Тогда
		...
		Если ЭтоСправочник Тогда
			ДополнитьПравилаЗаполнения(ПравилаЗаполнения, "Наименование", "title");
			ДополнитьПравилаЗаполнения(ПравилаЗаполнения, "Код", "regNumber", Ложь);
			
		ИначеЕсли ЭтоДокумент Тогда
			ДополнитьПравилаЗаполнения(ПравилаЗаполнения, "Номер", "regNumber", Ложь);
			ДополнитьПравилаЗаполнения(ПравилаЗаполнения, "Дата", "regDate", Ложь);
