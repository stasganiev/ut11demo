...
// Регистрирует данные для обработчика обновления
// Выбираем только те элементы, в которых полностью не заполнена колонка "СпособПодтвержденияДокумента"
// 
// Параметры:
//  Параметры - Структура - параметры.
//
Процедура ЗарегистрироватьДанныеКОбработке(Параметры) Экспорт
	...
	|ГДЕ
	|	НастройкиОбменСБанкамиИсходящиеДокументы.ИсходящийДокумент В(&ВидыДокументовПодписываемыхПоМаршруту)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ИНАЧЕ 0
	|		КОНЕЦ) = 0"; 
	ВидыДокументовПодписываемыхПоМаршруту = ОбменСБанкамиСлужебныйПовтИсп.ВидыДокументовПодписываемыхПоМаршруту(Истина);
	Запрос.УстановитьПараметр("ВидыДокументовПодписываемыхПоМаршруту", ВидыДокументовПодписываемыхПоМаршруту);
	Результат = Запрос.Выполнить().Выгрузить();
	МассивСсылок = Результат.ВыгрузитьКолонку("Ссылка");
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, МассивСсылок);
...
// Процедура - ПерезаполнитьНастройкиОбменСБанками - обработчик обновления, 
//  перезаполняет табличную часть ИсходящиеДокументы и колонку "СпособПодтвержденияДокумента"
//
// Параметры:
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура ПерезаполнитьНастройкиОбменСБанками(Параметры) Экспорт
	
	ДанныеУжеВыбраны = Ложь;
	Если Параметры.Очередь <> 0 Тогда
	...
	Если НЕ ДанныеУжеВыбраны Тогда 
		...
		|УПОРЯДОЧИТЬ ПО
		|	
		|	НастройкиОбменСБанками.Ссылка УБЫВ";
		
		ПоследняяСсылка = Неопределено;
		Если Параметры.Свойство("ПоследняяСсылка", ПоследняяСсылка) Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДополнительныеУсловия", " НастройкиОбменСБанками.Ссылка < &ПоследняяСсылка " );
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			СправочникОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ОбменСБанкамиСлужебный.ЗаполнитьСпособПодтвержденияИсходящихДокументов(СправочникОбъект, СправочникОбъект.ИсходящиеДокументы.Выгрузить());
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(СправочникОбъект);
			
			Параметры.Вставить("ПоследняяСсылка", Выборка.Ссылка);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() не удалось обработать элемент по причине:
					|%1'"),
	...
	Если ПроблемныхОбъектов > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() не удалось обработать часть элементов. В %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.НастройкиОбменСБанками, ,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() обработала очередную порцию элементов: %1 элементов'"), ОбъектовОбработано));
...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	Исключение
		Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
			СлужебныеДанныеФайла = ОбменСБанкамиСлужебный.ПолучитьHexСтрокуИзДвоичныхДанныхСРазделителями(Результат.Тело);
			...
			ТекстОшибки = НСтр("ru = 'Ошибка чтения ответа банка.
									...
									|HEX данные файла:
									|%3'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ПодробноеПредставлениеОшибки, ИмяФайлаРезультата, СлужебныеДанныеФайла);
